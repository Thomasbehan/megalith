...
Megalith is a developer tool. The MCP HTTP server runs locally inside the Unreal Editor and is intended for use by AI coding assistants on the same machine. It is **not** designed to be exposed to untrusted networks or browsers.
...
**Out of scope** (assumed compromised — Megalith is the least of your worries):
...
- Verify the `Megalith-SHA256:` value in the release notes against the SHA256 of the downloaded zip before extracting (auto-updater does this for you when enabled)
...
- The MCP server binds to all network interfaces (limitation of UE's `FHttpServerModule`). If your machine is on an untrusted LAN, either:
  ...
  - Set `bMcpServerEnabled = false` in Editor Preferences > Plugins > Megalith when not actively using AI tooling
