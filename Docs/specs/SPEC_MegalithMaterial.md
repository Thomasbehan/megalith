# Megalith — MegalithMaterial Module
...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, UnrealEd, MaterialEditor, EditorScriptingUtilities, RenderCore, RHI, Slate, SlateCore, Json, JsonUtilities

### Classes

| Class | Responsibility |
|-------|---------------|
| `FMegalithMaterialModule` | Registers 63 material actions |
| `FMegalithMaterialActions` | Static handlers + helpers for loading materials and serializing expressions |
...
| `render_preview` | Save preview PNG to Saved/Megalith/previews/ |
...
`MegalithMaterialBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `material` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 5 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`).
...
**fill_kind catalogue (2 — enumerated against `MegalithMaterialBulkFillAdapter.cpp`):**
