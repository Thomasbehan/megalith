# Megalith — MegalithNiagara Module
...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, UnrealEd, Niagara, NiagaraCore, NiagaraEditor, Json, JsonUtilities, AssetTools, Slate, SlateCore

### Classes

| Class | Responsibility |
|-------|---------------|
| `FMegalithNiagaraModule` | Registers 129 Niagara actions (108 baseline in `MegalithNiagaraActions.cpp` + 1 layout in `MegalithNiagaraLayoutActions.cpp` + 9 timing actions in `MegalithNiagaraTimingActions.cpp` + 1 stateless-emitter factory in `MegalithNiagaraActions.cpp` + 7 issue #64 Tranche 2 search/discovery + 2 PR #65 CustomHlsl-text read/write). The 2 CustomHlsl-text actions ride public UPROPERTY reflection and are always registered regardless of `WITH_NIAGARA_WIZARD_PRIVATE` |
| `FMegalithNiagaraActions` | Static handlers + extensive private helpers |
| `FMegalithNiagaraLayoutActions` | `auto_layout` Blueprint Assist bridge for Niagara graphs |
| `MegalithNiagaraHelpers` | 6 reimplemented NiagaraEditor functions (non-exported APIs) |
...
`MegalithNiagaraBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `niagara` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 5 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`).
...
**fill_kind catalogue (3 — enumerated against `MegalithNiagaraBulkFillAdapter.cpp`):**
...
The ParameterMap bridge graph generated by `create_module_from_hlsl` / `create_function_from_hlsl` links against an engine-private NiagaraEditor wizard API. That linkage is gated behind the `WITH_NIAGARA_WIZARD_PRIVATE` flag in `MegalithNiagara.Build.cs`:

- **Dev builds:** ON — the full ParameterMap bridge path is available.
- **Release builds (`MEGALITH_RELEASE_BUILD=1`):** forced OFF — the wizard linkage is compiled out and an internal fallback path is used so the actions still register and behave gracefully.
