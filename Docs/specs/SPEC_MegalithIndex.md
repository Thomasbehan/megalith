...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, UnrealEd, AssetRegistry, Json, JsonUtilities, SQLiteCore, Slate, SlateCore, BlueprintGraph, KismetCompiler, EditorSubsystem

### Classes

| Class | Responsibility |
|-------|---------------|
| `FMegalithIndexModule` | Registers 12 project actions (7 baseline + 1 v0.17.0 cross-module `audit_orphan_assets` + 3 test/profiling harness Wave 1 + 1 (2026-06-10) `export_asset_text`, Gap 11) |
| `FMegalithIndexDatabase` | RAII SQLite wrapper. 13 tables + 2 FTS5 + 6 triggers + 1 meta. DELETE journal mode, 64MB cache. Schema v2: `saved_hash` column (Blake3 `FIoHash` hex), `schema_version` meta key. Schema v3: `deep_indexed_hash` + `deep_index_attempts` columns on `assets` (full-index resume — see **Full-Index Resume**) |
| `UMegalithIndexSubsystem` | UEditorSubsystem. 3-layer indexing (startup delta, live AR callbacks, full fallback). Hash-based startup catch-up. Live batched AR delegates on 2s timer. Deep asset indexing with game-thread batching. Batches every 100 assets. Progress notifications |
| `IMegalithIndexer` | Pure virtual interface: GetSupportedClasses(), IndexAsset(), GetName(), IsSentinel(), SupportsIncrementalIndex(), IndexScoped() |
...
| `audit_orphan_assets` | `asset_class_filter` (optional), `limit` (50, cap 200), `cursor` (optional) | **v0.17.0 (cross-module from `MegalithReflectionIntel`).** List `/Game/.../*.uasset` assets with ZERO `IAssetRegistry` referencers AND zero entries in `cpp_asset_edges`. Strictest orphan signal for pre-release cleanup. Excludes `/Engine/*` + `/Memory/*`. Read-only, cursor-paginated |
...
| `cleanup_generated_assets` | `paths[]` (required), `dry_run` (default true), `require_no_referencers` (default true), `remove_empty_folders` (default false) | Safely delete generated throwaway assets with reference checks. **HARD allowlist guard:** refuses any path outside `/Game/Tests/Megalith/`. Dry-run by default (reports what would be deleted without touching disk); `require_no_referencers` skips any asset still referenced from outside the request set; `remove_empty_folders` prunes now-empty folders under the allowlist |
...
**DB Location:** `Plugins/Megalith/Saved/ProjectIndex.db`

### Search Error Contract

`FMegalithIndexDatabase::FullTextSearch` has a 4-argument overload returning
`EMegalithProjectSearchStatus` (`Succeeded` / `InvalidQuery` / `InternalError`); the
...
`MegalithProjectSearchDetail::IsFts5QuerySyntaxError` matches the five messages the
...
so it is correctly refused — and `MegalithProjectSearchDetail::DescribeUnknownColumn`
...
> grammar of our own. `Megalith.ProjectSearch.ColumnQualified` locks this.
...
`MegalithProjectSearchActionDetail::MaxQueryLength` (4096); `limit` must be an
...
The offline `megalith_query.exe` and `megalith_offline.py` mirror this
...
On editor startup, `UMegalithIndexSubsystem` runs a fast delta engine:
...
`megalith_reindex()` defaults to incremental mode (Layer 1 logic). Passing `force=true` triggers a full wipe-and-rebuild: drops all table data, re-enumerates, and re-indexes every asset. Used when the DB is suspected corrupt or after schema migrations.
...
|---|---|
| `StartFullIndex()` (`UFUNCTION`, and `megalith_reindex force=true`) | **Always wipes.** The explicit start-over request. |
| `ResumeFullIndex()` (startup auto-index, `OnAssetRegistryFilesLoaded`, `Megalith.StartIndex`) | Continues an interrupted run when one is recoverable; otherwise wipes and starts fresh. |

`Megalith.StartIndex` resumes; `Megalith.StartIndex force` wipes.
...
- `megalith_reindex()` with no arguments on an interrupted index still takes the full-reset path (`CanDoIncrementalIndex()` requires `last_full_index`, which an interrupted run does not have). Use `Megalith.StartIndex` to resume.

**IMegalithIndexer Interface Additions**
