# Megalith — MegalithBlueprint Module
...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, UnrealEd, BlueprintGraph, EnhancedInput, Json, JsonUtilities

### Classes

| Class | Responsibility |
|-------|---------------|
| `FMegalithBlueprintModule` | Registers ~120 blueprint actions (count approximate — query `megalith_discover("blueprint")` for the live figure; includes the 2026-06-10 `find_variable_references`, Gap 5) |
| `FMegalithBlueprintActions` | Static handlers. Uses `FMegalithAssetUtils::LoadAssetByPath<UBlueprint>` |
| `FMegalithBlueprintContractActions` | Variable-contract reconciliation: `compare_class_variable_contract` (diff engine) + `promote_variables_to_parent`. Pin-type-aware descriptor extraction shared by both. |
| `MegalithBlueprintInternal` | Helpers: AddGraphArray, FindGraphByName, PinTypeToString, SerializePin/Node, TraceExecFlow, FindEntryNode |

> **Unity-safe file-local helpers (#68).** Internal-linkage helpers (anonymous-namespace functions/types, file-`static`s) must carry file-unique names or live in per-file named namespaces — matching the MegalithUI model — so they don't collide when adaptive/full unity concatenates same-module `.cpp`s into one translation unit. (The previously-global `InterpModeToString` in `MegalithBlueprintNodeActions.cpp` is now `NodeInterpModeToString`.)
...
**Variable Contract Reconciliation (2) — `MegalithBlueprintContractActions.cpp`**
...
| `set_function_params` | `asset_path`, `function_name`, `params` | Set input/output parameters on a function. Every requested pin type is parsed strictly and **up front**, before any pin is created — a typo'd `type` errors with a reason instead of silently producing a `bool` pin, and no partial pin set is left behind. `set_event_dispatcher_params` behaves the same way (it replaces the signature wholesale, so it validates before stripping the old pins). |
...
**SwitchEnum / CallFunction resolution (v0.21.0, #74):** both `add_node` and `resolve_node` share these resolvers, so `resolve_node` previews exactly what `add_node` will create. No action-count change.

- **`K2Node_SwitchEnum` — user-defined enums.** The switch node now resolves user-defined `UENUM`s, not just native ones: by short name, by `/Script` path, and by loading an unloaded `UserDefinedEnum` asset on demand. Supply the enum via the new `enum` / `enum_path` aliases; `k2node_switchenum` is accepted as a node-class alias.
- **`K2Node_CallFunction` — Blueprint-defined functions.** CallFunction resolution now finds Blueprint-defined functions in addition to native/engine ones — both `self` (functions on the Blueprint being edited) and functions on an external Blueprint resolved via `target_class`.
...
> **`EnhancedInput` dep (2026-06-07)** added to `MegalithBlueprint.Build.cs` for `scaffold_locomotion_input` (Enhanced Input action / mapping-context resolution).

**CDO Bulk Fill / Describe (Phase 1 of bulk_fill framework — 2)**
| Action | Params | Description |
|--------|--------|-------------|
| `set_cdo_properties` | `asset_path`, `properties`, `dry_run`?, `strict`? | Bulk-fill multiple CDO properties from a JSON tree in a single transaction. `properties` is a nested JSON object whose keys are UPROPERTY names and values are scalars / structs / arrays / maps / sets per the FProperty reflection schema. Routes through the central `FMegalithBulkFillRegistry` "blueprint" adapter. Same dual-path asset load as `set_cdo_property` (Blueprint CDO first, then generic UObject — DataAsset, DataTable, GameplayEffect, AbilitySet, InputAction). Engine edit cradle (transaction → Modify → PreEditChange → write → ReparentTransientInstancedSubobjects → FireFullCradle) preserved per Issue #29. `dry_run=true` walks the tree via `FMegalithReflectionWalker::InspectTree` WITHOUT mutation and returns the full `FDryRunReport`. `strict=true` promotes silent drops / clamps / unknown-fields to hard errors and cancels the transaction. |
| `describe_cdo_schema` | `asset_path` | Return the rich `FSchemaDescriptor` tree (type names, ImportText forms, enum-value lists, clamp ranges, nested struct/array/map children) for an asset's CDO. Counterpart to `get_cdo_properties` when the caller needs the schema, not the current values. Use before authoring `set_cdo_property` / `set_cdo_properties` payloads to discover the legal ImportText grammar. v0.21.0 (#74): the `TMap` ImportText hint is now the correct **positional** form `((Key,Value))`, matching what the importer accepts (the prior hint implied a keyed form it rejects). |
| `set_property_at_path` | `asset_path`, `path`, `value`, `dry_run`?, `strict`?, `create_missing_keys`?, `save`? | **Surgical nested-path writer.** Sets ONE leaf value deep inside a CDO / UObject asset addressed by a dotted+bracket `path` — `.` descends into a struct member, `[N]` indexes an array, `[Key]` keys a map. Example: `Standing.Gaits[Jog].Starts.Forward`. Routes through `FMegalithReflectionWalker::ResolvePath` (path → leaf `FProperty*` + value pointer) then `WriteLeaf` (the SAME value coercion as `set_cdo_property`: scalars, enum names, ImportText struct literals, `FObjectProperty` hard refs, **`TSoftObjectPtr` asset paths**, sets). Unlike `set_cdo_properties`, which rebuilds whole arrays/maps from a JSON tree, this leaves sibling array elements / map keys **untouched** — true in-place edit (array index via `FScriptArrayHelper::GetRawPtr`; map value via `FScriptMapHelper::FindMapIndexWithKey` + `GetValuePtr`). **Map-key resolution:** the `[Key]` token is `ImportText_Direct`'d through the map's `KeyProp`, so enum names (`[Jog]`), integers, FName / string keys all resolve. **Enum leaf values** resolve by name via the shared walker. **EditDefaultsOnly bypass:** the write uses the same engine edit cradle as `set_cdo_property` (transaction → Modify → PreEditChange → write → ReparentTransientInstancedSubobjects → FireFullCradle → MarkPackageDirty). That cradle is FProperty-reflection-side, NOT the editor's `PropertyAccessUtil` edit-flag gate, so it writes `EditDefaultsOnly` DataAsset fields the editor refuses on saved instances. `create_missing_keys=true` adds an absent `[Key]` (default-initialised value via `AddPair`) before writing; default is a clean error on a missing key. `dry_run=true` resolves the path against the live object (key-creation forced off) and validates the leaf write against a scratch buffer WITHOUT mutating; returns the `FDryRunReport` plus `resolved_leaf_type`. `save=true` writes the package to disk (`UPackage::SavePackage`) after the edit; default is `MarkPackageDirty` only. Fully generic reflection — no type coupling. |
...
| Action | Params | Description |
|--------|--------|-------------|
| `seed_data_asset` | `save_path`, `class_name`, `tree` (nested JSON), `dry_run`?, `strict`?, `skip_save`?, `read_back_values`? | Create a DataAsset AND populate it from a nested `tree` in one atomic call — sugar over `create_data_asset` + `bulk_fill apply`. Create body reuses `HandleCreateDataAsset`; fill reuses `FMegalithReflectionWalker::WriteTree`. Returns `asset_path`, `actual_class`, `field_writes[]`, `errors`, `saved`. **`read_back_values` (default false, 2026-06-07):** after the write transaction completes, re-walk the written top-level props through `FMegalithReflectionReader` (the read-side serializer in `MegalithCore`) and attach `values: { field: <json> }` to the success payload — inline verify-after-write in the same call. The readback runs *after* the transaction and never re-opens it or dirties the package. (For populating an *existing* DataAsset, use `bulk_fill_query("apply")` / `set_cdo_properties` instead — this action is for the create-then-populate scaffold case.) |
...
> - **`get_cdo_properties`** — the canonical verify path for a *standalone or pre-existing* DataAsset you did NOT just write through `seed_data_asset` (independent live read of any DataAsset's values; routes through the same shared `FMegalithReflectionReader` serializer).
...
`MegalithBlueprint/Private/MegalithBlueprintComponentResolver.{h,cpp}` is **the** component resolver for the
...
- `ICH->Modify()` is called before `CreateOverridenComponentTemplate`. The engine's own call sites skip it
  because they run inside an editor transaction Megalith does not have.
...
*CurveTable (5) — first CurveTable surface in Megalith*
| Action | Params | Description |
|--------|--------|-------------|
...
`MegalithBlueprintBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `blueprint` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 1 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`, implementation plan `Docs/plans/2026-05-11-megalith-mcp-ergonomics.md`).
...
**fill_kind catalogue (1 — enumerated against `MegalithBlueprintBulkFillAdapter.cpp`):**
