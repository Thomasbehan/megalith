# Megalith — MegalithAnimation Module
...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, UnrealEd, AnimGraph, AnimGraphRuntime, BlueprintGraph, AnimationBlueprintLibrary, PoseSearch, BlendStackEditor, AnimationModifiers, EditorScriptingUtilities, Json, JsonUtilities
...
| Action | Description |
|--------|-------------|
| (3 actions in `MegalithControlRigWriteActions.cpp` — graph node creation, pin configuration, variable management) |
...
`MegalithAnimationBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `animation` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 5 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`).
...
**fill_kind catalogue (2 — enumerated against `MegalithAnimationBulkFillAdapter.cpp`):**
...
A dedicated namespace for inspecting and editing `UChooserTable` assets, registered from `MegalithAnimation`. **All actions are `#if WITH_CHOOSER` gated** — they register only when the Chooser plugin (`Engine/Plugins/Chooser`) is present. The namespace registers no actions in builds without it.
