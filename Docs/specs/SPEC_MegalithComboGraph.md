# Megalith — MegalithComboGraph Module
...
MegalithComboGraph provides MCP coverage of the ComboGraph marketplace plugin. It covers combo graph CRUD, node and edge management, gameplay effect and cue assignment per node, ability creation/linking, and full-graph scaffolding from montage lists.
...
`MegalithComboGraphBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `combograph` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 5 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`). Combograph's adapter was deliberately scheduled LAST in the per-namespace pilot order (Phase 5+) because of the `TargetType` non-UPROPERTY problem documented below.
...
**fill_kind catalogue (2 — enumerated against `MegalithComboGraphBulkFillAdapter.cpp`):**
...
> **Precompiled plugin integration.** ComboGraph is a marketplace plugin with precompiled binaries. MegalithComboGraph uses UObject reflection (`FindPropertyByName`, `FProperty::GetValue_InContainer`) and `UComboGraphFactory` (discovered via reflection) rather than linking against ComboGraph headers. This makes the integration version-agnostic as long as property names are stable.
>
