# Megalith — MegalithLogicDriver Module
...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore
...
MegalithLogicDriver provides MCP coverage of the Logic Driver Pro marketplace plugin. It covers state machine asset CRUD, graph read/write, node configuration, runtime/PIE control, JSON spec-based generation, scaffolding templates, discovery, component management, and text-based graph visualization.
...
`MegalithLogicDriverBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `logicdriver` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 5 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`).
...
**fill_kind catalogue (2 — enumerated against `MegalithLogicDriverBulkFillAdapter.cpp`):**
...
- **Reflection-only architecture preserved.** The adapter uses `FindPropertyByName` + `GetValue_InContainer` against state CDOs and transition CDOs, matching the rest of MegalithLogicDriver's reflection-only stance. No direct linkage against Logic Driver headers.
...
> **Precompiled plugin integration.** Logic Driver Pro is a marketplace plugin with precompiled binaries. MegalithLogicDriver uses UObject reflection (`FindPropertyByName`, `FProperty::GetValue_InContainer`) and factory classes discovered via reflection rather than linking against Logic Driver headers. The 3-location Build.cs detection finds SMSystem/SMSystemEditor modules whether installed as a project plugin, engine marketplace plugin, or engine plugin.
>
