...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, AudioMixer, AudioEditor, AssetTools, Json, JsonUtilities, Slate, SlateCore, UnrealEd
...
**Conditional:** MetaSound features wrapped in `#if WITH_METASOUND`. When MetaSound is absent, MetaSound Builder + introspection actions are inert but all other actions (Sound Cue, CRUD, batch, query, perception) function normally. Build.cs auto-detects MetaSound at `Engine/Plugins/Runtime/Metasound`. Companion deep indexer `FMetaSoundIndexer` lives in MegalithIndex (also v0.14.10 [Unreleased] from PR #18).
**Settings toggle:** `bEnableAudio` (default: True)

MegalithAudio provides MCP coverage of audio asset creation, inspection, batch management, Sound Cue graph building, MetaSound graph building, and AI Perception sound binding. It covers the 5 configurable audio asset types (SoundAttenuation, SoundClass, SoundMix, SoundConcurrency, SoundSubmix), read-only SoundWave inspection, Sound Cue node graph construction, MetaSound Builder API integration, and `UAssetUserData`-based perception stimulus authoring.

**No overlap with runtime audio playback plugins** — a runtime audio plugin owns footstep/surface/movement audio playback; MegalithAudio owns editor-time asset creation, management, and inspection.

### Action Categories

| Category | Actions | Source file | Description |
|----------|---------|-------------|-------------|
| Asset CRUD | 16 | `MegalithAudioAssetActions.cpp` | Create/get/set triads for SoundAttenuation, SoundClass, SoundMix, SoundConcurrency, SoundSubmix + `create_test_wave` (Phase J F18) |
| Query & Search | 10 | `MegalithAudioQueryActions.cpp` | List/search audio assets, hierarchy inspection, reference queries, stats, audio health checks (missing class, no attenuation, unused) |
| Batch Operations | 10 | `MegalithAudioBatchActions.cpp` | Batch assign sound class/attenuation/submix/concurrency/compression/looping/virtualization, batch rename, batch set properties, apply audio template |
| Sound Cue Graph | 21 | `MegalithAudioSoundCueActions.cpp` | Sound Cue CRUD, node add/remove/connect, graph read, node property editing, `build_sound_cue_from_spec` (power action), 5 template cues (random, layered, looping, crossfade, switch), validate, preview, delete |
| MetaSound Graph (Builder API, write-side) | 25 | `MegalithAudioMetaSoundActions.cpp` | MetaSound Source/Patch creation, node add/remove/connect/disconnect, graph inputs/outputs, interface management, graph read (live builder), node discovery, `build_metasound_from_spec` (power action), 4 template MetaSounds (oneshot, ambient, synth, interactive), preset, variables, layout |
| MetaSound Document Introspection (read-side, v0.14.10 [Unreleased]) | 12 | `MegalithAudioMetaSoundIntrospectionActions.cpp` | Read-only walk of `IMetaSoundDocumentInterface::GetConstDocument()` → `FMetasoundFrontendDocument`. Project-wide enumeration (`list_metasounds`), per-asset page list (`list_metasound_documents`), full-document walk (`get_metasound_document`), summary counts (`get_metasound_summary`), per-node instance inspection (`inspect_metasound_node_instance`), edge enumeration (`get_metasound_document_connections`), variables (`get_metasound_document_variables`), user-facing inputs/outputs (`get_metasound_user_parameters`), node search (`search_metasound_document_nodes`), asset metadata (`get_metasound_info`), external class deps (`get_metasound_dependencies`), validator (`validate_metasound`). PR #18 by @alakangas, refactored into `audio_query` namespace. Distinct from the Builder API actions above — those read live builder state, these read on-disk doc state. |
| AI Perception Binding | 4 | `MegalithAudioPerceptionActions.cpp` | `bind_sound_to_perception`, `unbind_sound_from_perception`, `get_sound_perception_binding`, `list_perception_bound_sounds`. Authored via `UMegalithSoundPerceptionUserData` + runtime subsystem. **F11** strict input validation (loudness/max_range floors, tag length cap, sense_class allowlist) |
...
Companion deep indexer: `FMetaSoundIndexer` lives in `MegalithIndex/Private/Indexers/MetaSoundIndexer.cpp`, also from PR #18 by @alakangas. Sentinel-class registration mirroring `FNiagaraIndexer`. Walks all project MetaSounds at reindex time and records nodes / edges / variables / dependencies into `ProjectIndex.db` for cross-asset query via `project_query`. Throttled via `FMegalithMemoryHelper`. Toggle: `bIndexMetaSounds` (default true) under Editor Preferences → Plugins → Megalith → Indexing → Deep Indexers.
...
`MegalithAudioBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `audio` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 5 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`).
...
**fill_kind catalogue (3 — enumerated against `MegalithAudioBulkFillAdapter.cpp`):**
...
>
> **SoundWave is read-only.** MegalithAudio does not create SoundWaves (they are imported assets). `get_sound_wave_info` reads properties; `batch_set_sound_wave_properties` can modify UPROPERTY fields via reflection.
>
> **Future phases (not yet implemented).** Phase 3-6 planned (~69 additional actions): Audio Scene & Environment (~18), Audio Modulation & Quartz (~18), Analysis & Automation (~20), Middleware Bridges (~13). See `Docs/specs/2026-04-08-megalith-audio-phase3-6-design.md`.
