# Megalith — MegalithUI Module

**Parent:** [SPEC_CORE.md](../SPEC_CORE.md)
**Engine:** Unreal Engine 5.7+
**Version:** 0.22.0 (Beta) — architecture expansion Phase A–L landed 2026-04-26 (plan: [`Docs/plans/2026-04-25-megalith-ui-architecture-expansion.md`](../../../../Docs/plans/2026-04-25-megalith-ui-architecture-expansion.md)); optional EffectSurface provider decouple (Wave 1/2 + Final.1) landed 2026-04-27.

---

## MegalithUI

**Dependencies:** Core, CoreUObject, Engine, MegalithCore, UnrealEd, UMGEditor, UMG, Slate, SlateCore, Json, JsonUtilities, KismetCompiler, MovieScene, MovieSceneTracks, DeveloperSettings, AssetTools, ImageWrapper, ImageCore, Kismet, MaterialEditor, EditorSubsystem (Public — `UMegalithUIRegistrySubsystem` is exported), CommonUI (optional — `#if WITH_COMMONUI`)

**The optional EffectSurface provider is NOT a build-system dependency** (decoupled 2026-04-27). EffectSurface support is delivered via UClass-by-name reflection through `MegalithUI::GetEffectSurfaceClass()` — see § "Optional Dep Probe API" and § "Error Contract — Optional EffectSurface Provider Absence (-32010)". External providers may depend on MegalithUI for registry/spec structs, but MegalithUI must not depend on them.
**Total actions in `ui::` namespace:** **130** when `WITH_COMMONUI=1` (72 always-on owned by this module + 57 CommonUI owned by this module conditional on `WITH_COMMONUI` + 1 inline diagnostic `dump_style_cache_stats` registered from `MegalithUIModule.cpp` under the same gate) + **4 GAS UI binding aliases owned by `MegalithGAS`** (also registered into `ui::`, conditional on `WITH_GBA`). Without `WITH_COMMONUI`, the namespace registers **72** actions; without `WITH_GBA` the four bridge aliases are absent. Phase 3 of the 2026-05-16 UI Gap Audit (2026-05-16) landed 4 actions: 3 CommonUI scaffolders (`scaffold_main_menu`, `scaffold_settings_panel_with_tabs`, `scaffold_pause_menu`) + 1 always-on multi-screen menu builder (`build_menu_from_spec`).
...
|----------|-------|----------------|--------------|
| Widget CRUD | 9 | `MegalithUIActions.cpp` | always — Phase 2 (2026-05-16) added `rename_widget`, `dump_blueprint_compile_log` |
| Slot | 3 | `MegalithUISlotActions.cpp` | always |
| Templates | 8 | `MegalithUITemplateActions.cpp` | always |
| Styling | 6 | `MegalithUIStylingActions.cpp` | always |
| Animation v1 (deprecated) | 5 | `MegalithUIAnimationActions.cpp` | always — `create_animation` + `add_animation_keyframe` tagged `[DEPRECATED]` Phase L (2026-04-26) |
...
| Bindings | 4 | `MegalithUIBindingActions.cpp` | always |
| Settings scaffolds | 5 | `MegalithUISettingsActions.cpp` | always |
| Accessibility (non-CommonUI) | 4 | `MegalithUIAccessibilityActions.cpp` | always |
...
| Effect Surface Actions | 10 | `Actions/MegalithUIEffectActions.cpp` | always |
| Spec Builder + Serializer | 4 | `Actions/MegalithUISpecActions.cpp` | always — `build_ui_from_spec`, `dump_ui_spec_schema`, `dump_ui_spec`, plus Phase 3 (2026-05-16) `build_menu_from_spec` |
| Type Registry diagnostic | 4 | `MegalithUIRegistryActions.cpp` | always — `dump_property_allowlist`, plus Phase 2 (2026-05-16) `add_widget_variable`, `list_widget_property_enums`, plus Phase 4 (2026-05-23) `set_widget_is_variable` |
| **Always-on subtotal** | **72** | | |
| CommonUI Activatables | 8 | `CommonUI/MegalithCommonUIActivatableActions.cpp` | `WITH_COMMONUI` |
| CommonUI Buttons + Styling | 14 | `CommonUI/MegalithCommonUIButtonActions.cpp` | `WITH_COMMONUI` — Phase 2 (2026-05-16) added `apply_token_binding`, `convert_textblock_to_common`, `set_action_bar_button_class`; Phase 3 (2026-05-23) added `convert_border_to_common`, `reparent_widget_root` |
| CommonUI Input | 7 | `CommonUI/MegalithCommonUIInputActions.cpp` | `WITH_COMMONUI` |
| CommonUI Navigation/Focus | 8 | `CommonUI/MegalithCommonUINavigationActions.cpp` | `WITH_COMMONUI` — Phase 2 (2026-05-16) added `audit_focus_chain`; Phase 3 (2026-05-23) added `set_widget_navigation_bulk`, `dump_widget_navigation` |
| CommonUI Lists/Tabs/Groups | 7 | `CommonUI/MegalithCommonUIListActions.cpp` | `WITH_COMMONUI` |
| CommonUI Content widgets | 4 | `CommonUI/MegalithCommonUIContentActions.cpp` | `WITH_COMMONUI` |
| CommonUI Dialogs | 2 | `CommonUI/MegalithCommonUIDialogActions.cpp` | `WITH_COMMONUI` |
| CommonUI Audit + Lint | 4 | `CommonUI/MegalithCommonUIAuditActions.cpp` | `WITH_COMMONUI` |
| CommonUI Accessibility | 4 | `CommonUI/MegalithCommonUIAccessibilityActions.cpp` | `WITH_COMMONUI` |
| CommonUI Scaffolders | 3 | `CommonUI/MegalithCommonUITemplateActions.cpp` | `WITH_COMMONUI` — Phase 3 (2026-05-16) `scaffold_main_menu`, `scaffold_settings_panel_with_tabs`, `scaffold_pause_menu` |
| Style Service Diagnostics | 1 | inline lambda in `MegalithUIModule.cpp` | `WITH_COMMONUI` — `dump_style_cache_stats` |
| **CommonUI subtotal** | **62** | | conditional |
| **MegalithUI total** | **134** | | full configuration |
| GAS UI binding aliases | 4 | `MegalithGAS/Private/MegalithGASUIBindingActions.cpp` | `WITH_GBA` — registered cross-namespace into `ui::` |
...
|-------|---------------|
| `FMegalithUIModule` | Registers the 66 always-on actions owned by this module + 51 CommonUI actions when `WITH_COMMONUI`. Logs the live `ui` namespace action count at startup. Owns the `OnPostEngineInit` re-scan delegate that catches late-loading marketplace widget UClasses |
| `FMegalithUIActions` | Widget blueprint CRUD: create, inspect, add/remove widgets, property writes, compile |
| `FMegalithUISlotActions` | Layout slot operations: slot properties, anchor presets, widget movement |
| `FMegalithUITemplateActions` | High-level HUD/menu/panel scaffold templates (8 templates) |
| `FMegalithUIStylingActions` | Visual styling: brush, font, color scheme, text, image, batch style |
| `FMegalithUIAnimationActions` | UMG widget animation v1 CRUD: list, inspect, create, add/remove keyframes (the latter two are Phase L `[DEPRECATED]` — prefer the hoisted v2 surface in `AnimationCoreActions`) |
| `FMegalithUIBindingActions` | Event/property binding inspection, list view setup, widget binding queries |
| `FMegalithUISettingsActions` | Settings/save/audio/input remapping subsystem scaffolding (5 scaffolds) |
| `FMegalithUIAccessibilityActions` | Accessibility subsystem scaffold, audit, colorblind mode, text scale |
| `FMegalithUIRegistryActions` | Registers `dump_property_allowlist` (Phase B diagnostic) |
| `MegalithUI::FTextureIngestActions` / `FFontIngestActions` / `FAnimationCoreActions` / `FAnimationEventActions` / `FRoundedCornerActions` / `FShadowActions` / `FGradientActions` | Hoisted Design Import + Animation v2 verbs (Phase D, 2026-04-26) |
| `MegalithUI::FEffectSurfaceActions` | EffectSurface sub-bag setters + preset (Phase F, 2026-04-26) |
| `MegalithUI::FSpecActions` | `build_ui_from_spec` + `dump_ui_spec_schema` + `dump_ui_spec` (Phases H + J, 2026-04-26) + `build_menu_from_spec` (Phase 3 of the 2026-05-16 UI Gap Audit, MVP-STUB) |
| `MegalithCommonUITemplate::Register` | CommonUI headline scaffolders: `scaffold_main_menu` / `scaffold_settings_panel_with_tabs` / `scaffold_pause_menu` (Phase 3 of the 2026-05-16 UI Gap Audit). File-static handlers in `CommonUI/MegalithCommonUITemplateActions.cpp` — `WITH_COMMONUI` only |
| `UMegalithUIRegistrySubsystem` (UEditorSubsystem) | Live type registry + per-type property allowlist (Phase B) |
...
| `FMegalithUIStyleService` | CommonUI style asset dedup + canonical-library lookup (Phase G — `WITH_COMMONUI` only) |
| `UMegalithUISettings` (UDeveloperSettings) | Project Settings → Plugins → Megalith UI: `GeneratedStylesPath`, `CanonicalLibraryPath`, `StyleCacheCap`, `MaxNestingDepth`, `PathCacheCap` |
...
| Action | Params | Description |
|--------|--------|-------------|
| `set_slot_property` | `asset_path`, `widget_name`, `property_name`, `value` | Set a layout slot property (padding, alignment, size, etc.). As of v0.21.0 (#74) grid slots are supported — `row`, `column`, `row_span`, `column_span` on `UUniformGridSlot` and `UGridSlot`. |
...
- **Release escape hatch:** `MEGALITH_RELEASE_BUILD=1` forces `WITH_COMMONUI=0`, stripping CommonUI dependency from the released DLL
...
`convert_button_to_common` and other button-targeting actions auto-create a persistent Blueprint at `/Game/Megalith/CommonUI/MegalithDefaultCommonButton` (session-cached, saveable). This avoids requiring callers to pre-create a button subclass for simple operations.
...
| `create_bound_action_bar` | `wbp_path`, `widget_name`, `parent_widget?`, `action_button_class?` | Add a `UCommonBoundActionBar` to an existing WBP. Writes `ActionButtonClass` (default: `/Game/Megalith/CommonUI/MegalithDefaultCommonButton.MegalithDefaultCommonButton_C`) so the WBP compiles cleanly — bare bars previously failed `ValidateCompiledDefaults` (Bug #4 fix, 2026-05-16). |
...
Source: `CommonUI/MegalithCommonUITemplateActions.cpp`. Registered via `MegalithCommonUITemplate::Register` from `MegalithCommonUIActionsAggregator.cpp`.
...
## GAS Bridge Aliases (4 — namespace: "ui", source: `MegalithGAS`)

The MegalithGAS module also registers four cross-namespace aliases under `ui::` so that GAS-aware UI authoring tools see them in `ui_query` results without learning a separate namespace. The aliases dispatch to the same handlers as their `gas::` originals — there is no behavioural difference.

| Action (alias) | Canonical | Description |
|---|---|---|
| `ui::bind_widget_to_attribute` | `gas::bind_widget_to_attribute` | Bind a widget's display property to a GAS attribute via `UMegalithGASAttributeBindingClassExtension`. See [SPEC_MegalithGAS.md](SPEC_MegalithGAS.md) for full param schema |
...
**Registration site:** `Plugins/Megalith/Source/MegalithGAS/Private/MegalithGASUIBindingActions.cpp:571-577`. Both registrations call the same handler functor — `unbind_widget_attribute` is registered as the no-`s` form in both namespaces.

> **Why this is here, not in MegalithUI source:** the binding logic depends on GAS types (`FGameplayAttribute`, `UAttributeSet`) which would force MegalithUI to take a hard `WITH_GBA` dep. Aliasing keeps MegalithUI buildable in non-GAS projects while still letting `ui_query` callers find the action.
...
- **Bug #4 — `create_bound_action_bar` BS_Error.** Action now writes `ActionButtonClass` on the constructed `UCommonBoundActionBar` so the WBP compiles cleanly. New optional `action_button_class` param (string, defaults to `/Game/Megalith/CommonUI/MegalithDefaultCommonButton.MegalithDefaultCommonButton_C`).
...
**Purpose:** `set_widget_property`'s default (non-`raw_mode`) path gates writes through a per-type curated allowlist. This section documents which CommonUI properties have curated mappings (work without `raw_mode=true`) versus which require the legacy `raw_mode=true` bypass. Seed source: `Source/MegalithUI/Private/Registry/MegalithUIRegistrySubsystem.cpp:393-403` (CommonButtonBase curated block, expanded by Phase 1 Bug #2).
...
The token comes from `MakeTokenFromClassName(UCommonButtonBase::StaticClass())` which strips the leading `U` and returns `CommonButtonBase`. Subclasses inherit via the per-class walk in `FMegalithUIRegistrySubsystem::ResolveCuratedPropertyMapping`.
...
In `MegalithUIRegistrySubsystem.cpp::SeedCuratedAllowlist()` (the function that calls `AddMappingTo` at the lines cited above), append:
...
**Status:** Phases A–L all landed 2026-04-26 (plan: [`Docs/plans/2026-04-25-megalith-ui-architecture-expansion.md`](../../../../Docs/plans/2026-04-25-megalith-ui-architecture-expansion.md)). Phase L (this pass) marked v1 animation actions deprecated, refreshed action counts and roll-ups across all SPEC docs, and preserved one-major-release compatibility for legacy sibling UI aliases.

The Spec System promotes MegalithUI from a flat action toolbox into a schema-driven UI generator. A single MCP action `ui::build_ui_from_spec` accepts a typed JSON document (`FUISpecDocument`) and produces a fully-built `UWidgetBlueprint` transactionally — validate-then-mutate semantics guarantee the asset is either fully replaced or untouched.
...
- `MaxNestingDepth = 32` (Project Settings -> Plugins -> Megalith UI). Spec trees deeper than this are rejected before any mutation.
...
**Action surface:** `ui::build_menu_from_spec` (always-on — registered from `Actions/MegalithUISpecActions.cpp`, not WITH_COMMONUI-gated; per-screen builders may construct CommonUI widgets but the dispatch surface itself is engine-side).
...
2. Capture document-level metadata (`Name`, `ParentClass`, `Version`, `Metadata.AuthoringTool = "MegalithUI.SpecSerializer"`, `Metadata.SourceFile = AssetPath`).
...
- `Source/MegalithUI/Public/Spec/UISpecSerializer.h` -- `FUISpecSerializer::Dump` entry + `FUISpecSerializerInputs` / `FUISpecSerializerResult` structs.
- `Source/MegalithUI/Private/Spec/UISpecSerializer.cpp` -- per-slot/-content/-style/-effect/-commonui readers + recursive widget walker.
- `Source/MegalithUI/Private/Actions/MegalithUISpecActions.cpp` -- `ui::dump_ui_spec` action handler + the inverse `NodeToJson` / `SlotToJson` / `EffectToJson` / `CommonUIToJson` packers.
- `Source/MegalithUI/Public/Registry/UIReflectionHelper.h` (+ `.cpp`) -- new `ReadJsonPath` method (symmetric counterpart to `ApplyJsonPath`); read-side `ReadValueFromProperty` dispatch table mirrors the existing write-side `WriteValueToProperty`.
- `Source/MegalithUI/Private/Tests/UISpecRoundtripTests.cpp` -- automation tests J1 (`MegalithUI.SpecSerializer.RoundtripIdentity`), supported slot/content/style coverage (`MegalithUI.SpecSerializer.SupportedFields`), curated anchor coverage (`MegalithUI.SpecSerializer.PublicFields`), J4 (`MegalithUI.SpecSerializer.RoundtripCorpus` -- 5 representative WBP shapes), and a per-slot-type coverage test (`MegalithUI.SpecSerializer.SlotCoverage`).
...
**Action handler error shape (Phase K audit).** The 10 most-common-action error paths now emit `FMegalithActionResult::Error` with the message body equal to `FUISpecError::ToLLMReport()`. Consumers can dispatch on `R.bSuccess == false` and parse the message body using the same key:value labels they already use for `build_ui_from_spec` validation blocks. Audited actions:
...
| `ui::set_anchor_preset` | All 16 anchor-preset tokens (canonical list at `MegalithUIInternal::GetAnchorPresetNames`) |
...
| Path | Contents |
|------|----------|
| `Source/MegalithUI/Public/Spec/` | Document USTRUCTs (`UISpec.h`), validator interface (`UISpecValidator.h`) |
| `Source/MegalithUI/Private/Spec/` | Validator implementation, JSON parser (M5), builder pipeline (M5) |
| `Source/MegalithUI/Public/Registry/` | Type registry, property allowlist, reflection cache (M6) |
| `Source/MegalithUI/Private/Registry/` | Registry/cache implementations (M6) |
| `Source/MegalithUI/Public/Style/` | CommonUI style service public API (M7) |
| `Source/MegalithUI/Private/Style/` | Style service implementation (M7) |

### Phase A foundations (landed)

- `MegalithUICommon.h` / `.cpp` — exported `MEGALITHUI_API` helpers (`ParseColor`, `TryParseColor`, `LoadWidgetBlueprint`, `WidgetClassFromName`, anchor presets, alignment parsers, variable-name registration). The legacy `MegalithUIInternal::*` helpers now forward into these exports.
- `LogMegalithUISpec` log category — filterable separately from `LogMegalith` so spec-builder iteration noise is isolated.
...
Downstream UI bridge modules can declare `MegalithUI` as a private dependency and consume `MegalithUI::TryParseColor` instead of carrying local color parsers. The shared helper preserves the no-degamma byte-to-float path required by UI gradient/shadow materials (see `MegalithUICommon.h` for the `ParseColor` vs `TryParseColor` distinction). MegalithUI remains the provider of generic helpers; downstream bridge modules are not public Megalith dependencies.
...
1. Probe `MegalithUI::IsEffectSurfaceAvailable()` (R3b) — when false, return `-32010 ErrOptionalDepUnavailable` with a structured payload (see § "Error Contract" below). Single point of truth at the resolver entry, never per-handler.
2. Resolve the widget through `MegalithUI::GetEffectSurfaceClass()` + `IsA(EffectSurfaceClass)` — no `Cast<UEffectSurface>`.
...
5. Feature-flag bits use raw `int32` constants (`EffectFeature_RoundedCorners = 1<<0`, ...) declared at the top of `MegalithUIEffectActions.cpp`; the provider's source-of-truth header is cross-referenced in a comment but never `#include`d.
...
- The public Megalith release zip ships standalone — no optional-provider symbol references in `MegalithUI.dll` import table. Verified via `make_release.ps1` `$LeakSentinels` dumpbin /imports check.

**Drift-detection cadence:** the raw flag constants in `MegalithUIEffectActions.cpp` and the provider's canonical enum MUST stay aligned. If the provider renumbers its feature enum, the constants here become stale silently. Manual diff on every preset-touching PR; tracked as a documentation/audit concern.
...
Live total against the registration sites in `Plugins/Megalith/Source/MegalithUI/Private/`: **117** when `WITH_COMMONUI=1`, **66** without. The **42** baseline that prior revisions of this document quoted is the pre-expansion figure and is no longer accurate — see the action category roll-up at the top of this file for the current category breakdown.

---

## Type Registry (M5)

**Status:** Phase B landed; subsequent Phase C (`FUIPropertyPathCache` + safe `FUIReflectionHelper`), Phase E (EffectSurface curated mappings), and Phase L (count refresh) all landed 2026-04-26. See [`Docs/plans/2026-04-25-megalith-ui-architecture-expansion.md`](../../../../Docs/plans/2026-04-25-megalith-ui-architecture-expansion.md).
...
| Class | Responsibility |
|-------|---------------|
| `UMegalithUIRegistrySubsystem` | UEditorSubsystem owning the live registry + allowlist. Performs the initial reflection walk in `Initialize`, re-scans on `OnPostEngineInit` (registered from `FMegalithUIModule::StartupModule` because editor subsystems initialise after that delegate fires), and refreshes weak-pointer-stale entries on `FCoreUObjectDelegates::ReloadCompleteDelegate` for hot-reload survival. |
...
| `FMegalithUIRegistryActions` | Registers the `ui::dump_property_allowlist` diagnostic action. |
...
`FMegalithUIModule::StartupModule` registers an `OnPostEngineInit` lambda that calls `UMegalithUIRegistrySubsystem::RescanWidgetTypes()` once the engine has finished plugin initialisation. This catches marketplace widget packs (CommonUI, third-party UMG plugins) whose UClasses load after stock UMG. The lambda is owned at module scope so `ShutdownModule` can cleanly unregister it.

---

## Effect Surface (M5 — Phase E + Optional Provider Decouple)

**Status:** Phase E widget shipped 2026-04-26; **optional provider decouple Wave 1/2 + Final.1 shipped 2026-04-27**. The provider-owned material asset is still deferred (tracked in `Plugins/Megalith/Docs/ROADMAP.md`).

`UEffectSurface` is the SDF-driven UMG widget that supersedes `URoundedBorder`. One Slate `FSlateDrawElement::MakeBox` per paint pass, all visual maths in the material domain (User Interface), MID-driven by a typed config struct. The widget lives in an optional provider plugin outside the public Megalith release. MegalithUI accesses it via reflective `UClass*` lookup only — there is no compile-time `#include` of any provider header from MegalithUI source. The curated mappings, allowlist coverage, and Phase F MCP action wrappers all live in MegalithUI; they reach `UEffectSurface` through `MegalithUI::GetEffectSurfaceClass()` + `FUIReflectionHelper::ApplyJsonPath` against the curated paths.
...
| Provider-owned content asset | **DEFERRED** — optional-provider material path | SDF material that consumes the MID parameters and renders the surface. Authored via Megalith `material_query` at edit time. Until landed, `BaseMaterial.LoadSynchronous()` returns nullptr and the SDF surface paints invisibly (widget still compiles + runs). |
...
Allowlist tokens registered for `EffectSurface` in `UMegalithUIRegistrySubsystem::RegisterCuratedMappings`:
...
The provider-owned SDF material turns the MID parameter set into pixels. Its authoring requires the editor (Megalith `material_query("build_material_graph", ...)` only works when the editor is running), and Phase E was implemented offline. Therefore: this phase ships the C++ widget code with `TSoftObjectPtr<UMaterialInterface> BaseMaterial` defaulted to a provider-supplied asset path, but **does NOT** include the material asset itself. The widget compiles, links, and runs; the SDF surface paints invisibly until the asset lands. Material authoring is tracked as a follow-up task in `Plugins/Megalith/Docs/ROADMAP.md`.

### Tests

EffectSurface typed tests live with the optional provider module. Typed `UEffectSurface*` access is legal inside the module that defines the class, so assertion-side test code stays compact. MegalithUI exposes only the registry/spec/reflection APIs needed by provider-side tests; the reverse compile-time dependency is gone.

| Test | Location | Verifies |
|------|----------|----------|
| `MegalithUI.EffectSurface.SetCornerRadiiDirtiesMID` | Optional provider tests | E3 acceptance: `SetCornerRadii` flips `bMIDDirty`; next push writes matching `CornerRadii` vector parameter onto the MID. |
| `MegalithUI.EffectSurface.RegistryKnowsEffectSurface` | Optional provider tests | Reflection walk picks up `UEffectSurface` as a Content widget (single child). |
| `MegalithUI.EffectSurface.AllowlistCoversCuratedPaths` | Optional provider tests | Curated mapping coverage — every sub-bag has at least one allowlisted JSON path. |
| `MegalithUI.EffectSurfaceActions.*` (4 action-layer tests) | Optional provider tests | Action-handler smoke against a fixture WBP, with reflective post-state assertions through `FProperty::GetValue_InContainer` walks (not typed `Config.*` reads). All handlers route through `MegalithUI::IsEffectSurfaceAvailable()` first per § "Error Contract" — every test starts with the canonical skip-with-warning preamble. |
| `MegalithUI.AnimationRuntime.FadeInPlays` (was I4) | Optional provider tests | Runtime driver Tick + sample-at-time + stop-after-end. Moved out of `UIAnimationBuilderTests.cpp` (R5) so the MegalithUI-side builder test file no longer types any provider runtime symbol. Display name preserved for Session Frontend filter ergonomics. |

The Session Frontend display names are unchanged across the move; existing CI filters pinned to `MegalithUI.EffectSurface.*` / `MegalithUI.EffectSurfaceActions.*` / `MegalithUI.AnimationRuntime.FadeInPlays` keep working — the test display string is the public filter key, not the file path. The skip-with-warning pattern (per § "Error Contract — Test Surface (5.5.5)") means these tests **PASS-via-skip with `AddWarning`** when the optional provider is absent, never `return false`.
...
Declared in `Plugins/Megalith/Source/MegalithUI/Public/MegalithUICommon.h` inside `namespace MegalithUI`; implemented in `Plugins/Megalith/Source/MegalithUI/Private/MegalithUICommon.cpp`:

| Function | Returns | Behaviour |
|----------|---------|-----------|
| `MEGALITHUI_API UClass* GetEffectSurfaceClass()` | `UClass*` (or nullptr) | First call scans currently loaded widget classes for an `EffectSurface` provider class; subsequent calls hit a function-static `TWeakObjectPtr<UClass>` cache. Reset on `FCoreUObjectDelegates::ReloadCompleteDelegate` so Live Coding patches against the provider re-resolve cleanly. |
| `MEGALITHUI_API bool IsEffectSurfaceAvailable()` | `bool` | Convenience: `GetEffectSurfaceClass() != nullptr`. Shares the same function-static cache. |

### Boundary preservation

ZERO optional-provider headers in `MegalithUICommon.h` / `.cpp`. `UClass*` is a UE-core type; provider discovery is by loaded UClass name only. The hot-reload bind happens during `FMegalithUIModule::StartupModule` so the cache always reflects the current load state.

### Consumer surfaces

- **Action handlers** (`MegalithUIEffectActions.cpp`) — `ResolveEffectSurface` calls `IsEffectSurfaceAvailable()` first; on false, returns the canonical structured error (see § "Error Contract" below) before parsing `asset_path` / `widget_name`.
...
The names `MegalithUI::GetEffectSurfaceClass` and `MegalithUI::IsEffectSurfaceAvailable` are unique across the entire MegalithUI source tree (verified via Grep during R3b.4 implementation). The `MegalithUI::` namespace exports `ParseColor`, `TryParseColor`, `LoadWidgetBlueprint`, `WidgetClassFromName`, `GetAnchorPreset`, `ParseHAlign`, `ParseVAlign`, `MakeTokenFromClassName`, `RegisterVariableName`, `RegisterCreatedWidget`, `ReconcileWidgetVariableGuids`, plus the new probe pair — no `Get*EffectSurface*`, `Is*EffectSurface*`, or `*Available*` collision.

---

## Error Contract — Optional EffectSurface Provider Absence (-32010)

**Status:** Shipped 2026-04-27 as §5.5 of the decouple plan. **Load-bearing** runtime contract for what happens when `MegalithUI::IsEffectSurfaceAvailable() == false` (the optional provider is physically absent, OR present but its module has not loaded yet, OR loaded but its `UEffectSurface` UClass cannot be resolved). This is the public-release reality.
...
|----------|-------|--------|
| `FMegalithJsonUtils::ErrOptionalDepUnavailable` | `-32010` | `Plugins/Megalith/Source/MegalithCore/Public/MegalithJsonUtils.h:53` |

The reserved range `-32011..-32019` is left open for future sibling-plugin codes (e.g. `ErrFeatureGated` if "missing plugin" later needs to split from "feature flag off"). Existing "X not available" sites in MegalithAI / MegalithAudio / MegalithGAS still default to `-32603` via `Error(const FString&)`; their migration to `-32010` is OUT OF SCOPE for the decouple — the rule is "new code uses `-32010`; existing code stays `-32603` until a separate hygiene pass".
...
Behaviour: `ResolveEffectSurface` probes FIRST (before parsing `asset_path` / `widget_name` — fail-fast). On unavailable, returns the structured error built by `MegalithUIInternal::MakeOptionalDepUnavailableError` (single point of truth — never inlined per call site).
...
Implementation reads via `HasAnyEffectConfigured` + `GetEffectSurfaceSpecPayload` `ProcessEvent` calls (R3b accessors on the provider side), then deserialises the returned JSON document into `FUISpecEffect` via `ReadJsonPath` + manual `FProperty` descent. Source: `Plugins/Megalith/Source/MegalithUI/Private/Spec/UISpecSerializer.cpp:540` onwards.
...
Canonical pattern (verbatim — match `Plugins/Megalith/Source/MegalithUI/Private/Tests/Hoisted/SetRoundedCornersTests.cpp:98-105`):

```cpp
bool FFooTest::RunTest(const FString& Parameters)
{
    if (!MegalithUI::IsEffectSurfaceAvailable())
    {
...
| Probe | `MegalithUI::IsEffectSurfaceAvailable()` | Returns `false` | n/a | Direct caller-visible — the truth-source |

### Pattern reusability for future siblings

The probe / error-code / handler / serializer / test / builder sextuple is the canonical pattern for any future MegalithUI optional dep. To onboard a new sibling-plugin widget type:

1. Add a `MegalithUI::GetXClass()` + `MegalithUI::IsXAvailable()` pair to `MegalithUICommon.h/.cpp` — same function-static `TWeakObjectPtr<UClass>` cache + `ReloadCompleteDelegate` reset.
2. Reuse `FMegalithJsonUtils::ErrOptionalDepUnavailable` (`-32010`) — only allocate a new code when the contract semantics change.
...
|----------|-------|----------|
| (inline `-32011`) | `-32011` | `Plugins/Megalith/Source/MegalithUI/Private/CommonUI/MegalithCommonUIButtonActions.cpp` (`HandleApplyTokenBinding`) |

The code is inline (not yet promoted to a `FMegalithJsonUtils::Err*` constant) because Tokenforge is the only consumer for now. If a second `apply_*_binding` style action joins later, promote `-32011` to `FMegalithJsonUtils::ErrTokenforgeRuntimeUnavailable` so both handlers reference the same identifier.
...
| Path | Contents |
|------|----------|
| `Source/MegalithUI/Private/Actions/Hoisted/TextureIngestActions.{h,cpp}` | `MegalithUI::FTextureIngestActions` |
| `Source/MegalithUI/Private/Actions/Hoisted/FontIngestActions.{h,cpp}` | `MegalithUI::FFontIngestActions` |
| `Source/MegalithUI/Private/Actions/Hoisted/AnimationCoreActions.{h,cpp}` | `MegalithUI::FAnimationCoreActions` (3 actions) |
| `Source/MegalithUI/Private/Actions/Hoisted/AnimationEventActions.{h,cpp}` | `MegalithUI::FAnimationEventActions` (2 actions) |
| `Source/MegalithUI/Private/Actions/Hoisted/RoundedCornerActions.{h,cpp}` | `MegalithUI::FRoundedCornerActions` |
| `Source/MegalithUI/Private/Actions/Hoisted/ShadowActions.{h,cpp}` | `MegalithUI::FShadowActions` |
| `Source/MegalithUI/Private/Actions/Hoisted/GradientActions.{h,cpp}` | `MegalithUI::FGradientActions` |
| `Source/MegalithUI/Private/Tests/Hoisted/*Tests.cpp` | Test category `MegalithUI.<ActionName>.*` |
| `Source/MegalithUI/Private/Tests/Hoisted/MegalithUITestFixtureUtils.h` | Throwaway-WBP construction helpers under `MegalithUI::TestUtils` |
...
|---------|--------|---------|--------|
| `FUIAnimationMovieSceneBuilder` | MegalithUI (editor-only) | `Build(FUISpecDocument, UWidgetBlueprint*, FUIAnimationBuildOptions)` | `UWidgetAnimation` on `UWidgetBlueprint::Animations`, full MovieScene tree (binding → float track → section → channel) |
...
| Path | Contents |
|------|----------|
| `Plugins/Megalith/Source/MegalithUI/Public/Animation/UISpecAnimation.h` | `FUIAnimationBuildOptions`, `FUIAnimationRebuildResult`, `EUIAnimationRebuildPolicy` |
| `Plugins/Megalith/Source/MegalithUI/Public/Animation/UIAnimationMovieSceneBuilder.h` / `Private/Animation/UIAnimationMovieSceneBuilder.cpp` | `FUIAnimationMovieSceneBuilder` |
| `Plugins/Megalith/Source/MegalithUI/Private/Actions/Hoisted/AnimationCoreActions.{h,cpp}` | Phase I lifted the CSS-bezier math + binding/track helpers from `AnimationInternal::` (.cpp) into a public `MegalithUI::` surface (.h) so the builder can reuse them. Existing action handlers still call the .cpp helpers; the public surface forwards. |
...
| `Plugins/Megalith/Source/MegalithUI/Private/Tests/UIAnimationBuilderTests.cpp` | Test category `MegalithUI.AnimationBuilder.*` and `MegalithUI.AnimationRuntime.*` |

### Build.cs deps added by Phase I

None new. `MovieScene` and `MovieSceneTracks` were already in MegalithUI's PrivateDependencyModuleNames (Phase D). The runtime driver lives outside public Megalith; MegalithUI reaches optional runtime surfaces only through exported generic structs and reflection.
...
| Test | Verifies |
|------|----------|
| `MegalithUI.AnimationBuilder.MovieSceneOpacityTrack` | Build produces a `UWidgetAnimation` with a float track on `RenderOpacity`, 2 keys, correct values |
| `MegalithUI.AnimationRuntime.FadeInPlays` | Runtime driver advances opacity 0 → 1 over 10 ticks of 0.1s, mid-curve sample at 0.5s ≈ 0.5, IsPlaying false after end |
| `MegalithUI.AnimationBuilder.PreserveNamedSurvivesRebuild` | After two builds with the same animation name (5x duration delta), `PreserveAnimationsNamed=["Preserved"]` keeps UObject identity + key count unchanged |
...
`FMegalithUIStyleService` is a process-singleton (held by `TUniquePtr` in `MegalithUIStyleService.cpp`, lifecycle anchored to `FMegalithUIModule`) that dedupes CommonUI style assets — `UCommonButtonStyle` / `UCommonTextStyle` / `UCommonBorderStyle` Blueprints created via the class-as-data pattern. Every call to `create_common_button_style` / `create_common_text_style` / `create_common_border_style` now flows through the service before any disk I/O.
...
2. **Canonical library lookup** — scans `UMegalithUISettings::CanonicalLibraryPath` (default `/Game/UI/Library/`) for a same-typed asset with the same name. Pre-authored library styles ALWAYS win over auto-generated ones.
...
Misses fall through to creation. The new asset is created at `UMegalithUISettings::GeneratedStylesPath` (default `/Game/UI/Styles/`) using `IAssetTools::CreateUniqueAssetName` to dodge name collisions. The created `_C` class is what `apply_style_to_widget` / `batch_retheme` consume.

### Hash function

`FMegalithUIStyleService::ComputeContentHash` canonicalises the JSON property bag (sort field names, `%.6f` for floats, UTF-8 byte serialisation) and feeds the result into `FCrc::MemCrc32`. The class name is mixed into the canonical buffer so identical bags against different style types (e.g. Button{ColorAndOpacity:white} vs Text{ColorAndOpacity:white}) hash distinctly.
...
### Settings (`UMegalithUISettings`)

Project Settings → Plugins → Megalith UI surfaces the following config entries (stored under `[/Script/MegalithUI.MegalithUISettings]` in `DefaultGame.ini`):
...
`UDeveloperSettings::OnSettingChanged` live-refresh wiring is **deferred** until Phase E's `MegalithUIRegistrySubsystem.cpp` curated-mapping work lands (Phase G could not safely touch the same .cpp file in parallel). Until that follow-up patch, edits in Project Settings require an editor restart to take effect.
...
The service is a static singleton (not a `UEditorSubsystem` member), held by `TUniquePtr` in `MegalithUIStyleService.cpp` and torn down from `FMegalithUIModule::ShutdownModule`. This avoids the parallel-conflict on `MegalithUIRegistrySubsystem.cpp` (Phase E owns that file for B7 curated-mapping additions). The trade-off: no automatic registry-driven lifecycle, but the service has no GC-tracked state beyond `TStrongObjectPtr<UClass>` entries (which the explicit `Shutdown()` releases cleanly), so there's nothing the subsystem framework would have done for free.

### Source layout

| Path | Contents |
|------|----------|
| `Source/MegalithUI/Public/MegalithUISettings.h` / `Private/MegalithUISettings.cpp` | `UMegalithUISettings` (UDeveloperSettings) |
| `Source/MegalithUI/Public/Style/MegalithUIStyleService.h` / `Private/Style/MegalithUIStyleService.cpp` | `FMegalithUIStyleService`, `FUIStyleEntry`, `FUIStyleResolution`, `FUIStyleCacheStats` |
| `Source/MegalithUI/Private/Tests/UIStyleServiceTests.cpp` | Test category `MegalithUI.StyleService.*` |
...
| 3 | ~~optional provider .uplugin dep declaration~~ | **RESOLVED 2026-04-27** — superseded by the optional provider decouple. The pragmatic fix landed: zero compile-time dep on the provider + reflective `MegalithUI::GetEffectSurfaceClass()` probe. The missing-plugin dependency warning no longer fires because there is no warrant for the dep — MegalithUI does not include any provider header. See § "Optional Dep Probe API" + § "Error Contract — Optional EffectSurface Provider Absence (-32010)". |
...
| 6 | `UDeveloperSettings::OnSettingChanged` live-refresh wiring | Deferred from Phase G to avoid a parallel-edit conflict with the Phase E curated-mapping work in `MegalithUIRegistrySubsystem.cpp`. Until landed, edits in Project Settings → Plugins → Megalith UI require an editor restart. |
| 7 | `FUIPropertyPathCache::Cap` honours `UMegalithUISettings::PathCacheCap` | Currently hardcoded; the settings field exists but the cache constructor does not yet read it. Trivial follow-up. |
...
| 10 | Preset-literal drift detector (post-decouple R2.9) | The R2.9 metadata table in `MegalithUIEffectActions.cpp` (6 named presets — `rounded-rect`, `pill`, `circle`, `glass`, `glowing-button`, `neon`) and the provider's canonical presets MUST stay byte-aligned. Currently relies on manual diff in PR review. Future automation: a provider-side unit test that round-trips each preset name through both code paths and asserts pixel-identical render. Tracked here so the next post-decouple audit picks it up. |
| 11 | EEffectSurfaceFeature drift detector (post-decouple R2.5) | Raw `int32` constants `EffectFeature_RoundedCorners = 1<<0` … `EffectFeature_InsetHighlight = 1<<10` in `MegalithUIEffectActions.cpp` shadow the provider's canonical feature enum. Same drift class as #10 — manual diff today; would be neat to gate with a provider-side static assert. |
| 12 | Editor-down semantic distinction (2026-05-16 UI gap audit Item #22) | Editor-down semantic distinction: when MCP transport drops, agents get a generic disconnect. Wrapper distinguishing 'editor crashed' vs 'MCP proxy crashed' + auto-suggest offline `megalith_query.exe` fallback. Out of scope for MegalithUI; lives in MCP proxy / transport layer. Captured for visibility — separate Megalith MCP proxy ticket. |
...
`bulk_fill` + `describe` are framework-level namespaces (not counted in the per-module surface). The Phase 4 adapter does not add any new `ui::*` action — `megalith_discover("ui")` count is unchanged. The `megalith_discover("bulk_fill")` / `megalith_discover("describe")` rolls now includes `{namespace: "ui", available: true, stub: false}` in dev builds and `{namespace: "ui", available: true, stub_branch: "WITH_COMMONUI=0 disables InputActionDataTable only"}` in release builds.

### Source layout

| Path | Contents |
|------|----------|
| `Source/MegalithUI/Private/MegalithUIBulkFillAdapter.h` | `FMegalithUIBulkFillAdapter` static-API declaration |
| `Source/MegalithUI/Private/MegalithUIBulkFillAdapter.cpp` | Adapter impl: `UIBulkFill`, `UIDescribe`, `Register`, `Unregister`. Process-wide `GUIBulkFillCriticalSection` lives at TU scope. |
| `Source/MegalithUI/Private/MegalithUIModule.cpp` | `FMegalithUIBulkFillAdapter::Register()` from `StartupModule`, `Unregister()` from `ShutdownModule` (both unconditional per H5) |

### Build.cs deps added by Phase 4

None. `UMG` + `UMGEditor` + `Json` + `JsonUtilities` + `UnrealEd` + `MegalithCore` were already present.
