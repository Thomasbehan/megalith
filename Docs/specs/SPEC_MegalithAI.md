...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, UnrealEd, AIModule, GameplayTasks, NavigationSystem, Json, JsonUtilities
**Namespace:** `ai` | **Tool:** `ai_query(action, params)` | **Actions:** ~223 (Phase J F8: +`add_perception_to_actor`, +`get_bt_graph`; test/profiling harness Wave 1: +`rebuild_navigation`, +`validate_nav_points`). Counts approximate — query `megalith_discover("ai")` for the live figure.
...
MegalithAI provides comprehensive MCP coverage of Unreal Engine's AI framework. It covers Behavior Trees, Blackboards, State Trees, Environment Query System (EQS), Smart Objects, AI Controllers, AI Perception, Navigation, runtime/PIE control, scaffolding templates, discovery, and advanced AI operations.

### Action Categories

Counts below are the **actual** registrations from `Source/MegalithAI/Private/MegalithAI*Actions.cpp` (verified 2026-04-26 post-Phase-J). Pre-J the spec listed `~N` aspirational figures that summed to 249, exceeding the 219 actually shipped. Audit corrected the table to literal counts.

| Category | Actions | Source file | Description |
|----------|---------|-------------|-------------|
| Behavior Trees | 32 | `MegalithAIBehaviorTreeActions.cpp` | BT CRUD, node management, decorator/service/task creation, composite nodes, spec-based generation. **F1** crash-hardening + **F8** `get_bt_graph` + **F15** invalid-vs-unknown GUID error split |
| Blackboards | 12 | `MegalithAIBlackboardActions.cpp` | BB CRUD, key management, key types, inheritance, inspection |
| State Trees | 35 | `MegalithAIStateTreeActions.cpp` | ST CRUD, state/transition management, conditions, tasks, spec-based generation. Conditional on `#if WITH_STATETREE` |
| EQS | 20 | `MegalithAIEQSActions.cpp` | EQS query CRUD, generator/test management, contexts, debugging |
| Smart Objects | 16 | `MegalithAISmartObjectActions.cpp` | SO definition CRUD, slot configuration, behavior binding. Conditional on `#if WITH_SMARTOBJECTS` |
| AI Controllers | 10 | `MegalithAIControllerActions.cpp` | Controller configuration, team assignment, focus management |
| Perception | 11 | `MegalithAIPerceptionActions.cpp` | Sight/hearing/damage/team sense configuration, stimulus management (AIController-only) |
| Perception Scaffold | 1 | `MegalithAIPerceptionScaffoldActions.cpp` | **F8** `add_perception_to_actor` — accepts ANY actor BP (not just AIControllers) plus a `senses` array |
| Navigation | 26 | `MegalithAINavigationActions.cpp` | NavMesh queries, path finding, nav link management, nav modifier volumes. **Test/profiling harness Wave 1:** +`rebuild_navigation`, +`validate_nav_points` |
| Runtime/PIE | 14 | `MegalithAIRuntimeActions.cpp` | Runtime BT/ST inspection, active task queries, blackboard value read/write in PIE |
| Scaffolding | 23 | `MegalithAIScaffoldActions.cpp` | Pre-built AI patterns: patrol, guard, investigate, flee, horror stalker, search area |
| Discovery | 11 | `MegalithAIDiscoveryActions.cpp` | AI asset overview, explain, compare, validate, search |
| Advanced | 12 | `MegalithAIAdvancedActions.cpp` | Mass Entity + Zone Graph cross-module integration (conditional `#if WITH_MASSENTITY`, `#if WITH_ZONEGRAPH`) |

**Total:** 32 + 12 + 35 + 20 + 16 + 10 + 11 + 1 + 26 + 14 + 23 + 11 + 12 = **223**.

### Test/Profiling Harness — Wave 1 nav actions

Two navigation actions added for the test/profiling harness workflow (`MegalithAINavigationActions.cpp`):
...
- **F15 (2026-04-26)** — `MegalithAIBehaviorTreeActions.cpp` 16 sites hoisted into `RequireBtNodeByGuid` helper — invalid-GUID and unknown-GUID errors now distinct.
...
| Class | Behaviour |
|-------|-----------|
| `AMegalithBehaviorTreeAIController` | A Blueprintable `AAIController` carrying a Behavior Tree `UPROPERTY`. On `OnPossess` it runs the assigned tree, so possessing a pawn with this controller auto-starts the AI's BT. The canonical way to start an authored wander tree without hand-wiring a `RunBehaviorTree` call. |
...
`MegalithAIBulkFillAdapter` registers under `FMegalithBulkFillRegistry` for the `ai` namespace, exposed via the framework-level `bulk_fill_query("apply", ...)` and `describe_query("schema", ...)` dispatchers. Phase 5 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`, implementation plan `Docs/plans/2026-05-11-megalith-mcp-ergonomics.md`).
...
**fill_kind catalogue (3 — enumerated against `MegalithAIBulkFillAdapter.cpp`):**
...
> **24K lines of C++ across 30 files.** MegalithAI is the largest domain module by code volume.
>
> **Multi-plugin conditional compilation.** Unlike single-guard modules (GAS, ComboGraph, LogicDriver), MegalithAI uses multiple compile-time guards. State Trees and Smart Objects are required; Mass Entity and Zone Graph are optional extensions that unlock additional actions when present.
