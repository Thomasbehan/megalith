...
**Dependencies:** Core, CoreUObject, Engine, MegalithCore, GameplayAbilities, GameplayTags
...
| UI Binding | 4 | `bind_widget_to_attribute`, `unbind_widget_attribute`, `list_attribute_bindings`, `clear_widget_attribute_bindings`. Authored via `UMegalithGASAttributeBindingClassExtension`. **Also registered as aliases in the `ui` namespace** (so `ui::bind_widget_to_attribute` and `gas::bind_widget_to_attribute` dispatch to the same handler — see `MegalithGASUIBindingActions.cpp:561-577`). The `ui::` aliases are documented in [SPEC_MegalithUI.md](SPEC_MegalithUI.md) "GAS Bridge Aliases" section |
...
- **F9 logging (2026-04-26)** — Observability adds + `LogMegalithGASUIBinding` / `LogMegalithGASUIBindingExt` retired into parent `LogMegalithGAS` category.
...
>
> **UI Binding cooked-build caveat.** `UMegalithGASAttributeBindingClassExtension` is an editor-only class — content WBPs that reference it will fail to apply bindings in cooked Steam builds. See [COOKED_BUILD_TODO.md](../COOKED_BUILD_TODO.md) for the resolution path (Option A/B/C deferred to pre-Steam-launch checkpoint).
>
...
The `gas` namespace registers a `FMegalithBulkFillRegistry` adapter (`MegalithGASBulkFillAdapter.cpp`) routed from the central `bulk_fill_query("apply")` and `describe_query("schema")` dispatchers. Phase 2 of the MCP ergonomics rollout (design spec `Docs/plans/2026-05-11-megalith-mcp-ergonomics-design.md`, implementation plan `Docs/plans/2026-05-11-megalith-mcp-ergonomics.md`). This collapses the 20-attr × 10-level ≈ 200-call grind on AttributeInit DataTables into a single transacted call.
...
**fill_kind catalogue (1 — enumerated against `MegalithGASBulkFillAdapter.cpp`):**
...
**H5 stub-adapter invariant:** the adapter's `Register()` call runs unconditionally from `FMegalithGASModule::StartupModule` regardless of `WITH_GBA`. The adapter BODY switches on `WITH_GBA` — the dev build wires the real handlers; release builds without GAS return a clean `"GAS optional dep not available (WITH_GBA=0)"` error. This guarantees `megalith_discover("gas")` action surface is identical across dev + release builds.
...
- `Plugins/Megalith/Source/MegalithGAS/Private/MegalithGASBulkFillAdapter.h` / `.cpp` — the adapter
- `Plugins/Megalith/Source/MegalithGAS/Private/MegalithGASModule.cpp` — `Register()` + `Unregister()` call sites
