// megalith_query.exe — Standalone offline query tool for Megalith databases.
// Replaces megalith_offline.py with zero Python dependency.
// Links sqlite3 amalgamation directly. No Unreal Engine dependency.
//
// Usage:
//   megalith_query.exe source <action> [params...] [--options]
//   megalith_query.exe project <action> [params...] [--options]
//   megalith_query.exe megalith guide [--section=NAME]
...
// ============================================================
// Fuzzy match — ports MegalithFuzzyMatchDetail::ScoreFuzzyMatches
// (Source/MegalithCore/Private/MegalithFuzzyMatch.cpp). The live version uses
...
// and the Python port in megalith_offline.py.
// ============================================================
...
static const std::vector<std::string>& offline_namespaces() {
    static const std::vector<std::string> ns = {
        "source", "project", "megalith", "cppreflect", "network", "decision", "risk"
    };
    return ns;
}
...
// project search. Mirrors FMegalithIndexDatabase::FullTextSearch's step check.
...
// storage/schema access. Mirrors MegalithProjectSearchDetail::IsFts5QuerySyntaxError.
...
// MegalithProjectSearchDetail::DescribeUnknownColumn.
...
// MegalithProjectSearchActionDetail::MaxQueryLength.
...
static Args parse_args(int argc, char* argv[]) {
    Args args;
    if (argc < 3) {
        std::cerr << "Usage: megalith_query <source|project> <action> [params...] [--options]\n\n"
                  << "Source actions:\n"
...
public:
    ...
    // FMegalithSourceActions handlers' content[].text rendering for parity.
    // ============================================================

    // Mirror of FMegalithSourceActions::DeriveIncludePath.
    ...
    // Mirror of FMegalithSourceActions::CompactDeclaration.
    ...
    // --- lint_header (item 7) ---
    // Mirror of FMegalithSourceActions::LintHeaderLines with an ALWAYS-EMPTY
...
// output against the Python sibling (Scripts/megalith_offline.py) and the live
...
static std::string resolve_db_dir() {
    ...
    // (exe at Plugins/Megalith/Tools/MegalithQuery/ or Plugins/Megalith/Binaries/)
    ...
    // Try ../../Saved/ (from Tools/MegalithQuery/)
...
// Resolve the Megalith plugin root (the dir containing Megalith.uplugin) by
// probing the same exe-relative layouts resolve_db_dir() uses. Returns empty
// if not found.
//   exe at Plugins/Megalith/Tools/MegalithQuery/ -> root is ../../
//   exe at Plugins/Megalith/Binaries/            -> root is ../
//   exe at Plugins/Megalith/ (plugin root)       -> root is ./
static fs::path resolve_plugin_root() {
    ...
    const fs::path candidates[] = {
        exe_dir / ".." / "..",   // Tools/MegalithQuery/
        exe_dir / "..",          // Binaries/
        exe_dir                  // plugin root
    };
    for (const auto& c : candidates) {
        if (fs::exists(c / "Megalith.uplugin"))
            return fs::canonical(c);
    }
    return fs::path();
}

// Read "VersionName" from Megalith.uplugin via a plain string scan (no JSON
...
static std::string parse_uplugin_version(const fs::path& plugin_root) {
    ...
    std::ifstream f((plugin_root / "Megalith.uplugin").string());
    if (!f.is_open())
        return "";
...
// ============================================================
// Megalith namespace — offline guide server
// ============================================================
//
// Mirrors the in-editor FMegalithGuideTool but WITHOUT the live registry
// overlay (offline has no registry — it just serves the markdown). The H2
// section split mirrors FMegalithGuideTool::SplitSections: a line is a section
...
// MegalithGuideTool.cpp so offline and in-editor agree on index ordering.
...
class MegalithActions {
public:
    // --- guide ---
    void print_guide(const Args& args) {
        fs::path plugin_root = resolve_plugin_root();
        fs::path guide_path = plugin_root.empty()
            ? fs::path("MEGALITH_GUIDE.md")
            : (plugin_root / "Docs" / "MEGALITH_GUIDE.md");

        std::ifstream f(guide_path.string());
        if (!f.is_open()) {
            std::cout << "Guide markdown not found at " << guide_path.string()
                      << "\nRestore Docs/MEGALITH_GUIDE.md (it ships in the release zip) "
                         "or pull the latest Megalith release, then retry." << std::endl;
            return;
        }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--version" || a == "-v") {
            ...
            std::string version = parse_uplugin_version(plugin_root);
            ojson out;
            out["tool"] = "megalith_query";
            out["plugin_version"] = version;          // Megalith.uplugin VersionName ("" if unresolved)
    ...
    if (args.ns == "source") {
    ...
    } else if (args.ns == "megalith") {
        // No database — the guide is served straight from Docs/MEGALITH_GUIDE.md.
        MegalithActions ma;

        static const std::map<std::string, std::function<void(MegalithActions&, const Args&)>> actions = {
            {"guide", [](MegalithActions& m, const Args& a) { m.print_guide(a); }},
        };

        auto it = actions.find(args.action);
        if (it == actions.end()) die("Unknown megalith action: " + args.action + " (expected 'guide')");
        it->second(ma, args);
        ...
        if (it == table.end()) {
            ...
            // megalith_discover(\"<ns>\") to enumerate valid actions in this
            ...
            out["error"] = "Unknown action: " + args.ns + "." + args.action +
                           " \xE2\x80\x94 call megalith_discover(\"" + args.ns +
