@echo off
REM Build megalith_query.exe — standalone SQLite query tool
...
set SOURCE_HASH=dev
set SRCHASH_RAW=
for /f "skip=1 tokens=*" %%H in ('certutil -hashfile megalith_query.cpp SHA256') do if not defined SRCHASH_RAW set SRCHASH_RAW=%%H
...
cl /EHsc /std:c++17 /O2 /MT /I ThirdParty /I ..\MegalithProxy\ThirdParty /DSQLITE_ENABLE_FTS5 /DSOURCE_HASH=\"%SOURCE_HASH%\" megalith_query.cpp ThirdParty\sqlite3.c /Fe:megalith_query.exe || goto :buildfail

:copy
if not exist ..\..\Binaries mkdir ..\..\Binaries
copy /Y megalith_query.exe ..\..\Binaries\megalith_query.exe || goto :copyfail
echo.
echo Built: Plugins\Megalith\Binaries\megalith_query.exe
exit /b 0
...
:copyfail
echo Failed to copy megalith_query.exe into Binaries.
exit /b 1
