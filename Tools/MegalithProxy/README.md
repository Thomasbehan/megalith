# Megalith MCP Proxy Configuration

## Current Configuration

The Megalith MCP is configured to use the C++ proxy executable:

```json
{
  "command": "<project-root>/Plugins/Megalith/Binaries/megalith_proxy.exe",
  "args": []
}
```
...
{
  "command": "python",
  "args": ["<project-root>/Scripts/megalith_proxy.py"]
}
```

Update the megalith entry in:
...
- **Python proxy:** `Scripts/megalith_proxy.py` — Stdio-to-HTTP proxy, survives editor restarts via background health polling
- **C++ proxy:** `Plugins/Megalith/Binaries/megalith_proxy.exe` — Native executable, faster startup
- **Backend:** Both connect to the same Megalith HTTP server running in the Unreal Editor
- **Editor-down startup:** Both proxies return a cached Megalith tool list when available, or a stable seed list of namespace/meta tools. This prevents MCP clients that do not fully refresh on `tools/list_changed` from starting with an empty Megalith catalog.
...
```
<project-root>/Saved/Logs/MegalithCalls.jsonl
```

Path resolution: `MEGALITH_PROJECT_ROOT` env var if set, otherwise the proxy's
...
| `namespace` | string | For `*_query` tools: the prefix (e.g. `editor`). For `megalith_*` tools: `megalith`. For non-`tools/call` methods: the method name (`initialize`, `tools/list`, `ping`). |
| `action` | string | For `*_query`: the `action` argument. For `megalith_*`: the suffix (`discover`, `status`, etc.). Empty otherwise. |
...
Set the environment variable `MEGALITH_CALL_LOG=0` before launching the proxy.
...
add `MegalithCalls.jsonl` to its exclusion list. The file is intentionally
local-only.
