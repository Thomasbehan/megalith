#!/usr/bin/env python3
"""
Megalith MCP stdio-to-HTTP proxy.

Sits between Claude Code (stdio) and Megalith (HTTP on localhost).
...
Usage (in .mcp.json):
  {"mcpServers": {"megalith": {"command": "python", "args": ["Plugins/Megalith/Scripts/megalith_proxy.py"]}}}
...
MEGALITH_URL = os.environ.get("MEGALITH_URL", "http://localhost:9316/mcp")
MEGALITH_HEALTH = MEGALITH_URL.replace("/mcp", "/health")
PROXY_NAME = "megalith-proxy"
PROXY_VERSION = "1.1.1"
TIMEOUT = 30.0
POLL_INTERVAL = 5.0
POLL_START_DELAY = 3.0

# Track Megalith availability for list_changed notifications
_megalith_was_up = None
...
def _log(msg: str) -> None:
    ...
    print(f"[megalith-proxy] {msg}", file=sys.stderr, flush=True)
...
def _resolve_call_log_path() -> Path:
    ...
    """
    root = os.environ.get("MEGALITH_PROJECT_ROOT") or os.getcwd()
    logs_dir = Path(root) / "Saved" / "Logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    return logs_dir / "MegalithCalls.jsonl"
...
def _init_call_log() -> None:
    ...
    _call_log_enabled = os.environ.get("MEGALITH_CALL_LOG", "1") != "0"
    if not _call_log_enabled:
        _log("Call log disabled (MEGALITH_CALL_LOG=0)")
        return
...
def _extract_namespace_action(msg: dict) -> tuple[str, str]:
    ...
    if name.startswith("megalith_"):
        return "megalith", name[len("megalith_"):]
...
def _post_megalith(body: str, timeout: float = TIMEOUT) -> str | None:
    """POST JSON-RPC to Megalith. Returns response body or None on failure."""
    try:
...
def _tools_cache_path() -> Path:
    ...
    cache_dir = base / "Megalith"
    cache_dir.mkdir(parents=True, exist_ok=True)

    host_port = MEGALITH_HEALTH.replace("http://", "").replace("https://", "")
    host_port = host_port.split("/", 1)[0]
    return cache_dir / f"megalith_proxy_tools_{_sanitize_cache_part(host_port)}.json"
...
def _seed_tools() -> list[dict]:
    ...
    for name in CORE_QUERY_TOOLS:
        ...
        tools.append(_make_tool(
            name,
            f"Query the {domain} domain. The editor may be offline at session start; retry after Megalith is healthy.",
            _query_tool_schema(),
        ))
...
def _fallback_tools_list(msg: dict) -> str:
    ...
    if cached:
        _log("Megalith down during tools/list — returning cached tools")
        return _result(msg.get("id"), {"tools": cached})

    _log("Megalith down during tools/list — returning seed tools")
    return _result(msg.get("id"), {"tools": _seed_tools()})


def _check_megalith_up() -> bool:
    """Lightweight health check via GET /health endpoint."""
    try:
        req = urllib.request.Request(MEGALITH_HEALTH, method="GET")
...
def check_megalith_state_change(stdout) -> None:
    """Check for state transition and notify if changed."""
    global _megalith_was_up
    is_up = _check_megalith_up()

    if _megalith_was_up is not None and is_up != _megalith_was_up:
        direction = "online" if is_up else "offline"
        _log(f"Megalith went {direction} — sending tools/list_changed")
        _send_list_changed(stdout)

    _megalith_was_up = is_up


def _health_poll_thread(stdout) -> None:
    """Background thread that polls Megalith and sends list_changed on state transitions."""
    time.sleep(POLL_START_DELAY)
    _log(f"Health poll started (interval={POLL_INTERVAL}s)")

    while True:
        try:
            check_megalith_state_change(stdout)
        except (BrokenPipeError, OSError):
...
def handle_initialize(msg: dict) -> str:
    ...
    return _result(msg.get("id"), {
        ...
        "instructions": (
            "Megalith MCP proxy for Unreal Engine. Tools are forwarded to the Unreal Editor. "
            "Before calling a domain action, check its schema instead of guessing: "
            "megalith_discover() lists namespaces, megalith_discover('<namespace>') lists a "
            "namespace's actions, and describe_query('action_schema', ...) returns an action's "
            "exact parameter schema. megalith_guide(section='recipes') gives cross-namespace "
...
def handle_tools_list(msg: dict) -> str:
    """Forward tools/list to Megalith. Stable cached/seed list if down."""
    t0 = time.perf_counter()
    resp = _post_megalith(json.dumps(msg))
...
def handle_tools_call(msg: dict) -> str:
    """Forward tools/call to Megalith. Graceful error if down."""
    t0 = time.perf_counter()
    resp = _post_megalith(json.dumps(msg))
    duration_ms = (time.perf_counter() - t0) * 1000.0
    _write_call_log_line(msg, resp, duration_ms)
    ...
    return _tool_error(
        msg.get("id"),
        f"Megalith MCP is not available (Unreal Editor not running). "
...
def main() -> None:
    ...
    _log(f"Started. Forwarding to {MEGALITH_URL}")
