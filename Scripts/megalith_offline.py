#!/usr/bin/env python3
"""
Stdlib-only offline DEV FALLBACK for the canonical megalith_query.exe.
  Kept byte-identical to the exe by Scripts/verify_offline_parity.py.
  Serves source / project / megalith subcommands plus all 20 Reflection-Intelligence
...
Megalith Offline CLI — query EngineSource.db and ProjectIndex.db without the editor.

Usage:
    python megalith_offline.py source <action> [params...]
    python megalith_offline.py project <action> [params...]
    python megalith_offline.py --version
...
FULL offline parity with the live RI adapters (Source/MegalithReflectionIntel/Private/
...
be served by this tool. `megalith guide` stays exe-only (it depends on runtime
...
# Parity revision string. MUST match the C++ exe sibling (Tools/MegalithQuery)
...
class SourceActions:
    ...
    # megalith_query.exe SourceActions methods (parity-gated).
    # ============================================================
    ...
    # --- shared composition helpers (mirror FMegalithSourceActions) ---
    ...
    # ------------------------------------------------------------------
    # item 7: lint_header — mirror of FMegalithSourceActions::LintHeaderLines
...
# ============================================================
# Project actions — mirrors Source/MegalithIndex (see project search notes below)
# ============================================================
...
def _describe_unknown_column(message: str) -> str:
    ...
    Mirrors MegalithProjectSearchDetail::DescribeUnknownColumn.
    """
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Megalith Offline CLI — query source/project/RI databases without the editor",
    ...
    # (`megalith_query.exe source generate_class_stub <parent> <class_name> <module>`).
    ...
    # (`megalith_query.exe cppreflect list_uproperties <ClassName>`); --class_name kept as alias.
...
def main():
    ...
    if len(sys.argv) >= 2 and not sys.argv[1].startswith("-"):
        ...
        if requested_ns not in OFFLINE_NAMESPACES:
            suffix = did_you_mean_suffix(requested_ns, OFFLINE_NAMESPACES + ["megalith"])
            msg = (f"Unknown namespace: {requested_ns} — call megalith_discover() "
    ...
    if len(sys.argv) >= 3 and sys.argv[1] in ACTIONS_BY_NS and not sys.argv[2].startswith("-"):
        ...
        if act not in ACTIONS_BY_NS[ns]:
            ...
            msg = (f"Unknown action: {ns}.{act} — call megalith_discover(\"{ns}\") "
