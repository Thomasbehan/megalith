...
#   the new tag with zero or partial assets. Megalith clients from v0.14.7
...
$script:PreV2MarkerPattern = 'Megalith-SHA256(-UE5\.[0-9]+)?:'
...
if ($SelfTest) {
    $H = 'a' * 64
    $mustReject = @(
        @{ Name = 'bare marker';        Body = "Megalith-SHA256: $H" },
        @{ Name = 'markdown bullet';    Body = "- Megalith-SHA256: $H" },
        @{ Name = 'table cell';         Body = "| Megalith-SHA256: | $H |" },
        @{ Name = 'prose prefix';       Body = "Legacy zip -- Megalith-SHA256: $H" },
        @{ Name = 'hash on next line';  Body = "Megalith-SHA256:`n$H" },
        @{ Name = 'engine-tagged';      Body = "- Megalith-SHA256-UE5.7: $H" },
        @{ Name = 'indented tagged';    Body = "    Megalith-SHA256-UE5.8: $H" }
    )
    $mustAccept = @(
        @{ Name = 'the three v2 markers'
           Body = "Megalith-SHA256-v2-UE5.7: $H`nMegalith-SHA256-v2-UE5.8: $H`nMegalith-SHA256-v2: $H" },
        @{ Name = 'v2 markers in prose/bullets'
           Body = "- Megalith-SHA256-v2: $H`n| Megalith-SHA256-v2-UE5.7: | $H |" }
    )
...
$expected = @(
    "Megalith-v$Version-UE5.7.zip",
    "Megalith-v$Version-UE5.8.zip",
    "Megalith-v$Version.zip"
)
...
# The deployed client parser (v0.21.0 MegalithUpdateSubsystem.cpp:338) is
#     FRegexPattern("Megalith-SHA256:\s*([0-9a-fA-F]{64})(?![0-9a-fA-F])")
...
#     - Megalith-SHA256: <64hex>              (markdown bullet; "-" is not \s)
#     | Megalith-SHA256: | <64hex> |          (table cell)
#     Legacy zip -- Megalith-SHA256: <64hex>  (any prose prefix)
...
# This cannot false-positive on the v2 names: after the "Megalith-SHA256"
...
$markerMap = @{
    "Megalith-SHA256-v2-UE5.7:" = "Megalith-v$Version-UE5.7.zip"
    "Megalith-SHA256-v2-UE5.8:" = "Megalith-v$Version-UE5.8.zip"
    "Megalith-SHA256-v2:"       = "Megalith-v$Version.zip"
}
$markerValues = @{}
...
if ($markerValues.ContainsKey("Megalith-SHA256-v2:") -and $markerValues.ContainsKey("Megalith-SHA256-v2-UE5.7:")) {
    if ($markerValues["Megalith-SHA256-v2:"] -ne $markerValues["Megalith-SHA256-v2-UE5.7:"]) {
