...
// MegalithMaterialBulkFillAdapter — Phase 5 Step 3 adapter.
...
// project precedent at MegalithMaterialActions.cpp:27-onward uses the same include set.
#include "Engine/Texture.h"

#define LOCTEXT_NAMESPACE "MegalithMaterialBulkFillAdapter"

namespace MegalithMaterialBulkFillInternal
{
	...
	static FDryRunReport HandleMICParameters(const FBulkFillSpec& Spec)
	{
		...
		if (!Spec.bDryRun)
		{
			...
			Transaction = MakeShared<FScopedTransaction>(
				LOCTEXT("MaterialBulkFill_MIC", "Megalith Material Bulk Fill — MIC Parameters"));
			MIC->Modify();
		}
		...
		if (Spec.Tree->TryGetObjectField(TEXT("scalars"), ScalarsObj) && ScalarsObj && (*ScalarsObj).IsValid())
		{
			...
			{
				const FString KVKeyStr = MegalithKeyToString(KV.Key);
		...
		if (Spec.Tree->TryGetObjectField(TEXT("textures"), TexturesObj) && TexturesObj && (*TexturesObj).IsValid())
		{
			for (const auto& KV : (*TexturesObj)->Values)
			{
				...
				UTexture* Tex = Cast<UTexture>(FMegalithAssetUtils::LoadAssetByPath(TexPath));
				if (!Tex)
				{
					RecordField(FString::Printf(TEXT("textures[%s]"), *KVKeyStr),
						TexPath, false,
		...
		if (Spec.Tree->TryGetObjectField(TEXT("switches"), SwitchesObj) && SwitchesObj && (*SwitchesObj).IsValid())
		{
			for (const auto& KV : (*SwitchesObj)->Values)
			{
				...
				if (!Spec.bDryRun)
				{
					// Project precedent: MegalithMaterialActions.cpp:2775 uses the
...
FDryRunReport FMegalithMaterialBulkFillAdapter::MaterialBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithMaterialBulkFillInternal;
...
FSchemaDescriptor FMegalithMaterialBulkFillAdapter::MaterialDescribe(const FString& TargetAsset)
{
	using namespace MegalithMaterialBulkFillInternal;
...
void FMegalithMaterialBulkFillAdapter::Register()
{
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("material"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithMaterialBulkFillAdapter::MaterialBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithMaterialBulkFillAdapter::MaterialDescribe));
}

void FMegalithMaterialBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("material"));
}
