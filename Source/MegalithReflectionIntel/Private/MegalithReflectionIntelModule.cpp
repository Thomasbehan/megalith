...
//
// DEVIATION (vs plan §6 row "Modify MegalithSourceSubsystem.cpp — register
// FDecisionRecordIndexer in the indexer roster"):
// UMegalithSourceSubsystem does NOT expose an indexer-roster API in the live
// codebase; it owns a single FMegalithSourceIndexer for C++ source mining
// only. To keep dependency direction correct (MegalithReflectionIntel ->
// MegalithSource is the legal direction; the reverse would be circular),
...
// indexer writes) through UMegalithSourceSubsystem's already-open EngineSource.db
...
DEFINE_LOG_CATEGORY(LogMegalithReflectionIntel);

#define LOCTEXT_NAMESPACE "FMegalithReflectionIntelModule"
...
namespace
{
	...
	/**
	 * Resolve the live FMegalithReflectionIntelModule instance from inside a
	 ...
	 */
	FMegalithReflectionIntelModule* GetReflectionIntelModulePtr()
	{
		return FModuleManager::GetModulePtr<FMegalithReflectionIntelModule>(
			TEXT("MegalithReflectionIntel"));
	}

	/**
	 * Resolve the source subsystem's FMegalithSourceDatabase wrapper (NOT the raw
	 ...
	 */
	FMegalithSourceDatabase* GetSharedSourceDatabase()
	{
		if (!GEditor)
		{
			return nullptr;
		}
		UMegalithSourceSubsystem* SourceSS =
			GEditor->GetEditorSubsystem<UMegalithSourceSubsystem>();
...
void FMegalithReflectionIntelModule::StartupModule()
{
	...
	ReloadCompleteHandle = FCoreUObjectDelegates::ReloadCompleteDelegate.AddRaw(
		this, &FMegalithReflectionIntelModule::OnReloadComplete);
	...
	UE_LOG(LogMegalithReflectionIntel, Log,
		TEXT("Megalith — ReflectionIntel module loaded (decision_query: 5 actions, "
...
void FMegalithReflectionIntelModule::ShutdownModule()
{
	...
	// EngineSource.db (it borrows UMegalithSourceSubsystem's). Nothing to tear
	...
	FMegalithToolRegistry::Get().UnregisterNamespace(TEXT("decision"));
	// Phase 2 — risk_query is fully owned by this module so unregister wholesale.
	FMegalithToolRegistry::Get().UnregisterNamespace(TEXT("risk"));
	// Phase 3a — cppreflect_query is fully owned by this module.
	FMegalithToolRegistry::Get().UnregisterNamespace(TEXT("cppreflect"));
	// Phase 4a — network_query is fully owned by this module.
	FMegalithToolRegistry::Get().UnregisterNamespace(TEXT("network"));
	// Phase 4a — pipeline_query is fully owned by this module (no other
	// module registers there in Phase 4a).
	FMegalithToolRegistry::Get().UnregisterNamespace(TEXT("pipeline"));
	// Maintenance — reflect_query is fully owned by this module.
	FMegalithToolRegistry::Get().UnregisterNamespace(TEXT("reflect"));
	// NOTE: we do NOT unregister the `source` / `material` / `niagara` /
	// `blueprint` / `project` namespaces here — MegalithSource / MegalithMaterial
	// / MegalithNiagara / MegalithBlueprint / MegalithIndex own the lion's share
	...
	// ships with those modules loaded before us (the `MegalithCore` dep chain
	// enforces order) so shutdown-time leaks are bounded to one audit action
	// entry per host namespace per process exit.
}

FSQLiteDatabase* FMegalithReflectionIntelModule::GetOrOpenCachedQueryDb()
{
	...
	FMegalithSourceDatabase* SharedDb = GetSharedSourceDatabase();
	if (!SharedDb)
	{
		return nullptr;
	}
...
void FMegalithReflectionIntelModule::ResetCachedQueryDb()
{
...
void FMegalithReflectionIntelModule::RegisterDecisionActions()
{
	FDecisionQueryAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::RegisterRiskActions()
{
	FRiskQueryAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::RegisterSourceAuditActions()
{
	FModuleDepRealityAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::RegisterCppReflectActions()
{
	FCppReflectQueryAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::RegisterNetworkActions()
{
	FNetworkQueryAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::RegisterAuditActions()
{
	FAuditAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::RegisterPipelineActions()
{
	FPipelineAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::RegisterMaintenanceActions()
{
	FReflectMaintenanceAdapter::RegisterActions(FMegalithToolRegistry::Get());
}

void FMegalithReflectionIntelModule::OnReloadComplete(EReloadCompleteReason /*Reason*/)
{
...
bool FMegalithReflectionIntelModule::RunDecisionIndexerOnce(FString& OutStatus)
{
	...
	FMegalithReflectionIntelModule* Self = GetReflectionIntelModulePtr();
	...
	const UMegalithReflectionIntelSettings* Settings = UMegalithReflectionIntelSettings::Get();
	...
	if (!SharedDb || !SharedDb->GetRawHandle())
	{
		...
		UE_LOG(LogMegalithReflectionIntel, Verbose, TEXT("%s"), *OutStatus);
	...
	{
		Roots.Add(TEXT("Docs"));
		Roots.Add(TEXT("Plugins/Megalith/Docs"));
	...
	if (Self)
	{
		...
		{
			UE_LOG(LogMegalithReflectionIntel, Warning,
...
bool FMegalithReflectionIntelModule::RunRiskIndexersOnce(FString& OutStatus)
{
	...
	// We do NOT scan the Plugins/Megalith folder root — only its Source/ —
	...
	UE_LOG(LogMegalithReflectionIntel, Log, TEXT("%s"), *OutStatus);
...
FString FMegalithReflectionIntelModule::GetRiskNoReposHint()
{
	return TEXT("No git repository was found, so risk / co-change data is empty. "
		...
		"Set GitRepoRoots (Editor Preferences > Plugins > Megalith Reflection Intel > Risk) to your "
		"repository path, or enable bProbeAncestorsForGitRoot if the project sits inside a larger repository.");
}

TArray<FString> FMegalithReflectionIntelModule::ResolveGitRepoRoots(
...
TArray<FString> FMegalithReflectionIntelModule::ResolveArtefactRoots(
...
bool FMegalithReflectionIntelModule::RunCppReflectIndexersOnce(FString& OutStatus)
{
...
bool FMegalithReflectionIntelModule::RunNetworkIndexerOnce(FString& OutStatus)
{
...
bool FMegalithReflectionIntelModule::ForceRebuildReflectionTables(
...
IMPLEMENT_MODULE(FMegalithReflectionIntelModule, MegalithReflectionIntel)
