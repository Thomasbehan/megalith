...
//
// MegalithRIMetaTable — indexer-code-version stamping for the RI reflect_* tables.
//
...
// subsystem `megalith_ri_meta` table. On adapter lazy-bootstrap, if
...
namespace MegalithRIMeta
{
	...
	/**
	 * Idempotent CREATE TABLE for `megalith_ri_meta`. Returns true on success.
	 ...
	 * megalith_ri_meta.code_version binds through int32. The cast happens HERE,
	 ...
	 *
	 * Pass the roots exactly as FMegalithReflectionIntelModule::ResolveGitRepoRoots
