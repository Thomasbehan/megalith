...
namespace
{
	...
	/**
	 * Replace C# `//` line comments and `/<star> ... <star>/` block comments with a single
	 ...
	 * appear INSIDE comments. String-literal aware: a `//` or `/<star>` inside a "..."
...
void FModuleDepRealityAdapter::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("source"), TEXT("audit_module_dep_reality"),
		TEXT("Audit: find UE type references in .h/.cpp files whose declaring "
		     ...
		     "Cursor pagination."),
		FMegalithActionHandler::CreateStatic(&FModuleDepRealityAdapter::HandleAuditModuleDepReality),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("suggest_build_cs_deps"),
		...
		FMegalithActionHandler::CreateStatic(&FModuleDepRealityAdapter::HandleSuggestBuildCsDeps),
		FParamSchemaBuilder()
	...
	// We do NOT re-set the `source` dispatcher annotation here — MegalithSource
...
FMegalithSourceDatabase* FModuleDepRealityAdapter::GetSharedSourceDb()
{
	...
	UMegalithSourceSubsystem* SourceSS = GEditor->GetEditorSubsystem<UMegalithSourceSubsystem>();
...
FMegalithActionResult FModuleDepRealityAdapter::HandleAuditModuleDepReality(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FModuleDepRealityAdapter::HandleSuggestBuildCsDeps(const TSharedPtr<FJsonObject>& Params)
{
	ensure(IsInGameThread());

	// Resolve the FMegalithSourceDatabase wrapper (NOT the raw handle yet) so we can
	// hold its lock for the duration of the borrow per the GetRawHandle contract
	// (MegalithSourceDatabase.h:116-120). The raw handle is fetched under the lock
	// in Stage 3, where the only statement prepare/step/finalize happens.
	FMegalithSourceDatabase* SharedDb = GetSharedSourceDb();
	...
	{
		return FMegalithActionResult::Error(TEXT("Provide `file_path` and/or a non-empty `symbols` array."));
	}
	...
	//
	// BORROW CONTRACT (MegalithSourceDatabase.h:116-120): the raw handle is not
