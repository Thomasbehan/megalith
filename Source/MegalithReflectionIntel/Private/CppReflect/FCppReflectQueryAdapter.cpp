...
// into MegalithCore is a Phase 5+ item and out of scope.
...
namespace
{
	 ...
	 * A/U/I prefix — verified in MegalithCppParser.cpp:170 where Kind is set to
	 ...
	 * ConvertRelativePathToFull module root (verified MegalithSourceIndexer.cpp
	 * IndexModule/IndexCppFile + MegalithSourceSubsystem.cpp:282-292). A direct
...
void FCppReflectQueryAdapter::RegisterActions(FMegalithToolRegistry& Registry)
{
	// ---- get_uclass ----
	Registry.RegisterAction(TEXT("cppreflect"), TEXT("get_uclass"),
		...
		FMegalithActionHandler::CreateStatic(&FCppReflectQueryAdapter::HandleGetUClass),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("cppreflect"), TEXT("list_uproperties"),
		...
		FMegalithActionHandler::CreateStatic(&FCppReflectQueryAdapter::HandleListUProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("cppreflect"), TEXT("list_ufunctions"),
		TEXT("List UFUNCTION rows. Filter by `class_name` or restrict to "
		     ...
		     "Cursor pagination."),
		FMegalithActionHandler::CreateStatic(&FCppReflectQueryAdapter::HandleListUFunctions),
		FParamSchemaBuilder()
			.Optional(TEXT("class_name"), TEXT("string"),
				TEXT("Filter by owning_class (exact match)"))
	...
	Registry.RegisterAction(TEXT("cppreflect"), TEXT("find_interface_impls"),
		...
		FMegalithActionHandler::CreateStatic(&FCppReflectQueryAdapter::HandleFindInterfaceImpls),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("cppreflect"), TEXT("find_class_specifier"),
		TEXT("Find UCLASS rows whose `flags` column contains the given token. "
		     ...
		     "Cursor pagination."),
		FMegalithActionHandler::CreateStatic(&FCppReflectQueryAdapter::HandleFindClassSpecifier),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("cppreflect"), TEXT("list_class_specifiers"),
		...
		FMegalithActionHandler::CreateStatic(&FCppReflectQueryAdapter::HandleListClassSpecifiers),
...
FSQLiteDatabase* FCppReflectQueryAdapter::GetRawDB()
{
	...
	if (!Module->HasAttemptedCppReflectBootstrap())
	{
		...
		if (bTableExists)
		{
			...
			const int32 CurrentVersion = MegalithRIMeta::GetIndexerCodeVersion(TEXT("cppreflect"));
		...
		if (!bTableExists || bVersionMismatch)
		{
			...
			FMegalithReflectionIntelModule::RunCppReflectIndexersOnce(IndexerStatus);
...
FMegalithActionResult FCppReflectQueryAdapter::HandleGetUClass(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("`class_name` is required."),
	...
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (reflect_uclasses absent?)."));
	}
	int32 BindIdx = 1;
...
FMegalithActionResult FCppReflectQueryAdapter::HandleListUProperties(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (reflect_uproperties absent?)."));
	}
	int32 BindIdx = 1;
...
FMegalithActionResult FCppReflectQueryAdapter::HandleListUFunctions(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
	...
	if (!Stmt.Create(*DB, *Sql))
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (reflect_ufunctions absent?)."));
	}
	int32 BindIdx = 1;
	if (!ClassName.IsEmpty()) { Stmt.SetBindingValueByIndex(BindIdx++, ClassName); }
	Stmt.SetBindingValueByIndex(BindIdx++, Limit);
...
FMegalithActionResult FCppReflectQueryAdapter::HandleFindInterfaceImpls(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("`interface_name` is required."),
			FMegalithJsonUtils::ErrInvalidParams);
	}
...
FMegalithActionResult FCppReflectQueryAdapter::HandleFindClassSpecifier(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("`specifier_name` is required."),
			FMegalithJsonUtils::ErrInvalidParams);
	}
...
FMegalithActionResult FCppReflectQueryAdapter::HandleListClassSpecifiers(const TSharedPtr<FJsonObject>& /*Params*/)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
