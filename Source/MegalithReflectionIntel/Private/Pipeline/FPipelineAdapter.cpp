...
// out to multiple namespaces' actions through FMegalithToolRegistry::ExecuteAction
...
void FPipelineAdapter::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("pipeline"), TEXT("pr_review"),
		...
		FMegalithActionHandler::CreateStatic(&FPipelineAdapter::HandlePRReview),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("pipeline"), TEXT("release_readiness"),
		TEXT("Composer: release pre-flight envelope. Composes:\n"
		     "  • megalith_status — current action count + module status\n"
		...
		FMegalithActionHandler::CreateStatic(&FPipelineAdapter::HandleReleaseReadiness),
		FParamSchemaBuilder()
...
FMegalithActionResult FPipelineAdapter::HandlePRReview(const TSharedPtr<FJsonObject>& Params)
{
	...
	return FMegalithActionResult::Success(Envelope);
}
...
FMegalithActionResult FPipelineAdapter::HandleReleaseReadiness(const TSharedPtr<FJsonObject>& Params)
{
	...
	// megalith_status — server health, version, module status.
	{
		TSharedPtr<FJsonObject> P = MakeShared<FJsonObject>();
		Envelope->SetObjectField(TEXT("megalith_status"),
			CallSection(TEXT("megalith"), TEXT("status"), P));
	}
