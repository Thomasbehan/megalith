...
//
// MegalithRIMetaTable — implementation. See header for the stale-detection
// contract. Plain SQLite ops; caller is expected to hold the shared DB lock
// (the RI module's borrow path always holds the FMegalithSourceDatabase lock
...
namespace MegalithRIMeta
{
	int32 GetIndexerCodeVersion(const FString& Subsystem)
	{
		...
		// project-relative (`Plugins/Megalith/Source/...`), so the two maps never
	...
	int32 ComputeRiskConfigFingerprint(const TArray<FString>& ResolvedGitRepoRoots)
	{
		...
		if (const UMegalithReflectionIntelSettings* Settings = UMegalithReflectionIntelSettings::Get())
		{
	...
	bool EnsureMetaTable(FSQLiteDatabase& DB)
	{
		...
		if (!Stmt.Create(DB, TEXT(
			"CREATE TABLE IF NOT EXISTS megalith_ri_meta ("
			...
			");")))
		{
			UE_LOG(LogMegalithReflectionIntel, Warning,
				TEXT("MegalithRIMeta: CREATE TABLE prepare failed for megalith_ri_meta"));
			return false;
		}
		return Stmt.Execute();
	}
	...
	bool ReadStoredVersion(FSQLiteDatabase& DB, const FString& Subsystem, int32& OutVersion)
	{
		...
		FSQLitePreparedStatement Stmt;
		if (!Stmt.Create(DB, TEXT(
			"SELECT code_version FROM megalith_ri_meta WHERE subsystem = ? LIMIT 1;")))
		{
			return false;
		}
	...
	bool WriteStoredVersion(FSQLiteDatabase& DB, const FString& Subsystem, int32 Version)
	{
		...
		FSQLitePreparedStatement Stmt;
		if (!Stmt.Create(DB, TEXT(
			"INSERT OR REPLACE INTO megalith_ri_meta "
			"(subsystem, code_version, stamped_at) VALUES (?, ?, ?);")))
		{
			UE_LOG(LogMegalithReflectionIntel, Warning,
				TEXT("MegalithRIMeta: UPSERT prepare failed for subsystem '%s'"), *Subsystem);
			return false;
		}
		Stmt.SetBindingValueByIndex(1, Subsystem);
