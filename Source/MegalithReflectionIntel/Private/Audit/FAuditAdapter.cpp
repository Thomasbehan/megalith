...
// Consolidation into MegalithCore is Phase 5+ work.
...
void FAuditAdapter::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("material"), TEXT("audit_orphan_materials"),
		...
		FMegalithActionHandler::CreateStatic(&FAuditAdapter::HandleAuditOrphanMaterials),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("audit_cross_asset_refs"),
		...
		FMegalithActionHandler::CreateStatic(&FAuditAdapter::HandleAuditNiagaraCrossAssetRefs),
		FParamSchemaBuilder()
			.Optional(TEXT("limit"), TEXT("integer"),
				TEXT("Max rows per page (default 50, hard cap 200)"), TEXT("50"))
			.Optional(TEXT("cursor"), TEXT("string"),
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("audit_cdo_drift"),
		TEXT("Audit: surface Blueprint child CDOs whose property defaults differ "
		     ...
		     "Read-only. Cursor pagination."),
		FMegalithActionHandler::CreateStatic(&FAuditAdapter::HandleAuditCdoDrift),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("project"), TEXT("audit_orphan_assets"),
		...
		FMegalithActionHandler::CreateStatic(&FAuditAdapter::HandleAuditOrphanAssets),
		FParamSchemaBuilder()
	...
	// primary host modules (MegalithMaterial / MegalithNiagara / MegalithBlueprint
	// / MegalithIndex). Adding an action preserves existing annotations because
...
FMegalithActionResult FAuditAdapter::HandleAuditOrphanMaterials(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("IAssetRegistry::GetAllAssets failed"));
	}
...
FMegalithActionResult FAuditAdapter::HandleAuditNiagaraCrossAssetRefs(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (cpp_asset_edges absent?)."));
	}
...
FMegalithActionResult FAuditAdapter::HandleAuditCdoDrift(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FAuditAdapter::HandleAuditOrphanAssets(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
