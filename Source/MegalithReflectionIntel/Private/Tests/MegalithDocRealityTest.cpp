...
// registry (`FMegalithToolRegistry::Get().GetActions(namespace)` ->
// `FMegalithActionInfo::ParamSchema`). The ParamSchema is a flat JSON object of
//   param_name -> { "type": <string>, "required": <bool>, "kind"?: <string>, ... }
// exactly the shape `describe_query("action_schema")` returns as "params"
// (see MegalithBulkFillActions.cpp HandleDescribeActionSchema ~line 226). The
// `kind` field is emitted ONLY when non-default (Other) — see
// MegalithParamSchema.h AddParam (`SetStringField(TEXT("kind"), ...)`), and the
// dispatcher reads it back the same way at MegalithToolRegistry.cpp ~line 384.
//
// HOW TO EXTEND
// -------------
...
namespace MegalithDocRealityTestDetail
{
	...
	static const FMegalithActionInfo* FindAction(const FString& Namespace, const FString& Action,
		TArray<FMegalithActionInfo>& OutActionsHolder)
	{
		OutActionsHolder = FMegalithToolRegistry::Get().GetActions(Namespace);
		return OutActionsHolder.FindByPredicate(
			[&Action](const FMegalithActionInfo& Info){ return Info.Action == Action; });
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDocRealityDriftTest,
	"Megalith.ReflectionIntel.DocRealityDrift",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDocRealityDriftTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDocRealityTestDetail;
	...
	for (const FDocClaim& Claim : Claims)
	{
		...
		TArray<FMegalithActionInfo> ActionsHolder;
		const FMegalithActionInfo* Found = FindAction(Namespace, Action, ActionsHolder);
