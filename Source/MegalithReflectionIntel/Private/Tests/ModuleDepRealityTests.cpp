...
// MegalithSource's exact schema. Phase 2 §12 test 5 explicitly asks for a
...
namespace MegalithModDepTestDetail
{
	static FString GetFixturePath(const FString& Sub)
	{
		...
		if (Plugin.IsValid())
		{
			return Plugin->GetBaseDir()
				/ TEXT("Source") / TEXT("MegalithReflectionIntel")
				/ TEXT("Private") / TEXT("Tests") / TEXT("Fixtures") / Sub;
		}
		return FPaths::ProjectPluginsDir()
			/ TEXT("Megalith") / TEXT("Source") / TEXT("MegalithReflectionIntel")
			/ TEXT("Private") / TEXT("Tests") / TEXT("Fixtures") / Sub;
	}
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.ReflectionIntel.SourceAudit.SampleBuildCsParse",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FModuleDepRealitySampleBuildCsParseTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithModDepTestDetail;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.ReflectionIntel.SourceAudit.Registration",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FModuleDepRealityRegistrationTest::RunTest(const FString& /*Parameters*/)
{
	const FMegalithToolRegistry& Reg = FMegalithToolRegistry::Get();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.ReflectionIntel.SourceAudit.SuggestBuildCsDepsForward",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.ReflectionIntel.Risk.QueryRegistration",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
