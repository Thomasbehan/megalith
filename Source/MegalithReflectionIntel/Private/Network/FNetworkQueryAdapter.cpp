...
// MegalithCore is a Phase 5+ item and explicitly out of Phase 4a scope.
...
void FNetworkQueryAdapter::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("network"), TEXT("list_replicated_classes"),
		...
		FMegalithActionHandler::CreateStatic(&FNetworkQueryAdapter::HandleListReplicatedClasses),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("network"), TEXT("list_rpc_functions"),
		...
		FMegalithActionHandler::CreateStatic(&FNetworkQueryAdapter::HandleListRPCFunctions),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("network"), TEXT("list_onrep_handlers"),
		...
		FMegalithActionHandler::CreateStatic(&FNetworkQueryAdapter::HandleListOnRepHandlers),
		FParamSchemaBuilder()
			.Optional(TEXT("class_name"), TEXT("string"),
				TEXT("Filter by owning_class (exact match)"))
			.Optional(TEXT("limit"), TEXT("integer"),
	...
	Registry.RegisterAction(TEXT("network"), TEXT("audit_unbalanced_onreps"),
		TEXT("Audit: surface UPROPERTY(ReplicatedUsing=OnRep_Foo) declarations "
		     ...
		     "Cursor pagination."),
		FMegalithActionHandler::CreateStatic(&FNetworkQueryAdapter::HandleAuditUnbalancedOnReps),
		FParamSchemaBuilder()
			.Optional(TEXT("limit"), TEXT("integer"),
				TEXT("Max rows per page (default 50, hard cap 200)"), TEXT("50"))
			.Optional(TEXT("cursor"), TEXT("string"),
...
FSQLiteDatabase* FNetworkQueryAdapter::GetRawDB()
{
	...
	if (!Module->HasAttemptedNetworkBootstrap())
	{
		...
		if (bTableExists)
		{
			...
			const int32 CurrentVersion = MegalithRIMeta::GetIndexerCodeVersion(TEXT("network"));
		...
		if (!bTableExists || bVersionMismatch)
		{
			...
			FMegalithReflectionIntelModule::RunNetworkIndexerOnce(IndexerStatus);
...
FMegalithActionResult FNetworkQueryAdapter::HandleListReplicatedClasses(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (reflect_replicated_properties absent?)."));
	}
...
FMegalithActionResult FNetworkQueryAdapter::HandleListRPCFunctions(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
...
FMegalithActionResult FNetworkQueryAdapter::HandleListOnRepHandlers(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
...
FMegalithActionResult FNetworkQueryAdapter::HandleAuditUnbalancedOnReps(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB)
	{
