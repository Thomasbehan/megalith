...
// straight from MEGALITH_GUIDE.md Phase 1 § Heuristic Rules:
...
//     MegalithIndex/Private/Indexers/MeshCatalogIndexer.cpp:36-43. We use the
...
//     MegalithCore/Private/MegalithCoreModule.cpp:141 (same Megalith codebase
//     already uses this pattern).
//   - FFileHelper::LoadFileToString: existing pattern across MegalithIndex.
//   - FRegexPattern/FRegexMatcher: VERIFIED at
//     MegalithSource/Private/MegalithSourceActions.cpp:196-198.
	...
	FString& OutStatus)
{
	...
	MegalithRIMeta::WriteStoredVersion(DB, TEXT("decision"),
...
bool FDecisionRecordIndexer::EnsureSchema(FSQLiteDatabase& DB)
{
	...
	if (!Exec(MegalithDecisionSchema::GetCreateRecordsTableSQL())) { return false; }
	if (!Exec(MegalithDecisionSchema::GetCreateSupersedesTableSQL())) { return false; }
	// Indices are nice-to-have; failure here is non-fatal but unlikely.
	Exec(MegalithDecisionSchema::GetCreateRecordsIndexStatusSQL());
	Exec(MegalithDecisionSchema::GetCreateRecordsIndexPathSQL());
	Exec(MegalithDecisionSchema::GetCreateSupersedesIndexToSQL());
	return true;
}
