...
//
// DEVIATION (vs plan §0 mention of FMegalithSourceDatabase shared handle):
// The plan implies a `GetRawDatabase()` accessor on FMegalithSourceDatabase, but
...
namespace
{
	...
	// Resolve the EngineSource.db path the same way UMegalithSourceSubsystem does.
	FString ResolveEngineSourceDbPath()
	{
		...
		return FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("EngineSource.db");
	}
}
...
void FDecisionQueryAdapter::RegisterActions(FMegalithToolRegistry& Registry)
{
	// ---- list_decisions ----
	Registry.RegisterAction(TEXT("decision"), TEXT("list_decisions"),
		...
		FMegalithActionHandler::CreateStatic(&FDecisionQueryAdapter::HandleListDecisions),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("decision"), TEXT("get_decision"),
		...
		FMegalithActionHandler::CreateStatic(&FDecisionQueryAdapter::HandleGetDecision),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("decision"), TEXT("list_stale"),
		...
		FMegalithActionHandler::CreateStatic(&FDecisionQueryAdapter::HandleListStale),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("decision"), TEXT("find_supersession_chain"),
		...
		FMegalithActionHandler::CreateStatic(&FDecisionQueryAdapter::HandleFindSupersessionChain),
		FParamSchemaBuilder()
			.Required(TEXT("decision_id"), TEXT("string"),
	...
	Registry.RegisterAction(TEXT("decision"), TEXT("find_referent_decisions"),
		...
		FMegalithActionHandler::CreateStatic(&FDecisionQueryAdapter::HandleFindReferentDecisions),
		FParamSchemaBuilder()
			.Required(TEXT("decision_id"), TEXT("string"),
...
FSQLiteDatabase* FDecisionQueryAdapter::GetRawDB()
{
	...
	// UMegalithSourceSubsystem's already-open handle via GetOrOpenCachedQueryDb;
	...
	// FMegalithReflectionIntelModule::RunDecisionIndexerOnce to run the indexer.
	...
	if (!Module->HasAttemptedBootstrap())
	{
		...
		if (bTableExists)
		{
			...
			const int32 CurrentVersion = MegalithRIMeta::GetIndexerCodeVersion(TEXT("decision"));
		...
		if (!bTableExists || bVersionMismatch)
		{
			...
			FMegalithReflectionIntelModule::RunDecisionIndexerOnce(IndexerStatus);
...
FMegalithActionResult FDecisionQueryAdapter::HandleListDecisions(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (decision_records absent?)."));
	}
...
FMegalithActionResult FDecisionQueryAdapter::HandleGetDecision(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	...
	{
		return FMegalithActionResult::Error(TEXT("`decision_id` is required."),
	...
	if (Stmt.Step() != ESQLitePreparedStatementStepResult::Row)
	{
		...
		return FMegalithActionResult::Success(Empty);
	}
...
FMegalithActionResult FDecisionQueryAdapter::HandleListStale(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB) { return FMegalithActionResult::Error(TEXT("EngineSource.db not available.")); }
	...
	if (MaxAgeDays <= 0)
	{
		return FMegalithActionResult::Error(TEXT("`max_age_days` must be positive."),
			FMegalithJsonUtils::ErrInvalidParams);
	}
...
FMegalithActionResult FDecisionQueryAdapter::HandleFindSupersessionChain(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB) { return FMegalithActionResult::Error(TEXT("EngineSource.db not available.")); }
...
FMegalithActionResult FDecisionQueryAdapter::HandleFindReferentDecisions(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB) { return FMegalithActionResult::Error(TEXT("EngineSource.db not available.")); }
