...
namespace
{
		...
		FString& OutErr)
	{
		...
		while (FPlatformProcess::IsProcRunning(Proc))
		{
			...
			// because git log on the Megalith plugin completes in well under
			// 1s in normal use.
		}
	...
	FString& OutStatus)
{
	...
	if (!EnsureSchema(DB))
	{
		...
		UE_LOG(LogMegalithReflectionIntel, Error, TEXT("%s"), *OutStatus);
		return false;
	}
	...
	MegalithRIMeta::EnsureMetaTable(DB);
	...
	// Timeout — keep modest. The Megalith plugin's git log emits ~MB in <500ms
	...
	for (const FString& RawRoot : GitRepoRoots)
	{
		...
		{
			UE_LOG(LogMegalithReflectionIntel, Verbose,
	...
	{
		OutStatus += TEXT(" | ");
		OutStatus += FMegalithReflectionIntelModule::GetRiskNoReposHint();
		UE_LOG(LogMegalithReflectionIntel, Warning, TEXT("%s"), *OutStatus);
	}
	else
	{
	...
	MegalithRIMeta::WriteStoredVersion(DB, TEXT("risk"),
	...
	MegalithRIMeta::WriteStoredVersion(DB, MegalithRIMeta::GetRiskConfigSubsystemKey(),
		MegalithRIMeta::ComputeRiskConfigFingerprint(GitRepoRoots));
	return true;
}
	...
	FString& OutPrefixToStrip)
{
	...
	// <Project>/Plugins/Megalith turns `Source/X.cpp` into
	// `Plugins/Megalith/Source/X.cpp`.
...
bool FGitCoChangeIndexer::EnsureSchema(FSQLiteDatabase& DB)
{
	auto Exec = [&DB](const TCHAR* Sql) -> bool
	{
		...
		{
			UE_LOG(LogMegalithReflectionIntel, Error,
	...
	if (!Exec(MegalithRiskSchema::GetCreateCoChangePairsTableSQL())) { return false; }
	if (!Exec(MegalithRiskSchema::GetCreateFileChurnTableSQL())) { return false; }
	// Indices are nice-to-have; failure non-fatal.
	Exec(MegalithRiskSchema::GetCreateCoChangePairsIndexFileASQL());
	Exec(MegalithRiskSchema::GetCreateCoChangePairsIndexFileBSQL());
	Exec(MegalithRiskSchema::GetCreateFileChurnIndexPathSQL());
	return true;
}
	...
	const TMap<FString, int64>& LastTouched)
{
	...
	// can hit ~10K on the Megalith repo per plan §3, where transaction-less
