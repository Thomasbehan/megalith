...
// Plan: Plugins/Megalith/Docs/plans/2026-05-28-reflection-intelligence.md (Phase 2 — v0.17.0).
//
...
// Phase 1 decision adapter; consolidating into MegalithCore is a Phase 5+
...
namespace
{
	...
	{
		const FMegalithReflectionIntelModule* Module =
		...
		OutScanned = FMegalithReflectionIntelModule::ResolveGitRepoRoots(&OutSkipped);
	}
		...
		const TArray<TPair<FString, FString>>& Skipped)
	{
		...
		{
			Diag->SetStringField(TEXT("hint"), FMegalithReflectionIntelModule::GetRiskNoReposHint());
		}
		return Diag;
	}
...
void FRiskQueryAdapter::RegisterActions(FMegalithToolRegistry& Registry)
{
	// ---- get_hotspot_score ----
	Registry.RegisterAction(TEXT("risk"), TEXT("get_hotspot_score"),
		...
		FMegalithActionHandler::CreateStatic(&FRiskQueryAdapter::HandleGetHotspotScore),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("risk"), TEXT("get_cochange_pairs"),
		...
		FMegalithActionHandler::CreateStatic(&FRiskQueryAdapter::HandleGetCoChangePairs),
		FParamSchemaBuilder()
			.RequiredDiskPath(TEXT("file_path"),
				TEXT("Project-relative source file path"))
	...
	Registry.RegisterAction(TEXT("risk"), TEXT("get_file_churn"),
		TEXT("Report per-repo commit-touch count + last-touched Unix timestamp "
		     "for `file_path`."),
		FMegalithActionHandler::CreateStatic(&FRiskQueryAdapter::HandleGetFileChurn),
		FParamSchemaBuilder()
			.RequiredDiskPath(TEXT("file_path"),
				TEXT("Project-relative source file path"))
			.Optional(TEXT("repo_tag"), TEXT("string"),
				TEXT("Optional repo tag filter — the mined repository's folder name "
				     "(e.g. \"Megalith\" for a repo at Plugins/Megalith)"))
			.Build());
	...
	Registry.RegisterAction(TEXT("risk"), TEXT("get_release_window_hotspots"),
		...
		FMegalithActionHandler::CreateStatic(&FRiskQueryAdapter::HandleGetReleaseWindowHotspots),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("risk"), TEXT("list_conditional_gates"),
		...
		FMegalithActionHandler::CreateStatic(&FRiskQueryAdapter::HandleListConditionalGates),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("risk"), TEXT("get_mining_status"),
		...
		FMegalithActionHandler::CreateStatic(&FRiskQueryAdapter::HandleGetMiningStatus),
...
FSQLiteDatabase* FRiskQueryAdapter::GetRawDB()
{
	...
	if (!Module->HasAttemptedRiskBootstrap())
	{
		...
		if (bTableExists)
		{
			...
			const bool bHasStamp = MegalithRIMeta::ReadStoredVersion(
				*DB, TEXT("risk"), StoredVersion);
			const int32 CurrentVersion = MegalithRIMeta::GetIndexerCodeVersion(TEXT("risk"));
			...
			if (!bVersionMismatch)
			{
				...
				const bool bHasConfig = MegalithRIMeta::ReadStoredVersion(
					*DB, MegalithRIMeta::GetRiskConfigSubsystemKey(), StoredConfig);
				const int32 CurrentConfig = MegalithRIMeta::ComputeRiskConfigFingerprint(
					FMegalithReflectionIntelModule::ResolveGitRepoRoots(nullptr));
		...
		if (!bTableExists || bVersionMismatch)
		{
			...
			FMegalithReflectionIntelModule::RunRiskIndexersOnce(IndexerStatus);
...
FMegalithActionResult FRiskQueryAdapter::HandleGetHotspotScore(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (risk_hotspot_scores absent?)."));
	}
...
FMegalithActionResult FRiskQueryAdapter::HandleGetCoChangePairs(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB) { return FMegalithActionResult::Error(TEXT("EngineSource.db not available.")); }
	...
	if (!Stmt.Create(*DB, TEXT(
		...
		"LIMIT ? OFFSET ?;")))
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (git_cochange_pairs absent?)."));
	}
	int32 BindIdx = 1;
...
FMegalithActionResult FRiskQueryAdapter::HandleGetFileChurn(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB) { return FMegalithActionResult::Error(TEXT("EngineSource.db not available.")); }
	...
	if (!Stmt.Create(*DB, *Sql))
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (git_file_churn absent?)."));
	}
	int32 BindIdx = 1;
...
FMegalithActionResult FRiskQueryAdapter::HandleGetReleaseWindowHotspots(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB) { return FMegalithActionResult::Error(TEXT("EngineSource.db not available.")); }
	...
	if (!Stmt.Create(*DB, TEXT(
		...
		"LIMIT ? OFFSET ?;")))
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed."));
	}
...
FMegalithActionResult FRiskQueryAdapter::HandleListConditionalGates(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = GetRawDB();
	if (!DB) { return FMegalithActionResult::Error(TEXT("EngineSource.db not available.")); }
	...
	if (!Stmt.Create(*DB, *Sql))
	{
		return FMegalithActionResult::Error(TEXT("SELECT prepare failed (reflect_conditional_gates absent?)."));
	}
	int32 BindIdx = 1;
...
FMegalithActionResult FRiskQueryAdapter::HandleGetMiningStatus(const TSharedPtr<FJsonObject>& /*Params*/)
{
	...
	if (DB)
	{
		...
		int32 StoredVersion = 0;
		const bool bHasVersion = MegalithRIMeta::ReadStoredVersion(*DB, TEXT("risk"), StoredVersion);
		int32 StoredConfig = 0;
		const bool bHasConfig = MegalithRIMeta::ReadStoredVersion(
			*DB, MegalithRIMeta::GetRiskConfigSubsystemKey(), StoredConfig);
		const int32 CurrentConfig = MegalithRIMeta::ComputeRiskConfigFingerprint(
