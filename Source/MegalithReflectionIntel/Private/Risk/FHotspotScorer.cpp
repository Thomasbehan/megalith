...
// from the existing `files` table populated by MegalithSourceSubsystem, 3)
// normalise both signals 0..1, 4) multiply for the composite score.
//
// The `files` table schema (per `MegalithSourceSchema.h:29-37`):
...
bool FHotspotScorer::EnsureSchema(FSQLiteDatabase& DB)
{
	...
	if (!Exec(MegalithRiskSchema::GetCreateHotspotScoresTableSQL())) { return false; }
	Exec(MegalithRiskSchema::GetCreateHotspotScoresIndexScoreSQL());
	return true;
}
