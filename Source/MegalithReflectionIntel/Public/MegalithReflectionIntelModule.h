...
DECLARE_LOG_CATEGORY_EXTERN(LogMegalithReflectionIntel, Log, All);

// UE 5.7: enum class declared without underlying type at UObject/UObjectGlobals.h:3216;
// forward-declare without an underlying type to match (see MegalithSourceSubsystem.h for precedent).
...
/**
 * MegalithReflectionIntel — Phase 1 of Reflection Intelligence (v0.17.0).
 *
 * Hosts the deterministic markdown decision-record indexer (FDecisionRecordIndexer)
 * and registers the `decision_query` namespace (5 actions) into FMegalithToolRegistry.
 *
 * The indexer writes to the EngineSource.db SQLite file (owned by
 * UMegalithSourceSubsystem). New tables `decision_records` and
 ...
 * BORROWS UMegalithSourceSubsystem's already-open FSQLiteDatabase handle (via
 * FMegalithSourceDatabase::GetRawHandle) instead of opening a second handle on
 ...
 */
class FMegalithReflectionIntelModule : public IModuleInterface
{
public:
	...
	/**
	 * Return the SHARED EngineSource.db handle owned by UMegalithSourceSubsystem,
	 ...
	 * Called from UMegalithReflectionIntelSettings::PostEditChangeProperty so a
...
private:
	...
	// own handle on EngineSource.db — it borrows UMegalithSourceSubsystem's
