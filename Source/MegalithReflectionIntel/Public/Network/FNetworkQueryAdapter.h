...
// the central FMegalithToolRegistry. Pure read-only handlers; no indexing
...
class MEGALITHREFLECTIONINTEL_API FNetworkQueryAdapter
{
public:
...
private:
	// --- Action handlers ---
	static FMegalithActionResult HandleListReplicatedClasses(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleListRPCFunctions(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleListOnRepHandlers(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleAuditUnbalancedOnReps(const TSharedPtr<FJsonObject>& Params);
