...
// FMegalithToolRegistry. Pure read-only handlers; no indexing happens here
...
//   - `file_path` / `path_filter` params tagged EMegalithParamKind::DiskPath.
...
class MEGALITHREFLECTIONINTEL_API FRiskQueryAdapter
{
public:
...
private:
	// Handlers
	static FMegalithActionResult HandleGetHotspotScore(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleGetCoChangePairs(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleGetFileChurn(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleGetReleaseWindowHotspots(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleListConditionalGates(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleGetMiningStatus(const TSharedPtr<FJsonObject>& Params);
