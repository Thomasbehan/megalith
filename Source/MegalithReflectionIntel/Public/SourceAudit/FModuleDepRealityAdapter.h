...
// `source` namespace (the same namespace MegalithSource owns), surfacing the
// module-dep-reality audit. Multi-module registration against one namespace is
// supported by FMegalithToolRegistry (registry keys are `namespace.action`),
// so this lives in MegalithReflectionIntel while extending source_query.
//
// Action surface:
...
//      MegalithSource `symbols` × `files` × `modules` join. Skip candidates
...
class MEGALITHREFLECTIONINTEL_API FModuleDepRealityAdapter
{
public:
...
private:
	static FMegalithActionResult HandleAuditModuleDepReality(const TSharedPtr<FJsonObject>& Params);
	 ...
	 */
	static FMegalithActionResult HandleSuggestBuildCsDeps(const TSharedPtr<FJsonObject>& Params);
	 ...
	 * statement use per the borrow contract (MegalithSourceDatabase.h:116-120).
	 ...
	 */
	static class FMegalithSourceDatabase* GetSharedSourceDb();
};
