...
// FMegalithToolRegistry. Multi-module registration against one namespace is
// supported by FMegalithToolRegistry (registry keys are `namespace.action`),
// so these live in MegalithReflectionIntel while extending the namespaces
// owned by sibling Megalith modules. Same pattern as Phase 2's
...
class MEGALITHREFLECTIONINTEL_API FAuditAdapter
{
public:
	 ...
	 *  Megalith modules that primarily populate the namespace. */
	static void RegisterActions(FMegalithToolRegistry& Registry);

private:
	// --- Action handlers ---
	static FMegalithActionResult HandleAuditOrphanMaterials(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleAuditNiagaraCrossAssetRefs(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleAuditCdoDrift(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleAuditOrphanAssets(const TSharedPtr<FJsonObject>& Params);
