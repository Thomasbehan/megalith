...
#include "MegalithReflectionIntelSettings.generated.h"

/**
 * Editor Preferences > Plugins > Megalith Reflection Intel.
 ...
 */
UCLASS(config=MegalithSettings, defaultconfig, meta=(DisplayName="Megalith Reflection Intel"))
class MEGALITHREFLECTIONINTEL_API UMegalithReflectionIntelSettings : public UDeveloperSettings
{
	GENERATED_BODY()

public:
	UMegalithReflectionIntelSettings();

	/** Cheap, allocation-free CDO accessor. */
	static const UMegalithReflectionIntelSettings* Get();
	 ...
	 * sensible defaults: Docs/, Plugins/Megalith/Docs/, .claude/rules/.
	 */
	UPROPERTY(EditAnywhere, config, Category="Decision")
	 ...
	 * write this machine's layout into every downstream DefaultMegalithSettings.ini.
	 */
	UPROPERTY(EditAnywhere, config, Category="Risk")
	 ...
	 * otherwise dominates Megalith's nested-git history.
	 */
	UPROPERTY(EditAnywhere, config, Category="Risk")
	 ...
	 * reflection footprint is large (~488K LOC); we mine project + Megalith
	 ...
	 *
	 * Tagged with v0.17.0 ergonomics EMegalithParamKind::DiskPath spirit —
