 ...
 * state (the writer-side actions in MegalithAudio use the builder subsystem
 ...
 * thread, and uses FMegalithMemoryHelper for batched GC + editor yielding.
 */
class FMetaSoundIndexer : public IMegalithIndexer
{
public:
...
private:
	 ...
	 */
	void IndexMetaSoundAsset(UObject* MetaSoundAsset, const FString& AssetClassLabel, FMegalithIndexDatabase& DB, int64 AssetId);
};
