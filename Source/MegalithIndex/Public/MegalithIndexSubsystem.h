...
#include "MegalithIndexSubsystem.generated.h"
...
/**
 * Editor subsystem that orchestrates the Megalith project index.
 * Owns the SQLite database, manages indexers, runs background indexing.
 */
UCLASS()
class MEGALITHINDEX_API UMegalithIndexSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()
...
public:
	 ...
	 * fresh one. This is what the automatic startup path and `Megalith.StartIndex`
	 * use: a crash mid-index must not cost the batches already committed.
	 *
	 * UFUNCTION because MegalithCore reaches MegalithIndex only through reflection,
	 * and `megalith_reindex` without `force` must be able to land here. Without it
	...
	FMegalithIndexDatabase* GetDatabase() { return Database.Get(); }
	...
	void RegisterIndexer(TSharedPtr<IMegalithIndexer> Indexer);
...
private:
	...
	{
	public:
		FIndexingTask(UMegalithIndexSubsystem* InOwner);
	...
	TUniquePtr<FMegalithIndexDatabase> Database;
	TArray<TSharedPtr<IMegalithIndexer>> Indexers;
	TMap<FString, TSharedPtr<IMegalithIndexer>> ClassToIndexer;
