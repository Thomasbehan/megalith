...
FMegalithActionResult FProjectSearchGameplayTagsAction::Execute(const TSharedPtr<FJsonObject>& Params)
{
	FString Query = Params->GetStringField(TEXT("query"));
	if (Query.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'query' parameter is required"), -32602);
	}
	...
	FMegalithIndexDatabase* DB = Subsystem->GetDatabase();
	if (!DB || !DB->IsOpen())
	{
		return FMegalithActionResult::Error(TEXT("Project index database not available"));
	}

	FSQLiteDatabase* RawDB = DB->GetRawDatabase();
	if (!RawDB)
	{
		return FMegalithActionResult::Error(TEXT("Raw database handle not available"));
	}
