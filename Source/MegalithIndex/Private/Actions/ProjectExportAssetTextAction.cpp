...
FMegalithActionResult FProjectExportAssetTextAction::Execute(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'asset_path' parameter is required"), -32602);
	}
	...
	if (Params->TryGetNumberField(TEXT("max_bytes"), MaxBytesRaw))
	{
		...
		if (MaxBytes <= 0)
		{
			return FMegalithActionResult::Error(TEXT("'max_bytes' must be a positive integer"), -32602);
		}
