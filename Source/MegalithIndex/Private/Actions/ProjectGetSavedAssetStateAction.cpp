...
FMegalithActionResult FProjectGetSavedAssetStateAction::Execute(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Asset '%s' not found in registry or on disk"), *PackagePath));
	}
...
TSharedPtr<FJsonObject> FProjectGetSavedAssetStateAction::GetSchema()
{
	return FParamSchemaBuilder()
		.Required(TEXT("asset_path"), TEXT("string"), TEXT("Package path of the asset (e.g. /Game/Tests/Megalith/Foo)"))
		.Build();
}
