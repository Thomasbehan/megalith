...
FMegalithActionResult FProjectRefreshAssetsAction::Execute(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* PathsArray = nullptr;
	if (!Params->TryGetArrayField(TEXT("asset_paths"), PathsArray) || !PathsArray || PathsArray->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("'asset_paths' array parameter is required (at least one /Game/... path)"), -32602);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("No valid /Game/... paths were supplied in 'asset_paths'"), -32602);
	}
...
TSharedPtr<FJsonObject> FProjectRefreshAssetsAction::GetSchema()
{
	return FParamSchemaBuilder()
		.Required(TEXT("asset_paths"), TEXT("array"), TEXT("Array of /Game/... package or directory paths to rescan (e.g. [\"/Game/Tests/Megalith/Foo\"])"))
