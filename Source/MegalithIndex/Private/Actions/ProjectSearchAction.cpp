...
namespace MegalithProjectSearchActionDetail
{
	/**
...
FMegalithActionResult FProjectSearchAction::Execute(const TSharedPtr<FJsonObject>& Params)
{
	FString Query;
	if (!Params.IsValid() || !Params->TryGetStringField(TEXT("query"), Query))
	{
		return FMegalithActionResult::Error(TEXT("'query' must be a string"), -32602);
	}

	Query.TrimStartAndEndInline();
	if (Query.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'query' must not be empty"), -32602);
	}
	if (Query.Len() > MegalithProjectSearchActionDetail::MaxQueryLength)
	{
	...
	if (Params->HasField(TEXT("limit")))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("'limit' must be an integer"), -32602);
		}
		Limit = static_cast<int32>(FMath::Clamp(RawLimit, 1.0, 1000.0));
	}

	UMegalithIndexSubsystem* Subsystem = GEditor
		? GEditor->GetEditorSubsystem<UMegalithIndexSubsystem>()
		: nullptr;
	if (!Subsystem)
	{
	...
	FMegalithIndexDatabase* Database = Subsystem->GetDatabase();
	if (!Database || !Database->IsOpen())
	{
		return FMegalithActionResult::Error(TEXT("Project index database is not open"));
	}

	TArray<FSearchResult> SearchResults;
	FString SearchError;
	const EMegalithProjectSearchStatus SearchStatus =
		Database->FullTextSearch(Query, Limit, SearchResults, SearchError);
	if (SearchStatus != EMegalithProjectSearchStatus::Succeeded)
	{
		const bool bInvalidQuery = SearchStatus == EMegalithProjectSearchStatus::InvalidQuery;
		return FMegalithActionResult::Error(
			FString::Printf(
				TEXT("%s: %s"),
