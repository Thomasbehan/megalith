...
namespace
{
	...
	const TCHAR* GTestsAllowlistPrefix = TEXT("/Game/Tests/Megalith/");
...
FMegalithActionResult FProjectCleanupGeneratedAssetsAction::Execute(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* PathsArray = nullptr;
	if (!Params->TryGetArrayField(TEXT("paths"), PathsArray) || !PathsArray || PathsArray->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("'paths' array parameter is required (at least one /Game/Tests/Megalith/... path)"), -32602);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("No valid paths supplied in 'paths'"), -32602);
	}
...
TSharedPtr<FJsonObject> FProjectCleanupGeneratedAssetsAction::GetSchema()
{
	return FParamSchemaBuilder()
		.Required(TEXT("paths"), TEXT("array"), TEXT("Array of /Game/Tests/Megalith/... asset package paths to delete (allowlist-enforced)"))
