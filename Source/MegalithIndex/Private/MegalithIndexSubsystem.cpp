...
#include "MegalithMemoryHelper.h"
#include "MegalithCompilerSafeDispatch.h"
...
//
// Bare `Megalith.StartIndex` RESUMES an interrupted index; `Megalith.StartIndex
...
static FAutoConsoleCommand GMegalithStartIndexCommand(
	TEXT("Megalith.StartIndex"),
	TEXT("Starts a Megalith project index, resuming an interrupted one if present. Pass 'force' to wipe and start over."),
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		if (!GEditor)
		{
			UE_LOG(LogMegalithIndex, Warning, TEXT("Megalith.StartIndex: GEditor not available — cannot start index"));
			return;
		}
		...
		if (!Subsystem)
		{
			UE_LOG(LogMegalithIndex, Warning, TEXT("Megalith.StartIndex: MegalithIndex subsystem not available — cannot start index"));
			return;
		}
		...
		UE_LOG(LogMegalithIndex, Log, TEXT("Megalith.StartIndex: manual %s requested"),
		...
		if (!bStarted)
		{
			UE_LOG(LogMegalithIndex, Warning, TEXT("Megalith.StartIndex: index did not start — see the preceding message"));
		}
	}));

void UMegalithIndexSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	Database = MakeUnique<FMegalithIndexDatabase>();
	FString DbPath = GetDatabasePath();

	if (!Database->Open(DbPath))
	{
		UE_LOG(LogMegalithIndex, Error, TEXT("Failed to open index database at %s"), *DbPath);
		return;
	}
	...
	if (!GetDefault<UMegalithSettings>()->bEnableIndex)
	{
		UE_LOG(LogMegalithIndex, Log, TEXT("MegalithIndex: indexing disabled via bEnableIndex=false; skipping index run"));
		return;
	}
	...
	if (ShouldAutoIndex())
	{
		...
		// empty until 'Megalith.StartIndex' is run manually (escape hatch for the
		...
		if (GetDefault<UMegalithSettings>()->bDeferFirstTimeIndex)
		{
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("%s"), bResumable
				? TEXT("MegalithIndex: an interrupted index is pending but deferred via bDeferFirstTimeIndex; run Megalith.StartIndex to resume it")
				: TEXT("MegalithIndex: first-time index deferred via bDeferFirstTimeIndex; run Megalith.StartIndex to begin"));
			return;
		}
		...
		if (AR.IsLoadingAssets())
			AR.OnFilesLoaded().AddUObject(this, &UMegalithIndexSubsystem::OnAssetRegistryFilesLoaded);
		else
			ResumeFullIndex();
	}
	else if (CanDoIncrementalIndex())
	{
		UE_LOG(LogMegalithIndex, Log, TEXT("Existing index found — deferring incremental catch-up until AR ready"));
		if (AR.IsLoadingAssets())
			AR.OnFilesLoaded().AddUObject(this, &UMegalithIndexSubsystem::OnAssetRegistryFilesLoadedIncremental);
		else
			StartIncrementalIndex();
	}
	else
	{
		UE_LOG(LogMegalithIndex, Log, TEXT("Schema v1 DB — forcing full reindex to populate hashes"));
		if (AR.IsLoadingAssets())
			AR.OnFilesLoaded().AddUObject(this, &UMegalithIndexSubsystem::OnAssetRegistryFilesLoaded);
		else
			StartFullIndex();
...
void UMegalithIndexSubsystem::OnAssetRegistryFilesLoaded()
{
	...
	if (ShouldAutoIndex())
	{
		UE_LOG(LogMegalithIndex, Log, TEXT("Asset Registry fully loaded -- starting full project index"));
		ResumeFullIndex();
	}
	else
	{
		UE_LOG(LogMegalithIndex, Log, TEXT("Asset Registry loaded but auto-index no longer needed (already indexed)"));
	}
}

void UMegalithIndexSubsystem::OnAssetRegistryFilesLoadedIncremental()
{
...
void UMegalithIndexSubsystem::Deinitialize()
{
	...
	if (IndexingTaskPtr.IsValid())
	{
		if (bIsIndexing)
		{
			UE_LOG(LogMegalithIndex, Warning, TEXT("Indexing was still in progress during shutdown — force-stopped"));
		}
...
void UMegalithIndexSubsystem::RegisterIndexer(TSharedPtr<IMegalithIndexer> Indexer)
{
	...
	UE_LOG(LogMegalithIndex, Verbose, TEXT("Registered indexer: %s (%d classes)"),
		*Indexer->GetName(), Indexer->GetSupportedClasses().Num());
}

void UMegalithIndexSubsystem::RegisterDefaultIndexers()
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Registered %d indexers"), Indexers.Num());
}

bool UMegalithIndexSubsystem::CanAcceptIndexRequest() const
{
	return !bIsIndexing && Database.IsValid() && Database->IsOpen();
}

bool UMegalithIndexSubsystem::StartFullIndex()
{
	return StartFullIndexInternal(/*bForceReset=*/true);
}

bool UMegalithIndexSubsystem::ResumeFullIndex()
{
	return StartFullIndexInternal(/*bForceReset=*/false);
}

bool UMegalithIndexSubsystem::StartFullIndexInternal(bool bForceReset)
{
	if (bIsIndexing)
	{
		UE_LOG(LogMegalithIndex, Warning, TEXT("Indexing already in progress"));
		return false;
	}

	if (!Database.IsValid() || !Database->IsOpen())
	{
		UE_LOG(LogMegalithIndex, Warning, TEXT("Index database is not open — cannot start a full index"));
		return false;
	}
	...
	else
	{
		...
		{
			UE_LOG(LogMegalithIndex, Error, TEXT("Failed to reset the index database — aborting the full index"));
	...
	NotifConfig.TitleText = FText::FromString(TEXT("Megalith"));
	NotifConfig.ProgressText = FText::FromString(TEXT("Indexing project..."));
	NotifConfig.bCanCancel = true;
	NotifConfig.LogCategory = &LogMegalithIndex;
	...
	if (!IndexingThread)
	{
		...
		UE_LOG(LogMegalithIndex, Error, TEXT("Failed to create the indexing thread — no index is running"));
		IndexingTaskPtr.Reset();
		OnIndexingFinished(false);
		return false;
	}

	UE_LOG(LogMegalithIndex, Log, TEXT("Background indexing started"));
	return true;
}

float UMegalithIndexSubsystem::GetProgress() const
{
...
TArray<FSearchResult> UMegalithIndexSubsystem::Search(const FString& Query, int32 Limit)
{
	if (!Database.IsValid() || !Database->IsOpen()) return {};
	return Database->FullTextSearch(Query, Limit);
}

TSharedPtr<FJsonObject> UMegalithIndexSubsystem::FindReferences(const FString& PackagePath)
{
	if (!Database.IsValid() || !Database->IsOpen()) return nullptr;
	return Database->FindReferences(PackagePath);
}

TArray<FIndexedAsset> UMegalithIndexSubsystem::FindByType(const FString& AssetClass, int32 Limit, int32 Offset)
{
	if (!Database.IsValid() || !Database->IsOpen()) return {};
	return Database->FindByType(AssetClass, Limit, Offset);
}

TSharedPtr<FJsonObject> UMegalithIndexSubsystem::GetStats()
{
	if (!Database.IsValid() || !Database->IsOpen()) return nullptr;
	return Database->GetStats();
}

TSharedPtr<FJsonObject> UMegalithIndexSubsystem::GetAssetDetails(const FString& PackagePath)
{
	if (!Database.IsValid() || !Database->IsOpen()) return nullptr;
	return Database->GetAssetDetails(PackagePath);
}

TArray<FIndexedPluginInfo> UMegalithIndexSubsystem::GatherMarketplacePluginPaths() const
{
    ...
    for (const TSharedRef<IPlugin>& Plugin : ContentPlugins)
    {
        ...
        UE_LOG(LogMegalithIndex, Log, TEXT("Marketplace plugin found: %s (mount: %s)"),
            *Info.FriendlyName, *Info.MountPath);

        Result.Add(MoveTemp(Info));
    }

    UE_LOG(LogMegalithIndex, Log, TEXT("Found %d marketplace plugins to index"), Result.Num());
    return Result;
}
...
UMegalithIndexSubsystem::FIndexingTask::FIndexingTask(UMegalithIndexSubsystem* InOwner)
	: Owner(InOwner)
{
}

uint32 UMegalithIndexSubsystem::FIndexingTask::Run()
{
	const UMegalithSettings* GlobalSettings = GetDefault<UMegalithSettings>();
	...
	const TWeakObjectPtr<UMegalithIndexSubsystem> WeakOwner(Owner);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("Full index starting"));
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Indexing %d assets..."), TotalAssets.Load());

	FMegalithIndexDatabase* DB = Owner->Database.Get();
	if (!DB || !DB->IsOpen())
	{
		AsyncTask(ENamedThreads::GameThread, [WeakOwner]()
		{
			if (UMegalithIndexSubsystem* Subsystem = WeakOwner.Get())
			{
	...
	auto RequireTransaction = [&bTransactionFailure](bool bOk, const TCHAR* What) -> bool
	{
		if (!bOk)
		{
			...
			UE_LOG(LogMegalithIndex, Error, TEXT("Index transaction failure (%s) — this run will not be marked complete"), What);
		}
		return bOk;
	};
	...
	struct FDeepIndexEntry
	{
		...
		int64 AssetId;
		TSharedPtr<IMegalithIndexer> Indexer;
		FString SavedHash;
	};
	...
	for (int32 i = 0; i < AllAssets.Num(); ++i)
	{
		...
		TSharedPtr<IMegalithIndexer>* FoundIndexer = Owner->ClassToIndexer.Find(IndexedAsset.AssetClass);
		if (FoundIndexer && FoundIndexer->IsValid())
		{
			...
			const EMegalithDeepIndexQueueDecision Decision =
				MegalithDecideDeepIndexQueueEntry(StoredDeepHash, StoredAttempts, IndexedAsset.SavedHash);

			switch (Decision)
			{
			case EMegalithDeepIndexQueueDecision::SkipAlreadyIndexed:
				AlreadyDeepIndexed++;
				break;

			case EMegalithDeepIndexQueueDecision::SkipPoisonAsset:
			...
			case EMegalithDeepIndexQueueDecision::Queue:
			default:
		...
		if (Indexed % BatchSize == 0)
		{
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Indexed %d / %d assets (%d errors)"),
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Asset class distribution (top 20):"));
	ClassDistribution.ValueSort([](int32 A, int32 B) { return A > B; });
	int32 Shown = 0;
	for (const auto& Pair : ClassDistribution)
	{
		if (Shown++ >= 20) break;
		UE_LOG(LogMegalithIndex, Log, TEXT("  %s: %d"), *Pair.Key, Pair.Value);
	}

	UE_LOG(LogMegalithIndex, Log, TEXT("Deep index queue: %d assets across %d classes"),
		DeepIndexQueue.Num(), QueuedClassDistribution.Num());
	for (const auto& Pair : QueuedClassDistribution)
	{
		UE_LOG(LogMegalithIndex, Log, TEXT("  Queued %s: %d"), *Pair.Key, Pair.Value);
	}

	RequireTransaction(DB->CommitTransaction(), TEXT("metadata pass commit"));

	UE_LOG(LogMegalithIndex, Log, TEXT("Metadata pass complete: %d assets indexed, %d errors"), Indexed, Errors);
	...
	if (PoisonedPaths.Num() > 0)
	{
		...
		{
			UE_LOG(LogMegalithIndex, Error,
				TEXT("Deep indexing skipped '%s': %d interrupted attempts. Its graph/variable data will be missing. Run 'Megalith.StartIndex force' (or megalith_reindex force=true) to clear the counters and retry."),
				*Path, MegalithMaxDeepIndexAttempts);
		}
	...
	{
		const UMegalithSettings* Settings = GetDefault<UMegalithSettings>();
		FMegalithMemoryHelper::LogTierStartupOnce();
		const int32 DeepBatchSize = FMath::Max(1, FMegalithMemoryHelper::GetResolvedDeepIndexBatchSize());
		const int32 GCFrequency = FMath::Max(1, Settings->GCFrequencyBatches);
		const SIZE_T MemoryBudgetMB = static_cast<SIZE_T>(FMegalithMemoryHelper::GetResolvedMemoryBudgetMB());
		const float YieldTime = Settings->YieldTimeSeconds;

		UE_LOG(LogMegalithIndex, Log, TEXT("Starting deep indexing pass for %d assets (batch size: %d, GC every %d batches, memory budget: %llu MB)..."),
			DeepIndexQueue.Num(), DeepBatchSize, GCFrequency, MemoryBudgetMB);

		if (bLogMemory)
		{
			FMegalithMemoryHelper::LogMemoryStats(TEXT("Deep index start"));
		}
		...
		for (int32 BatchStart = 0; BatchStart < TotalDeep && !bShouldStop; BatchStart += DeepBatchSize)
		{
			...
			if (FMegalithMemoryHelper::ShouldThrottle(MemoryBudgetMB))
			{
				UE_LOG(LogMegalithIndex, Log, TEXT("Memory budget exceeded, forcing GC and yielding..."));
				
				FEvent* GCEvent = FPlatformProcess::GetSynchEventFromPool(true);
				AsyncTask(ENamedThreads::GameThread, [GCEvent, YieldTime]()
				{
					FMegalithMemoryHelper::ForceGarbageCollection(true);
					FMegalithMemoryHelper::YieldToEditor();
					if (YieldTime > 0.0f)
					{
				...
				if (bLogMemory)
				{
					FMegalithMemoryHelper::LogMemoryStats(TEXT("After throttle GC"));
				}
			}

			// Check for critical memory situation
			if (FMegalithMemoryHelper::IsMemoryCritical())
			{
				UE_LOG(LogMegalithIndex, Warning, TEXT("Critical memory situation detected (<2GB available). Pausing indexing..."));
			...
			FMegalithCompilerSafeDispatch::RunOnGameThreadWhenCompilerIdle(
				[DB, BatchSlice = MoveTemp(BatchSlice), &DeepIndexed, &DeepErrors, &bTransactionFailure, FrameBudgetSeconds]()
			{
				if (!DB->BeginTransaction())
				{
					bTransactionFailure = true;
					UE_LOG(LogMegalithIndex, Error, TEXT("Index transaction failure (deep batch begin) — this run will not be marked complete"));
					return;
				}
				...
				for (const FDeepIndexEntry& Entry : BatchSlice)
				{
					...
					if (LoadedAsset)
					{
						...
						{
							DeepErrors++;
							UE_LOG(LogMegalithIndex, Warning, TEXT("Deep indexer '%s' failed for: %s"),
								*Entry.Indexer->GetName(),
								*Entry.AssetData.PackageName.ToString());
						}

						// Mark asset for unloading to help GC
						FMegalithMemoryHelper::TryUnloadPackage(LoadedAsset, bWasLoaded);
					}
					else
					{
						DeepErrors++;
						UE_LOG(LogMegalithIndex, Warning, TEXT("Failed to load asset for deep indexing: %s (class: %s)"),
							*Entry.AssetData.PackageName.ToString(),
				...
				{
					bTransactionFailure = true;
					UE_LOG(LogMegalithIndex, Error, TEXT("Index transaction failure (deep batch commit) — this run will not be marked complete"));
				}
			},
			BatchEvent);
			...
			if (BatchNumber % GCFrequency == 0)
			{
				...
				{
					FMegalithMemoryHelper::ForceGarbageCollection(false);
					FMegalithMemoryHelper::YieldToEditor();
			...
			if (BatchNumber % 10 == 0)
			{
				UE_LOG(LogMegalithIndex, Log, TEXT("Deep indexed %d / %d assets (%d ok, %d errors)"),
					BatchEnd, TotalDeep, DeepIndexed.Load(), DeepErrors.Load());

				if (bLogMemory)
				{
					FMegalithMemoryHelper::LogMemoryStats(FString::Printf(TEXT("After batch %d"), BatchNumber));
				}
			}
		}
		...
		UE_LOG(LogMegalithIndex, Log, TEXT("Deep indexing complete: %d indexed, %d errors"),
			DeepIndexed.Load(), DeepErrors.Load());

		if (bLogMemory)
		{
			FMegalithMemoryHelper::LogMemoryStats(TEXT("Deep index complete"));
		}
	}
	...
	auto RunSentinelInTransaction = [&bTransactionFailure](FMegalithIndexDatabase* InDB, const TSharedPtr<IMegalithIndexer>& InIndexer)
	{
		if (!InDB->BeginTransaction())
		{
			bTransactionFailure = true;
			UE_LOG(LogMegalithIndex, Error, TEXT("Index transaction failure (post-pass begin) — this run will not be marked complete"));
			return;
		}
		...
		{
			bTransactionFailure = true;
			UE_LOG(LogMegalithIndex, Error, TEXT("Index transaction failure (post-pass commit) — this run will not be marked complete"));
		}
	};
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Starting post-pass indexers..."));
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* DepIndexer = Owner->ClassToIndexer.Find(TEXT("__Dependencies__"));
		if (DepIndexer && DepIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running dependency indexer..."));
			TSharedPtr<IMegalithIndexer> DepIndexerCopy = *DepIndexer;
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Dependency indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* LevelIndexer = Owner->ClassToIndexer.Find(TEXT("__Levels__"));
		if (LevelIndexer && LevelIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running level indexer..."));
			TSharedPtr<IMegalithIndexer> LevelIndexerCopy = *LevelIndexer;
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Level indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* DTIndexer = Owner->ClassToIndexer.Find(TEXT("__DataTables__"));
		if (DTIndexer && DTIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running DataTable indexer..."));
			TSharedPtr<IMegalithIndexer> DTIndexerCopy = *DTIndexer;
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("DataTable indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* CfgIndexer = Owner->ClassToIndexer.Find(TEXT("__Configs__"));
		if (CfgIndexer && CfgIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running config indexer..."));
			RunSentinelInTransaction(DB, *CfgIndexer);
			UE_LOG(LogMegalithIndex, Log, TEXT("Config indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* CppIndexer = Owner->ClassToIndexer.Find(TEXT("__CppSymbols__"));
		if (CppIndexer && CppIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running C++ symbol indexer..."));
			RunSentinelInTransaction(DB, *CppIndexer);
			UE_LOG(LogMegalithIndex, Log, TEXT("C++ symbol indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* AnimIndexer = Owner->ClassToIndexer.Find(TEXT("__Animations__"));
		if (AnimIndexer && AnimIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running animation indexer..."));
			TSharedPtr<IMegalithIndexer> AnimIndexerCopy = *AnimIndexer;
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Animation indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* TagIndexer = Owner->ClassToIndexer.Find(TEXT("__GameplayTags__"));
		if (TagIndexer && TagIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running gameplay tag indexer..."));
			TSharedPtr<IMegalithIndexer> TagIndexerCopy = *TagIndexer;
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Gameplay tag indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* NiagaraIndexerPtr = Owner->ClassToIndexer.Find(TEXT("__Niagara__"));
		if (NiagaraIndexerPtr && NiagaraIndexerPtr->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running Niagara indexer..."));
			TSharedPtr<IMegalithIndexer> NiagaraIndexerCopy = *NiagaraIndexerPtr;
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Niagara indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}
	...
	if (!CheckCancellation())
	{
		...
		TSharedPtr<IMegalithIndexer>* MeshCatIndexer = Owner->ClassToIndexer.Find(TEXT("__MeshCatalog__"));
		if (MeshCatIndexer && MeshCatIndexer->IsValid())
		{
			double SentinelStart = FPlatformTime::Seconds();
			UE_LOG(LogMegalithIndex, Log, TEXT("Running mesh catalog indexer..."));
			TSharedPtr<IMegalithIndexer> MeshCatIndexerCopy = *MeshCatIndexer;
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Mesh catalog indexer completed in %.2fs"), FPlatformTime::Seconds() - SentinelStart);
			GCBetweenIndexers();
		}
	}

	UE_LOG(LogMegalithIndex, Log, TEXT("Post-pass indexers complete"));
	...
	if (bStructurallyComplete)
	{
		...
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("Full index complete (%d assets indexed, %d errors)"), Indexed, Errors);
		}
		else
		{
			UE_LOG(LogMegalithIndex, Error,
	...
	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("Full index complete"));
	}
...
void UMegalithIndexSubsystem::OnIndexingFinished(bool bSuccess)
{
	bIsIndexing = false;
	...
	if (TaskNotification)
	{
		TaskNotification->SetComplete(
			FText::FromString(TEXT("Megalith")),
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Indexing %s"),
		bSuccess ? TEXT("completed successfully") : TEXT("failed or was cancelled"));
}

FString UMegalithIndexSubsystem::GetDatabasePath() const
{
	...
	return FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("ProjectIndex.db");
}

bool UMegalithIndexSubsystem::ShouldAutoIndex() const
{
...
bool UMegalithIndexSubsystem::CanDoIncrementalIndex() const
{
...
bool UMegalithIndexSubsystem::StartIncrementalIndex()
{
	...
	if (!Database.IsValid() || !Database->IsOpen())
	{
		UE_LOG(LogMegalithIndex, Warning, TEXT("Index database is not open — cannot start an incremental index"));
		return false;
	}
	bIsIndexing = true;
	UnregisterLiveCallbacks();
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Starting incremental index..."));
	...
	if (const UMegalithSettings* Settings = GetDefault<UMegalithSettings>())
	{
		for (const FString& CustomPath : Settings->AdditionalContentPaths)
		{
			if (!CustomPath.IsEmpty())
	...
	{
		UE_LOG(LogMegalithIndex, Log, TEXT("No changes detected. Incremental index complete."));
		bIsIndexing = false;
		RegisterLiveCallbacks();
		return true;
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Incremental index complete."));
	bIsIndexing = false;
	RegisterLiveCallbacks();
	return true;
}
...
void UMegalithIndexSubsystem::ProcessDeepIndexQueue(const TSet<FString>& PathsToIndex)
{
	...
	for (const FString& PackagePath : PathsToIndex)
	{
		...
		for (const FAssetData& AssetData : Assets)
		{
			...
			TSharedPtr<IMegalithIndexer>* Indexer = ClassToIndexer.Find(ClassName);
			if (!Indexer) continue;
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Deep-indexed %d assets from %d paths"), Indexed, PathsToIndex.Num());
}

void UMegalithIndexSubsystem::RunScopedSentinels(const TSet<FString>& ChangedPaths, const TSet<FString>& RemovedPaths)
{
	...
	for (const auto& Indexer : Indexers)
	{
		if (Indexer->IsSentinel() && Indexer->SupportsIncrementalIndex())
		{
			...
			UE_LOG(LogMegalithIndex, Log, TEXT("Scoped sentinel %s completed in %.2fs"), *Indexer->GetName(), Duration);
		}
	}
}

void UMegalithIndexSubsystem::RegisterLiveCallbacks()
{
	...
	OnAssetsAddedHandle = AR.OnAssetsAdded().AddUObject(this, &UMegalithIndexSubsystem::OnAssetsAddedCallback);
	OnAssetsRemovedHandle = AR.OnAssetsRemoved().AddUObject(this, &UMegalithIndexSubsystem::OnAssetsRemovedCallback);
	OnAssetRenamedHandle = AR.OnAssetRenamed().AddUObject(this, &UMegalithIndexSubsystem::OnAssetRenamedCallback);
	OnAssetsUpdatedOnDiskHandle = AR.OnAssetsUpdatedOnDisk().AddUObject(this, &UMegalithIndexSubsystem::OnAssetsUpdatedOnDiskCallback);

	if (GEditor)
	{
		GEditor->GetTimerManager()->SetTimer(
			LiveIndexTimerHandle,
			FTimerDelegate::CreateUObject(this, &UMegalithIndexSubsystem::ProcessPendingChanges),
			2.0f, /*bLoop=*/ true);
	}

	UE_LOG(LogMegalithIndex, Log, TEXT("Live index callbacks registered."));
}

void UMegalithIndexSubsystem::UnregisterLiveCallbacks()
{
...
void UMegalithIndexSubsystem::OnAssetsAddedCallback(TConstArrayView<FAssetData> Assets)
{
...
void UMegalithIndexSubsystem::OnAssetsRemovedCallback(TConstArrayView<FAssetData> Assets)
{
	if (bIsIndexing) return;
	for (const FAssetData& AssetData : Assets)
	{
...
void UMegalithIndexSubsystem::OnAssetRenamedCallback(const FAssetData& AssetData, const FString& OldObjectPath)
{
	if (bIsIndexing) return;
	PendingChanges.Add({EIndexChangeType::Renamed, AssetData, OldObjectPath});
}

void UMegalithIndexSubsystem::OnAssetsUpdatedOnDiskCallback(TConstArrayView<FAssetData> Assets)
{
	if (bIsIndexing) return;
	for (const FAssetData& AssetData : Assets)
...
void UMegalithIndexSubsystem::ProcessPendingChanges()
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Processing %d pending index changes (%d raw)"),
	...
	for (const FPendingIndexChange& Change : LocalChanges)
	{
		...
		case EIndexChangeType::Renamed:
		{
			...
			{
				UE_LOG(LogMegalithIndex, Verbose, TEXT("Asset moved: %s -> %s"), *OldPackageName, *NewPath);
			}
			else
			{
				FIndexedAsset IndexedAsset;
