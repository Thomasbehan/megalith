...
DEFINE_LOG_CATEGORY(LogMegalithIndex);
...
-- C++ symbols (from tree-sitter via MegalithSource)
...
FMegalithIndexDatabase::FMegalithIndexDatabase()
{
}
...
bool FMegalithIndexDatabase::Open(const FString& InDbPath)
{
	if (Database)
	{
		Close();
	...
	{
		UE_LOG(LogMegalithIndex, Error, TEXT("Failed to open index database: %s"), *DbPath);
		delete Database;
		Database = nullptr;
		return false;
	}
	...
	if (!CreateTables())
	{
		UE_LOG(LogMegalithIndex, Error, TEXT("Failed to create index tables"));
		Close();
		return false;
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("Index database opened: %s"), *DbPath);
	return true;
}

void FMegalithIndexDatabase::Close()
{
	if (Database)
	{
		Database->Close();
...
bool FMegalithIndexDatabase::IsOpen() const
{
	return Database != nullptr && Database->IsValid();
}

bool FMegalithIndexDatabase::ResetDatabase()
{
...
bool FMegalithIndexDatabase::BeginTransaction()
{
	return ExecuteSQL(TEXT("BEGIN TRANSACTION;"));
}

bool FMegalithIndexDatabase::CommitTransaction()
{
	return ExecuteSQL(TEXT("COMMIT;"));
}

bool FMegalithIndexDatabase::RollbackTransaction()
{
...
int64 FMegalithIndexDatabase::InsertAsset(const FIndexedAsset& Asset)
{
	if (!IsOpen()) return -1;
...
TOptional<FIndexedAsset> FMegalithIndexDatabase::GetAssetByPath(const FString& PackagePath)
{
	if (!IsOpen()) return {};
...
int64 FMegalithIndexDatabase::GetAssetId(const FString& PackagePath)
{
	if (!IsOpen()) return -1;
...
bool FMegalithIndexDatabase::DeleteAssetAndRelated(int64 AssetId)
{
...
int64 FMegalithIndexDatabase::InsertNode(const FIndexedNode& Node)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedNode> FMegalithIndexDatabase::GetNodesForAsset(int64 AssetId)
{
...
int64 FMegalithIndexDatabase::InsertConnection(const FIndexedConnection& Conn)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedConnection> FMegalithIndexDatabase::GetConnectionsForAsset(int64 AssetId)
{
...
int64 FMegalithIndexDatabase::InsertVariable(const FIndexedVariable& Var)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedVariable> FMegalithIndexDatabase::GetVariablesForAsset(int64 AssetId)
{
...
int64 FMegalithIndexDatabase::InsertParameter(const FIndexedParameter& Param)
{
	if (!IsOpen()) return -1;
...
int64 FMegalithIndexDatabase::InsertDependency(const FIndexedDependency& Dep)
{
	if (!IsOpen()) return -1;
...
TArray<FIndexedDependency> FMegalithIndexDatabase::GetDependenciesForAsset(int64 AssetId)
{
...
TArray<FIndexedDependency> FMegalithIndexDatabase::GetReferencersOfAsset(int64 AssetId)
{
	TArray<FIndexedDependency> Result;
	if (!IsOpen()) return Result;
...
int64 FMegalithIndexDatabase::InsertActor(const FIndexedActor& Actor)
{
	if (!IsOpen()) return -1;
...
int64 FMegalithIndexDatabase::InsertTag(const FIndexedTag& Tag)
{
	if (!IsOpen()) return -1;
...
int64 FMegalithIndexDatabase::GetOrCreateTag(const FString& TagName, const FString& ParentTag)
{
	if (!IsOpen()) return -1;
...
int64 FMegalithIndexDatabase::InsertTagReference(const FIndexedTagReference& Ref)
{
	if (!IsOpen()) return -1;
...
int64 FMegalithIndexDatabase::InsertConfig(const FIndexedConfig& Config)
{
	if (!IsOpen()) return -1;
...
int64 FMegalithIndexDatabase::InsertCppSymbol(const FIndexedCppSymbol& Symbol)
{
	if (!IsOpen()) return -1;
...
int64 FMegalithIndexDatabase::InsertDataTableRow(const FIndexedDataTableRow& Row)
{
	if (!IsOpen()) return -1;
...
bool FMegalithIndexDatabase::WriteMeta(const FString& Key, const FString& Value)
{
	if (!IsOpen()) return false;
...
FString FMegalithIndexDatabase::ReadMeta(const FString& Key) const
{
...
bool FMegalithIndexDatabase::DeleteMeta(const FString& Key)
{
	if (!IsOpen()) return false;
...
namespace MegalithIndexMetaKeys
{
...
EMegalithDeepIndexQueueDecision MegalithDecideDeepIndexQueueEntry(
	...
	const FString& CurrentSavedHash)
{
	...
	{
		return EMegalithDeepIndexQueueDecision::SkipAlreadyIndexed;
	}

	if (StoredAttempts >= MegalithMaxDeepIndexAttempts)
	{
		return EMegalithDeepIndexQueueDecision::SkipPoisonAsset;
	}

	return EMegalithDeepIndexQueueDecision::Queue;
}

bool FMegalithIndexDatabase::SupportsIndexResume() const
{
	if (!Database || !Database->IsValid()) return false;
	return FCString::Atoi(*ReadMeta(TEXT("schema_version"))) >= 3;
}

bool FMegalithIndexDatabase::BeginFullIndex()
{
	if (!IsOpen()) return false;
	...
	if (!WriteMeta(MegalithIndexMetaKeys::FullIndexState, MegalithIndexMetaKeys::FullIndexInProgress) || !DeleteMeta(MegalithIndexMetaKeys::LastFullIndex))
	{
		RollbackTransaction();
		return false;
	}
...
bool FMegalithIndexDatabase::IsFullIndexInProgress() const
{
	return ReadMeta(MegalithIndexMetaKeys::FullIndexState) == MegalithIndexMetaKeys::FullIndexInProgress;
}

bool FMegalithIndexDatabase::CompleteFullIndex(const FString& UtcNow)
{
	if (!IsOpen()) return false;

	if (!BeginTransaction()) return false;

	if (!WriteMeta(MegalithIndexMetaKeys::LastFullIndex, UtcNow) || !DeleteMeta(MegalithIndexMetaKeys::FullIndexState))
	{
		RollbackTransaction();
		return false;
	}
...
bool FMegalithIndexDatabase::SetDeepIndexedHash(int64 AssetId, const FString& Hash)
{
	if (!IsOpen()) return false;
...
bool FMegalithIndexDatabase::BumpDeepIndexAttempts(const TArray<int64>& AssetIds)
{
	if (!IsOpen()) return false;
...
bool FMegalithIndexDatabase::ClearDeepIndexAttempts(int64 AssetId)
{
	if (!IsOpen()) return false;
...
TArray<FString> FMegalithIndexDatabase::GetSkippedAssetPaths() const
{
	TArray<FString> Paths;
	const FString Raw = ReadMeta(MegalithIndexMetaKeys::SkippedAssets);
	if (!Raw.IsEmpty())
	{
...
bool FMegalithIndexDatabase::RecordSkippedAssetPaths(const TArray<FString>& Paths)
{
	if (!IsOpen()) return false;
	...
	return WriteMeta(MegalithIndexMetaKeys::SkippedAssets, FString::Join(Merged, TEXT("\n")));
}
...
TArray<FString> FMegalithIndexDatabase::GetAllIndexedPaths()
{
...
FString FMegalithIndexDatabase::GetSavedHash(const FString& PackagePath)
{
...
TMap<FString, FString> FMegalithIndexDatabase::GetAllPathsAndHashes()
{
...
bool FMegalithIndexDatabase::DeleteAssetByPath(const FString& PackagePath)
{
	if (!IsOpen()) return false;
...
bool FMegalithIndexDatabase::UpdateAssetPath(const FString& OldPath, const FString& NewPath, const FString& NewAssetName)
{
	if (!IsOpen()) return false;
...
bool FMegalithIndexDatabase::UpdateAssetMetadata(const FIndexedAsset& Asset)
{
	if (!IsOpen()) return false;
...
bool FMegalithIndexDatabase::DeleteChildDataForAsset(int64 AssetId)
{
	if (!IsOpen()) return false;
...
bool FMegalithIndexDatabase::UpdateSavedHash(const FString& PackagePath, const FString& HashHex)
{
	if (!IsOpen()) return false;
...
namespace MegalithProjectSearchDetail
{
	/**
...
TArray<FSearchResult> FMegalithIndexDatabase::FullTextSearch(const FString& Query, int32 Limit)
{
	TArray<FSearchResult> Results;
	FString SearchError;
	if (FullTextSearch(Query, Limit, Results, SearchError) != EMegalithProjectSearchStatus::Succeeded)
	{
		UE_LOG(LogMegalithIndex, Error, TEXT("Project search failed: %s"), *SearchError);
	}
	return Results;
}

EMegalithProjectSearchStatus FMegalithIndexDatabase::FullTextSearch(
	const FString& Query,
	int32 Limit,
	...
	FString& OutError)
{
	...
	if (!IsOpen())
	{
		...
		return EMegalithProjectSearchStatus::InternalError;
	}
		...
		FString& AttemptError) -> EAttempt
	{
		...
		for (;;)
		{
			...
			if (StepResult != ESQLitePreparedStatementStepResult::Row)
			{
				...
				if (MegalithProjectSearchDetail::IsFts5UnknownColumnError(DatabaseError))
				{
					AttemptError = DatabaseError;
					return EAttempt::NotApplicable;
				}
				if (MegalithProjectSearchDetail::IsFts5QuerySyntaxError(DatabaseError))
				{
					AttemptError = DatabaseError;
	...
	auto ToStatus = [](EAttempt Attempt)
	{
		return Attempt == EAttempt::InvalidQuery
			? EMegalithProjectSearchStatus::InvalidQuery
			: EMegalithProjectSearchStatus::InternalError;
	};
	...
	{
		OutResults.Reset();
		OutError = MegalithProjectSearchDetail::DescribeUnknownColumn(AssetError);
		return EMegalithProjectSearchStatus::InvalidQuery;
	}
	...
	return EMegalithProjectSearchStatus::Succeeded;
}
...
TSharedPtr<FJsonObject> FMegalithIndexDatabase::GetStats()
{
	...
	// `megalith_reindex(force=true)` (or `Megalith.StartIndex force`) clears it.
...
TSharedPtr<FJsonObject> FMegalithIndexDatabase::GetAssetDetails(const FString& PackagePath)
{
...
TArray<FIndexedAsset> FMegalithIndexDatabase::FindByType(const FString& AssetClass, int32 Limit, int32 Offset)
{
...
TSharedPtr<FJsonObject> FMegalithIndexDatabase::FindReferences(const FString& PackagePath)
{
...
bool FMegalithIndexDatabase::CreateTables()
{
	...
	for (const FString& Stmt : Statements)
	{
		...
		{
			UE_LOG(LogMegalithIndex, Warning, TEXT("Schema statement failed: %s -- Error: %s"),
	...
	if (!bAllSucceeded)
	{
		UE_LOG(LogMegalithIndex, Warning, TEXT("Some schema statements failed -- FTS5 may not be available in this SQLite build"));
	}

	return true; // Return true even if FTS fails -- basic tables should work
}

bool FMegalithIndexDatabase::ExecuteSQL(const FString& SQL)
{
	if (!Database || !Database->IsValid())
	{
		UE_LOG(LogMegalithIndex, Error, TEXT("Cannot execute SQL -- database not open"));
		return false;
	}

	if (!Database->Execute(*SQL))
	{
		UE_LOG(LogMegalithIndex, Error, TEXT("SQL execution failed: %s"), *Database->GetLastError());
		return false;
	}
	return true;
}
