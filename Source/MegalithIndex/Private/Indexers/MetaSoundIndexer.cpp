...
bool FMetaSoundIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("MetaSoundIndexer: Found %d MetaSound assets to index (batch size: %d)"),
		MetaSoundAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("MetaSoundIndexer start"));
	}
	...
	{
		// Compiler-idle gate is enforced by FMegalithCompilerSafeDispatch at the call site (mirrors NiagaraIndexer).

		// Memory budget check before each batch
		if (FMegalithMemoryHelper::ShouldThrottle(MemoryBudgetMB))
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("MetaSoundIndexer: Memory budget exceeded, forcing GC..."));
			FMegalithMemoryHelper::ForceGarbageCollection(true);
			FMegalithMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(TEXT("MetaSoundIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			// Mark for unloading
			FMegalithMemoryHelper::TryUnloadPackage(Loaded, bWasLoaded);
		}
		...
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("MetaSoundIndexer: processed %d / %d assets"),
				BatchEnd, MetaSoundAssets.Num());

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(FString::Printf(TEXT("MetaSoundIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("MetaSoundIndexer: indexed %d MetaSound assets"), AssetsIndexed);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("MetaSoundIndexer complete"));
	}

	return true;
}

void FMetaSoundIndexer::IndexMetaSoundAsset(UObject* MetaSoundAsset, const FString& AssetClassLabel, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	{
		UE_LOG(LogMegalithIndex, Verbose, TEXT("MetaSoundIndexer: '%s' does not implement IMetaSoundDocumentInterface — skipping."),
