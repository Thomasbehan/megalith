 ...
 */
class FGameplayTagIndexer : public IMegalithIndexer
{
public:
...
private:
	...
	int64 IndexTagNode(const struct FGameplayTagNode& Node, FMegalithIndexDatabase& DB);

	/** Scan all indexed assets for gameplay tag references in their asset registry tags */
	void ScanAssetTagReferences(FMegalithIndexDatabase& DB);
};
