...
bool FLevelIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("LevelIndexer: Found %d World assets to index (batch size: %d)"),
		WorldAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("LevelIndexer start"));
	}
	...
	for (int32 i = 0; i < WorldAssets.Num(); i += BatchSize)
	{
		...
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("LevelIndexer: Memory budget exceeded, forcing GC..."));
			FMegalithMemoryHelper::ForceGarbageCollection(true);
			FMegalithMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(TEXT("LevelIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			if (!World || !World->PersistentLevel)
			{
				...
				FMegalithMemoryHelper::TryUnloadPackage(Package, bWasLoaded);
				continue;
			}
			...
			if (World != EditorWorld)
			{
				if (bContainsLandscape)
				{
					...
					if (!bWasLoaded)
					{
						...
						UE_LOG(LogMegalithIndex, Verbose,
				...
				else
				{
					...
					FMegalithMemoryHelper::TryUnloadPackage(World, bWasLoaded);
				}
			}
		...
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("LevelIndexer: processed %d / %d levels"),
				LevelsProcessed, WorldAssets.Num());

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(FString::Printf(TEXT("LevelIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("LevelIndexer: indexed %d levels, %d actors total"),
		LevelsProcessed, ActorsInserted);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("LevelIndexer complete"));
	}

	return true;
}

int32 FLevelIndexer::IndexActorsInLevel(ULevel* Level, FMegalithIndexDatabase& DB, int64 AssetId)
{
	if (!Level) return 0;
