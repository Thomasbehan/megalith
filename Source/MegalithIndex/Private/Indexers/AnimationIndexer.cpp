...
static void LoadAndIndexAnimAsset(void* Ctx)
{
	...
	FMegalithMemoryHelper::TryUnloadPackage(Loaded, C->bWasLoaded);
}

bool FAnimationIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	IAssetRegistry& Registry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry").Get();

	// Get settings for batching
	const UMegalithSettings* Settings = GetDefault<UMegalithSettings>();
	const int32 BatchSize = FMath::Max(1, FMegalithMemoryHelper::GetResolvedPostPassBatchSize());
	...
	auto IndexAllOfType = [&](UClass* Class, FAnimIndexCallContext::EType Type, const TCHAR* TypeName) -> int32
	{
		...
		FARFilter Filter;
		for (const FName& ContentPath : UMegalithSettings::GetIndexedContentPaths())
		{
		...
		UE_LOG(LogMegalithIndex, Log, TEXT("AnimationIndexer: Processing %d %s assets"), Assets.Num(), TypeName);
		...
		for (int32 i = 0; i < Assets.Num(); i += BatchSize)
		{
			...
			{
				UE_LOG(LogMegalithIndex, Log, TEXT("AnimationIndexer: Memory budget exceeded, forcing GC..."));
			...
			for (int32 j = i; j < BatchEnd; ++j)
			{
				...
				else
				{
					...
					// reports success.
					UE_LOG(LogMegalithIndex, Warning, TEXT("AnimationIndexer: crashed loading/indexing '%s' - skipping"), *AD.GetSoftObjectPath().ToString());
				}
#else
			...
			{
				UE_LOG(LogMegalithIndex, Error, TEXT("AnimationIndexer: failed to commit batch %d — that batch's rows are lost"), BatchNumber);
			}
			...
			{
				UE_LOG(LogMegalithIndex, Log, TEXT("AnimationIndexer: %s progress %d / %d"), TypeName, BatchEnd, Assets.Num());
			}
		}
	...
	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("AnimationIndexer start"));
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("AnimationIndexer: indexed %d animation assets"), TotalIndexed);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("AnimationIndexer complete"));
	}

	return true;
}

void FAnimationIndexer::IndexAnimSequence(UAnimSequence* AnimSeq, FMegalithIndexDatabase& DB, int64 AssetId)
{
...
void FAnimationIndexer::IndexAnimMontage(UAnimMontage* Montage, FMegalithIndexDatabase& DB, int64 AssetId)
{
...
void FAnimationIndexer::IndexBlendSpace(UBlendSpace* BlendSpace, FMegalithIndexDatabase& DB, int64 AssetId)
{
...
void FAnimationIndexer::IndexPoseAsset(UPoseAsset* PoseAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	if (!PoseAsset) return;
