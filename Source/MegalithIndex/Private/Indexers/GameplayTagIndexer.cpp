...
bool FGameplayTagIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("GameplayTagIndexer: indexed %d root tag trees, scanned assets for references"), TagsInserted);
	return true;
}

int64 FGameplayTagIndexer::IndexTagNode(const FGameplayTagNode& Node, FMegalithIndexDatabase& DB)
{
...
void FGameplayTagIndexer::ScanAssetTagReferences(FMegalithIndexDatabase& DB)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("GameplayTagIndexer: inserted %d tag references across assets"), RefsInserted);
}
