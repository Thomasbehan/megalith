...
bool FMeshCatalogIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	FSQLiteDatabase* RawDB = DB.GetRawDatabase();
	if (!RawDB)
	{
		UE_LOG(LogMegalithIndex, Error, TEXT("MeshCatalogIndexer: No raw database available"));
		return false;
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("MeshCatalogIndexer: Found %d StaticMesh assets to catalog (batch size: %d)"),
		MeshAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("MeshCatalogIndexer start"));
	}
	...
	for (int32 i = 0; i < MeshAssets.Num(); i += BatchSize)
	{
		...
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("MeshCatalogIndexer: Memory budget exceeded, forcing GC..."));
			FMegalithMemoryHelper::ForceGarbageCollection(true);
			FMegalithMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(TEXT("MeshCatalogIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			{
				FMegalithMemoryHelper::TryUnloadPackage(Mesh, bWasLoaded);
				Errors++;
				continue;
			}
		...
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("MeshCatalogIndexer: cataloged %d / %d meshes"),
				Indexed, MeshAssets.Num());

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(FString::Printf(TEXT("MeshCatalogIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("MeshCatalogIndexer: Cataloged %d meshes (%d errors)"), Indexed, Errors);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("MeshCatalogIndexer complete"));
	}
