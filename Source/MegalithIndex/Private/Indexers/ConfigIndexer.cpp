...
bool FConfigIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("ConfigIndexer: indexed %d entries from %d .ini files"), TotalEntries, IniFiles.Num());
	return true;
}
...
int32 FConfigIndexer::ParseAndInsertIniFile(const FString& FilePath, FMegalithIndexDatabase& DB)
{
