 ...
 */
class FGASIndexer : public IMegalithIndexer
{
public:
...
private:
	...
	int64 IndexAbility(UObject* CDO, const FString& AssetPath, FMegalithIndexDatabase& DB, int64 AssetId);

	/** Index a single GameplayEffect Blueprint CDO */
	int64 IndexEffect(UObject* CDO, const FString& AssetPath, FMegalithIndexDatabase& DB, int64 AssetId);

	/** Index an AttributeSet class (Blueprint or native) */
	int64 IndexAttributeSet(UClass* Class, FMegalithIndexDatabase& DB, int64 AssetId);

	/** Index a GameplayCue notify (Static or Actor) */
	int64 IndexGameplayCue(UObject* CDO, const FString& AssetPath, FMegalithIndexDatabase& DB, int64 AssetId, bool bIsActor);
