...
namespace
{
	...
	TArray<FBPWithCDO> FindBlueprintCDOsOfClass(UClass* NativeBaseClass, FMegalithIndexDatabase& DB)
	{
...
bool FGASIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("GASIndexer: indexed %d GAS assets (%d abilities, %d effects, %d attribute sets, %d cues)"),
...
int64 FGASIndexer::IndexAbility(UObject* CDO, const FString& AssetPath, FMegalithIndexDatabase& DB, int64 AssetId)
{
	if (!CDO) return -1;
...
int64 FGASIndexer::IndexEffect(UObject* CDO, const FString& AssetPath, FMegalithIndexDatabase& DB, int64 AssetId)
{
	if (!CDO) return -1;
...
int64 FGASIndexer::IndexAttributeSet(UClass* Class, FMegalithIndexDatabase& DB, int64 AssetId)
{
	if (!Class) return -1;
...
int64 FGASIndexer::IndexGameplayCue(UObject* CDO, const FString& AssetPath, FMegalithIndexDatabase& DB, int64 AssetId, bool bIsActor)
{
	if (!CDO) return -1;
