...
bool FNiagaraIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("NiagaraIndexer: Found %d Niagara systems to index (batch size: %d)"),
		NiagaraAssets.Num(), BatchSize);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("NiagaraIndexer start"));
	}
	...
	{
		// Compiler-idle gate is enforced by FMegalithCompilerSafeDispatch at the call site (see issue #19).

		// Memory budget check before each batch
		if (FMegalithMemoryHelper::ShouldThrottle(MemoryBudgetMB))
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("NiagaraIndexer: Memory budget exceeded, forcing GC..."));
			FMegalithMemoryHelper::ForceGarbageCollection(true);
			FMegalithMemoryHelper::YieldToEditor();

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(TEXT("NiagaraIndexer after throttle GC"));
			}
		}
		...
		for (int32 j = i; j < BatchEnd; ++j)
		{
			...
			// Mark for unloading
			FMegalithMemoryHelper::TryUnloadPackage(System, bWasLoaded);
		}
		...
		{
			UE_LOG(LogMegalithIndex, Log, TEXT("NiagaraIndexer: processed %d / %d systems"),
				BatchEnd, NiagaraAssets.Num());

			if (bLogMemory)
			{
				FMegalithMemoryHelper::LogMemoryStats(FString::Printf(TEXT("NiagaraIndexer batch %d"), BatchNumber));
			}
		}
	}
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("NiagaraIndexer: indexed %d Niagara systems"), SystemsIndexed);

	if (bLogMemory)
	{
		FMegalithMemoryHelper::LogMemoryStats(TEXT("NiagaraIndexer complete"));
	}

	return true;
}

void FNiagaraIndexer::IndexNiagaraSystem(UNiagaraSystem* System, FMegalithIndexDatabase& DB, int64 AssetId)
{
	if (!System) return;
