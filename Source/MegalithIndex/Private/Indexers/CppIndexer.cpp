...
bool FCppIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("CppIndexer: found %d source files (Source/ + Plugins/)"), SourceFiles.Num());
	...
	UE_LOG(LogMegalithIndex, Log, TEXT("CppIndexer: indexed %d symbols from %d source files"), TotalSymbols, SourceFiles.Num());
	return true;
}

int32 FCppIndexer::ParseSourceFile(const FString& FilePath, const FString& RelativePath, FMegalithIndexDatabase& DB)
{
