...
bool FDataAssetIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
...
TSharedPtr<FJsonObject> FDataAssetIndexer::SerializeObjectProperties(UObject* Object)
{
	...
	for (TFieldIterator<FProperty> It(ObjClass, EFieldIteratorFlags::IncludeSuper, EFieldIteratorFlags::ExcludeDeprecated); It; ++It)
	{
		...
		// Shared FProperty->JSON serializer (hoisted into MegalithCore). The
		// former FDataAssetIndexer::PropertyToJsonValue copy was subsumed by it.
		PropsObj->SetField(Prop->GetName(), FMegalithReflectionReader::PropertyToJsonValue(Prop, ValuePtr, Object));
	}
