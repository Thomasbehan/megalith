...
void FMegalithCompilerSafeDispatch::RunOnGameThreadWhenCompilerIdle(
            ...
            if (RemainingAssets == 0 || bTimedOut)
            {
                if (bTimedOut && RemainingAssets != 0)
                {
                    UE_LOG(LogMegalithIndex, Warning,
                        TEXT("FMegalithCompilerSafeDispatch timed out after %.1fs with %d assets still compiling — running work anyway."),
