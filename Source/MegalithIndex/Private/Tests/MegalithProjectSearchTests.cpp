...
// Plan: Plugins/Megalith/Docs/plans/2026-08-01-pr-issue-sweep.md (Phase 1, T06/T07)
//
// Goals:
...
//     SPEC_MegalithIndex.md "Search Error Contract".
...
// MegalithCore tests in that module's folder.
...
namespace MegalithProjectSearchTestDetail
{
	...
	static const TCHAR* AssetToken = TEXT("MegalithSearchFixtureAsset");
	static const TCHAR* NodeToken = TEXT("MegalithSearchFixtureNode");

	/** Temp index database seeded with one asset and one graph node. */
	struct FFixture
	{
		FMegalithIndexDatabase Database;
		FString DbPath;
		...
		bool Open()
		{
			...
			FIndexedAsset Asset;
			Asset.PackagePath = TEXT("/Game/Tests/Megalith/MegalithSearchFixtureAsset");
	...
	static FMegalithActionResult InvokeAction(const TSharedPtr<FJsonObject>& Params)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithProjectSearchColumnQualifiedTest,
	"Megalith.ProjectSearch.ColumnQualified",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithProjectSearchColumnQualifiedTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithProjectSearchTestDetail;
	...
	const EMegalithProjectSearchStatus AssetStatus = Fixture.Database.FullTextSearch(
		FString::Printf(TEXT("asset_name:%s"), AssetToken), 50, Results, Error);
	TestEqual(TEXT("asset_name: filter succeeds"), AssetStatus, EMegalithProjectSearchStatus::Succeeded);
	TestTrue(TEXT("asset_name: filter returns the fixture asset"), Results.Num() > 0);

	// The mirror image: fts_assets has no `node_name`.
	const EMegalithProjectSearchStatus NodeStatus = Fixture.Database.FullTextSearch(
		FString::Printf(TEXT("node_name:%s"), NodeToken), 50, Results, Error);
	TestEqual(TEXT("node_name: filter succeeds"), NodeStatus, EMegalithProjectSearchStatus::Succeeded);
	...
	const EMegalithProjectSearchStatus SpanStatus = Fixture.Database.FullTextSearch(
		FString::Printf(TEXT("asset_name:%s OR node_name:%s"), AssetToken, NodeToken), 50, Results, Error);
	TestEqual(TEXT("cross-table filter is an invalid query"), SpanStatus, EMegalithProjectSearchStatus::InvalidQuery);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithProjectSearchErrorClassificationTest,
	"Megalith.ProjectSearch.ErrorClassification",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithProjectSearchErrorClassificationTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithProjectSearchTestDetail;
	...
	const EMegalithProjectSearchStatus UnbalancedStatus = Fixture.Database.FullTextSearch(
		FString::Printf(TEXT("%s AND ("), AssetToken), 50, Results, Error);
	TestEqual(TEXT("unbalanced paren is an invalid query"), UnbalancedStatus, EMegalithProjectSearchStatus::InvalidQuery);
	...
	const EMegalithProjectSearchStatus BadNearStatus = Fixture.Database.FullTextSearch(
		FString::Printf(TEXT("%s NEAR/3 Other"), AssetToken), 50, Results, Error);
	TestEqual(TEXT("NEAR/3 is an invalid query"), BadNearStatus, EMegalithProjectSearchStatus::InvalidQuery);

	// The form that actually parses.
	const EMegalithProjectSearchStatus GoodNearStatus = Fixture.Database.FullTextSearch(
		FString::Printf(TEXT("NEAR(%s Other, 3)"), AssetToken), 50, Results, Error);
	TestEqual(TEXT("NEAR(a b, N) is valid"), GoodNearStatus, EMegalithProjectSearchStatus::Succeeded);

	// A column no table exposes -- and the message must name it, so a typo is a
	// five-second fix rather than a filed issue.
	const EMegalithProjectSearchStatus UnknownColumnStatus = Fixture.Database.FullTextSearch(
		TEXT("megalith_no_such_column:Anything"), 50, Results, Error);
	TestEqual(TEXT("unknown column is an invalid query"), UnknownColumnStatus, EMegalithProjectSearchStatus::InvalidQuery);
	TestTrue(TEXT("unknown-column error names the offending column"),
		Error.Contains(TEXT("megalith_no_such_column")));
	...
	const EMegalithProjectSearchStatus NoMatchStatus = Fixture.Database.FullTextSearch(
		TEXT("MegalithSearchFixtureNothingMatchesThis"), 50, Results, Error);
	TestEqual(TEXT("zero matches is a success"), NoMatchStatus, EMegalithProjectSearchStatus::Succeeded);
	TestEqual(TEXT("zero matches returns no results"), Results.Num(), 0);

	// A closed database is an INTERNAL failure, not a caller error.
	FMegalithIndexDatabase Unopened;
	const EMegalithProjectSearchStatus ClosedStatus =
		Unopened.FullTextSearch(TEXT("Anything"), 50, Results, Error);
	TestEqual(TEXT("closed database is an internal error"), ClosedStatus, EMegalithProjectSearchStatus::InternalError);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithProjectSearchLimitClampTest,
	"Megalith.ProjectSearch.LimitClamp",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithProjectSearchLimitClampTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithProjectSearchTestDetail;
	...
	const EMegalithProjectSearchStatus ZeroStatus =
		Fixture.Database.FullTextSearch(AssetToken, 0, Results, Error);
	TestEqual(TEXT("limit 0 succeeds"), ZeroStatus, EMegalithProjectSearchStatus::Succeeded);
	TestTrue(TEXT("limit 0 clamps up to 1 rather than returning nothing"), Results.Num() > 0);

	// A negative limit is clamped the same way.
	const EMegalithProjectSearchStatus NegativeStatus =
		Fixture.Database.FullTextSearch(AssetToken, -5, Results, Error);
	TestEqual(TEXT("negative limit succeeds"), NegativeStatus, EMegalithProjectSearchStatus::Succeeded);
	...
	const EMegalithProjectSearchStatus HugeStatus =
		Fixture.Database.FullTextSearch(AssetToken, TNumericLimits<int32>::Max(), Results, Error);
	TestEqual(TEXT("huge limit succeeds"), HugeStatus, EMegalithProjectSearchStatus::Succeeded);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithProjectSearchValidationTest,
	"Megalith.ProjectSearch.Validation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithProjectSearchValidationTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithProjectSearchTestDetail;

	auto ExpectInvalidParams = [this](const TCHAR* What, const TSharedPtr<FJsonObject>& Params)
	{
		const FMegalithActionResult R = InvokeAction(Params);
	...
	const FMegalithActionResult AtCapResult = InvokeAction(AtCap);
