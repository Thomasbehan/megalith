...
// Plan: Plugins/Megalith/Docs/plans/2026-08-01-pr-issue-sweep.md (Phase 6, T29)
//
// Goals:
...
namespace MegalithIndexRecoveryTestDetail
{
	static const TCHAR* AssetPath = TEXT("/Game/Tests/Megalith/MegalithRecoveryFixtureAsset");
	static const TCHAR* SecondAssetPath = TEXT("/Game/Tests/Megalith/MegalithRecoveryFixtureAssetB");
	...
	struct FFixture
	{
		...
		int64 InsertAsset(const TCHAR* PackagePath, const TCHAR* SavedHash)
		{
			...
			Asset.AssetName = TEXT("MegalithRecoveryFixtureAsset");
	...
	static int64 CountRows(FMegalithIndexDatabase& Database, const TCHAR* Table)
	{
	...
	static bool HasAssetColumn(FMegalithIndexDatabase& Database, const TCHAR* ColumnName)
	{
		FSQLiteDatabase* Raw = Database.GetRawDatabase();
		if (!Raw)
		{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithIndexRecoveryLifecycleTest,
	"Megalith.Index.Recovery.Lifecycle",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithIndexRecoveryLifecycleTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithIndexRecoveryTestDetail;
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithIndexRecoveryCheckpointAtomicityTest,
	"Megalith.Index.Recovery.CheckpointAtomicity",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithIndexRecoveryCheckpointAtomicityTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithIndexRecoveryTestDetail;
	...
	Node.NodeName = TEXT("MegalithRecoveryFixtureNode");
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithIndexRecoverySchemaMigrationTest,
	"Megalith.Index.Recovery.SchemaMigrationV2ToV3",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithIndexRecoverySchemaMigrationTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithIndexRecoveryTestDetail;
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithIndexRecoveryPoisonPillTest,
	"Megalith.Index.Recovery.PoisonPillSkipsAfterTwoAttempts",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithIndexRecoveryPoisonPillTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithIndexRecoveryTestDetail;
	...
	TestEqual(TEXT("an asset at one attempt is still queued"),
		MegalithDecideDeepIndexQueueEntry(AfterOne->DeepIndexedHash, AfterOne->DeepIndexAttempts, ContentHash),
		EMegalithDeepIndexQueueDecision::Queue);
	...
	TestEqual(TEXT("an asset at two attempts is skipped as poisonous"),
		MegalithDecideDeepIndexQueueEntry(AfterTwo->DeepIndexedHash, AfterTwo->DeepIndexAttempts, ContentHash),
		EMegalithDeepIndexQueueDecision::SkipPoisonAsset);
	...
	TestEqual(TEXT("the healthy asset is still queued"),
		MegalithDecideDeepIndexQueueEntry(Healthy->DeepIndexedHash, Healthy->DeepIndexAttempts, ContentHash),
		EMegalithDeepIndexQueueDecision::Queue);
	...
	TestEqual(TEXT("the skipped asset stays out of the queue"),
		MegalithDecideDeepIndexQueueEntry(AfterSkip->DeepIndexedHash, AfterSkip->DeepIndexAttempts, ContentHash),
		EMegalithDeepIndexQueueDecision::SkipAlreadyIndexed);
	...
	TestEqual(TEXT("the counter still holds it out when the hash stamp cannot"),
		MegalithDecideDeepIndexQueueEntry(FString(), AfterSkip->DeepIndexAttempts, FString()),
		EMegalithDeepIndexQueueDecision::SkipPoisonAsset);
	TestEqual(TEXT("even a content change does not revive it while the counter stands"),
		MegalithDecideDeepIndexQueueEntry(AfterSkip->DeepIndexedHash, AfterSkip->DeepIndexAttempts, TEXT("ffffffffffffffffffffffffffffffffffffffff")),
		EMegalithDeepIndexQueueDecision::SkipPoisonAsset);
	...
	TestEqual(TEXT("a cleared, changed asset is queued again"),
		MegalithDecideDeepIndexQueueEntry(AfterClear->DeepIndexedHash, AfterClear->DeepIndexAttempts, TEXT("ffffffffffffffffffffffffffffffffffffffff")),
		EMegalithDeepIndexQueueDecision::Queue);
	...
	const int64 DurabilityId = Fixture.InsertAsset(TEXT("/Game/Tests/Megalith/MegalithRecoveryDurability"), ContentHash);
	...
	WorkNode.NodeName = TEXT("MegalithRecoveryDurabilityNode");
	...
	TOptional<FIndexedAsset> AfterRollback = Fixture.Database.GetAssetByPath(TEXT("/Game/Tests/Megalith/MegalithRecoveryDurability"));
	...
	AfterRollback = Fixture.Database.GetAssetByPath(TEXT("/Game/Tests/Megalith/MegalithRecoveryDurability"));
