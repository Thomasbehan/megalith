...
DEFINE_LOG_CATEGORY(LogMegalithMemory);
...
SIZE_T FMegalithMemoryHelper::GetCurrentMemoryUsageMB()
{
	FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
	return Stats.UsedPhysical / (1024 * 1024);
}

SIZE_T FMegalithMemoryHelper::GetAvailableMemoryMB()
{
	FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
	return Stats.AvailablePhysical / (1024 * 1024);
}

bool FMegalithMemoryHelper::ShouldThrottle(SIZE_T BudgetMB)
{
	SIZE_T CurrentUsageMB = GetCurrentMemoryUsageMB();
	return CurrentUsageMB > BudgetMB;
}

void FMegalithMemoryHelper::ForceGarbageCollection(bool bFullPurge)
{
	if (!IsInGameThread())
	{
		UE_LOG(LogMegalithMemory, Verbose, TEXT("ForceGarbageCollection called from non-game thread - skipping"));
		return;
	}
	...
	{
		UE_LOG(LogMegalithMemory, Verbose, TEXT("GC already in progress - skipping to avoid reentrant GC crash"));
		return;
	}
	...
	{
		UE_LOG(LogMegalithMemory, Verbose, TEXT("GC cooldown active (%.2fs since last GC) - skipping"), TimeSinceLastGC);
		return;
	}
	...
	{
		UE_LOG(LogMegalithMemory, Verbose, TEXT("TryCollectGarbage returned false - GC deferred by engine"));
		return;
	}
}

bool FMegalithMemoryHelper::TryUnloadPackage(UObject* Asset, bool bWasAlreadyLoaded)
{
	if (!Asset)
	{
		return false;
...
void FMegalithMemoryHelper::YieldToEditor()
{
	if (!IsInGameThread())
	{
		return;
	...
	// dispatched via FMegalithCompilerSafeDispatch's FTSTicker, runs on one real
...
void FMegalithMemoryHelper::LogMemoryStats(const FString& Context)
{
	FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
...
// still override per-project via Project Settings > Megalith > Indexing > Performance.
...
int32 FMegalithMemoryHelper::GetInstalledRamGB()
{
	const FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
	return static_cast<int32>(Stats.TotalPhysical / (1024ULL * 1024ULL * 1024ULL));
}

int32 FMegalithMemoryHelper::GetResolvedMemoryBudgetMB()
{
...
int32 FMegalithMemoryHelper::GetResolvedDeepIndexBatchSize()
{
	const UMegalithSettings* Settings = GetDefault<UMegalithSettings>();
...
int32 FMegalithMemoryHelper::GetResolvedPostPassBatchSize()
{
	const UMegalithSettings* Settings = GetDefault<UMegalithSettings>();
...
void FMegalithMemoryHelper::LogTierStartupOnce()
{
	...
	UE_LOG(LogMegalithMemory, Log,
		TEXT("Indexer tier: %d GB installed -> %d MB budget, batch(deep=%d, post=%d). Override in Project Settings > Megalith > Indexing > Performance."),
