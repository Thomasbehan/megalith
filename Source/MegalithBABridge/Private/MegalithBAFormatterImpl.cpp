...
#define MEGALITH_BA_4_9_OR_LATER 1
#else
#define MEGALITH_BA_4_9_OR_LATER 0
#endif

bool FMegalithBAFormatterImpl::SupportsGraph(UEdGraph* Graph) const
{
	if (!Graph)
	{
		return false;
...
bool FMegalithBAFormatterImpl::FormatGraph(
	UEdGraph* Graph,
...
FMegalithFormatterInfo FMegalithBAFormatterImpl::GetFormatterInfo(UEdGraph* Graph) const
{
	FMegalithFormatterInfo Info;
...
TSharedPtr<FBAGraphHandler> FMegalithBAFormatterImpl::FindHandlerForGraph(UEdGraph* Graph) const
{
	if (!Graph)
	{
		return nullptr;
