...
void FMegalithConfigActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("config"), TEXT("resolve_setting"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithConfigActions::ResolveSetting),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("config"), TEXT("explain_setting"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithConfigActions::ExplainSetting),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("config"), TEXT("diff_from_default"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithConfigActions::DiffFromDefault),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("config"), TEXT("search_config"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithConfigActions::SearchConfig),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("config"), TEXT("get_section"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithConfigActions::GetSection),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("config"), TEXT("get_config_files"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithConfigActions::GetConfigFiles),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("config"), TEXT("set_developer_setting"),
		TEXT("DEV-ONLY (write): set a property on a UDeveloperSettings CDO at runtime. "
			 "Resolves a settings class by short-name (e.g. 'MegalithReflectionIntelSettings') "
		...
		FMegalithActionHandler::CreateStatic(&FMegalithConfigActions::SetDeveloperSetting),
		FParamSchemaBuilder()
			.Required(TEXT("class"), TEXT("string"),
				TEXT("Settings class short-name (e.g. 'MegalithReflectionIntelSettings') or full path ('/Script/MegalithReflectionIntel.MegalithReflectionIntelSettings')."))
...
FString FMegalithConfigActions::ResolveConfigFilePath(const FString& ShortName)
{
...
TArray<TPair<FString, FString>> FMegalithConfigActions::GetConfigHierarchy(const FString& Category)
{
...
FMegalithActionResult FMegalithConfigActions::ResolveSetting(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithConfigActions::ExplainSetting(const TSharedPtr<FJsonObject>& Params)
{
	FString Category = Params->GetStringField(TEXT("file"));
	FString Section = Params->GetStringField(TEXT("section"));
	FString Key = Params->GetStringField(TEXT("key"));
...
FMegalithActionResult FMegalithConfigActions::DiffFromDefault(const TSharedPtr<FJsonObject>& Params)
{
	FString Category = Params->GetStringField(TEXT("file"));
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No config found for category '%s'"), *Category));
	}
...
FMegalithActionResult FMegalithConfigActions::SearchConfig(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithConfigActions::GetSection(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Config file not found: '%s' (resolved to '%s')"), *FileShortName, *FilePath));
	}

	FString FileContents;
	if (!FFileHelper::LoadFileToString(FileContents, *FilePath))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to read config file: '%s'"), *FilePath));
	}
	...
	if (!bInSection)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Section '%s' not found in '%s'"), *Section, *FileShortName));
	}
...
FMegalithActionResult FMegalithConfigActions::GetConfigFiles(const TSharedPtr<FJsonObject>& Params)
{
	FString FilterCategory = Params->HasField(TEXT("category")) ? Params->GetStringField(TEXT("category")) : TEXT("");
...
//     Module is `DeveloperSettings` (NOT `Engine`) — added to MegalithConfig.Build.cs.
//
...
//     to the resolved INI (e.g. `Config/MegalithSettings.ini`).
// ============================================================================

FMegalithActionResult FMegalithConfigActions::SetDeveloperSetting(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("set_developer_setting: missing params object"));
	}
	...
	// Common 'U'-prefix convenience (matches MegalithBlueprint/MegalithAnimation patterns).
	...
	if (TargetClass == nullptr)
	{
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("set_developer_setting: unknown class '%s' — supply short name "
				 "like 'MegalithReflectionIntelSettings' or full path "
				 "'/Script/MegalithReflectionIntel.MegalithReflectionIntelSettings'"),
			*ClassName));
	}
