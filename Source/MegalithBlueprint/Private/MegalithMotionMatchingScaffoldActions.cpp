...
void FMegalithMotionMatchingScaffoldActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_anim_class"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetAnimClass),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("apply_movement_preset"),
		...
		FMegalithActionHandler::CreateStatic(&HandleApplyMovementPreset),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_engine_component_typed"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddEngineComponentTyped),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_locomotion_input"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldLocomotionInput),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("bp_path"), TEXT("Character Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("validate_animbp_variable_contract"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateAnimBpVariableContract),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_motion_matching_character"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldMotionMatchingCharacter),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("get_inherited_component_override"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetInheritedComponentOverride),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_threadsafe_update"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldThreadSafeUpdate),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_pawn_owner_access"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddPawnOwnerAccess),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("scaffold_locomotion_anim_values"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldLocomotionAnimValues),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("apply_locomotion_speed_band"),
		...
		FMegalithActionHandler::CreateStatic(&HandleApplyLocomotionSpeedBand),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("bp_path"), TEXT("Character Blueprint asset path"))
...
namespace
{
	 ...
	 * Done by string match on the node class so MegalithBlueprint need not depend on
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleSetAnimClass(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (BpPath.IsEmpty())     return FMegalithActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (CompName.IsEmpty())   return FMegalithActionResult::Error(TEXT("Missing required parameter: component"));
	if (AnimBpPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: anim_bp_path"));

	UBlueprint* BP = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(BpPath);
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint not found: %s"), *BpPath));
	}

	UAnimBlueprint* ABP = FMegalithAssetUtils::LoadAssetByPath<UAnimBlueprint>(AnimBpPath);
	if (!ABP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Animation Blueprint not found: %s"), *AnimBpPath));
	}
	...
	const MegalithBlueprintComponentResolver::FResult MeshResolved =
	...
	if (!SMC)
	{
		return FMegalithActionResult::Error(MeshResolved.Error.IsEmpty()
			? FString::Printf(
	...
	Root->SetStringField(TEXT("source"), MegalithBlueprintComponentResolver::SourceToString(MeshResolved.Source));
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleApplyMovementPreset(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString Preset = Params->GetStringField(TEXT("preset"));

	if (BpPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (Preset.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: preset"));
	...
	if (!CmcClass)
	{
		return FMegalithActionResult::Error(TEXT("apply_movement_preset: UCharacterMovementComponent class not found"));
	}
	...
	const MegalithBlueprintComponentResolver::FResult CmcResolved =
		MegalithBlueprintComponentResolver::Resolve(BP, FString(), CmcClass, /*bCreateIchOverride=*/false);
	if (!CmcResolved.IsValid())
	{
		return FMegalithActionResult::Error(CmcResolved.Error.IsEmpty()
			? FString::Printf(
	...
	for (const TPair<FString, FString>& Write : Writes)
	{
		...
		FMegalithActionResult R = FMegalithBlueprintComponentActions::HandleSetComponentProperty(Sub);
		if (!R.bSuccess)
		{
			return FMegalithActionResult::Error(FString::Printf(
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleAddEngineComponentTyped(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	...
	if (BpPath.IsEmpty())   return FMegalithActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (CompType.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: component_type"));
	if (CompName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: component_name"));
	...
	if (!CompClass)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Component class not found: %s"), *CompType));
	}
	...
	return FMegalithBlueprintComponentActions::HandleAddComponent(Sub);
}
...
namespace
{
	...
	UObject* CreateInputAsset(UClass* AssetClass, const FString& PackagePath, FString& OutError)
	{
		...
		if (FMegalithAssetUtils::AssetExists(PackagePath))
		{
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleScaffoldLocomotionInput(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString ImcPath = Params->GetStringField(TEXT("imc_path"));

	if (BpPath.IsEmpty())  return FMegalithActionResult::Error(TEXT("Missing required parameter: bp_path"));
	if (ImcPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: imc_path"));
	...
	{
		return FMegalithActionResult::Error(TEXT("'actions' is required and must be a non-empty array of {name, value_type}"));
	}
	...
	if (!ImcObj)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("scaffold_locomotion_input: %s"), *CreateError));
	}
	...
	for (const TSharedPtr<FJsonValue>& Entry : *ActionsArr)
	{
		...
		if (!Obj.IsValid())
		{
			return FMegalithActionResult::Error(TEXT("Each 'actions' entry must be an object {name, value_type}"));
		}
		FString ActName;
		Obj->TryGetStringField(TEXT("name"), ActName);
		if (ActName.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("Each 'actions' entry requires a non-empty 'name'"));
		}
		FString ValueType;
	...
	for (const FString& ActName : ActionNames)
	{
		...
		FMegalithActionResult NR = FMegalithBlueprintNodeActions::HandleAddNode(NodeSub);
		if (NR.bSuccess)
		{
			NodesAdded++;
		}
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleValidateAnimBpVariableContract(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleScaffoldMotionMatchingCharacter(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString AnimBpPath = Params->GetStringField(TEXT("anim_bp_path"));
	...
	if (!BP)
	{
		...
		FMegalithActionResult CR = FMegalithBlueprintCompileActions::HandleCreateBlueprint(CreateSub);
		if (!CR.bSuccess)
		{
			return FMegalithActionResult::Error(FString::Printf(
		...
		BP = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(BpPath);
		if (!BP)
		{
			return FMegalithActionResult::Error(TEXT("scaffold_motion_matching_character: BP created but failed to reload"));
		}
	}
	else
	{
			...
			!CurrentParent.Equals(FString(TEXT("A")) + ParentClass, ESearchCase::IgnoreCase))
		{
			...
			FMegalithActionResult RR = FMegalithBlueprintGraphActions::HandleReparentBlueprint(ReparentSub);
	...
	if (!Mesh.IsEmpty())
	{
		...
		USkeletalMesh* MeshAsset = FMegalithAssetUtils::LoadAssetByPath<USkeletalMesh>(Mesh);
		if (!MeshSMC)
		{
	...
	// --- Compose 5.1: set_anim_class on the 'Mesh' component ---
	{
		...
		FMegalithActionResult AR = HandleSetAnimClass(AnimSub);
		if (!AR.bSuccess)
		{
			return FMegalithActionResult::Error(FString::Printf(
	...
	// --- Compose 5.2: apply_movement_preset ---
	{
		...
		FMegalithActionResult PR = HandleApplyMovementPreset(PresetSub);
		if (!PR.bSuccess)
		{
			return FMegalithActionResult::Error(FString::Printf(
	...
	// --- Compile ---
	{
		...
		FMegalithActionResult CR = FMegalithBlueprintCompileActions::HandleCompileBlueprint(CompileSub);
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleGetInheritedComponentOverride(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	const FString CompName = Params->GetStringField(TEXT("component"));
	FString SingleProp;
...
namespace
{
	...
	bool EnsureThreadSafeUpdate(const FString& AbpPath, UAnimBlueprint* ABP, bool& bOutCreated, FString& OutError)
	{
		...
		FMegalithActionResult R = FMegalithBlueprintGraphActions::HandleOverrideParentFunction(Sub);
		if (!R.bSuccess)
		{
	 ...
	 */
	void ExtractBulkNodeIds(const FMegalithActionResult& BulkResult, TMap<FString, FString>& OutMap)
	{
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleScaffoldThreadSafeUpdate(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	if (AbpPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: abp_path"));
	...
	if (!ABP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Animation Blueprint not found: %s"), *AbpPath));
	}
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleAddPawnOwnerAccess(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	if (AbpPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: abp_path"));
	...
	FMegalithActionResult NR = FMegalithBlueprintNodeActions::HandleAddNodesBulk(NodesSub);
	if (!NR.bSuccess)
	{
		return FMegalithActionResult::Error(FString::Printf(
	...
	if (!GetPawnNodeId.IsEmpty() && !CastNodeId.IsEmpty())
	{
		...
		FMegalithBlueprintNodeActions::HandleConnectPinsBulk(ConnSub); // best-effort wire
	}
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleScaffoldLocomotionAnimValues(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	if (AbpPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: abp_path"));
	...
	{
		if (!MegalithBlueprintInternal::FindGraphByName(ABP, TargetGraphName))
		{
			return FMegalithActionResult::Error(FString::Printf(
	...
	for (const FVarSpec& V : Vars)
	{
		...
		FMegalithActionResult VR = FMegalithBlueprintVariableActions::HandleAddVariable(VarSub);
	...
	auto AddPropAccessNode = [&](const TArray<FString>& APath) -> FString
	{
		...
		FMegalithActionResult R = FMegalithBlueprintNodeActions::HandleAddPropertyAccessNode(Sub);
	...
	FString EntryNodeId;
	if (UEdGraph* TsGraph = MegalithBlueprintInternal::FindGraphByName(ABP, TargetGraphName))
	{
	...
	int32 Connected = 0;
	{
		...
		ConnSub->SetArrayField(TEXT("connections"), Conns);
		FMegalithActionResult CR = FMegalithBlueprintNodeActions::HandleConnectPinsBulk(ConnSub);
	...
	// Set the GroundSpeed threshold default on the compare's B pin (3.0 cm/s).
	{
		...
		FMegalithBlueprintNodeActions::HandleSetPinDefault(DefSub); // best-effort
	}
...
FMegalithActionResult FMegalithMotionMatchingScaffoldActions::HandleApplyLocomotionSpeedBand(const TSharedPtr<FJsonObject>& Params)
{
	const FString BpPath = Params->GetStringField(TEXT("bp_path"));
	if (BpPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: bp_path"));
	...
	if (!Params->TryGetNumberField(TEXT("walk_speed"), WalkSpeed))
		return FMegalithActionResult::Error(TEXT("Missing required parameter: walk_speed"));
	if (!Params->TryGetNumberField(TEXT("run_speed"), RunSpeed))
		return FMegalithActionResult::Error(TEXT("Missing required parameter: run_speed"));
	if (!Params->TryGetNumberField(TEXT("crouch_speed"), CrouchSpeed))
		return FMegalithActionResult::Error(TEXT("Missing required parameter: crouch_speed"));
	...
	if (!CmcClass)
	{
		return FMegalithActionResult::Error(TEXT("apply_locomotion_speed_band: UCharacterMovementComponent class not found"));
	}
	const MegalithBlueprintComponentResolver::FResult CmcResolved =
		MegalithBlueprintComponentResolver::Resolve(BP, FString(), CmcClass, /*bCreateIchOverride=*/false);
	if (!CmcResolved.IsValid())
