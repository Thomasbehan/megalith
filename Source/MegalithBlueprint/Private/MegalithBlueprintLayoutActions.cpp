...
void FMegalithBlueprintLayoutActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("auto_layout"),
		...
		FParamSchemaBuilder()
			...
			.Optional(TEXT("formatter"), TEXT("string"), TEXT("Formatter: 'auto' (default, prefers Blueprint Assist if available), 'blueprint_assist' (BA only, fails if unavailable), or 'megalith' (built-in Sugiyama)"), TEXT("auto"))
			.Build());
}
...
FMegalithActionResult FMegalithBlueprintLayoutActions::HandleAutoLayout(const TSharedPtr<FJsonObject>& Params)
{
	// --- Parse params ---
	FString AssetPath;
	...
	if (LayoutMode == TEXT("selected"))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("layout_mode='selected' requires a non-empty 'node_ids' array."));
		}
	}
	...
	if (Formatter == TEXT("auto") || Formatter == TEXT("blueprint_assist"))
	{
		...
		if (bBAAvailable)
		{
			...
			if (IMegalithGraphFormatter::Get().FormatGraph(Graph, NodesFormatted, ErrorMessage))
			{
				...
				FMegalithFormatterInfo Info = IMegalithGraphFormatter::Get().GetFormatterInfo(Graph);
	...
	if (LayoutNodes.Num() == 0)
	{
		...
		R->SetStringField(TEXT("formatter_used"), TEXT("megalith"));
