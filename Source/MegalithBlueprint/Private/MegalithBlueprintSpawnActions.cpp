	...
	const FString& MobilityStr)
{
	...
	if (!MobilityStr.IsEmpty())
	{
		...
		if (MegalithParamUtils::ParseMobility(MobilityStr, Mobility))
		{
...
void FMegalithBlueprintSpawnActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("spawn_blueprint_actor"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSpawnBlueprintActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("batch_spawn_blueprint_actors"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBatchSpawnBlueprintActors),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithBlueprintSpawnActions::HandleSpawnBlueprintActor(const TSharedPtr<FJsonObject>& Params)
{
	// PIE guard
	if (GEditor && GEditor->IsPlayingSessionInEditor())
	{
		return FMegalithActionResult::Error(TEXT("Cannot spawn Blueprint actors during Play-In-Editor session"));
	}
	...
	FString ClassPath = MegalithParamUtils::NormalizeBlueprintClassPath(BlueprintPath);
	...
	// Editor world
	UWorld* World = MegalithParamUtils::GetEditorWorld();
	if (!World)
	{
		return FMegalithActionResult::Error(TEXT("No editor world available. Is a level open?"));
	}

	// Parse optional transform
	FVector Location(0, 0, 0);
	MegalithParamUtils::ParseVector(Params, TEXT("location"), Location);

	FRotator Rotation(0, 0, 0);
	MegalithParamUtils::ParseRotator(Params, TEXT("rotation"), Rotation);

	FVector Scale(1, 1, 1);
	MegalithParamUtils::ParseVector(Params, TEXT("scale"), Scale);
	...
	if (!TargetLevel)
	{
		return FMegalithActionResult::Error(SublevelError);
	}

	// Begin transaction
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SpawnBPActor", "Megalith: Spawn Blueprint Actor"));
	...
	Result->SetArrayField(TEXT("location"), MegalithParamUtils::VectorToJsonArray(NewActor->GetActorLocation()));
	{
	...
	Result->SetArrayField(TEXT("scale"), MegalithParamUtils::VectorToJsonArray(NewActor->GetActorScale3D()));
...
FMegalithActionResult FMegalithBlueprintSpawnActions::HandleBatchSpawnBlueprintActors(const TSharedPtr<FJsonObject>& Params)
{
	// PIE guard
	if (GEditor && GEditor->IsPlayingSessionInEditor())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: count (must be >= 1)"));
	}
	int32 Count = FMath::RoundToInt32(CountD);
	if (Count > 1000)
	{
		return FMegalithActionResult::Error(TEXT("count exceeds maximum of 1000"));
	}
	...
	MegalithParamUtils::ParseVector(Params, TEXT("origin"), Origin);

	double Spacing = 200.0;
	Params->TryGetNumberField(TEXT("spacing"), Spacing);
	if (Spacing <= 0)
	{
		return FMegalithActionResult::Error(TEXT("spacing must be positive"));
	}
	...
	MegalithParamUtils::ParseVector(Params, TEXT("direction"), Direction);
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "BatchSpawnBP", "Megalith: Batch Spawn Blueprint Actors"));
	...
	for (int32 i = 0; i < Count; i++)
	{
		...
		EntryObj->SetArrayField(TEXT("location"), MegalithParamUtils::VectorToJsonArray(NewActor->GetActorLocation()));
