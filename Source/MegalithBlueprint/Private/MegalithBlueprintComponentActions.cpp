...
void FMegalithBlueprintComponentActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddComponent),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("rename_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRenameComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("reparent_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleReparentComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_component_property"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetComponentProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("duplicate_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
...
static FProperty* MegalithFindComponentProperty(UClass* ComponentClass, const FString& PropName)
{
...
FMegalithActionResult FMegalithBlueprintComponentActions::HandleAddComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	...
	{
		return FMegalithActionResult::Error(TEXT("component_class is required"));
	}
	...
	if (!CompClass)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Component class not found: %s"), *ClassName));
	}
	if (!CompClass->IsChildOf(UActorComponent::StaticClass()))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Class '%s' is not a UActorComponent"), *ClassName));
	}
	...
	if (!ParentName.IsEmpty())
	{
		...
		if (!ParentNode)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Parent component not found: %s"), *ParentName));
		}
...
FMegalithActionResult FMegalithBlueprintComponentActions::HandleRemoveComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (CompName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("component_name is required"));
	}

	USCS_Node* Node = FindSCSNodeByName(BP->SimpleConstructionScript, FName(*CompName));
	if (!Node)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Component not found: %s"), *CompName));
	}
...
FMegalithActionResult FMegalithBlueprintComponentActions::HandleRenameComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (CompName.IsEmpty()) return FMegalithActionResult::Error(TEXT("component_name is required"));
	if (NewName.IsEmpty())  return FMegalithActionResult::Error(TEXT("new_name is required"));
	if (FName(*NewName).IsNone()) return FMegalithActionResult::Error(TEXT("'None' is a reserved FName and cannot be used as a component name"));
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("A component named '%s' already exists"), *NewName));
	}
	...
	for (const FBPVariableDescription& Var : BP->NewVariables)
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Cannot rename component to '%s': a variable with that name already exists"), *NewName));
		}
	}
	...
	for (UEdGraph* Graph : BP->FunctionGraphs)
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Cannot rename component to '%s': a function with that name already exists"), *NewName));
		}
	}
...
FMegalithActionResult FMegalithBlueprintComponentActions::HandleReparentComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (!NewParent.IsEmpty())
	{
		...
		if (!NewParentNode)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("New parent component not found: %s"), *NewParent));
		}
		if (NewParentNode == Node)
		{
			return FMegalithActionResult::Error(TEXT("Cannot reparent a component to itself"));
		}
		...
		{
			return FMegalithActionResult::Error(TEXT("Cannot reparent component to one of its own descendants"));
		}
	}
...
FMegalithActionResult FMegalithBlueprintComponentActions::HandleSetComponentProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (CompName.IsEmpty()) return FMegalithActionResult::Error(TEXT("component_name is required"));
	if (PropName.IsEmpty()) return FMegalithActionResult::Error(TEXT("property_name is required"));

	// One resolver for the whole module — see MegalithBlueprintComponentResolver.h. Write intent
	...
	FProperty* Prop = MegalithFindComponentProperty(Template->GetClass(), PropName);
	if (!Prop)
	{
	...
	if (ObjProp)
	{
		...
		if (!bIsNone)
		{
			...
			NewObject = FMegalithAssetUtils::LoadAssetByPath(UObject::StaticClass(), Path);
			if (!NewObject)
			{
				return FMegalithActionResult::Error(FString::Printf(
	...
	using EComponentSource = MegalithBlueprintComponentResolver::ESource;
	...
	if (bNeedsStructuralRecompile)
	{
		...
		const MegalithBlueprintComponentResolver::FResult After =
			MegalithBlueprintComponentResolver::Resolve(
				BP, CompName, UActorComponent::StaticClass(), /*bCreateIchOverride=*/false);

		if (After.IsValid())
		{
			if (FProperty* AfterProp = MegalithFindComponentProperty(After.Template->GetClass(), PropName))
			{
...
FMegalithActionResult FMegalithBlueprintComponentActions::HandleDuplicateComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Component '%s' has no ComponentClass"), *CompName));
	}
	...
	if (!NewNode)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create duplicate SCS node"));
	}
