...
// =============================================================================
// MegalithPinTypeGrammarTest.cpp
//
// Table test for the shared pin-type grammar (MegalithCore/Public/MegalithPinTypeGrammar.h).
//
// WHY IT LIVES IN MegalithBlueprint AND NOT MegalithCore:
//   MegalithCore.Build.cs lists no BlueprintGraph, and the UEdGraphSchema_K2::PC_*
//   constants are dllimport'd from that module — referencing them from a MegalithCore
...
namespace MegalithPinTypeGrammarTest
{
	static const TCHAR* const FixtureEnumPackage = TEXT("/Game/Tests/Megalith/pintype/E_MegalithPinGrammarFixture");
	static const TCHAR* const FixtureEnumName    = TEXT("E_MegalithPinGrammarFixture");
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithPinTypeGrammarTokenTableTest,
	"Megalith.PinTypeGrammar.TokenTable",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithPinTypeGrammarTokenTableTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithPinTypeGrammarTest;
		...
		const TCHAR* ExpectedSubObjectName, EPinContainerType ExpectedContainer)
	{
		...
		FString Error;
		const bool bParsed = MegalithPinTypeGrammar::TryParsePinType(TypeStr, PinType, Error);
	...
	auto ExpectFail = [this](const FString& TypeStr)
	{
		...
		const FName Sentinel(TEXT("MegalithUntouchedSentinel"));
		FEdGraphPinType PinType;
	...
	ExpectFail(TEXT("enum:MegalithBogusEnumThatDoesNotExist"));
	ExpectFail(TEXT("struct:MegalithBogusStructThatDoesNotExist"));
	ExpectFail(TEXT("object:MegalithBogusClassThatDoesNotExist"));
	ExpectFail(TEXT("softobject:MegalithBogusClassThatDoesNotExist"));
	ExpectFail(TEXT("MegalithDefinitelyNotAPinType"));
	...
	ExpectFail(TEXT("map:string:object:MegalithBogusClassThatDoesNotExist"));
	...
	// not regress to PC_Enum, which is the whole of issue #115.
	{
		const FEdGraphPinType Legacy =
			MegalithPinTypeGrammar::ParsePinTypeFromString(TEXT("enum:ESlateVisibility"));
		...
		TestEqual(TEXT("enum: round-trips through PinTypeToString"),
			MegalithPinTypeGrammar::ContainerPrefix(Legacy) + MegalithPinTypeGrammar::PinTypeToString(Legacy),
