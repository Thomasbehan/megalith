...
namespace MegalithStringTableInternal
{
	static UStringTable* ResolveStringTable(const FString& AssetPath, FString& OutError)
	{
		UStringTable* StringTable = FMegalithAssetUtils::LoadAssetByPath<UStringTable>(AssetPath);
		if (!StringTable)
		{
...
void FMegalithBlueprintStringTableActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("read_string_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleReadStringTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_string_table_entries"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetStringTableEntries),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_string_table_entry"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveStringTableEntry),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("StringTable asset path"))
...
FMegalithActionResult FMegalithBlueprintStringTableActions::HandleReadStringTable(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithStringTableInternal;
...
FMegalithActionResult FMegalithBlueprintStringTableActions::HandleSetStringTableEntries(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithStringTableInternal;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: entries (array of {key, source_string})"));
	}

	FString Error;
	UStringTable* StringTable = ResolveStringTable(AssetPath, Error);
	if (!StringTable) { return FMegalithActionResult::Error(Error); }
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithBlueprintStringTableActions", "SetStringTableEntries", "Megalith Set StringTable Entries"));
	StringTable->Modify();
...
FMegalithActionResult FMegalithBlueprintStringTableActions::HandleRemoveStringTableEntry(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithStringTableInternal;
	...
	if (Key.IsEmpty())       { return FMegalithActionResult::Error(TEXT("Missing required parameter: key")); }
	...
	StringTable->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithBlueprintStringTableActions", "RemoveStringTableEntry", "Megalith Remove StringTable Entry"));
	StringTable->Modify();
