...
void FMegalithBlueprintBuildActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("build_blueprint_from_spec"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBuildBlueprintFromSpec),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithBlueprintBuildActions::HandleBuildBlueprintFromSpec(const TSharedPtr<FJsonObject>& Params)
{
	// ---- Load Blueprint ----
	FString AssetPath;
	...
	// ---- Begin transaction ----
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "BuildBPFromSpec", "Build Blueprint From Spec"));
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("variables"), VarSpecs))
		{
			for (int32 i = 0; i < VarSpecs.Num(); ++i)
			{
				...
				FMegalithActionResult Result = FMegalithBlueprintVariableActions::HandleAddVariable(SubParams);
				if (Result.bSuccess)
				{
					VariablesCreated++;
				}
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("components"), CompSpecs))
		{
			for (int32 i = 0; i < CompSpecs.Num(); ++i)
			{
				...
				FMegalithActionResult Result = FMegalithBlueprintComponentActions::HandleAddComponent(SubParams);
				if (Result.bSuccess)
				{
					ComponentsCreated++;
				}
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("nodes"), NodeSpecs))
		{
			for (int32 i = 0; i < NodeSpecs.Num(); ++i)
			{
				...
				FMegalithActionResult Result = FMegalithBlueprintNodeActions::HandleAddNode(SubParams);
				if (Result.bSuccess)
				{
					NodesCreated++;
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("connections"), ConnSpecs))
		{
			for (int32 i = 0; i < ConnSpecs.Num(); ++i)
			{
				...
				FMegalithActionResult Result = FMegalithBlueprintNodeActions::HandleConnectPins(SubParams);
				if (Result.bSuccess)
				{
					ConnectionsMade++;
				}
	...
	// ============================================================
	{
		...
		if (ParseJsonArray(Params, TEXT("pin_defaults"), PinSpecs))
		{
			for (int32 i = 0; i < PinSpecs.Num(); ++i)
			{
				...
				FMegalithActionResult Result = FMegalithBlueprintNodeActions::HandleSetPinDefault(SubParams);
				if (Result.bSuccess)
				{
					PinDefaultsSet++;
				}
	...
	if (bAutoCompile)
	{
		...
		FMegalithActionResult CResult = FMegalithBlueprintCompileActions::HandleCompileBlueprint(CompileParams);
		bCompiled = true;
