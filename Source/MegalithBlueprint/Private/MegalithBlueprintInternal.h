...
namespace MegalithBlueprintInternal
{
	/**
	...
	inline UBlueprint* LoadBlueprintFromParams(const TSharedPtr<FJsonObject>& Params, FString& OutAssetPath)
	{
		...
		UBlueprint* BP = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(OutAssetPath);
		if (BP) return BP;
	...
	// MegalithCore/Public/MegalithPinTypeGrammar.h. These two thin forwarders keep
	// the ~20 existing MegalithBlueprintInternal:: call sites working unchanged.
	inline FString PinTypeToString(const FEdGraphPinType& PinType)
	{
		return MegalithPinTypeGrammar::PinTypeToString(PinType);
	}

	inline FString ContainerPrefix(const FEdGraphPinType& PinType)
	{
		return MegalithPinTypeGrammar::ContainerPrefix(PinType);
	}
	...
	inline TSharedPtr<FJsonObject> SerializeNode(UEdGraphNode* Node, bool bSafeTitle = false)
	{
		...
		MegalithPropertyAccessReader::SerializePropertyAccessBlock(Node, NObj);
	 ...
	 */
	inline FMegalithActionResult ResolveDelegateOwnerAndProperty(
