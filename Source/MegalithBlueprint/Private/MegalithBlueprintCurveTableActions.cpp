...
namespace MegalithCurveTableInternal
{
	...
	{
		UCurveTable* CurveTable = FMegalithAssetUtils::LoadAssetByPath<UCurveTable>(AssetPath);
		if (!CurveTable)
		{
...
void FMegalithBlueprintCurveTableActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("read_curve_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleReadCurveTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_curve_table_keys"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetCurveTableKeys),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_curve_table_row"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddCurveTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("CurveTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_curve_table_row"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveCurveTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("CurveTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("rename_curve_table_row"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRenameCurveTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("CurveTable asset path"))
...
FMegalithActionResult FMegalithBlueprintCurveTableActions::HandleReadCurveTable(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithCurveTableInternal;
...
FMegalithActionResult FMegalithBlueprintCurveTableActions::HandleSetCurveTableKeys(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithCurveTableInternal;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: keys (array of {time, value})"));
	}

	FString Error;
	UCurveTable* CurveTable = ResolveCurveTable(AssetPath, Error);
	if (!CurveTable) { return FMegalithActionResult::Error(Error); }
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithBlueprintCurveTableActions", "SetCurveTableKeys", "Megalith Set CurveTable Keys"));
	CurveTable->Modify();
...
FMegalithActionResult FMegalithBlueprintCurveTableActions::HandleAddCurveTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithCurveTableInternal;
	...
	CurveTable->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithBlueprintCurveTableActions", "AddCurveTableRow", "Megalith Add CurveTable Row"));
	CurveTable->Modify();
...
FMegalithActionResult FMegalithBlueprintCurveTableActions::HandleRemoveCurveTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithCurveTableInternal;
	...
	CurveTable->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithBlueprintCurveTableActions", "RemoveCurveTableRow", "Megalith Remove CurveTable Row"));
	CurveTable->Modify();
	FCurveTableEditorUtils::BroadcastPreChange(CurveTable, FCurveTableEditorUtils::ECurveTableChangeInfo::RowList);
...
FMegalithActionResult FMegalithBlueprintCurveTableActions::HandleRenameCurveTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithCurveTableInternal;
	...
	CurveTable->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithBlueprintCurveTableActions", "RenameCurveTableRow", "Megalith Rename CurveTable Row"));
	CurveTable->Modify();
	FCurveTableEditorUtils::BroadcastPreChange(CurveTable, FCurveTableEditorUtils::ECurveTableChangeInfo::RowList);
