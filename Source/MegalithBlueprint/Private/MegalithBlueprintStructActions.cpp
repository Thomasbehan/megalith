...
#include "Reflection/MegalithReflectionReader.h"
...
void FMegalithBlueprintStructActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_user_defined_struct"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateUserDefinedStruct),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_user_defined_enum"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateUserDefinedEnum),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_data_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateDataTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_data_table_row"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddDataTableRow),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("get_data_table_rows"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetDataTableRows),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path, e.g. /Game/Data/DT_Weapons"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_data_asset"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateDataAsset),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("seed_data_asset"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSeedDataAsset),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithBlueprintStructActions::HandleCreateUserDefinedStruct(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: save_path"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: fields (array of {name, type, default_value?})"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid save_path — must contain at least one '/': %s"), *SavePath));
	}
	FString AssetName = SavePath.Mid(LastSlash + 1);
	if (AssetName.IsEmpty())
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("save_path must not end with '/': %s"), *SavePath));
	}
	...
	if (!Package)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create package at path: %s"), *SavePath));
	}
	Package->FullyLoad();
	...
	if (!Struct)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("FStructureEditorUtils::CreateUserDefinedStruct failed for: %s"), *AssetName));
	}
	...
	for (const TSharedPtr<FJsonValue>& FieldVal : *FieldsArray)
	{
		...
		FEdGraphPinType PinType = MegalithPinTypeGrammar::ParsePinTypeFromString(TypeStr);
...
FMegalithActionResult FMegalithBlueprintStructActions::HandleCreateUserDefinedEnum(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: values (array of strings)"));
	}
	...
	if (!Enum)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("FEnumEditorUtils::CreateUserDefinedEnum failed for: %s"), *AssetName));
	}
...
FMegalithActionResult FMegalithBlueprintStructActions::HandleCreateDataTable(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: row_struct"));
	}
	...
	if (!DataTable)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create UDataTable: %s"), *AssetName));
	}
...
FMegalithActionResult FMegalithBlueprintStructActions::HandleAddDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path"));
	}

	FString RowName = Params->GetStringField(TEXT("row_name"));
	if (RowName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: row_name"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: values (JSON object of column->value)"));
	}

	// Load the DataTable
	UDataTable* DataTable = FMegalithAssetUtils::LoadAssetByPath<UDataTable>(AssetPath);
	if (!DataTable)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("DataTable not found: %s"), *AssetPath));
	}

	const UScriptStruct* RowStruct = DataTable->GetRowStruct();
	if (!RowStruct)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("DataTable '%s' has no RowStruct set"), *AssetPath));
	}
	...
	for (const auto& Pair : (*ValuesObj)->Values)
	{
		...
		if (FMegalithReflectionWalker::ResolveUserDefinedEnumToken(Prop, ValueStr, ResolvedEnumValue))
		{
...
FMegalithActionResult FMegalithBlueprintStructActions::HandleGetDataTableRows(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithBlueprintStructActions::HandleCreateDataAsset(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: class_name"));
	}
	...
	FScopedTransaction Transaction(NSLOCTEXT("MegalithBlueprintStructActions",
		"CreateDataAsset", "Megalith Create Data Asset"));
	NewAsset->Modify();
	...
	for (TFieldIterator<FProperty> It(ResolvedClass); It; ++It)
	{
		...
		if (Prop->HasAnyPropertyFlags(CPF_Transient | CPF_Deprecated))
			continue;
		MegalithEditCradle::FireFullCradle(NewAsset, Prop);
	}
...
namespace MegalithSeedDataAssetInternal
{
	// Serialise an FDryRunReport's field writes into the response (same shape as
	// FMegalithDryRunGuard::ReportToJson, inlined here to avoid extra includes).
...
FMegalithActionResult FMegalithBlueprintStructActions::HandleSeedDataAsset(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithSeedDataAssetInternal;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: tree (JSON object of property->value)"));
	}
	...
	if (bDryRun)
	{
		...
		FDryRunReport Report = FMegalithReflectionWalker::InspectTree(Tree, ResolvedClass, CDO, Spec);
	...
	// Engine edit cradle + walker fill (mirror MegalithBlueprintBulkFillAdapter).
	NewAsset->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT("MegalithBlueprintStructActions",
		"SeedDataAsset", "Megalith Seed Data Asset"));
	NewAsset->Modify();
	NewAsset->PreEditChange(nullptr);

	FDryRunReport Report = FMegalithReflectionWalker::WriteTree(Tree, ResolvedClass, NewAsset, NewAsset, Spec);
	...
	{
		FProperty* TopProp = FMegalithReflectionWalker::FindPropertyForwarding(ResolvedClass, MegalithKeyToString(KV.Key));
		if (TopProp)
		{
			MegalithEditCradle::ReparentTransientInstancedSubobjects(NewAsset, TopProp);
			MegalithEditCradle::FireFullCradle(NewAsset, TopProp);
		}
	}
		...
		for (const auto& KV : Tree->Values)
		{
			...
			if (TopProp)
			{
				...
				Values->SetField(TopProp->GetName(),
					FMegalithReflectionReader::PropertyToJsonValue(TopProp, ValuePtr, NewAsset));
			}
		}
