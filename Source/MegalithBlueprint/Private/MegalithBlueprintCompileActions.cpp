...
void FMegalithBlueprintCompileActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("compile_blueprint"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCompileBlueprint),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("validate_blueprint"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateBlueprint),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("create_blueprint"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateBlueprint),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("duplicate_blueprint"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateBlueprint),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("get_dependencies"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetDependencies),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("save_asset"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSaveAsset),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("save_dirty_assets"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSaveDirtyAssets),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithBlueprintCompileActions::HandleCompileBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
...
FMegalithActionResult FMegalithBlueprintCompileActions::HandleValidateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
...
FMegalithActionResult FMegalithBlueprintCompileActions::HandleCreateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: parent_class"));
	}
	...
	if (!ParentClass)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Parent class not found: %s"), *ParentClassName));
	}
	...
	if (!NewBP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create Blueprint at: %s"), *SavePath));
	}
	...
	if (CDO)
	{
		...
		FScopedTransaction Transaction(NSLOCTEXT("MegalithBlueprintCompileActions",
			"CreateBlueprint", "Megalith Create Blueprint"));
		CDO->Modify();
		for (TFieldIterator<FProperty> It(CDO->GetClass()); It; ++It)
		{
			...
			if (Prop->HasAnyPropertyFlags(CPF_Transient | CPF_Deprecated))
				continue;
			MegalithEditCradle::FireFullCradle(CDO, Prop);
		}
	}
...
FMegalithActionResult FMegalithBlueprintCompileActions::HandleDuplicateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (NewPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: new_path"));
	}
...
FMegalithActionResult FMegalithBlueprintCompileActions::HandleGetDependencies(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
...
FMegalithActionResult FMegalithBlueprintCompileActions::HandleSaveAsset(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!bSaved)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to save asset: %s"), *AssetPath));
	}
	return FMegalithActionResult::Success(Root);
}
...
FMegalithActionResult FMegalithBlueprintCompileActions::HandleSaveDirtyAssets(const TSharedPtr<FJsonObject>& Params)
{
