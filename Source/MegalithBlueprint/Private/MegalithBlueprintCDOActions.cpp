...
namespace MegalithCDOPhase1Internal
{
	FMegalithActionResult HandleSetCDOProperties(const TSharedPtr<FJsonObject>& Params);
	FMegalithActionResult HandleDescribeCDOSchema(const TSharedPtr<FJsonObject>& Params);
}

// --- Registration ---

void FMegalithBlueprintCDOActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("get_cdo_properties"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetCDOProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_cdo_property"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetCDOProperty),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_property_at_path"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetPropertyAtPath),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint or UObject asset path (e.g. /Game/Data/DA_MyData)"))
	...
	// Routes through FMegalithBulkFillRegistry's "blueprint" adapter. Feature parity
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_cdo_properties"),
		...
		FMegalithActionHandler::CreateStatic(&MegalithCDOPhase1Internal::HandleSetCDOProperties),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint or UObject asset path (e.g. /Game/Data/DA_MyData)"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("describe_cdo_schema"),
		...
		FMegalithActionHandler::CreateStatic(&MegalithCDOPhase1Internal::HandleDescribeCDOSchema),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint or UObject asset path (e.g. /Game/Data/DA_MyData)"))
			.Build());
}
...
// FMegalithReflectionReader (MegalithCore) so the CDO read path, the DataAsset
...
namespace MegalithCDOInternal
{
	TSharedPtr<FJsonValue> PropertyToJsonValue(FProperty* Prop, const void* ValuePtr, const UObject* CDO)
	{
		return FMegalithReflectionReader::PropertyToJsonValue(Prop, ValuePtr, CDO);
	}
}

// --- Handler ---

FMegalithActionResult FMegalithBlueprintCDOActions::HandleGetCDOProperties(const TSharedPtr<FJsonObject>& Params)
{
	...
	else
	{
		...
		TargetObject = FMegalithAssetUtils::LoadAssetByPath(AssetPath);
		if (TargetObject)
		{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Asset not found or has no class: %s"), *AssetPath));
	}
	...
	for (TFieldIterator<FProperty> It(TargetClass, EFieldIteratorFlags::IncludeSuper, EFieldIteratorFlags::ExcludeDeprecated); It; ++It)
	{
		...
		PropObj->SetField(TEXT("value"), MegalithCDOInternal::PropertyToJsonValue(Prop, ValuePtr, TargetObject));
...
FMegalithActionResult FMegalithBlueprintCDOActions::HandleSetCDOProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: property_name"));
	}
	...
	BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (BP && BP->GeneratedClass)
	{
		TargetClass = BP->GeneratedClass;
		TargetObject = TargetClass->GetDefaultObject(false);
	...
	// guard pulls dry_run + strict from Params per FMegalithDryRunGuard contract.
	FMegalithDryRunGuard DryRunGuard(Params);
	if (DryRunGuard.IsDryRun())
	{
		...
		const FDryRunReport Report = FMegalithReflectionWalker::InspectTree(
	...
	FScopedTransaction Transaction(NSLOCTEXT("MegalithBlueprintCDOActions", "SetCDOProperty", "Megalith Set CDO Property"));
	TargetObject->Modify();
	...
	MegalithEditCradle::ReparentTransientInstancedSubobjects(TargetObject, Prop);
	...
	MegalithEditCradle::FireFullCradle(TargetObject, Prop);
...
//
// Resolves a dotted+bracket path to ONE leaf via FMegalithReflectionWalker::
...
// ---------------------------------------------------------------------------
FMegalithActionResult FMegalithBlueprintCDOActions::HandleSetPropertyAtPath(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("set_property_at_path requires params"));
	}
	...
	if (!JsonVal.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("'value' field missing or null"));
	}
	...
	if (DryRunGuard.IsDryRun())
	{
		FMegalithReflectionWalker::FPathResolveResult Resolved =
			FMegalithReflectionWalker::ResolvePath(TargetClass, TargetObject, Path, /*bCreateMissingKeys=*/false);
		if (!Resolved.bOk)
		{
			return FMegalithActionResult::Error(FString::Printf(
		...
		const FBulkFillFieldWrite W = FMegalithReflectionWalker::WriteLeaf(
		...
		FMegalithActionResult Result = DryRunGuard.MakeDryRunResponse(Report);
	...
	TargetObject->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT("MegalithBlueprintCDOActions", "SetPropertyAtPath", "Megalith Set Property At Path"));
	TargetObject->Modify();

	// Resolve the path on the live object (may mutate map storage if create_missing_keys).
	FMegalithReflectionWalker::FPathResolveResult Resolved =
		FMegalithReflectionWalker::ResolvePath(TargetClass, TargetObject, Path, bCreateMissingKeys);
	if (!Resolved.bOk)
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(FString::Printf(
	...
	FProperty* HeadProp = FMegalithReflectionWalker::FindPropertyForwarding(TargetClass, HeadMember);
	if (HeadProp)
	{
		FEditPropertyChain PropertyChain;
	...
	if (HeadProp)
	{
		MegalithEditCradle::ReparentTransientInstancedSubobjects(TargetObject, HeadProp);
		MegalithEditCradle::FireFullCradle(TargetObject, HeadProp);
	}
	...
	// --- Optional save to disk (pattern: MegalithAudioPerceptionActions.cpp:96).
	bool bSaved = false;
	FString SaveError;
	if (bSave)
	{
...
// Thin wrappers that route through FMegalithBulkFillRegistry's "blueprint"
// adapter (registered from FMegalithBlueprintModule::StartupModule).
// ---------------------------------------------------------------------------
...
namespace MegalithCDOPhase1Internal
{
	FMegalithActionResult HandleSetCDOProperties(const TSharedPtr<FJsonObject>& Params)
	{
		if (!Params.IsValid())
		{
			return FMegalithActionResult::Error(TEXT("set_cdo_properties requires params"));
		}
		...
		if (!FMegalithBulkFillRegistry::Get().HasAdapter(TEXT("blueprint")))
		{
			return FMegalithActionResult::Error(
	...
	FMegalithActionResult HandleDescribeCDOSchema(const TSharedPtr<FJsonObject>& Params)
	{
		if (!Params.IsValid())
		{
			return FMegalithActionResult::Error(TEXT("describe_cdo_schema requires params"));
		}
		...
		const FSchemaDescriptor Root = FMegalithBulkFillRegistry::Get().DispatchDescribe(
			TEXT("blueprint"), AssetPath);

		// Serialise the descriptor tree to JSON. We mirror the central dispatcher's
		// shape (MegalithBulkFillActions.cpp::DescriptorToJson) but inline a tiny
		// recursive emitter here to avoid coupling this module to MegalithCore's
		...
		return FMegalithActionResult::Success(ToJson(Root));
	}
} // namespace MegalithCDOPhase1Internal
