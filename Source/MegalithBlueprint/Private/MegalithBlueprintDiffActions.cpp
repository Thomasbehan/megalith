...
void FMegalithBlueprintDiffActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("compare_blueprints"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCompareBlueprints),
		FParamSchemaBuilder()
...
namespace
{
	...
	TArray<FVarInfo> CollectVariables(UBlueprint* BP)
	{
		...
		for (const FBPVariableDescription& Var : BP->NewVariables)
		{
			...
			Info.Type = MegalithBlueprintInternal::ContainerPrefix(Var.VarType) +
...
FMegalithActionResult FMegalithBlueprintDiffActions::HandleCompareBlueprints(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (PathA.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path_a"));
	}
	if (PathB.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path_b"));
	}

	UBlueprint* BPA = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(PathA);
	if (!BPA)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint A not found: %s"), *PathA));
	}

	UBlueprint* BPB = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(PathB);
	if (!BPB)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint B not found: %s"), *PathB));
	}
