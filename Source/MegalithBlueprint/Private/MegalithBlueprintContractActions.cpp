...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithBpContract, Log, All);
...
// ============================================================
namespace MegalithBpContractInternal
{
	...
	static UStruct* ContractResolveSide(const FString& Side, UClass*& OutClass, UBlueprint*& OutBP, FString& OutKind, FString& OutError)
	{
		...
		if (Side.Contains(TEXT("/")))
		{
			...
			FString Resolved;
			if (UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Synthetic, Resolved))
			{
		...
		// Native class name resolution (mirrors MegalithBlueprintStructActions resolver).
...
void FMegalithBlueprintContractActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("compare_class_variable_contract"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCompareClassVariableContract),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("promote_variables_to_parent"),
		...
		FMegalithActionHandler::CreateStatic(&HandlePromoteVariablesToParent),
		FParamSchemaBuilder()
	...
	UE_LOG(LogMegalithBpContract, Log, TEXT("MegalithBlueprint Contract: registered 2 actions"));
}
...
FMegalithActionResult FMegalithBlueprintContractActions::HandleCompareClassVariableContract(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithBpContractInternal;

	const FString LeftSide  = Params->GetStringField(TEXT("left"));
	const FString RightSide = Params->GetStringField(TEXT("right"));
	if (LeftSide.IsEmpty())  return FMegalithActionResult::Error(TEXT("Missing required parameter: left"));
	if (RightSide.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: right"));
	...
	if (!LeftStruct) return FMegalithActionResult::Error(LeftErr);
	...
	if (!RightStruct) return FMegalithActionResult::Error(RightErr);
...
FMegalithActionResult FMegalithBlueprintContractActions::HandlePromoteVariablesToParent(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithBpContractInternal;
	...
	if (!BP->GeneratedClass)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint '%s' has no GeneratedClass (needs compile)."), *AssetPath));
	}
	...
	if (!Params->TryGetArrayField(TEXT("variables"), RequestedVars) || !RequestedVars || RequestedVars->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: variables (non-empty array of names)"));
	}
