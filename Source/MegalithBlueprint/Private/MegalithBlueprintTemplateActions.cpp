...
void FMegalithBlueprintTemplateActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("apply_template"),
		...
		FMegalithActionHandler::CreateStatic(&HandleApplyTemplate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("list_templates"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListTemplates),
		FParamSchemaBuilder()
			.Build());
}
		...
		const TMap<FString, FString>& Defaults)
	{
		...
		if (UserParams.IsValid())
		{
			for (const auto& Pair : UserParams->Values)
			{
				...
				{
					Result.Add(MegalithKeyToString(Pair.Key), Pair.Value->AsString());
				}
				else if (Pair.Value->Type == EJson::Number)
				{
					Result.Add(MegalithKeyToString(Pair.Key), FString::SanitizeFloat(Pair.Value->AsNumber()));
				}
				else if (Pair.Value->Type == EJson::Boolean)
				{
					Result.Add(MegalithKeyToString(Pair.Key), Pair.Value->AsBool() ? TEXT("true") : TEXT("false"));
				}
			}
		}
...
FMegalithActionResult FMegalithBlueprintTemplateActions::HandleListTemplates(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithBlueprintTemplateActions::HandleApplyTemplate(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: template_name"));
	}
	...
	FMegalithActionResult BatchResult = FMegalithBlueprintNodeActions::HandleBatchExecute(BatchParams);
