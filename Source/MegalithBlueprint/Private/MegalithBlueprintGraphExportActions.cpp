...
void FMegalithBlueprintGraphExportActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("export_graph"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExportGraph),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("copy_nodes"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCopyNodes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("duplicate_graph"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateGraph),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
...
namespace
{
	...
	TSharedPtr<FJsonObject> SerializeNodeExtended(UEdGraphNode* Node)
	{
		...
		TSharedPtr<FJsonObject> NObj = MegalithBlueprintInternal::SerializeNode(Node);
...
FMegalithActionResult FMegalithBlueprintGraphExportActions::HandleExportGraph(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
...
FMegalithActionResult FMegalithBlueprintGraphExportActions::HandleCopyNodes(const TSharedPtr<FJsonObject>& Params)
{
	// Load source Blueprint
	FString SourceAssetPath = Params->GetStringField(TEXT("source_asset"));
	if (SourceAssetPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: source_asset"));
	}
	UBlueprint* SourceBP = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(SourceAssetPath);
	if (!SourceBP)
	{
		// Try level blueprint fallback
		SourceBP = MegalithBlueprintInternal::TryLoadLevelBlueprint(SourceAssetPath);
	}
	if (!SourceBP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Source Blueprint not found: %s"), *SourceAssetPath));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: target_asset"));
	}
	UBlueprint* TargetBP = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(TargetAssetPath);
	if (!TargetBP)
	{
		TargetBP = MegalithBlueprintInternal::TryLoadLevelBlueprint(TargetAssetPath);
	}
	if (!TargetBP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Target Blueprint not found: %s"), *TargetAssetPath));
	}

	// Find source graph
	FString SourceGraphName = Params->GetStringField(TEXT("source_graph"));
	UEdGraph* SourceGraph = MegalithBlueprintInternal::FindGraphByName(SourceBP, SourceGraphName);
	if (!SourceGraph)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Source graph not found: %s"), *SourceGraphName));
	}

	// Find target graph
	FString TargetGraphName = Params->GetStringField(TEXT("target_graph"));
	UEdGraph* TargetGraph = MegalithBlueprintInternal::FindGraphByName(TargetBP, TargetGraphName);
	if (!TargetGraph)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Target graph not found: %s"), *TargetGraphName));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: node_ids"));
	}
...
FMegalithActionResult FMegalithBlueprintGraphExportActions::HandleDuplicateGraph(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: graph_name"));
	}
	...
	UEdGraph* SourceGraph = MegalithBlueprintInternal::FindGraphByName(BP, GraphName);
	if (!SourceGraph)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Graph not found: %s"), *GraphName));
	}
	...
	if (MegalithBlueprintInternal::FindGraphByName(BP, NewName))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("A graph named '%s' already exists in this Blueprint"), *NewName));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Schema does not allow duplication of graph '%s'"), *GraphName));
	}
	...
	if (!DuplicatedGraph)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to duplicate graph '%s'"), *GraphName));
	}
