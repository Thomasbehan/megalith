...
// delegate to FMegalithReflectionWalker (MegalithCore). Writes reuse the
...
namespace MegalithDataTableInternal
{
	...
	// values object, so FMegalithReflectionWalker (which matches on internal names)
	...
	static TSharedPtr<FJsonObject> ToCanonicalTree(const UScriptStruct* RowStruct, const TSharedPtr<FJsonObject>& FriendlyValues)
	{
		...
		{
			if (FProperty* Prop = ResolveRowProperty(RowStruct, MegalithKeyToString(Pair.Key)))
			{
				Tree->SetField(Prop->GetName(), Pair.Value);
			}
			else
			{
				Tree->SetField(MegalithKeyToString(Pair.Key), Pair.Value);
			}
		}
		return Tree;
	}
	...
	static TArray<TSharedPtr<FJsonValue>> BuildSchemaArray(UScriptStruct* RowStruct)
	{
		...
		const FSchemaDescriptor Root = FMegalithReflectionWalker::DescribeStruct(RowStruct);
...
void FMegalithBlueprintDataTableActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("read_data_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleReadDataTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("describe_data_table_schema"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDescribeDataTableSchema),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path, e.g. /Game/Data/DT_Weapons"))
			.Build());
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_data_table_rows"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetDataTableRows),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path, e.g. /Game/Data/DT_Weapons"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_data_table_row"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveDataTableRow),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("rename_data_table_row"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRenameDataTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("duplicate_data_table_row"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateDataTableRow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("export_data_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExportDataTable),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),       TEXT("DataTable asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("import_data_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleImportDataTable),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("DataTable asset path"))
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleReadDataTable(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithDataTableInternal;
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleDescribeDataTableSchema(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithDataTableInternal;
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleSetDataTableRows(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithDataTableInternal;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: rows (array of {row_name, values, mode?})"));
	}
	...
	FMegalithDryRunGuard Guard(Params);
	...
	if (!bDryRun)
	{
		...
		Transaction = MakeUnique<FScopedTransaction>(NSLOCTEXT(
			"MegalithBlueprintDataTableActions", "SetDataTableRows", "Megalith Set DataTable Rows"));
		DataTable->Modify();
	}
	...
	for (const TSharedPtr<FJsonValue>& RowVal : *RowsArray)
	{
		...
		FDryRunReport RowReport = bDryRun
			? FMegalithReflectionWalker::InspectTree(CanonicalTree, RowStruct, RowData, Spec)
			: FMegalithReflectionWalker::WriteTree(CanonicalTree, RowStruct, RowData, nullptr, Spec);
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleRemoveDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithDataTableInternal;

	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	const FString RowName = Params->GetStringField(TEXT("row_name"));
	if (AssetPath.IsEmpty()) { return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path")); }
	if (RowName.IsEmpty())   { return FMegalithActionResult::Error(TEXT("Missing required parameter: row_name")); }

	const UScriptStruct* RowStruct = nullptr;
	FString Error;
	UDataTable* DataTable = ResolveDataTable(AssetPath, RowStruct, Error);
	if (!DataTable) { return FMegalithActionResult::Error(Error); }
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleRenameDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithDataTableInternal;
	...
	if (AssetPath.IsEmpty()) { return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path")); }
	if (OldName.IsEmpty())   { return FMegalithActionResult::Error(TEXT("Missing required parameter: old_name")); }
	if (NewName.IsEmpty())   { return FMegalithActionResult::Error(TEXT("Missing required parameter: new_name")); }
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleDuplicateDataTableRow(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithDataTableInternal;
	...
	const FString NewName = Params->GetStringField(TEXT("new_name"));
	if (AssetPath.IsEmpty())  { return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path")); }
	if (SourceRow.IsEmpty())  { return FMegalithActionResult::Error(TEXT("Missing required parameter: source_row")); }
	if (NewName.IsEmpty())    { return FMegalithActionResult::Error(TEXT("Missing required parameter: new_name")); }
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleExportDataTable(const TSharedPtr<FJsonObject>& Params)
{
#if WITH_EDITOR
	using namespace MegalithDataTableInternal;
	...
	return FMegalithActionResult::Success(Root);
#else
	return FMegalithActionResult::Error(TEXT("export_data_table requires an editor build (GetTableAsJSON/CSV are WITH_EDITOR-only)."));
#endif
}
...
FMegalithActionResult FMegalithBlueprintDataTableActions::HandleImportDataTable(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithDataTableInternal;
	...
	if (AssetPath.IsEmpty()) { return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path")); }
	if (Text.IsEmpty())      { return FMegalithActionResult::Error(TEXT("Missing required parameter: text")); }
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithBlueprintDataTableActions", "ImportDataTable", "Megalith Import DataTable"));
	DataTable->Modify();
