	...
	if (!TargetClassName.IsEmpty())
	{
		...
		UBlueprint* TargetBP = nullptr;
		{
			...
			if (BpPath.Contains(TEXT("/")))
			{
				...
				TargetBP = MegalithBlueprintInternal::LoadBlueprintFromParams(Synthetic, ResolvedPath);
			}
		}
...
// ============================================================
//  MegalithBlueprintInternal helpers
// ============================================================

bool MegalithBlueprintInternal::HasCustomEventNamed(UBlueprint* BP, FName EventName)
{
...
void FMegalithBlueprintNodeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddNode),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("remove_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveNode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("connect_pins"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConnectPins),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),   TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("disconnect_pins"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDisconnectPins),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),   TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_pin_default"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetPinDefault),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_node_position"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetNodePosition),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("Blueprint asset path"))
			.Required(TEXT("node_id"),     TEXT("string"), TEXT("Node ID"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("resolve_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleResolveNode),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_nodes_bulk"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddNodesBulk),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("connect_pins_bulk"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConnectPinsBulk),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),   TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_pin_defaults_bulk"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetPinDefaultsBulk),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_timeline"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddTimeline),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),    TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_event_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddEventNode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_comment_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddCommentNode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("get_timeline_data"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetTimelineData),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),     TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_timeline_track"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddTimelineTrack),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),     TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_timeline_keys"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetTimelineKeys),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),     TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("promote_pin_to_variable"),
		...
		FMegalithActionHandler::CreateStatic(&HandlePromotePinToVariable),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),     TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_property_access"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddPropertyAccess),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"),  TEXT("Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_property_access_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddPropertyAccessNode),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint not found: %s"), *AssetPath));
	}

	FString NodeType = Params->GetStringField(TEXT("node_type"));
	if (NodeType.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: node_type"));
	}
	...
	UEdGraph* Graph = MegalithBlueprintInternal::FindGraphByName(BP, GraphName);
	if (!Graph)
	{
	...
	if (NodeType == TEXT("CallFunction"))
	{
		...
		if (FuncName.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("CallFunction node requires 'function_name'"));
		}
		...
		{
			return FMegalithActionResult::Error(RefError);
		}
	...
	else if (NodeType == TEXT("VariableGet"))
	{
		...
		if (VarName.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("VariableGet node requires 'variable_name'"));
		}
	...
	else if (NodeType == TEXT("VariableSet"))
	{
		...
		if (VarName.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("VariableSet node requires 'variable_name'"));
		}
	...
	else if (NodeType == TEXT("CustomEvent"))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("CustomEvent node requires 'event_name'"));
		}

		if (MegalithBlueprintInternal::HasCustomEventNamed(BP, FName(*EventName)))
		{
			return FMegalithActionResult::Error(FString::Printf(
	...
	else if (NodeType == TEXT("MacroInstance"))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("MacroInstance node requires 'macro_name'"));
		}
		...
		{
			UBlueprint* MacroBP = FMegalithAssetUtils::LoadAssetByPath<UBlueprint>(MacroBPPath);
			if (MacroBP)
			{
	...
	else if (NodeType == TEXT("SpawnActorFromClass"))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("SpawnActorFromClass node requires 'actor_class'"));
		}
	...
	else if (NodeType == TEXT("DynamicCast"))
	{
		...
		if (CastClassName.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("DynamicCast node requires 'cast_class' (e.g. cast_class=MyPawn)"));
		}
	...
	else if (NodeType == TEXT("MakeStruct"))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("MakeStruct node requires 'struct_type' (e.g. struct_type=Vector)"));
		}
	...
	else if (NodeType == TEXT("BreakStruct"))
	{
		...
		if (StructType.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("BreakStruct node requires 'struct_type' (e.g. struct_type=Vector)"));
		}
	...
	else if (NodeType == TEXT("SwitchOnEnum"))
	{
		...
		if (EnumType.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("SwitchOnEnum node requires 'enum_type' (e.g. enum_type=ECollisionChannel, or a /Script or /Game asset path for a UserDefinedEnum)"));
		}
	...
	else if (NodeType == TEXT("Return"))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Return nodes can only be added to function graphs, not event graphs"));
		}
	...
	else if (NodeType == TEXT("ComponentBoundEvent"))
	{
		...
		FObjectProperty* CompProp = MegalithBlueprintInternal::FindComponentProperty(BP, FName(*CompNameStr));
		if (!CompProp)
		{
			return FMegalithActionResult::Error(FString::Printf(
		...
		FMulticastDelegateProperty* DelegateProp = MegalithBlueprintInternal::FindMulticastDelegateProperty(
	...
	else if (NodeType == TEXT("AddDelegate"))
	{
		...
		FMegalithActionResult Resolved = MegalithBlueprintInternal::ResolveDelegateOwnerAndProperty(
	...
	if (!NewNode)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create node — NewObject returned null"));
	}
	...
	TSharedPtr<FJsonObject> Root = MegalithBlueprintInternal::SerializeNode(NewNode, bGenericFallback);
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddPropertyAccess(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("add_property_access requires 'member_class'"));
	}

	const FString MemberName = Params->GetStringField(TEXT("member_name"));
	if (MemberName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("add_property_access requires 'member_name'"));
	}
	...
	if (!NewNode)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create property-access node — NewObject returned null"));
	}
	...
	TSharedPtr<FJsonObject> Root = MegalithBlueprintInternal::SerializeNode(NewNode);
	Root->SetStringField(TEXT("asset_path"), AssetPath);
	Root->SetStringField(TEXT("graph"), Graph->GetName());
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddPropertyAccessNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("add_property_access_node requires a non-empty 'path' array of strings"));
	}
	for (const TSharedPtr<FJsonValue>& Elem : *PathArr)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Each 'path' element must be a non-empty string"));
		}
		Path.Add(S);
	}
	...
	if (!Node)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create UK2Node_PropertyAccess — NewObject returned null"));
	}
	...
	TSharedPtr<FJsonObject> Root = MegalithBlueprintInternal::SerializeNode(Node);
	Root->SetStringField(TEXT("asset_path"), AssetPath);
	Root->SetStringField(TEXT("graph"), Graph->GetName());
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleRemoveNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (NodeId.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: node_id"));
	}

	FString GraphName = Params->GetStringField(TEXT("graph_name"));
	UEdGraphNode* Node = MegalithBlueprintInternal::FindNodeById(BP, GraphName, NodeId);
	if (!Node)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node not found: %s"), *NodeId));
	}
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleConnectPins(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (SourceNodeId.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: source_node"));
	if (SourcePinName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: source_pin"));
	if (TargetNodeId.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: target_node"));
	if (TargetPinName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required parameter: target_pin"));
	...
	UEdGraphNode* SrcNode = MegalithBlueprintInternal::FindNodeById(BP, GraphName, SourceNodeId, &SrcMatchGraphs);
	if (!SrcNode)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Source node not found: %s"), *SourceNodeId));
	}
	...
	UEdGraphNode* TgtNode = MegalithBlueprintInternal::FindNodeById(BP, GraphName, TargetNodeId, &TgtMatchGraphs);
	if (!TgtNode)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Target node not found: %s"), *TargetNodeId));
	}
	...
	UEdGraphPin* SrcPin = MegalithBlueprintInternal::FindPinOnNode(SrcNode, SourcePinName, EGPD_MAX, &SrcAvailPins);
	if (!SrcPin)
	{
		return FMegalithActionResult::Error(FString::Printf(
	...
	UEdGraphPin* TgtPin = MegalithBlueprintInternal::FindPinOnNode(TgtNode, TargetPinName, EGPD_MAX, &TgtAvailPins);
	if (!TgtPin)
	{
		return FMegalithActionResult::Error(FString::Printf(
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleDisconnectPins(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (PinName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: pin_name"));
	}
	...
	FString AvailPins;
	UEdGraphPin* Pin = MegalithBlueprintInternal::FindPinOnNode(Node, PinName, EGPD_MAX, &AvailPins);
	if (!Pin)
	{
		return FMegalithActionResult::Error(FString::Printf(
	...
	else
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("'target_pin' is required when 'target_node' is specified"));
		}

		UEdGraphNode* TargetNode = MegalithBlueprintInternal::FindNodeById(BP, GraphName, TargetNodeId);
		if (!TargetNode)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Target node not found: %s"), *TargetNodeId));
		}
		...
		UEdGraphPin* TargetPin = MegalithBlueprintInternal::FindPinOnNode(TargetNode, TargetPinName, EGPD_MAX, &TgtAvailPins2);
		if (!TargetPin)
		{
			return FMegalithActionResult::Error(FString::Printf(
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleSetPinDefault(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	UEdGraphPin* Pin = MegalithBlueprintInternal::FindPinOnNode(Node, PinName, EGPD_MAX, &SetPinAvailPins);
	if (!Pin)
	{
		return FMegalithActionResult::Error(FString::Printf(
	...
	if (bIsRefPin)
	{
		...
		UObject* Resolved = MegalithBlueprintInternal::ResolveDefaultObjectForPin(Pin, Value, ResolveError);
		if (!Resolved)
		{
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleSetNodePosition(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (!Params->TryGetArrayField(TEXT("position"), PosArray) || !PosArray || PosArray->Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("'position' must be an array of [x, y]"));
	}
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleBatchExecute(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "BPBatchExec", "BP Batch Execute"));
	...
	for (int32 i = 0; i < Ops.Num(); ++i)
	{
		...
		// Variable ops
		else if (OpName == TEXT("add_variable"))           SubResult = FMegalithBlueprintVariableActions::HandleAddVariable(SubParams);
		else if (OpName == TEXT("remove_variable"))        SubResult = FMegalithBlueprintVariableActions::HandleRemoveVariable(SubParams);
		else if (OpName == TEXT("rename_variable"))        SubResult = FMegalithBlueprintVariableActions::HandleRenameVariable(SubParams);
		else if (OpName == TEXT("set_variable_type"))      SubResult = FMegalithBlueprintVariableActions::HandleSetVariableType(SubParams);
		else if (OpName == TEXT("set_variable_defaults"))  SubResult = FMegalithBlueprintVariableActions::HandleSetVariableDefaults(SubParams);
		else if (OpName == TEXT("add_local_variable"))     SubResult = FMegalithBlueprintVariableActions::HandleAddLocalVariable(SubParams);
		else if (OpName == TEXT("remove_local_variable"))  SubResult = FMegalithBlueprintVariableActions::HandleRemoveLocalVariable(SubParams);
		// Component ops
		else if (OpName == TEXT("add_component"))          SubResult = FMegalithBlueprintComponentActions::HandleAddComponent(SubParams);
		else if (OpName == TEXT("remove_component"))       SubResult = FMegalithBlueprintComponentActions::HandleRemoveComponent(SubParams);
		else if (OpName == TEXT("rename_component"))       SubResult = FMegalithBlueprintComponentActions::HandleRenameComponent(SubParams);
		else if (OpName == TEXT("reparent_component"))     SubResult = FMegalithBlueprintComponentActions::HandleReparentComponent(SubParams);
		else if (OpName == TEXT("set_component_property")) SubResult = FMegalithBlueprintComponentActions::HandleSetComponentProperty(SubParams);
		else if (OpName == TEXT("duplicate_component"))    SubResult = FMegalithBlueprintComponentActions::HandleDuplicateComponent(SubParams);
		// Graph/interface ops
		else if (OpName == TEXT("add_function"))           SubResult = FMegalithBlueprintGraphActions::HandleAddFunction(SubParams);
		else if (OpName == TEXT("remove_function"))        SubResult = FMegalithBlueprintGraphActions::HandleRemoveFunction(SubParams);
		else if (OpName == TEXT("rename_function"))        SubResult = FMegalithBlueprintGraphActions::HandleRenameFunction(SubParams);
		else if (OpName == TEXT("add_macro"))              SubResult = FMegalithBlueprintGraphActions::HandleAddMacro(SubParams);
		else if (OpName == TEXT("add_event_dispatcher"))   SubResult = FMegalithBlueprintGraphActions::HandleAddEventDispatcher(SubParams);
		else if (OpName == TEXT("set_function_params"))    SubResult = FMegalithBlueprintGraphActions::HandleSetFunctionParams(SubParams);
		else if (OpName == TEXT("implement_interface"))        SubResult = FMegalithBlueprintGraphActions::HandleImplementInterface(SubParams);
		else if (OpName == TEXT("remove_interface"))           SubResult = FMegalithBlueprintGraphActions::HandleRemoveInterface(SubParams);
		// Wave 5 scaffolding ops
		else if (OpName == TEXT("scaffold_interface_implementation")) SubResult = FMegalithBlueprintGraphActions::HandleScaffoldInterfaceImplementation(SubParams);
		...
		else if (OpName == TEXT("add_replicated_variable"))    SubResult = FMegalithBlueprintVariableActions::HandleAddReplicatedVariable(SubParams);
		// Phase 1 expansion (1E/1F — handlers added by parallel agent)
		else if (OpName == TEXT("save_asset"))                 SubResult = FMegalithBlueprintCompileActions::HandleSaveAsset(SubParams);
		else if (OpName == TEXT("remove_macro"))               SubResult = FMegalithBlueprintGraphActions::HandleRemoveMacro(SubParams);
		else if (OpName == TEXT("rename_macro"))               SubResult = FMegalithBlueprintGraphActions::HandleRenameMacro(SubParams);
		// CDO ops
		else if (OpName == TEXT("set_cdo_property"))           SubResult = FMegalithBlueprintCDOActions::HandleSetCDOProperty(SubParams);
		// Phase 3A timeline read/edit
		...
		else if (OpName == TEXT("export_graph"))                SubResult = FMegalithBlueprintGraphExportActions::HandleExportGraph(SubParams);
		else if (OpName == TEXT("copy_nodes"))                  SubResult = FMegalithBlueprintGraphExportActions::HandleCopyNodes(SubParams);
		else if (OpName == TEXT("duplicate_graph"))             SubResult = FMegalithBlueprintGraphExportActions::HandleDuplicateGraph(SubParams);
		// Phase 6 layout
		else if (OpName == TEXT("auto_layout"))                 SubResult = FMegalithBlueprintLayoutActions::HandleAutoLayout(SubParams);
	...
	if (bCompileOnComplete)
	{
		...
		FMegalithActionResult CompileResult = FMegalithBlueprintCompileActions::HandleCompileBlueprint(CompileParams);
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleResolveNode(const TSharedPtr<FJsonObject>& Params)
{
	FString NodeType = Params->GetStringField(TEXT("node_type"));
	if (NodeType.IsEmpty())
	{
	...
	if (NodeType == TEXT("CallFunction"))
	{
		...
		if (FuncName.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("CallFunction requires 'function_name'"));
		}
		...
		if (!Params->GetStringField(TEXT("asset_path")).IsEmpty())
		{
			...
			ContextBP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, ResolveAssetPath);
		}
		if (ContextBP)
		{
	...
	else if (NodeType == TEXT("SwitchOnEnum"))
	{
		...
		if (EnumType.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("SwitchOnEnum requires 'enum_type' (e.g. enum_type=ECollisionChannel, or a /Script or /Game asset path for a UserDefinedEnum)"));
		}
	...
	else if (NodeType == TEXT("MacroInstance"))
	{
		...
		if (!MacroBlueprint)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Macro blueprint not found: %s"), *MacroBP));
		}
		...
		if (!MacroGraph)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Macro '%s' not found in '%s'"), *MacroName, *MacroBP));
		}
	         ...
	         NodeType == TEXT("CallDelegate"))
	{
		...
		{
			FString AssetPath;
			if (UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath))
			{
	...
	if (!Node)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create transient node"));
	}
	...
	for (const UEdGraphPin* Pin : Node->Pins)
	{
		...
		PinObj->SetStringField(TEXT("type"),
			MegalithBlueprintInternal::ContainerPrefix(Pin->PinType) +
			MegalithBlueprintInternal::PinTypeToString(Pin->PinType));
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddNodesBulk(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required field: nodes"));
	}
	...
	else if (NodesField->Type == EJson::String)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Failed to parse nodes string as JSON array"));
		}
	}
	else
	{
		return FMegalithActionResult::Error(TEXT("'nodes' must be an array"));
	}
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "BPAddNodesBulk", "BP Add Nodes Bulk"));
	...
	for (int32 i = 0; i < NodesArr.Num(); ++i)
	{
		...
		FMegalithActionResult Result = HandleAddNode(SubParams);
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleConnectPinsBulk(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required field: connections"));
	}
	...
	else if (ConnField->Type == EJson::String)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Failed to parse connections string as JSON array"));
		}
	}
	else
	{
		return FMegalithActionResult::Error(TEXT("'connections' must be an array"));
	}

	FString SharedGraphName = Params->GetStringField(TEXT("graph_name"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "BPConnectPinsBulk", "BP Connect Pins Bulk"));
	...
	for (int32 i = 0; i < ConnArr.Num(); ++i)
	{
		...
		FMegalithActionResult Result = HandleConnectPins(SubParams);
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleSetPinDefaultsBulk(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required field: defaults"));
	}
	...
	else if (DefaultsField->Type == EJson::String)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Failed to parse defaults string as JSON array"));
		}
	}
	else
	{
		return FMegalithActionResult::Error(TEXT("'defaults' must be an array"));
	}

	FString SharedGraphName = Params->GetStringField(TEXT("graph_name"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "BPSetPinDefaultsBulk", "BP Set Pin Defaults Bulk"));
	...
	for (int32 i = 0; i < DefaultsArr.Num(); ++i)
	{
		...
		FMegalithActionResult Result = HandleSetPinDefault(SubParams);
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddTimeline(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("This Blueprint type does not support timelines (must be Actor-based)"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Timeline template was created but not found in BP->Timelines — aborting"));
	}
	...
	if (!TimelineNode)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create UK2Node_Timeline node object"));
	}
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddEventNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	if (EventName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: event_name"));
	}
	...
	if (DeclaringClass && EventFunc)
	{
		...
		TSharedPtr<FJsonObject> Root = MegalithBlueprintInternal::SerializeNode(EventNode);
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddCommentNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: text"));
	}
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandlePromotePinToVariable(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	// Find the node
	UEdGraphNode* SourceNode = MegalithBlueprintInternal::FindNodeById(BP, GraphName, NodeId);
	if (!SourceNode)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node not found: %s"), *NodeId));
	}
	...
	UEdGraphPin* Pin = MegalithBlueprintInternal::FindPinOnNode(SourceNode, PinName, EGPD_MAX, &PromoteAvailPins);
	if (!Pin)
	{
		return FMegalithActionResult::Error(FString::Printf(
	...
	{
		return FMegalithActionResult::Error(TEXT("Cannot promote execution (exec) pins to variables"));
	}

	// Validate: not wildcard
	if (Pin->PinType.PinCategory == UEdGraphSchema_K2::PC_Wildcard)
	{
		return FMegalithActionResult::Error(TEXT("Cannot promote wildcard pins to variables — resolve the type first"));
	}
	...
	if (!GraphName.IsEmpty())
	{
		Graph = MegalithBlueprintInternal::FindGraphByName(BP, GraphName);
	}
	else
	{
	...
	if (!Graph)
	{
		return FMegalithActionResult::Error(TEXT("Could not locate graph containing the node"));
	}

	// Build pin type string for the response before modifying anything
	FString TypeStr = MegalithBlueprintInternal::ContainerPrefix(Pin->PinType) +
	                  MegalithBlueprintInternal::PinTypeToString(Pin->PinType);
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleGetTimelineData(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleAddTimelineTrack(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: timeline_name"));
	}

	FString TrackNameStr = Params->GetStringField(TEXT("track_name"));
	if (TrackNameStr.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: track_name"));
	}
	...
	if (!Template)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Timeline '%s' not found in Blueprint"), *TimelineName));
	}
	...
	if (!OwnerClass)
	{
		return FMegalithActionResult::Error(TEXT("Blueprint has no GeneratedClass — compile the Blueprint first"));
	}
...
FMegalithActionResult FMegalithBlueprintNodeActions::HandleSetTimelineKeys(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MegalithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: keys"));
	}
	...
	else if (KeysField->Type == EJson::String)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Failed to parse keys string as JSON array"));
		}
	}
	else
	{
