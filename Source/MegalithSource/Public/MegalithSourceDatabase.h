...
DECLARE_LOG_CATEGORY_EXTERN(LogMegalithSource, Log, All);

struct FMegalithSourceSymbol
{
	int64 Id = 0;
	FString Name;
...
struct FMegalithSourceReference
{
	int64 Id = 0;
...
struct FMegalithSourceInheritance
{
	int64 Id = 0;
	FString Name;
	FString QualifiedName;
...
struct FMegalithSourceModuleStats
{
	FString Name;
	FString Path;
	FString ModuleType;
...
struct FMegalithDeprecationRow
{
	FString Version;
	FString Message;
	FString Kind;
 ...
 */
class MEGALITHSOURCE_API FMegalithSourceDatabase
{
public:
	 ...
	 *
	 * MegalithReflectionIntel's read-only query adapters (decision / risk /
	...
	// --- Symbol queries ---
	TArray<FMegalithSourceSymbol> SearchSymbolsFTS(const FString& Query, int32 Limit = 20);
	TArray<FMegalithSourceSymbol> GetSymbolsByName(const FString& Name, const FString& Kind = TEXT(""));
	TOptional<FMegalithSourceSymbol> GetSymbolById(int64 Id);

	// --- File queries ---
	FString GetFilePath(int64 FileId);
	TOptional<FMegalithSourceFile> FindFileBySuffix(const FString& Suffix);
	TOptional<FMegalithSourceFile> FindFileByPath(const FString& Path);
	...
	// --- Reference queries ---
	TArray<FMegalithSourceReference> GetReferencesTo(int64 SymbolId, const FString& RefKind = TEXT(""), int32 Limit = 50);
	TArray<FMegalithSourceReference> GetReferencesFrom(int64 SymbolId, const FString& RefKind = TEXT(""), int32 Limit = 50);

	// --- Inheritance queries ---
	TArray<FMegalithSourceInheritance> GetParents(int64 SymbolId);
	TArray<FMegalithSourceInheritance> GetChildren(int64 SymbolId);

	// --- Module queries ---
	TOptional<FMegalithSourceModuleStats> GetModuleStats(const FString& ModuleName);
	TArray<FMegalithSourceSymbol> GetSymbolsInModule(const FString& ModuleName, const FString& Kind = TEXT(""), int32 Limit = 200);

	// --- Source FTS ---
	TArray<FMegalithSourceChunk> SearchSourceFTS(const FString& Query, const FString& Scope = TEXT("all"), int32 Limit = 20);
	TArray<FMegalithSourceChunk> SearchSourceFTSFiltered(const FString& Query, const FString& Scope, const FString& Module, const FString& PathFilter, int32 Limit);
	TArray<FMegalithSourceSymbol> SearchSymbolsFTSFiltered(const FString& Query, const FString& Kind, const FString& Module, const FString& PathFilter, int32 Limit);
	...
	TOptional<FMegalithDeprecationRow> GetDeprecation(const FString& SymbolName);
	/** Batch lookup: maps each input symbol name that IS deprecated to its row. Absent keys = not deprecated. */
	TMap<FString, FMegalithDeprecationRow> GetDeprecationsBatch(const TArray<FString>& SymbolNames);
...
private:
	FMegalithSourceSymbol ReadSymbolFromStatement(FSQLitePreparedStatement& Stmt);
	FMegalithSourceReference ReadReferenceFromStatement(FSQLitePreparedStatement& Stmt, bool bIsRefTo);
