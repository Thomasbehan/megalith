...
class FMegalithSourceDatabase;

/**
 * 9 engine source intelligence actions + 1 reindex trigger.
 * Ports the Python unreal-source-mcp server tools to native C++.
 */
class FMegalithSourceActions
{
public:
	static void RegisterAll();

	// Public for unit testing (MegalithCppErgonomicsTest.cpp) — pure, stateless helpers.
	/**
	...
	//     handlers; public for unit testing — see MegalithCppErgonomicsTest.cpp).
	 ...
	 */
	static bool ResolveIncludeForSymbol(FMegalithSourceDatabase* DB, const FString& Symbol,
	 ...
	 * has a class row).
	 */
	static bool ResolveFirstSignature(FMegalithSourceDatabase* DB, const FString& Symbol,
	 ...
	 */
	static bool SymbolExists(FMegalithSourceDatabase* DB, const FString& Symbol);
...
private:
	// Action handlers
	static FMegalithActionResult HandleReadSource(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleFindReferences(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleFindCallers(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleFindCallees(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleSearchSource(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleGetClassHierarchy(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleGetModuleInfo(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleGetSymbolContext(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleReadFile(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleTriggerReindex(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleTriggerProjectReindex(const TSharedPtr<FJsonObject>& Params);

	// Phase 1 — demand-proven lookups (LLM C++ authoring ergonomics)
	static FMegalithActionResult HandleGetIncludePath(const TSharedPtr<FJsonObject>& Params);    // item 1
	static FMegalithActionResult HandleGetSignature(const TSharedPtr<FJsonObject>& Params);      // item 2
	static FMegalithActionResult HandleCheckDeprecations(const TSharedPtr<FJsonObject>& Params); // item 3

	// Phase 2 — round-trip compression
	static FMegalithActionResult HandleVerifySymbols(const TSharedPtr<FJsonObject>& Params);     // item 4
	static FMegalithActionResult HandleFindExampleUsage(const TSharedPtr<FJsonObject>& Params);  // item 5

	// Phase 3 — pre-flight lint + stub gen
	static FMegalithActionResult HandleLintHeader(const TSharedPtr<FJsonObject>& Params);        // item 7
	static FMegalithActionResult HandleGenerateClassStub(const TSharedPtr<FJsonObject>& Params); // item 9

	// Helpers
	static FMegalithSourceDatabase* GetDB();
	...
	static bool ResolveOwningModule(FMegalithSourceDatabase* DB, const FString& Symbol, FString& OutModule, FString& OutBuildCsNote);
	...
	static void WalkAncestors(FMegalithSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited);
	static void WalkDescendants(FMegalithSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited);
};
