...
FMegalithReferenceBuilder::FMegalithReferenceBuilder(FMegalithSourceDatabase& InDB, const TMap<FString, int64>& InSymbolMap)
	: DB(InDB)
...
int32 FMegalithReferenceBuilder::ExtractReferences(const FString& FilePath, int64 FileId)
{
	FString Source;
	if (!FFileHelper::LoadFileToString(Source, *FilePath))
	{
		UE_LOG(LogMegalithSource, Warning, TEXT("ReferenceBuilder: failed to read %s"), *FilePath);
		return 0;
	}

	// Strip UE macros first, then comments/strings — same pipeline as the first-pass parser
	FString Clean = MegalithUEPreprocessor::PreprocessSource(Source);
...
// as MegalithUEPreprocessor::PreprocessSource().
//
// Handles:
...
FString FMegalithReferenceBuilder::StripCommentsAndStrings(const FString& Source)
{
...
int32 FMegalithReferenceBuilder::ExtractCallReferences(const TArray<FString>& Lines, int64 FileId)
{
...
int32 FMegalithReferenceBuilder::ExtractTypeReferences(const TArray<FString>& Lines, int64 FileId)
{
...
int64 FMegalithReferenceBuilder::ResolveSymbol(const FString& Name) const
{
	if (Name.IsEmpty())
	{
		return 0;
