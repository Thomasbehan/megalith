...
void FMegalithSourceActions::RegisterAll()
{
	...
	Registry.RegisterAction(TEXT("source"), TEXT("read_source"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleReadSource),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_references"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleFindReferences),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_callers"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleFindCallers),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_callees"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleFindCallees),
		FParamSchemaBuilder()
			.Required(TEXT("symbol"), TEXT("string"), TEXT("Function name"))
			.Optional(TEXT("limit"), TEXT("integer"), TEXT("Max results"), TEXT("50"))
			.Build());
	...
	Registry.RegisterAction(TEXT("source"), TEXT("search_source"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleSearchSource),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_class_hierarchy"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleGetClassHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_module_info"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleGetModuleInfo),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_symbol_context"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleGetSymbolContext),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("read_file"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleReadFile),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("trigger_reindex"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleTriggerReindex),
	...
	Registry.RegisterAction(TEXT("source"), TEXT("trigger_project_reindex"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleTriggerProjectReindex),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_include_path"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleGetIncludePath),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("get_signature"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleGetSignature),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("check_deprecations"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleCheckDeprecations),
		FParamSchemaBuilder()
	...
	FMegalithDispatcherAnnotations SourceAnnotations;
	...
	Registry.RegisterAction(TEXT("source"), TEXT("verify_symbols"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleVerifySymbols),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("find_example_usage"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleFindExampleUsage),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("lint_header"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleLintHeader),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("source"), TEXT("generate_class_stub"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithSourceActions::HandleGenerateClassStub),
		FParamSchemaBuilder()
...
FMegalithSourceDatabase* FMegalithSourceActions::GetDB()
{
	if (!GEditor) return nullptr;
	UMegalithSourceSubsystem* Subsystem = Cast<UMegalithSourceSubsystem>(GEditor->GetEditorSubsystemBase(UMegalithSourceSubsystem::StaticClass()));
	if (!Subsystem) return nullptr;
	return Subsystem->GetDatabase();
}

FString FMegalithSourceActions::ShortPath(const FString& FullPath)
{
...
FString FMegalithSourceActions::DeriveIncludePath(const FString& IndexedFilePath, bool& bOutIncludable, FString& OutWarning)
{
	bOutIncludable = true;
	OutWarning.Empty();
...
bool FMegalithSourceActions::ResolveOwningModule(FMegalithSourceDatabase* DB, const FString& Symbol, FString& OutModule, FString& OutBuildCsNote)
{
	OutModule.Empty();
	...
	TArray<FMegalithSourceSymbol> Symbols = DB->GetSymbolsByName(LookupName);
...
bool FMegalithSourceActions::ResolveIncludeForSymbol(FMegalithSourceDatabase* DB, const FString& Symbol,
	FString& OutInclude, bool& OutIncludable, FString& OutModule, FString& OutWarning)
{
	...
	const FMegalithSourceSymbol* Chosen = &Symbols[0];
	for (const FMegalithSourceSymbol& S : Symbols)
	{
...
bool FMegalithSourceActions::ResolveFirstSignature(FMegalithSourceDatabase* DB, const FString& Symbol,
	FString& OutSignature, FString& OutSource)
{
	...
	TArray<FMegalithSourceSymbol> Symbols = DB->GetSymbolsByName(MethodName, TEXT("function"));
	for (const FMegalithSourceSymbol& S : Symbols)
	{
	...
	TArray<FMegalithSourceChunk> Chunks = DB->SearchSourceFTS(Symbol, TEXT("all"), 50);
	for (const FMegalithSourceChunk& Chunk : Chunks)
	{
...
bool FMegalithSourceActions::SymbolExists(FMegalithSourceDatabase* DB, const FString& Symbol)
{
	if (!DB) return false;
	...
	const FString NeedlePattern = MethodName + TEXT("(");
	TArray<FMegalithSourceChunk> Chunks = DB->SearchSourceFTS(Symbol, TEXT("all"), 25);
	for (const FMegalithSourceChunk& Chunk : Chunks)
	{
		const FString FilePath = DB->GetFilePath(Chunk.FileId);
		TArray<FString> FileLines;
...
FString FMegalithSourceActions::ReadFileLines(const FString& FilePath, int32 StartLine, int32 EndLine)
{
...
bool FMegalithSourceActions::IsForwardDeclaration(const FString& FilePath, int32 LineStart, int32 LineEnd)
{
...
FString FMegalithSourceActions::ExtractMembers(const FString& FilePath, int32 StartLine, int32 EndLine)
{
	TArray<FString> Lines;
	if (!FFileHelper::LoadFileToStringArray(Lines, *FilePath))
	{
...
FString FMegalithSourceActions::MakeTextResult(const FString& Text)
{
	// Return text as a JSON result with a "text" field
	// (But the registry expects FMegalithActionResult with a JSON object)
...
FMegalithActionResult FMegalithSourceActions::HandleReadSource(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
		return FMegalithActionResult::Error(TEXT("Engine source DB not available. Run source.trigger_reindex first."));
	}
	...
	TArray<FMegalithSourceSymbol> Symbols = DB->GetSymbolsByName(Symbol);
	if (Symbols.Num() == 0)
	{
	...
	if (Symbols.Num() == 0)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No symbol found matching '%s'."), *Symbol));
	}
	...
	if (bHasDefinition)
	{
		TArray<FMegalithSourceSymbol> Filtered;
		for (const auto& Sym : Symbols)
		{
	...
	ContentArr.Add(MakeShared<FJsonValueObject>(ContentItem));
	ResultObj->SetArrayField(TEXT("content"), ContentArr);
	return FMegalithActionResult::Success(ResultObj);
}
...
FMegalithActionResult FMegalithSourceActions::HandleFindReferences(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
		return FMegalithActionResult::Error(TEXT("Engine source DB not available."));
	}
	...
	for (const auto& Sym : Symbols)
	{
		TArray<FMegalithSourceReference> Refs = DB->GetReferencesTo(Sym.Id, RefKind, Limit);
...
FMegalithActionResult FMegalithSourceActions::HandleFindCallers(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TArray<FMegalithSourceSymbol> Symbols = DB->GetSymbolsByName(Function, TEXT("function"));
	if (Symbols.Num() == 0)
	{
		TArray<FMegalithSourceSymbol> AllSyms = DB->SearchSymbolsFTS(Function, 5);
	...
	if (Symbols.Num() == 0)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No function found matching '%s'."), *Function));
	}

	TArray<FString> Lines;
	for (const auto& Sym : Symbols)
	{
		TArray<FMegalithSourceReference> Refs = DB->GetReferencesTo(Sym.Id, TEXT("call"), Limit);
		for (const auto& Ref : Refs)
		{
...
FMegalithActionResult FMegalithSourceActions::HandleFindCallees(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	for (const auto& Sym : Symbols)
	{
		TArray<FMegalithSourceReference> Refs = DB->GetReferencesFrom(Sym.Id, TEXT("call"), Limit);
		for (const auto& Ref : Refs)
		{
...
FMegalithActionResult FMegalithSourceActions::HandleSearchSource(const TSharedPtr<FJsonObject>& Params)
{
	...
	const uint32 CurrentHash = MegalithCursorCodec::ComputeQueryHash(
	...
	if (!CursorIn.IsEmpty())
	{
		MegalithCursorCodec::FCursorState State;
		if (!MegalithCursorCodec::Decode(CursorIn, State))
		{
	...
	TArray<FMegalithSourceSymbol> SymResultsAll;
	if (!bPastCap)
	{
	...
	if (SymRowsThisPage > 0)
	{
		...
		{
			const FMegalithSourceSymbol& Sym = SymResultsAll[i];
			FString FilePath = DB->GetFilePath(Sym.FileId);
	...
	TArray<FMegalithSourceChunk> SourceMergedDeduped;
	if (!bPastCap)
	{
		TSet<FString> Seen;
		for (const FString& S : Scopes)
		{
			TArray<FMegalithSourceChunk> ScopeBatch = DB->SearchSourceFTSFiltered(Query, S, Module, PathFilter, N);
			for (const FMegalithSourceChunk& Match : ScopeBatch)
			{
	...
	if (SrcRowsThisPage > 0)
	{
		...
		{
			const FMegalithSourceChunk& Match = SourceMergedDeduped[i];
	...
	{
		MegalithCursorCodec::FCursorState OutState;
		...
		ResultObj->SetStringField(TEXT("next_cursor"), MegalithCursorCodec::Encode(OutState));
	}
...
void FMegalithSourceActions::WalkAncestors(FMegalithSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited)
{
	if (Indent > MaxDepth || Visited.Contains(SymId)) return;
	Visited.Add(SymId);

	TArray<FMegalithSourceInheritance> Parents = DB->GetParents(SymId);
...
void FMegalithSourceActions::WalkDescendants(FMegalithSourceDatabase* DB, int64 SymId, TArray<FString>& Lines, int32 Indent, int32 MaxDepth, FHierarchyCounter& Counter, TSet<int64>& Visited)
{
	if (Indent > MaxDepth || Visited.Contains(SymId)) return;
	Visited.Add(SymId);

	TArray<FMegalithSourceInheritance> Children = DB->GetChildren(SymId);
...
FMegalithActionResult FMegalithSourceActions::HandleGetClassHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TArray<FMegalithSourceSymbol> Symbols = DB->GetSymbolsByName(ClassName, TEXT("class"));
	if (Symbols.Num() == 0) Symbols = DB->GetSymbolsByName(ClassName, TEXT("struct"));
	if (Symbols.Num() == 0)
	{
		TArray<FMegalithSourceSymbol> AllSyms = DB->SearchSymbolsFTS(ClassName, 5);
		for (const auto& S : AllSyms)
		{
	...
	if (Symbols.Num() == 0)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No class or struct found matching '%s'."), *ClassName));
	}
	...
	const FMegalithSourceSymbol& Sym = Symbols[0];
	FString FilePath = DB->GetFilePath(Sym.FileId);
	TArray<FString> Lines;
...
FMegalithActionResult FMegalithSourceActions::HandleGetModuleInfo(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TOptional<FMegalithSourceModuleStats> Stats = DB->GetModuleStats(ModuleName);
	if (!Stats.IsSet())
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No module found matching '%s'."), *ModuleName));
	}

	const FMegalithSourceModuleStats& S = Stats.GetValue();
	TArray<FString> Lines;
	...
	// Show key classes
	TArray<FMegalithSourceSymbol> KeyClasses = DB->GetSymbolsInModule(ModuleName, TEXT("class"), 20);
	if (KeyClasses.Num() > 0)
	{
		Lines.Add(TEXT(""));
...
FMegalithActionResult FMegalithSourceActions::HandleGetSymbolContext(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
...
FMegalithActionResult FMegalithSourceActions::HandleReadFile(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	else
	{
		...
		TOptional<FMegalithSourceFile> F = DB->FindFileByPath(NormalizedPath);
		if (F.IsSet())
		{
			ResolvedPath = F->Path;
		}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No file found matching '%s'."), *Path));
	}
...
FMegalithActionResult FMegalithSourceActions::HandleTriggerReindex(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor)
	{
		return FMegalithActionResult::Error(TEXT("Editor not available."));
	}

	UMegalithSourceSubsystem* Subsystem = Cast<UMegalithSourceSubsystem>(GEditor->GetEditorSubsystemBase(UMegalithSourceSubsystem::StaticClass()));
	if (!Subsystem)
	{
		return FMegalithActionResult::Error(TEXT("MegalithSourceSubsystem not available."));
	}

	if (Subsystem->IsIndexing())
	{
		return FMegalithActionResult::Error(TEXT("Indexing already in progress."));
	}
...
FMegalithActionResult FMegalithSourceActions::HandleTriggerProjectReindex(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor)
	{
		return FMegalithActionResult::Error(TEXT("Editor not available."));
...
FMegalithActionResult FMegalithSourceActions::HandleGetIncludePath(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	if (Symbol.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'symbol' is required."));
	}
...
FString FMegalithSourceActions::CompactDeclaration(const TArray<FString>& Lines, int32 StartIdx)
{
...
FMegalithActionResult FMegalithSourceActions::HandleGetSignature(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	if (Overloads.Num() == 0)
	{
		...
		TArray<FMegalithSourceChunk> Chunks = DB->SearchSourceFTS(FtsQuery, TEXT("all"), 50);
	...
	if (Overloads.Num() == 0)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No signature found for '%s'."), *Symbol));
	}
...
FMegalithActionResult FMegalithSourceActions::HandleCheckDeprecations(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("'symbols' must be a non-empty array of symbol names."));
	}
	...
	TMap<FString, FMegalithDeprecationRow> Deprecated = DB->GetDeprecationsBatch(SymbolNames);
	...
	for (const FString& Name : SymbolNames)
	{
		...
		const FMegalithDeprecationRow* Found = Deprecated.Find(Name);
		if (Found)
		{
...
FMegalithActionResult FMegalithSourceActions::HandleVerifySymbols(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	TMap<FString, FMegalithDeprecationRow> Deprecated;
	...
	for (const FString& Symbol : SymbolNames)
	{
		...
		else if (const FMegalithDeprecationRow* Dep = Deprecated.Find(MethodName))
		{
...
// cursor-paginate via MegalithCursorCodec + the rerun-slice scheme (the same
// moving-FTS-index rationale as search_source).
// ============================================================================

FMegalithActionResult FMegalithSourceActions::HandleFindExampleUsage(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	{
		MegalithCursorCodec::FCursorState State;
		if (!MegalithCursorCodec::Decode(CursorIn, State) || State.QueryHash != CurrentHash)
		{
			TSharedPtr<FJsonObject> ErrData = MakeShared<FJsonObject>();
			ErrData->SetStringField(TEXT("error_code"), TEXT("INVALID_CURSOR"));
			return FMegalithActionResult::Error(
	...
	TArray<FMegalithSourceChunk> Chunks = DB->SearchSourceFTS(Symbol, TEXT("all"), FTS_FETCH);
	for (const FMegalithSourceChunk& Chunk : Chunks)
	{
		const FString FilePath = DB->GetFilePath(Chunk.FileId);
		TArray<FString> FileLines;
		...
		for (int32 i = WinStart; i < WinEnd; ++i)
		{
			...
			// mirrors (megalith_query.cpp / megalith_offline.py) use, so the
...
TArray<FMegalithSourceActions::FLintFinding> FMegalithSourceActions::LintHeaderLines(
	const FString& FilePath, const TArray<FString>& Lines, const TSet<FString>& ValidSpecifiers)
{
	using namespace MegalithLintDetail;
...
FMegalithActionResult FMegalithSourceActions::HandleLintHeader(const TSharedPtr<FJsonObject>& Params)
{
	const FString FilePath = Params->GetStringField(TEXT("file_path"));
	if (FilePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'file_path' is required."));
	}

	TArray<FString> Lines;
	if (!FFileHelper::LoadFileToStringArray(Lines, *FilePath))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Could not read header file: %s"), *FilePath));
	}
	...
	TSet<FString> ValidSpecifiers;
	{
		...
		if (Registry.HasAction(TEXT("source"), TEXT("list_class_specifiers")))
		{
			...
			FMegalithActionResult SpecRes = Registry.ExecuteAction(TEXT("source"), TEXT("list_class_specifiers"), SpecParams);
...
void FMegalithSourceActions::GenerateClassStubText(
...
FMegalithActionResult FMegalithSourceActions::HandleGenerateClassStub(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithSourceDatabase* DB = GetDB();
	if (!DB || !DB->IsOpen())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("'parent', 'class_name', and 'module' are all required."));
	}

	// Resolve the parent's class row. UCLASS-derived parents ONLY (v1).
	TArray<FMegalithSourceSymbol> ParentRows = DB->GetSymbolsByName(ParentClass);
	...
	const FMegalithSourceSymbol* ParentSym = nullptr;
	for (const FMegalithSourceSymbol& S : ParentRows)
	{
	...
	const FMegalithSourceSymbol* HeaderSym = ParentSym;
	for (const FMegalithSourceSymbol& S : ParentRows)
	{
	...
	{
		TArray<FMegalithSourceSymbol> CtorRows = DB->GetSymbolsByName(ParentClass, TEXT("function"));
		bool bSawAnyCtor = false;
		bool bSawPlainCtor = false;
		bool bSawObjInitCtor = false;
		for (const FMegalithSourceSymbol& S : CtorRows)
		{
			if (S.Signature.IsEmpty()) continue;
