...
#include "MegalithUEPreprocessor.h"
...
FParsedFileResult FMegalithCppParser::ParseFile(const FString& FilePath)
{
	...
	{
		UE_LOG(LogMegalithSource, Warning, TEXT("CppParser: Failed to read file: %s"), *FilePath);
		return Result;
	}
	...
	FString CleanSource = MegalithUEPreprocessor::PreprocessSource(Source);
...
void FMegalithCppParser::ExtractIncludes(const FString& Source, FParsedFileResult& Result)
{
...
void FMegalithCppParser::ExtractClassesAndStructs(const TArray<FString>& OrigLines, const TArray<FString>& CleanLines, FParsedFileResult& Result)
{
...
void FMegalithCppParser::ExtractEnums(const TArray<FString>& OrigLines, const TArray<FString>& CleanLines, FParsedFileResult& Result)
{
	const int32 NumLines = CleanLines.Num();
	TSet<FString> FoundNames;
...
void FMegalithCppParser::ExtractFreeFunctions(const TArray<FString>& OrigLines, const TArray<FString>& CleanLines, FParsedFileResult& Result)
{
	const int32 NumLines = CleanLines.Num();
...
void FMegalithCppParser::ExtractMacrosAndTypedefs(const TArray<FString>& OrigLines, FParsedFileResult& Result)
{
...
void FMegalithCppParser::ExtractMembers(const TArray<FString>& OrigLines, int32 BodyStartLine, int32 BodyEndLine,
...
FString FMegalithCppParser::ExtractDocstring(const TArray<FString>& Lines, int32 TargetLine, bool bSkipUEMacro)
{
...
int32 FMegalithCppParser::FindClosingBrace(const TArray<FString>& Lines, int32 OpenBraceLine)
{
...
bool FMegalithCppParser::IsUEMacroLine(const FString& Line)
{
	FString Trimmed = Line.TrimStartAndEnd();
...
void FMegalithCppParser::ParseBaseClasses(const FString& InheritanceClause, TArray<FString>& OutBases)
{
