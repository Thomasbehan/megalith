...
// Plan: Plugins/Megalith/Docs/plans/2026-06-10-llm-cpp-ergonomics-actions.md (§12 Phase 1).
//
// Tests use disposable SQLite DBs at FPaths::AutomationTransientDir(), never the
// real EngineSource.db. Fixtures live under
// Source/MegalithSource/Private/Tests/Fixtures/CppErgoCorpus/.
//
...
#include "MegalithSourceIndexer.h"
...
namespace MegalithCppErgoTestDetail
{
	/** Resolve the fixture corpus dir relative to the Megalith plugin install. */
...
bool FCppErgoDeprecationSchemaBootstrapTest::RunTest(const FString& /*Parameters*/)
{
	...
	const FString DbPath = MakeTempDbPath();
	{
		...
		TOptional<FMegalithDeprecationRow> Got = DB.GetDeprecation(TEXT("Foo"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.DeprecationIndexExtraction",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoDeprecationIndexExtractionTest::RunTest(const FString& /*Parameters*/)
{
	...
	{
		FMegalithSourceIndexer Indexer;
	...
	TOptional<FMegalithDeprecationRow> Foo = DB.GetDeprecation(TEXT("Foo"));
	...
	TOptional<FMegalithDeprecationRow> Baz = DB.GetDeprecation(TEXT("Baz"));
	...
	// MegalithCppParser now indexes such plain classes and ExtractMembers extracts the
	// class-body methods Foo/Baz as `function` symbol rows. The deprecation extractor
	// resolves symbol_id by EXACT NAME (MegalithSourceIndexer SymbolNameToId.Find), so
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.IncludePathDerivation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoIncludePathDerivationTest::RunTest(const FString& /*Parameters*/)
{
	...
	// Public/ -> strip, includable.
	{
		...
		const FString Out = FMegalithSourceActions::DeriveIncludePath(In, bIncludable, Warning);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.SignatureCompaction",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoSignatureCompactionTest::RunTest(const FString& /*Parameters*/)
{
	...
	{
		const FString Sig = FMegalithSourceActions::CompactDeclaration(Lines, MultiIdx);
	...
	{
		const FString Sig = FMegalithSourceActions::CompactDeclaration(Lines, InlineIdx);
	...
	{
		const FString Sig = FMegalithSourceActions::CompactDeclaration(Lines, MacroIdx);
...
// extension so UBT's `*.cpp` glob never compiles them into MegalithSource (a
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.VerifySymbolsComposition",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoVerifySymbolsCompositionTest::RunTest(const FString& /*Parameters*/)
{
	...
	TestTrue(TEXT("class-body method exists:true"),
		FMegalithSourceActions::SymbolExists(&DB, TEXT("UCppErgoUsage::CallMe")));

	// Owning class itself exists.
	TestTrue(TEXT("class exists:true"),
		FMegalithSourceActions::SymbolExists(&DB, TEXT("UCppErgoUsage")));

	// A symbol absent from the corpus reports exists:false (no error).
	TestFalse(TEXT("missing symbol exists:false"),
		FMegalithSourceActions::SymbolExists(&DB, TEXT("UThisDoesNotExistAnywhere::Nope")));

	// Signature resolution for the class-body method comes from declaration_read.
	{
		FString Sig, Source;
		const bool bOk = FMegalithSourceActions::ResolveFirstSignature(&DB, TEXT("UCppErgoUsage::CallMe"), Sig, Source);
	...
	{
		TOptional<FMegalithDeprecationRow> Dep = DB.GetDeprecation(TEXT("Foo"));
...
// distinct call-site lines for `CallMe(`; the rerun-slice + MegalithCursorCodec
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.FindExampleUsagePagination",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoFindExampleUsagePaginationTest::RunTest(const FString& /*Parameters*/)
{
	...
	TArray<FMegalithSourceChunk> Chunks = DB.SearchSourceFTS(Symbol, TEXT("all"), 400);
	...
	const int32 Limit = 5;
	const uint32 QHash = MegalithCursorCodec::ComputeQueryHash(
	...
	if (P0Rows >= Limit && (P0Start + Limit) < Total)
	{
		...
		NextCursor = MegalithCursorCodec::Encode(OutState);
	}
	TestFalse(TEXT("page 0 emits next_cursor"), NextCursor.IsEmpty());

	// Decode the cursor and fetch page 1.
	MegalithCursorCodec::FCursorState Decoded;
	const bool bDecoded = MegalithCursorCodec::Decode(NextCursor, Decoded);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.LintHeaderRuleTable",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoLintHeaderRuleTableTest::RunTest(const FString& /*Parameters*/)
{
	...
	auto LintFixture = [&](const FString& FixtureName, const FString& RealHeaderName,
		const TSet<FString>& Specs) -> TArray<FMegalithSourceActions::FLintFinding>
	{
		...
		return FMegalithSourceActions::LintHeaderLines(Path, Lines, Specs);
	};

	auto HasRule = [](const TArray<FMegalithSourceActions::FLintFinding>& F, const TCHAR* RuleId) -> bool
	{
		for (const FMegalithSourceActions::FLintFinding& Fi : F) { if (Fi.RuleId == RuleId) return true; }
		return false;
	};
	...
	// missing ones.
	auto TestExactRules = [&](const TCHAR* Label, const TArray<FMegalithSourceActions::FLintFinding>& F,
		const TArray<FString>& Expected)
	{
		TSet<FString> Got;
		for (const FMegalithSourceActions::FLintFinding& Fi : F) { Got.Add(Fi.RuleId); }
		TSet<FString> Want;
	...
	{
		TArray<FMegalithSourceActions::FLintFinding> F = LintFixture(TEXT("LintClean.h.fixture"), TEXT("LintClean.h"), NoSpecs);
		TestEqual(TEXT("clean header zero findings"), F.Num(), 0);
	}

	// (a) missing GENERATED_BODY — EXACTLY {missing_generated_body}.
	{
		TArray<FMegalithSourceActions::FLintFinding> F = LintFixture(TEXT("LintMissingGeneratedBody.h.fixture"), TEXT("LintMissingGeneratedBody.h"), NoSpecs);
		TestExactRules(TEXT("(a) LintMissingGeneratedBody exact rules"), F, { TEXT("missing_generated_body") });
		// Each finding carries a message.
		for (const FMegalithSourceActions::FLintFinding& Fi : F)
		{
	...
	{
		TArray<FMegalithSourceActions::FLintFinding> F = LintFixture(TEXT("LintGeneratedNotLast.h.fixture"), TEXT("LintGeneratedNotLast.h"), NoSpecs);
	...
	{
		TArray<FMegalithSourceActions::FLintFinding> F = LintFixture(TEXT("LintNameMismatch.h.fixture"), TEXT("LintNameMismatch.h"), NoSpecs);
	...
	{
		TArray<FMegalithSourceActions::FLintFinding> F = LintFixture(TEXT("LintMissingApiMacro.h.fixture"), TEXT("LintMissingApiMacro.h"), NoSpecs);
	...
	{
		TArray<FMegalithSourceActions::FLintFinding> F = LintFixture(TEXT("LintOrphanUproperty.h.fixture"), TEXT("LintOrphanUproperty.h"), NoSpecs);
	...
	// instead lint an in-memory header carrying UCLASS(NotARealSpecifier).
	{
		...
		TArray<FMegalithSourceActions::FLintFinding> F =
			FMegalithSourceActions::LintHeaderLines(FakePath, Lines, Specs);
		...
		TArray<FMegalithSourceActions::FLintFinding> F2 =
			FMegalithSourceActions::LintHeaderLines(FakePath, Lines, TSet<FString>());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.GenerateClassStubText",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoGenerateClassStubTextTest::RunTest(const FString& /*Parameters*/)
{
	...
	{
		FString H, C;
		FMegalithSourceActions::GenerateClassStubText(
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.GenerateClassStubNeverWrites",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoGenerateClassStubNeverWritesTest::RunTest(const FString& /*Parameters*/)
{
	...
	// Confirm the registered schema exposes no write_to_disk param.
	{
		...
		{
			const TArray<FMegalithActionInfo> Actions = Registry.GetActions(TEXT("source"));
			for (const FMegalithActionInfo& Info : Actions)
			{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"Megalith.Source.CppErgonomics.AllmanClassIndexing",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCppErgoAllmanClassIndexingTest::RunTest(const FString& /*Parameters*/)
{
	...
	auto ClassRow = [&](const TCHAR* Name) -> TOptional<FMegalithSourceSymbol>
	{
		// Try both kinds — GetSymbolsByName with empty kind returns class OR struct.
		TArray<FMegalithSourceSymbol> Rows = DB.GetSymbolsByName(FString(Name));
		for (const FMegalithSourceSymbol& R : Rows)
		{
		...
		return TOptional<FMegalithSourceSymbol>();
	};

	// 1) Plain allman class FAllmanPlain -> class row with a real line span, members extracted.
	{
		TOptional<FMegalithSourceSymbol> Row = ClassRow(TEXT("FAllmanPlain"));
		...
		for (const FMegalithSourceSymbol& M : DB.GetSymbolsByName(TEXT("Member")))
		{
			if (M.Kind == TEXT("variable") && M.QualifiedName == TEXT("FAllmanPlain::Member")) { bFoundVar = true; }
		}
		for (const FMegalithSourceSymbol& M : DB.GetSymbolsByName(TEXT("GetMember")))
		{
		...
		for (int32 N = 0; N <= 11; ++N)
		{
			...
			bool bFound = false;
			for (const FMegalithSourceSymbol& M : DB.GetSymbolsByName(MemberName))
			{
	...
	{
		TOptional<FMegalithSourceSymbol> Row = ClassRow(TEXT("FAllmanDerived"));
		TestTrue(TEXT("FAllmanDerived indexed"), Row.IsSet());
		if (Row.IsSet())
		{
			TArray<FMegalithSourceInheritance> Parents = DB.GetParents(Row.GetValue().Id);
			bool bHasBase = false;
			for (const FMegalithSourceInheritance& P : Parents)
			{
	...
	{
		TOptional<FMegalithSourceSymbol> Row = ClassRow(TEXT("FOneLiner"));
	...
	//    (template-param guard), but the real FAllmanTpl declaration must index.
	{
		...
		bool bBogusT = false;
		for (const FMegalithSourceSymbol& R : DB.GetSymbolsByName(TEXT("T")))
		{
	...
	//    implementation records the class row (best-effort) but extracts NO members.
	{
		...
		for (const FMegalithSourceSymbol& M : DB.GetSymbolsByName(TEXT("NeverReached")))
		{
