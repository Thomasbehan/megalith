...
//
// DEVIATION NOTE: plan §6 file-table specifies `Source/MegalithSource/Tests/...`.
// This file lives under `Source/MegalithSource/Private/Tests/...` so UBT's
// auto-include of `Private/` picks it up without a Build.cs change. Matches
// the established precedent (see `MegalithFuzzyMatchTest.cpp` deviation note).
//
// Test names use the `Megalith.CursorPagination.*` prefix to match Megalith
// convention (NOT `Leviathan.Megalith.*`).
...
namespace MegalithCursorPaginationTestDetail
{
	...
	static FMegalithActionResult Call(
		const FString& Query,
		int32 Limit,
	...
	static bool HasTotalEstimate(const FMegalithActionResult& R)
	{
	...
	static bool HasNextCursor(const FMegalithActionResult& R, FString& OutCursor)
	{
		OutCursor.Reset();
		if (!R.bSuccess || !R.Result.IsValid()) return false;
	...
	static bool IsSourceIndexAvailable()
	{
		...
		FMegalithActionResult Probe = Call(TEXT("class"), 1);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithCursorPaginationRoundTripTest,
	"Megalith.CursorPagination.RoundTrip",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithCursorPaginationRoundTripTest::RunTest(const FString& /*Parameters*/)
{
	MegalithCursorCodec::FCursorState In;
	In.QueryHash = 0xDEADBEEFu;
	In.SymbolPage = 2;
	In.SourcePage = 3;
	In.CachedTotalEstimate = 420;

	const FString Encoded = MegalithCursorCodec::Encode(In);
	TestFalse(TEXT("Encoded cursor is non-empty"), Encoded.IsEmpty());

	MegalithCursorCodec::FCursorState Out;
	const bool bDecoded = MegalithCursorCodec::Decode(Encoded, Out);
	...
	MegalithCursorCodec::FCursorState Junk;
	TestFalse(TEXT("Garbage base64 rejected cleanly"),
		MegalithCursorCodec::Decode(TEXT("this-is-not-base64!!!"), Junk));

	// Empty string → false out.
	TestFalse(TEXT("Empty cursor rejected"),
		MegalithCursorCodec::Decode(TEXT(""), Junk));
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithCursorPaginationQueryMismatchRejectionTest,
	"Megalith.CursorPagination.QueryMismatchRejection",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithCursorPaginationQueryMismatchRejectionTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithCursorPaginationTestDetail;
	...
	MegalithCursorCodec::FCursorState Bogus;
	Bogus.QueryHash = 0x12345678u;
	Bogus.SymbolPage = 1;
	Bogus.SourcePage = 1;
	Bogus.CachedTotalEstimate = 99;
	const FString BogusCursor = MegalithCursorCodec::Encode(Bogus);

	FMegalithActionResult R = Call(TEXT("FooBarBaz_QueryHashMismatchProbe_2026"), 10, BogusCursor);
	...
	FMegalithActionResult R2 = Call(TEXT("FooBarBaz_QueryHashMismatchProbe_2026"), 10,
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithCursorPaginationHardCapTest,
	"Megalith.CursorPagination.HardCap",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithCursorPaginationHardCapTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithCursorPaginationTestDetail;
	...
	FMegalithActionResult R = Call(TEXT("class"), 2000);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithCursorPaginationTotalEstimatePageZeroOnlyTest,
	"Megalith.CursorPagination.TotalEstimatePageZeroOnly",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithCursorPaginationTotalEstimatePageZeroOnlyTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithCursorPaginationTestDetail;
	if (!IsSourceIndexAvailable())
	{
	...
	FMegalithActionResult Page0 = Call(TEXT("class"), 5);
	...
	FMegalithActionResult Page1 = Call(TEXT("class"), 5, NextCursor);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithCursorPaginationPageBoundaryNoNextCursorTest,
	"Megalith.CursorPagination.PageBoundaryNoNextCursor",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithCursorPaginationPageBoundaryNoNextCursorTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithCursorPaginationTestDetail;
	if (!IsSourceIndexAvailable())
	{
	...
	FMegalithActionResult R = Call(TEXT("FZeroToOne_VeryUnlikelyTokenName_2026"), 500);
