...
#include "MegalithSourceSchema.h"
#include "SQLiteDatabase.h"
#include "Misc/Paths.h"
#include "HAL/PlatformFileManager.h"

DEFINE_LOG_CATEGORY(LogMegalithSource);
...
FMegalithSourceDatabase::FMegalithSourceDatabase()
{
}
...
bool FMegalithSourceDatabase::Open(const FString& DbPath)
{
	...
	{
		UE_LOG(LogMegalithSource, Warning, TEXT("Engine source DB not found: %s"), *DbPath);
		return false;
	}
	...
	{
		UE_LOG(LogMegalithSource, Error, TEXT("Failed to open engine source DB: %s"), *DbPath);
		delete Database;
		Database = nullptr;
		return false;
	}
	...
	// same file by MegalithReflectionIntel: UE 5.7's custom `unreal-fs` SQLite VFS
	...
	{
		UE_LOG(LogMegalithSource, Warning,
			TEXT("Open: PRAGMA journal_mode=DELETE failed on '%s' (continuing — hygiene only)"),
			*DbPath);
	}

	UE_LOG(LogMegalithSource, Log, TEXT("Engine source DB opened: %s"), *DbPath);
	return true;
}

void FMegalithSourceDatabase::Close()
{
	FScopeLock Lock(&DbLock);
	if (Database)
	{
...
bool FMegalithSourceDatabase::IsOpen() const
{
	FScopeLock Lock(&DbLock);
	return Database != nullptr && Database->IsValid();
}

FSQLiteDatabase* FMegalithSourceDatabase::GetRawHandle() const
{
	// Deliberately NOT locked here: the borrower (MegalithReflectionIntel) holds
...
FString FMegalithSourceDatabase::EscapeFTS(const FString& Query)
{
	// Replace :: with space
...
FMegalithSourceSymbol FMegalithSourceDatabase::ReadSymbolFromStatement(FSQLitePreparedStatement& Stmt)
{
	FMegalithSourceSymbol Sym;
...
FMegalithSourceReference FMegalithSourceDatabase::ReadReferenceFromStatement(FSQLitePreparedStatement& Stmt, bool bIsRefTo)
{
	FMegalithSourceReference Ref;
...
TArray<FMegalithSourceSymbol> FMegalithSourceDatabase::GetSymbolsByName(const FString& Name, const FString& Kind)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceSymbol> Result;
...
TArray<FMegalithSourceSymbol> FMegalithSourceDatabase::SearchSymbolsFTS(const FString& Query, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceSymbol> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TOptional<FMegalithSourceSymbol> FMegalithSourceDatabase::GetSymbolById(int64 Id)
{
	FScopeLock Lock(&DbLock);
...
FString FMegalithSourceDatabase::GetFilePath(int64 FileId)
{
	FScopeLock Lock(&DbLock);
...
TOptional<FMegalithSourceFile> FMegalithSourceDatabase::FindFileBySuffix(const FString& Suffix)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
...
bool FMegalithSourceDatabase::GetFileModuleInfo(int64 FileId, FString& OutModuleName, FString& OutBuildCsPath)
{
	FScopeLock Lock(&DbLock);
	OutModuleName.Empty();
...
TOptional<FMegalithSourceFile> FMegalithSourceDatabase::FindFileByPath(const FString& Path)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
...
TArray<FMegalithSourceReference> FMegalithSourceDatabase::GetReferencesTo(int64 SymbolId, const FString& RefKind, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceReference> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FMegalithSourceReference> FMegalithSourceDatabase::GetReferencesFrom(int64 SymbolId, const FString& RefKind, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceReference> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FMegalithSourceInheritance> FMegalithSourceDatabase::GetParents(int64 SymbolId)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceInheritance> Result;
	if (!Database || !Database->IsValid()) return Result;
	...
	while (Stmt.Step() == ESQLitePreparedStatementStepResult::Row)
	{
		FMegalithSourceInheritance Inh;
...
TArray<FMegalithSourceInheritance> FMegalithSourceDatabase::GetChildren(int64 SymbolId)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceInheritance> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TOptional<FMegalithSourceModuleStats> FMegalithSourceDatabase::GetModuleStats(const FString& ModuleName)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
	...
	FMegalithSourceModuleStats Stats;
	int64 ModId = 0;
...
TArray<FMegalithSourceSymbol> FMegalithSourceDatabase::GetSymbolsInModule(const FString& ModuleName, const FString& Kind, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceSymbol> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FMegalithSourceChunk> FMegalithSourceDatabase::SearchSourceFTS(const FString& Query, const FString& Scope, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceChunk> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FMegalithSourceChunk> FMegalithSourceDatabase::SearchSourceFTSFiltered(const FString& Query, const FString& Scope, const FString& Module, const FString& PathFilter, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceChunk> Result;
	if (!Database || !Database->IsValid()) return Result;
...
TArray<FMegalithSourceSymbol> FMegalithSourceDatabase::SearchSymbolsFTSFiltered(const FString& Query, const FString& Kind, const FString& Module, const FString& PathFilter, int32 Limit)
{
	FScopeLock Lock(&DbLock);
	TArray<FMegalithSourceSymbol> Result;
	if (!Database || !Database->IsValid()) return Result;
...
int32 FMegalithSourceDatabase::CountSymbolsFTSFiltered(const FString& Query, const FString& Kind, const FString& Module, const FString& PathFilter)
{
	FScopeLock Lock(&DbLock);
...
int32 FMegalithSourceDatabase::CountSourceFTSFiltered(const FString& Query, const FString& Scope, const FString& Module, const FString& PathFilter)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
...
bool FMegalithSourceDatabase::OpenForWriting(const FString& DbPath)
{
	FScopeLock Lock(&DbLock);
	...
	{
		UE_LOG(LogMegalithSource, Error, TEXT("OpenForWriting: failed to open/create DB: %s"), *DbPath);
		delete Database;
		Database = nullptr;
		return false;
	}
	...
	// (FMegalithSourceDatabase::GetRawHandle) rather than opening its own. We keep
	...
	UE_LOG(LogMegalithSource, Log, TEXT("Engine source DB opened for writing: %s"), *DbPath);
	return true;
}
...
bool FMegalithSourceDatabase::CreateTablesIfNeeded()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid())
	{
		UE_LOG(LogMegalithSource, Error, TEXT("CreateTablesIfNeeded: DB not open"));
		return false;
	}

	if (!ExecuteMulti(*Database, MegalithSourceSchema::DDL_Tables))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("CreateTablesIfNeeded: DDL_Tables failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, MegalithSourceSchema::DDL_FTS))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("CreateTablesIfNeeded: DDL_FTS failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, MegalithSourceSchema::DDL_Triggers))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("CreateTablesIfNeeded: DDL_Triggers failed — %s"), *Database->GetLastError());
		return false;
	}
	...
	MetaStmt.SetBindingValueByIndex(2, FString::FromInt(MegalithSourceSchema::SchemaVersion));
	MetaStmt.Step();

	UE_LOG(LogMegalithSource, Log, TEXT("Schema created/verified (version %d)"), MegalithSourceSchema::SchemaVersion);
	return true;
}

bool FMegalithSourceDatabase::ResetDatabase()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid())
	{
		UE_LOG(LogMegalithSource, Error, TEXT("ResetDatabase: DB not open"));
		return false;
	}

	if (!ExecuteMulti(*Database, MegalithSourceSchema::DDL_Drop))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("ResetDatabase: drop failed — %s"), *Database->GetLastError());
		return false;
	}

	UE_LOG(LogMegalithSource, Log, TEXT("ResetDatabase: all tables dropped, recreating schema"));

	// Execute DDL inline (we're already holding DbLock, can't call CreateTablesIfNeeded)
	if (!ExecuteMulti(*Database, MegalithSourceSchema::DDL_Tables))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("ResetDatabase: DDL_Tables failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, MegalithSourceSchema::DDL_FTS))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("ResetDatabase: DDL_FTS failed — %s"), *Database->GetLastError());
		return false;
	}
	if (!ExecuteMulti(*Database, MegalithSourceSchema::DDL_Triggers))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("ResetDatabase: DDL_Triggers failed — %s"), *Database->GetLastError());
		return false;
	}
	...
	UE_LOG(LogMegalithSource, Log, TEXT("ResetDatabase: schema recreated successfully"));
	return true;
}
...
bool FMegalithSourceDatabase::BeginTransaction()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return false;
...
bool FMegalithSourceDatabase::CommitTransaction()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return false;
...
bool FMegalithSourceDatabase::RollbackTransaction()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return false;
...
int64 FMegalithSourceDatabase::InsertModule(const FString& Name, const FString& Path, const FString& ModuleType, const FString& BuildCsPath)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
	...
	UE_LOG(LogMegalithSource, Warning, TEXT("InsertModule: could not retrieve id for '%s'"), *Name);
	return 0;
}

int64 FMegalithSourceDatabase::InsertFile(const FString& FilePath, int64 ModuleId, const FString& FileType, int32 LineCount, double LastModified)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
	...
	UE_LOG(LogMegalithSource, Warning, TEXT("InsertFile: could not retrieve id for '%s'"), *FilePath);
	return 0;
}

int64 FMegalithSourceDatabase::InsertSymbol(
...
void FMegalithSourceDatabase::InsertInheritance(int64 ChildId, int64 ParentId)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FMegalithSourceDatabase::InsertReference(int64 FromSymbolId, int64 ToSymbolId, const FString& RefKind, int64 FileId, int32 Line)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FMegalithSourceDatabase::InsertInclude(int64 FileId, const FString& IncludedPath, int32 Line)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FMegalithSourceDatabase::InsertSourceChunks(int64 FileId, const TArray<FString>& Lines)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
void FMegalithSourceDatabase::SetMeta(const FString& Key, const FString& Value)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
FString FMegalithSourceDatabase::GetMeta(const FString& Key)
{
	FScopeLock Lock(&DbLock);
...
void FMegalithSourceDatabase::InsertDeprecation(int64 SymbolId, const FString& SymbolName, const FString& Version, const FString& Message, const FString& Kind)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return;
...
TOptional<FMegalithDeprecationRow> FMegalithSourceDatabase::GetDeprecation(const FString& SymbolName)
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return {};
...
TMap<FString, FMegalithDeprecationRow> FMegalithSourceDatabase::GetDeprecationsBatch(const TArray<FString>& SymbolNames)
{
	FScopeLock Lock(&DbLock);
	TMap<FString, FMegalithDeprecationRow> Result;
	if (!Database || !Database->IsValid()) return Result;
...
int32 FMegalithSourceDatabase::GetDeprecationCount()
{
	FScopeLock Lock(&DbLock);
	if (!Database || !Database->IsValid()) return 0;
...
int32 FMegalithSourceDatabase::LoadExistingSymbols(
	...
	TMap<FString, TPair<int32,int32>>& OutClassSpans)
{
	...
	UE_LOG(LogMegalithSource, Log, TEXT("LoadExistingSymbols: loaded %d symbols (%d classes/structs)"),
