...
#include "MegalithCppParser.h"
#include "MegalithShaderParser.h"
#include "MegalithReferenceBuilder.h"
#include "HAL/RunnableThread.h"
#include "HAL/FileManager.h"
#include "Misc/Paths.h"
#include "Internationalization/Regex.h"

namespace MegalithSourceIndexerDetail
{
...
FMegalithSourceIndexer::FMegalithSourceIndexer()
{
}
...
void FMegalithSourceIndexer::SetSourcePath(const FString& InPath)   { SourcePath = InPath; }
void FMegalithSourceIndexer::SetShaderPath(const FString& InPath)   { ShaderPath = InPath; }
void FMegalithSourceIndexer::SetProjectPath(const FString& InPath)  { ProjectPath = InPath; }
void FMegalithSourceIndexer::SetDatabasePath(const FString& InPath) { DbPath = InPath; }
void FMegalithSourceIndexer::SetCleanBuild(bool bClean)             { bCleanBuild = bClean; }
void FMegalithSourceIndexer::SetIndexProjectSource(bool bIndex)     { bIndexProjectSource = bIndex; }

// ============================================================
// Thread control
// ============================================================

bool FMegalithSourceIndexer::StartAsync()
{
	if (bIsRunning) return false;
	Thread = FRunnableThread::Create(this, TEXT("MegalithSourceIndexer"), 0, TPri_BelowNormal);
	if (!Thread)
	{
		// Run() will never execute, so nothing else would ever broadcast. Without
		// this the caller's bIsIndexing stays set forever.
		CompleteRun(TEXT("Failed to create the indexing thread"));
		return false;
	}
	return true;
}

bool FMegalithSourceIndexer::RunSynchronous()
{
	if (bIsRunning) return false;
	Init();
	Run();
...
void FMegalithSourceIndexer::RequestStop()
{
	bShouldStop = true;
}

bool FMegalithSourceIndexer::IsRunning() const
{
	return bIsRunning;
}

FSourceIndexDiagnostics FMegalithSourceIndexer::GetDiagnostics() const
{
...
bool FMegalithSourceIndexer::Init()
{
	return true;
}

void FMegalithSourceIndexer::Stop()
{
	bShouldStop = true;
}

uint32 FMegalithSourceIndexer::Run()
{
	bIsRunning = true;
	...
	{
		CompleteRun(*FString::Printf(TEXT("Failed to open DB for writing: %s"), *DbPath));
		return 1;
	}
	...
	if (!SourcePath.IsEmpty())
	{
		...
		UE_LOG(LogMegalithSource, Log, TEXT("Indexer: Found %d engine modules"), EngineModules.Num());
	...
	if (bIndexProjectSource && !ProjectPath.IsEmpty() && !bShouldStop)
	{
		...
		if (!bCleanBuild)
		{
			UE_LOG(LogMegalithSource, Log, TEXT("Indexer: Loading existing symbols for incremental indexing..."));
		...
		UE_LOG(LogMegalithSource, Log, TEXT("Indexer: Found %d project modules"), ProjectModules.Num());
...
void FMegalithSourceIndexer::CompleteRun(const TCHAR* FailureReason)
{
	if (FailureReason)
	{
		UE_LOG(LogMegalithSource, Error, TEXT("Indexer: %s"), FailureReason);
		TotalErrors++;
	}
	...
	if (!FailureReason)
	{
		UE_LOG(LogMegalithSource, Log, TEXT("Indexer complete: %d files, %d symbols, %d errors"), Files, Symbols, Errors);
	}
...
void FMegalithSourceIndexer::DiscoverEngineModules(TArray<FModuleEntry>& OutModules)
{
		...
		IFileManager::Get().IterateDirectory(*CategoryDir, [&](const TCHAR* Path, bool bIsDir) -> bool
		{
			if (bIsDir)
			{
				...
				OutModules.Add({ FString(Path), DirName, Category,
					MegalithSourceIndexerDetail::DeriveBuildCsPath(FString(Path), DirName) });
			}
		...
		if (bIsDir)
		{
			...
			if (DirName == TEXT("Source"))
			{
				...
				OutModules.Add({ FString(Path), ModuleName, TEXT("Plugin"),
					MegalithSourceIndexerDetail::DeriveBuildCsPath(FString(Path), ModuleName) });
			}
		}
		return true;
	});
...
void FMegalithSourceIndexer::DiscoverProjectModules(TArray<FModuleEntry>& OutModules)
{
		...
		if (bIsDir)
		{
			...
			if (DirName == TEXT("Source"))
			{
				...
				OutModules.Add({ FullPath, ModuleName, ModuleType,
					MegalithSourceIndexerDetail::DeriveBuildCsPath(FullPath, ModuleName) });
			}
		}
		return true;
	});
...
void FMegalithSourceIndexer::IndexModule(const FModuleEntry& Module, FMegalithSourceDatabase& DB)
{
...
int32 FMegalithSourceIndexer::IndexCppFile(const FString& FilePath, int64 ModuleId, FMegalithSourceDatabase& DB)
{
	FMegalithCppParser Parser;
...
int32 FMegalithSourceIndexer::IndexShaderFile(const FString& FilePath, int64 ModuleId, FMegalithSourceDatabase& DB)
{
	FMegalithShaderParser Parser;
	FParsedFileResult ParseResult = Parser.ParseFile(FilePath);
...
void FMegalithSourceIndexer::UpdateSymbolMap(const FString& Name, int64 SymId, int32 LineStart, int32 LineEnd)
{
...
void FMegalithSourceIndexer::UpdateClassMap(const FString& Name, int64 SymId, int32 LineStart, int32 LineEnd)
{
	if (Name.StartsWith(TEXT("_bases_"))) return;
...
void FMegalithSourceIndexer::Finalize(FMegalithSourceDatabase& DB)
{
	UE_LOG(LogMegalithSource, Log, TEXT("Indexer: Finalizing — resolving inheritance..."));
	...
	UE_LOG(LogMegalithSource, Log, TEXT("Indexer: Extracting references from %d new files..."), NewFileIds.Num());

	FMegalithReferenceBuilder RefBuilder(DB, SymbolNameToId);
	...
	for (int64 FileId : NewFileIds)
	{
		...
		if (FilesProcessed % 500 == 0)
		{
			DB.CommitTransaction();
			DB.BeginTransaction();
			UE_LOG(LogMegalithSource, Log, TEXT("  References: %d files processed, %d refs found"), FilesProcessed, RefCount);
		}
	}
	DB.CommitTransaction();

	UE_LOG(LogMegalithSource, Log, TEXT("Indexer: Reference extraction complete — %d refs from %d files"), RefCount, FilesProcessed);
