...
UMegalithSourceSubsystem::~UMegalithSourceSubsystem()
{
	delete Indexer;
	Indexer = nullptr;
}

void UMegalithSourceSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	Database = MakeUnique<FMegalithSourceDatabase>();
	...
	if (PlatformFile.FileExists(*DbPath))
	{
		...
		{
			UE_LOG(LogMegalithSource, Log, TEXT("Engine source DB loaded from %s"), *DbPath);
		}
	}
	else
	{
		UE_LOG(LogMegalithSource, Log, TEXT("Engine source DB not found at %s — run source.trigger_reindex to create it"), *DbPath);
	}
	...
	ReloadCompleteHandle = FCoreUObjectDelegates::ReloadCompleteDelegate.AddUObject(
		this, &UMegalithSourceSubsystem::OnReloadComplete);
}

void UMegalithSourceSubsystem::Deinitialize()
{
...
void UMegalithSourceSubsystem::OnReloadComplete(EReloadCompleteReason Reason)
{
	...
	if (bIsIndexing)
	{
		UE_LOG(LogMegalithSource, Verbose,
...
bool UMegalithSourceSubsystem::TriggerReindex()
{
	if (bIsIndexing)
	{
		UE_LOG(LogMegalithSource, Warning, TEXT("Indexing already in progress"));
		return false;
	}
	...
	bIsIndexing = true;
		...
		delete Indexer;
	Indexer = new FMegalithSourceIndexer();
	...
	const TWeakObjectPtr<UMegalithSourceSubsystem> WeakThis(this);
	Indexer->OnComplete.AddLambda([WeakThis, DbPath](int32 Files, int32 Symbols, int32 Errors)
	{
		...
		{
			UMegalithSourceSubsystem* Subsystem = WeakThis.Get();
			if (!Subsystem || Subsystem->bIsShuttingDown)
			{
				return;
			}
			Subsystem->bIsIndexing = false;
			UE_LOG(LogMegalithSource, Log, TEXT("C++ source indexing complete: %d files, %d symbols, %d errors"), Files, Symbols, Errors);
			Subsystem->ReopenDatabase(DbPath);
		});
	});

	UE_LOG(LogMegalithSource, Log, TEXT("Starting full source indexing (engine + project) via C++ indexer"));
	if (!Indexer->StartAsync())
	{
		UE_LOG(LogMegalithSource, Error, TEXT("Failed to start the source indexing thread — no reindex is running"));
		bIsIndexing = false;
		return false;
	}
...
bool UMegalithSourceSubsystem::TriggerProjectReindex()
{
	if (bIsIndexing)
	{
		UE_LOG(LogMegalithSource, Warning, TEXT("Indexing already in progress"));
	...
	if (!PlatformFile.FileExists(*DbPath))
	{
		UE_LOG(LogMegalithSource, Error, TEXT("EngineSource.db not found at %s — run full TriggerReindex() first"), *DbPath);
		return false;
	}
	...
	Indexer->OnComplete.AddLambda([WeakThis, DbPath](int32 Files, int32 Symbols, int32 Errors)
	{
		AsyncTask(ENamedThreads::GameThread, [WeakThis, DbPath, Files, Symbols, Errors]()
		{
			...
			Subsystem->bIsIndexing = false;
			UE_LOG(LogMegalithSource, Log, TEXT("Project source indexing complete: %d files, %d symbols, %d errors"), Files, Symbols, Errors);
			Subsystem->ReopenDatabase(DbPath);
		});
	});

	UE_LOG(LogMegalithSource, Log, TEXT("Starting project source indexing (incremental) via C++ indexer"));
	if (!Indexer->StartAsync())
	{
		UE_LOG(LogMegalithSource, Error, TEXT("Failed to start the source indexing thread — no reindex is running"));
		bIsIndexing = false;
...
void UMegalithSourceSubsystem::ReopenDatabase(const FString& DbPath)
{
	if (Database.IsValid())
	{
		IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
		if (PlatformFile.FileExists(*DbPath))
		{
			if (Database->Open(DbPath))
			{
				UE_LOG(LogMegalithSource, Log, TEXT("Engine source DB reopened: %s"), *DbPath);
			}
		}
	}
}

FString UMegalithSourceSubsystem::GetDatabasePath() const
{
...
FString UMegalithSourceSubsystem::GetEngineSourcePath() const
{
	const UMegalithSettings* Settings = UMegalithSettings::Get();
...
FString UMegalithSourceSubsystem::GetEngineShaderPath() const
{
	return FPaths::ConvertRelativePathToFull(FPaths::EngineDir() / TEXT("Shaders"));
}

FString UMegalithSourceSubsystem::GetProjectPath() const
{
