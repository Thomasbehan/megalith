...
FMegalithHttpServer::FMegalithHttpServer()
{
}
...
bool FMegalithHttpServer::Start(int32 Port)
{
	if (bIsRunning)
	{
		UE_LOG(LogMegalith, Warning, TEXT("HTTP server already running on port %d"), BoundPort);
		return true;
	}
	...
	for (int32 Attempt = 1; Attempt <= MaxAttempts; ++Attempt)
	{
		if (Attempt > 1)
		{
			UE_LOG(LogMegalith, Warning, TEXT("HTTP bind attempt %d/%d on port %d — waiting %.1fs"),
		...
		{
			UE_LOG(LogMegalith, Warning, TEXT("GetHttpRouter failed on port %d (attempt %d)"), Port, Attempt);
			continue;
		}
		...
		if (ProbePort(Port))
		{
			...
			UE_LOG(LogMegalith, Log, TEXT("Megalith MCP server listening on port %d (attempt %d)"), Port, Attempt);
			return true;
		}

		UE_LOG(LogMegalith, Warning, TEXT("Port %d not listening after StartAllListeners (attempt %d)"), Port, Attempt);
	}

	UE_LOG(LogMegalith, Error, TEXT("Failed to bind Megalith MCP server on port %d after %d attempts (~%ds total)"),
...
void FMegalithHttpServer::Stop()
{
	...
	UE_LOG(LogMegalith, Log, TEXT("Megalith MCP server stopped"));
}

void FMegalithHttpServer::DeactivateRoutes()
{
	if (HttpRouter.IsValid())
	{
	...
	// enough to take Megalith off the wire; the listener keeps holding the
	// port until the editor exits.
	bIsRunning = false;
	BoundPort = 0;
}

void FMegalithHttpServer::BindRoutes()
{
	...
	RouteHandles.Add(HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FMegalithHttpServer::HandlePostMcp)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp")),
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FMegalithHttpServer::HandleGetMcp)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp")),
		EHttpServerRequestVerbs::VERB_DELETE,
		FHttpRequestHandler::CreateRaw(this, &FMegalithHttpServer::HandleDeleteMcp)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp")),
		EHttpServerRequestVerbs::VERB_OPTIONS,
		FHttpRequestHandler::CreateRaw(this, &FMegalithHttpServer::HandleOptions)));

	RouteHandles.Add(HttpRouter->BindRoute(
		FHttpPath(TEXT("/health")),
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FMegalithHttpServer::HandleHealthCheck)));
...
bool FMegalithHttpServer::ProbePort(int32 Port)
{
	...
	FSocket* Socket = SocketSubsystem->CreateSocket(NAME_Stream, TEXT("MegalithProbe"), false);
...
bool FMegalithHttpServer::Restart(int32 Port)
{
	DeactivateRoutes();
...
bool FMegalithHttpServer::HandlePostMcp(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	{
		TSharedPtr<FJsonObject> Err = FMegalithJsonUtils::ErrorResponse(
			nullptr, FMegalithJsonUtils::ErrParseError, TEXT("Empty request body — send a JSON-RPC 2.0 request, e.g. {\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\"}."));
		auto Response = MakeJsonResponse(FMegalithJsonUtils::Serialize(Err), EHttpServerResponseCodes::BadRequest);
	...
	// Try parse as JSON
	TSharedPtr<FJsonObject> JsonRequest = FMegalithJsonUtils::Parse(BodyString);
	...
	else
	{
		...
		{
			TSharedPtr<FJsonObject> Err = FMegalithJsonUtils::ErrorResponse(
				nullptr, FMegalithJsonUtils::ErrParseError, TEXT("Invalid JSON — body must be a valid JSON-RPC 2.0 request object or an array of them for batch."));
			auto Response = MakeJsonResponse(FMegalithJsonUtils::Serialize(Err), EHttpServerResponseCodes::BadRequest);
			AddCorsHeaders(*Response, Request);
			OnComplete(MoveTemp(Response));
			return true;
	...
	{
		ResponseBody = FMegalithJsonUtils::Serialize(Responses[0]);
	}
	else
	{
...
bool FMegalithHttpServer::HandleGetMcp(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
...
bool FMegalithHttpServer::HandleDeleteMcp(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	auto Response = FHttpServerResponse::Ok();
	AddCorsHeaders(*Response, Request);
	OnComplete(MoveTemp(Response));
...
bool FMegalithHttpServer::HandleOptions(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	auto Response = FHttpServerResponse::Ok();
	AddCorsHeaders(*Response, Request);
	OnComplete(MoveTemp(Response));
...
bool FMegalithHttpServer::HandleHealthCheck(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	Health->SetStringField(TEXT("version"), MEGALITH_VERSION);
	...
	Health->SetNumberField(TEXT("tools_registered"), FMegalithToolRegistry::Get().GetActionCount());
...
TSharedPtr<FJsonObject> FMegalithHttpServer::ProcessJsonRpcRequest(const TSharedPtr<FJsonObject>& Request)
{
	if (!Request.IsValid())
	{
		return FMegalithJsonUtils::ErrorResponse(nullptr, FMegalithJsonUtils::ErrInvalidRequest, TEXT("Invalid request object — must be a JSON object with jsonrpc, method, and id fields."));
	}
	...
	{
		return FMegalithJsonUtils::ErrorResponse(nullptr, FMegalithJsonUtils::ErrInvalidRequest, TEXT("Missing or invalid jsonrpc version — set \"jsonrpc\" to the string \"2.0\"."));
	}
	...
	{
		return FMegalithJsonUtils::ErrorResponse(nullptr, FMegalithJsonUtils::ErrInvalidRequest, TEXT("Missing method field — set \"method\" to one of: initialize, tools/list, tools/call, ping."));
	}
	...
	UE_LOG(LogMegalith, Verbose, TEXT("JSON-RPC: %s (id=%s)"), *Method, Id.IsValid() ? *Id->AsString() : TEXT("notification"));
	...
	{
		Response = FMegalithJsonUtils::ErrorResponse(Id, FMegalithJsonUtils::ErrMethodNotFound,
...
TSharedPtr<FJsonObject> FMegalithHttpServer::HandleInitialize(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& Params)
{
	...
	ServerInfo->SetStringField(TEXT("name"), TEXT("megalith"));
	ServerInfo->SetStringField(TEXT("version"), MEGALITH_VERSION);
	...
	Result->SetStringField(TEXT("instructions"),
		TEXT("Megalith MCP server for Unreal Engine. ")
		TEXT("Before calling a domain action, check its schema instead of guessing: ")
		TEXT("megalith_discover() lists namespaces, megalith_discover('<namespace>') lists a ")
		...
		TEXT("exact parameter schema. megalith_guide(section='recipes') gives cross-namespace ")
		TEXT("workflows, decision matrices, and gotchas."));

	return FMegalithJsonUtils::SuccessResponse(Id, MakeShared<FJsonValueObject>(Result));
}

TSharedPtr<FJsonObject> FMegalithHttpServer::HandleToolsList(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		TArray<FMegalithActionInfo> Actions = Registry.GetActions(Namespace);
		...
		if (Namespace == TEXT("megalith"))
		{
			// Core tools are individual: megalith_discover, megalith_status
			for (const FMegalithActionInfo& ActionInfo : Actions)
			{
				TSharedPtr<FJsonObject> CoreTool = MakeShared<FJsonObject>();
				CoreTool->SetStringField(TEXT("name"), FString::Printf(TEXT("megalith_%s"), *ActionInfo.Action));
		...
		else
		{
			...
			// the list on demand via megalith_discover("<namespace>").
			...
			const FString Description = FString::Printf(
				TEXT("Query the %s domain. Call megalith_discover(\"%s\") for the action list (name + description); pass detail=true or call describe_query action_schema for an action's full param schema."),
				*Namespace, *Namespace);
			...
			// describe_query action_schema (or megalith_discover detail=true).
			...
			ParamsProp->SetStringField(TEXT("description"),
				FString::Printf(TEXT("Parameters for the action. megalith_discover(\"%s\") is terse by default (name + description); pass detail=true for full param schemas, or call describe_query action_schema for one action."), *Namespace));
			...
			// FMegalithToolRegistry::SetDispatcherAnnotations at module init).
			// Untagged dispatchers leave IsAnyNonDefault()==false → no
			// `annotations` block on the wire.
			const FMegalithDispatcherAnnotations DispatcherAnn = Registry.GetDispatcherAnnotations(Namespace);
...
TSharedPtr<FJsonObject> FMegalithHttpServer::HandleToolsCall(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FMegalithJsonUtils::ErrorResponse(Id, FMegalithJsonUtils::ErrInvalidParams, TEXT("Missing params — tools/call params must include \"name\" and optionally \"arguments\"."));
	}

	FString ToolName;
	if (!Params->TryGetStringField(TEXT("name"), ToolName))
	{
		return FMegalithJsonUtils::ErrorResponse(Id, FMegalithJsonUtils::ErrInvalidParams, TEXT("Missing tool name — set params.name to a tool like megalith_discover or <namespace>_query."));
	}
	...
	if (ToolName.StartsWith(TEXT("megalith_")))
	{
		// Core tool: megalith_discover -> namespace="megalith", action="discover"
		Namespace = TEXT("megalith");
	...
	else if (ToolName.EndsWith(TEXT("_query")) || ToolName.EndsWith(TEXT(".query")))
	{
		...
		{
			return FMegalithJsonUtils::ErrorResponse(Id, FMegalithJsonUtils::ErrInvalidParams,
				TEXT("Missing 'action' in arguments — for *_query tools, set arguments.action; call megalith_discover(\"<namespace>\") to enumerate."));
		}
	...
	{
		return FMegalithJsonUtils::ErrorResponse(Id, FMegalithJsonUtils::ErrMethodNotFound,
			FString::Printf(TEXT("Unknown tool: %s — tool must start with megalith_ or end with _query; call tools/list to enumerate."), *ToolName));
	}
	...
	FMegalithActionResult ActionResult = FMegalithToolRegistry::Get().ExecuteAction(Namespace, Action, Arguments);

	// Calculate duration
	double DurationMs = (FPlatformTime::Seconds() - ActionStartTimeSeconds) * 1000.0;
	UE_LOG(LogMegalith, Verbose, TEXT("Megalith action %s.%s completed in %.2f ms"), *Namespace, *Action, DurationMs);
	...
	if (ActionResult.bSuccess)
	{
		...
		{
			TextContent->SetStringField(TEXT("text"), FMegalithJsonUtils::Serialize(ActionResult.Result));
		}
		else
		{
...
TSharedPtr<FJsonObject> FMegalithHttpServer::HandlePing(const TSharedPtr<FJsonValue>& Id)
{
	return FMegalithJsonUtils::SuccessResponse(Id, MakeShared<FJsonValueObject>(MakeShared<FJsonObject>()));
}
...
TUniquePtr<FHttpServerResponse> FMegalithHttpServer::MakeJsonResponse(const FString& JsonBody, EHttpServerResponseCodes Code)
{
...
TUniquePtr<FHttpServerResponse> FMegalithHttpServer::MakeSseResponse(const TArray<TSharedPtr<FJsonObject>>& Messages)
{
	FString SseBody;
	for (const TSharedPtr<FJsonObject>& Msg : Messages)
	{
		SseBody += TEXT("event: message\ndata: ");
		SseBody += FMegalithJsonUtils::Serialize(Msg);
		SseBody += TEXT("\n\n");
	}
...
void FMegalithHttpServer::AddCorsHeaders(FHttpServerResponse& Response, const FHttpServerRequest& Request)
{
