...
FString FMegalithAssetUtils::ResolveAssetPath(const FString& InPath)
{
	FString Path = InPath;
...
UPackage* FMegalithAssetUtils::LoadPackageByPath(const FString& AssetPath)
{
	FString Resolved = ResolveAssetPath(AssetPath);
	UPackage* Package = LoadPackage(nullptr, *Resolved, LOAD_None);
	if (!Package)
	{
		UE_LOG(LogMegalith, Warning, TEXT("Failed to load package: %s"), *Resolved);
	}
	return Package;
}

UObject* FMegalithAssetUtils::LoadAssetByPath(const FString& AssetPath)
{
...
UObject* FMegalithAssetUtils::LoadAssetByPath(UClass* ExpectedClass, const FString& AssetPath)
{
	...
	UE_LOG(LogMegalith, Warning, TEXT("Failed to load asset: %s (tried: %s)"), *AssetPath, *NormalizedFull);
	return nullptr;
}

bool FMegalithAssetUtils::AssetExists(const FString& AssetPath)
{
	FString Resolved = ResolveAssetPath(AssetPath);
...
TArray<FAssetData> FMegalithAssetUtils::GetAssetsByClass(const FTopLevelAssetPath& ClassPath, const FString& PackagePath)
{
	IAssetRegistry& AssetRegistry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry").Get();
...
FString FMegalithAssetUtils::GetAssetName(const FString& AssetPath)
{
