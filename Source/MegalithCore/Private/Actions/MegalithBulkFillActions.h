...
// registration. Pattern mirrors MegalithCoreTools.h.
 ...
 * per-namespace handlers via FMegalithBulkFillRegistry; this class wires the
 * outer JSON-RPC actions that route to the registry.
 */
class FMegalithBulkFillActions
{
public:
	/** Register the 4 actions (bulk_fill.apply, bulk_fill.list_namespaces, describe.schema, describe.list_targets). */
	static void RegisterAll();

	/** Called from FMegalithCoreModule::ShutdownModule (mirrors UnregisterNamespace pattern). */
