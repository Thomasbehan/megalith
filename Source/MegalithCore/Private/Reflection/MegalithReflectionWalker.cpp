...
// FMegalithReflectionWalker implementation. Phase 0 framework primitive.
...
namespace
{
	...
	FString ComposeExampleToken(const FProperty* Prop, int32 Depth)
	{
		...
		if (!Enum)
		{
			Enum = FMegalithReflectionWalker::RecoverUserDefinedEnum(Prop);
		}
		if (Enum)
		{
...
// ---------------------------------------------------------------------------
// Lookup helper. Mirrors MegalithBlueprintCDOActions.cpp:385-396 — exact match
...
// ---------------------------------------------------------------------------
FProperty* FMegalithReflectionWalker::FindPropertyForwarding(UStruct* Struct, const FString& Name)
{
	if (!Struct)
	{
		return nullptr;
...
// ---------------------------------------------------------------------------
FMegalithReflectionWalker::FPathResolveResult FMegalithReflectionWalker::ResolvePath(
	UStruct* TopStruct,
	void* Container,
	const FString& Path,
...
// ---------------------------------------------------------------------------
FBulkFillFieldWrite FMegalithReflectionWalker::WriteLeaf(
	FProperty* LeafProp,
	void* LeafPtr,
...
// ---------------------------------------------------------------------------
UEnum* FMegalithReflectionWalker::RecoverUserDefinedEnum(const FProperty* Prop)
{
#if WITH_EDITOR
	if (!Prop)
	{
...
// ---------------------------------------------------------------------------
bool FMegalithReflectionWalker::ResolveUserDefinedEnumToken(const FProperty* Prop, const FString& Token, int64& OutValue)
{
#if WITH_EDITOR
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::DispatchByPropertyType(
	FProperty* Prop,
	void* ValuePtr,
	const TSharedPtr<FJsonValue>& JsonVal,
	UObject* Owner,
...
// MegalithBlueprintCDOActions.cpp:451-475 path so behaviour matches set_cdo_property).
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteScalar(FProperty* Prop, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, UObject* Owner, FBulkFillFieldWrite& OutWrite)
{
	FString ValStr;
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteEnum(FEnumProperty* EnumProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, FBulkFillFieldWrite& OutWrite)
{
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteSoftObjectRef(FSoftObjectProperty* SoftProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, FBulkFillFieldWrite& OutWrite)
{
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteObjectRef(FObjectProperty* ObjProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, UObject* /*Container*/, FBulkFillFieldWrite& OutWrite)
{
	const FString PathStr = JsonVal->AsString();
	OutWrite.ProposedValue = PathStr;
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteArray(FArrayProperty* ArrayProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, UObject* Owner, const FBulkFillSpec& Spec, FDryRunReport& OutReport, const FString& PathPrefix, FBulkFillFieldWrite& OutWrite)
{
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteMap(FMapProperty* MapProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, UObject* Owner, const FBulkFillSpec& Spec, FDryRunReport& OutReport, const FString& PathPrefix, FBulkFillFieldWrite& OutWrite)
{
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteSet(FSetProperty* SetProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, UObject* Owner, const FBulkFillSpec& Spec, FDryRunReport& OutReport, const FString& PathPrefix, FBulkFillFieldWrite& OutWrite)
{
	const TArray<TSharedPtr<FJsonValue>>* JsonArray = nullptr;
	if (!JsonVal->TryGetArray(JsonArray) || !JsonArray)
	{
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::WriteStruct(FStructProperty* StructProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, UObject* Owner, const FBulkFillSpec& Spec, FDryRunReport& OutReport, const FString& PathPrefix, FBulkFillFieldWrite& OutWrite)
{
...
// ---------------------------------------------------------------------------
FDryRunReport FMegalithReflectionWalker::WriteTree(
...
// Guarantees: test Leviathan.Megalith.Reflection.DryRunNoSideEffects passes.
// ---------------------------------------------------------------------------
FDryRunReport FMegalithReflectionWalker::InspectTree(
	const TSharedPtr<FJsonObject>& Tree,
	UStruct* TopStruct,
...
// ---------------------------------------------------------------------------
void FMegalithReflectionWalker::PopulateClampMeta(FProperty* Prop, FSchemaDescriptor& OutDesc)
{
...
// ---------------------------------------------------------------------------
FSchemaDescriptor FMegalithReflectionWalker::DescribeStruct(UStruct* TopStruct, int32 MaxDepth)
{
