...
// FMegalithDryRunGuard implementation. Phase 0 framework primitive.
...
FMegalithDryRunGuard::FMegalithDryRunGuard(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return;
...
TSharedPtr<FJsonObject> FMegalithDryRunGuard::ReportToJson(const FDryRunReport& Report)
{
...
FMegalithActionResult FMegalithDryRunGuard::MakeDryRunResponse(const FDryRunReport& Report) const
{
	return FMegalithActionResult::Success(ReportToJson(Report));
}
