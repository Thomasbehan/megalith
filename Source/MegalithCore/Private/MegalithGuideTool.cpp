...
void FMegalithGuideTool::RegisterAll()
{
	...
	Registry.RegisterAction(
		TEXT("megalith"), TEXT("guide"),
		TEXT("Editorial cross-namespace workflow guide for AI agents: onboarding script, cross-namespace recipes, X-vs-Y decision matrices, error-to-recovery maps, namespace->Skill pointers, and Megalith-specific gotchas. Section-keyed (pass section=\"recipes\") to bound context cost. Hand-authored markdown + a live registry overlay (per-namespace action counts, plugin version)."),
		FMegalithActionHandler::CreateStatic(&FMegalithGuideTool::HandleGuide),
		Schema
	);
	...
	Registry.SetActionAnnotations(TEXT("megalith"), TEXT("guide"),
		/*bReadOnly=*/true, /*bDestructive=*/false, /*bIdempotent=*/true,
		TEXT("Megalith usage guide"));
}

bool FMegalithGuideTool::LoadGuideMarkdown(FString& OutMarkdown, FString& OutErrorMessage)
{
	...
	// Resolve the plugin base dir, then Docs/MEGALITH_GUIDE.md.
	...
	const FString GuidePath = FPaths::Combine(Plugin->GetBaseDir(), TEXT("Docs"), TEXT("MEGALITH_GUIDE.md"));

	FString FileContents;
	if (!FFileHelper::LoadFileToString(FileContents, *GuidePath))
	{
		OutErrorMessage = FString::Printf(
			TEXT("Guide markdown not found or unreadable at '%s'. Restore Docs/MEGALITH_GUIDE.md (it ships in the release zip) or pull the latest Megalith release; then call megalith_guide() again."),
			*GuidePath);
		return false;
	}
...
void FMegalithGuideTool::SplitSections(const FString& Markdown, TMap<FString, FString>& OutSections, TArray<FString>& OutOrderedNames)
{
	OutSections.Reset();
...
TSharedPtr<FJsonObject> FMegalithGuideTool::BuildLiveOverlay()
{
...
FMegalithActionResult FMegalithGuideTool::HandleGuide(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(LoadError, FMegalithJsonUtils::ErrInternalError);
	}
