...
// Automation tests for terse megalith_discover(namespace).
// Plan: Plugins/Megalith/Docs/plans/2026-06-19-terse-discover.md (Field 10)
//
// Goals:
...
// MegalithCore tests in this folder.
...
namespace MegalithDiscoverTerseTestDetail
{
	/** Run megalith.discover with the given params. */
	static FMegalithActionResult Discover(const TSharedPtr<FJsonObject>& Params)
	{
		return FMegalithToolRegistry::Get().ExecuteAction(TEXT("megalith"), TEXT("discover"), Params);
	}

	/** Build a map of action name -> emitted description from a discover result's actions array. */
	static TMap<FString, FString> DescriptionsByAction(const FMegalithActionResult& R)
	{
	...
	/** Full (unfiltered) action count for the megalith namespace — the discoverability baseline. */
	static int32 FullActionCount()
	{
		return FMegalithToolRegistry::Get().GetActions(TEXT("megalith")).Num();
	}

	/** Pull the `actions` array out of a successful discover result, or null. */
	static const TArray<TSharedPtr<FJsonValue>>* GetActionsArray(const FMegalithActionResult& R)
	{
		if (!R.bSuccess || !R.Result.IsValid())
		{
			return nullptr;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverTerseDefaultTest,
	"Megalith.Discover.Terse.Default",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverTerseDefaultTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;

	TSharedPtr<FJsonObject> Params = MakeShared<FJsonObject>();
	Params->SetStringField(TEXT("namespace"), TEXT("megalith"));

	const FMegalithActionResult R = Discover(Params);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverDetailOptInTest,
	"Megalith.Discover.Terse.DetailOptIn",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverDetailOptInTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
	...
	if (Arr)
	{
		// In detail mode every megalith action that carries a ParamSchema emits
		// `params`. At least one megalith action (discover/update) has a schema, so
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverVerboseAliasTest,
	"Megalith.Discover.Terse.VerboseAlias",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverVerboseAliasTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;

	TSharedPtr<FJsonObject> VerboseParams = MakeShared<FJsonObject>();
	VerboseParams->SetStringField(TEXT("namespace"), TEXT("megalith"));
	VerboseParams->SetBoolField(TEXT("verbose"), true);
	const FMegalithActionResult VerboseR = Discover(VerboseParams);
	TestTrue(TEXT("verbose discover succeeds"), VerboseR.bSuccess);

	TSharedPtr<FJsonObject> DetailParams = MakeShared<FJsonObject>();
	DetailParams->SetStringField(TEXT("namespace"), TEXT("megalith"));
	DetailParams->SetBoolField(TEXT("detail"), true);
	const FMegalithActionResult DetailR = Discover(DetailParams);
	...
	{
		const FString VerboseStr = FMegalithJsonUtils::Serialize(VerboseR.Result);
		const FString DetailStr = FMegalithJsonUtils::Serialize(DetailR.Result);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverFilterTest,
	"Megalith.Discover.Terse.Filter",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverFilterTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverFullListTest,
	"Megalith.Discover.Terse.FullList",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverFullListTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverPaginationTest,
	"Megalith.Discover.Terse.Pagination",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverPaginationTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
...
// actually fires). HardCap mirrors MegalithTerseOneLineDescription (150).
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverTerseTrimInvariantTest,
	"Megalith.Discover.Terse.TrimInvariant",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverTerseTrimInvariantTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
	...
	const FMegalithActionResult TerseR =
		FMegalithToolRegistry::Get().ExecuteAction(TEXT("megalith"), TEXT("discover"), TerseParams);

	TSharedPtr<FJsonObject> DetailParams = MakeShared<FJsonObject>();
	DetailParams->SetStringField(TEXT("namespace"), TEXT("blueprint"));
	DetailParams->SetBoolField(TEXT("detail"), true);
	const FMegalithActionResult DetailR =
		FMegalithToolRegistry::Get().ExecuteAction(TEXT("megalith"), TEXT("discover"), DetailParams);
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverUnknownNamespaceTest,
	"Megalith.Discover.Terse.UnknownNamespace",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverUnknownNamespaceTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverPaginationOverflowTest,
	"Megalith.Discover.Terse.PaginationOverflow",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverPaginationOverflowTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithDiscoverCrossNamespaceTest,
	"Megalith.Discover.Terse.CrossNamespace",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithDiscoverCrossNamespaceTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithDiscoverTerseTestDetail;
	...
	// Explicit limit=0 still means ALL, matching the per-namespace contract.
	{
		...
		const FMegalithActionResult AllR = Discover(AllParams);
	...
	// A whitespace-only filter must NOT flip into cross-namespace mode.
	{
		...
		const FMegalithActionResult WsR = Discover(WsParams);
