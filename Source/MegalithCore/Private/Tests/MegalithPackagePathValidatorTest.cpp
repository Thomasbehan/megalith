...
// Automation tests for MegalithCore::ValidatePackagePath.
//
...
namespace MegalithPackagePathTestDetail
{
	struct FPassCase
	{
		const TCHAR* Input;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithCorePackagePathTest,
	"Megalith.Core.PackagePath",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithCorePackagePathTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithPackagePathTestDetail;
	...
	for (const FPassCase& Case : PassCases)
	{
		...
		const FString Error = MegalithCore::ValidatePackagePath(Path);
	...
	// host project enables.
	{
		...
		{
			FString Path = PluginRoot + TEXT("MegalithPathCase/Asset_01");
			...
			FString ObjectPath = PluginRoot + TEXT("MegalithPathCase/Asset_01.Asset_01");
			const FString ObjectError = MegalithCore::ValidatePackagePath(ObjectPath);
			TestTrue(
	...
	const FFailCase FailCases[] =
	{
		...
		{ TEXT("/MegalithNotAMountPoint/Foo"), TEXT("unmounted root") },
	...
	// -----------------------------------------------------------------------
	{
		...
		const FString Error = MegalithCore::ValidatePackagePath(FString(TEXT("/Game/Foo Bar")), Out);
	...
	{
		FString Out;
		const FString Error = MegalithCore::ValidatePackagePath(FString(TEXT("/Game/Foo/M_Bar.M_Bar")), Out);
