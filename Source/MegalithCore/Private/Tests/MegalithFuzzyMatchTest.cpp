...
// This file lives under `Source/MegalithCore/Private/Tests/...` so UBT's
// auto-include of `Private/` picks it up without a Build.cs change. Matches the
// existing precedent at `Private/Tests/MegalithResponseShapingTest.cpp` and
// `Private/Reflection/Tests/MegalithReflectionWalkerTest.cpp`.
...
namespace MegalithFuzzyMatchTestDetail
{
	...
	static bool GetSuggestions(
		const FMegalithActionResult& Result,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithFuzzyMatchActionTypoCorpusTest,
	"Megalith.FuzzyMatch.ActionTypoCorpus",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithFuzzyMatchActionTypoCorpusTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithFuzzyMatchTestDetail;
	FMegalithToolRegistry& Registry = FMegalithToolRegistry::Get();

	// Skip if the `megalith` namespace is somehow not registered — the test
	// has no signal to assert on.
	if (Registry.GetActions(TEXT("megalith")).Num() == 0)
	{
		AddWarning(TEXT("Skipping: `megalith` namespace not registered in this test run"));
		return true;
	}

	// Three typo variants of well-known megalith actions.
	const TArray<TPair<FString, FString>> Typos = {
		{ TEXT("monolih_discover"), TEXT("discover") }, // dispatched as namespace="megalith", action="monolih_discover"
	...
	{
		FMegalithActionResult Result = Registry.ExecuteAction(
			TEXT("megalith"), T.Key, MakeShared<FJsonObject>());
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithFuzzyMatchNamespaceTypoCorpusTest,
	"Megalith.FuzzyMatch.NamespaceTypoCorpus",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithFuzzyMatchNamespaceTypoCorpusTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithFuzzyMatchTestDetail;
	FMegalithToolRegistry& Registry = FMegalithToolRegistry::Get();
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithFuzzyMatchNoMatchPathTest,
	"Megalith.FuzzyMatch.NoMatchPath",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithFuzzyMatchNoMatchPathTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithFuzzyMatchTestDetail;
	FMegalithToolRegistry& Registry = FMegalithToolRegistry::Get();
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithFuzzyMatchLockReleaseHandoverTest,
	"Megalith.FuzzyMatch.LockReleaseHandover",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithFuzzyMatchLockReleaseHandoverTest::RunTest(const FString& /*Parameters*/)
{
	FMegalithToolRegistry& Registry = FMegalithToolRegistry::Get();
	...
	double BaselineSec = 0.0;
	{
		...
		FMegalithActionResult R = Registry.ExecuteAction(
			TEXT("megalith"), TEXT("status"), MakeShared<FJsonObject>());
...
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMegalithFuzzyMatchScoringOrderTest,
	"Megalith.FuzzyMatch.ScoringOrder",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithFuzzyMatchScoringOrderTest::RunTest(const FString& /*Parameters*/)
{
	using namespace MegalithFuzzyMatchTestDetail;
	...
	TArray<MegalithFuzzyMatchDetail::FFuzzyCandidate> Top3 =
		MegalithFuzzyMatchDetail::ScoreFuzzyMatches(Needle, Corpus, 3);
