...
{
	FString Namespace;
	FString SettingsField;   // bool property name on UMegalithSettings
	...
	static const TArray<FKnownOptionalModule> Modules = {
		{
			...
			TEXT("bEnableGAS"),
			TEXT("gas_query"),
			TEXT("MegalithGAS module provides Gameplay Ability System tooling (attributes, abilities, effects, cues). Requires GameplayAbilities plugin (engine-bundled).")
		},
		{
			TEXT("combograph"),
			TEXT("bEnableComboGraph"),
			TEXT("combograph_query"),
			TEXT("MegalithComboGraph module provides combo graph tooling (nodes, edges, transitions, effects). Requires ComboGraph plugin (Fab marketplace).")
		}
	};
	return Modules;
}
...
static FString MegalithTerseOneLineDescription(const FString& Full)
{
...
void FMegalithCoreTools::RegisterAll()
{
	...
	// megalith_discover
	{
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&FMegalithCoreTools::HandleDiscover),
			Schema
		);
		// Survivor A (plan §3.A) — read-only + idempotent enumeration.
		Registry.SetActionAnnotations(TEXT("megalith"), TEXT("discover"),
			/*bReadOnly=*/true, /*bDestructive=*/false, /*bIdempotent=*/true,
			TEXT("Discover Megalith actions"));
	}

	// megalith_status
	{
		Registry.RegisterAction(
			TEXT("megalith"), TEXT("status"),
			TEXT("Get Megalith server health: version, uptime, port, registered action count, module status."),
			FMegalithActionHandler::CreateStatic(&FMegalithCoreTools::HandleStatus)
		);
		// Survivor A (plan §3.A) — pure server-health probe; read-only + idempotent.
		Registry.SetActionAnnotations(TEXT("megalith"), TEXT("status"),
			/*bReadOnly=*/true, /*bDestructive=*/false, /*bIdempotent=*/true,
			TEXT("Megalith server status"));
	}
	...
	// megalith_update
	{
		...
		Registry.RegisterAction(
			TEXT("megalith"), TEXT("update"),
			TEXT("Check for or install Megalith updates from GitHub Releases."),
			FMegalithActionHandler::CreateStatic(&FMegalithCoreTools::HandleUpdate),
			Schema
		);
		// Survivor A (plan §3.A) — DELIBERATELY UNANNOTATED. The 'install'
		// action variant modifies plugin source on disk and is not safely
		// read-only. Per plan §3.A: "DO NOT annotate megalith_update".
	}

	// megalith_reindex
	{
		Registry.RegisterAction(
			TEXT("megalith"), TEXT("reindex"),
			TEXT("Re-index the Megalith project database. Incremental by default (delta only). Pass force=true for full wipe+rebuild."),
			FMegalithActionHandler::CreateStatic(&FMegalithCoreTools::HandleReindex)
		);
		...
		Registry.SetActionAnnotations(TEXT("megalith"), TEXT("reindex"),
			/*bReadOnly=*/false, /*bDestructive=*/false, /*bIdempotent=*/true,
			TEXT("Rebuild Megalith index"));
	}

	// megalith_guide — editorial cross-namespace workflow guide (separate tool file,
	// one-tool-per-file; registers into the "megalith" namespace).
	FMegalithGuideTool::RegisterAll();
}

FMegalithActionResult FMegalithCoreTools::HandleDiscover(const TSharedPtr<FJsonObject>& Params)
{
	FMegalithToolRegistry& Registry = FMegalithToolRegistry::Get();
	...
	auto MatchesFilter = [&Filter](const FMegalithActionInfo& Info)
	{
		return Info.Action.Contains(Filter, ESearchCase::IgnoreCase)
			|| Info.Description.Contains(Filter, ESearchCase::IgnoreCase);
	};
	auto MakeActionValue = [&bDetail](const FMegalithActionInfo& Info, bool bIncludeNamespace)
	{
		...
		ActionObj->SetStringField(TEXT("description"),
			bDetail ? Info.Description : MegalithTerseOneLineDescription(Info.Description));
	...
	if (!FilterNamespace.IsEmpty())
	{
		...
		TArray<FMegalithActionInfo> Actions = Registry.GetActions(FilterNamespace);
		if (Actions.Num() == 0)
		{
			...
			if (Found)
			{
				...
				if (Settings)
				{
					const FBoolProperty* Prop = CastField<FBoolProperty>(
						UMegalithSettings::StaticClass()->FindPropertyByName(*Found->SettingsField));
					if (Prop)
					{
				...
				if (!bSettingEnabled)
				{
					...
					Result->SetStringField(TEXT("hint"),
						FString::Printf(TEXT("Enable in Project Settings > Plugins > Megalith > Modules > Optional (%s), then restart the editor."),
							*Found->SettingsField));
				}
				else
				{
		...
		{
			Actions = Actions.FilterByPredicate([&FilterCategory](const FMegalithActionInfo& Info)
			{
	...
	else
	{
		...
		for (const FString& Ns : Namespaces)
		{
			TArray<FMegalithActionInfo> Actions = Registry.GetActions(Ns);
		...
		for (const FKnownOptionalModule& Mod : OptionalModules)
		{
			...
			{
				const FBoolProperty* Prop = CastField<FBoolProperty>(
					UMegalithSettings::StaticClass()->FindPropertyByName(*Mod.SettingsField));
				if (Prop)
				{
					bSettingEnabled = Prop->GetPropertyValue_InContainer(Settings);
				}
			...
			OptObj->SetStringField(TEXT("hint"), bSettingEnabled ? Mod.InstallHint
				: FString::Printf(TEXT("Enable in Project Settings > Plugins > Megalith > Modules > Optional (%s), then restart the editor."), *Mod.SettingsField));
		...
		Result->SetStringField(TEXT("guide_hint"), TEXT("Call megalith_guide() for editorial cross-namespace workflow recipes, decision matrices, and error-recovery maps. Section-keyed to bound context cost."));
	}

	return FMegalithActionResult::Success(Result);
}

FMegalithActionResult FMegalithCoreTools::HandleStatus(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();

	// Version
	Result->SetStringField(TEXT("version"), MEGALITH_VERSION);

	// Server status
	FMegalithHttpServer* Server = FMegalithCoreModule::Get().GetHttpServer();
...
FMegalithActionResult FMegalithCoreTools::HandleUpdate(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!GEditor)
	{
		return FMegalithActionResult::Error(TEXT("GEditor not available"));
	}

	UMegalithUpdateSubsystem* UpdateSubsystem = GEditor->GetEditorSubsystem<UMegalithUpdateSubsystem>();
	if (!UpdateSubsystem)
	{
		return FMegalithActionResult::Error(TEXT("MegalithUpdateSubsystem not available"));
	}
	...
	{
		const FMegalithVersionInfo& Info = UpdateSubsystem->GetVersionInfo();
...
FMegalithActionResult FMegalithCoreTools::HandleReindex(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();

	if (!FModuleManager::Get().IsModuleLoaded(TEXT("MegalithIndex")))
	{
		Result->SetStringField(TEXT("status"), TEXT("module_not_loaded"));
		Result->SetStringField(TEXT("message"), TEXT("MegalithIndex module is not loaded."));
		return FMegalithActionResult::Success(Result);
	}
	...
	UClass* IndexSubsystemClass = FindObject<UClass>(nullptr, TEXT("/Script/MegalithIndex.MegalithIndexSubsystem"));
	if (!IndexSubsystemClass)
	{
		Result->SetStringField(TEXT("status"), TEXT("subsystem_unavailable"));
		Result->SetStringField(TEXT("message"), TEXT("MegalithIndexSubsystem class not found."));
		return FMegalithActionResult::Success(Result);
	}
	...
	{
		Result->SetStringField(TEXT("status"), TEXT("subsystem_unavailable"));
		Result->SetStringField(TEXT("message"), TEXT("MegalithIndexSubsystem instance not available."));
		return FMegalithActionResult::Success(Result);
	}
	...
	else
	{
		...
		{
			// Fallback if CanDoIncrementalIndex not found (old MegalithIndex version)
			FuncName = TEXT("StartFullIndex");
		}
	...
	// MegalithCore reaches MegalithIndex only through reflection, so the return
	// property is checked rather than assumed. If a future MegalithIndex drops
	...
	if (!CastField<FBoolProperty>(Func->GetReturnProperty()))
	{
		...
		Result->SetStringField(TEXT("message"),
			FString::Printf(TEXT("%s triggered (this MegalithIndex build does not report a start result)."), KindLabel));
		return FMegalithActionResult::Success(Result);
	}
	...
	else
	{
		...
		Result->SetStringField(TEXT("message"),
			FString::Printf(TEXT("%s did not start — indexing is already running, or the index database is not open. Check megalith_status and the Megalith log."), KindLabel));
	}
