...
// =============================================================================
//  FMegalithParamSchema — K2 alias rewriting + K3 unknown-key detection
// =============================================================================

bool FMegalithParamSchema::ApplyAliases(
	...
	FString& OutCollision)
{
	...
	{
		const FString Canonical = MegalithKeyToString(Pair.Key);
...
TArray<FString> FMegalithParamSchema::FindUnknownKeys(
	const TSharedPtr<FJsonObject>& Schema,
	const TSharedPtr<FJsonObject>& Params)
{
	...
	for (const auto& Pair : Schema->Values)
	{
		Allowed.Add(MegalithKeyToString(Pair.Key));
...
bool FMegalithParamSchema::IsStrictParamsEnabled()
{
...
// =============================================================================
//  FMegalithToolRegistry
// =============================================================================

FMegalithToolRegistry& FMegalithToolRegistry::Get()
{
	static FMegalithToolRegistry Instance;
	return Instance;
}

void FMegalithToolRegistry::RegisterAction(
	...
	const FString& Category)
{
	...
	{
		UE_LOG(LogMegalith, Warning, TEXT("Overwriting existing action: %s"), *Key);
	}
	...
	UE_LOG(LogMegalith, Verbose, TEXT("Registered action: %s — %s"), *Key, *Description);
}

void FMegalithToolRegistry::UnregisterNamespace(const FString& Namespace)
{
	FScopeLock Lock(&RegistryLock);
	...
	if (TArray<FString>* Keys = NamespaceActions.Find(Namespace))
	{
		...
		UE_LOG(LogMegalith, Log, TEXT("Unregistered namespace: %s (%d actions)"), *Namespace, Keys->Num());
		NamespaceActions.Remove(Namespace);
	}
}

FMegalithActionResult FMegalithToolRegistry::ExecuteAction(
	const FString& Namespace,
	const FString& Action,
	const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!RegAction)
	{
		...
		TArray<MegalithFuzzyMatchDetail::FFuzzyCandidate> Top3 =
			MegalithFuzzyMatchDetail::ScoreFuzzyMatches(FuzzyNeedle, CandidateKeys, /*TopN=*/3);
		...
		for (const MegalithFuzzyMatchDetail::FFuzzyCandidate& C : Top3)
		{
		...
		if (bKnownNamespace)
		{
			return FMegalithActionResult::Error(
				FString::Printf(TEXT("Unknown action: %s.%s — call megalith_discover(\"%s\") to enumerate valid actions in this namespace.%s"),
					*Namespace, *Action, *Namespace, *MsgSuffix),
				FMegalithJsonUtils::ErrMethodNotFound
			).WithErrorData(ErrorDataObj);
		}
		else
		{
			return FMegalithActionResult::Error(
				FString::Printf(TEXT("Unknown namespace: %s — call megalith_discover() to enumerate valid namespaces.%s"),
					*Namespace, *MsgSuffix),
				FMegalithJsonUtils::ErrMethodNotFound
			).WithErrorData(ErrorDataObj);
		}
	...
	const FMegalithActionInfo& ActionInfo = RegAction->Info;
	...
	{
		FString Collision;
		if (!FMegalithParamSchema::ApplyAliases(ActionInfo.ParamSchema, EffectiveParams, Collision))
		{
			return FMegalithActionResult::Error(Collision, FMegalithJsonUtils::ErrInvalidParams);
		}
	}
	...
	if (ActionInfo.ParamSchema.IsValid())
	{
		for (const auto& SchemaPair : ActionInfo.ParamSchema->Values)
		{
			...
			const FString KeyStr = MegalithKeyToString(SchemaPair.Key);
			FString StrVal;
	...
	if (ActionInfo.ParamSchema.IsValid())
	{
		...
		if (Missing.Num() > 0)
		{
			...
			for (const auto& P : EffectiveParams->Values) Provided.Add(MegalithKeyToString(P.Key));
			return FMegalithActionResult::Error(
				FString::Printf(TEXT("Missing required param(s): [%s]. Provided keys: [%s] — inspect the action's parameter schema via megalith_discover(\"<namespace>\") and supply all required fields."),
	...
	if (ActionInfo.ParamSchema.IsValid())
	{
		for (const auto& SchemaPair : ActionInfo.ParamSchema->Values)
		{
			...
			const EMegalithParamKind Kind = MegalithParamKind::FromString(KindStr);
			if (Kind != EMegalithParamKind::AssetPath && Kind != EMegalithParamKind::DiskPath)
			{
				continue;
			}

			const FString ParamName = MegalithKeyToString(SchemaPair.Key);
			FString Value;
			...
			if (Kind == EMegalithParamKind::AssetPath)
			{
	...
	if (ActionInfo.ParamSchema.IsValid())
	{
		Unknown = FMegalithParamSchema::FindUnknownKeys(ActionInfo.ParamSchema, EffectiveParams);
		...
		if (Unknown.Num() > 0)
		{
			...
			if (FMegalithParamSchema::IsStrictParamsEnabled())
			{
				return FMegalithActionResult::Error(
	...
	FMegalithActionHandler HandlerCopy = RegAction->Handler;
	Lock.Unlock();

	FMegalithActionResult ActionResult = HandlerCopy.Execute(EffectiveParams);
...
TArray<FString> FMegalithToolRegistry::GetNamespaces() const
{
	FScopeLock Lock(&RegistryLock);
...
TArray<FMegalithActionInfo> FMegalithToolRegistry::GetActions(const FString& Namespace) const
{
	FScopeLock Lock(&RegistryLock);
	TArray<FMegalithActionInfo> Result;
...
TArray<FMegalithActionInfo> FMegalithToolRegistry::GetAllActions() const
{
	FScopeLock Lock(&RegistryLock);
	TArray<FMegalithActionInfo> Result;
...
bool FMegalithToolRegistry::HasAction(const FString& Namespace, const FString& Action) const
{
	FScopeLock Lock(&RegistryLock);
	return Actions.Contains(MakeKey(Namespace, Action));
}

int32 FMegalithToolRegistry::GetActionCount() const
{
	FScopeLock Lock(&RegistryLock);
	return Actions.Num();
}

void FMegalithToolRegistry::SetDispatcherAnnotations(
	const FString& Namespace,
	const FMegalithDispatcherAnnotations& Annotations)
{
	FScopeLock Lock(&RegistryLock);
	DispatcherAnnotations.Add(Namespace, Annotations);
}

FMegalithDispatcherAnnotations FMegalithToolRegistry::GetDispatcherAnnotations(const FString& Namespace) const
{
	FScopeLock Lock(&RegistryLock);
	if (const FMegalithDispatcherAnnotations* Found = DispatcherAnnotations.Find(Namespace))
	{
		return *Found;
	}
	return FMegalithDispatcherAnnotations{};
}

void FMegalithToolRegistry::SetActionAnnotations(
	const FString& Namespace,
	const FString& Action,
	bool bReadOnly,
	bool bDestructive,
