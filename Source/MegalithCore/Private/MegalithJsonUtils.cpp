...
DEFINE_LOG_CATEGORY(LogMegalith);

TSharedPtr<FJsonObject> FMegalithJsonUtils::SuccessResponse(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonValue>& Result)
{
...
TSharedPtr<FJsonObject> FMegalithJsonUtils::ErrorResponse(const TSharedPtr<FJsonValue>& Id, int32 Code, const FString& Message, const TSharedPtr<FJsonValue>& Data)
{
...
TSharedPtr<FJsonObject> FMegalithJsonUtils::SuccessObject(const TSharedPtr<FJsonValue>& Id, const TSharedPtr<FJsonObject>& ResultObj)
{
	return SuccessResponse(Id, MakeShared<FJsonValueObject>(ResultObj));
}

TSharedPtr<FJsonObject> FMegalithJsonUtils::SuccessString(const TSharedPtr<FJsonValue>& Id, const FString& Message)
{
...
FString FMegalithJsonUtils::Serialize(const TSharedPtr<FJsonObject>& JsonObject)
{
...
TSharedPtr<FJsonObject> FMegalithJsonUtils::Parse(const FString& JsonString)
{
...
TSharedRef<FJsonValueArray> FMegalithJsonUtils::StringArrayToJson(const TArray<FString>& Strings)
{
...
namespace MegalithResponseShapingDetail
{
	...
	static bool ReadStringArrayParam(const TSharedPtr<FJsonObject>& Params, const TCHAR* Key, TSet<FString>& Out)
	{
		...
		// in MegalithHttpServer.cpp:691-701.
		FString StrValue;
		if (Params->TryGetStringField(Key, StrValue) && !StrValue.IsEmpty())
		{
			...
			if (FJsonSerializer::Deserialize(Reader, ParsedValue) && ParsedValue.IsValid() && ParsedValue->Type == EJson::Array)
			{
				...
				UE_LOG(LogMegalith, Verbose, TEXT("ReadStringArrayParam: recovered stringified JSON array for key '%s' (%d entries)"), Key, Out.Num());
				return Out.Num() > 0;
			}
		}
		...
		TArray<FString>& Warnings)
	{
		...
		for (const auto& Pair : Response->Values)
		{
			...
			{
				ListPayloadKeys.Add(MegalithKeyToString(Pair.Key));
			}
		}
		...
		for (const TSharedPtr<FJsonValue>& RowVal : *RowsPtr)
		{
			...
			for (const auto& KVP : RowObj->Values) { ExistingKeys.Add(MegalithKeyToString(KVP.Key)); }
		...
		TArray<FString>& Warnings)
	{
		...
		if (!bAnyPathResolved)
		{
			...
			for (const auto& KVP : Response->Values) { TopLevelKeys.Add(MegalithKeyToString(KVP.Key)); }
			TopLevelKeys.Sort();
			Warnings.Add(FString::Printf(
		...
		{
			Response->SetField(MegalithKeyToString(Pair.Key), Pair.Value);
		}
	}
	...
	TArray<FString>& Warnings)
{
	...
	const bool bHasFields     = MegalithResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_fields"),      FieldsSet);
	const bool bHasOmit       = MegalithResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_omit"),        OmitSet);
	...
	if (bHasRowFields)
	{
		MegalithResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_row_fields"), RowFieldsSet);
	}
	if (bHasPathFields)
	{
		MegalithResponseShapingDetail::ReadStringArrayParam(Params, TEXT("_path_fields"), PathFieldsSet);
	}
	...
	if (!Params->TryGetBoolField(TEXT("_compact_json"), bCompact))
	{
		...
		if (Params->TryGetStringField(TEXT("_compact_json"), CompactStr) && !CompactStr.IsEmpty())
		{
			...
			if (bCompact)
			{
				UE_LOG(LogMegalith, Verbose, TEXT("ApplyResponseShaping: recovered stringified bool for '_compact_json' ('%s')"), *CompactStr);
			}
		}
	}
	...
	if (bHasFields)
	{
		...
		for (const auto& KVP : Response->Values) { Existing.Add(MegalithKeyToString(KVP.Key)); }
	...
	if (bHasRowFields)
	{
		MegalithResponseShapingDetail::ApplyRowFieldsFilter(Response, RowFieldsSet, Warnings);
	}
	...
	if (bHasPathFields)
	{
		MegalithResponseShapingDetail::ApplyPathFieldsFilter(Response, PathFieldsSet, Warnings);
	}
	...
	if (bCompact)
	{
		...
		for (const FString& K : Existing)
		{
			...
			if (MegalithResponseShapingDetail::IsEmptyForCompact(Val))
			{
				Response->RemoveField(K);
			}
		}
