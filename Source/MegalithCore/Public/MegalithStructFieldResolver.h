 ...
 *
 * Lives in MegalithCore (Public) so module-internal readers in either MegalithAnimation
 * (anim-instance sampling) or MegalithEditor (PIE object property read + function-arg
 * marshalling) share one resolver. Exported via MEGALITHCORE_API.
 */
namespace MegalithStructField
{
	 ...
	 */
	MEGALITHCORE_API FResolved Resolve(const UObject* Container, const FString& Path);
}
