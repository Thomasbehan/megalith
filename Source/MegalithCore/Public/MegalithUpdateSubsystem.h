...
#include "MegalithUpdateSubsystem.generated.h"

struct FMegalithVersionInfo
{
	FString Current;
	FString Pending;
	bool bStaging = false;

	static FMegalithVersionInfo LoadFromDisk();
	void SaveToDisk() const;
	static FString GetVersionFilePath();
};

UCLASS()
class MEGALITHCORE_API UMegalithUpdateSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()
...
public:
	...
	const FMegalithVersionInfo& GetVersionInfo() const { return VersionInfo; }
...
private:
	...
	FMegalithVersionInfo VersionInfo;
	...
	/** SHA256 hash parsed from the GitHub release notes (Megalith-SHA256-v2[-UE5.x]:
