...
// Megalith MCP Ergonomics framework primitives — Phase 0.
// Adapters in subsequent phases (1-5) consume these types via FMegalithBulkFillRegistry.

#pragma once

#include "CoreMinimal.h"
#include "Dom/JsonObject.h"
#include "MegalithBulkFillTypes.generated.h"

/**
 * Per-key write outcome inside a bulk_fill transaction.
 * Emitted by FMegalithReflectionWalker; consumed by FBulkFillResult / FDryRunReport.
 */
USTRUCT(BlueprintType)
struct MEGALITHCORE_API FBulkFillFieldWrite
{
	GENERATED_BODY()

	/** Dotted path inside the tree (e.g. "tests[2].score_equation", "Properties[Icon:soft]"). */
	UPROPERTY(BlueprintReadOnly, Category="Megalith|BulkFill")
	FString Path;
 ...
 */
USTRUCT(BlueprintType)
struct MEGALITHCORE_API FBulkFillSpec
{
	GENERATED_BODY()
 ...
 */
USTRUCT(BlueprintType)
struct MEGALITHCORE_API FDryRunReport
{
	GENERATED_BODY()
 ...
 */
USTRUCT(BlueprintType)
struct MEGALITHCORE_API FSchemaDescriptor
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category="Megalith|Describe")
	FString FieldPath;
	 ...
	 * FMegalithBulkFillActions::DescriptorToJson (MegalithBulkFillActions.cpp), NOT via
	 ...
	 */
	UPROPERTY(BlueprintReadWrite, Category="Megalith|Describe")
	float RangeMin = 0.f;
