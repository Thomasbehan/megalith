...
MEGALITHCORE_API DECLARE_LOG_CATEGORY_EXTERN(LogMegalith, Log, All);
 ...
 *
 * `MegalithKeyToString(Pair.Key)` yields an `FString` from either key type, keeping
 ...
 */
FORCEINLINE const FString& MegalithKeyToString(const FString& Key) { return Key; }
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
FORCEINLINE FString MegalithKeyToString(const UE::FSharedString& Key) { return FString(*Key); }
#endif
 ...
 */
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
FORCEINLINE UE::FSharedString MegalithMakeJsonKey(const FString& Key) { return UE::FSharedString(*Key); }
#else
FORCEINLINE const FString& MegalithMakeJsonKey(const FString& Key) { return Key; }
#endif

class MEGALITHCORE_API FMegalithJsonUtils
{
public:
	...
	// --- Megalith server-defined error codes (JSON-RPC -32000..-32099 range) ---
	//
 ...
 */
MEGALITHCORE_API void ApplyResponseShaping(
