...
#include "MegalithSettings.generated.h"

UENUM()
enum class EMegalithLogVerbosity : uint8
{
	Quiet,
	Normal,
	Verbose,
...
UCLASS(config=Megalith, defaultconfig, meta=(DisplayName="Megalith"))
class MEGALITHCORE_API UMegalithSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	/** Master enable for the Megalith MCP HTTP server. Set false to disable
	...
	UPROPERTY(config, EditAnywhere, Category="Indexing|Performance", DisplayName="Defer First-Time Index",
		meta=(ToolTip="If true, first-time indexing won't run automatically. Use 'Megalith.StartIndex' console command to trigger."))
	...
	UPROPERTY(config, EditAnywhere, Category="Modules|Optional",
		meta=(DisplayName="Enable Blueprint Assist Integration",
			  ToolTip="When enabled and Blueprint Assist is installed, provides enhanced graph formatting via the IMegalithGraphFormatter bridge."))
	...
	/** Log verbosity for Megalith systems */
	UPROPERTY(config, EditAnywhere, Category="Logging")
	EMegalithLogVerbosity LogVerbosity = EMegalithLogVerbosity::Normal;

	// --- Helpers ---

	static const UMegalithSettings* Get();
