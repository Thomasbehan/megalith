...
//   2. Canonical-library — scan `UMegalithUISettings::CanonicalLibraryPath`
...
// `UMegalithUISettings::GeneratedStylesPath`.
//
// Why a static singleton instead of a UEditorSubsystem member:
//   Phase E (parallel to this work) is editing
//   `MegalithUIRegistrySubsystem.cpp` for curated-mapping additions. Owning
 ...
 */
struct MEGALITHUI_API FUIStyleEntry
{
 ...
 */
struct MEGALITHUI_API FUIStyleResolution
{
 ...
 */
struct MEGALITHUI_API FUIStyleCacheStats
{
    int32 CacheSize = 0;
    int64 Hits = 0;
    int64 Misses = 0;
 ...
 * tied to the module (see `MegalithUIModule.cpp` for shutdown).
 */
class MEGALITHUI_API FMegalithUIStyleService
{
public:
    /**
     ...
     */
    static FMegalithUIStyleService& Get();

    /**
     * Tear down the singleton. Called from `FMegalithUIModule::ShutdownModule`
    ...
    /**
     * Public dtor — required because TUniquePtr<FMegalithUIStyleService>'s
     ...
     */
    ~FMegalithUIStyleService() = default;

private:
    FMegalithUIStyleService() = default;
    FMegalithUIStyleService(const FMegalithUIStyleService&) = delete;
    FMegalithUIStyleService& operator=(const FMegalithUIStyleService&) = delete;
