...
// MegalithUICommon.h
//
// Shared, exported helpers for MegalithUI and any consumer that needs the
// same color / widget-blueprint / alignment parsing logic. The historical
// home for this code was the private header `MegalithUIInternal.h` which is
// unreachable from sibling plugins; this header replaces the duplicated
// copies, and the inline namespace `MegalithUIInternal` versions are now
 ...
 * Kept distinct from `LogMegalith` (MegalithCore) so spec-builder iteration
 * noise can be filtered independently of the broader Megalith log surface.
 */
MEGALITHUI_API DECLARE_LOG_CATEGORY_EXTERN(LogMegalithUISpec, Log, All);
...
namespace MegalithUI
{
     ...
     * MegalithUIInternal behaviour — callers wanting failure signalling should
     * use `TryParseColor` instead.
     */
    MEGALITHUI_API FLinearColor ParseColor(const FString& ColorStr);
     ...
     */
    MEGALITHUI_API bool TryParseColor(const FString& InStr, FLinearColor& OutColor);
     ...
     */
    MEGALITHUI_API UWidgetBlueprint* LoadWidgetBlueprint(
     ...
     */
    MEGALITHUI_API UClass* WidgetClassFromName(const FString& ClassName);
     ...
     */
    MEGALITHUI_API FName MakeTokenFromClassName(const UClass* Class);
     ...
     */
    MEGALITHUI_API void RegisterVariableName(UWidgetBlueprint* WBP, const FName& VariableName);

    /** Convenience: register the supplied widget's FName as a variable. */
    MEGALITHUI_API void RegisterCreatedWidget(UWidgetBlueprint* WBP, UWidget* Widget);
     ...
     */
    MEGALITHUI_API void ReconcileWidgetVariableGuids(UWidgetBlueprint* WBP);
    ...
    // provider module. This keeps public Megalith source free of private
    // provider names while still preserving the runtime contract: if the
    // provider is loaded and the class is registered, Megalith can use it.
    //
    // Cache lifecycle:
     ...
     * cleanly.
     */
    MEGALITHUI_API UClass* GetEffectSurfaceClass();

    /** Convenience: returns true iff an optional EffectSurface provider is loaded. */
    MEGALITHUI_API bool IsEffectSurfaceAvailable();
}
