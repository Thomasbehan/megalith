...
// MegalithUISettings.h — Phase G
//
// UDeveloperSettings-derived configuration for the MegalithUI module. Drives
// asset paths used by the style service, cache caps, and validator limits.
// Lives in /Script/MegalithUI.MegalithUISettings — surfaces in Project Settings
// under the "Plugins" category.
//
// Module dependency reminder: UDeveloperSettings is in the `DeveloperSettings`
// module, NOT `Engine`. MegalithUI.Build.cs has been updated accordingly.
...
// MegalithUI registry subsystem to that delegate lets cache caps and paths
// re-apply mid-session — but the wiring is DEFERRED to a follow-up task to
// avoid touching `MegalithUIRegistrySubsystem.cpp` while Phase E owns it for
...
#include "MegalithUISettings.generated.h"

/**
 * Project Settings → Plugins → Megalith UI.
 *
 ...
 *
 * Stored under `[/Script/MegalithUI.MegalithUISettings]` in `DefaultGame.ini`
 * (the `config` UCLASS specifier keys to the per-game settings file).
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "Megalith UI"))
class MEGALITHUI_API UMegalithUISettings : public UDeveloperSettings
{
    GENERATED_BODY()
...
public:
     ...
     */
    static const UMegalithUISettings* Get();
