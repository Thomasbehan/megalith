...
// MegalithUIRegistrySubsystem.cpp
...
void UMegalithUIRegistrySubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    // UMegalithUISettings.PathCacheCap default scheduled for Phase G.
    ...
    ReloadCompleteHandle = FCoreUObjectDelegates::ReloadCompleteDelegate.AddUObject(
        this, &UMegalithUIRegistrySubsystem::OnReloadComplete);

    UE_LOG(LogMegalithUISpec, Log,
        TEXT("UMegalithUIRegistrySubsystem initialised — %d widget types registered"),
        TypeRegistry.Num());
}

void UMegalithUIRegistrySubsystem::Deinitialize()
{
...
UMegalithUIRegistrySubsystem* UMegalithUIRegistrySubsystem::Get()
{
    if (GEditor)
    {
        return GEditor->GetEditorSubsystem<UMegalithUIRegistrySubsystem>();
    }
    return nullptr;
}

void UMegalithUIRegistrySubsystem::RescanWidgetTypes()
{
    PopulateFromReflectionWalk();
    RegisterCuratedMappings();
    ...
    UE_LOG(LogMegalithUISpec, Log,
        TEXT("UMegalithUIRegistrySubsystem rescan complete — %d widget types registered"),
        TypeRegistry.Num());
}

int32 UMegalithUIRegistrySubsystem::RefreshAfterReload()
{
    ...
    UE_LOG(LogMegalithUISpec, Log,
        TEXT("UMegalithUIRegistrySubsystem reload refresh — %d entries refreshed"),
        Refreshed);

    return Refreshed;
}

void UMegalithUIRegistrySubsystem::PopulateFromReflectionWalk()
{
    TypeRegistry.Reset();
    ...
    if (!WidgetBaseClass)
    {
        UE_LOG(LogMegalithUISpec, Error,
    ...
    for (TObjectIterator<UClass> ClassIt; ClassIt; ++ClassIt)
    {
        ...
        Entry.Token = MegalithUI::MakeTokenFromClassName(Class);
    ...
    UE_LOG(LogMegalithUISpec, Verbose,
        TEXT("UMegalithUIRegistrySubsystem reflection walk: considered=%d, skipped(abstract)=%d, skipped(deprecated)=%d, registered=%d"),
        ConsideredCount, SkippedAbstract, SkippedDeprecated, TypeRegistry.Num());
}

void UMegalithUIRegistrySubsystem::ClassifyWidgetClass(
    UClass* WidgetClass,
...
void UMegalithUIRegistrySubsystem::OnReloadComplete(EReloadCompleteReason /*Reason*/)
{
    RefreshAfterReload();
}
...
void UMegalithUIRegistrySubsystem::RegisterCuratedMappings()
{
    // ----- Panels -----
