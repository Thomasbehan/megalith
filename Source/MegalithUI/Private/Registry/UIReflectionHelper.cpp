...
namespace
{
    ...
    // Routes hex-string parses through MegalithUI::ParseColor (degamma path).
    bool ParseLinearColor(const TSharedPtr<FJsonValue>& Value, FLinearColor& Out)
    {
        ...
        {
            FLinearColor Parsed;
            if (MegalithUI::TryParseColor(Value->AsString(), Parsed))
            {
                Out = Parsed;
                return true;
            }
    ...
    bool bRawMode)
{
    ...
    if (!bRawMode)
    {
        ...
        const FName WidgetToken = MegalithUI::MakeTokenFromClassName(Root->GetClass());
