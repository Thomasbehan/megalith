...
namespace MegalithUI::AnimationEventInternal
{
...
FMegalithActionResult MegalithUI::FAnimationEventActions::HandleAddAnimationEventTrack(
    const TSharedPtr<FJsonObject>& Params)
{
    using namespace MegalithUI::AnimationEventInternal;
    ...
    {
        return FMegalithActionResult::Error(TEXT("events must be a non-empty array"), -32602);
    }
    ...
    if (!Scene)
    {
        return FMegalithActionResult::Error(TEXT("Animation has no MovieScene"), -32603);
    }
    ...
    if (!EventTrack)
    {
        ...
        if (!EventTrack)
        {
            return FMegalithActionResult::Error(TEXT("Failed to create UMovieSceneEventTrack"), -32603);
        }
    }
    ...
    if (!TriggerSection)
    {
        return FMegalithActionResult::Error(TEXT("Failed to create UMovieSceneEventTriggerSection"), -32603);
    }
...
#else
    return FMegalithActionResult::Error(TEXT("Animation event authoring requires editor (WITH_EDITORONLY_DATA)"), -32603);
#endif
}


FMegalithActionResult MegalithUI::FAnimationEventActions::HandleBindAnimationToEvent(
    const TSharedPtr<FJsonObject>& Params)
{
    using namespace MegalithUI::AnimationEventInternal;
    ...
    {
        return FMegalithActionResult::Error(TEXT("Missing or empty required param: widget_event"), -32602);
    }
    ...
    if (!GenClass)
    {
        return FMegalithActionResult::Error(TEXT("Widget Blueprint has no GeneratedClass after compilation"), -32603);
    }

    UBlueprintGeneratedClass* BPGC = Cast<UBlueprintGeneratedClass>(GenClass);
    if (!BPGC)
    {
        return FMegalithActionResult::Error(TEXT("GeneratedClass is not a UBlueprintGeneratedClass"), -32603);
    }
    ...
    return FMegalithActionResult::Success(Result);
#else
    return FMegalithActionResult::Error(TEXT("Animation delegate binding requires editor (WITH_EDITORONLY_DATA)"), -32603);
#endif
}


void MegalithUI::FAnimationEventActions::Register(FMegalithToolRegistry& Registry)
{
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&MegalithUI::FAnimationEventActions::HandleAddAnimationEventTrack));
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&MegalithUI::FAnimationEventActions::HandleBindAnimationToEvent));
}
