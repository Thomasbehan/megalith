...
/**
 * MegalithUI -- UMG widget animation authoring (multi-track, weighted-tangent, spring-bake).
 *
 ...
 *
 * Phase I exposure: the helper namespace `MegalithUI::AnimationInternal` (defined in the
 * .cpp) is mirrored here as a thin public-to-MegalithUI surface so the new
...
namespace MegalithUI
{
    struct FAnimationCoreActions
    {
        ...
        static FMegalithActionResult HandleCreateAnimationV2(const TSharedPtr<FJsonObject>& Params);
        static FMegalithActionResult HandleAddBezierEasedSegment(const TSharedPtr<FJsonObject>& Params);
        static FMegalithActionResult HandleBakeSpringAnimation(const TSharedPtr<FJsonObject>& Params);
    };
     ...
     * for the arrival side. See `Plugins/Megalith/Source/MegalithUI/Private/Actions/Hoisted/AnimationCoreActions.cpp`
     ...
     */
    MEGALITHUI_API void ComputeBezierWeightedTangents(
     ...
     */
    MEGALITHUI_API UWidgetAnimation* FindOrCreateWidgetAnimation(
     ...
     */
    MEGALITHUI_API FGuid EnsureWidgetPossessableBinding(
     ...
     */
    MEGALITHUI_API FMovieSceneFloatChannel* EnsureFloatTrackSectionChannel(
        UMovieScene* Scene,
    ...
    MEGALITHUI_API void InsertFloatChannelKey(
