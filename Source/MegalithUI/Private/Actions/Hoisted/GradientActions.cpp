...
// MegalithUI shared color parser (no-degamma path; matches MD_UI material expectations).
#include "MegalithUICommon.h"

namespace MegalithUI::GradientInternal
{
...
FMegalithActionResult MegalithUI::FGradientActions::HandleCreateGradientMidFromSpec(const TSharedPtr<FJsonObject>& Params)
{
    using namespace MegalithUI::GradientInternal;
    ...
    {
        return FMegalithActionResult::Error(TEXT("Missing or empty required param: parent_material"), -32602);
    }
    FString Destination;
    if (!Params->TryGetStringField(TEXT("destination"), Destination) || Destination.IsEmpty())
    {
        return FMegalithActionResult::Error(TEXT("Missing or empty required param: destination"), -32602);
    }
    ...
    {
        return FMegalithActionResult::Error(TEXT("Missing or invalid required param: spec (object)"), -32602);
    }
    ...
    {
        return FMegalithActionResult::Error(TEXT("spec.stops must be a non-empty array"), -32602);
    }
    ...
    for (int32 i = 0; i < StopsToApply; ++i)
    {
        ...
        FLinearColor Parsed;
        if (!MegalithUI::TryParseColor(ColorStr, Parsed))
        {
            return FMegalithActionResult::Error(
    ...
    if (!MIC)
    {
        return FMegalithActionResult::Error(TEXT("Failed to create UMaterialInstanceConstant"), -32603);
    }
...
void MegalithUI::FGradientActions::Register(FMegalithToolRegistry& Registry)
{
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&MegalithUI::FGradientActions::HandleCreateGradientMidFromSpec));
}
