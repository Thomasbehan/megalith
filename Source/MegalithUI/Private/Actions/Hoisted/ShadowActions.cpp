...
namespace MegalithUI::ShadowInternal
{
        ...
        FString& OutError)
    {
        ...
        if (!MegalithUI::TryParseColor(ColorStr, OutSpec.Color))
        {
...
FMegalithActionResult MegalithUI::FShadowActions::HandleApplyBoxShadow(const TSharedPtr<FJsonObject>& Params)
{
    using namespace MegalithUI::ShadowInternal;
    ...
    if (bHasSingle)
    {
        ...
        {
            return FMegalithActionResult::Error(Err, -32602);
        }
        Specs.Add(Spec);
    }
    else
    ...
    for (int32 LayerIndex = Specs.Num() - 1; LayerIndex >= 0; --LayerIndex)
    {
        ...
        MegalithUI::RegisterCreatedWidget(WBP, ShadowImg);
...
void MegalithUI::FShadowActions::Register(FMegalithToolRegistry& Registry)
{
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&MegalithUI::FShadowActions::HandleApplyBoxShadow));
}
