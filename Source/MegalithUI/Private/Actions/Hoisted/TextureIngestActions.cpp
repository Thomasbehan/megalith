...
namespace MegalithUI::TextureIngestInternal
{
...
FMegalithActionResult MegalithUI::FTextureIngestActions::HandleImportTextureFromBytes(const TSharedPtr<FJsonObject>& Params)
{
    using namespace MegalithUI::TextureIngestInternal;
    ...
    {
        return FMegalithActionResult::Error(TEXT("Missing or empty required param: bytes_b64"), -32602);
    }

    FString FormatHint;
    if (!Params->TryGetStringField(TEXT("format_hint"), FormatHint) || FormatHint.IsEmpty())
    {
        return FMegalithActionResult::Error(TEXT("Missing or empty required param: format_hint"), -32602);
    }
    ...
    {
        return FMegalithActionResult::Error(TEXT("Base64 decode of bytes_b64 failed or produced empty buffer"), -32602);
    }
    ...
    if (!Texture)
    {
        return FMegalithActionResult::Error(TEXT("Failed to create UTexture2D object"), -32603);
    }
...
void MegalithUI::FTextureIngestActions::Register(FMegalithToolRegistry& Registry)
{
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&MegalithUI::FTextureIngestActions::HandleImportTextureFromBytes));
}
