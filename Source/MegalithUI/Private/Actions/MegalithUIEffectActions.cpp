...
// MegalithUI shared color parser + log category + EffectSurface probe API
// (R3b — `MegalithUI::GetEffectSurfaceClass()` / `IsEffectSurfaceAvailable()`).
#include "MegalithUICommon.h"

// MegalithUIInternal — `MakeOptionalDepUnavailableError` (§5.5 contract helper).
...
// through:
//
//   1. The probe API in MegalithUICommon.h
//      (`MegalithUI::IsEffectSurfaceAvailable()` /
//       `MegalithUI::GetEffectSurfaceClass()`).
...
// `MegalithUIInternal::MakeOptionalDepUnavailableError` helper:
//
...
namespace MegalithUI::EffectActionsInternal
{
    // -------------------------------------------------------------------------
        ...
        FMegalithActionResult& OutError)
    {
        ...
        UClass* EffectSurfaceClass = MegalithUI::GetEffectSurfaceClass();
        if (!EffectSurfaceClass)
        {
            OutError = MegalithUIInternal::MakeOptionalDepUnavailableError(
                TEXT("EffectSurface"),
        ...
        if (!Params.IsValid())
        {
            OutError = FMegalithActionResult::Error(TEXT("Missing params object"), -32602);
            return nullptr;
        }
        ...
        {
            OutError = FMegalithActionResult::Error(TEXT("Missing or empty required param: asset_path"), -32602);
            return nullptr;
        }
        if (!Params->TryGetStringField(TEXT("widget_name"), WidgetName) || WidgetName.IsEmpty())
        {
            OutError = FMegalithActionResult::Error(TEXT("Missing or empty required param: widget_name"), -32602);
            return nullptr;
        }

        UWidgetBlueprint* WBP = MegalithUI::LoadWidgetBlueprint(AssetPath, OutError);
        if (!WBP)
        {
        ...
        TArray<FString>& OutFailures)
    {
        ...
        const FName Token = MegalithUI::MakeTokenFromClassName(Surface->GetClass());
        ...
        FLinearColor& OutColor)
    {
        ...
        return MegalithUI::TryParseColor(S, OutColor);
    }
    ...
    static FString MakeShadowArrayText(const TArray<TSharedPtr<FJsonValue>>& Layers)
    {
        ...
        for (int32 i = 0; i < Count; ++i)
        {
            ...
            {
                MegalithUI::TryParseColor(ColorStr, Color);
            }
    ...
    static FMegalithActionResult MakeSuccess(
...
using namespace MegalithUI::EffectActionsInternal;

// ----- 1. set_effect_surface_corners ------------------------------------------
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetCorners(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetFill(const TSharedPtr<FJsonObject>& Params)
{
    UWidgetBlueprint* WBP = nullptr;
    FMegalithActionResult Err;
    UWidget* Surface = ResolveEffectSurface(Params, WBP, Err);
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetBorder(const TSharedPtr<FJsonObject>& Params)
{
    UWidgetBlueprint* WBP = nullptr;
    FMegalithActionResult Err;
    UWidget* Surface = ResolveEffectSurface(Params, WBP, Err);
...
namespace MegalithUI::EffectActionsInternal
{
    static FMegalithActionResult HandleShadowStack(
        const TSharedPtr<FJsonObject>& Params,
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetDropShadow(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetInnerShadow(const TSharedPtr<FJsonObject>& Params)
{
    return HandleShadowStack(Params,
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetGlow(const TSharedPtr<FJsonObject>& Params)
{
    UWidgetBlueprint* WBP = nullptr;
    FMegalithActionResult Err;
    UWidget* Surface = ResolveEffectSurface(Params, WBP, Err);
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetFilter(const TSharedPtr<FJsonObject>& Params)
{
    UWidgetBlueprint* WBP = nullptr;
    FMegalithActionResult Err;
    UWidget* Surface = ResolveEffectSurface(Params, WBP, Err);
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetBackdropBlur(const TSharedPtr<FJsonObject>& Params)
{
    UWidgetBlueprint* WBP = nullptr;
    FMegalithActionResult Err;
    UWidget* Surface = ResolveEffectSurface(Params, WBP, Err);
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleSetInsetHighlight(const TSharedPtr<FJsonObject>& Params)
{
    UWidgetBlueprint* WBP = nullptr;
    FMegalithActionResult Err;
    UWidget* Surface = ResolveEffectSurface(Params, WBP, Err);
...
// `MegalithUI::EffectSurfacePresets::MakeConfigForPreset` (header in
...
FMegalithActionResult MegalithUI::FEffectSurfaceActions::HandleApplyPreset(const TSharedPtr<FJsonObject>& Params)
{
    UWidgetBlueprint* WBP = nullptr;
    FMegalithActionResult Err;
    UWidget* Surface = ResolveEffectSurface(Params, WBP, Err);
...
void MegalithUI::FEffectSurfaceActions::Register(FMegalithToolRegistry& Registry)
{
    Registry.RegisterAction(
        ...
        TEXT("Set EffectSurface corner radii (and optional smoothness). Composes against allowlist-gated "
             ...
             "smoothness?, compile?."),
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetCorners));
    ...
    Registry.RegisterAction(
        ...
        TEXT("Set EffectSurface fill: solid colour, linear gradient, or radial gradient. ORs the matching "
             ...
             "{x,y}), compile?."),
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetFill));
    ...
    Registry.RegisterAction(
        ...
        TEXT("Set EffectSurface border (width + colour + offset + optional glow halo). ORs Border into "
             ...
             "glow_color?, compile?."),
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetBorder));
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetDropShadow));
    ...
    Registry.RegisterAction(
        ...
        TEXT("Set EffectSurface inner-shadow stack (CSS-style layered list, capped at 4). ORs InnerShadow "
             "into Effect.FeatureFlags only when the layer list is non-empty. Params: asset_path, "
             "widget_name, layers=[{offset:[x,y], blur, spread, color}, ...], compile?."),
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetInnerShadow));
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetGlow));
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetFilter));
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetBackdropBlur));
    ...
    Registry.RegisterAction(
        ...
        TEXT("Set EffectSurface inset-highlight bag (Material-Design elevation overlay). ORs "
             ...
             "Left=8), compile?."),
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleSetInsetHighlight));
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&FEffectSurfaceActions::HandleApplyPreset));
}
