...
namespace MegalithUI::SpecActionsInternal
{
    // ------------------------------------------------------------------
    ...
    static FLinearColor ParseColor(const TSharedPtr<FJsonObject>& Obj, const TCHAR* Field)
    {
        ...
        {
            FLinearColor C;
            if (MegalithUI::TryParseColor(S, C))
            {
                return C;
            }
        }
    ...
    static void ParseEffect(const TSharedPtr<FJsonObject>& Obj, FUISpecEffect& OutEffect)
    {
        ...
        auto ParseShadowArray = [&Obj](const TCHAR* Field, TArray<FUISpecEffectShadow>& Out, bool bDefaultInset)
        {
            ...
            for (const TSharedPtr<FJsonValue>& V : *ShArr)
            {
                ...
                {
                    FLinearColor C;
                    if (MegalithUI::TryParseColor(HexColor, C))
                    {
                        S.Color = C;
                    }
                }
        ...
        FUISpecValidationResult& OutValidation)
    {
        ...
        if (SpecObj->TryGetObjectField(TEXT("styles"), Sub) && Sub)
        {
            ...
            {
                const TPair<FString, TSharedPtr<FJsonValue>> Pair(MegalithKeyToString(PairIt.Key), PairIt.Value);
    ...
    static FMegalithActionResult HandleBuildUIFromSpec(const TSharedPtr<FJsonObject>& Params)
    {
        if (!Params.IsValid())
        {
        ...
        {
            return FMegalithActionResult::Error(TEXT("Missing or invalid required param: spec (must be a JSON object)"), -32602);
        }
        ...
        if (!ParseDocument(*SpecObjPtr, Document, ParseValidation))
        {
            ...
            return FMegalithActionResult::Success(PackResponse(R, /*bDryRun=*/false));
        }
        ...
        return FMegalithActionResult::Success(PackResponse(R, Inputs.bDryRun));
    }
     ...
     */
    static FMegalithActionResult HandleDumpUISpecSchema(const TSharedPtr<FJsonObject>& /*Params*/)
    {
        TSharedPtr<FJsonObject> Out = MakeShared<FJsonObject>();
    ...
    static FMegalithActionResult HandleDumpUISpec(const TSharedPtr<FJsonObject>& Params)
    {
        if (!Params.IsValid())
        {
            return FMegalithActionResult::Error(TEXT("Missing params object"), -32602);
        ...
        return FMegalithActionResult::Success(PackDumpResponse(R));
    }
    ...
    static FMegalithActionResult HandleBuildMenuFromSpec(const TSharedPtr<FJsonObject>& Params)
    {
        if (!Params.IsValid())
        {
            return FMegalithActionResult::Error(TEXT("Missing params object"), -32602);
...
// Action register entry-point — called once from FMegalithUIModule::StartupModule.
// Declaration lives in Actions/MegalithUISpecActions.h.
void MegalithUI::FSpecActions::Register(FMegalithToolRegistry& Registry)
{
    using namespace MegalithUI::SpecActionsInternal;
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleBuildUIFromSpec),
        FParamSchemaBuilder()
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleDumpUISpecSchema),
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleDumpUISpec),
        FParamSchemaBuilder()
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleBuildMenuFromSpec),
        FParamSchemaBuilder()
