...
// MegalithUIBulkFillAdapter — Phase 4 adapter for the FMegalithBulkFillRegistry.
...
// FMegalithUIModule::StartupModule. The adapter BODY splits inside the function:
...
#define LOCTEXT_NAMESPACE "MegalithUIBulkFillAdapter"
...
namespace MegalithUIBulkFillInternal
{
	// Build a single-error FDryRunReport for use when target resolution / validation
	// fails. Mirrors MegalithGASBulkFillAdapter.cpp::MakeResolveFailureReport
		...
		FDryRunReport& OutReport)
	{
		...
		{
			const FString FieldName = MegalithKeyToString(FieldKV.Key);
	...
	static FDryRunReport HandleDataTableRows(const FBulkFillSpec& Spec)
	{
		...
		if (!Spec.bDryRun)
		{
			...
			Transaction = MakeShared<FScopedTransaction>(
				LOCTEXT("UIBulkFill_DataTableRows", "Megalith UI Bulk Fill — DataTable Rows"));
			DT->Modify();
		}
		...
		for (const auto& RowKV : RowsObj->Values)
		{
			...
			WriteDataTableRow(DT, RowStruct, MegalithKeyToString(RowKV.Key), RowObj, Spec, Report);
		}
	...
	static FDryRunReport HandleInputActionDataTable(const FBulkFillSpec& Spec)
	{
		...
		if (!Spec.bDryRun)
		{
			...
			Transaction = MakeShared<FScopedTransaction>(
				LOCTEXT("UIBulkFill_InputActionDT",
					"Megalith UI Bulk Fill — Input Action DT (single transaction)"));
			DT->Modify();
		}
		...
		for (const auto& RowKV : RowsObj->Values)
		{
			...
			WriteDataTableRow(DT, InputActionStruct, MegalithKeyToString(RowKV.Key), RowObj, Spec, Report);
		}
		...
		if (!Spec.bDryRun)
		{
			...
			Transaction = MakeShared<FScopedTransaction>(
				LOCTEXT("UIBulkFill_WidgetProperties",
					"Megalith UI Bulk Fill — Widget Properties"));
			Widget->Modify();
		}
...
// ---------------------------------------------------------------------------
FDryRunReport FMegalithUIBulkFillAdapter::UIBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithUIBulkFillInternal;
...
//
FSchemaDescriptor FMegalithUIBulkFillAdapter::UIDescribe(const FString& TargetAsset)
{
	using namespace MegalithUIBulkFillInternal;
...
// ---------------------------------------------------------------------------
// Registration entry-points (called from FMegalithUIModule::StartupModule
// / ShutdownModule).
//
...
// This guarantees `megalith_discover("ui")` returns the same row in dev and
// release builds.
//
...
// ---------------------------------------------------------------------------
void FMegalithUIBulkFillAdapter::Register()
{
	...
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("ui"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithUIBulkFillAdapter::UIBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithUIBulkFillAdapter::UIDescribe));
}

void FMegalithUIBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("ui"));
}
