...
// MegalithUIStyleService.cpp — Phase G
...
#include "CommonUI/MegalithCommonUIHelpers.h"
...
namespace
{
    ...
    TUniquePtr<FMegalithUIStyleService> GStyleServiceInstance;
    ...
    void AppendCanonicalObject(FString& Out, const TSharedPtr<FJsonObject>& Object)
    {
        ...
        {
            Keys.Add(MegalithKeyToString(Pair.Key));
        }
        Keys.Sort();
        ...
        for (int32 i = 0; i < Keys.Num(); ++i)
        {
            ...
            const TSharedPtr<FJsonValue>& Field = Object->Values.FindChecked(MegalithMakeJsonKey(Keys[i]));
...
FMegalithUIStyleService& FMegalithUIStyleService::Get()
{
    if (!GStyleServiceInstance.IsValid())
    {
        GStyleServiceInstance = TUniquePtr<FMegalithUIStyleService>(new FMegalithUIStyleService());
    }
    return *GStyleServiceInstance;
}

void FMegalithUIStyleService::Shutdown()
{
    GStyleServiceInstance.Reset();
}

void FMegalithUIStyleService::Reset()
{
...
uint32 FMegalithUIStyleService::ComputeContentHash(
    UClass* StyleClass,
...
FName FMegalithUIStyleService::StyleTypeToken(UClass* StyleClass)
{
...
FString FMegalithUIStyleService::DeriveNameFromHash(UClass* StyleClass, uint32 Hash)
{
...
void FMegalithUIStyleService::InsertEntry(FUIStyleEntry&& Entry)
{
    ...
    const UMegalithUISettings* Settings = UMegalithUISettings::Get();
...
void FMegalithUIStyleService::EvictOldest()
{
...
UClass* FMegalithUIStyleService::TryFindInCanonicalLibrary(
    UClass* StyleClass,
    const FString& AssetName) const
{
    ...
    const FString Folder = UMegalithUISettings::NormalizeFolderPath(Settings->CanonicalLibraryPath);
    if (Folder.IsEmpty())
    {
        return nullptr;
    }
...
UClass* FMegalithUIStyleService::CreateNewStyleAsset(
    UClass* StyleClass,
    ...
    FString& OutError)
{
    ...
    const FString Folder = UMegalithUISettings::NormalizeFolderPath(PackagePath);
    if (Folder.IsEmpty())
    {
    ...
    UBlueprint* BP = FKismetEditorUtilities::CreateBlueprint(
        ...
        BPTYPE_Normal, FName(TEXT("MegalithUI")));
    ...
    if (CDO && Properties.IsValid())
    {
        for (const auto& Pair : Properties->Values)
        {
            ...
            if (P)
            {
                MegalithCommonUI::SetPropertyFromJson(CDO, P, Pair.Value);
            }
        }
    }
...
FUIStyleResolution FMegalithUIStyleService::ResolveOrCreate(
    UClass* StyleClass,
    const FString& AssetName,
    const FString& PackagePath,
    const TSharedPtr<FJsonObject>& Properties)
{
    ...
    if (UClass* LibClass = TryFindInCanonicalLibrary(StyleClass, EffectiveName))
    {
        ...
        Entry.PackagePath = UMegalithUISettings::NormalizeFolderPath(
            UMegalithUISettings::Get()->CanonicalLibraryPath);
        ...
        Result.PackagePath = UMegalithUISettings::NormalizeFolderPath(
            UMegalithUISettings::Get()->CanonicalLibraryPath);
    ...
    const FString TargetFolder = PackagePath.IsEmpty()
        ? (Settings ? UMegalithUISettings::NormalizeFolderPath(Settings->GeneratedStylesPath) : FString(TEXT("/Game/UI/Styles")))
        : UMegalithUISettings::NormalizeFolderPath(PackagePath);
...
FUIStyleCacheStats FMegalithUIStyleService::GetStats() const
{
    FScopeLock Lock(&CacheLock);
