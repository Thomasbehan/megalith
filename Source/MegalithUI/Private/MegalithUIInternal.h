...
#include "MegalithJsonUtils.h"        // R3b: ErrOptionalDepUnavailable for the §5.5 helper
...
// `MegalithUI::` namespace in `MegalithUICommon.h`. The inline wrappers are
// retained so existing call sites compile unchanged during the migration.
// New code should call `MegalithUI::Foo` directly.
...
namespace MegalithUIInternal
{
    ...
    inline FMegalithActionResult MakeOptionalDepUnavailableError(
        ...
        const FString& Alternative)
    {
        ...
        FMegalithActionResult Result = FMegalithActionResult::Error(
            Message, FMegalithJsonUtils::ErrOptionalDepUnavailable);

        // Populate the structured payload alongside the error message. The
        // FMegalithActionResult::Error static initialises Result to nullptr;
    ...
    // Phase K — convert an FUISpecError to an FMegalithActionResult::Error. The
    ...
    inline FMegalithActionResult MakeErrorFromSpecError(const FUISpecError& Err, int32 Code = -32602)
    {
        return FMegalithActionResult::Error(Err.ToLLMReport(), Code);
    }
    ...
    // MegalithUI::GetAnchorPreset (MegalithUICommon.cpp:193-214). Kept here in
    ...
    // Load a widget blueprint by asset path. Forwards to MegalithUI::LoadWidgetBlueprint
    // (Phase A hoist — see MegalithUICommon.h).
    inline UWidgetBlueprint* LoadWidgetBlueprint(const FString& AssetPath, FMegalithActionResult& OutError)
    {
        return MegalithUI::LoadWidgetBlueprint(AssetPath, OutError);
    }

    // Map of widget class short names to UClass pointers. Forwards to
    // MegalithUI::WidgetClassFromName (Phase A hoist — see MegalithUICommon.h).
    inline UClass* WidgetClassFromName(const FString& ClassName)
    {
        return MegalithUI::WidgetClassFromName(ClassName);
    }
    ...
    // Variable-name registration helpers. Forward to MegalithUI:: (Phase A hoist).
    inline void RegisterVariableName(UWidgetBlueprint* WBP, const FName& VariableName)
    {
        MegalithUI::RegisterVariableName(WBP, VariableName);
    }

    inline void RegisterCreatedWidget(UWidgetBlueprint* WBP, UWidget* Widget)
    {
        MegalithUI::RegisterCreatedWidget(WBP, Widget);
    }
    ...
    inline UWidgetBlueprint* CreateNewWidgetBlueprint(const FString& SavePath, FMegalithActionResult& OutError)
    {
        FString PackagePath, AssetName;
        SavePath.Split(TEXT("/"), &PackagePath, &AssetName, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
        if (AssetName.IsEmpty())
        {
            OutError = FMegalithActionResult::Error(TEXT("Invalid save_path — must contain at least one / separator"));
            return nullptr;
        }
        ...
        if (!WBP)
        {
            OutError = FMegalithActionResult::Error(TEXT("UWidgetBlueprintFactory::FactoryCreateNew returned null"));
            return nullptr;
        }
    ...
    // Alignment parsers. Forward to MegalithUI:: (Phase A hoist).
    inline EHorizontalAlignment ParseHAlign(const FString& S)
    {
        return MegalithUI::ParseHAlign(S);
    }

    inline EVerticalAlignment ParseVAlign(const FString& S)
    {
        return MegalithUI::ParseVAlign(S);
    }
}
