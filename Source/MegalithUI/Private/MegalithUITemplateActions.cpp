// MegalithUITemplateActions.cpp
#include "MegalithUITemplateActions.h"
#include "MegalithUIInternal.h"
#include "MegalithParamSchema.h"

void FMegalithUITemplateActions::RegisterActions(FMegalithToolRegistry& Registry)
{
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateHudElement),
        FParamSchemaBuilder()
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateMenu),
        FParamSchemaBuilder()
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateSettingsPanel),
        FParamSchemaBuilder()
            .RequiredAssetPath(TEXT("save_path"), TEXT("Asset path for the new Widget Blueprint"))
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateDialog),
        FParamSchemaBuilder()
            .RequiredAssetPath(TEXT("save_path"), TEXT("Asset path for the new Widget Blueprint"))
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateNotificationToast),
        FParamSchemaBuilder()
            .RequiredAssetPath(TEXT("save_path"), TEXT("Asset path for the new Widget Blueprint"))
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateLoadingScreen),
        FParamSchemaBuilder()
            .RequiredAssetPath(TEXT("save_path"), TEXT("Asset path for the new Widget Blueprint"))
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateInventoryGrid),
        FParamSchemaBuilder()
            .RequiredAssetPath(TEXT("save_path"), TEXT("Asset path for the new Widget Blueprint"))
    ...
    Registry.RegisterAction(
        ...
        FMegalithActionHandler::CreateStatic(&HandleCreateSaveSlotList),
        FParamSchemaBuilder()
            .RequiredAssetPath(TEXT("save_path"), TEXT("Asset path for the new Widget Blueprint"))
...
// --- create_hud_element ---
FMegalithActionResult FMegalithUITemplateActions::HandleCreateHudElement(const TSharedPtr<FJsonObject>& Params)
{
    FString AssetPath = Params->GetStringField(TEXT("asset_path"));
    FString ElementType = Params->GetStringField(TEXT("element_type"));
    if (ElementType.IsEmpty())
    {
        return FMegalithActionResult::Error(TEXT("Missing required param: element_type"));
    }

    FString Prefix = MegalithUIInternal::GetOptionalString(Params, TEXT("widget_name_prefix"));
    ...
    {
        return FMegalithActionResult::Error(TEXT("Widget Blueprint has no widget tree or root widget"));
    }

    UPanelWidget* Root = Cast<UPanelWidget>(WBP->WidgetTree->RootWidget);
    if (!Root)
    {
        return FMegalithActionResult::Error(TEXT("Root widget is not a panel — cannot add children"));
    }
    ...
    if (ElementType == TEXT("crosshair"))
    {
        ...
        MegalithUIInternal::ConfigureCanvasSlot(Container->Slot, TEXT("center"), FVector2D::ZeroVector, FVector2D(64, 64), false, FVector2D(0.5f, 0.5f));
    ...
    else if (ElementType == TEXT("health_bar"))
    {
        ...
        Root->AddChild(VBox);
        MegalithUIInternal::ConfigureCanvasSlot(VBox->Slot, TEXT("bottom_left"), FVector2D(20, -80), FVector2D(300, 60), false, FVector2D(0.f, 1.f));
    ...
    else if (ElementType == TEXT("ammo_counter"))
    {
        ...
        Root->AddChild(HBox);
        MegalithUIInternal::ConfigureCanvasSlot(HBox->Slot, TEXT("bottom_right"), FVector2D(-20, -40), FVector2D(200, 40), false, FVector2D(1.f, 1.f));
    ...
    else if (ElementType == TEXT("stamina_bar"))
    {
        UVerticalBox* VBox = WBP->WidgetTree->ConstructWidget<UVerticalBox>(UVerticalBox::StaticClass(), FName(*(Prefix + TEXT("_Container"))));
        Root->AddChild(VBox);
        MegalithUIInternal::ConfigureCanvasSlot(VBox->Slot, TEXT("bottom_left"), FVector2D(20, -150), FVector2D(300, 40), false, FVector2D(0.f, 1.f));
    ...
    else if (ElementType == TEXT("interaction_prompt"))
    {
        UVerticalBox* VBox = WBP->WidgetTree->ConstructWidget<UVerticalBox>(UVerticalBox::StaticClass(), FName(*(Prefix + TEXT("_Container"))));
        Root->AddChild(VBox);
        MegalithUIInternal::ConfigureCanvasSlot(VBox->Slot, TEXT("bottom_center"), FVector2D(0, -120), FVector2D(400, 60), false, FVector2D(0.5f, 1.f));
    ...
    else if (ElementType == TEXT("damage_indicator"))
    {
        ...
        UOverlay* Container = WBP->WidgetTree->ConstructWidget<UOverlay>(UOverlay::StaticClass(), FName(*(Prefix + TEXT("_Container"))));
        Root->AddChild(Container);
        MegalithUIInternal::ConfigureCanvasSlot(Container->Slot, TEXT("stretch_fill"));
    ...
    else if (ElementType == TEXT("compass"))
    {
        ...
        Root->AddChild(SBox);
        MegalithUIInternal::ConfigureCanvasSlot(SBox->Slot, TEXT("top_center"), FVector2D(0, 20), FVector2D(400, 40), false, FVector2D(0.5f, 0.f));
    ...
    else if (ElementType == TEXT("subtitles"))
    {
        ...
        Root->AddChild(Container);
        MegalithUIInternal::ConfigureCanvasSlot(Container->Slot, TEXT("stretch_horizontal"), FVector2D(100, -80), FVector2D(-200, 60), false, FVector2D(0.f, 1.f));
    ...
    else if (ElementType == TEXT("flashlight_battery"))
    {
        UHorizontalBox* HBox = WBP->WidgetTree->ConstructWidget<UHorizontalBox>(UHorizontalBox::StaticClass(), FName(*(Prefix + TEXT("_Container"))));
        Root->AddChild(HBox);
        MegalithUIInternal::ConfigureCanvasSlot(HBox->Slot, TEXT("bottom_right"), FVector2D(-20, -100), FVector2D(180, 30), false, FVector2D(1.f, 1.f));
...
// --- create_menu ---
FMegalithActionResult FMegalithUITemplateActions::HandleCreateMenu(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (MenuType.IsEmpty())
    {
        return FMegalithActionResult::Error(TEXT("Missing required param: menu_type"));
    }
    ...
    FMegalithActionResult Err;
    UWidgetBlueprint* WBP = MegalithUIInternal::CreateNewWidgetBlueprint(SavePath, Err);
    if (!WBP) return Err;
    ...
    Root->AddChild(BG);
    MegalithUIInternal::ConfigureCanvasSlot(BG->Slot, TEXT("stretch_fill"));
    ...
    MegalithUIInternal::ConfigureCanvasSlot(Title->Slot, TEXT("top_center"), FVector2D(0, 80), FVector2D(600, 60), true, FVector2D(0.5f, 0.f));
    ...
    if (MenuType == TEXT("credits"))
    {
        ...
        MegalithUIInternal::ConfigureCanvasSlot(CreditsScroll->Slot, TEXT("center"), FVector2D(0, 40), FVector2D(500, 400), false, FVector2D(0.5f, 0.5f));
    ...
    if (MenuType == TEXT("credits"))
    {
        MegalithUIInternal::ConfigureCanvasSlot(ButtonBox->Slot, TEXT("bottom_center"), FVector2D(0, -40), FVector2D(300, 60), true, FVector2D(0.5f, 1.f));
    }
    else
    {
        MegalithUIInternal::ConfigureCanvasSlot(ButtonBox->Slot, TEXT("center"), FVector2D(0, 40), FVector2D(300, 0), true, FVector2D(0.5f, 0.5f));
    }
    ...
    // Compile and save
    MegalithUIInternal::SaveAndCompileWidgetBlueprint(WBP, SavePath);
...
FMegalithActionResult FMegalithUITemplateActions::HandleCreateSettingsPanel(const TSharedPtr<FJsonObject>& Params)
{
    FString SavePath = Params->GetStringField(TEXT("save_path"));
    if (SavePath.IsEmpty())
    {
    ...
    MegalithUIInternal::ConfigureCanvasSlot(MainVBox->Slot, TEXT("stretch_fill"));
...
// --- create_dialog ---
FMegalithActionResult FMegalithUITemplateActions::HandleCreateDialog(const TSharedPtr<FJsonObject>& Params)
{
    FString SavePath = Params->GetStringField(TEXT("save_path"));
    if (SavePath.IsEmpty())
    {
    ...
    FString Title = MegalithUIInternal::GetOptionalString(Params, TEXT("title"));
    if (Title.IsEmpty()) Title = TEXT("Confirm");
    FString Body = MegalithUIInternal::GetOptionalString(Params, TEXT("body"));
    if (Body.IsEmpty()) Body = TEXT("Are you sure?");
    FString ConfirmText = MegalithUIInternal::GetOptionalString(Params, TEXT("confirm_text"));
    if (ConfirmText.IsEmpty()) ConfirmText = TEXT("Yes");
    FString CancelText = MegalithUIInternal::GetOptionalString(Params, TEXT("cancel_text"));
    ...
    Root->AddChild(DimBG);
    MegalithUIInternal::ConfigureCanvasSlot(DimBG->Slot, TEXT("stretch_fill"));
    ...
    MegalithUIInternal::ConfigureCanvasSlot(DialogBox->Slot, TEXT("center"), FVector2D::ZeroVector, FVector2D(450, 250), false, FVector2D(0.5f, 0.5f));
...
FMegalithActionResult FMegalithUITemplateActions::HandleCreateNotificationToast(const TSharedPtr<FJsonObject>& Params)
{
    FString SavePath = Params->GetStringField(TEXT("save_path"));
    if (SavePath.IsEmpty())
    {
    ...
    FString Position = MegalithUIInternal::GetOptionalString(Params, TEXT("position"));
    ...
    Root->AddChild(Toast);
    MegalithUIInternal::ConfigureCanvasSlot(Toast->Slot, AnchorPreset, Offset, FVector2D(350, 60), false, Align);
...
FMegalithActionResult FMegalithUITemplateActions::HandleCreateLoadingScreen(const TSharedPtr<FJsonObject>& Params)
{
    FString SavePath = Params->GetStringField(TEXT("save_path"));
    if (SavePath.IsEmpty())
    {
    ...
    const bool bShowProgress = MegalithUIInternal::GetOptionalBool(Params, TEXT("show_progress"), true);
    const bool bShowTips = MegalithUIInternal::GetOptionalBool(Params, TEXT("show_tips"), true);
    const bool bShowSpinner = MegalithUIInternal::GetOptionalBool(Params, TEXT("show_spinner"), true);
    ...
    if (bShowTips)
    {
        ...
        Root->AddChild(Tip);
        MegalithUIInternal::ConfigureCanvasSlot(Tip->Slot, TEXT("bottom_center"), FVector2D(0, bShowProgress ? -100 : -60), FVector2D(600, 40), true, FVector2D(0.5f, 1.f));
    ...
    if (bShowSpinner)
    {
        ...
        MegalithUIInternal::ConfigureCanvasSlot(Spinner->Slot, TEXT("bottom_right"), FVector2D(-60, -60), FVector2D(48, 48), false, FVector2D(1.f, 1.f));
        WidgetsCreated++;
    }
...
FMegalithActionResult FMegalithUITemplateActions::HandleCreateInventoryGrid(const TSharedPtr<FJsonObject>& Params)
{
    FString SavePath = Params->GetStringField(TEXT("save_path"));
    if (SavePath.IsEmpty())
    {
    ...
    MegalithUIInternal::ConfigureCanvasSlot(Scroll->Slot, TEXT("center"),
...
FMegalithActionResult FMegalithUITemplateActions::HandleCreateSaveSlotList(const TSharedPtr<FJsonObject>& Params)
{
    FString SavePath = Params->GetStringField(TEXT("save_path"));
    if (SavePath.IsEmpty())
    {
    ...
    Root->AddChild(MainVBox);
    MegalithUIInternal::ConfigureCanvasSlot(MainVBox->Slot, TEXT("center"), FVector2D::ZeroVector, FVector2D(500, 0), true, FVector2D(0.5f, 0.5f));
