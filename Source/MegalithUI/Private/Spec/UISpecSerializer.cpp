...
// Access flows through MegalithUI::GetEffectSurfaceClass() (UClass* probe) +
// the curated `Effect.*` JSON path mapping in MegalithUIRegistrySubsystem.
// See the optional provider decouple notes in the Megalith UI spec.
...
namespace MegalithUI::SpecSerializerInternal
{
    /**
    ...
    static FName ClassToToken(const UClass* WidgetClass)
    {
        ...
        return MegalithUI::MakeTokenFromClassName(WidgetClass);
    }
    ...
    static FName AnchorPresetFromAnchors(const FAnchors& Anchors)
    {
        ...
        {
            const FAnchors Candidate = MegalithUI::GetAnchorPreset(FString(PresetName));
    ...
    //     pulled UEffectSurface / FEffectSurfaceConfig symbols into MegalithUI,
    //     forcing the optional provider to be a hard compile-time dep.
    //   * R3 (this file): the cast becomes a UClass* probe via
    //     `MegalithUI::GetEffectSurfaceClass()`; scalar reads route through
    ...
    //   When `MegalithUI::GetEffectSurfaceClass()` returns nullptr (provider
    ...
    //
    // Single source of truth: the probe is `MegalithUI::IsEffectSurfaceAvailable()`
    // (R3b.4) and the class lookup is `MegalithUI::GetEffectSurfaceClass()`
    ...
    static void SerializeEffect(UWidget* Widget, FUISpecEffect& OutEffect, bool& bOutHasEffect)
    {
        ...
        const FName WidgetToken = MegalithUI::MakeTokenFromClassName(EffectSurfaceClass);
...
{
    using namespace MegalithUI::SpecSerializerInternal;
    ...
    Doc.Metadata.AuthoringTool = TEXT("MegalithUI.SpecSerializer");
