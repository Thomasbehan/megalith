...
#define LOCTEXT_NAMESPACE "MegalithUISpecBuilder"


namespace MegalithUI::SpecBuilderInternal
{
    // ------------------------------------------------------------------
     ...
     * + plan §H13.6 + UMegalithUISettings::MaxNestingDepth ctor default).
     */
    static int32 GetMaxNestingDepth()
    {
        if (const UMegalithUISettings* Settings = UMegalithUISettings::Get())
        {
        ...
        FUISpecValidationResult& OutResult)
    {
        ...
        if (Depth > MaxDepth)
        {
            ...
            E.Message  = FString::Printf(
                TEXT("Spec tree depth exceeds MaxNestingDepth (%d). Reduce nesting or raise the limit in Project Settings → Plugins → Megalith UI."),
                MaxDepth);
        ...
        if (!Node.Type.IsNone())
        {
            ...
            if (!Entry && Node.CustomClassPath.IsEmpty())
            {
                ...
                UClass* Legacy = MegalithUI::WidgetClassFromName(Node.Type.ToString());
                if (!Legacy)
                {
                    FUISpecError E;
                    E.Severity = EUISpecErrorSeverity::Error;
    ...
    static UClass* ResolveWidgetClass(const FUISpecNode& Node, const FUITypeRegistry& Registry)
    {
        ...
        if (!Node.Type.IsNone())
        {
            return MegalithUI::WidgetClassFromName(Node.Type.ToString());
        }
        return nullptr;
    }
        ...
        UPanelWidget* ParentPanel)
    {
        ...
        else
        {
            ...
            WidgetClass = MegalithUI::WidgetClassFromName(Node.Type.ToString());
        }
        ...
        case EUIContainerKind::Panel:
            Constructed = MegalithUI::PanelBuilder::BuildPanel(
                Context, Node, WidgetClass, ParentPanel);
            break;
        case EUIContainerKind::Content:
        case EUIContainerKind::Leaf:
        default:
            Constructed = MegalithUI::LeafBuilder::BuildLeafOrContent(
                Context, Node, WidgetClass, ParentPanel);
            break;
        }
        ...
        {
            MegalithUI::CommonUIBuilder::ApplyCommonUI(Context, Node, Constructed);
        }
        ...
        if (Node.bHasEffect)
        {
            MegalithUI::EffectSurfaceBuilder::ApplyEffect(Context, Node, Constructed);
        }
    ...
    static void RollbackCreatedAsset(UWidgetBlueprint* WBP, UPackage* Package)
    {
        if (WBP)
        {
            ...
            if (!bDeleted)
            {
                UE_LOG(LogMegalithUISpec, Warning,
...
{
    using namespace MegalithUI::SpecBuilderInternal;
    ...
    UMegalithUIRegistrySubsystem* RegistrySub = UMegalithUIRegistrySubsystem::Get();
    if (Doc.Root.IsValid())
    {
        TSet<FName> VisitedIds;
    ...
    if (bPreExisting && Inputs.bOverwrite && !Inputs.bDryRun)
    {
        if (WBP->WidgetTree)
        {
            ...
            // package — same recipe MegalithUITestFixtureUtils uses, which
    ...
    if (Doc.Root.IsValid() && WBP->WidgetTree)
    {
        ...
        if (RootWidget)
        {
            ...
            MegalithUI::RegisterCreatedWidget(WBP, RootWidget);
        }
        else
        {
