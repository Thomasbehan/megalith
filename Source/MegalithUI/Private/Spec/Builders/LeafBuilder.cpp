...
namespace MegalithUI::LeafBuilderInternal
{
    ...
    static void ApplyAlignmentAndPadding(const FUISpecSlot& S, SlotType* Slot)
    {
        ...
        if (!S.HAlign.IsNone())
        {
            Slot->SetHorizontalAlignment(MegalithUI::ParseHAlign(S.HAlign.ToString()));
        }
        if (!S.VAlign.IsNone())
        {
            Slot->SetVerticalAlignment(MegalithUI::ParseVAlign(S.VAlign.ToString()));
        }
        ...
        UPanelWidget* ParentPanel)
    {
        ...
        MegalithUI::RegisterCreatedWidget(Context.TargetWBP, Constructed);
        return Constructed;
    }
    ...
    static void ApplySlotFields(const FUISpecNode& Node, UWidget* Widget)
    {
        ...
        if (UCanvasPanelSlot* CSlot = Cast<UCanvasPanelSlot>(Slot))
        {
            ...
            {
                CSlot->SetAnchors(MegalithUI::GetAnchorPreset(S.AnchorPreset.ToString()));
            }
        ...
        else if (UUniformGridSlot* UGSlot = Cast<UUniformGridSlot>(Slot))
        {
            if (!S.HAlign.IsNone())
            {
                UGSlot->SetHorizontalAlignment(MegalithUI::ParseHAlign(S.HAlign.ToString()));
            }
            if (!S.VAlign.IsNone())
            {
                UGSlot->SetVerticalAlignment(MegalithUI::ParseVAlign(S.VAlign.ToString()));
            }
            if (!S.Position.IsNearlyZero())
            {
...
UWidget* MegalithUI::LeafBuilder::BuildLeafOrContent(
    FUIBuildContext& Context,
    const FUISpecNode& Node,
    UClass* WidgetClass,
    UPanelWidget* ParentPanel)
{
    using namespace MegalithUI::LeafBuilderInternal;
