...
namespace MegalithUI::AnimationBuilderInternal
{
     ...
     * `MegalithUI::InsertFloatChannelKey`. v1 honours scalar values only —
     ...
     */
    static MegalithUI::FAnimationCoreKey TranslateKeyframe(const FUISpecKeyframe& InKey)
    {
        MegalithUI::FAnimationCoreKey OutKey;
    ...
    static void ExpandFromToSugar(
        ...
        TArray<MegalithUI::FAnimationCoreKey>& OutKeys)
    {
        ...
        MegalithUI::FAnimationCoreKey K0;
        K0.Time = Animation.Delay;
        K0.Value = 0.f;
        K0.Interp = TEXT("cubic");

        MegalithUI::FAnimationCoreKey K1;
        ...
        {
            MegalithUI::ComputeBezierWeightedTangents(
...
{
#if WITH_EDITORONLY_DATA
    using namespace MegalithUI::AnimationBuilderInternal;
    ...
    UWidgetAnimation* WidgetAnim = MegalithUI::FindOrCreateWidgetAnimation(TargetWBP, AnimNameStr);
    ...
    TArray<MegalithUI::FAnimationCoreKey> Keys;
    ...
    double MinTime = 0.0;
    double MaxTime = 0.0;
    for (const MegalithUI::FAnimationCoreKey& Key : Keys)
    {
    ...
    const FGuid PossessableGuid = MegalithUI::EnsureWidgetPossessableBinding(
        WidgetAnim, Scene, WidgetNameStr, bIsRoot);

    const TRange<FFrameNumber> SectionRange(StartFrame, EndFrame);
    FMovieSceneFloatChannel* Channel = MegalithUI::EnsureFloatTrackSectionChannel(
    ...
    for (const MegalithUI::FAnimationCoreKey& Key : Keys)
    {
        MegalithUI::InsertFloatChannelKey(Channel, TickRes, Key);
