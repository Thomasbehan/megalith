...
/**
 * MegalithUI.Common.ParseColor.Hex
 *
 ...
 * inline `MegalithUIInternal::ParseColor` it replaced.
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FMegalithUIParseColorHexTest,
    "MegalithUI.Common.ParseColor.Hex",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithUIParseColorHexTest::RunTest(const FString& /*Parameters*/)
{
    ...
    {
        const FLinearColor C = MegalithUI::ParseColor(TEXT("#FF8800"));
    ...
    {
        const FLinearColor C = MegalithUI::ParseColor(TEXT("not-a-color"));
...
/**
 * MegalithUI.Common.TryParseColor.Hex
 *
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FMegalithUITryParseColorHexTest,
    "MegalithUI.Common.TryParseColor.Hex",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMegalithUITryParseColorHexTest::RunTest(const FString& /*Parameters*/)
{
    ...
    {
        FLinearColor C;
        TestTrue(TEXT("#FF8800 returns true"), MegalithUI::TryParseColor(TEXT("#FF8800"), C));
    ...
    {
        FLinearColor C;
        TestTrue(TEXT("#FF8800AA returns true"), MegalithUI::TryParseColor(TEXT("#FF8800AA"), C));
    ...
    {
        FLinearColor C;
        TestTrue(TEXT("#F80 returns true"), MegalithUI::TryParseColor(TEXT("#F80"), C));
    ...
    {
        FLinearColor C;
        TestTrue(TEXT("1.0,0.5,0.25 returns true"), MegalithUI::TryParseColor(TEXT("1.0,0.5,0.25"), C));
    ...
    {
        FLinearColor C;
        TestTrue(TEXT("1,0.5,0.25,0.8 returns true"), MegalithUI::TryParseColor(TEXT("1,0.5,0.25,0.8"), C));
    ...
    // Malformed strings return false and leave OutColor untouched.
    {
        ...
        TestFalse(TEXT("garbage returns false"), MegalithUI::TryParseColor(TEXT("not-a-color"), C));
        TestEqual(TEXT("OutColor untouched R"), C.R, 0.123f, KINDA_SMALL_NUMBER);

        TestFalse(TEXT("empty returns false"), MegalithUI::TryParseColor(TEXT(""), C));
        TestFalse(TEXT("#XYZ wrong length"), MegalithUI::TryParseColor(TEXT("#XYZW"), C));
        TestFalse(TEXT("only one comma part"), MegalithUI::TryParseColor(TEXT("1.0,0.5"), C));
    }
