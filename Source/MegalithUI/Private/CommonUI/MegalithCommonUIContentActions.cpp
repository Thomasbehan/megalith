...
namespace MegalithCommonUIContent
{
	// ----- 4.F.1 configure_numeric_text ----------------------------------------

	static FMegalithActionResult HandleConfigureNumericText(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() ||
		...
		if (!N) return FMegalithActionResult::Error(TEXT("target is not a UCommonNumericTextBlock"));
	...
	static FMegalithActionResult HandleConfigureRotator(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() ||
			!Params->TryGetStringField(TEXT("widget_name"), WidgetName))
		...
		if (!R) return FMegalithActionResult::Error(TEXT("target is not a UCommonRotator"));
	...
	static FMegalithActionResult AddWidgetToWbp(UClass* Cls, const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!Parent) return FMegalithActionResult::Error(TEXT("parent panel not found"));
	...
	static FMegalithActionResult HandleCreateLazyImage(const TSharedPtr<FJsonObject>& Params)
	{
	...
	static FMegalithActionResult HandleCreateLoadGuard(const TSharedPtr<FJsonObject>& Params)
	{
	...
	void Register(FMegalithToolRegistry& Registry)
	{
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConfigureNumericText),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConfigureRotator),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateLazyImage),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateLoadGuard),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Target WBP path"))
