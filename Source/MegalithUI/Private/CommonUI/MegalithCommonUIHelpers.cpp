...
namespace MegalithCommonUI
{
	FMegalithActionResult CreateAsset(
		UClass* AssetClass,
		...
		UObject*& OutAsset)
	{
		...
		if (!AssetClass)
		{
			return FMegalithActionResult::Error(TEXT("CreateAsset: AssetClass is null"));
		}
		if (PackagePath.IsEmpty() || AssetName.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("CreateAsset: PackagePath and AssetName required"));
		}
		...
		if (!Package)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("CreateAsset: CreatePackage failed for '%s'"), *FullPath));
		}

		// Collision check — both in-memory (FindObject) and on-disk (AssetRegistry).
		if (FindObject<UObject>(Package, *AssetName))
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("CreateAsset: asset already exists at '%s'"), *FullPath));
		}
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("CreateAsset: asset already exists at '%s' (on disk)"), *FullPath));
		}

		UObject* NewAsset = NewObject<UObject>(Package, AssetClass, FName(*AssetName), RF_Public | RF_Standalone);
		if (!NewAsset)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("CreateAsset: NewObject returned null for class '%s'"), *AssetClass->GetName()));
		}
	...
	FMegalithActionResult LoadWidgetForMutation(
		...
		UWidget*& OutWidget)
	{
		...
		if (!Wbp)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("LoadWidgetForMutation: failed to load WBP '%s'"), *WbpPath));
		}
		if (!Wbp->WidgetTree)
		{
			return FMegalithActionResult::Error(TEXT("LoadWidgetForMutation: WBP has no WidgetTree"));
		}
		...
		if (!Found)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("LoadWidgetForMutation: widget '%s' not found in '%s'"),
	...
	FMegalithActionResult CompileAndSaveWidgetBlueprint(UWidgetBlueprint* Wbp)
	{
		if (!Wbp)
		{
			return FMegalithActionResult::Error(TEXT("CompileAndSaveWidgetBlueprint: Wbp is null"));
		}
	...
	FMegalithActionResult InvokeRuntimeFunction(
		UObject* Target,
		...
		TSharedPtr<FJsonObject>& OutResult)
	{
		...
		if (!Target)
		{
			return FMegalithActionResult::Error(TEXT("InvokeRuntimeFunction: Target is null"));
		}

		UFunction* Func = Target->FindFunction(FunctionName);
		if (!Func)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("InvokeRuntimeFunction: function '%s' not found on '%s'"),
		...
		return FMegalithActionResult::Success(OutResult);
	}
