...
#include "Style/MegalithUIStyleService.h"
...
namespace MegalithCommonUIButton
{
	...
	// the asset-creation path now delegates to FMegalithUIStyleService which:
	//   1. Looks up the cache by asset_name (instant return on repeat call).
	//   2. Falls back to a canonical-library scan
	//      (UMegalithUISettings::CanonicalLibraryPath).
	...
	static FMegalithActionResult CreateStyleAsset(
		UClass* StyleClass,
		const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!StyleClass)
			return FMegalithActionResult::Error(TEXT("CreateStyleAsset: StyleClass is null"));
		...
		FUIStyleResolution Resolution = FMegalithUIStyleService::Get().ResolveOrCreate(
			StyleClass, AssetName, PackagePath, Properties);

		if (!Resolution.IsValid())
		{
			return FMegalithActionResult::Error(Resolution.Error.IsEmpty()
	...
	static FMegalithActionResult HandleCreateCommonButtonStyle(const TSharedPtr<FJsonObject>& Params)
	{
	...
	static FMegalithActionResult HandleCreateCommonTextStyle(const TSharedPtr<FJsonObject>& Params)
	{
	...
	static FMegalithActionResult HandleCreateCommonBorderStyle(const TSharedPtr<FJsonObject>& Params)
	{
	...
	static FMegalithActionResult HandleApplyStyleToWidget(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName, StyleAssetPath;
		if (!Params.IsValid() ||
			!Params->TryGetStringField(TEXT("widget_name"), WidgetName) ||
			!Params->TryGetStringField(TEXT("style_asset"), StyleAssetPath))
			return FMegalithActionResult::Error(TEXT("wbp_path, widget_name, style_asset required"));
		...
		if (!StyleClass)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to resolve style class '%s'"), *StyleAssetPath));
		...
		if (UCommonButtonBase* Btn = Cast<UCommonButtonBase>(Target))
		{
			if (!StyleClass->IsChildOf(UCommonButtonStyle::StaticClass()))
				return FMegalithActionResult::Error(TEXT("style_asset is not a UCommonButtonStyle"));
		...
		else if (UCommonTextBlock* Txt = Cast<UCommonTextBlock>(Target))
		{
			if (!StyleClass->IsChildOf(UCommonTextStyle::StaticClass()))
				return FMegalithActionResult::Error(TEXT("style_asset is not a UCommonTextStyle"));
		...
		else if (UCommonBorder* Brd = Cast<UCommonBorder>(Target))
		{
			if (!StyleClass->IsChildOf(UCommonBorderStyle::StaticClass()))
				return FMegalithActionResult::Error(TEXT("style_asset is not a UCommonBorderStyle"));
			Brd->SetStyle(StyleClass);
			AppliedVia = TEXT("UCommonBorder::SetStyle");
		}
		else
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Widget '%s' is not a button/text/border — cannot apply style"), *WidgetName));
		}
	...
	static FMegalithActionResult HandleConfigureCommonButton(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		...
		if (!Btn)
			return FMegalithActionResult::Error(TEXT("target is not a UCommonButtonBase"));
	...
	// `TokenforgeCommonButton` over the transient `MegalithDefaultCommonButton`
	...
	static FString ResolveDefaultButtonClass()
	{
		...
		return TEXT("/Game/Megalith/CommonUI/MegalithDefaultCommonButton.MegalithDefaultCommonButton_C");
	}

	static FMegalithActionResult HandleConvertButtonToCommon(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("wbp_path and widget_name required"));
		...
		else
		{
			...
			// resolvable, use it. The transient `MegalithDefaultCommonButton`
			// fallback below stays in place for the Tokenforge-absent case.
			const FString ResolvedDefault = ResolveDefaultButtonClass();
			if (!ResolvedDefault.StartsWith(TEXT("/Game/Megalith/CommonUI/MegalithDefaultCommonButton")))
			{
			...
			// MegalithDefaultCommonButton creation path if the Tokenforge probe
			...
			if (!TargetClass)
			{
				...
				{
					const FString DefaultPath = TEXT("/Game/Megalith/CommonUI/MegalithDefaultCommonButton");
					...
					if (!BP)
					{
						...
						if (Pkg)
						{
							BP = FKismetEditorUtilities::CreateBlueprint(
								...
								Pkg,
								TEXT("MegalithDefaultCommonButton"),
								BPTYPE_Normal,
				...
				if (!TargetClass)
					return FMegalithActionResult::Error(TEXT("Failed to create default CommonButton subclass. Pass a concrete target_class explicitly."));
			}
		}
		...
		if (!OldBtn)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Widget '%s' is not a UButton"), *WidgetName));

		UPanelSlot* ParentSlot = OldBtn->Slot;
		UPanelWidget* Parent = ParentSlot ? ParentSlot->Parent : nullptr;
		if (!Parent)
			return FMegalithActionResult::Error(TEXT("Cannot convert root-level button — parent required for reparent"));
		...
		MegalithUI::RegisterCreatedWidget(Wbp, NewBtn);
		...
		FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Wbp);
		MegalithUI::ReconcileWidgetVariableGuids(Wbp);
		FKismetEditorUtilities::CompileBlueprint(Wbp);
		Wbp->GetOutermost()->MarkPackageDirty();
	...
	// pre-built class, route the bag through FMegalithUIStyleService::Get()
	// .ResolveOrCreate to get back the class to apply — same pattern as
	// CreateStyleAsset above.

	static FMegalithActionResult HandleBatchRetheme(const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!Params.IsValid() ||
			...
			return FMegalithActionResult::Error(TEXT("folder_path, old_style, new_style required"));
	...
	static FMegalithActionResult HandleConfigureCommonText(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("wbp_path and widget_name required"));
		...
		if (!Txt)
			return FMegalithActionResult::Error(TEXT("target is not a UCommonTextBlock"));
	...
	static FMegalithActionResult HandleConfigureCommonBorder(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("wbp_path and widget_name required"));
		...
		if (!Brd)
			return FMegalithActionResult::Error(TEXT("target is not a UCommonBorder"));
	...
	// MegalithBlueprint/Private/MegalithBlueprintNodeActions.cpp and would
	// require either (a) cross-module include of those helpers (currently NOT
	// in MegalithUI.Build.cs PrivateDependencyModuleNames) or (b) re-implementing
	// the K2Node construction + pin-wiring inside MegalithUI. Either path
	...
	static FMegalithActionResult HandleApplyTokenBinding(const TSharedPtr<FJsonObject>& Params)
	{
		// --- Tokenforge availability probe (FULL impl) --------------------------
		// Mirrors the -32010 EffectSurface pattern from SPEC_MegalithUI §
		...
		// range (-32011..-32019 left open per MegalithJsonUtils.h:50).
		...
		if (!bTokenforgeAvailable)
		{
			...
			FMegalithActionResult Err = FMegalithActionResult::Error(
		...
		{
			return FMegalithActionResult::Error(TEXT("widget_name, target_property, token_key required"));
		}
	...
	static FMegalithActionResult HandleConvertTextBlockToCommon(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("wbp_path and widget_name required"));
		...
		if (!OldTxt)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Widget '%s' is not a UTextBlock"), *WidgetName));

		UPanelSlot* ParentSlot = OldTxt->Slot;
		UPanelWidget* Parent = ParentSlot ? ParentSlot->Parent : nullptr;
		if (!Parent)
			return FMegalithActionResult::Error(TEXT("Cannot convert root-level text block — parent required for reparent"));
		...
		if (!NewTxt)
			return FMegalithActionResult::Error(TEXT("ConstructWidget<UCommonTextBlock> returned null"));
		...
		MegalithUI::RegisterCreatedWidget(Wbp, NewTxt);
	...
	// Phase 1 Bug #4 (MegalithCommonUIInputActions.cpp:265-282) but additionally
	...
	static FMegalithActionResult HandleSetActionBarButtonClass(const TSharedPtr<FJsonObject>& Params)
	{
		FString WbpPath = MegalithCommonUI::GetWbpPath(Params);
		if (WbpPath.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path (or asset_path) required"));
		...
		{
			return FMegalithActionResult::Error(TEXT("widget_name and button_class required"));
		}
		...
		// (MegalithCommonUIInputActions.cpp:271-282). ActionButtonClass is a
	...
	static FMegalithActionResult HandleConvertBorderToCommon(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("wbp_path and widget_name required"));
		...
		if (!OldBorder)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Widget '%s' is not a UBorder"), *WidgetName));
		// A UCommonBorder is itself a UBorder; converting it would be a no-op.
		if (OldBorder->IsA<UCommonBorder>())
			return FMegalithActionResult::Error(FString::Printf(TEXT("Widget '%s' is already a UCommonBorder"), *WidgetName));

		UPanelSlot* ParentSlot = OldBorder->Slot;
		UPanelWidget* Parent = ParentSlot ? ParentSlot->Parent : nullptr;
		const bool bIsRoot = (Wbp->WidgetTree && Wbp->WidgetTree->RootWidget == OldBorder);
		if (!Parent && !bIsRoot)
			return FMegalithActionResult::Error(TEXT("Cannot convert a border with no parent that is also not the tree root"));
		...
		if (!NewBorder)
			return FMegalithActionResult::Error(TEXT("ConstructWidget<UCommonBorder> returned null"));
		...
		MegalithUI::RegisterCreatedWidget(Wbp, NewBorder);
	...
	static FMegalithActionResult HandleReparentWidgetRoot(const TSharedPtr<FJsonObject>& Params)
	{
		FString NewClassName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("new_class"), NewClassName) || NewClassName.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path and new_class required"));
		FString WbpPath = MegalithCommonUI::GetWbpPath(Params);
		if (WbpPath.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path (or asset_path) required"));
		...
		if (!OldRoot)
			return FMegalithActionResult::Error(TEXT("WBP has no root widget to reparent"));
		...
		MegalithUI::RegisterCreatedWidget(Wbp, NewRoot);
	...
	void Register(FMegalithToolRegistry& Registry)
	{
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConvertButtonToCommon),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConfigureCommonButton),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateCommonButtonStyle),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateCommonTextStyle),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateCommonBorderStyle),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("package_path"), TEXT("Folder path"))
				.Required(TEXT("asset_name"), TEXT("string"), TEXT("Asset name"))
				.Optional(TEXT("properties"), TEXT("object"), TEXT("Initial property values"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleApplyStyleToWidget),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleBatchRetheme),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConfigureCommonText),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConfigureCommonBorder),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleApplyTokenBinding),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConvertTextBlockToCommon),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path"))
		...
		// in a WBP. Mirrors Phase 1 Bug #4 (MegalithCommonUIInputActions.cpp:265-282)
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleSetActionBarButtonClass),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConvertBorderToCommon),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path (alias: asset_path)"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleReparentWidgetRoot),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path (alias: asset_path)"))
