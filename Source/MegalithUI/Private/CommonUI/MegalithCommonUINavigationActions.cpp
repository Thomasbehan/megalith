...
namespace MegalithCommonUINavigation
{
	...
	static FMegalithActionResult HandleSetWidgetNavigation(const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!Params.IsValid() ||
			...
			return FMegalithActionResult::Error(TEXT("wbp_path, widget_name, direction, rule required"));
		...
		if (!ParseDirection(DirStr, Dir))
			return FMegalithActionResult::Error(TEXT("direction must be Up/Down/Left/Right/Next/Previous"));
		EUINavigationRule Rule;
		if (!ParseRule(RuleStr, Rule))
			return FMegalithActionResult::Error(TEXT("rule must be Escape/Stop/Wrap/Explicit/Custom/CustomBoundary"));
		...
		if (Rule == EUINavigationRule::Explicit)
		{
			...
			if (!Params->TryGetStringField(TEXT("explicit_target"), ExplicitTargetName))
				return FMegalithActionResult::Error(TEXT("rule=Explicit requires explicit_target widget name"));
			...
			if (!ExplicitTarget)
				return FMegalithActionResult::Error(FString::Printf(TEXT("explicit_target '%s' not found"), *ExplicitTargetName));
	...
	static FMegalithActionResult HandleSetWidgetNavigationBulk(const TSharedPtr<FJsonObject>& Params)
	{
		FString WbpPath = MegalithCommonUI::GetWbpPath(Params);
		if (WbpPath.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path (or asset_path) required"));
		...
		if (!Params.IsValid() || !Params->TryGetArrayField(TEXT("entries"), Entries) || !Entries)
			return FMegalithActionResult::Error(TEXT("entries array required: [{widget_name, direction, rule, explicit_target?}]"));
		...
		if (Written > 0)
		{
			...
			if (bSave)
			{
				MegalithCommonUI::CompileAndSaveWidgetBlueprint(Wbp);
			}
			else
			{
				FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Wbp);
	...
	static FMegalithActionResult HandleDumpWidgetNavigation(const TSharedPtr<FJsonObject>& Params)
	{
		FString WbpPath = MegalithCommonUI::GetWbpPath(Params);
		if (WbpPath.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path (or asset_path) required"));
	...
	static FMegalithActionResult HandleSetInitialFocusTarget(const TSharedPtr<FJsonObject>& Params)
	{
		FString TargetWidgetName;
		if (!Params.IsValid() ||
			!Params->TryGetStringField(TEXT("target_widget"), TargetWidgetName))
			return FMegalithActionResult::Error(TEXT("wbp_path and target_widget required"));
		FString WbpPath = MegalithCommonUI::GetWbpPath(Params);
		if (WbpPath.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path (or asset_path) required"));
		...
		if (!Wbp->GeneratedClass->IsChildOf(UCommonActivatableWidget::StaticClass()))
			return FMegalithActionResult::Error(TEXT("WBP is not a UCommonActivatableWidget — focus target requires activatable parent"));
		...
		if (!bTargetFound)
			return FMegalithActionResult::Error(FString::Printf(TEXT("target_widget '%s' not in WBP tree"), *TargetWidgetName));
	...
	static FMegalithActionResult HandleForceFocus(const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("widget_name required"));

		UWidget* Target = FindWidgetInPIE(FName(*WidgetName));
		if (!Target)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Widget '%s' not found in PIE"), *WidgetName));
	...
	static FMegalithActionResult HandleGetFocusPath(const TSharedPtr<FJsonObject>& Params)
	{
		if (!MegalithCommonUI::GetPIEWorld())
			return FMegalithActionResult::Error(TEXT("requires PIE session"));

		if (!FSlateApplication::IsInitialized())
			return FMegalithActionResult::Error(TEXT("Slate application not initialized"));
	...
	static FMegalithActionResult HandleAuditFocusChain(const TSharedPtr<FJsonObject>& Params)
	{
		FString WbpPath = MegalithCommonUI::GetWbpPath(Params);
		if (WbpPath.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path (or asset_path) required"));
	...
	static FMegalithActionResult HandleRequestRefreshFocus(const TSharedPtr<FJsonObject>& Params)
	{
		if (!MegalithCommonUI::GetPIEWorld())
			return FMegalithActionResult::Error(TEXT("requires PIE session"));
		...
		FString WidgetName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("widget_name required (activatable widget FName)"));
		...
		if (!Found)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Activatable widget '%s' not found in PIE"), *WidgetName));
	...
	void Register(FMegalithToolRegistry& Registry)
	{
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleSetWidgetNavigation),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleSetWidgetNavigationBulk),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleDumpWidgetNavigation),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path (alias: asset_path)"))
		...
		Registry.RegisterAction(
			...
			TEXT("Store DesiredFocusTargetName FName UPROPERTY on a UCommonActivatableWidget CDO. WBP must expose this UPROPERTY and override NativeGetDesiredFocusTarget. Satisfying parent classes: UTokenforgeActivatableWidget, UMegalithReduceMotionAwareWidget. If parent doesn't expose the UPROPERTY, use ui::add_widget_variable to add it first."),
			FMegalithActionHandler::CreateStatic(&HandleSetInitialFocusTarget),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleForceFocus),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleGetFocusPath),
			nullptr,
			Cat);
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleRequestRefreshFocus),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleAuditFocusChain),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Widget Blueprint path (alias: asset_path)"))
				.Build(),
			Cat);
