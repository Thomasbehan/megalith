...
namespace MegalithCommonUIAccessibility
{
	...
	static FMegalithActionResult HandleEnforceFocusRing(const TSharedPtr<FJsonObject>& Params)
	{
		FString FolderPath;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("folder_path"), FolderPath))
			return FMegalithActionResult::Error(TEXT("folder_path required"));
	...
	static FMegalithActionResult HandleWrapWithReduceMotionGate(const TSharedPtr<FJsonObject>& Params)
	{
		FString FolderPath;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("folder_path"), FolderPath))
			return FMegalithActionResult::Error(TEXT("folder_path required"));
		...
		// Megalith doesn't itself ship a base class; project owners typically
		...
		static const TArray<FString> SuggestedParentClasses = {
			...
			TEXT("UMegalithReduceMotionAwareWidget")
		};
	...
	static FMegalithActionResult HandleSetTextScaleBinding(const TSharedPtr<FJsonObject>& Params)
	{
		FString FolderPath;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("folder_path"), FolderPath))
			return FMegalithActionResult::Error(TEXT("folder_path required"));
	...
	static FMegalithActionResult HandleApplyHighContrastVariant(const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!Params.IsValid() ||
			...
			return FMegalithActionResult::Error(TEXT("folder_path, normal_style, high_contrast_style required"));

		UClass* OldClass = LoadClass<UObject>(nullptr, *StyleNormal);
		UClass* NewClass = LoadClass<UObject>(nullptr, *StyleHC);
		if (!OldClass || !NewClass)
			return FMegalithActionResult::Error(TEXT("Failed to resolve normal/high_contrast style class paths"));
	...
	void Register(FMegalithToolRegistry& Registry)
	{
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleEnforceFocusRing),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleWrapWithReduceMotionGate),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleSetTextScaleBinding),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("folder_path"), TEXT("Folder to scan"))
				.Build(),
			Cat);
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleApplyHighContrastVariant),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("folder_path"), TEXT("Folder to scan"))
