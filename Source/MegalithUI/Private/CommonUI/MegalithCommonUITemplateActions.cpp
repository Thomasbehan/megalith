// MegalithCommonUITemplateActions.cpp
//
...
// extern entry point `Register(FMegalithToolRegistry&)` is the only symbol
// visible to MegalithCommonUIActionsAggregator.cpp. WITH_COMMONUI-gated so
...
// listed in MegalithUI.Build.cs PrivateDependencyModuleNames.
...
// MegalithCommonUINavigationActions.cpp precedent (line 19) uses.
#include "Blueprint/WidgetNavigation.h"
#include "Types/NavigationMetaData.h"
#include "Types/SlateEnums.h"

namespace MegalithCommonUITemplate
{
    // ----- Shared: default UCommonButtonBase path ------------------------------
    //
    // Mirrors the Phase 1 Bug #4 contract from create_bound_action_bar
    // (MegalithCommonUIInputActions.cpp:199). Action-bar + per-button defaults
    // both resolve to the same Megalith stock button so a fresh scaffold
    // compiles without manual class-assignment.
    static const TCHAR* DefaultActionButtonClassPath =
        TEXT("/Game/Megalith/CommonUI/MegalithDefaultCommonButton.MegalithDefaultCommonButton_C");
    ...
    // (MegalithCommonUIButtonActions.cpp:783-786).
    static UClass* ResolveActivatableParentClass(bool& bOutUsedTokenforge)
    {
        ...
        if (TokenforgePlugin.IsValid() && TokenforgePlugin->IsEnabled())
        {
            ...
            // MegalithCommonUIActivatableActions.cpp:78-80 precedent does the
    ...
    //
    // Mirrors the create_activatable_widget flow (MegalithCommonUIActivatableActions.cpp:55-104):
    ...
    // save.
    static FMegalithActionResult CreateBlankActivatableWBP(
        ...
        FString& OutAssetName)
    {
        if (!ParentClass)
        {
            return FMegalithActionResult::Error(TEXT("ParentClass null"));
        }
        FString PackagePath;
        if (!SplitSavePath(SavePath, PackagePath, OutAssetName))
        {
            return FMegalithActionResult::Error(TEXT("save_path must contain at least one / separator"));
        }
        ...
        if (!OutWbp)
        {
            return FMegalithActionResult::Error(TEXT("UWidgetBlueprintFactory returned null"));
        }
        return FMegalithActionResult::Success(MakeShared<FJsonObject>());
    }

    // ----- Shared: stamp DesiredFocusTargetName on the WBP --------------------
    //
    // The set_initial_focus_target action (MegalithCommonUINavigationActions.cpp:124-183)
    ...
    // (MegalithUIRegistryActions.cpp:248). Caller-side override does not need
    ...
    static FMegalithActionResult HandleScaffoldMainMenu(const TSharedPtr<FJsonObject>& Params)
    {
        FString SavePath;
        ...
        FString AssetName;
        FMegalithActionResult CreateRes = CreateBlankActivatableWBP(SavePath, ParentClass, Wbp, Package, AssetName);
        ...
        // an action_button_class or stand up the default Megalith button.
    ...
    static FMegalithActionResult HandleScaffoldSettingsPanelWithTabs(const TSharedPtr<FJsonObject>& Params)
    {
        FString SavePath;
        if (!Params.IsValid() || !Params->TryGetStringField(TEXT("save_path"), SavePath))
        {
    ...
    static FMegalithActionResult HandleScaffoldPauseMenu(const TSharedPtr<FJsonObject>& Params)
    {
        FString SavePath;
        if (!Params.IsValid() || !Params->TryGetStringField(TEXT("save_path"), SavePath))
        {
        ...
        // uses at MegalithCommonUIButtonActions.cpp:249).
        if (QuitButton)
        {
    ...
    //
    // Called from MegalithCommonUIActionsAggregator.cpp. The forward-declaration
    // in the aggregator (`namespace MegalithCommonUITemplate { void Register(...); }`)
    ...
    // MegalithCommonUIActivatableActions.cpp etc.).
    void Register(FMegalithToolRegistry& Registry)
    {
        ...
        Registry.RegisterAction(
            ...
            TEXT("Phase 3 Tier-3 scaffolder. One-shot main-menu WBP: Overlay root + Title + "
                 ...
                 "ActionButtonClass set to MegalithDefaultCommonButton_C (or action_button_class "
            ...
            FMegalithActionHandler::CreateStatic(&HandleScaffoldMainMenu),
            FParamSchemaBuilder()
                ...
                .Optional(TEXT("action_button_class"), TEXT("string"),
                    TEXT("UCommonButtonBase subclass path with _C suffix (default: /Game/Megalith/CommonUI/MegalithDefaultCommonButton.MegalithDefaultCommonButton_C)"))
        ...
        Registry.RegisterAction(
            ...
            TEXT("Phase 3 Tier-3 scaffolder. One-shot settings-panel WBP with tab list + animated "
                 ...
                 "with ActionButtonClass set to MegalithDefaultCommonButton_C. Parent class follows "
            ...
            FMegalithActionHandler::CreateStatic(&HandleScaffoldSettingsPanelWithTabs),
            FParamSchemaBuilder()
                ...
                .Optional(TEXT("action_button_class"), TEXT("string"),
                    TEXT("UCommonButtonBase subclass path (default: /Game/Megalith/CommonUI/MegalithDefaultCommonButton.MegalithDefaultCommonButton_C)"))
                .Build(),
            Cat);
        ...
        Registry.RegisterAction(
            ...
            FMegalithActionHandler::CreateStatic(&HandleScaffoldPauseMenu),
            FParamSchemaBuilder()
                ...
                .Optional(TEXT("parent_class"), TEXT("string"),
                    TEXT("Override parent class (default: Tokenforge if enabled, else UCommonActivatableWidget)"))
                .Optional(TEXT("action_button_class"), TEXT("string"),
                    TEXT("UCommonButtonBase subclass path (default: MegalithDefaultCommonButton_C)"))
