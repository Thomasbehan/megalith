...
namespace MegalithCommonUIAudit
{
	// ----- 5.H.1 audit_commonui_widget -----------------------------------------
	...
	static FMegalithActionResult HandleAuditCommonUIWidget(const TSharedPtr<FJsonObject>& Params)
	{
		if (!Params.IsValid())
	...
	static FMegalithActionResult HandleExportCommonUIReport(const TSharedPtr<FJsonObject>& Params)
	{
	...
	static FMegalithActionResult HandleHotReloadStyles(const TSharedPtr<FJsonObject>& Params)
	{
	...
	static FMegalithActionResult HandleDumpActionRouterState(const TSharedPtr<FJsonObject>& Params)
	{
		UWorld* PIE = MegalithCommonUI::GetPIEWorld();
		if (!PIE) return FMegalithActionResult::Error(TEXT("requires PIE session"));

		APlayerController* PC = PIE->GetFirstPlayerController();
		if (!PC) return FMegalithActionResult::Error(TEXT("no player controller"));
		ULocalPlayer* LP = PC->GetLocalPlayer();
		if (!LP) return FMegalithActionResult::Error(TEXT("no local player"));
	...
	void Register(FMegalithToolRegistry& Registry)
	{
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleAuditCommonUIWidget),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleExportCommonUIReport),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleHotReloadStyles),
			nullptr,
			Cat);
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleDumpActionRouterState),
			nullptr,
			Cat);
	}
}
