...
namespace MegalithCommonUIActivatable
{
	// ----- 2.A.1 create_activatable_widget -------------------------------------

	static FMegalithActionResult HandleCreateActivatableWidget(const TSharedPtr<FJsonObject>& Params)
	{
		FString SavePath;
		...
		if (!Package)
			return FMegalithActionResult::Error(FString::Printf(TEXT("CreatePackage failed for '%s'"), *SavePath));
		...
		if (!Wbp)
			return FMegalithActionResult::Error(TEXT("WidgetBlueprintFactory returned null"));
	...
	static FMegalithActionResult AddContainerToWbp(
		...
		const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!Parent)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Parent panel '%s' not found or not a UPanelWidget"), *ParentWidgetName));

		UWidget* New = Wbp->WidgetTree->ConstructWidget<UWidget>(ContainerClass, FName(*WidgetName));
		if (!New)
			return FMegalithActionResult::Error(FString::Printf(TEXT("ConstructWidget failed for class '%s'"), *ContainerClass->GetName()));
	...
	static FMegalithActionResult HandleCreateActivatableStack(const TSharedPtr<FJsonObject>& Params)
	{
	...
	static FMegalithActionResult HandleCreateActivatableSwitcher(const TSharedPtr<FJsonObject>& Params)
	{
		...
		if (!SwitcherClass)
			return FMegalithActionResult::Error(TEXT("UCommonActivatableWidgetSwitcher class not found"));
		return AddContainerToWbp(SwitcherClass, Params);
	}

	// ----- 2.A.4 configure_activatable ------------------------------------------

	static FMegalithActionResult HandleConfigureActivatable(const TSharedPtr<FJsonObject>& Params)
	{
		if (!Params.IsValid())
			return FMegalithActionResult::Error(TEXT("wbp_path required"));
		FString WbpPath = MegalithCommonUI::GetWbpPath(Params);
		if (WbpPath.IsEmpty())
			return FMegalithActionResult::Error(TEXT("wbp_path (or asset_path) required"));
		...
		if (!GenClass || !GenClass->IsChildOf(UCommonActivatableWidget::StaticClass()))
			return FMegalithActionResult::Error(TEXT("WBP generated class is not a UCommonActivatableWidget"));

		UCommonActivatableWidget* CDO = Cast<UCommonActivatableWidget>(GenClass->GetDefaultObject());
		if (!CDO)
			return FMegalithActionResult::Error(TEXT("CDO null"));
	...
	static FMegalithActionResult HandlePushToActivatableStack(const TSharedPtr<FJsonObject>& Params)
	{
		if (!MegalithCommonUI::GetPIEWorld())
			return FMegalithActionResult::Error(TEXT("requires PIE session"), FMegalithJsonUtils::ErrInvalidParams);
		...
		if (!Params.IsValid() ||
			...
			return FMegalithActionResult::Error(TEXT("container_name and widget_class required"));
		...
		if (!ResolvedClass)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to resolve widget class '%s'"), *WidgetClassPath));
	...
	static FMegalithActionResult HandlePopActivatableStack(const TSharedPtr<FJsonObject>& Params)
	{
		if (!MegalithCommonUI::GetPIEWorld())
			return FMegalithActionResult::Error(TEXT("requires PIE session"), FMegalithJsonUtils::ErrInvalidParams);
		...
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("container_name"), ContainerName))
			return FMegalithActionResult::Error(TEXT("container_name required"));
	...
	static FMegalithActionResult HandleGetActivatableStackState(const TSharedPtr<FJsonObject>& Params)
	{
		if (!MegalithCommonUI::GetPIEWorld())
			return FMegalithActionResult::Error(TEXT("requires PIE session"), FMegalithJsonUtils::ErrInvalidParams);
	...
	static FMegalithActionResult HandleSetActivatableTransition(const TSharedPtr<FJsonObject>& Params)
	{
		FString WidgetName;
		if (!Params.IsValid() || !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
			return FMegalithActionResult::Error(TEXT("wbp_path and widget_name required"));
		...
		if (!Container)
			return FMegalithActionResult::Error(TEXT("target widget is not a UCommonActivatableWidgetContainerBase"));
	...
	void Register(FMegalithToolRegistry& Registry)
	{
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateActivatableWidget),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateActivatableStack),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleCreateActivatableSwitcher),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Target Widget Blueprint path"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleConfigureActivatable),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandlePushToActivatableStack),
			FParamSchemaBuilder()
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandlePopActivatableStack),
			FParamSchemaBuilder()
				.Required(TEXT("container_name"), TEXT("string"), TEXT("FName of the container widget in the live UMG tree"))
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleGetActivatableStackState),
			FParamSchemaBuilder()
				.Required(TEXT("container_name"), TEXT("string"), TEXT("FName of the container widget in the live UMG tree"))
				.Build(),
			Cat);
		...
		Registry.RegisterAction(
			...
			FMegalithActionHandler::CreateStatic(&HandleSetActivatableTransition),
			FParamSchemaBuilder()
				.RequiredAssetPath(TEXT("wbp_path"), TEXT("Target Widget Blueprint path"))
