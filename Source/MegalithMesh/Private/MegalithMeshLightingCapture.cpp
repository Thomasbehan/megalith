...
UTextureRenderTarget2D* MegalithLightingCapture::CreateTransientRT(int32 Width, int32 Height)
{
...
USceneCaptureComponent2D* MegalithLightingCapture::CreateLightingCapture(UWorld* World, UTextureRenderTarget2D* RT)
{
...
USceneCaptureComponent2D* MegalithLightingCapture::CreateOrthoLightingCapture(UWorld* World, UTextureRenderTarget2D* RT, float OrthoWidth)
{
...
bool MegalithLightingCapture::CaptureAtPoint(USceneCaptureComponent2D* Capture, const FVector& Location,
...
bool MegalithLightingCapture::ReadPixelsHDR(UTextureRenderTarget2D* RT, TArray<FLinearColor>& OutPixels)
{
	if (!RT)
	{
		return false;
...
void MegalithLightingCapture::CleanupCapture(USceneCaptureComponent2D* Capture)
{
	if (Capture)
	{
...
float MegalithLightingCapture::ComputeLuminance(const FLinearColor& Color)
{
	// Rec. 709 luminance coefficients
	return 0.2126f * Color.R + 0.7152f * Color.G + 0.0722f * Color.B;
}

float MegalithLightingCapture::AverageLuminance(const TArray<FLinearColor>& Pixels)
{
	if (Pixels.Num() == 0)
	{
		return 0.0f;
...
FLinearColor MegalithLightingCapture::AverageColor(const TArray<FLinearColor>& Pixels)
{
	if (Pixels.Num() == 0)
	{
...
float MegalithLightingCapture::EstimateColorTemperature(const FLinearColor& Color)
{
...
TArray<MegalithLightingCapture::FLightInfo> MegalithLightingCapture::GatherLights(UWorld* World)
{
...
TArray<MegalithLightingCapture::FLightInfo> MegalithLightingCapture::GatherLightsInRegion(UWorld* World, const FBox& Region)
{
...
float MegalithLightingCapture::ComputeAnalyticLuminance(UWorld* World, const FVector& Point,
	const TArray<FLightInfo>& Lights, int32& OutDominantLight, bool bTraceShadows)
{
	...
	FCollisionQueryParams ShadowTraceParams(SCENE_QUERY_STAT(MegalithAnalyticShadow), true);
...
bool MegalithLightingCapture::IsInShadow(UWorld* World, const FVector& Point, const FVector& LightLocation)
{
	if (!World)
	{
		return true;
	...
	FHitResult Hit;
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithShadowTrace), true);
...
bool MegalithLightingCapture::CapturePointsAtlas(UWorld* World, const TArray<FVector>& Points, int32 TileSize,
...
float MegalithLightingCapture::ExtractTileLuminance(const TArray<FLinearColor>& AtlasPixels, int32 AtlasWidth,
...
FLinearColor MegalithLightingCapture::ExtractTileColor(const TArray<FLinearColor>& AtlasPixels, int32 AtlasWidth,
	int32 TileIndex, int32 TileSize, int32 TilesPerRow)
{
	const int32 TileCol = TileIndex % TilesPerRow;
	const int32 TileRow = TileIndex / TilesPerRow;
...
MegalithLightingCapture::FCoverageResult MegalithLightingCapture::CaptureCoverageMap(
...
TArray<MegalithLightingCapture::FDarkZone> MegalithLightingCapture::FloodFillDarkZones(
