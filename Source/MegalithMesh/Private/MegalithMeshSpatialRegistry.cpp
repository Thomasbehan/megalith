...
TSharedPtr<FJsonObject> FSpatialBlock::ToJson() const
{
	...
	{
		RoomsArr.Add(MakeShared<FJsonValueObject>(FMegalithMeshSpatialRegistry::RoomToJson(R)));
	}
	...
	{
		DoorsArr.Add(MakeShared<FJsonValueObject>(FMegalithMeshSpatialRegistry::DoorToJson(D)));
	}
	...
	if (Json->TryGetArrayField(TEXT("rooms"), RoomsArr) && RoomsArr)
	{
		for (const auto& RVal : *RoomsArr)
		{
			...
			if (RObj->TryGetObjectField(TEXT("tags"), TagsPtr) && TagsPtr && (*TagsPtr).IsValid())
			{
				...
				{
					R.Tags.Add(MegalithKeyToString(Key), Val->AsString());
				}
			}
	...
	if (Json->TryGetObjectField(TEXT("adjacency_graph"), GraphPtr) && GraphPtr && (*GraphPtr).IsValid())
	{
		for (const auto& [RoomId, Val] : (*GraphPtr)->Values)
		{
			...
			if (Val->TryGetArray(EdgeArr) && EdgeArr)
			{
				...
				Block.AdjacencyGraph.Add(MegalithKeyToString(RoomId), MoveTemp(Edges));
			}
		}
	}
...
TMap<FString, FSpatialBlock>& FMegalithMeshSpatialRegistry::GetBlockMap()
{
	static TMap<FString, FSpatialBlock> GBlocks;
	return GBlocks;
}

FSpatialBlock& FMegalithMeshSpatialRegistry::GetBlock(const FString& BlockId)
{
...
bool FMegalithMeshSpatialRegistry::HasBlock(const FString& BlockId)
{
	return GetBlockMap().Contains(BlockId);
}

void FMegalithMeshSpatialRegistry::ClearBlock(const FString& BlockId)
{
	GetBlockMap().Remove(BlockId);
}

void FMegalithMeshSpatialRegistry::ClearAll()
{
	GetBlockMap().Empty();
}

TArray<FString> FMegalithMeshSpatialRegistry::GetLoadedBlockIds()
{
	TArray<FString> Keys;
...
FString FMegalithMeshSpatialRegistry::GetSaveDirectory()
{
	return FPaths::ProjectSavedDir() / TEXT("Megalith") / TEXT("SpatialRegistry");
}

bool FMegalithMeshSpatialRegistry::EnsureSaveDirectory()
{
...
bool FMegalithMeshSpatialRegistry::ParseVector(const TSharedPtr<FJsonObject>& Obj, const FString& FieldName, FVector& OutVec)
{
...
bool FMegalithMeshSpatialRegistry::ParseBounds(const TSharedPtr<FJsonObject>& Obj, FBox& OutBox)
{
	const TArray<TSharedPtr<FJsonValue>>* MinArr = nullptr;
	const TArray<TSharedPtr<FJsonValue>>* MaxArr = nullptr;
...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshSpatialRegistry::VecToJsonArray(const FVector& V)
{
...
TSharedPtr<FJsonObject> FMegalithMeshSpatialRegistry::RoomToJson(const FSpatialRoom& Room)
{
	auto J = MakeShared<FJsonObject>();
...
TSharedPtr<FJsonObject> FMegalithMeshSpatialRegistry::DoorToJson(const FSpatialDoor& Door)
{
	auto J = MakeShared<FJsonObject>();
...
void FMegalithMeshSpatialRegistry::BuildAdjacencyFromBuilding(FSpatialBlock& Block,
...
TArray<FString> FMegalithMeshSpatialRegistry::BFS(const FSpatialBlock& Block,
...
void FMegalithMeshSpatialRegistry::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 1. register_building
	Registry.RegisterAction(TEXT("mesh"), TEXT("register_building"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::RegisterBuilding),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("register_room"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::RegisterRoom),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("register_street_furniture"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::RegisterStreetFurniture),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("query_room_at"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::QueryRoomAt),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("query_adjacent_rooms"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::QueryAdjacentRooms),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("query_rooms_by_filter"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::QueryRoomsByFilter),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("query_building_exits"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::QueryBuildingExits),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("path_between_rooms"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::PathBetweenRooms),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("save_block_descriptor"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::SaveBlockDescriptor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("load_block_descriptor"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshSpatialRegistry::LoadBlockDescriptor),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshSpatialRegistry::RegisterBuilding(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: building_descriptor (object)"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("building_descriptor must contain a 'building_id' field"));
	}
...
FMegalithActionResult FMegalithMeshSpatialRegistry::RegisterRoom(const TSharedPtr<FJsonObject>& Params)
{
	FString RoomId, RoomType, BuildingId;
	if (!Params->TryGetStringField(TEXT("room_id"), RoomId) || RoomId.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: room_id"));
	if (!Params->TryGetStringField(TEXT("room_type"), RoomType) || RoomType.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: room_type"));
	if (!Params->TryGetStringField(TEXT("building_id"), BuildingId) || BuildingId.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: building_id"));

	int32 FloorIndex = 0;
	if (Params->HasField(TEXT("floor_index")))
		FloorIndex = static_cast<int32>(Params->GetNumberField(TEXT("floor_index")));
	else
		return FMegalithActionResult::Error(TEXT("Missing required param: floor_index"));

	const TSharedPtr<FJsonObject>* BoundsPtr = nullptr;
	if (!Params->TryGetObjectField(TEXT("world_bounds"), BoundsPtr) || !BoundsPtr || !(*BoundsPtr).IsValid())
		return FMegalithActionResult::Error(TEXT("Missing required param: world_bounds (object with min/max arrays)"));

	FBox WorldBounds(ForceInit);
	if (!ParseBounds(*BoundsPtr, WorldBounds))
		return FMegalithActionResult::Error(TEXT("Invalid world_bounds: needs {min: [x,y,z], max: [x,y,z]}"));
	...
	if (Params->TryGetObjectField(TEXT("tags"), TagsPtr) && TagsPtr && (*TagsPtr).IsValid())
	{
		for (const auto& [Key, Val] : (*TagsPtr)->Values)
		{
			Room.Tags.Add(MegalithKeyToString(Key), Val->AsString());
		}
	}
...
FMegalithActionResult FMegalithMeshSpatialRegistry::RegisterStreetFurniture(const TSharedPtr<FJsonObject>& Params)
{
	FString FurnitureId, FurnitureType;
	if (!Params->TryGetStringField(TEXT("furniture_id"), FurnitureId) || FurnitureId.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: furniture_id"));
	if (!Params->TryGetStringField(TEXT("furniture_type"), FurnitureType) || FurnitureType.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: furniture_type"));

	FVector WorldPos;
	if (!ParseVector(Params, TEXT("world_position"), WorldPos))
		return FMegalithActionResult::Error(TEXT("Missing required param: world_position (array of 3 numbers)"));
...
FMegalithActionResult FMegalithMeshSpatialRegistry::QueryRoomAt(const TSharedPtr<FJsonObject>& Params)
{
	FVector Position;
	if (!ParseVector(Params, TEXT("position"), Position))
		return FMegalithActionResult::Error(TEXT("Missing required param: position (array of 3 numbers)"));
...
FMegalithActionResult FMegalithMeshSpatialRegistry::QueryAdjacentRooms(const TSharedPtr<FJsonObject>& Params)
{
	FString RoomId;
	if (!Params->TryGetStringField(TEXT("room_id"), RoomId) || RoomId.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: room_id"));
	...
	if (!HasBlock(BlockId))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Block '%s' not loaded"), *BlockId));

	const FSpatialBlock& Block = GetBlock(BlockId);

	if (!Block.Rooms.Contains(RoomId))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Room '%s' not found in block '%s'"), *RoomId, *BlockId));
...
FMegalithActionResult FMegalithMeshSpatialRegistry::QueryRoomsByFilter(const TSharedPtr<FJsonObject>& Params)
{
	FString BlockId = TEXT("default");
	Params->TryGetStringField(TEXT("block_id"), BlockId);
	...
	for (const auto& [Id, Room] : Block.Rooms)
	{
		...
		if (bHasTagsFilter)
		{
			...
			{
				const FString* RoomTag = Room.Tags.Find(MegalithKeyToString(Key));
...
FMegalithActionResult FMegalithMeshSpatialRegistry::QueryBuildingExits(const TSharedPtr<FJsonObject>& Params)
{
	FString BuildingId;
	if (!Params->TryGetStringField(TEXT("building_id"), BuildingId) || BuildingId.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: building_id"));
	...
	if (!Bldg)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Building '%s' not found in block '%s'"), *BuildingId, *BlockId));
...
FMegalithActionResult FMegalithMeshSpatialRegistry::PathBetweenRooms(const TSharedPtr<FJsonObject>& Params)
{
	FString FromRoom, ToRoom;
	if (!Params->TryGetStringField(TEXT("from_room_id"), FromRoom) || FromRoom.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: from_room_id"));
	if (!Params->TryGetStringField(TEXT("to_room_id"), ToRoom) || ToRoom.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: to_room_id"));
	...
	if (!Block.Rooms.Contains(FromRoom))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Room '%s' not found in block '%s'"), *FromRoom, *BlockId));
	if (!Block.Rooms.Contains(ToRoom))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Room '%s' not found in block '%s'"), *ToRoom, *BlockId));
...
FMegalithActionResult FMegalithMeshSpatialRegistry::SaveBlockDescriptor(const TSharedPtr<FJsonObject>& Params)
{
	FString BlockId = TEXT("default");
	Params->TryGetStringField(TEXT("block_id"), BlockId);

	if (!HasBlock(BlockId))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Block '%s' not loaded — nothing to save"), *BlockId));

	if (!EnsureSaveDirectory())
		return FMegalithActionResult::Error(TEXT("Failed to create save directory"));
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to write temp file: %s"), *TmpPath));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to rename temp file to: %s"), *FilePath));
	}
...
FMegalithActionResult FMegalithMeshSpatialRegistry::LoadBlockDescriptor(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (BlockId.IsEmpty() && Filename.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Provide either 'block_id' or 'filename'"));
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load file: %s"), *FilePath));
	}

	TSharedPtr<FJsonObject> Json = FMegalithJsonUtils::Parse(JsonStr);
	if (!Json.IsValid())
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to parse JSON from: %s"), *FilePath));
	}
