...
void FMegalithMeshHorrorActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_sightlines"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::AnalyzeSightlines),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_hiding_spots"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::FindHidingSpots),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_ambush_points"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::FindAmbushPoints),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_choke_points"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::AnalyzeChokePoints),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_escape_routes"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::AnalyzeEscapeRoutes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("classify_zone_tension"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::ClassifyZoneTension),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_pacing_curve"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::AnalyzePacingCurve),
		FParamSchemaBuilder()
			.Required(TEXT("path_points"), TEXT("array"), TEXT("Array of path positions [[x,y,z], ...]"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_dead_ends"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorActions::FindDeadEnds),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshHorrorActions::AnalyzeSightlines(const TSharedPtr<FJsonObject>& Params)
{
	FVector Location;
	...
	MegalithMeshUtils::ParseVector(Params, TEXT("forward"), Forward);
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithSightlines), true);
...
FMegalithActionResult FMegalithMeshHorrorActions::FindHidingSpots(const TSharedPtr<FJsonObject>& Params)
{
	FVector RegionMin, RegionMax;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("region_min"), RegionMin))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: region_min"));
	}
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("region_max"), RegionMax))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: region_max"));
	}

	TArray<FVector> Viewpoints;
	if (!MHorror_ParseVectorArray(Params, TEXT("viewpoints"), Viewpoints))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: viewpoints (array of [x,y,z])"));
	}
	...
	for (int32 X = 0; X < XCount; ++X)
	{
		for (int32 Y = 0; Y < YCount; ++Y)
		{
			...
			float Concealment = MegalithMeshAnalysis::ComputeConcealment(World, TestPt, Viewpoints);
...
FMegalithActionResult FMegalithMeshHorrorActions::FindAmbushPoints(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithMeshHorrorActions::AnalyzeChokePoints(const TSharedPtr<FJsonObject>& Params)
{
	FVector Start, End;
	...
	TArray<FVector> PathPoints;
	float PathDist;
	if (!MegalithMeshAnalysis::FindNavPath(World, Start, End, PathPoints, PathDist, static_cast<float>(AgentRadius)))
	{
		return FMegalithActionResult::Error(TEXT("No navmesh path found between start and end. Build navigation first."));
	}
	...
	for (int32 i = 0; i < Clearances.Num(); ++i)
	{
		...
		if (C.TotalWidth < ChokeThreshold)
		{
			...
			float FlankDist;
			if (MegalithMeshAnalysis::FindNavPath(World, FlankStart, FlankEnd, FlankPath, FlankDist))
			{
...
FMegalithActionResult FMegalithMeshHorrorActions::AnalyzeEscapeRoutes(const TSharedPtr<FJsonObject>& Params)
{
	FVector Location;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("location"), Location))
	{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithEscape), true);
	...
	for (const FExitActor& Exit : Exits)
	{
		...
		if (!MegalithMeshAnalysis::FindNavPath(World, Location, ExitLoc, PathPts, PathDist))
		{
			continue;
		}
...
FMegalithActionResult FMegalithMeshHorrorActions::ClassifyZoneTension(const TSharedPtr<FJsonObject>& Params)
{
	FVector Location;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("location"), Location))
	{
	...
	{
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithTension), true);
		FVector Origin = Location + FVector(0, 0, 170.0f);
	...
	Inputs.RoomVolume = MegalithMeshAnalysis::ApproximateRoomVolume(World, Location, static_cast<float>(Radius) * 3.0f);
	Inputs.ExitCount = MegalithMeshAnalysis::CountExits(World, Location, static_cast<float>(Radius) * 4.0f);

	float TensionScore = MegalithMeshAnalysis::ComputeTensionScore(Inputs);
	MegalithMeshAnalysis::ETensionLevel Level = MegalithMeshAnalysis::ClassifyTension(TensionScore);

	auto Result = MakeShared<FJsonObject>();
	Result->SetNumberField(TEXT("tension_score"), FMath::RoundToInt(TensionScore));
	Result->SetStringField(TEXT("tension_level"), MegalithMeshAnalysis::TensionLevelToString(Level));
...
FMegalithActionResult FMegalithMeshHorrorActions::AnalyzePacingCurve(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FVector> PathPoints;
	if (!MHorror_ParseVectorArray(Params, TEXT("path_points"), PathPoints) || PathPoints.Num() < 2)
	{
	...
	for (int32 i = 0; i < SamplePoints.Num(); ++i)
	{
		...
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithPacing), true);
		...
		Inputs.CeilingHeight = MegalithMeshAnalysis::MeasureCeilingHeight(World, SamplePoints[i]);
		...
		float Score = MegalithMeshAnalysis::ComputeTensionScore(Inputs);
		auto Level = MegalithMeshAnalysis::ClassifyTension(Score);

		FPacingSample S;
		S.Location = SamplePoints[i];
		S.TensionScore = Score;
		S.TensionLevel = MegalithMeshAnalysis::TensionLevelToString(Level);
...
FMegalithActionResult FMegalithMeshHorrorActions::FindDeadEnds(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	if (!bHasRegion)
	{
		...
		if (!NavSys)
		{
			return FMegalithActionResult::Error(TEXT("Navigation system not available. Build navigation first."));
		}

		ANavigationData* NavData = NavSys->GetDefaultNavDataInstance(FNavigationSystem::DontCreate);
		if (!NavData)
		{
			return FMegalithActionResult::Error(TEXT("Navmesh not built."));
		}
		...
		{
			return FMegalithActionResult::Error(TEXT("Could not determine navmesh bounds. Specify region_min/region_max manually."));
		}
	...
	TArray<MegalithMeshAnalysis::FDeadEnd> DeadEnds = MegalithMeshAnalysis::FloodFillDeadEnds(World, Region, static_cast<float>(GridSize));
