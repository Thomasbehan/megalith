...
#if WITH_GEOMETRYSCRIPT
UMegalithMeshHandlePool* FMegalithMeshTechArtActions::Pool = nullptr;

void FMegalithMeshTechArtActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
#endif
...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshTechArtActions::VectorToJsonArray(const FVector& V)
{
...
void FMegalithMeshTechArtActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 16.1 import_mesh
	Registry.RegisterAction(TEXT("mesh"), TEXT("import_mesh"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::ImportMesh),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("export_mesh"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::ExportMesh),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("fix_mesh_quality"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::FixMeshQuality),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("auto_generate_lods"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::AutoGenerateLods),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the StaticMesh"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_texel_density"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::AnalyzeTexelDensity),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_material_cost_in_region"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::AnalyzeMaterialCostInRegion),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_mesh_collision"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::SetMeshCollision),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the StaticMesh"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_lightmap_density"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTechArtActions::AnalyzeLightmapDensity),
		FParamSchemaBuilder()
			.Optional(TEXT("actor_name"), TEXT("string"), TEXT("Single actor to analyze"))
...
FMegalithActionResult FMegalithMeshTechArtActions::ImportMesh(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("'files' array is required and must not be empty"));
	}

	FString Destination = Params->GetStringField(TEXT("destination"));
	if (Destination.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'destination' is required"));
	}
	...
	for (const auto& Val : *FilesArray)
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("File not found: %s"), *Path));
		}
		Filenames.Add(Path);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Import returned no objects. Check file paths and format."));
	}
...
FMegalithActionResult FMegalithMeshTechArtActions::ExportMesh(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	const FString FilePath = Params->GetStringField(TEXT("file_path"));
	if (AssetPath.IsEmpty() || FilePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'asset_path' and 'file_path' are required"));
	}
	if (!FilePath.EndsWith(TEXT(".fbx"), ESearchCase::IgnoreCase))
	{
		return FMegalithActionResult::Error(TEXT("'file_path' must end with .fbx"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("File already exists and replace_existing=false: %s"), *FilePath));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Asset is not a StaticMesh or SkeletalMesh: %s (%s)"),
	...
	if (!Exporter)
	{
		return FMegalithActionResult::Error(TEXT("No FBX exporter found for this asset class — ensure the FBX import/export plugin is enabled"));
	}
	...
	if (!bSucceeded)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Export failed for asset %s -> %s"), *AssetPath, *FilePath));
	}
...
FMegalithActionResult FMegalithMeshTechArtActions::FixMeshQuality(const TSharedPtr<FJsonObject>& Params)
{
#if !WITH_GEOMETRYSCRIPT
	return FMegalithActionResult::Error(TEXT("fix_mesh_quality requires the GeometryScripting plugin"));
#else
	if (!Pool)
	{
		return FMegalithActionResult::Error(TEXT("GeometryScript handle pool not available"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load mesh: %s"), *CreateError));
	}
	...
	if (!DynMesh)
	{
		...
		return FMegalithActionResult::Error(GetError);
	}
	...
	{
		Pool->ReleaseHandle(HandleName);
		return FMegalithActionResult::Error(FString::Printf(TEXT("Fixed mesh but failed to save: %s"), *SaveError));
	}
...
FMegalithActionResult FMegalithMeshTechArtActions::AutoGenerateLods(const TSharedPtr<FJsonObject>& Params)
{
#if !WITH_GEOMETRYSCRIPT
	return FMegalithActionResult::Error(TEXT("auto_generate_lods requires the GeometryScripting plugin"));
#else
	if (!Pool)
	{
		return FMegalithActionResult::Error(TEXT("GeometryScript handle pool not available"));
	...
	FString Error;
	UStaticMesh* SM = MegalithMeshUtils::LoadStaticMesh(AssetPath, Error);
	if (!SM)
	{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load source mesh: %s"), *CreateError));
	}
	...
	for (int32 Lod = 1; Lod <= LodCount; ++Lod)
	{
		...
		if (!Pool->CreateHandle(LodHandle, FString::Printf(TEXT("internal:lod:%s:%d"), *Lod0Handle, Lod), CreateError))
		{
			...
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create LOD%d handle: %s"), Lod, *CreateError));
		}
		...
		if (!LodMesh)
		{
			...
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to get LOD%d handle"), Lod));
		}
...
FMegalithActionResult FMegalithMeshTechArtActions::AnalyzeTexelDensity(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		FString Error;
		AActor* Actor = MegalithMeshUtils::FindActorByName(Params->GetStringField(TEXT("actor_name")), Error);
		if (!Actor)
		{
			return FMegalithActionResult::Error(Error);
		}
	...
	else if (Params->HasField(TEXT("region_min")) && Params->HasField(TEXT("region_max")))
	{
		...
		MegalithMeshUtils::ParseVector(Params, TEXT("region_min"), RegionMin);
	...
	{
		return FMegalithActionResult::Error(TEXT("Provide either 'actor_name' or 'region_min'+'region_max'"));
	}
...
FMegalithActionResult FMegalithMeshTechArtActions::AnalyzeMaterialCostInRegion(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Provide 'actors' array, 'center'+'radius', or 'region_min'+'region_max'"));
	}
...
FMegalithActionResult FMegalithMeshTechArtActions::SetMeshCollision(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("'collision_type' is required"));
	}
	...
	if (!BodySetup)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create or get BodySetup"));
	}
	...
	if (Params->HasField(TEXT("auto_convex")))
	{
		...
		if (!Pool)
		{
			return FMegalithActionResult::Error(TEXT("auto_convex requires GeometryScript handle pool"));
		}
...
#else
		return FMegalithActionResult::Error(TEXT("auto_convex requires the GeometryScripting plugin"));
#endif
	}
...
FMegalithActionResult FMegalithMeshTechArtActions::AnalyzeLightmapDensity(const TSharedPtr<FJsonObject>& Params)
{
	double TargetDensity = Params->HasField(TEXT("target_density"))
