...
FMegalithMeshContextPropActions::FScopedMeshTransaction::FScopedMeshTransaction(const FText& Description)
...
FMegalithMeshContextPropActions::FScopedMeshTransaction::~FScopedMeshTransaction()
{
	if (bOwnsTransaction && GEditor)
	{
...
void FMegalithMeshContextPropActions::FScopedMeshTransaction::Cancel()
{
	if (bOwnsTransaction && GEditor)
	{
...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshContextPropActions::VectorToJsonArray(const FVector& V)
{
...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshContextPropActions::RotatorToJsonArray(const FRotator& R)
{
	TArray<TSharedPtr<FJsonValue>> Arr;
...
bool FMegalithMeshContextPropActions::ParseJsonArrayToVector(const TArray<TSharedPtr<FJsonValue>>& Arr, FVector& Out)
{
...
FString FMegalithMeshContextPropActions::GetPropKitsDirectory()
{
...
TSharedPtr<FJsonObject> FMegalithMeshContextPropActions::LoadPropKit(const FString& KitName, FString& OutError)
{
...
bool FMegalithMeshContextPropActions::SavePropKit(const FString& KitName, const TSharedPtr<FJsonObject>& KitJson, FString& OutError)
{
	FString KitDir = GetPropKitsDirectory();
	FString FilePath = KitDir / KitName + TEXT(".json");
...
TArray<FVector2D> FMegalithMeshContextPropActions::PoissonDiskSample2D(float Width, float Height, float MinSpacing, int32 MaxCount, FRandomStream& RandStream)
{
...
TArray<AActor*> FMegalithMeshContextPropActions::FindActorsWithOwnerTag(UWorld* World, const FString& VolumeName)
{
...
void FMegalithMeshContextPropActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("scatter_on_surface"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::ScatterOnSurface),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_room_disturbance"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::SetRoomDisturbance),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("configure_physics_props"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::ConfigurePhysicsProps),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("settle_props"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::SettleProps),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_prop_kit"),
		TEXT("Author a prop kit JSON file: items with relative positions, rotation, scale, spawn chances. Saved to Saved/Megalith/PropKits/."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::CreatePropKit),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_prop_kit"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::PlacePropKit),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("scatter_on_walls"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::ScatterOnWalls),
		FParamSchemaBuilder()
			.Required(TEXT("volume_name"), TEXT("string"), TEXT("Name of the blockout volume"))
			.Required(TEXT("asset_paths"), TEXT("array"), TEXT("Array of StaticMesh asset paths to scatter"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("scatter_on_ceiling"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshContextPropActions::ScatterOnCeiling),
		FParamSchemaBuilder()
			.Required(TEXT("volume_name"), TEXT("string"), TEXT("Name of the blockout volume"))
			.Required(TEXT("asset_paths"), TEXT("array"), TEXT("Array of StaticMesh asset paths to scatter"))
...
FMegalithActionResult FMegalithMeshContextPropActions::ScatterOnSurface(const TSharedPtr<FJsonObject>& Params)
{
	FString SurfaceActorName;
	if (!Params->TryGetStringField(TEXT("surface_actor"), SurfaceActorName))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: surface_actor"));
	}
	...
	FString FindError;
	AActor* SurfaceActor = MegalithMeshUtils::FindActorByName(SurfaceActorName, FindError);
	if (!SurfaceActor)
	{
		return FMegalithActionResult::Error(FindError);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Surface actor is too small for prop scattering (< 1cm in X or Y)"));
	}
	...
	// Spawn actors
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Scatter On Surface")));

	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithSurfaceTrace), true);
	...
	for (const FVector2D& Sample : Samples)
	{
		...
		if (!CollisionMode.Equals(TEXT("none"), ESearchCase::IgnoreCase))
		{
			...
			MegalithMeshUtils::FPropPlacementResult PlacementResult = MegalithMeshUtils::ValidatePropPlacement(
		...
		PropActor->Tags.Add(FName(TEXT("Megalith.SurfaceProp")));
		PropActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.SurfaceOwner:%s"), *SurfaceActorName)));
...
FMegalithActionResult FMegalithMeshContextPropActions::SetRoomDisturbance(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: disturbance"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("No actors found with tag Megalith.Owner:%s"), *VolumeName));
	}
	...
	FRandomStream RandStream(Seed);
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Set Room Disturbance")));
	...
	FCollisionQueryParams FloorTraceParams(SCENE_QUERY_STAT(MegalithDisturbanceFloorTrace), true);
...
FMegalithActionResult FMegalithMeshContextPropActions::ConfigurePhysicsProps(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	if (Params->TryGetArrayField(TEXT("actor_names"), ActorNamesArr) && ActorNamesArr->Num() > 0)
	{
		...
		{
			FString FindError;
			AActor* Actor = MegalithMeshUtils::FindActorByName(Val->AsString(), FindError);
			if (!Actor)
			{
				return FMegalithActionResult::Error(FindError);
			}
	...
	{
		return FMegalithActionResult::Error(TEXT("Provide either actor_names or volume_name"));
	}
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Configure Physics Props")));
...
FMegalithActionResult FMegalithMeshContextPropActions::SettleProps(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	FRandomStream RandStream(Seed);
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Settle Props")));

	int32 Settled = 0;
	int32 FellThrough = 0;
	TArray<TSharedPtr<FJsonValue>> SettledArr;

	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithSettleTrace), true);
...
FMegalithActionResult FMegalithMeshContextPropActions::CreatePropKit(const TSharedPtr<FJsonObject>& Params)
{
	FString KitName;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: items"));
	}
	...
	for (int32 i = 0; i < ItemsArr->Num(); ++i)
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Item %d is not a valid JSON object"), i));
		}
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Item %d missing 'label'"), i));
		}
		if (!ItemObj->TryGetStringField(TEXT("asset_path"), AssetPath))
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Item %d ('%s') missing 'asset_path'"), i, *Label));
		}
		...
		if (!Mesh)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Item %d ('%s'): %s"), i, *Label, *MeshError));
		}
...
FMegalithActionResult FMegalithMeshContextPropActions::PlacePropKit(const TSharedPtr<FJsonObject>& Params)
{
	FString KitName;
	if (!Params->TryGetStringField(TEXT("kit_name"), KitName))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: kit_name"));
	}

	FVector Location;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("location"), Location))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: location"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Prop kit has no items"));
	}
	...
	FRandomStream RandStream(Seed);
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Place Prop Kit")));
	...
	for (const auto& ItemVal : *ItemsArr)
	{
		...
		PropActor->Tags.Add(FName(TEXT("Megalith.PropKit")));
		PropActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Kit:%s"), *KitName)));
...
FMegalithActionResult FMegalithMeshContextPropActions::ScatterOnWalls(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FString FindError;
	AActor* Volume = MegalithMeshUtils::FindActorByName(VolumeName, FindError);
	if (!Volume)
	{
		return FMegalithActionResult::Error(FindError);
	}
	...
	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithWallTrace), true);
	...
	if (WallHits.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No wall surfaces found via traces. Ensure the volume contains geometry with walls."));
	}
	...
	// Spawn
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Scatter On Walls")));
	...
	for (const FWallHit& WH : FilteredHits)
	{
		...
		PropActor->Tags.Add(FName(TEXT("Megalith.WallProp")));
...
FMegalithActionResult FMegalithMeshContextPropActions::ScatterOnCeiling(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithCeilingTrace), true);
	TraceParams.AddIgnoredActor(Volume);
	...
	{
		return FMegalithActionResult::Error(TEXT("No ceiling surfaces found via upward traces. Ensure the volume has geometry above."));
	}

	// Spawn
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Scatter On Ceiling")));
	...
	for (const FCeilingHit& CH : CeilingHits)
	{
		...
		PropActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Owner:%s"), *VolumeName)));
		PropActor->Tags.Add(FName(TEXT("Megalith.CeilingProp")));
