...
#include "MegalithMeshStorytellingPatterns.h"
#include "MegalithMeshAcoustics.h"
...
FString FMegalithMeshPresetActions::GetPatternsDirectory()
{
	FString Dir = FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("Megalith") / TEXT("Patterns");
	IFileManager::Get().MakeDirectory(*Dir, true);
	return Dir;
}

FString FMegalithMeshPresetActions::GetAcousticProfilesDirectory()
{
	FString Dir = FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("Megalith") / TEXT("AcousticProfiles");
	IFileManager::Get().MakeDirectory(*Dir, true);
	return Dir;
}

FString FMegalithMeshPresetActions::GetTensionProfilesDirectory()
{
	FString Dir = FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("Megalith") / TEXT("TensionProfiles");
	IFileManager::Get().MakeDirectory(*Dir, true);
	return Dir;
}

FString FMegalithMeshPresetActions::GetPresetsDirectory()
{
	FString Dir = FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("Megalith") / TEXT("Presets");
	IFileManager::Get().MakeDirectory(*Dir, true);
	return Dir;
}

FString FMegalithMeshPresetActions::GetTemplatesDirectory()
{
	FString Dir = FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("Megalith") / TEXT("Templates");
	IFileManager::Get().MakeDirectory(*Dir, true);
	return Dir;
}

FString FMegalithMeshPresetActions::GetPropKitsDirectory()
{
	FString Dir = FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("Megalith") / TEXT("PropKits");
	IFileManager::Get().MakeDirectory(*Dir, true);
	return Dir;
}
...
TSharedPtr<FJsonObject> FMegalithMeshPresetActions::LoadJsonFile(const FString& FilePath, FString& OutError)
{
	FString JsonStr;
...
bool FMegalithMeshPresetActions::SaveJsonFile(const FString& FilePath, const TSharedPtr<FJsonObject>& JsonObj, FString& OutError)
{
	FString JsonStr;
...
TArray<FString> FMegalithMeshPresetActions::ListJsonFiles(const FString& Directory)
{
...
TSharedPtr<FJsonObject> FMegalithMeshPresetActions::StorytellingElementToJson(const FStorytellingElement& Elem)
{
...
TSharedPtr<FJsonObject> FMegalithMeshPresetActions::StorytellingPatternToJson(const FStorytellingPattern& Pattern)
{
	auto Obj = MakeShared<FJsonObject>();
...
TSharedPtr<FJsonObject> FMegalithMeshPresetActions::AcousticPropsToJson(const FString& SurfaceName, float Absorption, float TransmissionLossdB, float FootstepLoudness)
{
	auto Obj = MakeShared<FJsonObject>();
...
bool FMegalithMeshPresetActions::ValidateStorytellingPatternJson(const TSharedPtr<FJsonObject>& Json, FString& OutError)
{
	FString Name;
...
bool FMegalithMeshPresetActions::ValidateAcousticProfileJson(const TSharedPtr<FJsonObject>& Json, FString& OutError)
{
	FString Name;
	if (!Json->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
...
bool FMegalithMeshPresetActions::ValidateTensionProfileJson(const TSharedPtr<FJsonObject>& Json, FString& OutError)
{
	FString Name;
	if (!Json->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
...
bool FMegalithMeshPresetActions::ValidateGenrePresetJson(const TSharedPtr<FJsonObject>& Json, FString& OutError)
{
	FString Name;
	if (!Json->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
...
void FMegalithMeshPresetActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 1. list_storytelling_patterns
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_storytelling_patterns"),
		TEXT("List all available storytelling patterns: built-in horror defaults + user-created patterns from Saved/Megalith/Patterns/. Returns name, description, element count, and source (built-in vs user)."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::ListStorytellingPatterns),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_storytelling_pattern"),
		TEXT("Author a new storytelling pattern JSON. Defines element types (decal/prop), radial distribution, size ranges, spawn counts, rotation/scale variance. Saved to Saved/Megalith/Patterns/."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::CreateStorytellingPattern),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_acoustic_profiles"),
		TEXT("List all acoustic profiles: built-in horror defaults (12 surfaces from MegalithMeshAcoustics) + user-created profiles from Saved/Megalith/AcousticProfiles/. Returns profile name, genre, surface count, and source."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::ListAcousticProfiles),
		FParamSchemaBuilder()
			.Optional(TEXT("source_filter"), TEXT("string"), TEXT("Filter by source: 'built-in', 'user', or omit for all"))
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_acoustic_profile"),
		TEXT("Author an acoustic property set for a genre. Each surface defines absorption (0-1), transmission_loss_db, and footstep_loudness. Saved to Saved/Megalith/AcousticProfiles/."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::CreateAcousticProfile),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_tension_profile"),
		TEXT("Define tension scoring weights for a genre. Override how sightline_length, ceiling_height, room_volume, exit_count, lighting_level, and audio_reverb contribute to the tension score. Saved to Saved/Megalith/TensionProfiles/."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::CreateTensionProfile),
		FParamSchemaBuilder()
			.Required(TEXT("name"), TEXT("string"), TEXT("Profile name (used as filename)"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_genre_presets"),
		TEXT("List all available genre preset packs from Saved/Megalith/Presets/. Shows name, genre, version, and content summary (pattern count, profile count, etc.)."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::ListGenrePresets),
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("export_genre_preset"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::ExportGenrePreset),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("import_genre_preset"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPresetActions::ImportGenrePreset),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshPresetActions::ListStorytellingPatterns(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Built-in patterns from MegalithMeshStorytellingPatterns.h
...
FMegalithActionResult FMegalithMeshPresetActions::CreateStorytellingPattern(const TSharedPtr<FJsonObject>& Params)
{
	FString Name;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: description"));
	}

	const TArray<TSharedPtr<FJsonValue>>* ElementsArr;
	if (!Params->TryGetArrayField(TEXT("elements"), ElementsArr) || ElementsArr->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: elements"));
	}
	...
	for (int32 i = 0; i < ElementsArr->Num(); ++i)
	{
		...
		if (!(*ElementsArr)[i]->TryGetObject(ElemObjPtr) || !ElemObjPtr || !(*ElemObjPtr).IsValid())
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Element %d is not a valid JSON object"), i));
		}
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Element %d missing required field: label"), i));
		}

		FString Type;
		if (!ElemObj->TryGetStringField(TEXT("type"), Type))
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Element %d missing required field: type"), i));
		}
...
FMegalithActionResult FMegalithMeshPresetActions::ListAcousticProfiles(const TSharedPtr<FJsonObject>& Params)
{
	FString SourceFilter;
	Params->TryGetStringField(TEXT("source_filter"), SourceFilter);
	...
	if (SourceFilter.IsEmpty() || SourceFilter.Equals(TEXT("built-in"), ESearchCase::IgnoreCase))
	{
		TMap<FString, MegalithMeshAcoustics::FAcousticProperties> Defaults = MegalithMeshAcoustics::GetHardcodedDefaults();
...
FMegalithActionResult FMegalithMeshPresetActions::CreateAcousticProfile(const TSharedPtr<FJsonObject>& Params)
{
	FString Name;
	if (!Params->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: surfaces"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("'horror_default' is a built-in profile name. Choose a different name."));
	}
	...
	for (int32 i = 0; i < SurfacesArr->Num(); ++i)
	{
		...
		if (!(*SurfacesArr)[i]->TryGetObject(SurfObjPtr) || !SurfObjPtr || !(*SurfObjPtr).IsValid())
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Surface %d is not a valid JSON object"), i));
		}
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Surface %d missing required field: surface_name"), i));
		}

		double Absorption = 0.02;
		if (!SurfObj->TryGetNumberField(TEXT("absorption"), Absorption))
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Surface '%s' missing required field: absorption"), *SurfName));
		}
		if (Absorption < 0.0 || Absorption > 1.0)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Surface '%s' absorption must be 0-1, got %.3f"), *SurfName, Absorption));
		}
...
FMegalithActionResult FMegalithMeshPresetActions::CreateTensionProfile(const TSharedPtr<FJsonObject>& Params)
{
	FString Name;
	if (!Params->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: factors"));
	}

	if ((*FactorsPtr)->Values.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("factors must contain at least one factor weight"));
	}
	...
	for (const auto& Pair : (*FactorsPtr)->Values)
	{
		if (!KnownFactors.Contains(MegalithKeyToString(Pair.Key)))
		{
			UnknownFactors.Add(MegalithKeyToString(Pair.Key));
		}
		...
		{
			return FMegalithActionResult::Error(FString::Printf(
				TEXT("Factor '%s' must be a JSON object with at least a 'weight' field"), *MegalithKeyToString(Pair.Key)));
		}

		double Weight = 0.0;
		if (!(*FactorObjPtr)->TryGetNumberField(TEXT("weight"), Weight))
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Factor '%s' missing required field: weight"), *Pair.Key));
		}
		if (Weight < 0.0 || Weight > 1.0)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Factor '%s' weight must be 0-1, got %.3f"), *Pair.Key, Weight));
		}
...
FMegalithActionResult FMegalithMeshPresetActions::ListGenrePresets(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithMeshPresetActions::ExportGenrePreset(const TSharedPtr<FJsonObject>& Params)
{
	FString Name;
	if (!Params->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
...
FMegalithActionResult FMegalithMeshPresetActions::ImportGenrePreset(const TSharedPtr<FJsonObject>& Params)
{
	FString PresetName;
	if (!Params->TryGetStringField(TEXT("preset_name"), PresetName) || PresetName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: preset_name"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid preset JSON: %s"), *ValidateError));
	}
