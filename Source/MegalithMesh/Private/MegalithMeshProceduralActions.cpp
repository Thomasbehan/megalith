...
#include "MegalithMeshProceduralCache.h"
...
UMegalithMeshHandlePool* FMegalithMeshProceduralActions::Pool = nullptr;

void FMegalithMeshProceduralActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
...
void FMegalithMeshProceduralActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_parametric_mesh"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreateParametricMesh),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_horror_prop"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreateHorrorProp),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_structure"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreateStructure),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_building_shell"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreateBuildingShell),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_maze"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreateMaze),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_pipe_network"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreatePipeNetwork),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_fragments"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreateFragments),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_terrain_patch"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::CreateTerrainPatch),
		FParamSchemaBuilder()
...
void FMegalithMeshProceduralActions::ParseDimensions(const TSharedPtr<FJsonObject>& Params,
...
TSharedPtr<FJsonObject> FMegalithMeshProceduralActions::ParseSubParams(const TSharedPtr<FJsonObject>& Params)
{
...
void FMegalithMeshProceduralActions::CleanupMesh(UDynamicMesh* Mesh, bool bHadBooleans)
{
	if (!Mesh) return;
...
bool FMegalithMeshProceduralActions::SaveMeshToAsset(UDynamicMesh* Mesh, const FString& SavePath, bool bOverwrite, FString& OutError)
{
	if (!Pool)
	{
		OutError = GS_ERROR;
...
AActor* FMegalithMeshProceduralActions::PlaceMeshInScene(const FString& AssetPath, const FVector& Location, const FRotator& Rotation, const FString& Label, bool bSnapToFloor /*= true*/, const FString& Folder /*= TEXT("")*/)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World) return nullptr;

	UStaticMesh* SM = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(AssetPath);
...
FMegalithActionResult FMegalithMeshProceduralActions::CreateParametricMesh(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
	}

	FString Type;
	if (!Params->TryGetStringField(TEXT("type"), Type))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: type"));
	}
	...
	if (!Mesh)
	{
		return FMegalithActionResult::Error(TEXT("Failed to allocate UDynamicMesh"));
	}
	...
	if (Type == TEXT("chair"))
	{
		...
		if (!BuildChair(Mesh, Width, Depth, Height, SeatH, BackH, LegT, BuildError))
			return FMegalithActionResult::Error(BuildError);
	}
	...
	if (!FinalErr.IsEmpty())
	{
		return FMegalithActionResult::Error(FinalErr);
	}
...
FMegalithActionResult FMegalithMeshProceduralActions::CreateHorrorProp(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
...
bool FMegalithMeshProceduralActions::BuildChair(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildTable(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildDesk(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildShelf(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildCabinet(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildBed(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildDoorFrame(UDynamicMesh* Mesh, float Width, float Height,
...
bool FMegalithMeshProceduralActions::BuildWindowFrame(UDynamicMesh* Mesh, float Width, float Height,
...
bool FMegalithMeshProceduralActions::BuildStairs(UDynamicMesh* Mesh, float Width, float StepHeight, float StepDepth,
...
bool FMegalithMeshProceduralActions::BuildRamp(UDynamicMesh* Mesh, float Width, float Depth, float Height,
	FString& OutError)
{
	FGeometryScriptPrimitiveOptions Opts;
...
bool FMegalithMeshProceduralActions::BuildPillar(UDynamicMesh* Mesh, float Radius, float Height,
...
bool FMegalithMeshProceduralActions::BuildCounter(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildToilet(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildSink(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildBathtub(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildBarricade(UDynamicMesh* Mesh, float Width, float Height, float Depth,
...
bool FMegalithMeshProceduralActions::BuildDebrisPile(UDynamicMesh* Mesh, float Radius, float Height,
...
bool FMegalithMeshProceduralActions::BuildCage(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildCoffin(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildGurney(UDynamicMesh* Mesh, float Width, float Depth, float Height,
...
bool FMegalithMeshProceduralActions::BuildBrokenWall(UDynamicMesh* Mesh, float Width, float Height, float Thickness,
...
bool FMegalithMeshProceduralActions::BuildVentGrate(UDynamicMesh* Mesh, float Width, float Height, float Depth,
...
bool FMegalithMeshProceduralActions::ParseVectorArray(const TSharedPtr<FJsonObject>& Params, const FString& Key, TArray<FVector>& Out)
{
...
bool FMegalithMeshProceduralActions::ParseVector2DArray(const TSharedPtr<FJsonObject>& Params, const FString& Key, TArray<FVector2D>& Out)
{
	const TArray<TSharedPtr<FJsonValue>>* Arr;
	if (!Params->TryGetArrayField(Key, Arr) || Arr->Num() == 0)
	{
...
TArray<FVector2D> FMegalithMeshProceduralActions::MakeCirclePolygon(float Radius, int32 Segments)
{
...
TArray<FVector2D> FMegalithMeshProceduralActions::InsetPolygon2D(const TArray<FVector2D>& Polygon, float InsetDist)
{
...
FString FMegalithMeshProceduralActions::FinalizeProceduralMesh(UDynamicMesh* Mesh, const TSharedPtr<FJsonObject>& Params,
	...
	const FString& ActionName, const FString& Category)
{
	...
	if (bUseCache)
	{
		Hash = FMegalithMeshProceduralCache::Get().ComputeHash(ActionName, Params);
		FString CachedPath;
		if (FMegalithMeshProceduralCache::Get().TryGetCached(Hash, CachedPath))
		{
			...
			if (bPlace)
			{
				...
				MegalithMeshUtils::ParseVector(Params, TEXT("location"), Location);
	...
	if (SavePath.IsEmpty() && bAutoSave && !Category.IsEmpty())
	{
		...
		FString HashForPath = bUseCache ? Hash : FMegalithMeshProceduralCache::Get().ComputeHash(ActionName, Params);
		SavePath = FMegalithMeshProceduralCache::Get().GenerateAutoPath(Category, Type, Width, Depth, Height, HashForPath, Seed);
	}
	...
	if (!SavePath.IsEmpty())
	{
		...
		if (bUseCache && !Hash.IsEmpty())
		{
			...
			FMegalithMeshProceduralCache::Get().Register(Hash, SavePath, ActionName, Type, TriCount, Params);
			Result->SetStringField(TEXT("hash"), Hash);
		}
...
FMegalithActionResult FMegalithMeshProceduralActions::CreateStructure(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
...
FMegalithActionResult FMegalithMeshProceduralActions::CreateBuildingShell(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
	...
	{
		return FMegalithActionResult::Error(TEXT("'footprint' must be an array of at least 3 [x, y] points (CCW winding)"));
	}
...
TArray<FMegalithMeshProceduralActions::FMazeWall> FMegalithMeshProceduralActions::GenerateMaze_RecursiveBacktracker(int32 GridW, int32 GridH, int32 Seed)
{
	FRandomStream Rng(Seed);
...
TArray<FMegalithMeshProceduralActions::FMazeWall> FMegalithMeshProceduralActions::GenerateMaze_Prims(int32 GridW, int32 GridH, int32 Seed)
{
	FRandomStream Rng(Seed);
	int32 CellCount = GridW * GridH;
...
TArray<FMegalithMeshProceduralActions::FMazeWall> FMegalithMeshProceduralActions::GenerateMaze_BinaryTree(int32 GridW, int32 GridH, int32 Seed)
{
	FRandomStream Rng(Seed);
...
FMegalithActionResult FMegalithMeshProceduralActions::CreateMaze(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
...
FMegalithActionResult FMegalithMeshProceduralActions::CreatePipeNetwork(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
	...
	{
		return FMegalithActionResult::Error(TEXT("'path_points' must be an array of at least 2 [x, y, z] points"));
	}
...
FMegalithActionResult FMegalithMeshProceduralActions::CreateFragments(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: source_handle"));
	}
	...
	if (!SourceMesh)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Handle '%s' not found: %s"), *SourceHandle, *HandleErr));
	}
	...
	if (!WorkMesh)
	{
		return FMegalithActionResult::Error(TEXT("Failed to allocate working mesh"));
	}
...
FMegalithActionResult FMegalithMeshProceduralActions::CreateTerrainPatch(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR);
...
void FMegalithMeshProceduralActions::RegisterCacheActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_cached_meshes"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::ListCachedMeshes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("clear_cache"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::ClearCacheAction),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("validate_cache"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::ValidateCacheAction),
		FParamSchemaBuilder()
			.Build());

	Registry.RegisterAction(TEXT("mesh"), TEXT("get_cache_stats"),
		TEXT("Get cache statistics: entry counts by type, total entries."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshProceduralActions::GetCacheStatsAction),
		FParamSchemaBuilder()
			.Build());
}

FMegalithActionResult FMegalithMeshProceduralActions::ListCachedMeshes(const TSharedPtr<FJsonObject>& Params)
{
	FString TypeFilter;
	...
	TSharedPtr<FJsonObject> Result = FMegalithMeshProceduralCache::Get().ListEntries(TypeFilter, Limit);
	return FMegalithActionResult::Success(Result);
}

FMegalithActionResult FMegalithMeshProceduralActions::ClearCacheAction(const TSharedPtr<FJsonObject>& Params)
{
	FString TypeFilter;
	Params->TryGetStringField(TEXT("type"), TypeFilter);

	int32 Removed = FMegalithMeshProceduralCache::Get().ClearCache(TypeFilter);
...
FMegalithActionResult FMegalithMeshProceduralActions::ValidateCacheAction(const TSharedPtr<FJsonObject>& Params)
{
	int32 Removed = FMegalithMeshProceduralCache::Get().ValidateCache();
...
FMegalithActionResult FMegalithMeshProceduralActions::GetCacheStatsAction(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Result = FMegalithMeshProceduralCache::Get().GetStats();
	return FMegalithActionResult::Success(Result);
}
