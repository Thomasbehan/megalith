...
UMegalithMeshHandlePool* FMegalithMeshArchFeatureActions::Pool = nullptr;

void FMegalithMeshArchFeatureActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
...
float FMegalithMeshArchFeatureActions::GetFloat(const TSharedPtr<FJsonObject>& P, const FString& Key, float Default)
{
	return P->HasField(Key) ? static_cast<float>(P->GetNumberField(Key)) : Default;
}

int32 FMegalithMeshArchFeatureActions::GetInt(const TSharedPtr<FJsonObject>& P, const FString& Key, int32 Default)
{
	return P->HasField(Key) ? static_cast<int32>(P->GetNumberField(Key)) : Default;
}

FString FMegalithMeshArchFeatureActions::GetString(const TSharedPtr<FJsonObject>& P, const FString& Key, const FString& Default)
{
	FString Val;
	return P->TryGetStringField(Key, Val) ? Val : Default;
}

bool FMegalithMeshArchFeatureActions::GetBool(const TSharedPtr<FJsonObject>& P, const FString& Key, bool Default)
{
...
FAttachmentContext FMegalithMeshArchFeatureActions::ParseBuildingContext(const TSharedPtr<FJsonObject>& Params)
{
...
FRotator FMegalithMeshArchFeatureActions::ComputeWallRotation(const FVector& WallNormal)
{
...
FVector FMegalithMeshArchFeatureActions::ComputeAttachmentPosition(
...
void FMegalithMeshArchFeatureActions::EmitWallOpenings(
...
void FMegalithMeshArchFeatureActions::FinalizeGeometry(UDynamicMesh* Mesh)
{
	if (!Mesh) return;
	...
	FMegalithMeshProceduralActions::CleanupMesh(Mesh, /*bHadBooleans=*/false);
}

FString FMegalithMeshArchFeatureActions::SaveAndPlace(UDynamicMesh* Mesh, const TSharedPtr<FJsonObject>& Params,
	...
	const FAttachmentContext& AttachCtx, float FeatureWidth, float FeatureDepth)
{
	...
	AActor* Actor = FMegalithMeshProceduralActions::PlaceMeshInScene(
...
void FMegalithMeshArchFeatureActions::BuildRailingGeometry(UDynamicMesh* Mesh, const TArray<FVector>& Path,
...
void FMegalithMeshArchFeatureActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_balcony"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshArchFeatureActions::CreateBalcony),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_porch"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshArchFeatureActions::CreatePorch),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_fire_escape"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshArchFeatureActions::CreateFireEscape),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_ramp_connector"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshArchFeatureActions::CreateRampConnector),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_railing"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshArchFeatureActions::CreateRailing),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshArchFeatureActions::CreateBalcony(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool) return FMegalithActionResult::Error(GS_ERROR_ARCH);
	...
	if (!Params->HasField(TEXT("width")))
		return FMegalithActionResult::Error(TEXT("Missing required param: width"));
	if (!Params->HasField(TEXT("depth")))
		return FMegalithActionResult::Error(TEXT("Missing required param: depth"));
	...
	if (Width <= 0.0f || Depth <= 0.0f)
		return FMegalithActionResult::Error(TEXT("width and depth must be positive"));
	...
	if (RailStyle != TEXT("simple") && RailStyle != TEXT("bars") && RailStyle != TEXT("solid"))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid railing_style '%s'. Valid: simple, bars, solid"), *RailStyle));

	// Create mesh
	UDynamicMesh* Mesh = NewObject<UDynamicMesh>(Pool);
	if (!Mesh) return FMegalithActionResult::Error(TEXT("Failed to allocate UDynamicMesh"));
...
FMegalithActionResult FMegalithMeshArchFeatureActions::CreatePorch(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool) return FMegalithActionResult::Error(GS_ERROR_ARCH);
...
FMegalithActionResult FMegalithMeshArchFeatureActions::CreateFireEscape(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool) return FMegalithActionResult::Error(GS_ERROR_ARCH);
	...
	if (!Params->HasField(TEXT("floor_count")))
		return FMegalithActionResult::Error(TEXT("Missing required param: floor_count"));
	if (!Params->HasField(TEXT("floor_height")))
		return FMegalithActionResult::Error(TEXT("Missing required param: floor_height"));
	...
	if (FloorCount < 1) return FMegalithActionResult::Error(TEXT("floor_count must be >= 1"));
	if (FloorHeight <= 0.0f) return FMegalithActionResult::Error(TEXT("floor_height must be positive"));
...
FMegalithActionResult FMegalithMeshArchFeatureActions::CreateRampConnector(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool) return FMegalithActionResult::Error(GS_ERROR_ARCH);
	...
	if (!Params->HasField(TEXT("rise")))
		return FMegalithActionResult::Error(TEXT("Missing required param: rise"));

	const float Rise = static_cast<float>(Params->GetNumberField(TEXT("rise")));
	if (Rise <= 0.0f) return FMegalithActionResult::Error(TEXT("rise must be positive"));
	...
	if (SlopeRatio <= 0.0f || SlopeRatio > 1.0f)
		return FMegalithActionResult::Error(TEXT("slope_ratio must be between 0 and 1 (exclusive)"));
...
FMegalithActionResult FMegalithMeshArchFeatureActions::CreateRailing(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool) return FMegalithActionResult::Error(GS_ERROR_ARCH);
	...
	if (!Params->TryGetArrayField(TEXT("points"), PointsArr) || !PointsArr || PointsArr->Num() < 2)
		return FMegalithActionResult::Error(TEXT("Missing or invalid 'points' array (need at least 2 points as [x,y,z])"));
	...
	if (Style != TEXT("simple") && Style != TEXT("bars") && Style != TEXT("solid"))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid style '%s'. Valid: simple, bars, solid"), *Style));
