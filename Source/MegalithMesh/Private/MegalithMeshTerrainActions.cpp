...
#include "MegalithMeshBuildingTypes.h"
...
UMegalithMeshHandlePool* FMegalithMeshTerrainActions::Pool = nullptr;

void FMegalithMeshTerrainActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
...
FString FMegalithMeshTerrainActions::StrategyToString(EFoundationStrategy S)
{
	switch (S)
	{
...
FMegalithMeshTerrainActions::EFoundationStrategy FMegalithMeshTerrainActions::StringToStrategy(const FString& S)
{
...
void FMegalithMeshTerrainActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// ---- sample_terrain_grid ----
	Registry.RegisterAction(TEXT("mesh"), TEXT("sample_terrain_grid"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTerrainActions::SampleTerrainGrid),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_building_site"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTerrainActions::AnalyzeBuildingSite),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_foundation"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTerrainActions::CreateFoundation),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_retaining_wall"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTerrainActions::CreateRetainingWall),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_building_on_terrain"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTerrainActions::PlaceBuildingOnTerrain),
		FParamSchemaBuilder()
...
FBox2D FMegalithMeshTerrainActions::ComputePolygonBounds(const TArray<FVector2D>& Polygon)
{
...
bool FMegalithMeshTerrainActions::IsPointInPolygon(const FVector2D& Point, const TArray<FVector2D>& Polygon)
{
	int32 N = Polygon.Num();
	if (N < 3) return false;
...
float FMegalithMeshTerrainActions::InterpolateTerrainZ(const FTerrainSample& Terrain, float WorldX, float WorldY)
{
...
bool FMegalithMeshTerrainActions::ParsePolygon(const TSharedPtr<FJsonObject>& Params, const FString& Key,
...
bool FMegalithMeshTerrainActions::SampleTerrain(UWorld* World, const FVector& Center, const FVector2D& Size,
	...
	ECollisionChannel Channel, FTerrainSample& OutSample, FString& OutError)
{
	...
	FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithTerrainSample), true);
...
TSharedPtr<FJsonObject> FMegalithMeshTerrainActions::TerrainSampleToJson(const FTerrainSample& Sample)
{
...
bool FMegalithMeshTerrainActions::ParseTerrainSample(const TSharedPtr<FJsonObject>& Json,
	FTerrainSample& OutSample, FString& OutError)
{
	...
	// Center
	MegalithMeshUtils::ParseVector(Json, TEXT("center"), OutSample.Center);
	...
	// Average normal
	MegalithMeshUtils::ParseVector(Json, TEXT("avg_normal"), OutSample.AvgNormal);

	// Slope direction
	MegalithMeshUtils::ParseVector(Json, TEXT("slope_direction"), OutSample.SlopeDirection);
...
FMegalithMeshTerrainActions::FSiteAnalysis FMegalithMeshTerrainActions::AnalyzeSite(
...
bool FMegalithMeshTerrainActions::BuildFlatPad(UDynamicMesh* Mesh, const TArray<FVector2D>& Footprint,
...
bool FMegalithMeshTerrainActions::BuildCutAndFill(UDynamicMesh* Mesh, const TArray<FVector2D>& Footprint,
...
bool FMegalithMeshTerrainActions::BuildStepped(UDynamicMesh* Mesh, const TArray<FVector2D>& Footprint,
...
bool FMegalithMeshTerrainActions::BuildPiers(UDynamicMesh* Mesh, const TArray<FVector2D>& Footprint,
...
bool FMegalithMeshTerrainActions::BuildWalkoutBasement(UDynamicMesh* Mesh, const TArray<FVector2D>& Footprint,
...
bool FMegalithMeshTerrainActions::BuildADARamp(UDynamicMesh* Mesh, float Rise, float Width,
...
bool FMegalithMeshTerrainActions::BuildRetainingWallGeometry(UDynamicMesh* Mesh,
...
FMegalithActionResult FMegalithMeshTerrainActions::SampleTerrainGrid(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
		return FMegalithActionResult::Error(TEXT("No editor world"));
	}

	// Parse center
	FVector Center;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("center"), Center))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid 'center' array [x, y, z]"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid 'size' array [width, height]"));
	}
	FVector2D Size(
	...
	{
		return FMegalithActionResult::Error(SampleError);
	}
...
FMegalithActionResult FMegalithMeshTerrainActions::AnalyzeBuildingSite(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(PolyError);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing 'terrain_samples' object"));
	}
	FTerrainSample Terrain;
	FString TerrainError;
	if (!ParseTerrainSample(*TerrainObj, Terrain, TerrainError))
	{
		return FMegalithActionResult::Error(TerrainError);
	}
...
FMegalithActionResult FMegalithMeshTerrainActions::CreateFoundation(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR_TERRAIN);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: strategy"));
	}
	...
	// Cleanup normals
	FMegalithMeshProceduralActions::CleanupMesh(Mesh, false);

	// Save to asset
	bool bOverwrite = Params->HasField(TEXT("overwrite")) ? Params->GetBoolField(TEXT("overwrite")) : false;
	FString SaveErr;
	if (!FMegalithMeshProceduralActions::SaveMeshToAsset(Mesh, SavePath, bOverwrite, SaveErr))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Foundation built but save failed: %s"), *SaveErr));
	}

	// Optionally place in scene
	AActor* PlacedActor = nullptr;
	FVector Location = FVector::ZeroVector;
	bool bHasLocation = MegalithMeshUtils::ParseVector(Params, TEXT("location"), Location);
	...
	{
		PlacedActor = FMegalithMeshProceduralActions::PlaceMeshInScene(
...
FMegalithActionResult FMegalithMeshTerrainActions::CreateRetainingWall(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR_TERRAIN);
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid 'start' [x, y, z]"));
	}
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("end"), End))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid 'end' [x, y, z]"));
	}
	...
	// Save
	FString SaveErr;
	if (!FMegalithMeshProceduralActions::SaveMeshToAsset(Mesh, SavePath, false, SaveErr))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Retaining wall built but save failed: %s"), *SaveErr));
	}
...
FMegalithActionResult FMegalithMeshTerrainActions::PlaceBuildingOnTerrain(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR_TERRAIN);
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing 'building_descriptor' object"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: save_path_prefix"));
	}
		...
		PrecomputedTerrain && (*PrecomputedTerrain).IsValid())
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid terrain_samples: %s"), *TerrainError));
		}
	}
	else
	{
		...
		if (!SampleTerrain(World,
			...
			Terrain, SampleError))
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Terrain sampling failed: %s"), *SampleError));
		}
	}
	...
	if (!FoundationMesh)
	{
		return FMegalithActionResult::Error(TEXT("Failed to allocate UDynamicMesh for foundation"));
	}
	...
	FMegalithMeshProceduralActions::CleanupMesh(FoundationMesh, false);

	FString SaveErr;
	if (!FMegalithMeshProceduralActions::SaveMeshToAsset(FoundationMesh, FoundationPath, true, SaveErr))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Foundation save failed: %s"), *SaveErr));
	}

	AActor* FoundationActor = FMegalithMeshProceduralActions::PlaceMeshInScene(
		...
		for (const FPolyEdge& Edge : PolyEdges)
		{
			if (Edge.DotUphill > 0.1f)
			{
				...
				if (WallMesh)
				{
					...
					{
						FMegalithMeshProceduralActions::CleanupMesh(WallMesh, false);
						FString WallSaveErr;
						if (FMegalithMeshProceduralActions::SaveMeshToAsset(WallMesh, WallPath, true, WallSaveErr))
						{
							AActor* WallActor = FMegalithMeshProceduralActions::PlaceMeshInScene(
	...
	if (bHospice && Analysis.bNeedsRamp)
	{
		...
		if (RampMesh)
		{
			...
			{
				FMegalithMeshProceduralActions::CleanupMesh(RampMesh, false);
				FString RampSaveErr;
				if (FMegalithMeshProceduralActions::SaveMeshToAsset(RampMesh, RampPath, true, RampSaveErr))
				{
					RampActor = FMegalithMeshProceduralActions::PlaceMeshInScene(
	...
	{
		FString FindErr;
		AActor* Actor = MegalithMeshUtils::FindActorByName(ActorName, FindErr);
		if (Actor)
		{
