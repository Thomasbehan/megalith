...
void UMegalithMeshHandlePool::Initialize() {}
void UMegalithMeshHandlePool::Teardown() {}
bool UMegalithMeshHandlePool::CreateHandle(const FString&, const FString&, FString& OutError) { OutError = TEXT("GeometryScript not available"); return false; }
UDynamicMesh* UMegalithMeshHandlePool::GetHandle(const FString&, FString& OutError) { OutError = TEXT("GeometryScript not available"); return nullptr; }
bool UMegalithMeshHandlePool::ReleaseHandle(const FString&) { return false; }
bool UMegalithMeshHandlePool::SaveHandle(const FString&, const FString&, bool, FString& OutError, const FString&, int32) { OutError = TEXT("GeometryScript not available"); return false; }
TSharedPtr<FJsonObject> UMegalithMeshHandlePool::ListHandles() const { return MakeShared<FJsonObject>(); }
...
void UMegalithMeshHandlePool::Initialize()
{
	EvictionTimerHandle = FTSTicker::GetCoreTicker().AddTicker(
		TEXT("MegalithMeshHandlePool_Eviction"),
...
void UMegalithMeshHandlePool::TouchHandle(const FString& HandleName)
{
	LastAccessTime.FindOrAdd(HandleName) = FPlatformTime::Seconds();
}

bool UMegalithMeshHandlePool::CreateHandle(const FString& HandleName, const FString& Source, FString& OutError)
{
...
UDynamicMesh* UMegalithMeshHandlePool::GetHandle(const FString& HandleName, FString& OutError)
{
...
bool UMegalithMeshHandlePool::ReleaseHandle(const FString& HandleName)
{
	if (!Handles.Contains(HandleName))
	{
		return false;
...
bool UMegalithMeshHandlePool::SaveHandle(const FString& HandleName, const FString& TargetPath, bool bOverwrite, FString& OutError,
	const FString& CollisionMode, int32 MaxHulls)
{
	...
	if (ExistingPackage)
	{
		...
		if (ExistingPackage)
		{
			FString TrashedName = FString::Printf(TEXT("/Temp/__megalith_evicted_%s_%s"),
...
TSharedPtr<FJsonObject> UMegalithMeshHandlePool::ListHandles() const
{
...
bool UMegalithMeshHandlePool::OnEvictionCheck(float /*DeltaTime*/)
{
	...
	{
		UE_LOG(LogMegalith, Log, TEXT("Megalith — Evicting idle mesh handle '%s'"), *HandleName);
...
UDynamicMesh* UMegalithMeshHandlePool::CreateFromPrimitive(const FString& PrimitiveType, FString& OutError)
{
...
UDynamicMesh* UMegalithMeshHandlePool::CreateFromAsset(const FString& AssetPath, FString& OutError)
{
