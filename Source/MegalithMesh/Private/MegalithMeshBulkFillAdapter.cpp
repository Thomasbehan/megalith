...
// MegalithMeshBulkFillAdapter — Phase 5 Step 5 adapter.
//
...
//
// `megalith_reindex` silent-prerequisite annotation: the describe tree surfaces
...
#define LOCTEXT_NAMESPACE "MegalithMeshBulkFillAdapter"

namespace MegalithMeshBulkFillInternal
{
	...
	static FDryRunReport HandleSurfaceDataTable(const FBulkFillSpec& Spec)
	{
		...
		if (!Spec.bDryRun)
		{
			...
			Transaction = MakeShared<FScopedTransaction>(
				LOCTEXT("MeshBulkFill_DT", "Megalith Mesh Bulk Fill — Surface DT"));
			DT->Modify();
		}
		...
		for (const auto& RowKV : (*RowsObj)->Values)
		{
			...
			WriteSurfaceRow(DT, RowStruct, MegalithKeyToString(RowKV.Key), RowObj, Spec, Report);
		}
	...
	static FSchemaDescriptor BuildTopLevelDescribe()
	{
		...
		// megalith_reindex silent-prerequisite annotation.
		FSchemaDescriptor Reindex;
		Reindex.FieldPath = TEXT("(prerequisite: megalith_reindex)");
		...
		Reindex.ImportTextForm = TEXT(
			...
			"Adapter callers SHOULD invoke `megalith_reindex` after structural mesh asset changes "
...
FDryRunReport FMegalithMeshBulkFillAdapter::MeshBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithMeshBulkFillInternal;
...
FSchemaDescriptor FMegalithMeshBulkFillAdapter::MeshDescribe(const FString& TargetAsset)
{
	using namespace MegalithMeshBulkFillInternal;
...
void FMegalithMeshBulkFillAdapter::Register()
{
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("mesh"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithMeshBulkFillAdapter::MeshBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithMeshBulkFillAdapter::MeshDescribe));
}

void FMegalithMeshBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("mesh"));
}
