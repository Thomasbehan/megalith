...
TArray<TSharedPtr<FJsonValue>> MegalithMeshAnalysis::VectorToJsonArray(const FVector& V)
{
...
float MegalithMeshAnalysis::ComputeConcealment(UWorld* World, const FVector& TestPoint, const TArray<FVector>& Viewpoints)
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithConcealment), true);
...
TArray<MegalithMeshAnalysis::FPathClearance> MegalithMeshAnalysis::MeasurePathClearance(
	UWorld* World, const TArray<FVector>& PathPoints, float MaxWidth)
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithClearance), true);
...
const TCHAR* MegalithMeshAnalysis::TensionLevelToString(ETensionLevel Level)
{
	switch (Level)
	{
...
float MegalithMeshAnalysis::ComputeTensionScore(const FTensionInputs& Inputs)
{
...
MegalithMeshAnalysis::ETensionLevel MegalithMeshAnalysis::ClassifyTension(float Score)
{
...
float MegalithMeshAnalysis::MeasureCeilingHeight(UWorld* World, const FVector& Location, float MaxHeight)
{
	if (!World)
	{
		return 0.0f;
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithCeiling), true);
	FHitResult Hit;
...
float MegalithMeshAnalysis::ApproximateRoomVolume(UWorld* World, const FVector& Location, float MaxRadius, int32 RayCount)
{
	if (!World || RayCount < 4)
	{
		return 0.0f;
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithVolume), true);
...
int32 MegalithMeshAnalysis::CountExits(UWorld* World, const FVector& Location, float TestRadius, int32 Directions)
{
...
TArray<MegalithMeshAnalysis::FDeadEnd> MegalithMeshAnalysis::FloodFillDeadEnds(UWorld* World, const FBox& Region, float GridSize)
{
...
bool MegalithMeshAnalysis::GetNavSystem(UWorld* World, UNavigationSystemV1*& OutNavSys, ANavigationData*& OutNavData, FString& OutError)
{
	if (!World)
	{
...
bool MegalithMeshAnalysis::FindNavPath(UWorld* World, const FVector& Start, const FVector& End,
