...
// Split into separate .cpp but uses same class: FMegalithMeshTemplateActions
// We register the 4 validation actions from the template actions' RegisterActions
// by calling this helper from MegalithMeshTemplateActions.cpp.
...
FMegalithActionResult FMegalithMeshTemplateActions::ValidateGameReady(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
...
FMegalithActionResult FMegalithMeshTemplateActions::SuggestLodStrategy(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	if (!Params->TryGetStringField(TEXT("asset_path"), AssetPath) || AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithMeshTemplateActions::BatchValidate(const TSharedPtr<FJsonObject>& Params)
{
	...
	for (const FString& AssetPath : FlaggedPaths)
	{
		...
		FString LoadError;
		UStaticMesh* SM = MegalithMeshUtils::LoadStaticMesh(AssetPath, LoadError);
		if (SM)
		{
			FStaticMeshRenderData* RenderData = SM->GetRenderData();
			if (RenderData && RenderData->LODResources.Num() > 0)
	...
	if (!DB)
	{
		Result->SetStringField(TEXT("warning"), TEXT("Mesh catalog not available — run megalith_reindex() first for SQL pre-filtering. Falling back to slower asset-load validation."));
	}
...
FMegalithActionResult FMegalithMeshTemplateActions::CompareLodChain(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	if (!Params->TryGetStringField(TEXT("asset_path"), AssetPath) || AssetPath.IsEmpty())
	{
