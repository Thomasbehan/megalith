...
FMegalithMeshDecalActions::FScopedMeshTransaction::FScopedMeshTransaction(const FText& Description)
...
FMegalithMeshDecalActions::FScopedMeshTransaction::~FScopedMeshTransaction()
{
	if (bOwnsTransaction && GEditor)
	{
...
void FMegalithMeshDecalActions::FScopedMeshTransaction::Cancel()
{
	if (bOwnsTransaction && GEditor)
	{
...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshDecalActions::VectorToJsonArray(const FVector& V)
{
...
bool FMegalithMeshDecalActions::ValidateDecalMaterial(const FString& MaterialPath, FString& OutError)
{
	UMaterialInterface* MatInterface = FMegalithAssetUtils::LoadAssetByPath<UMaterialInterface>(MaterialPath);
	if (!MatInterface)
	{
...
ADecalActor* FMegalithMeshDecalActions::SpawnAlignedDecal(
	UWorld* World,
...
FVector FMegalithMeshDecalActions::CatmullRom(const FVector& P0, const FVector& P1, const FVector& P2, const FVector& P3, float t)
{
...
TArray<FVector> FMegalithMeshDecalActions::SampleSplinePath(const TArray<FVector>& PathPoints, float Spacing)
{
...
void FMegalithMeshDecalActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_decals"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDecalActions::PlaceDecals),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_along_path"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDecalActions::PlaceAlongPath),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_prop_density"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDecalActions::AnalyzePropDensity),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_storytelling_scene"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDecalActions::PlaceStorytellingScene),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshDecalActions::PlaceDecals(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(MatError);
	}
	...
	if (!Material)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load material: %s"), *MaterialPath));
	}
	...
	if (Params->TryGetArrayField(TEXT("locations"), LocationsArr) && LocationsArr->Num() > 0)
	{
		...
		if (Locations.Num() == 0)
		{
			return FMegalithActionResult::Error(TEXT("locations array provided but no valid [x,y,z] entries found"));
		}
	}
	else
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Must provide either 'locations' (array of [x,y,z]) or 'region' ({center, extent}) with 'count'"));
		}
		...
		if (!MegalithMeshUtils::ParseVector(*RegionObj, TEXT("center"), Center) ||
			!MegalithMeshUtils::ParseVector(*RegionObj, TEXT("extent"), Extent))
		{
			return FMegalithActionResult::Error(TEXT("region must have 'center' and 'extent' as [x,y,z] arrays"));
		}
	...
	// Spawn decals
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Place Decals")));

	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithDecalTrace), true);
	...
	for (const FVector& Loc : Locations)
	{
		...
		Decal->Tags.Add(FName(TEXT("Megalith.Decal")));
...
FMegalithActionResult FMegalithMeshDecalActions::PlaceAlongPath(const TSharedPtr<FJsonObject>& Params)
{
	// Parse path points
	const TArray<TSharedPtr<FJsonValue>>* PathArr;
	if (!Params->TryGetArrayField(TEXT("path_points"), PathArr) || PathArr->Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("Missing or insufficient path_points (need at least 2 waypoints as [x,y,z] arrays)"));
	}
	...
	if (PathPoints.Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("Need at least 2 valid [x,y,z] path points"));
	}

	FString AssetPath;
	if (!Params->TryGetStringField(TEXT("asset_or_decal"), AssetPath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: asset_or_decal"));
	}
	...
	UMaterialInterface* MatTest = FMegalithAssetUtils::LoadAssetByPath<UMaterialInterface>(AssetPath);
	if (MatTest)
	{
	...
	else
	{
		// Try as static mesh
		FString MeshError;
		PropMesh = MegalithMeshUtils::LoadStaticMesh(AssetPath, MeshError);
		if (!PropMesh)
		{
			return FMegalithActionResult::Error(FString::Printf(
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Place Along Path")));

	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithPathTrace), true);
	...
	for (int32 i = 0; i < SampledPoints.Num(); ++i)
	{
		...
		if (bIsDecal)
		{
			...
			Decal->Tags.Add(FName(TEXT("Megalith.PathDecal")));
			if (!Pattern.IsEmpty())
			{
				Decal->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Pattern:%s"), *Pattern)));
			}
			Decal->SetFolderPath(FName(*Folder));
		...
		else
		{
			...
			PropActor->Tags.Add(FName(TEXT("Megalith.PathProp")));
			if (!Pattern.IsEmpty())
			{
				PropActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Pattern:%s"), *Pattern)));
			}
...
FMegalithActionResult FMegalithMeshDecalActions::AnalyzePropDensity(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	...
	if (!Volume)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Volume not found: %s"), *VolumeName));
	}
	...
	// Count actors in each cell (exclude the volume itself and Megalith infrastructure)
	int32 TotalActors = 0;
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		AActor* Actor = *It;
...
FMegalithActionResult FMegalithMeshDecalActions::PlaceStorytellingScene(const TSharedPtr<FJsonObject>& Params)
{
	FVector Location;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: pattern"));
	}
	...
	MegalithMeshUtils::ParseVector(Params, TEXT("direction"), Direction);
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Place Storytelling Scene")));

	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithStorytellingTrace), true);
	...
	for (const FStorytellingElement& Elem : Pattern->Elements)
	{
		...
		for (int32 i = 0; i < Count; ++i)
		{
			...
			DecalActor->Tags.Add(FName(TEXT("Megalith.StorytellingDecal")));
			DecalActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Pattern:%s"), *PatternName)));
			DecalActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Element:%s"), *Elem.Label)));
