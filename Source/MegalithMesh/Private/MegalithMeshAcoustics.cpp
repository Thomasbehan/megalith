...
namespace
{
	...
	{
		const FString& TablePath = GetDefault<UMegalithSettings>()->SurfaceAcousticsTablePath;
	...
	bool TryGetFromTable(const FString& RowName, MegalithMeshAcoustics::FAcousticProperties& Out)
	{
		...
		const FAcousticSurfaceRow* Row =
			Table->FindRow<FAcousticSurfaceRow>(FName(*RowName), TEXT("MegalithAcoustics"));
		if (!Row)
		{
			return false;
		}
...
MegalithMeshAcoustics::FAcousticProperties MegalithMeshAcoustics::GetPropertiesForSurface(EPhysicalSurface SurfaceType)
{
...
MegalithMeshAcoustics::FAcousticProperties MegalithMeshAcoustics::GetPropertiesForName(const FString& SurfaceName)
{
	// Try DataTable first
	FAcousticProperties Props;
...
MegalithMeshAcoustics::FAcousticProperties MegalithMeshAcoustics::GetDefaultProperties()
{
	FAcousticProperties Props;
...
TMap<FString, MegalithMeshAcoustics::FAcousticProperties> MegalithMeshAcoustics::GetHardcodedDefaults()
{
...
const TCHAR* MegalithMeshAcoustics::AcousticTypeToString(ERoomAcousticType Type)
{
	switch (Type)
	{
...
float MegalithMeshAcoustics::ComputeSabineRT60(float VolumeM3, float TotalAbsorption)
{
...
MegalithMeshAcoustics::ERoomAcousticType MegalithMeshAcoustics::ClassifyRT60(float RT60Seconds)
{
...
float MegalithMeshAcoustics::ComputeDistanceAttenuation(float Distance, float RefDist)
{
	if (Distance <= RefDist)
	{
		return 1.0f;
...
float MegalithMeshAcoustics::DbToLinear(float dB)
{
	// factor = 10^(-dB/20)
	return FMath::Pow(10.0f, -dB / 20.0f);
}

float MegalithMeshAcoustics::TraceOcclusion(UWorld* World, const FVector& From, const FVector& To,
	int32& OutWallCount, float& OutTotalLossdB)
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithOcclusion), true);
...
TArray<MegalithMeshAcoustics::FSoundPath> MegalithMeshAcoustics::FindSoundPaths(
	...
	int32 MaxBounces, int32 CandidateSurfaces)
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithSoundPath), true);
	QueryParams.bReturnPhysicalMaterial = true;
...
MegalithMeshAcoustics::FReverbSuggestion MegalithMeshAcoustics::SuggestReverbSettings(
...
MegalithMeshAcoustics::FIndirectPathResult MegalithMeshAcoustics::FindIndirectNavmeshPath(
	UWorld* World, const FVector& From, const FVector& To)
{
