...
UMegalithMeshHandlePool* FMegalithMeshRoofActions::Pool = nullptr;

void FMegalithMeshRoofActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
...
void FMegalithMeshRoofActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("generate_roof"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshRoofActions::GenerateRoof),
		FParamSchemaBuilder()
...
bool FMegalithMeshRoofActions::ParseFootprint(const TSharedPtr<FJsonObject>& Params, TArray<FVector2D>& OutPoly, FString& OutError)
{
...
void FMegalithMeshRoofActions::ComputeFootprintOBB(const TArray<FVector2D>& Polygon,
...
TArray<FVector2D> FMegalithMeshRoofActions::ExpandPolygon(const TArray<FVector2D>& Polygon, float Distance)
{
...
bool FMegalithMeshRoofActions::BuildGableRoof(UDynamicMesh* Mesh, const TArray<FVector2D>& FootprintPoly,
...
bool FMegalithMeshRoofActions::BuildHipRoof(UDynamicMesh* Mesh, const TArray<FVector2D>& FootprintPoly,
	const TArray<FVector2D>& OverhangPoly, float PitchDeg, float HeightOffset,
	float Overhang, FString& OutError)
{
	FVector2D Center, LongAxis, ShortAxis;
...
bool FMegalithMeshRoofActions::BuildFlatRoof(UDynamicMesh* Mesh, const TArray<FVector2D>& FootprintPoly,
...
bool FMegalithMeshRoofActions::BuildShedRoof(UDynamicMesh* Mesh, const TArray<FVector2D>& FootprintPoly,
	const TArray<FVector2D>& OverhangPoly, float PitchDeg, float HeightOffset,
...
bool FMegalithMeshRoofActions::BuildGambrelRoof(UDynamicMesh* Mesh, const TArray<FVector2D>& FootprintPoly,
	const TArray<FVector2D>& OverhangPoly, float PitchDeg, float HeightOffset,
	float Overhang, FString& OutError)
{
	FVector2D Center, LongAxis, ShortAxis;
...
FMegalithActionResult FMegalithMeshRoofActions::GenerateRoof(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR_ROOF);
	}
	...
	if (!bOK)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Roof build failed: %s"), *BuildErr));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Roof geometry built but save failed: %s"), *SaveErr));
	}
