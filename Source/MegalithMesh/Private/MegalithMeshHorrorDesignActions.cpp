...
namespace
{
	TArray<TSharedPtr<FJsonValue>> MHd_VecToArr(const FVector& V)
	{
		return MegalithMeshAnalysis::VectorToJsonArray(V);
	}

	/** Parse an array of [x,y,z] arrays from a JSON field */
	bool MHd_ParseVectorArray(const TSharedPtr<FJsonObject>& Params, const FString& Key, TArray<FVector>& Out)
	{
	...
	bool MHd_ParseStringArray(const TSharedPtr<FJsonObject>& Params, const FString& Key, TArray<FString>& Out)
	{
		const TArray<TSharedPtr<FJsonValue>>* Arr;
	...
	{
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithHorrorDesign), true);
	...
	float QuickPerpendicularClearance(UWorld* World, const FVector& Location, const FVector& Forward)
	{
		...
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithHorrorDesignClearance), true);
	...
	{
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithHorrorDesignWall), true);
		FVector TestPt = Location + FVector(0, 0, 50.0f);
		const float MaxDist = 500.0f;
...
void FMegalithMeshHorrorDesignActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("predict_player_paths"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorDesignActions::PredictPlayerPaths),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("evaluate_spawn_point"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorDesignActions::EvaluateSpawnPoint),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("suggest_scare_positions"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorDesignActions::SuggestScarePositions),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("evaluate_encounter_pacing"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshHorrorDesignActions::EvaluateEncounterPacing),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshHorrorDesignActions::PredictPlayerPaths(const TSharedPtr<FJsonObject>& Params)
{
	FVector Start, End;
	...
	if (!MHd_ParseStringArray(Params, TEXT("strategies"), Strategies) || Strategies.Num() == 0)
	{
	...
	MHd_ParseVectorArray(Params, TEXT("waypoints"), Waypoints);
	...
	auto BuildWaypointPath = [&](const TArray<FVector>& Via) -> bool
	{
		...
		for (int32 i = 0; i <= Via.Num(); ++i)
		{
			...
			TArray<FVector> Seg;
			float SegDist;
			if (!MegalithMeshAnalysis::FindNavPath(World, Prev, Next, Seg, SegDist, static_cast<float>(AgentRadius)))
			{
				return false;
			}
	...
	for (const FString& Strategy : Strategies)
	{
		if (Strategy.Equals(TEXT("shortest"), ESearchCase::IgnoreCase))
		{
			...
			else if (MegalithMeshAnalysis::FindNavPath(World, Start, End, DirectPath, DirectDist, static_cast<float>(AgentRadius)))
			{
	...
	for (const FStrategyPath& SP : AllPaths)
	{
		...
		{
			PtsArr.Add(MakeShared<FJsonValueArray>(MHd_VecToArr(Pt)));
		}
...
FMegalithActionResult FMegalithMeshHorrorDesignActions::EvaluateSpawnPoint(const TSharedPtr<FJsonObject>& Params)
{
	FVector Location;
	...
	if (!MHd_ParseVectorArray(Params, TEXT("player_paths"), PlayerPath) || PlayerPath.Num() == 0)
	{
		FVector PlayerLoc;
	...
	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MegalithSpawnEval), true);
	...
	if (PlayerPath.Num() > 0)
	{
		float Concealment = MegalithMeshAnalysis::ComputeConcealment(World, Location, PlayerPath);
	...
	float LightingScore = 0.0f;
	{
		...
		int32 DominantIdx = -1;
		float Luminance = MegalithLightingCapture::ComputeAnalyticLuminance(World, Location, Lights, DominantIdx);
	...
	if (PlayerPath.Num() > 0)
	{
		...
		float OcclusionFactor = MegalithMeshAcoustics::TraceOcclusion(World, Location, NearestPlayerPt, WallCount, TotalLossdB);
	...
	int32 NearbyExitCount = 0;
	{
		...
		NearbyExitCount = MegalithMeshAnalysis::CountExits(World, Location, 2000.0f);
	...
	if (PlayerPath.Num() >= 2)
	{
		...
		TArray<MegalithMeshAnalysis::FPathClearance> Clearances = MegalithMeshAnalysis::MeasurePathClearance(World, PlayerPath, 500.0f);
	...
	Result->SetArrayField(TEXT("suggestions"), Suggestions);
	{
		...
		ANavigationData* NavData = nullptr;
		FString NavErr;
		Result->SetBoolField(TEXT("navmesh_available"), MegalithMeshAnalysis::GetNavSystem(World, NavSys, NavData, NavErr));
	}
...
FMegalithActionResult FMegalithMeshHorrorDesignActions::SuggestScarePositions(const TSharedPtr<FJsonObject>& Params)
{
	...
	for (int32 i = 0; i < SamplePoints.Num(); ++i)
	{
		...
		// Escape options
		Cand.EscapeOptions = MegalithMeshAnalysis::CountExits(World, SamplePoints[i], 1500.0f, 6);

		// Suggested direction: perpendicular to path, preferring the side with more concealment
		FVector Right = FVector::CrossProduct(Forward, FVector::UpVector).GetSafeNormal();
		if (Right.IsNearlyZero()) Right = FVector::RightVector;

		float RightConc = MegalithMeshAnalysis::ComputeConcealment(World,
			SamplePoints[i] + Right * 200.0f, { SamplePoints[i] });
		float LeftConc = MegalithMeshAnalysis::ComputeConcealment(World,
			SamplePoints[i] - Right * 200.0f, { SamplePoints[i] });
		...
		{
			int32 DummyIdx;
			float Lum = MegalithLightingCapture::ComputeAnalyticLuminance(World, SamplePoints[i], Lights, DummyIdx);
	...
	{
		auto Obj = MakeShared<FJsonObject>();
		Obj->SetArrayField(TEXT("location"), MHd_VecToArr(S.Location));
		...
		Obj->SetStringField(TEXT("tension_level"),
			MegalithMeshAnalysis::TensionLevelToString(MegalithMeshAnalysis::ClassifyTension(S.TensionContext)));
		Obj->SetNumberField(TEXT("escape_options"), S.EscapeOptions);
		Obj->SetArrayField(TEXT("suggested_direction"), MHd_VecToArr(S.SuggestedDirection));
...
FMegalithActionResult FMegalithMeshHorrorDesignActions::EvaluateEncounterPacing(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FVector> PathPoints;
	if (!MHd_ParseVectorArray(Params, TEXT("path_points"), PathPoints) || PathPoints.Num() < 2)
	{
	...
	TArray<FEncounter> Encounters;
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Missing or invalid required param: encounters (array of encounter objects)"));
		}
	...
	if (Encounters.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No valid encounters parsed from the encounters array."));
	}
	...
	for (int32 i = 0; i < Encounters.Num(); ++i)
	{
		...
		Obj->SetArrayField(TEXT("location"), MHd_VecToArr(Encounters[i].Location));
