...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshPerformanceActions::VectorToJsonArray(const FVector& V)
{
...
void FMegalithMeshPerformanceActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_region_performance"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPerformanceActions::GetRegionPerformance),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("estimate_placement_cost"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPerformanceActions::EstimatePlacementCost),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_overdraw_hotspots"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPerformanceActions::FindOverdrawHotspots),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_shadow_cost"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPerformanceActions::AnalyzeShadowCost),
		FParamSchemaBuilder()
			.Required(TEXT("region_min"), TEXT("array"), TEXT("Min corner of region [x, y, z]"))
			.Required(TEXT("region_max"), TEXT("array"), TEXT("Max corner of region [x, y, z]"))
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_triangle_budget"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshPerformanceActions::GetTriangleBudget),
		FParamSchemaBuilder()
			.Required(TEXT("viewpoint"), TEXT("array"), TEXT("Camera position [x, y, z]"))
			.Optional(TEXT("view_direction"), TEXT("array"), TEXT("Camera forward direction [x, y, z] (default: +X)"))
			.Optional(TEXT("fov"), TEXT("number"), TEXT("Field of view in degrees"), TEXT("90"))
...
FMegalithActionResult FMegalithMeshPerformanceActions::GetRegionPerformance(const TSharedPtr<FJsonObject>& Params)
{
	FVector RegionMin, RegionMax;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("region_min"), RegionMin))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: region_min (array of 3 numbers)"));
	}
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("region_max"), RegionMax))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: region_max (array of 3 numbers)"));
	}
...
FMegalithActionResult FMegalithMeshPerformanceActions::EstimatePlacementCost(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* AssetsArr;
	if (!Params->TryGetArrayField(TEXT("assets"), AssetsArr) || AssetsArr->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: assets (array of {asset_path, count} objects)"));
	}

	if (AssetsArr->Num() > 100)
	{
		return FMegalithActionResult::Error(TEXT("Too many asset entries (max 100)"));
	}
	...
	for (const TSharedPtr<FJsonValue>& Val : *AssetsArr)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Each entry in assets must be an object with asset_path and count"));
		}

		FString AssetPath;
		if (!(*EntryObj)->TryGetStringField(TEXT("asset_path"), AssetPath) || AssetPath.IsEmpty())
		{
			return FMegalithActionResult::Error(TEXT("Each entry requires a non-empty asset_path"));
		}
		...
		{
			USkeletalMesh* SkM = FMegalithAssetUtils::LoadAssetByPath<USkeletalMesh>(AssetPath);
			if (SkM)
			{
...
FMegalithActionResult FMegalithMeshPerformanceActions::FindOverdrawHotspots(const TSharedPtr<FJsonObject>& Params)
{
	FVector Viewpoint;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("viewpoint"), Viewpoint))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: viewpoint (array of 3 numbers)"));
	}

	FVector ViewDirection(1.0, 0.0, 0.0); // Default: +X forward
	MegalithMeshUtils::ParseVector(Params, TEXT("view_direction"), ViewDirection);
...
FMegalithActionResult FMegalithMeshPerformanceActions::AnalyzeShadowCost(const TSharedPtr<FJsonObject>& Params)
{
	FVector RegionMin, RegionMax;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("region_min"), RegionMin))
	{
...
FMegalithActionResult FMegalithMeshPerformanceActions::GetTriangleBudget(const TSharedPtr<FJsonObject>& Params)
{
	FVector Viewpoint;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("viewpoint"), Viewpoint))
	{
