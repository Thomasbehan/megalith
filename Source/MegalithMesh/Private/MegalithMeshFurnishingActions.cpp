...
void FMegalithMeshFurnishingActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("furnish_room"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFurnishingActions::FurnishRoom),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("furnish_building"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFurnishingActions::FurnishBuilding),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_furniture_presets"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFurnishingActions::ListFurniturePresets),
		FParamSchemaBuilder()
...
FString FMegalithMeshFurnishingActions::GetPresetsDirectory()
{
	return FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("Megalith"), TEXT("Saved"), TEXT("Megalith"), TEXT("FurniturePresets"));
}

TSharedPtr<FJsonObject> FMegalithMeshFurnishingActions::LoadPreset(const FString& RoomType)
{
	...
	TSharedPtr<FJsonObject> Parsed = FMegalithJsonUtils::Parse(JsonStr);
	return Parsed;
}

TArray<FString> FMegalithMeshFurnishingActions::GetAvailablePresets()
{
...
bool FMegalithMeshFurnishingActions::ParseVec3(const TArray<TSharedPtr<FJsonValue>>& Arr, FVector& Out)
{
...
bool FMegalithMeshFurnishingActions::ParseBoundsObject(const TSharedPtr<FJsonObject>& BoundsObj, FBox& Out)
{
...
FMegalithMeshFurnishingActions::EPlacementZone FMegalithMeshFurnishingActions::ParsePlacementZone(const FString& Str)
{
...
FMegalithMeshFurnishingActions::EWallRule FMegalithMeshFurnishingActions::ParseWallRule(const FString& Str)
{
...
FMegalithMeshFurnishingActions::FFurnitureItem FMegalithMeshFurnishingActions::ParseFurnitureItem(const TSharedPtr<FJsonObject>& ItemJson)
{
	FFurnitureItem Item;
...
FMegalithMeshFurnishingActions::FRoomWalls FMegalithMeshFurnishingActions::ComputeRoomWalls(
...
int32 FMegalithMeshFurnishingActions::ResolveWallIndex(EWallRule Rule, const FRoomWalls& Walls)
{
	switch (Rule)
	{
...
bool FMegalithMeshFurnishingActions::CheckCollision(
...
bool FMegalithMeshFurnishingActions::ComputePlacement(
...
void FMegalithMeshFurnishingActions::ApplyDensityOverrides(
...
void FMegalithMeshFurnishingActions::ApplyDecay(
...
TSharedPtr<FJsonObject> FMegalithMeshFurnishingActions::BuildParametricMeshParams(
	const FFurnitureItem& Item,
...
FMegalithActionResult FMegalithMeshFurnishingActions::FurnishRoom(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: world_bounds"));
	}

	FBox RoomBounds;
	if (!ParseBoundsObject(*BoundsObj, RoomBounds))
	{
		return FMegalithActionResult::Error(TEXT("Invalid world_bounds format — expected {min: [x,y,z], max: [x,y,z]}"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Preset '%s' has no 'items' array"), *PresetName));
	}
	...
	for (const FFurnitureItem& Item : FurnitureItems)
	{
		...
		for (int32 i = 0; i < Count; ++i)
		{
			...
			FMegalithActionResult MeshResult = Registry.ExecuteAction(TEXT("mesh"), TEXT("create_parametric_mesh"), MeshParams);
...
FMegalithActionResult FMegalithMeshFurnishingActions::FurnishBuilding(const TSharedPtr<FJsonObject>& Params)
{
	// --- Parse required params ---
	FString BuildingId;
	...
	FSpatialBlock& Block = FMegalithMeshSpatialRegistry::GetBlock(BlockId);
	...
	for (const FString& RoomId : AllRoomIds)
	{
		...
		FMegalithActionResult RoomResult = Registry.ExecuteAction(TEXT("mesh"), TEXT("furnish_room"), RoomParams);
...
FMegalithActionResult FMegalithMeshFurnishingActions::ListFurniturePresets(const TSharedPtr<FJsonObject>& Params)
{
	FString FilterType;
	...
	for (const FString& PresetName : Available)
	{
		...
		if (Preset.IsValid())
		{
			...
			if (Preset->TryGetObjectField(TEXT("density_overrides"), OverridesObj) && OverridesObj)
			{
				...
				{
					DensityModes.Add(MakeShared<FJsonValueString>(MegalithKeyToString(OverridePair.Key)));
				}
