...
namespace MegalithMeshUtils
{

UStaticMesh* LoadStaticMesh(const FString& Path, FString& OutError)
{
	UObject* Obj = FMegalithAssetUtils::LoadAssetByPath(Path);
	if (!Obj)
	{
...
{
	return MegalithParamUtils::ParseVector(Params, Key, Out);
}

bool ParseRotator(const TSharedPtr<FJsonObject>& Params, const FString& Key, FRotator& Out)
{
	return MegalithParamUtils::ParseRotator(Params, Key, Out);
}
...
{
	return MegalithParamUtils::GetEditorWorld();
}

MegalithMeshUtils::FBlockoutTags ParseBlockoutTags(const AActor* Actor)
{
	FBlockoutTags Result;
	if (!Actor) return Result;

	// --- Blueprint property reflection path (BP_MegalithBlockoutVolume) ---
	...
	if (RoomTypeProp && ActorClass->GetName().Contains(TEXT("MegalithBlockout")))
	{
	...
	// --- Tag parsing path (ABlockingVolume with Megalith.* actor tags) ---
	for (const FName& Tag : Actor->Tags)
	{
		FString TagStr = Tag.ToString();

		// Parse Megalith.Room:{type}
		if (TagStr.StartsWith(TEXT("Megalith.Room:"), ESearchCase::IgnoreCase))
		{
			Result.RoomType = TagStr.Mid(14); // len("Megalith.Room:") = 14
		}
		// Parse Megalith.Tag:{tag}
		else if (TagStr.StartsWith(TEXT("Megalith.Tag:"), ESearchCase::IgnoreCase))
		{
			Result.Tags.Add(TagStr.Mid(13)); // len("Megalith.Tag:") = 13
		}
		// Parse Megalith.Density:{density}
		else if (TagStr.StartsWith(TEXT("Megalith.Density:"), ESearchCase::IgnoreCase))
		{
			Result.Density = TagStr.Mid(17); // len("Megalith.Density:") = 17
		}
		// Parse Megalith.AllowPhysics
		else if (TagStr.Equals(TEXT("Megalith.AllowPhysics"), ESearchCase::IgnoreCase))
		{
			Result.bAllowPhysics = true;
		}
		// Parse Megalith.FloorHeight:{value}
		else if (TagStr.StartsWith(TEXT("Megalith.FloorHeight:"), ESearchCase::IgnoreCase))
		{
			FString Val = TagStr.Mid(21); // len("Megalith.FloorHeight:") = 21
			Result.FloorHeight = FCString::Atof(*Val);
		}
		// Parse Megalith.HasWalls
		else if (TagStr.Equals(TEXT("Megalith.HasWalls"), ESearchCase::IgnoreCase))
		{
			Result.bHasWalls = true;
		}
		// Parse Megalith.HasCeiling
		else if (TagStr.Equals(TEXT("Megalith.HasCeiling"), ESearchCase::IgnoreCase))
		{
