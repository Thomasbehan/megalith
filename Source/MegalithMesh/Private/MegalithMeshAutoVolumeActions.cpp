...
void FMegalithMeshAutoVolumeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("auto_volumes_for_building"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAutoVolumeActions::AutoVolumesForBuilding),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("auto_volumes_for_block"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAutoVolumeActions::AutoVolumesForBlock),
		FParamSchemaBuilder()
			.Optional(TEXT("block_id"), TEXT("string"), TEXT("Block ID (default: 'default')"), TEXT("default"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("spawn_nav_link"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAutoVolumeActions::SpawnNavLink),
		FParamSchemaBuilder()
...
FString FMegalithMeshAutoVolumeActions::DetermineReverbPreset(
...
FBox FMegalithMeshAutoVolumeActions::ComputeBuildingBounds(const FSpatialBlock& Block, const FSpatialBuilding& Building)
{
...
FVector FMegalithMeshAutoVolumeActions::WallToDirection(const FString& Wall)
{
...
AActor* FMegalithMeshAutoVolumeActions::SpawnNavLinkActor(
...
FMegalithActionResult FMegalithMeshAutoVolumeActions::AutoVolumesForBuilding(const TSharedPtr<FJsonObject>& Params)
{
	FString BuildingId;
	...
	if (bSpawnAudio)
	{
		for (const auto& FloorPair : Building->FloorToRoomIds)
		{
			...
			for (const FString& RoomId : FloorPair.Value)
			{
				...
				FMegalithActionResult AudioResult = Registry.ExecuteAction(TEXT("mesh"), TEXT("spawn_volume"), AudioParams);
	...
	if (bSpawnTriggers)
	{
		for (const FString& DoorId : Building->ExteriorDoorIds)
		{
			...
			FMegalithActionResult TrigResult = Registry.ExecuteAction(TEXT("mesh"), TEXT("spawn_volume"), TrigParams);
...
FMegalithActionResult FMegalithMeshAutoVolumeActions::AutoVolumesForBlock(const TSharedPtr<FJsonObject>& Params)
{
	FString BlockId = TEXT("default");
	Params->TryGetStringField(TEXT("block_id"), BlockId);
	...
	for (const auto& BuildingPair : Block.Buildings)
	{
		...
		FMegalithActionResult BuildingResult = Registry.ExecuteAction(TEXT("mesh"), TEXT("auto_volumes_for_building"), BuildingParams);
...
FMegalithActionResult FMegalithMeshAutoVolumeActions::SpawnNavLink(const TSharedPtr<FJsonObject>& Params)
{
	// Parse start point
	const TArray<TSharedPtr<FJsonValue>>* StartArr = nullptr;
	if (!Params->TryGetArrayField(TEXT("start"), StartArr) || !StartArr)
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: start (array of 3 numbers)"));
	}
	FVector StartPoint;
	if (!AutoVolumeHelpers::ParseVectorFromArray(*StartArr, StartPoint))
	{
		return FMegalithActionResult::Error(TEXT("Invalid start: expected [x, y, z]"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: end (array of 3 numbers)"));
	}
	FVector EndPoint;
	if (!AutoVolumeHelpers::ParseVectorFromArray(*EndArr, EndPoint))
	{
		return FMegalithActionResult::Error(TEXT("Invalid end: expected [x, y, z]"));
	}
	...
	if (!NavLink)
	{
		return FMegalithActionResult::Error(TEXT("Failed to spawn NavLinkProxy. Ensure AIModule is enabled in your project."));
	}
