...
void FMegalithMeshLevelDesignActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_light"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::PlaceLight),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_light_properties"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::SetLightProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_actor_material"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::SetActorMaterial),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("swap_material_in_level"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::SwapMaterialInLevel),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_replace_mesh"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::FindReplaceMesh),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_lod_screen_sizes"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::SetLodScreenSizes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_instancing_candidates"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::FindInstancingCandidates),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("convert_to_hism"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::ConvertToHism),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_actor_component_properties"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLevelDesignActions::GetActorComponentProperties),
		FParamSchemaBuilder()
			.Required(TEXT("actor_name"), TEXT("string"), TEXT("Actor name or label"))
...
TArray<FString> FMegalithMeshLevelDesignActions::ApplyLightProperties(ULightComponent* LightComp, const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithMeshLevelDesignActions::PlaceLight(const TSharedPtr<FJsonObject>& Params)
{
	FString TypeStr;
	if (!Params->TryGetStringField(TEXT("type"), TypeStr))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: type (point, spot, rect, directional)"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid light type: '%s'. Use point, spot, rect, or directional."), *TypeStr));
	}

	LevelDesignHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Place Light")));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to spawn %s"), *ClassName));
	}
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Spawned light actor has no ULightComponent"));
	}
...
FMegalithActionResult FMegalithMeshLevelDesignActions::SetLightProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
	...
	if (!LightComp)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Actor '%s' has no ULightComponent"), *ActorName));
	}

	LevelDesignHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Set Light Properties")));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("No valid light properties provided"));
	}
...
FMegalithActionResult FMegalithMeshLevelDesignActions::SetActorMaterial(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
	if (!Params->TryGetStringField(TEXT("actor_name"), ActorName))
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: material"));
	}
	...
	// Load material
	UMaterialInterface* Material = FMegalithAssetUtils::LoadAssetByPath<UMaterialInterface>(MaterialPath);
	if (!Material)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Material not found: %s"), *MaterialPath));
	}
	...
	if (Params->TryGetStringField(TEXT("component_name"), ComponentName) && !ComponentName.IsEmpty())
	{
		...
		if (!MeshComp)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Mesh component '%s' not found on actor '%s'"), *ComponentName, *ActorName));
		}
	}
	else
	{
		...
		if (!MeshComp)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Actor '%s' has no mesh component"), *ActorName));
		}
	}
	...
	if (bUseSlotName)
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Material slot name '%s' not found on component"), *SlotName));
		}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Slot index %d out of range (component has %d slots)"), SlotIndex, MeshComp->GetNumMaterials()));
	}
	...
	LevelDesignHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Set Actor Material")));
...
FMegalithActionResult FMegalithMeshLevelDesignActions::SwapMaterialInLevel(const TSharedPtr<FJsonObject>& Params)
{
	FString SourcePath, TargetPath;
	if (!Params->TryGetStringField(TEXT("source_material"), SourcePath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: source_material"));
	}
	if (!Params->TryGetStringField(TEXT("target_material"), TargetPath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: target_material"));
	}

	bool bPreview = false;
	Params->TryGetBoolField(TEXT("preview"), bPreview);

	UMaterialInterface* SourceMat = FMegalithAssetUtils::LoadAssetByPath<UMaterialInterface>(SourcePath);
	if (!SourceMat)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Source material not found: %s"), *SourcePath));
	}

	UMaterialInterface* TargetMat = nullptr;
	if (!bPreview)
	{
		TargetMat = FMegalithAssetUtils::LoadAssetByPath<UMaterialInterface>(TargetPath);
		if (!TargetMat)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Target material not found: %s"), *TargetPath));
		}
	}
	...
	if (!bPreview)
	{
		Transaction = MakeShared<LevelDesignHelpers::FScopedMeshTransaction>(FText::FromString(TEXT("Megalith: Swap Material In Level")));
	}
...
FMegalithActionResult FMegalithMeshLevelDesignActions::FindReplaceMesh(const TSharedPtr<FJsonObject>& Params)
{
	FString SourcePath, TargetPath;
	if (!Params->TryGetStringField(TEXT("source_mesh"), SourcePath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: source_mesh"));
	}
	if (!Params->TryGetStringField(TEXT("target_mesh"), TargetPath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: target_mesh"));
	}
	...
	UStaticMesh* SourceMesh = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(SourcePath);
	if (!SourceMesh)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Source mesh not found: %s"), *SourcePath));
	}

	UStaticMesh* TargetMesh = nullptr;
	if (!bPreview)
	{
		TargetMesh = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(TargetPath);
		if (!TargetMesh)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Target mesh not found: %s"), *TargetPath));
		}
	}
	...
	if (!bPreview)
	{
		Transaction = MakeShared<LevelDesignHelpers::FScopedMeshTransaction>(FText::FromString(TEXT("Megalith: Find Replace Mesh")));
	}
		...
		for (UStaticMeshComponent* SMC : SMCs)
		{
			...
			if (bMatch)
			{
				if (!bPreview)
				{
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("Megalith: Mesh swap on '%s' — material slot count differs (source: %d, target: %d). Material overrides may be invalid."),
...
FMegalithActionResult FMegalithMeshLevelDesignActions::SetLodScreenSizes(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: screen_sizes"));
	}

	UStaticMesh* SM = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(AssetPath);
	if (!SM)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Static mesh not found: %s"), *AssetPath));
	}

	int32 LODCount = SM->GetNumSourceModels();
	if (ScreenSizesArr->Num() > LODCount)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Provided %d screen sizes but mesh only has %d LODs"), ScreenSizesArr->Num(), LODCount));
	}
	...
	for (int32 i = 0; i < ScreenSizesArr->Num(); ++i)
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Screen sizes must be monotonically decreasing. Size[%d]=%.4f >= Size[%d]=%.4f"), i, Val, i - 1, NewSizes.Last()));
		}
		NewSizes.Add(Val);
	}
	...
	LevelDesignHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Set LOD Screen Sizes")));
...
FMegalithActionResult FMegalithMeshLevelDesignActions::FindInstancingCandidates(const TSharedPtr<FJsonObject>& Params)
{
	int32 MinCount = 5;
	double MinCountVal;
	...
	bool bHasRegion = MegalithMeshUtils::ParseVector(Params, TEXT("region_min"), RegionMin)
...
FMegalithActionResult FMegalithMeshLevelDesignActions::ConvertToHism(const TSharedPtr<FJsonObject>& Params)
{
	FString MeshPath;
	if (!Params->TryGetStringField(TEXT("mesh"), MeshPath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: mesh"));
	}

	const TArray<TSharedPtr<FJsonValue>>* ActorsArr;
	if (!Params->TryGetArrayField(TEXT("actors"), ActorsArr) || ActorsArr->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: actors"));
	}

	bool bPreserveMaterials = true;
	Params->TryGetBoolField(TEXT("preserve_materials"), bPreserveMaterials);

	UStaticMesh* Mesh = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(MeshPath);
	if (!Mesh)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Static mesh not found: %s"), *MeshPath));
	}
	...
	LevelDesignHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Convert to HISM")));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Failed to spawn HISM host actor"));
	}
	...
	{
		HISMActor->SetActorLabel(FString::Printf(TEXT("HISM_%s"), *FMegalithAssetUtils::GetAssetName(MeshPath)));
	}
...
FMegalithActionResult FMegalithMeshLevelDesignActions::GetActorComponentProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
	if (!Params->TryGetStringField(TEXT("actor_name"), ActorName))
	{
	...
	if (!ComponentName.IsEmpty())
	{
		...
		if (!TargetComp)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Component '%s' not found on actor '%s'"), *ComponentName, *ActorName));
		}
	}
	else if (!ComponentClass.IsEmpty())
	{
		...
		if (!TargetComp)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("No component of class '%s' found on actor '%s'"), *ComponentClass, *ActorName));
		}
	}
	else
	{
		...
		if (!TargetComp)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Actor '%s' has no root component"), *ActorName));
		}
	}
