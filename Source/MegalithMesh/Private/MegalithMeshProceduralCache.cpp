...
const TSet<FString> FMegalithMeshProceduralCache::ExcludeKeys = {
	TEXT("handle"),
	TEXT("save_path"),
	TEXT("overwrite"),
	TEXT("place_in_scene"),
...
FMegalithMeshProceduralCache& FMegalithMeshProceduralCache::Get()
{
	static FMegalithMeshProceduralCache Instance;
	return Instance;
}
...
FString FMegalithMeshProceduralCache::GetCacheDirectory()
{
	FString Dir = FPaths::ProjectPluginsDir() / TEXT("Megalith") / TEXT("Saved") / TEXT("Megalith") / TEXT("ProceduralCache");
	IFileManager::Get().MakeDirectory(*Dir, true);
	return Dir;
}

FString FMegalithMeshProceduralCache::GetManifestPath()
{
...
void FMegalithMeshProceduralCache::EnsureLoaded()
{
	if (!bManifestLoaded)
	{
		LoadManifest();
...
void FMegalithMeshProceduralCache::LoadManifest()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[MegalithCache] Failed to parse manifest, starting fresh: %s"), *Path);
		Manifest = MakeShared<FJsonObject>();
		Manifest->SetNumberField(TEXT("version"), 1);
		Manifest->SetObjectField(TEXT("entries"), MakeShared<FJsonObject>());
		return;
...
void FMegalithMeshProceduralCache::SaveManifest()
{
	if (!Manifest.IsValid())
	{
		return;
	...
	{
		UE_LOG(LogTemp, Error, TEXT("[MegalithCache] Failed to serialize manifest"));
		return;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("[MegalithCache] Failed to write temp manifest: %s"), *TempPath);
		return;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("[MegalithCache] Failed to rename temp manifest to: %s"), *ManifestPath);
...
FString FMegalithMeshProceduralCache::SerializeValue(const TSharedPtr<FJsonValue>& Val)
{
...
FString FMegalithMeshProceduralCache::SortedJsonSerialize(const TSharedPtr<FJsonObject>& Obj)
{
	if (!Obj.IsValid())
	{
		return TEXT("{}");
	...
	TArray<FString> Keys;
	for (const auto& KVPair : Obj->Values) { Keys.Add(MegalithKeyToString(KVPair.Key)); }
	Keys.Sort();
	...
	for (int32 i = 0; i < Keys.Num(); ++i)
	{
		...
		const TSharedPtr<FJsonValue>& Val = Obj->Values[MegalithMakeJsonKey(Keys[i])];
...
FString FMegalithMeshProceduralCache::SortedArraySerialize(const TArray<TSharedPtr<FJsonValue>>& Arr)
{
	FString Result = TEXT("[");
...
FString FMegalithMeshProceduralCache::ComputeHash(const FString& ActionName, const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Params.IsValid())
	{
		...
		{
			if (!ExcludeKeys.Contains(MegalithKeyToString(Pair.Key)))
			{
...
bool FMegalithMeshProceduralCache::TryGetCached(const FString& Hash, FString& OutAssetPath)
{
	EnsureLoaded();
	...
	if (!FPackageName::DoesPackageExist(AssetPath))
	{
		...
		Entries->RemoveField(Hash);
		SaveManifest();
		UE_LOG(LogTemp, Log, TEXT("[MegalithCache] Removed stale entry %s -> %s"), *Hash.Left(8), *AssetPath);
		return false;
	}
...
void FMegalithMeshProceduralCache::Register(const FString& Hash, const FString& AssetPath,
	...
	const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogTemp, Log, TEXT("[MegalithCache] Registered %s -> %s (%s, %d tris)"),
...
FString FMegalithMeshProceduralCache::GenerateAutoPath(const FString& Category, const FString& Type,
...
int32 FMegalithMeshProceduralCache::ValidateCache()
{
	EnsureLoaded();
	...
	for (const auto& Pair : Entries->Values)
	{
		...
		{
			StaleHashes.Add(MegalithKeyToString(Pair.Key));
			continue;
		}
	...
	{
		SaveManifest();
		UE_LOG(LogTemp, Log, TEXT("[MegalithCache] Validated cache: removed %d stale entries"), StaleHashes.Num());
	}

	return StaleHashes.Num();
}

int32 FMegalithMeshProceduralCache::ClearCache(const FString& TypeFilter)
{
	EnsureLoaded();
	...
	if (TypeFilter.IsEmpty())
	{
		...
		Manifest->SetObjectField(TEXT("entries"), MakeShared<FJsonObject>());
		SaveManifest();
		UE_LOG(LogTemp, Log, TEXT("[MegalithCache] Cleared all %d cache entries"), Count);
		return Count;
	}
	...
	for (const auto& Pair : Entries->Values)
	{
		...
		{
			ToRemove.Add(MegalithKeyToString(Pair.Key));
		}
	}
	...
	{
		SaveManifest();
		UE_LOG(LogTemp, Log, TEXT("[MegalithCache] Cleared %d entries of type '%s'"), ToRemove.Num(), *TypeFilter);
	}

	return ToRemove.Num();
}

TSharedPtr<FJsonObject> FMegalithMeshProceduralCache::GetStats()
{
	EnsureLoaded();
...
TSharedPtr<FJsonObject> FMegalithMeshProceduralCache::ListEntries(const FString& TypeFilter, int32 Limit)
{
	EnsureLoaded();
