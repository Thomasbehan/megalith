...
UMegalithMeshHandlePool* FMegalithMeshFacadeActions::Pool = nullptr;

void FMegalithMeshFacadeActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
...
void FMegalithMeshFacadeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("generate_facade"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFacadeActions::GenerateFacade),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_facade_styles"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFacadeActions::ListFacadeStyles),
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("apply_horror_damage"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFacadeActions::ApplyHorrorDamage),
		FParamSchemaBuilder()
...
FString FMegalithMeshFacadeActions::GetFacadeStylesDir()
{
	return FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("Megalith"), TEXT("Saved"), TEXT("Megalith"), TEXT("FacadeStyles"));
}

bool FMegalithMeshFacadeActions::LoadFacadeStyle(const FString& StyleName, FFacadeStyle& OutStyle)
{
...
FVector FMegalithMeshFacadeActions::GetFaceWidthAxis(const FExteriorFaceDef& Face)
{
...
FTransform FMegalithMeshFacadeActions::ComputeFaceTransform(const FExteriorFaceDef& Face)
{
...
TArray<float> FMegalithMeshFacadeActions::ComputeWindowPositions(
...
void FMegalithMeshFacadeActions::BuildWallSlab(UDynamicMesh* Mesh,
...
void FMegalithMeshFacadeActions::CutOpenings(UDynamicMesh* Mesh,
...
void FMegalithMeshFacadeActions::CutOpeningsSelectionInset(UDynamicMesh* Mesh,
	const FExteriorFaceDef& Face, const TArray<FWindowPlacement>& Windows,
...
void FMegalithMeshFacadeActions::AddWindowFrames(UDynamicMesh* Mesh,
	const FExteriorFaceDef& Face, const TArray<FWindowPlacement>& Windows,
...
void FMegalithMeshFacadeActions::AddDoorFrames(UDynamicMesh* Mesh,
...
void FMegalithMeshFacadeActions::AddCornice(UDynamicMesh* Mesh,
...
void FMegalithMeshFacadeActions::AddBeltCourse(UDynamicMesh* Mesh,
...
void FMegalithMeshFacadeActions::AddGlassPanes(UDynamicMesh* Mesh,
	const FExteriorFaceDef& Face, const TArray<FWindowPlacement>& Windows,
...
void FMegalithMeshFacadeActions::AddBoardedWindows(UDynamicMesh* Mesh,
	const FExteriorFaceDef& Face, const TArray<FWindowPlacement>& Windows,
...
void FMegalithMeshFacadeActions::AddCrackGeometry(UDynamicMesh* Mesh,
...
FMegalithActionResult FMegalithMeshFacadeActions::GenerateFacade(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR_FACADE);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing 'save_path'"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Building descriptor has no 'exterior_faces' array"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("No valid exterior faces parsed from building descriptor"));
	}
	...
	AActor* Actor = FMegalithMeshProceduralActions::PlaceMeshInScene(SavePath, Location, FRotator::ZeroRotator, Label, false, Folder);
...
FMegalithActionResult FMegalithMeshFacadeActions::ListFacadeStyles(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithMeshFacadeActions::ApplyHorrorDamage(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR_FACADE);
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing 'target_actor'"));
	}
	...
	if (!TargetActor)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Actor '%s' not found"), *TargetActorName));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("No damage geometry generated (damage_level may be too low or actor too small)"));
	}

	// Cleanup
	FMegalithMeshProceduralActions::CleanupMesh(DamageMesh, false);
	...
	bool bOverwrite = Params->HasField(TEXT("overwrite")) ? Params->GetBoolField(TEXT("overwrite")) : false;
	FString SaveErr;
	if (!FMegalithMeshProceduralActions::SaveMeshToAsset(DamageMesh, DamageSavePath, bOverwrite, SaveErr))
	{
		return FMegalithActionResult::Error(FString::Printf(
	...
	AActor* DamageActor = FMegalithMeshProceduralActions::PlaceMeshInScene(
	...
	if (DamageActor)
	{
		...
		DamageActor->Tags.Add(FName(TEXT("Megalith.Procedural")));
	}
