...
DECLARE_LOG_CATEGORY_EXTERN(LogMegalithCityBlock, Log, All);
DEFINE_LOG_CATEGORY(LogMegalithCityBlock);

UMegalithMeshHandlePool* FMegalithMeshCityBlockActions::Pool = nullptr;

void FMegalithMeshCityBlockActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
...
void FMegalithMeshCityBlockActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_city_block"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshCityBlockActions::CreateCityBlock),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_lot_layout"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshCityBlockActions::CreateLotLayout),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_street"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshCityBlockActions::CreateStreet),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_street_furniture"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshCityBlockActions::PlaceStreetFurniture),
		FParamSchemaBuilder()
...
bool FMegalithMeshCityBlockActions::TryExecuteAction(const FString& Action,
	const TSharedPtr<FJsonObject>& Params, TSharedPtr<FJsonObject>& OutResult, FString& OutError)
{
	...
	FMegalithActionResult Result = Registry.ExecuteAction(TEXT("mesh"), Action, Params);
	if (Result.bSuccess)
	{
	...
	UE_LOG(LogMegalithCityBlock, Warning, TEXT("Action '%s' failed or not available: %s"), *Action, *OutError);
	return false;
}
...
FString FMegalithMeshCityBlockActions::GetPresetsDirectory()
{
	return FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("Megalith"), TEXT("Saved"), TEXT("Megalith"), TEXT("BlockPresets"));
}

bool FMegalithMeshCityBlockActions::LoadPreset(const FString& PresetName, TSharedPtr<FJsonObject>& OutPreset, FString& OutError)
{
	FString FilePath;
...
void FMegalithMeshCityBlockActions::ApplyPresetDefaults(const TSharedPtr<FJsonObject>& Preset, const TSharedPtr<FJsonObject>& Params)
{
...
TArray<FString> FMegalithMeshCityBlockActions::GetGenreArchetypes(const FString& Genre, int32 BuildingCount, FRandomStream& Rng)
{
...
TArray<FString> FMegalithMeshCityBlockActions::GetGenreFurniture(const FString& Genre)
{
	if (Genre == TEXT("suburban"))
	{
...
TArray<FMegalithMeshCityBlockActions::FBlockLot> FMegalithMeshCityBlockActions::SubdivideOBBRecursive(
...
TArray<FMegalithMeshCityBlockActions::FBlockLot> FMegalithMeshCityBlockActions::SubdivideGrid(
	float OriginX, float OriginY, float Width, float Height,
	int32 TargetCount, float MinWidth, float MinHeight,
	int32& LotCounter)
{
...
TArray<FMegalithMeshCityBlockActions::FBlockLot> FMegalithMeshCityBlockActions::SubdivideOrganic(
	float OriginX, float OriginY, float Width, float Height,
	int32 TargetCount, float MinWidth, float MinHeight,
...
FString FMegalithMeshCityBlockActions::ChooseFootprintShape(const FString& Genre, FRandomStream& Rng)
{
...
TArray<FVector2D> FMegalithMeshCityBlockActions::GenerateFootprint(
...
void FMegalithMeshCityBlockActions::ApplyBuildingDecay(float DecayLevel, int32 BuildingIndex,
...
void FMegalithMeshCityBlockActions::BuildRoadSurface(UDynamicMesh* Mesh,
...
void FMegalithMeshCityBlockActions::BuildSidewalk(UDynamicMesh* Mesh,
...
FMegalithActionResult FMegalithMeshCityBlockActions::CreateLotLayout(const TSharedPtr<FJsonObject>& Params)
{
	// Parse block_size
	...
	{
		return FMegalithActionResult::Error(TEXT("block_size must be [width, height] array"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("block_size dimensions must be >= 100 cm"));
	}

	const int32 LotCount = static_cast<int32>(Params->GetNumberField(TEXT("lot_count")));
	if (LotCount < 1 || LotCount > 100)
	{
		return FMegalithActionResult::Error(TEXT("lot_count must be between 1 and 100"));
	}
...
FMegalithActionResult FMegalithMeshCityBlockActions::CreateStreet(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("start must be [x, y] array"));
	}
	if (!Params->TryGetArrayField(TEXT("end"), EndArr) || !EndArr || EndArr->Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("end must be [x, y] array"));
	}

	FString SavePath;
	if (!Params->TryGetStringField(TEXT("save_path"), SavePath) || SavePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("save_path is required"));
	}
	...
	// Save to asset
	FString SaveError;
	if (!FMegalithMeshProceduralActions::SaveMeshToAsset(DynMesh, SavePath, true, SaveError))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to save street mesh: %s"), *SaveError));
	}
	...
	PlacedActor = FMegalithMeshProceduralActions::PlaceMeshInScene(SavePath, Location, FRotator::ZeroRotator, Label, false, Folder);
...
FMegalithActionResult FMegalithMeshCityBlockActions::PlaceStreetFurniture(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("street_start must be [x, y] array"));
	}
	if (!Params->TryGetArrayField(TEXT("street_end"), EndArr) || !EndArr || EndArr->Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("street_end must be [x, y] array"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Street segment too short for requested spacing"));
	}
...
FMegalithActionResult FMegalithMeshCityBlockActions::CreateCityBlock(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("save_path_prefix is required"));
	}
	...
	if (Params->TryGetStringField(TEXT("preset"), PresetName) && !PresetName.IsEmpty())
	{
		...
		if (LoadPreset(PresetName, Preset, PresetError))
		{
			...
			UE_LOG(LogMegalithCityBlock, Log, TEXT("Applied block preset: %s"), *PresetName);
		}
		else
		{
			UE_LOG(LogMegalithCityBlock, Warning, TEXT("Could not load preset '%s': %s (using defaults)"), *PresetName, *PresetError);
		}
	}
	...
	MegalithMeshUtils::ParseVector(Params, TEXT("location"), BlockOrigin);
	...
	UE_LOG(LogMegalithCityBlock, Log, TEXT("=== create_city_block: %d buildings, genre=%s, seed=%d ==="), BuildingCount, *Genre, Seed);
	...
	FMegalithActionResult LotResult = CreateLotLayout(LotParams);
	if (!LotResult.bSuccess)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Lot subdivision failed: %s"), *LotResult.ErrorMessage));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Lot layout returned no lots"));
	}

	UE_LOG(LogMegalithCityBlock, Log, TEXT("  Subdivided into %d lots"), LotsArr->Num());
	...
	for (int32 i = 0; i < LotsArr->Num() && i < BuildingCount; ++i)
	{
		...
		UE_LOG(LogMegalithCityBlock, Log, TEXT("  Building %d: lot at (%.0f, %.0f) size %.0f x %.0f, archetype=%s"),
		...
		{
			UE_LOG(LogMegalithCityBlock, Log, TEXT("    -> DESTROYED by decay (skipped)"));
		...
		if (!bHasFloorPlan)
		{
			UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Floor plan generation failed/unavailable: %s. Using simple box fallback."), *FloorPlanError);
		}
		...
		if (!bBuilt)
		{
			UE_LOG(LogMegalithCityBlock, Error, TEXT("    Building %d geometry generation FAILED: %s"), i, *BuildingError);
		...
		UE_LOG(LogMegalithCityBlock, Log, TEXT("    Building %d geometry: OK"), i);
		...
		{
			UE_LOG(LogMegalithCityBlock, Log, TEXT("    Facade %d: integrated (%d windows, %d doors)"),
				i,
		...
		{
			UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Facade %d: integration failed, no facade generated"), i);
			SkippedSteps.AddUnique(TEXT("facades"));
		}
		...
		if (!bSkipRoofs)
		{
			...
			{
				FString RoofType = FMegalithMeshFloorPlanGenerator::GetArchetypeRoofType(Archetypes[i]);
			...
			{
				UE_LOG(LogMegalithCityBlock, Log, TEXT("    Roof %d: OK"), i);
			...
			{
				UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Roof %d skipped (SP4 not available): %s"), i, *RoofError);
		...
		// Register building in spatial registry via SP6
		{
			...
			{
				UE_LOG(LogMegalithCityBlock, Log, TEXT("    Registered building %d in spatial registry"), i);
			}
			else
			{
				UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Spatial registry skipped: %s"), *RegError);
		...
		if (!bSkipFurnishing)
		{
			...
			{
				UE_LOG(LogMegalithCityBlock, Log, TEXT("    Furnish %d: OK"), i);
			...
			{
				UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Furnish %d skipped: %s"), i, *FurnishError);
		...
		if (!bSkipVolumes)
		{
			...
			{
				UE_LOG(LogMegalithCityBlock, Log, TEXT("    Volumes %d: OK"), i);
			...
			{
				UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Volumes %d skipped: %s"), i, *VolError);
		...
		if (!bSkipValidation)
		{
			...
			if (TryExecuteAction(TEXT("validate_building"), ValParams, ValResult, ValError))
			{
				...
				UE_LOG(LogMegalithCityBlock, Log, TEXT("    Validation building %d: score=%.2f"), i, ValScore);
				...
				if (bValidateAndRetry && ValScore < 0.5f)
				{
					...
					for (int32 Retry = 1; Retry <= MaxRetries; ++Retry)
					{
						...
						UE_LOG(LogMegalithCityBlock, Log, TEXT("    Retry %d/%d for building %d (score %.2f < 0.5), seed=%d"),
						...
						{
							UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Retry %d floor plan regen failed: %s (using previous grid)"), Retry, *RetryFPError);
						}
						...
						{
							UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Retry %d build failed: %s"), Retry, *RetryBuildError);
							continue;
						}
						...
						if (TryExecuteAction(TEXT("validate_building"), ValParams, RetryValResult, RetryValError))
						{
							...
							UE_LOG(LogMegalithCityBlock, Log, TEXT("    Retry %d score: %.2f (best: %.2f)"), Retry, RetryScore, BestScore);
						...
						{
							UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Retry %d validation failed: %s"), Retry, *RetryValError);
						}
					}
					...
					UE_LOG(LogMegalithCityBlock, Log, TEXT("    Final score for building %d after retries: %.2f"), i, ValScore);
				}
			...
			{
				UE_LOG(LogMegalithCityBlock, Warning, TEXT("    Validation skipped (action unavailable): %s"), *ValError);
	...
	if (!bSkipStreets)
	{
		...
		if (LotResult.Result->TryGetArrayField(TEXT("streets"), StreetsArr) && StreetsArr)
		{
			...
			for (const auto& StreetVal : *StreetsArr)
			{
				...
				FMegalithActionResult SR = CreateStreet(StreetParams);
				if (SR.bSuccess)
				{
					UE_LOG(LogMegalithCityBlock, Log, TEXT("  Street %d: OK"), StreetIdx);
					StreetResults.Add(MakeShared<FJsonValueObject>(SR.Result));
				}
				else
				{
					UE_LOG(LogMegalithCityBlock, Warning, TEXT("  Street %d failed: %s"), StreetIdx, *SR.ErrorMessage);
				}
	...
	if (!bSkipFurniture && !bSkipStreets)
	{
		...
		if (LotResult.Result->TryGetArrayField(TEXT("streets"), FurnStreetsArr) && FurnStreetsArr)
		{
			for (int32 FurnIdx = 0; FurnIdx < FurnStreetsArr->Num(); ++FurnIdx)
			{
				...
				FMegalithActionResult FurnResult = PlaceStreetFurniture(FurnParams);
				if (FurnResult.bSuccess)
				{
					FurnitureResults.Add(MakeShared<FJsonValueObject>(FurnResult.Result));
					UE_LOG(LogMegalithCityBlock, Log, TEXT("  Street furniture %d: OK"), FurnIdx);
				}
				else
				{
					UE_LOG(LogMegalithCityBlock, Warning, TEXT("  Street furniture %d failed: %s"), FurnIdx, *FurnResult.ErrorMessage);
				}
			}
		}
		...
		{
			UE_LOG(LogMegalithCityBlock, Log, TEXT("  Street furniture: %d segments furnished"), FurnitureResults.Num());
		}
	}
	else
	{
	...
	UE_LOG(LogMegalithCityBlock, Log, TEXT("=== create_city_block COMPLETE: %d buildings, %d streets, %d skipped steps ==="),
