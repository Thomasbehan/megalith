...
TMap<FString, TWeakObjectPtr<UMaterialInstanceDynamic>> FMegalithMeshBlockoutActions::CachedBlockoutMaterials;

// ============================================================================
// FScopedMeshTransaction
// ============================================================================

FMegalithMeshBlockoutActions::FScopedMeshTransaction::FScopedMeshTransaction(const FText& Description)
...
FMegalithMeshBlockoutActions::FScopedMeshTransaction::~FScopedMeshTransaction()
{
	if (bOwnsTransaction && GEditor)
	{
...
void FMegalithMeshBlockoutActions::FScopedMeshTransaction::Cancel()
{
	if (bOwnsTransaction && GEditor)
	{
...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshBlockoutActions::VectorToJsonArray(const FVector& V)
{
...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshBlockoutActions::RotatorToJsonArray(const FRotator& R)
{
	TArray<TSharedPtr<FJsonValue>> Arr;
...
static FString CheckBlockoutOverlap(UWorld* World, AActor* SpawnedActor, const FString& Label)
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithBlockoutOverlap), true);
	...
	// Filter: only warn about non-blockout actors (skip other Megalith primitives)
	...
	for (const FOverlapResult& Overlap : Overlaps)
	{
		...
		if (Other->Tags.Contains(FName(TEXT("Megalith.BlockoutPrimitive"))) ||
			Other->Tags.Contains(FName(TEXT("Megalith.ScatteredProp"))))
		{
			continue;
		}
...
ABlockingVolume* FMegalithMeshBlockoutActions::FindBlockingVolume(const FString& VolumeName, FString& OutError)
{
...
AActor* FMegalithMeshBlockoutActions::FindBlockoutVolumeAny(const FString& VolumeName, FString& OutError)
{
	...
	// Try Blueprint-based MegalithBlockout actors
	OutError.Reset();
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	{
		AActor* Actor = *It;
		if (!Actor->GetClass()->GetName().Contains(TEXT("MegalithBlockout")))
		{
			continue;
		}
	...
	if (BPMatches.Num() == 0)
	{
		OutError = FString::Printf(TEXT("Blockout volume not found: %s (checked ABlockingVolume tags and BP_MegalithBlockoutVolume actors)"), *VolumeName);
		return nullptr;
	}
...
UMaterialInstanceDynamic* FMegalithMeshBlockoutActions::GetBlockoutMaterial(const FString& Category)
{
	// Check cache first
...
bool FMegalithMeshBlockoutActions::HasMegalithTag(const AActor* Actor, const FString& TagPrefix)
{
...
FString FMegalithMeshBlockoutActions::GetMegalithTagValue(const AActor* Actor, const FString& TagPrefix)
{
...
FString FMegalithMeshBlockoutActions::GetBasicShapePath(const FString& ShapeName, bool& bValid)
{
	bValid = true;
...
FVector FMegalithMeshBlockoutActions::GetBlockoutActorSize(const AActor* Actor)
{
...
namespace BlockoutHelpers
{
	...
	{
		UMegalithIndexSubsystem* IndexSub = GEditor ?
			GEditor->GetEditorSubsystem<UMegalithIndexSubsystem>() : nullptr;
...
void FMegalithMeshBlockoutActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 1. get_blockout_volumes
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_blockout_volumes"),
		TEXT("Find all actors with Megalith.Blockout tag. Warns about misconfigured actors with partial Megalith tags."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::GetBlockoutVolumes),
		FParamSchemaBuilder()
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_blockout_volume_info"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::GetBlockoutVolumeInfo),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("setup_blockout_volume"),
		TEXT("Apply Megalith blockout tags to a BlockingVolume. Clears existing Megalith tags first."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::SetupBlockoutVolume),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_blockout_primitive"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::CreateBlockoutPrimitive),
		FParamSchemaBuilder()
			...
			.Optional(TEXT("volume_name"), TEXT("string"), TEXT("Owner volume name (sets Megalith.Owner tag)"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_blockout_primitives_batch"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::CreateBlockoutPrimitivesBatch),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_blockout_grid"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::CreateBlockoutGrid),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("match_asset_to_blockout"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::MatchAssetToBlockout),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("match_all_in_volume"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::MatchAllInVolume),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("apply_replacement"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::ApplyReplacement),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_actor_tags"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::SetActorTags),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("clear_blockout"),
		TEXT("Delete blockout primitives by Megalith.Owner tag. Respects keep_tagged to preserve replaced actors."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::ClearBlockout),
		FParamSchemaBuilder()
			.Required(TEXT("volume_name"), TEXT("string"), TEXT("Volume whose primitives to clear"))
			.Optional(TEXT("keep_tagged"), TEXT("boolean"), TEXT("If true, keep actors that are NOT Megalith.BlockoutPrimitive (already replaced)"), TEXT("false"))
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("export_blockout_layout"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::ExportBlockoutLayout),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("import_blockout_layout"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::ImportBlockoutLayout),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("scan_volume"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::ScanVolume),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("scatter_props"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::ScatterProps),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_blockout_blueprint"),
		TEXT("Create the BP_MegalithBlockoutVolume Blueprint asset in the project. One-time setup — creates /Game/Megalith/Blockout/BP_MegalithBlockoutVolume with editable RoomType, BlockoutTags, Density, physics, wall/ceiling properties. Drag into levels for blockout volumes with proper Details panel UX."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBlockoutActions::CreateBlockoutBlueprint),
		FParamSchemaBuilder()
			.OptionalAssetPathWithDefault(TEXT("save_path"), TEXT("Asset path to save the Blueprint"), TEXT("/Game/Megalith/Blockout/BP_MegalithBlockoutVolume"))
...
FMegalithActionResult FMegalithMeshBlockoutActions::GetBlockoutVolumes(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	{
		AActor* Actor = *It;
		bool bHasBlockoutSentinel = HasMegalithTag(Actor, TEXT("Megalith.Blockout"));
		bool bHasAnyMegalithTag = false;

		// Check for Blueprint-based blockout volume (has RoomType property + MegalithBlockout class name)
		bool bIsBPBlockout = false;
		if (!bHasBlockoutSentinel
			&& Actor->GetClass()->GetName().Contains(TEXT("MegalithBlockout"))
		...
		for (const FName& Tag : Actor->Tags)
		{
			if (Tag.ToString().StartsWith(TEXT("Megalith."), ESearchCase::IgnoreCase))
			{
				bHasAnyMegalithTag = true;
				break;
			}
		}
		...
		{
			MegalithMeshUtils::FBlockoutTags Parsed = MegalithMeshUtils::ParseBlockoutTags(Actor);
			...
			FString OwnerTag = FString::Printf(TEXT("Megalith.Owner:%s"), *Actor->GetActorNameOrLabel());
			int32 PrimitiveCount = 0;
			for (TActorIterator<AActor> PrimIt(World); PrimIt; ++PrimIt)
			{
				if (HasMegalithTag(*PrimIt, OwnerTag))
				{
					PrimitiveCount++;
				}
			}
		...
		else if (bHasAnyMegalithTag && !bHasBlockoutSentinel)
		{
			// Warn about actors with Megalith tags but no sentinel
			WarningsArr.Add(MakeShared<FJsonValueString>(
				FString::Printf(TEXT("Actor '%s' has Megalith.* tags but missing Megalith.Blockout sentinel"),
...
FMegalithActionResult FMegalithMeshBlockoutActions::GetBlockoutVolumeInfo(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: volume_name"));
	}
	...
	// Find the volume — can be any actor with Megalith.Blockout tag
	FString Error;
	AActor* VolumeActor = MegalithMeshUtils::FindActorByName(VolumeName, Error);
	if (!VolumeActor)
	{
	...
	bool bIsBlockout = HasMegalithTag(VolumeActor, TEXT("Megalith.Blockout"));
	bool bIsBPBlockout = (!bIsBlockout &&
		VolumeActor->GetClass()->FindPropertyByName(TEXT("RoomType")) != nullptr &&
		VolumeActor->GetClass()->GetName().Contains(TEXT("MegalithBlockout")));
	if (!bIsBlockout && !bIsBPBlockout)
	{
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("Actor '%s' is not a blockout volume (missing Megalith.Blockout tag or BP_MegalithBlockoutVolume class)"), *VolumeName));
	}

	MegalithMeshUtils::FBlockoutTags Parsed = MegalithMeshUtils::ParseBlockoutTags(VolumeActor);

	// Find all actors owned by this volume
	FString OwnerTag = FString::Printf(TEXT("Megalith.Owner:%s"), *VolumeName);
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (!HasMegalithTag(Actor, OwnerTag)) continue;
		...
		FString Label = GetMegalithTagValue(Actor, TEXT("Megalith.Label"));
		if (!Label.IsEmpty())
		{
			ActorObj->SetStringField(TEXT("label"), Label);
		}

		if (HasMegalithTag(Actor, TEXT("Megalith.BlockoutPrimitive")))
		{
...
FMegalithActionResult FMegalithMeshBlockoutActions::SetupBlockoutVolume(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: room_type"));
	}
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Setup Blockout Volume")));

	// Clear existing Megalith.* tags
	Volume->Tags.RemoveAll([](const FName& Tag)
	{
		return Tag.ToString().StartsWith(TEXT("Megalith."), ESearchCase::IgnoreCase);
	});

	// Apply sentinel
	Volume->Tags.Add(FName(TEXT("Megalith.Blockout")));

	// Room type
	Volume->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Room:%s"), *RoomType)));
	...
	if (Params->TryGetArrayField(TEXT("tags"), TagsArr))
	{
		...
		{
			Volume->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Tag:%s"), *Val->AsString())));
		}
	}
	...
	Volume->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Density:%s"), *Density)));
	...
	if (bAllowPhysics)
	{
		Volume->Tags.Add(FName(TEXT("Megalith.AllowPhysics")));
	}
	...
	{
		Volume->Tags.Add(FName(*FString::Printf(TEXT("Megalith.FloorHeight:%.1f"), FloorHeight)));
	}
...
FMegalithActionResult FMegalithMeshBlockoutActions::CreateBlockoutPrimitive(const TSharedPtr<FJsonObject>& Params)
{
	FString ShapeName;
	if (!Params->TryGetStringField(TEXT("shape"), ShapeName))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: shape"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: scale (array of 3 numbers)"));
	}
	...
	UStaticMesh* ShapeMesh = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(MeshPath);
	if (!ShapeMesh)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load BasicShape mesh: %s"), *MeshPath));
	}

	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Create Blockout Primitive")));
	...
	Actor->Tags.Add(FName(TEXT("Megalith.BlockoutPrimitive")));

	if (!VolumeName.IsEmpty())
	{
		Actor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Owner:%s"), *VolumeName)));
	}

	if (!Label.IsEmpty())
	{
		Actor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Label:%s"), *Label)));
		Actor->SetActorLabel(Label);
	}

	Actor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Shape:%s"), *ShapeName.ToLower())));
	Actor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Category:%s"), *Category)));
...
FMegalithActionResult FMegalithMeshBlockoutActions::CreateBlockoutPrimitivesBatch(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: primitives"));
	}
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Create Blockout Primitives Batch")));
	...
	for (int32 i = 0; i < PrimitivesArr->Num(); ++i)
	{
		...
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid primitive at index %d — expected object"), i));
		}
		...
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Primitive at index %d missing 'shape'"), i));
		}
		...
		FVector Location;
		if (!MegalithMeshUtils::ParseVector(SubParams, TEXT("location"), Location))
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Primitive at index %d missing valid 'location'"), i));
		}

		FVector Scale;
		if (!MegalithMeshUtils::ParseVector(SubParams, TEXT("scale"), Scale))
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Primitive at index %d missing valid 'scale'"), i));
		}
		...
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load mesh: %s"), *MeshPath));
		}

		FRotator Rotation(0, 0, 0);
		MegalithMeshUtils::ParseRotator(SubParams, TEXT("rotation"), Rotation);
		...
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to spawn primitive at index %d"), i));
		}
...
FMegalithActionResult FMegalithMeshBlockoutActions::CreateBlockoutGrid(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: cell_size (must be > 0)"));
	}
	...
	UStaticMesh* CubeMesh = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(TEXT("/Engine/BasicShapes/Cube.Cube"));
	if (!CubeMesh)
	{
		return FMegalithActionResult::Error(TEXT("Failed to load Cube BasicShape"));
	}

	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Create Blockout Grid")));
	...
	for (int32 xi = 0; xi < CellsX; ++xi)
	{
		for (int32 yi = 0; yi < CellsY; ++yi)
		{
			...
			Cell->Tags.Add(FName(TEXT("Megalith.BlockoutPrimitive")));
			Cell->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Owner:%s"), *VolumeName)));
			Cell->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Label:Grid_%d_%d"), xi, yi)));
			Cell->Tags.Add(FName(TEXT("Megalith.Shape:box")));
			Cell->Tags.Add(FName(TEXT("Megalith.Category:grid")));
...
FMegalithActionResult FMegalithMeshBlockoutActions::MatchAssetToBlockout(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
	if (!Params->TryGetStringField(TEXT("blockout_actor"), ActorName))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: blockout_actor"));
	}
	...
	if (!DB)
	{
		return FMegalithActionResult::Error(TEXT("Mesh catalog not available. Run megalith_reindex() first."));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Could not determine blockout actor size"));
	}

	// Parse tolerance
	double TolerancePct = GetDefault<UMegalithSettings>()->DefaultSizeMatchTolerance;
	...
	TSharedPtr<FJsonObject> CatalogResults = FMegalithMeshCatalog::SearchBySize(
	...
	FString BlockoutCategory = GetMegalithTagValue(Actor, TEXT("Megalith.Category"));
	...
	for (const auto& Val : *ResultsArr)
	{
		...
		FString BlockoutLabel = GetMegalithTagValue(Actor, TEXT("Megalith.Label"));
...
FMegalithActionResult FMegalithMeshBlockoutActions::MatchAllInVolume(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	if (!DB)
	{
		return FMegalithActionResult::Error(TEXT("No meshes indexed. Run megalith_reindex() first."));
	}

	// Check catalog isn't empty
	TSharedPtr<FJsonObject> Stats = FMegalithMeshCatalog::GetStats(*DB);
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		if (HasMegalithTag(*It, OwnerTag) && HasMegalithTag(*It, TEXT("Megalith.BlockoutPrimitive")))
		{
			Primitives.Add(*It);
		}
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("No blockout primitives found with Megalith.Owner:%s tag"), *VolumeName));
	}
	...
	for (AActor* Prim : Primitives)
	{
		...
		FMegalithActionResult MatchResult = MatchAssetToBlockout(SubParams);
...
FMegalithActionResult FMegalithMeshBlockoutActions::ApplyReplacement(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: replacements"));
	}
	...
	for (int32 i = 0; i < ReplacementsArr->Num(); ++i)
	{
		...
		FString ActorError;
		AActor* BlockoutActor = MegalithMeshUtils::FindActorByName(BlockoutName, ActorError);
		if (!BlockoutActor)
		{
			ValidationErrors.Add(MakeShared<FJsonValueString>(
		...
		FString MeshError;
		UStaticMesh* Mesh = MegalithMeshUtils::LoadStaticMesh(AssetPath, MeshError);
		if (!Mesh)
		{
			ValidationErrors.Add(MakeShared<FJsonValueString>(
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Apply Replacement")));
	...
	for (const FReplacementEntry& Entry : Entries)
	{
		...
		{
			FString TagStr = Tag.ToString();
			if (!TagStr.Equals(TEXT("Megalith.BlockoutPrimitive"), ESearchCase::IgnoreCase))
			{
				CopiedTags.Add(Tag);
			}
		}
		...
		if (!Replacement)
		{
			...
			Result->SetBoolField(TEXT("rolled_back"), true);
			return FMegalithActionResult::Error(TEXT("Failed to spawn replacement actor. All changes rolled back."));
		}
...
FMegalithActionResult FMegalithMeshBlockoutActions::SetActorTags(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: actor_tags"));
	}
	...
	for (const auto& Val : *ActorTagsArr)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Each entry must be an object with 'actor' and 'tags' fields"));
		}
		...
		{
			return FMegalithActionResult::Error(TEXT("Entry missing 'actor' field"));
		}
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Set Actor Tags")));
...
FMegalithActionResult FMegalithMeshBlockoutActions::ClearBlockout(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Clear Blockout")));
...
FMegalithActionResult FMegalithMeshBlockoutActions::ExportBlockoutLayout(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		AActor* Actor = *It;
		if (!HasMegalithTag(Actor, OwnerTag)) continue;
		if (!HasMegalithTag(Actor, TEXT("Megalith.BlockoutPrimitive"))) continue;
		...
		FString Shape = GetMegalithTagValue(Actor, TEXT("Megalith.Shape"));
		...
		FString Category = GetMegalithTagValue(Actor, TEXT("Megalith.Category"));
		if (!Category.IsEmpty())
		{
...
FMegalithActionResult FMegalithMeshBlockoutActions::ImportBlockoutLayout(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	else
	{
		...
		if (Params->TryGetStringField(TEXT("layout_json"), LayoutStr))
		{
			...
			{
				return FMegalithActionResult::Error(TEXT("Failed to parse layout_json string as JSON"));
			}
		}
		else
		{
			return FMegalithActionResult::Error(TEXT("Missing required param: layout_json (object or JSON string)"));
		}
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Layout JSON missing or empty 'primitives' array"));
	}
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Import Blockout Layout")));
	...
	for (const auto& Val : *PrimitivesArr)
	{
		...
		FVector RelativePos;
		if (!MegalithMeshUtils::ParseVector(*PrimObj, TEXT("relative_position"), RelativePos))
		{
			continue;
		}
		...
		FVector Size;
		if (!MegalithMeshUtils::ParseVector(*PrimObj, TEXT("size"), Size))
		{
			continue;
		}
		...
		MegalithMeshUtils::ParseRotator(*PrimObj, TEXT("rotation"), PrimRotation);
		...
		Actor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Shape:%s"), *Shape.ToLower())));
		Actor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Category:%s"), *Category)));
		Actor->SetFolderPath(FName(*FString::Printf(TEXT("Blockout/%s"), *VolumeName)));
...
FMegalithActionResult FMegalithMeshBlockoutActions::ScanVolume(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithScanVolume), true);
...
FMegalithActionResult FMegalithMeshBlockoutActions::ScatterProps(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: asset_paths"));
	}

	double CountD = 0;
	if (!Params->TryGetNumberField(TEXT("count"), CountD) || CountD <= 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: count (must be > 0)"));
	}
	...
	{
		FString MeshError;
		UStaticMesh* Mesh = MegalithMeshUtils::LoadStaticMesh(Val->AsString(), MeshError);
		if (!Mesh)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Asset '%s': %s"), *Val->AsString(), *MeshError));
		}
		Meshes.Add(Mesh);
	}
	...
	FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Scatter Props")));
	...
	FCollisionQueryParams FloorTraceParams(SCENE_QUERY_STAT(MegalithFloorTrace), true);
	...
	for (const FSample& Sample : Samples)
	{
		...
		if (bDoCollision)
		{
			MegalithMeshUtils::FPropPlacementResult PlacementResult =
				MegalithMeshUtils::ValidatePropPlacement(
					World,
					SpawnLocation,
					SpawnRotation.Quaternion(),
					PropHalfExtent,
		...
		PropActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Owner:%s"), *VolumeName)));
		PropActor->Tags.Add(FName(TEXT("Megalith.ScatteredProp")));
...
FMegalithActionResult FMegalithMeshBlockoutActions::CreateBlockoutBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = TEXT("/Game/Megalith/Blockout/BP_MegalithBlockoutVolume");
	...
	auto& Registry = FMegalithToolRegistry::Get();
	...
	// Step 1: Create the Blueprint
	{
		...
		if (!R.bSuccess)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create Blueprint: %s"), *R.ErrorMessage));
		}
	}
	...
	Result->SetStringField(TEXT("message"), TEXT("BP_MegalithBlockoutVolume created. Drag from Content Browser into levels. Configure RoomType, BlockoutTags, Density in Details panel."));
	return FMegalithActionResult::Success(Result);
}
