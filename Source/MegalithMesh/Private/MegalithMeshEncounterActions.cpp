...
#include "MegalithMeshLightingCapture.h"
...
namespace
{
	...
	{
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithEncounterSightline), true);
	...
	{
		MegalithMeshAnalysis::FTensionInputs Inputs;
		Inputs.AverageSightlineDistance = QuickAvgSightlineDistance(World, Location);
		Inputs.CeilingHeight = MegalithMeshAnalysis::MeasureCeilingHeight(World, Location);
		Inputs.RoomVolume = 0.0f;
		Inputs.ExitCount = 2;
		return MegalithMeshAnalysis::ComputeTensionScore(Inputs);
	}
	...
	float FullTensionScore(UWorld* World, const FVector& Location)
	{
		...
		Inputs.AverageSightlineDistance = QuickAvgSightlineDistance(World, Location);
		Inputs.CeilingHeight = MegalithMeshAnalysis::MeasureCeilingHeight(World, Location);
		Inputs.RoomVolume = MegalithMeshAnalysis::ApproximateRoomVolume(World, Location);
		Inputs.ExitCount = MegalithMeshAnalysis::CountExits(World, Location);
		return MegalithMeshAnalysis::ComputeTensionScore(Inputs);
	}
	...
	{
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithEncounterWall), true);
		...
		const FVector& RoomCenter, float RoomRadius)
	{
		...
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithEntranceDetect), true);
...
void FMegalithMeshEncounterActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 1. design_encounter
	Registry.RegisterAction(TEXT("mesh"), TEXT("design_encounter"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::DesignEncounter),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("suggest_patrol_route"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::SuggestPatrolRoute),
		FParamSchemaBuilder()
			.Required(TEXT("region"), TEXT("object"), TEXT("{ \"center\": [x,y,z], \"radius\": 2000 }"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_ai_territory"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::AnalyzeAiTerritory),
		FParamSchemaBuilder()
			.Required(TEXT("region"), TEXT("object"), TEXT("{ \"center\": [x,y,z], \"radius\": 2000 }"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("evaluate_safe_room"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::EvaluateSafeRoom),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_level_pacing_structure"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::AnalyzeLevelPacingStructure),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("generate_scare_sequence"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::GenerateScareSequence),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("validate_horror_intensity"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::ValidateHorrorIntensity),
		FParamSchemaBuilder()
			.Required(TEXT("start"), TEXT("array"), TEXT("Start position [x, y, z]"))
			.Required(TEXT("end"), TEXT("array"), TEXT("End position [x, y, z]"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("generate_hospice_report"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshEncounterActions::GenerateHospiceReport),
		FParamSchemaBuilder()
			.Required(TEXT("start"), TEXT("array"), TEXT("Start position [x, y, z]"))
			.Required(TEXT("end"), TEXT("array"), TEXT("End position [x, y, z]"))
...
FMegalithActionResult FMegalithMeshEncounterActions::DesignEncounter(const TSharedPtr<FJsonObject>& Params)
{
	FVector Center;
	float Radius;
	if (!ParseRegion(Params, Center, Radius))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: region (object with center [x,y,z] and radius)"));
	}
	...
	FString NavError;
	if (!MegalithMeshAnalysis::GetNavSystem(World, NavSys, NavData, NavError))
	{
	...
	int32 ExitCount = MegalithMeshAnalysis::CountExits(World, Center, Radius);
	...
	if (GridPoints.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No navmesh points found within region. Check that navmesh covers this area."));
	}

	// ---- 3. Gather lights for lighting analysis ----
	TArray<MegalithLightingCapture::FLightInfo> Lights = MegalithLightingCapture::GatherLights(World);
	...
	for (const FVector& Pt : GridPoints)
	{
		...
		Cand.Location = Pt;
		Cand.Concealment = MegalithMeshAnalysis::ComputeConcealment(World, Pt, CenterViewpoint);

		int32 DummyIdx;
		float Lum = MegalithLightingCapture::ComputeAnalyticLuminance(World, Pt, Lights, DummyIdx);
	...
	Result->SetStringField(TEXT("tension_level"),
		MegalithMeshAnalysis::TensionLevelToString(MegalithMeshAnalysis::ClassifyTension(RegionTension)));
...
FMegalithActionResult FMegalithMeshEncounterActions::SuggestPatrolRoute(const TSharedPtr<FJsonObject>& Params)
{
	FVector Center;
	float Radius;
	if (!ParseRegion(Params, Center, Radius))
	...
	if (GridPoints.Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("Insufficient navmesh coverage in region. Need at least 2 navigable points."));
	}
	...
	for (const FVector& Pt : GridPoints)
	{
		...
		SP.Concealment = MegalithMeshAnalysis::ComputeConcealment(World, Pt, CenterView);
	...
	{
		return FMegalithActionResult::Error(TEXT("Could not find enough spaced waypoints. Region may be too small or navmesh too sparse."));
	}
...
FMegalithActionResult FMegalithMeshEncounterActions::AnalyzeAiTerritory(const TSharedPtr<FJsonObject>& Params)
{
	FVector Center;
	float Radius;
	if (!ParseRegion(Params, Center, Radius))
	...
	if (GridPoints.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No navmesh points found within region."));
	}
	...
	for (const FVector& Pt : GridPoints)
	{
		float Concealment = MegalithMeshAnalysis::ComputeConcealment(World, Pt, CenterView);
...
FMegalithActionResult FMegalithMeshEncounterActions::EvaluateSafeRoom(const TSharedPtr<FJsonObject>& Params)
{
	FVector Center;
	float Radius;
	if (!ParseRegion(Params, Center, Radius))
	...
	TArray<MegalithLightingCapture::FLightInfo> Lights = MegalithLightingCapture::GatherLights(World);
	int32 DummyIdx;
	float CenterLum = MegalithLightingCapture::ComputeAnalyticLuminance(World, Center, Lights, DummyIdx);
	...
	if (Entrances.Num() > 0)
	{
		...
		for (const FEntranceInfo& Ent : Entrances)
		{
			...
			int32 WallCount = 0;
			float LossdB = 0.0f;
			float Occlusion = MegalithMeshAcoustics::TraceOcclusion(World, Outside, Center, WallCount, LossdB);
		...
		for (int32 d = 0; d < 4; ++d)
		{
			...
			int32 WC;
			float LdB;
			float Occ = MegalithMeshAcoustics::TraceOcclusion(World, TestPt, Center, WC, LdB);
	...
	float RoomVolume = MegalithMeshAnalysis::ApproximateRoomVolume(World, Center, Radius);
	float CeilingHeight = MegalithMeshAnalysis::MeasureCeilingHeight(World, Center);
	...
	float HospiceScore = 0.0f;
	{
		...
		TArray<MegalithMeshAnalysis::FPathClearance> Clearances;
		// Sample a few paths through the room
		for (const FEntranceInfo& Ent : Entrances)
		{
			TArray<FVector> PathPts;
			float PathDist;
			if (MegalithMeshAnalysis::FindNavPath(World, Ent.Location, Center, PathPts, PathDist))
			{
				auto C = MegalithMeshAnalysis::MeasurePathClearance(World, PathPts, 500.0f);
				Clearances.Append(C);
			}
		}
...
FMegalithActionResult FMegalithMeshEncounterActions::AnalyzeLevelPacingStructure(const TSharedPtr<FJsonObject>& Params)
{
	FVector Start, End;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("start"), Start))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: start (array of 3 numbers)"));
	}
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("end"), End))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: end (array of 3 numbers)"));
	}
	...
	float TotalDist = 0.0f;
	{
		...
		for (int32 i = 0; i < Sequence.Num() - 1; ++i)
		{
			TArray<FVector> Seg;
			float SegDist;
			if (MegalithMeshAnalysis::FindNavPath(World, Sequence[i], Sequence[i + 1], Seg, SegDist))
			{
	...
	if (FullPath.Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("Resolved path is too short for pacing analysis."));
	}
...
FMegalithActionResult FMegalithMeshEncounterActions::GenerateScareSequence(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FVector> PathPoints;
	if (!MEnc_ParseVectorArray(Params, TEXT("path_points"), PathPoints) || PathPoints.Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: path_points (array of at least 2 [x,y,z])"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Path too short for scare sequence generation. Need at least 500cm."));
	}
	...
	for (int32 i = 0; i < Sampled.Num(); ++i)
	{
		...
		int32 DummyIdx;
		float Lum = MegalithLightingCapture::ComputeAnalyticLuminance(World, Sampled[i], Lights, DummyIdx);
		S.Darkness = FMath::Clamp(1.0f - (Lum / 5.0f), 0.0f, 1.0f);
		S.EscapeCount = MegalithMeshAnalysis::CountExits(World, Sampled[i], 1500.0f, 6);
...
FMegalithActionResult FMegalithMeshEncounterActions::ValidateHorrorIntensity(const TSharedPtr<FJsonObject>& Params)
{
	FVector Start, End;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("start"), Start))
	{
	...
	TArray<FVector> PathPoints;
	float PathDist;
	if (!MegalithMeshAnalysis::FindNavPath(World, Start, End, PathPoints, PathDist))
	{
		return FMegalithActionResult::Error(TEXT("No navmesh path found. Build navigation first."));
	}
	...
	for (int32 i = 0; i < Sampled.Num(); ++i)
	{
		...
		S.EscapeRoutes = MegalithMeshAnalysis::CountExits(World, Sampled[i], 1500.0f, 6);
		Samples.Add(S);
	}
...
FMegalithActionResult FMegalithMeshEncounterActions::GenerateHospiceReport(const TSharedPtr<FJsonObject>& Params)
{
	FVector Start, End;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("start"), Start))
	{
	...
	// ============================================
	{
		...
		FMegalithActionResult IntResult = ValidateHorrorIntensity(IntensityParams);
	...
	// ============================================
	{
		...
		for (const FString& P : Profiles)
		{
			...
			if (P == TEXT("motor_impaired"))
			{
				...
				TArray<MegalithMeshAnalysis::FPathClearance> Clearances = MegalithMeshAnalysis::MeasurePathClearance(World, PathPoints, 500.0f);
				float MinWidth = 1000.0f;
				int32 NarrowCount = 0;
				for (const auto& C : Clearances)
				{
	...
	return FMegalithActionResult::Success(Report);
}
