...
void FMegalithMeshInspectionActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_mesh_info"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetMeshInfo),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_mesh_bounds"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetMeshBounds),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the mesh"))
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_mesh_materials"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetMeshMaterials),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the mesh"))
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_mesh_lods"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetMeshLods),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the mesh"))
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_mesh_collision"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetMeshCollision),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the mesh"))
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_mesh_uvs"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetMeshUvs),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the mesh"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_skeletal_mesh"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::AnalyzeSkeletalMesh),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_mesh_quality"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::AnalyzeMeshQuality),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("compare_meshes"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::CompareMeshes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_vertex_data"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetVertexData),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the mesh"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("search_meshes_by_size"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::SearchMeshesBySize),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_mesh_catalog_stats"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshInspectionActions::GetMeshCatalogStats),
...
namespace MeshInspectionHelpers
{
	...
	{
		UObject* Obj = FMegalithAssetUtils::LoadAssetByPath(AssetPath);
		if (!Obj)
		{
...
FMegalithActionResult FMegalithMeshInspectionActions::GetMeshInfo(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (UStaticMesh* SM = Cast<UStaticMesh>(Obj))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Mesh has no render data (empty or corrupt)"));
		}
		...
		Result->SetObjectField(TEXT("bounds"), MegalithMeshUtils::BoundsToJson(Bounds));
...
FMegalithActionResult FMegalithMeshInspectionActions::GetMeshBounds(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithMeshInspectionActions::GetMeshMaterials(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithMeshInspectionActions::GetMeshLods(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithMeshInspectionActions::GetMeshCollision(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithMeshInspectionActions::GetMeshUvs(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid LOD index %d"), LodIndex));
	}
	...
	if (NumTris > 500000)
	{
		UE_LOG(LogMegalith, Warning, TEXT("MegalithMesh: get_mesh_uvs on mesh with %d triangles — UV analysis may be slow"), NumTris);
	}
...
FMegalithActionResult FMegalithMeshInspectionActions::AnalyzeSkeletalMesh(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	FString Error;
	USkeletalMesh* SK = MegalithMeshUtils::LoadSkeletalMesh(AssetPath, Error);
	if (!SK)
	{
		return FMegalithActionResult::Error(Error);
	}
...
FMegalithActionResult FMegalithMeshInspectionActions::AnalyzeMeshQuality(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	if (NumTris > 1000000)
	{
		UE_LOG(LogMegalith, Warning, TEXT("MegalithMesh: analyze_mesh_quality on mesh with %d triangles — analysis may be slow"), NumTris);
	}
	...
	if (NumTris > TopologyTriangleLimit)
	{
		UE_LOG(LogMegalith, Warning,
			TEXT("MegalithMesh: analyze_mesh_quality skipping topology checks — mesh has %d triangles (limit %d)"),
	...
	else
	{
		...
		{
			UE_LOG(LogMegalith, Warning,
				TEXT("MegalithMesh: analyze_mesh_quality — GetMeshDescription(0) returned null, topology checks unavailable"));
			bTopologySkipped = true;
			NonManifoldEdges = -1;
			LooseVertices = -1;
			OpenBorderEdges = -1;
...
FMegalithActionResult FMegalithMeshInspectionActions::CompareMeshes(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("asset_path_a and asset_path_b are required"));
	}

	FString ErrorA, ErrorB;
	UObject* ObjA = MeshInspectionHelpers::LoadAnyMesh(PathA, ErrorA);
	if (!ObjA) return FMegalithActionResult::Error(ErrorA);
	UObject* ObjB = MeshInspectionHelpers::LoadAnyMesh(PathB, ErrorB);
	if (!ObjB) return FMegalithActionResult::Error(ErrorB);
...
FMegalithActionResult FMegalithMeshInspectionActions::GetVertexData(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithMeshInspectionActions::SearchMeshesBySize(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("min_bounds must be an array of 3 numbers"));
	}
	if (!Params->TryGetArrayField(TEXT("max_bounds"), MaxArr) || MaxArr->Num() != 3)
	{
		return FMegalithActionResult::Error(TEXT("max_bounds must be an array of 3 numbers"));
	}
	...
	return FMegalithActionResult::Success(FMegalithMeshCatalog::SearchBySize(*DB, MinBounds, MaxBounds, Category, ExcludeSizeClass, Limit));
}
...
FMegalithActionResult FMegalithMeshInspectionActions::GetMeshCatalogStats(const TSharedPtr<FJsonObject>& Params)
{
	FSQLiteDatabase* DB = MeshInspectionHelpers::GetCatalogDB();
	if (!DB)
	{
	...
	return FMegalithActionResult::Success(FMegalithMeshCatalog::GetStats(*DB));
}
