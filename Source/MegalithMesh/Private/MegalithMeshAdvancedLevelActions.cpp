...
namespace AdvancedLevelHelpers
{
	...
	bool ResolveActors(const TArray<TSharedPtr<FJsonValue>>& Names, TArray<AActor*>& OutActors, FString& OutError)
	{
		for (const auto& Val : Names)
		{
			...
			FString Err;
			AActor* Actor = MegalithMeshUtils::FindActorByName(Name, Err);
			if (!Actor)
			{
				OutError = Err;
				return false;
	...
	bool ParsePointOrActor(const TSharedPtr<FJsonObject>& Params, const FString& Key, FVector& OutPoint, FString& OutError)
	{
		...
		{
			if (!MegalithMeshUtils::ParseVector(Params, Key, OutPoint))
			{
...
void FMegalithMeshAdvancedLevelActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 1. manage_sublevel
	Registry.RegisterAction(TEXT("mesh"), TEXT("manage_sublevel"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::ManageSublevel),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_blueprint_actor"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::PlaceBlueprintActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("place_spline"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::PlaceSpline),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_prefab"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::CreatePrefab),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_blueprint_prefab"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::CreateBlueprintPrefab),
		FParamSchemaBuilder()
			.Required(TEXT("actor_names"), TEXT("array"), TEXT("Actor names to include in the prefab"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("spawn_prefab"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::SpawnPrefab),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("randomize_transforms"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::RandomizeTransforms),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_level_actors"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::GetLevelActors),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("measure_distance"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAdvancedLevelActions::MeasureDistance),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::ManageSublevel(const TSharedPtr<FJsonObject>& Params)
{
	FString SubAction;
	if (!Params->TryGetStringField(TEXT("sub_action"), SubAction))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: sub_action"));
	}
	...
	if (SubAction.Equals(TEXT("create"), ESearchCase::IgnoreCase))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Missing required param: level_path (for create)"));
		}
		...
		AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "CreateSublevel", "Megalith: Create Sublevel"));
		...
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create sublevel at: %s"), *LevelPath));
		}
	...
	if (SubAction.Equals(TEXT("add"), ESearchCase::IgnoreCase))
	{
		...
		if (!Params->TryGetStringField(TEXT("level_path"), LevelPath))
		{
			return FMegalithActionResult::Error(TEXT("Missing required param: level_path (for add)"));
		}

		AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "AddSublevel", "Megalith: Add Sublevel"));
		...
		{
			Transaction.Cancel();
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to add sublevel: %s"), *LevelPath));
		}
	...
	if (SubAction.Equals(TEXT("remove"), ESearchCase::IgnoreCase))
	{
		...
		if (!Params->TryGetStringField(TEXT("level_path"), LevelPath))
		{
			return FMegalithActionResult::Error(TEXT("Missing required param: level_path (for remove)"));
		}
		...
		if (!FoundLevel)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Sublevel not found: %s"), *LevelPath));
		}

		// Don't allow removing persistent level
		if (FoundLevel->GetLoadedLevel() == World->PersistentLevel)
		{
			return FMegalithActionResult::Error(TEXT("Cannot remove the persistent level"));
		}

		AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "RemoveSublevel", "Megalith: Remove Sublevel"));
	...
	if (SubAction.Equals(TEXT("move_actors"), ESearchCase::IgnoreCase))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Missing or empty required param: actor_names (for move_actors)"));
		}

		FString DestLevel;
		if (!Params->TryGetStringField(TEXT("dest_level"), DestLevel))
		{
			return FMegalithActionResult::Error(TEXT("Missing required param: dest_level (for move_actors)"));
		}
		...
		if (!DestLevelPtr)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Destination level not found: %s"), *DestLevel));
		}

		AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "MoveActorsToLevel", "Megalith: Move Actors to Level"));
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::PlaceBlueprintActor(const TSharedPtr<FJsonObject>& Params)
{
	FString BlueprintPath;
	if (!Params->TryGetStringField(TEXT("blueprint"), BlueprintPath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: blueprint"));
	}
	...
	AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "PlaceBPActor", "Megalith: Place Blueprint Actor"));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Failed to spawn Blueprint actor"));
	}
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::PlaceSpline(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: points (array of [x,y,z], minimum 2)"));
	}
	...
	if (!MeshPath.IsEmpty())
	{
		SplineMesh = FMegalithAssetUtils::LoadAssetByPath<UStaticMesh>(MeshPath);
		if (!SplineMesh)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Mesh not found: %s"), *MeshPath));
		}
	}
	...
	MegalithMeshUtils::ParseVector(Params, TEXT("location"), ActorLocation);
	...
	AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "PlaceSpline", "Megalith: Place Spline"));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Failed to spawn spline actor"));
	}
	...
	if (SplineMesh)
	{
		...
		if (NumSegments > 50)
		{
			UE_LOG(LogMegalith, Warning, TEXT("place_spline: %d segments — each is a separate draw call. Consider reducing point count."), NumSegments);
		}
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::CreatePrefab(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* ActorNamesJson = nullptr;
	if (!Params->TryGetArrayField(TEXT("actor_names"), ActorNamesJson) || !ActorNamesJson || ActorNamesJson->Num() == 0)
	{
	...
	if (!LISubsystem)
	{
		return FMegalithActionResult::Error(TEXT("LevelInstanceSubsystem not available — ensure Level Instance editor support is enabled"));
	}
	...
	AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "CreatePrefab", "Megalith: Create Prefab"));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Failed to create Level Instance. The engine's CreateLevelInstanceFrom triggers a Save As dialog (bUseSaveAs=true) which blocks MCP calls. Use spawn_prefab with an existing prefab instead, or create Level Instances manually in the editor."));
	}
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Level Instance created but could not be cast to AActor"));
	}
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::SpawnPrefab(const TSharedPtr<FJsonObject>& Params)
{
	FString PrefabPath;
	if (!Params->TryGetStringField(TEXT("prefab_path"), PrefabPath))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: prefab_path"));
	}
	...
	AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "SpawnPrefab", "Megalith: Spawn Prefab"));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Failed to spawn LevelInstance actor"));
	}
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::RandomizeTransforms(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* ActorNamesJson = nullptr;
	if (!Params->TryGetArrayField(TEXT("actor_names"), ActorNamesJson) || !ActorNamesJson || ActorNamesJson->Num() == 0)
	{
	...
	AdvancedLevelHelpers::FScopedMeshTransaction Transaction(NSLOCTEXT("Megalith", "RandomizeTransforms", "Megalith: Randomize Transforms"));
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::GetLevelActors(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	MegalithMeshUtils::ParseVector(Params, TEXT("center"), Center);
	...
	{
		FString Err;
		AActor* VolumeActor = MegalithMeshUtils::FindActorByName(VolumeName, Err);
		if (!VolumeActor)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Volume actor not found: %s"), *VolumeName));
		}
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::MeasureDistance(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(FromError);
	}
	if (!AdvancedLevelHelpers::ParsePointOrActor(Params, TEXT("to"), ToPoint, ToError))
	{
		return FMegalithActionResult::Error(ToError);
	}
	...
	if (bIncludeNavmesh)
	{
		...
		if (World)
		{
			...
			float PathDist = 0.0f;
			if (MegalithMeshAnalysis::FindNavPath(World, FromPoint, ToPoint, PathPoints, PathDist))
			{
...
FMegalithActionResult FMegalithMeshAdvancedLevelActions::CreateBlueprintPrefab(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* ActorNamesJson = nullptr;
	if (!Params->TryGetArrayField(TEXT("actor_names"), ActorNamesJson) || !ActorNamesJson || ActorNamesJson->Num() == 0)
	{
	...
	if (Actors.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No valid actors resolved from actor_names"));
	}

	AdvancedLevelHelpers::FScopedMeshTransaction Transaction(
		NSLOCTEXT("Megalith", "CreateBlueprintPrefab", "Megalith: Create Blueprint Prefab"));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("HarvestBlueprintFromActors failed — could not create Blueprint from the given actors"));
	}
