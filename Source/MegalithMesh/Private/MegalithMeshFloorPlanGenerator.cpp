...
DECLARE_LOG_CATEGORY_EXTERN(LogMegalithFloorPlan, Log, All);
DEFINE_LOG_CATEGORY(LogMegalithFloorPlan);

// ============================================================================
// Registration
// ============================================================================

void FMegalithMeshFloorPlanGenerator::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("generate_floor_plan"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFloorPlanGenerator::GenerateFloorPlan),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_building_archetypes"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFloorPlanGenerator::ListBuildingArchetypes),
		FParamSchemaBuilder()
			.Build());
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_building_archetype"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshFloorPlanGenerator::GetBuildingArchetype),
		FParamSchemaBuilder()
...
FMegalithMeshFloorPlanGenerator::EAdjacencyRelation
FMegalithMeshFloorPlanGenerator::FAdjacencyMatrix::GetRule(const FString& TypeA, const FString& TypeB) const
{
	// Check A->B
...
bool FMegalithMeshFloorPlanGenerator::FAdjacencyMatrix::ViolatesMustNot(const FString& TypeA, const FString& TypeB) const
{
...
FMegalithMeshFloorPlanGenerator::EPrivacyZone
FMegalithMeshFloorPlanGenerator::GetPrivacyZone(const FString& RoomType)
{
	// PUBLIC
...
bool FMegalithMeshFloorPlanGenerator::IsWetRoom(const FString& RoomType)
{
...
TSet<FIntPoint> FMegalithMeshFloorPlanGenerator::BuildPlumbingChaseSet(
...
TArray<TPair<int32, int32>> FMegalithMeshFloorPlanGenerator::FindMustNotViolations(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
...
TArray<TPair<int32, int32>> FMegalithMeshFloorPlanGenerator::FindUnmetMustRequirements(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	const TArray<FRoomDef>& Rooms, const FAdjacencyMatrix& Matrix)
{
...
bool FMegalithMeshFloorPlanGenerator::TrySwapRooms(
...
int32 FMegalithMeshFloorPlanGenerator::ValidateAndFixAdjacency(
	TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	TArray<FRoomDef>& Rooms, const FAdjacencyMatrix& Matrix, int32 MaxRetries)
{
	for (int32 Retry = 0; Retry < MaxRetries; ++Retry)
	{
		...
		if (Violations.Num() == 0)
		{
			UE_LOG(LogMegalithFloorPlan, Log, TEXT("Adjacency validation passed (retry %d): 0 MUST_NOT violations"), Retry);
			return 0;
		}

		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Adjacency validation retry %d: %d MUST_NOT violation(s), attempting swaps"),
		...
		for (const auto& Violation : Violations)
		{
			...
			for (int32 Candidate = 0; Candidate < Rooms.Num(); ++Candidate)
			{
				...
				{
					UE_LOG(LogMegalithFloorPlan, Log, TEXT("  Swapped '%s' <-> '%s' to fix violation (violations: %d -> %d)"),
		...
		if (!bFixedAny)
		{
			...
			{
				UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Unresolved MUST_NOT violation: '%s' (%s) adjacent to '%s' (%s)"),
...
TMap<int32, int32> FMegalithMeshFloorPlanGenerator::ComputeGraphDepthFromEntry(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	const TArray<FRoomDef>& Rooms)
{
...
int32 FMegalithMeshFloorPlanGenerator::ValidatePrivacyGradient(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	const TArray<FRoomDef>& Rooms, TArray<FString>& OutViolations)
{
	...
	for (int32 i = 0; i < Rooms.Num(); ++i)
	{
		...
		if (Zone == EPrivacyZone::PRIVATE && RoomDepth < 2)
		{
			...
			UE_LOG(LogMegalithFloorPlan, Warning, TEXT("%s"), *Msg);
			++ViolationCount;
		}
...
FMegalithMeshFloorPlanGenerator::ECirculationType
FMegalithMeshFloorPlanGenerator::ParseCirculationType(const FString& TypeStr)
{
...
void FMegalithMeshFloorPlanGenerator::InsertCorridorsForCirculation(
...
void FMegalithMeshFloorPlanGenerator::InsertCorridorsHubSpoke(
	TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	TArray<FRoomDef>& Rooms, const TArray<FAdjacencyRule>& Adjacency,
	bool bHospiceMode, FRandomStream& Rng)
{
	...
	{
		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Hub-spoke: all %d rooms adjacent to hub '%s', no corridor needed"),
	...
	if (CorridorRoom.GridCells.Num() > 0)
	{
		...
		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Hub-spoke: corridor with %d cells connects %d unreachable room(s) to hub"),
			Rooms.Last().GridCells.Num(), Unreachable.Num());
	}
}

void FMegalithMeshFloorPlanGenerator::InsertCorridorsRacetrack(
	TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	TArray<FRoomDef>& Rooms, const TArray<FAdjacencyRule>& Adjacency,
	bool bHospiceMode, FRandomStream& Rng)
{
	...
	if (CorridorRoom.GridCells.Num() > 0)
	{
		...
		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Racetrack: loop corridor with %d cells, core=(%d,%d)-(%d,%d)"),
...
void FMegalithMeshFloorPlanGenerator::InsertCorridorsEnfilade(
	TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	TArray<FRoomDef>& Rooms, const TArray<FAdjacencyRule>& Adjacency,
	bool bHospiceMode, FRandomStream& Rng)
{
	...
	if (Disconnected.Num() > 0)
	{
		...
		{
			Rooms.Add(MoveTemp(CorridorRoom));
			UE_LOG(LogMegalithFloorPlan, Log, TEXT("Enfilade: minimal corridor with %d cells to connect %d disconnected room(s)"),
				Rooms.Last().GridCells.Num(), Disconnected.Num());
		}
	}
	else
	{
		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Enfilade: all %d rooms connected directly, no corridor needed"), Rooms.Num());
	}
}
...
FString FMegalithMeshFloorPlanGenerator::GetArchetypeRoofType(const FString& ArchetypeName)
{
...
FString FMegalithMeshFloorPlanGenerator::GetArchetypeDirectory()
{
	return FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("Megalith"), TEXT("Saved"), TEXT("Megalith"), TEXT("BuildingArchetypes"));
}

bool FMegalithMeshFloorPlanGenerator::LoadArchetype(const FString& ArchetypeName, FBuildingArchetype& OutArchetype, FString& OutError)
{
	FString FilePath;
	...
	TSharedPtr<FJsonObject> Json = FMegalithJsonUtils::Parse(JsonString);
	if (!Json.IsValid())
	{
...
bool FMegalithMeshFloorPlanGenerator::ParseArchetypeJson(const TSharedPtr<FJsonObject>& Json, FBuildingArchetype& OutArchetype, FString& OutError)
{
	...
	if (Json->TryGetObjectField(TEXT("adjacency_matrix"), AdjMatObj) && AdjMatObj && (*AdjMatObj).IsValid())
	{
		...
		{
			const FString RoomTypeA = MegalithKeyToString(OuterPair.Key);
			...
			{
				const FString RoomTypeB = MegalithKeyToString(InnerPair.Key);
				FString RelStr;
...
TArray<FMegalithMeshFloorPlanGenerator::FRoomInstance> FMegalithMeshFloorPlanGenerator::ResolveRoomInstances(
...
FString FMegalithMeshFloorPlanGenerator::ValidateFootprintCapacity(
...
FString FMegalithMeshFloorPlanGenerator::ValidateStairwellRequirement(const FBuildingArchetype& Archetype)
{
...
void FMegalithMeshFloorPlanGenerator::CorrectAspectRatios(
	TArray<FGridRect>& Rects, const TArray<FRoomInstance>& Rooms, int32 GridW, int32 GridH)
{
	for (int32 i = 0; i < Rects.Num() && i < Rooms.Num(); ++i)
	{
		...
		{
			UE_LOG(LogMegalithFloorPlan, Log, TEXT("Correcting aspect ratio for '%s': %dx%d (%.1f) -> %dx%d (%.1f)"),
...
float FMegalithMeshFloorPlanGenerator::WorstAspectRatio(const TArray<float>& RowAreas, float SideLength)
{
...
void FMegalithMeshFloorPlanGenerator::LayoutRow(const TArray<int32>& RowIndices, const TArray<float>& Areas,
...
TArray<FMegalithMeshFloorPlanGenerator::FGridRect> FMegalithMeshFloorPlanGenerator::SquarifiedTreemapLayout(
...
TArray<TArray<int32>> FMegalithMeshFloorPlanGenerator::BuildGridFromRects(
...
TArray<FRoomDef> FMegalithMeshFloorPlanGenerator::BuildRoomDefs(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
...
bool FMegalithMeshFloorPlanGenerator::RoomsShareEdge(const FRoomDef& A, const FRoomDef& B)
{
...
TArray<FIntPoint> FMegalithMeshFloorPlanGenerator::FindCorridorPath(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
...
void FMegalithMeshFloorPlanGenerator::InsertCorridors(
	TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	TArray<FRoomDef>& Rooms, const TArray<FAdjacencyRule>& Adjacency,
	bool bHospiceMode, FRandomStream& Rng)
{
...
void FMegalithMeshFloorPlanGenerator::InsertRestAlcoves(
	TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
...
TArray<FIntPoint> FMegalithMeshFloorPlanGenerator::FindSharedEdge(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
...
TArray<FDoorDef> FMegalithMeshFloorPlanGenerator::PlaceDoors(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
	...
	const TArray<FRoomDef>& Rooms, TArray<FDoorDef>& Doors, bool bHospiceMode, FRandomStream& Rng)
{
	...
	if (PerimeterRoomIndices.IsEmpty())
	{
		UE_LOG(LogMegalithFloorPlan, Warning, TEXT("EnsureExteriorEntrance: no rooms touch building perimeter -- cannot place entrance"));
		return;
	}
	...
	if (BestCell.X < 0)
	{
		UE_LOG(LogMegalithFloorPlan, Warning, TEXT("EnsureExteriorEntrance: could not find perimeter cell on room '%s'"), *Rooms[BestRoomIdx].RoomId);
		return;
	}
	...
	if (SameWallCells.Num() > 1)
	{
		...
		if (HasPerpendicularWall(BestCell))
		{
			...
			if (!bFound)
			{
				UE_LOG(LogMegalithFloorPlan, Warning, TEXT("EnsureExteriorEntrance: all perimeter cells have perpendicular walls -- using midpoint anyway"));
			}
		}
	}
	...
	UE_LOG(LogMegalithFloorPlan, Log, TEXT("Placed exterior entrance: room='%s', wall=%s, cell=(%d,%d)"),
	...
	const TArray<FRoomDef>& Rooms, TArray<FDoorDef>& Doors, float CellSize)
{
	...
	for (FDoorDef& Door : Doors)
	{
		...
		{
			UE_LOG(LogMegalithFloorPlan, Warning,
	...
	if (WarningCount > 0)
	{
		UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Door clearance validation: %d warning(s) across %d doors"), WarningCount, Doors.Num());
	}
	else
	{
		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Door clearance validation: all %d doors passed"), Doors.Num());
	}
}
...
TSharedPtr<FJsonObject> FMegalithMeshFloorPlanGenerator::BuildOutputJson(
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH,
...
FString FMegalithMeshFloorPlanGenerator::GetTemplateDirectory()
{
	return FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("Megalith"), TEXT("Saved"), TEXT("Megalith"), TEXT("FloorPlanTemplates"));
}

FString FMegalithMeshFloorPlanGenerator::ArchetypeToTemplateCategory(const FString& ArchetypeName)
{
...
bool FMegalithMeshFloorPlanGenerator::LoadFloorPlanTemplate(
	...
	TArray<FStairwellDef>& OutStairwells, FString& OutError)
{
	...
	UE_LOG(LogMegalithFloorPlan, Log, TEXT("Loaded template '%s/%s': grid=%dx%d, %d rooms, %d doors, %d stairwells"),
		*Category, *TemplateName, OutGridW, OutGridH, OutRooms.Num(), OutDoors.Num(), OutStairwells.Num());

	return true;
}

FString FMegalithMeshFloorPlanGenerator::SelectTemplate(
	...
	const TSet<FString>& ExcludeTemplates)
{
	...
	for (const FString& File : Files)
	{
		...
		TSharedPtr<FJsonObject> Json = FMegalithJsonUtils::Parse(JsonStr);
	...
	for (int32 i = 0; i < PoolSize; ++i)
	{
		...
		if (Roll <= Accum)
		{
			UE_LOG(LogMegalithFloorPlan, Log, TEXT("Selected template '%s' from category '%s' (score=%.3f, pool=%d)"),
...
bool FMegalithMeshFloorPlanGenerator::ScaleTemplateGrid(
	...
	FString& OutError)
{
	...
	for (int32 i = 0; i < InOutRooms.Num(); ++i)
	{
		...
		if (MinDim < 3)
		{
			UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Post-scale: corridor '%s' narrowed to %d cells (min 3). Template may produce narrow passages."),
	...
	{
		UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Post-scale: no rooms touch exterior perimeter. Entrance may need re-placement."));
	}

	UE_LOG(LogMegalithFloorPlan, Log, TEXT("Scaled template grid from %dx%d to %dx%d (scale %.2fx%.2f)"),
...
FMegalithActionResult FMegalithMeshFloorPlanGenerator::GenerateFloorPlan(const TSharedPtr<FJsonObject>& Params)
{
	// ---- Parse params ----
	FString ArchetypeName;
	if (!Params->TryGetStringField(TEXT("archetype"), ArchetypeName) || ArchetypeName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required 'archetype' parameter"));

	double FootprintW = 0, FootprintH = 0;
	if (!Params->TryGetNumberField(TEXT("footprint_width"), FootprintW) || FootprintW <= 0)
		return FMegalithActionResult::Error(TEXT("Missing or invalid 'footprint_width' (must be > 0)"));
	if (!Params->TryGetNumberField(TEXT("footprint_height"), FootprintH) || FootprintH <= 0)
		return FMegalithActionResult::Error(TEXT("Missing or invalid 'footprint_height' (must be > 0)"));
	...
	{
		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Hospice mode: capping horror_level from %.2f to 0.3"), HorrorLevel);
		HorrorLevel = 0.3f;
	}
		...
		if (!ResolvedTemplate.IsEmpty() && !ResolvedCategory.IsEmpty())
		{
				...
				TemplateRooms, TemplateDoors, TemplateStairwells, TemplateError))
			{
				...
				if (TargetGridW != TemplateGridW || TargetGridH != TemplateGridH)
				{
					...
					{
						UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Template scaling failed (%s), using original template size"), *ScaleError);
				...
				UE_LOG(LogMegalithFloorPlan, Log,
			...
			{
				UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Template load failed: %s — falling back to treemap"), *TemplateError);
			}
		}
		else if (!TemplateError.IsEmpty())
		{
			UE_LOG(LogMegalithFloorPlan, Log, TEXT("Template selection: %s — falling back to treemap"), *TemplateError);
		}
	}
	...
	if (!StairwellError.IsEmpty())
		return FMegalithActionResult::Error(StairwellError);
	...
	{
		UE_LOG(LogMegalithFloorPlan, Warning, TEXT("%s — will generate with reduced room set"), *CapacityError);
		// Don't return error — let ResolveRoomInstances drop optional rooms to fit
	}

	UE_LOG(LogMegalithFloorPlan, Log, TEXT("Generating floor plan: archetype=%s, grid=%dx%d, cell=%.0f, seed=%d, hospice=%d, floor=%d, circulation=%d"),
	...
	if (Instances.Num() == 0)
		return FMegalithActionResult::Error(TEXT("Archetype produced zero room instances for this floor. Check per-floor room assignments."));
	...
	for (int32 i = 0; i < Rects.Num(); ++i)
	{
		...
		{
			UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Room '%s' has extreme aspect ratio %.1f (%dx%d, max=%.1f) -- may produce awkward geometry"),
	...
	if (Archetype.AdjacencyMatrix.Rules.Num() > 0)
	{
		...
		{
			UE_LOG(LogMegalithFloorPlan, Warning, TEXT("Unmet MUST adjacency: '%s' (%s) should be adjacent to '%s' (%s)"),
	...
	UE_LOG(LogMegalithFloorPlan, Log, TEXT("Floor plan generated: %d rooms, %d doors, seed=%d, floor=%d, adj_violations=%d, privacy_violations=%d, wet_clustered=%d/%d, horror=%.2f, locked=%d, horror_score=%.3f"),
...
FMegalithActionResult FMegalithMeshFloorPlanGenerator::ListBuildingArchetypes(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithMeshFloorPlanGenerator::GetBuildingArchetype(const TSharedPtr<FJsonObject>& Params)
{
	FString ArchetypeName;
	if (!Params->TryGetStringField(TEXT("archetype"), ArchetypeName) || ArchetypeName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required 'archetype' parameter"));
	...
	FString JsonString;
	if (!FFileHelper::LoadFileToString(JsonString, *FilePath))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Archetype not found: %s"), *FilePath));

	TSharedPtr<FJsonObject> Json = FMegalithJsonUtils::Parse(JsonString);
	if (!Json.IsValid())
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to parse archetype JSON: %s"), *FilePath));
...
int32 FMegalithMeshFloorPlanGenerator::FindRoomIndexById(const TArray<FRoomDef>& Rooms, const FString& RoomId)
{
	for (int32 i = 0; i < Rooms.Num(); ++i)
	{
...
TArray<TSet<int32>> FMegalithMeshFloorPlanGenerator::BuildRoomAdjacencyFromDoors(
	const TArray<FRoomDef>& Rooms, const TArray<FDoorDef>& Doors,
...
int32 FMegalithMeshFloorPlanGenerator::BFSShortestPath(const TArray<TSet<int32>>& Adj, int32 From, int32 To)
{
	if (From == To) return 0;
...
bool FMegalithMeshFloorPlanGenerator::IsGraphConnected(const TArray<TSet<int32>>& Adj, int32 NumRooms)
{
...
TSet<int32> FMegalithMeshFloorPlanGenerator::FindBridgeDoors(
	const TArray<FRoomDef>& Rooms, const TArray<FDoorDef>& Doors,
...
TSet<int32> FMegalithMeshFloorPlanGenerator::ApplyDoorLocking(
	const TArray<FRoomDef>& Rooms, const TArray<FDoorDef>& Doors,
	float LockRatio, FRandomStream& Rng)
{
	...
	UE_LOG(LogMegalithFloorPlan, Log, TEXT("Horror: locked %d/%d doors (target ratio %.0f%%)"),
...
TSet<int32> FMegalithMeshFloorPlanGenerator::AdjustDeadEndRatio(
	const TArray<FRoomDef>& Rooms, const TArray<FDoorDef>& Doors,
	TSet<int32> LockedDoors, float TargetRatio, FRandomStream& Rng)
{
	...
	UE_LOG(LogMegalithFloorPlan, Log, TEXT("Horror: dead-end ratio adjusted to %.1f%% (target %.1f%%)"),
...
TSet<int32> FMegalithMeshFloorPlanGenerator::ApplyLoopBreaking(
	const TArray<FRoomDef>& Rooms, const TArray<FDoorDef>& Doors,
	TSet<int32> LockedDoors, FRandomStream& Rng)
{
	...
	for (int32 i = 0; i < LoopDoors.Num(); ++i)
	{
		...
		UE_LOG(LogMegalithFloorPlan, Verbose, TEXT("Horror: loop-break locked %s"), *Doors[DoorIdx].DoorId);
...
int32 FMegalithMeshFloorPlanGenerator::ApplyWrongRoomInjection(
	TArray<FRoomDef>& Rooms, FRandomStream& Rng)
{
	...
	for (int32 c = 0; c < Count && CandidateRooms.Num() > 0; ++c)
	{
		...
		UE_LOG(LogMegalithFloorPlan, Log, TEXT("Horror: wrong-room injection '%s' -> '%s' (room %s)"),
...
TSet<int32> FMegalithMeshFloorPlanGenerator::ApplyHorrorModifiers(
...
void FMegalithMeshFloorPlanGenerator::ComputeSpaceSyntax(
	const TArray<FRoomDef>& Rooms, const TArray<FDoorDef>& Doors,
	const TSet<int32>& LockedDoors,
	TArray<FRoomSpaceSyntax>& OutPerRoom, FSpaceSyntaxScores& OutScores)
{
...
float FMegalithMeshFloorPlanGenerator::ComputeRoomTension(
...
void FMegalithMeshFloorPlanGenerator::EmitHorrorAndSpaceSyntaxJson(
