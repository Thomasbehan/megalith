...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithDebugView, Log, All);
...
void FMegalithMeshDebugViewActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("toggle_section_view"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDebugViewActions::ToggleSectionView),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("toggle_ceiling_visibility"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDebugViewActions::ToggleCeilingVisibility),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("capture_floor_plan"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDebugViewActions::CaptureFloorPlan),
		FParamSchemaBuilder()
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Full file path for PNG (default: auto-generated in Saved/Megalith/Captures/)"))
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("highlight_room"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDebugViewActions::HighlightRoom),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("save_camera_bookmark"),
		TEXT("Save the current editor viewport camera position and rotation as a named bookmark. "
			"Stored as JSON in Saved/Megalith/CameraBookmarks/."),
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDebugViewActions::SaveCameraBookmark),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("load_camera_bookmark"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDebugViewActions::LoadCameraBookmark),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("capture_building_views"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshDebugViewActions::CaptureBuildingViews),
		FParamSchemaBuilder()
			.Required(TEXT("building_id"), TEXT("string"), TEXT("Building ID in spatial registry"))
			.Optional(TEXT("block_id"), TEXT("string"), TEXT("Block ID for spatial registry"), TEXT("default"))
			.Optional(TEXT("resolution"), TEXT("integer"), TEXT("Image resolution in pixels (square)"), TEXT("512"))
			.OptionalDiskPath(TEXT("output_dir"), TEXT("Directory for PNG output (default: Saved/Megalith/Captures/)"))
...
FString FMegalithMeshDebugViewActions::GetBookmarkDirectory()
{
	return FPaths::ProjectSavedDir() / TEXT("Megalith") / TEXT("CameraBookmarks");
}

FString FMegalithMeshDebugViewActions::GetCapturesDirectory()
{
	return FPaths::ProjectSavedDir() / TEXT("Megalith") / TEXT("Captures");
}

bool FMegalithMeshDebugViewActions::EnsureDirectory(const FString& Path)
{
...
FMegalithActionResult FMegalithMeshDebugViewActions::ToggleSectionView(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("'enabled' (boolean) is required"));
	}
	...
	if (Params->TryGetStringField(TEXT("building_id"), BuildingId) && !BuildingId.IsEmpty())
	{
		...
		if (FMegalithMeshSpatialRegistry::HasBlock(BlockId))
		{
			const FSpatialBlock& Block = FMegalithMeshSpatialRegistry::GetBlock(BlockId);
...
FMegalithActionResult FMegalithMeshDebugViewActions::ToggleCeilingVisibility(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("'visible' (boolean) is required"));
	}
...
FMegalithActionResult FMegalithMeshDebugViewActions::CaptureFloorPlan(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	if (!Params->TryGetStringField(TEXT("building_id"), BuildingId) || BuildingId.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'building_id' (string) is required"));
	}
	...
	if (!FloorBounds.IsValid)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Could not determine building bounds for capture"));
		}
	}
...
FMegalithActionResult FMegalithMeshDebugViewActions::HighlightRoom(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("'room_id' (string) is required"));
	}
	...
	SpawnParams.Name = FName(*FString::Printf(TEXT("MegalithHighlight_%s"), *RoomId));
	...
	if (!HighlightActor)
	{
		return FMegalithActionResult::Error(TEXT("Failed to spawn highlight actor"));
	}

	// Set actor label and tags
	HighlightActor->SetActorLabel(FString::Printf(TEXT("Highlight_%s"), *RoomId));
	HighlightActor->Tags.Add(FName(TEXT("Megalith.DebugHighlight")));
	HighlightActor->Tags.Add(FName(*FString::Printf(TEXT("Megalith.Room:%s"), *RoomId)));
	...
	if (!CubeMesh)
	{
		...
		return FMegalithActionResult::Error(TEXT("Failed to load /Engine/BasicShapes/Cube mesh"));
	}
...
FMegalithActionResult FMegalithMeshDebugViewActions::SaveCameraBookmark(const TSharedPtr<FJsonObject>& Params)
{
	FString Name;
	if (!Params->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'name' (string) is required"));
	}
	...
	if (!ViewportClient)
	{
		return FMegalithActionResult::Error(TEXT("No active editor viewport found"));
	}
...
FMegalithActionResult FMegalithMeshDebugViewActions::LoadCameraBookmark(const TSharedPtr<FJsonObject>& Params)
{
	FString Name;
	if (!Params->TryGetStringField(TEXT("name"), Name) || Name.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Bookmark missing valid 'location' array"));
	}
	FVector Location(
	...
	{
		return FMegalithActionResult::Error(TEXT("Bookmark missing valid 'rotation' array"));
	}
	FRotator Rotation(
...
bool FMegalithMeshDebugViewActions::ComputeBuildingBounds(
...
TArray<AActor*> FMegalithMeshDebugViewActions::HideCeilingActors(UWorld* World, const FString& BuildingId)
{
...
void FMegalithMeshDebugViewActions::RestoreHiddenActors(const TArray<AActor*>& Actors)
{
...
bool FMegalithMeshDebugViewActions::CaptureViewToPNG(
	UWorld* World,
...
FMegalithActionResult FMegalithMeshDebugViewActions::CaptureBuildingViews(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	for (const FViewSpec& View : Views)
	{
		...
		if (!bOk)
		{
			UE_LOG(LogMegalithDebugView, Warning,
	...
	if (ViewsCaptured == 0)
	{
		return FMegalithActionResult::Error(TEXT("All view captures failed — check editor world and render state"));
	}
