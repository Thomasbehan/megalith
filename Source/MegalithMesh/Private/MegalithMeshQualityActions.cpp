...
TArray<TSharedPtr<FJsonValue>> FMegalithMeshQualityActions::VectorToJsonArray(const FVector& V)
{
...
void FMegalithMeshQualityActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("validate_naming_conventions"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::ValidateNamingConventions),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("batch_rename_assets"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::BatchRenameAssets),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("generate_proxy_mesh"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::GenerateProxyMesh),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("setup_hlod"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::SetupHlod),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_texture_budget"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::AnalyzeTextureBudget),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_framing"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::AnalyzeFraming),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("evaluate_monster_reveal"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::EvaluateMonsterReveal),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_co_op_balance"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::AnalyzeCoOpBalance),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("integration_hooks_stub"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshQualityActions::IntegrationHooksStub),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshQualityActions::ValidateNamingConventions(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Params->TryGetObjectField(TEXT("custom_rules"), CustomRulesObj) && CustomRulesObj->IsValid())
	{
		for (const auto& Pair : (*CustomRulesObj)->Values)
		{
			...
			{
				PrefixRules.Add(MegalithKeyToString(Pair.Key), Prefix);
			}
		}
	}
...
FMegalithActionResult FMegalithMeshQualityActions::BatchRenameAssets(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (PathsArr->Num() > 200)
	{
		return FMegalithActionResult::Error(TEXT("Too many assets to rename (max 200)"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Must specify at least one rename operation (find/replace, add_prefix, remove_prefix, add_suffix, remove_suffix)"));
	}
	...
	for (const TSharedPtr<FJsonValue>& PathVal : *PathsArr)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Each entry in asset_paths must be a non-empty string"));
		}
		...
		if (NewName.IsEmpty())
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Rename would result in empty name for %s"), *AssetPath));
		}
...
FMegalithActionResult FMegalithMeshQualityActions::GenerateProxyMesh(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Required: actor_names (array of at least 2 actor names to merge)"));
	}

	if (ActorNamesArr->Num() > 100)
	{
		return FMegalithActionResult::Error(TEXT("Too many actors to merge (max 100)"));
	}

	FString SavePath;
	if (!Params->TryGetStringField(TEXT("save_path"), SavePath) || SavePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Required: save_path (asset path for saved proxy mesh)"));
	}
	...
	for (const TSharedPtr<FJsonValue>& NameVal : *ActorNamesArr)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Each entry in actor_names must be a non-empty string"));
		}

		FString FindError;
		AActor* Actor = MegalithMeshUtils::FindActorByName(ActorName, FindError);
		if (!Actor)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Actor not found: %s — %s"), *ActorName, *FindError));
		}
		...
		if (SMCs.Num() == 0)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Actor '%s' has no StaticMeshComponents"), *ActorName));
		}
	...
	if (Components.Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("Need at least 2 static mesh components to merge"));
	}
	...
	if (!Package)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create package: %s"), *PackageName));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Merge produced no output assets. Check that actors have valid static meshes."));
	}
...
FMegalithActionResult FMegalithMeshQualityActions::SetupHlod(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath;
	if (!Params->TryGetStringField(TEXT("save_path"), SavePath) || SavePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Required: save_path (asset path for HLOD layer)"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid layer_type: %s. Use MeshMerge, MeshSimplify, MeshApproximate, or Custom"), *LayerTypeStr));
	}
	...
	if (!HLODLayer)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create UHLODLayer"));
	}
...
FMegalithActionResult FMegalithMeshQualityActions::AnalyzeTextureBudget(const TSharedPtr<FJsonObject>& Params)
{
	FString ScanPath;
	Params->TryGetStringField(TEXT("scan_path"), ScanPath);
...
FMegalithActionResult FMegalithMeshQualityActions::AnalyzeFraming(const TSharedPtr<FJsonObject>& Params)
{
	FVector CamLocation;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("camera_location"), CamLocation))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: camera_location"));
	}

	FRotator CamRotation;
	if (!MegalithMeshUtils::ParseRotator(Params, TEXT("camera_rotation"), CamRotation))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: camera_rotation"));
	}
	...
	{
		FString FindErr;
		AActor* FocalActor = MegalithMeshUtils::FindActorByName(FocalActorName, FindErr);
		if (FocalActor)
		{
...
FMegalithActionResult FMegalithMeshQualityActions::EvaluateMonsterReveal(const TSharedPtr<FJsonObject>& Params)
{
	FVector PlayerLoc;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("player_location"), PlayerLoc))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: player_location"));
	}

	FRotator PlayerRot;
	if (!MegalithMeshUtils::ParseRotator(Params, TEXT("player_rotation"), PlayerRot))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: player_rotation"));
	}

	FString MonsterName;
	if (!Params->TryGetStringField(TEXT("monster_actor"), MonsterName) || MonsterName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Required: monster_actor (name of the creature actor)"));
	}
	...
	FString FindErr;
	AActor* MonsterActor = MegalithMeshUtils::FindActorByName(MonsterName, FindErr);
	if (!MonsterActor)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Monster actor not found: %s — %s"), *MonsterName, *FindErr));
	}
...
FMegalithActionResult FMegalithMeshQualityActions::AnalyzeCoOpBalance(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Required: player_positions (array of at least 2 player positions [[x,y,z], ...])"));
	}

	if (PositionsArr->Num() > 8)
	{
		return FMegalithActionResult::Error(TEXT("Maximum 8 player positions supported"));
	}
	...
	for (const TSharedPtr<FJsonValue>& PosVal : *PositionsArr)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Each player position must be [x, y, z]"));
		}
...
FMegalithActionResult FMegalithMeshQualityActions::IntegrationHooksStub(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Unknown hook_name: %s. Use ai_director, gas_tension, telemetry, or all"), *HookName));
	}
