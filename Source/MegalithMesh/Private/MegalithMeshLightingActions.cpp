...
void FMegalithMeshLightingActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("sample_light_levels"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLightingActions::SampleLightLevels),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_dark_corners"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLightingActions::FindDarkCorners),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_light_transitions"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLightingActions::AnalyzeLightTransitions),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_light_coverage"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLightingActions::GetLightCoverage),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("suggest_light_placement"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshLightingActions::SuggestLightPlacement),
		FParamSchemaBuilder()
...
namespace
{
	...
	bool FindVolumeBounds(const TSharedPtr<FJsonObject>& Params, FVector& OutCenter, FVector& OutExtent, FString& OutError)
	{
		...
		{
			AActor* Volume = MegalithMeshUtils::FindActorByName(VolumeName, OutError);
			if (!Volume)
			{
				return false;
			}
	...
	TSharedPtr<FJsonObject> LightInfoToJson(const MegalithLightingCapture::FLightInfo& L)
	{
...
FMegalithActionResult FMegalithMeshLightingActions::SampleLightLevels(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FVector> Points;
	if (!MLight_ParseVectorArray(Params, TEXT("points"), Points))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: points (array of [x,y,z] arrays)"));
	}
	...
	TArray<MegalithLightingCapture::FLightInfo> Lights;
	if (bDoAnalytic)
	{
		Lights = MegalithLightingCapture::GatherLights(World);
	}
	...
	if (bDoCapture)
	{
		...
		if (Points.Num() > 10)
		{
			...
			if (MegalithLightingCapture::CapturePointsAtlas(World, Points, TileSize, TilesPerRow,
				AtlasPixels, AtlasW, AtlasH))
			{
				for (int32 i = 0; i < Points.Num(); ++i)
				{
					CaptureLuminances[i] = MegalithLightingCapture::ExtractTileLuminance(
						AtlasPixels, AtlasW, i, TileSize, TilesPerRow);
					CaptureColors[i] = MegalithLightingCapture::ExtractTileColor(
						AtlasPixels, AtlasW, i, TileSize, TilesPerRow);
				}
			}
		}
		...
		{
			// Individual captures
			UTextureRenderTarget2D* RT = MegalithLightingCapture::CreateTransientRT(8, 8);
			USceneCaptureComponent2D* Capture = MegalithLightingCapture::CreateLightingCapture(World, RT);

			for (int32 i = 0; i < Points.Num(); ++i)
			{
				TArray<FLinearColor> Pixels;
				if (MegalithLightingCapture::CaptureAtPoint(Capture, Points[i], FRotator(-90.0f, 0.0f, 0.0f), Pixels))
				{
					CaptureLuminances[i] = MegalithLightingCapture::AverageLuminance(Pixels);
					CaptureColors[i] = MegalithLightingCapture::AverageColor(Pixels);
				}
			}

			MegalithLightingCapture::CleanupCapture(Capture);
		}
	}
	...
	for (int32 i = 0; i < Points.Num(); ++i)
	{
		...
		if (bDoCapture)
		{
			...
			Sample->SetNumberField(TEXT("color_temperature"),
				MegalithLightingCapture::EstimateColorTemperature(CaptureColors[i]));
		}

		if (bDoAnalytic)
		{
			int32 DominantIdx = -1;
			float AnalyticLum = MegalithLightingCapture::ComputeAnalyticLuminance(World, Points[i], Lights, DominantIdx);
			...
			if (DominantIdx >= 0 && DominantIdx < Lights.Num())
			{
				...
				Sample->SetNumberField(TEXT("dominant_light_contribution"),
					MegalithLightingCapture::ComputeAnalyticLuminance(World, Points[i],
						TArray<MegalithLightingCapture::FLightInfo>{Lights[DominantIdx]}, DomDummy));
				...
				float UnshadowedLum = MegalithLightingCapture::ComputeAnalyticLuminance(
		...
		{
			int32 Dummy;
			FinalLum = MegalithLightingCapture::ComputeAnalyticLuminance(World, Points[i], Lights, Dummy);
		}
...
FMegalithActionResult FMegalithMeshLightingActions::FindDarkCorners(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	MegalithLightingCapture::FCoverageResult Coverage =
		MegalithLightingCapture::CaptureCoverageMap(World, Center, Extent, Resolution,
			static_cast<float>(Threshold), 0.2f);

	if (Coverage.LuminanceGrid.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Scene capture failed — ensure editor viewport is rendering"));
	}
	...
	TArray<MegalithLightingCapture::FDarkZone> DarkZones =
		MegalithLightingCapture::FloodFillDarkZones(Coverage.LuminanceGrid, Coverage.GridWidth, Coverage.GridHeight,
			static_cast<float>(Threshold), WorldMin, CellSize, 4);

	// Sort by area descending
	DarkZones.Sort([](const MegalithLightingCapture::FDarkZone& A, const MegalithLightingCapture::FDarkZone& B)
	{
	...
	for (const MegalithLightingCapture::FDarkZone& Zone : DarkZones)
	{
...
FMegalithActionResult FMegalithMeshLightingActions::AnalyzeLightTransitions(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FVector> PathPoints;
	if (!MLight_ParseVectorArray(Params, TEXT("path_points"), PathPoints) || PathPoints.Num() < 2)
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: path_points (need at least 2 points)"));
	}
...

	{
		...
		{
			TArray<FLinearColor> Pixels;
			if (MegalithLightingCapture::CaptureAtPoint(Capture, SamplePoints[i], FRotator(-90.0f, 0.0f, 0.0f), Pixels))
			{
				Luminances[i] = MegalithLightingCapture::AverageLuminance(Pixels);
			}
		}
...
FMegalithActionResult FMegalithMeshLightingActions::GetLightCoverage(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName) || VolumeName.IsEmpty())
	{
	...
	FString Error;
	AActor* Volume = MegalithMeshUtils::FindActorByName(VolumeName, Error);
	if (!Volume)
	{
		return FMegalithActionResult::Error(Error);
	}
	...
	if (Coverage.LuminanceGrid.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Scene capture failed"));
	}
	...
	TArray<MegalithLightingCapture::FLightInfo> Lights =
		MegalithLightingCapture::GatherLightsInRegion(World, Region);

	TArray<TSharedPtr<FJsonValue>> LightArr;
	int32 ShadowCasterCount = 0;
	for (const MegalithLightingCapture::FLightInfo& L : Lights)
	{
...
FMegalithActionResult FMegalithMeshLightingActions::SuggestLightPlacement(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName) || VolumeName.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: mood (horror_dim, safe_room, clinical, ambient)"));
	}
	Mood = Mood.ToLower();
	...
	FBox Region(Center - Extent, Center + Extent);
	TArray<MegalithLightingCapture::FLightInfo> ExistingLights =
		MegalithLightingCapture::GatherLightsInRegion(World, Region);
	...
	for (int32 gy = 1; gy <= GridSize; ++gy)
	{
		for (int32 gx = 1; gx <= GridSize; ++gx)
		{
			...
			int32 Dummy;
			S.AnalyticLuminance = MegalithLightingCapture::ComputeAnalyticLuminance(
	...
	for (const FGridSample& S : GridSamples)
	{
		...
		for (const MegalithLightingCapture::FLightInfo& L : ExistingLights)
		{
