...
FString FMegalithMeshTemplateActions::GetTemplatesDirectory()
{
...
TSharedPtr<FJsonObject> FMegalithMeshTemplateActions::LoadTemplate(const FString& TemplateName, FString& OutError)
{
...
bool FMegalithMeshTemplateActions::SaveTemplate(const FString& TemplateName, const TSharedPtr<FJsonObject>& TemplateJson, FString& OutError)
{
	FString TemplateDir = GetTemplatesDirectory();
	FString FilePath = TemplateDir / TemplateName + TEXT(".json");
...
bool FMegalithMeshTemplateActions::ParseJsonArrayToVector(const TArray<TSharedPtr<FJsonValue>>& Arr, FVector& Out)
{
...
void FMegalithMeshTemplateActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("list_room_templates"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::ListRoomTemplates),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_room_template"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::GetRoomTemplate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("apply_room_template"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::ApplyRoomTemplate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_room_template"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::CreateRoomTemplate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("validate_game_ready"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::ValidateGameReady),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("suggest_lod_strategy"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::SuggestLodStrategy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("batch_validate"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::BatchValidate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("compare_lod_chain"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshTemplateActions::CompareLodChain),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path of the StaticMesh"))
			.Build());
}
...
FMegalithActionResult FMegalithMeshTemplateActions::ListRoomTemplates(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithMeshTemplateActions::GetRoomTemplate(const TSharedPtr<FJsonObject>& Params)
{
	FString TemplateName;
	if (!Params->TryGetStringField(TEXT("template_name"), TemplateName))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: template_name"));
	}
	...
	return FMegalithActionResult::Success(TemplateJson);
}
...
FMegalithActionResult FMegalithMeshTemplateActions::ApplyRoomTemplate(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	...
	{
		FString FindError;
		Volume = MegalithMeshUtils::FindActorByName(VolumeName, FindError);
		if (!Volume)
		{
	...
	{
		return FMegalithActionResult::Error(TEXT("Template has no furniture entries"));
	}
	...
	if (!GEditor)
	{
		return FMegalithActionResult::Error(TEXT("No editor available"));
	}
	GEditor->BeginTransaction(FText::FromString(TEXT("Megalith: Apply Room Template")));
...
FMegalithActionResult FMegalithMeshTemplateActions::CreateRoomTemplate(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		for (const FName& Tag : Actor->Tags)
		{
			if (Tag.ToString().StartsWith(TEXT("Megalith.BlockoutPrimitive"), ESearchCase::IgnoreCase))
			{
				bIsPrimitive = true;
				break;
			}
		...
		for (const FName& Tag : Actor->Tags)
		{
			...
			if (TagStr.StartsWith(TEXT("Megalith.Shape:"), ESearchCase::IgnoreCase))
			{
				int32 ColonIdx;
			...
			else if (TagStr.StartsWith(TEXT("Megalith.Label:"), ESearchCase::IgnoreCase))
			{
				int32 ColonIdx;
				if (TagStr.FindChar(TEXT(':'), ColonIdx))
				{
			...
			else if (TagStr.StartsWith(TEXT("Megalith.Category:"), ESearchCase::IgnoreCase))
			{
				int32 ColonIdx;
				if (TagStr.FindChar(TEXT(':'), ColonIdx))
				{
