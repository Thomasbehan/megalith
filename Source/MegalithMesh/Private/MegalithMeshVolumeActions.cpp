...
UClass* FMegalithMeshVolumeActions::ResolveVolumeClass(const FString& TypeStr, FString& OutError)
{
...
bool FMegalithMeshVolumeActions::ExportPropertyValue(const FProperty* Prop, const void* ContainerPtr, FString& OutValue)
{
...
bool FMegalithMeshVolumeActions::ImportPropertyValue(FProperty* Prop, void* ContainerPtr, const FString& Value, UObject* OwnerObject)
{
	if (!Prop || !ContainerPtr)
	{
		return false;
...
void FMegalithMeshVolumeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 1. spawn_volume
	Registry.RegisterAction(TEXT("mesh"), TEXT("spawn_volume"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshVolumeActions::SpawnVolume),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_actor_properties"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshVolumeActions::GetActorProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("copy_actor_properties"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshVolumeActions::CopyActorProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("build_navmesh"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshVolumeActions::BuildNavmesh),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("select_actors"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshVolumeActions::SelectActors),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("snap_to_surface"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshVolumeActions::SnapToSurface),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_collision_preset"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshVolumeActions::SetCollisionPreset),
		FParamSchemaBuilder()
			.Required(TEXT("actor_name"), TEXT("string"), TEXT("Actor name or label"))
...
FMegalithActionResult FMegalithMeshVolumeActions::SpawnVolume(const TSharedPtr<FJsonObject>& Params)
{
	FString TypeStr;
	...
	MegalithMeshUtils::ParseVector(Params, TEXT("extent"), Extent);

	if (Extent.X <= 0 || Extent.Y <= 0 || Extent.Z <= 0)
	{
		return FMegalithActionResult::Error(TEXT("All extent values must be positive"));
	}
	...
	VolumeActionHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Spawn Volume")));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to spawn volume of type '%s'"), *TypeStr));
	}
...
FMegalithActionResult FMegalithMeshVolumeActions::GetActorProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
...
FMegalithActionResult FMegalithMeshVolumeActions::CopyActorProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString SourceName;
	if (!Params->TryGetStringField(TEXT("source_actor"), SourceName))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: source_actor"));
	}

	const TArray<TSharedPtr<FJsonValue>>* TargetsArr;
	if (!Params->TryGetArrayField(TEXT("target_actors"), TargetsArr) || TargetsArr->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param: target_actors"));
	}

	FString Error;
	AActor* SourceActor = MegalithMeshUtils::FindActorByName(SourceName, Error);
	if (!SourceActor)
	{
		return FMegalithActionResult::Error(Error);
	}
	...
	for (const auto& Val : *TargetsArr)
	{
		...
		FString TargetError;
		AActor* Target = MegalithMeshUtils::FindActorByName(TargetName, TargetError);
		if (!Target)
		{
			return FMegalithActionResult::Error(TargetError);
		}
	...
	VolumeActionHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Copy Actor Properties")));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("No copyable properties found on source"));
	}
...
FMegalithActionResult FMegalithMeshVolumeActions::BuildNavmesh(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	if (!NavSys)
	{
		return FMegalithActionResult::Error(TEXT("NavigationSystem not found. Ensure a NavMeshBoundsVolume exists in the level."));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Invalid mode. Use 'full' or 'dirty_only'."));
	}
...
FMegalithActionResult FMegalithMeshVolumeActions::SelectActors(const TSharedPtr<FJsonObject>& Params)
{
	FString SubAction;
	...
	if (Params->TryGetArrayField(TEXT("actors"), ActorsArr))
	{
		...
		{
			FString Error;
			AActor* Actor = MegalithMeshUtils::FindActorByName(Val->AsString(), Error);
			if (Actor)
			{
	...
	if (Params->TryGetObjectField(TEXT("filter"), FilterObj))
	{
		...
		if (bHasRadial)
		{
			MegalithMeshUtils::ParseVector(*FilterObj, TEXT("center"), Center);
		}
	...
	if (SubAction.Equals(TEXT("focus"), ESearchCase::IgnoreCase))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("No actors to focus on"));
		}
	...
	if (ResolvedActors.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No actors matched for selection"));
	}
...
FMegalithActionResult FMegalithMeshVolumeActions::SnapToSurface(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* ActorsArr;
	...
	{
		return FMegalithActionResult::Error(TEXT("direction must not be zero"));
	}
	Direction.Normalize();
	...
	if (TraceLength <= 0)
	{
		return FMegalithActionResult::Error(TEXT("trace_length must be positive"));
	}
	...
	VolumeActionHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Snap to Surface")));
	...
	for (AActor* Actor : Actors)
	{
		...
		FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MegalithSnapToSurface), true);
...
FMegalithActionResult FMegalithMeshVolumeActions::SetCollisionPreset(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
	if (!Params->TryGetStringField(TEXT("actor_name"), ActorName))
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: preset"));
	}
	...
	else
	{
		...
		if (!PrimitiveComp)
		{
			return FMegalithActionResult::Error(TEXT("Actor's root component is not a PrimitiveComponent — cannot set collision. Specify a component name."));
		}
	}
	...
	VolumeActionHelpers::FScopedMeshTransaction Transaction(FText::FromString(TEXT("Megalith: Set Collision Preset")));
