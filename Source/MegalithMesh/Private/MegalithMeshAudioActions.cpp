...
#include "MegalithMeshAnalysis.h"
...
namespace
{
	...
	{
		return MegalithMeshAnalysis::VectorToJsonArray(V);
	}
	...
	{
		FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithFloorTrace), true);
	...
	AActor* FindVolumeByName(UWorld* World, const FString& VolumeName, FString& OutError)
	{
		...
		AActor* Actor = MegalithMeshUtils::FindActorByName(VolumeName, OutError);
		return Actor;
	}
	...
	TSharedPtr<FJsonObject> AcousticPropsToJson(const MegalithMeshAcoustics::FAcousticProperties& Props)
	{
...
void FMegalithMeshAudioActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// === Read-Only (7) ===
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_audio_volumes"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::GetAudioVolumes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_surface_materials"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::GetSurfaceMaterials),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("estimate_footstep_sound"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::EstimateFootstepSound),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_room_acoustics"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::AnalyzeRoomAcoustics),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("analyze_sound_propagation"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::AnalyzeSoundPropagation),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_loud_surfaces"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::FindLoudSurfaces),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_sound_paths"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::FindSoundPaths),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("can_ai_hear_from"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::CanAiHearFrom),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("get_stealth_map"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::GetStealthMap),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("find_quiet_path"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::FindQuietPath),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("suggest_audio_volumes"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::SuggestAudioVolumes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_audio_volume"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::CreateAudioVolume),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("set_surface_type"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::SetSurfaceType),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_surface_datatable"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshAudioActions::CreateSurfaceDataTable),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithMeshAudioActions::GetAudioVolumes(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
...
FMegalithActionResult FMegalithMeshAudioActions::GetSurfaceMaterials(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	else
	{
		...
		if (MegalithMeshUtils::ParseVector(Params, TEXT("region_min"), RegionMin)
			&& MegalithMeshUtils::ParseVector(Params, TEXT("region_max"), RegionMax))
		{
	...
	if (!bHasRegion)
	{
		return FMegalithActionResult::Error(TEXT("Specify volume_name or region_min/region_max"));
	}
	...
	FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithSurfaceMaterials), true);
	QP.bReturnPhysicalMaterial = true;

	// Sample from multiple origins within the volume
	TMap<FString, int32> MaterialCounts;
	TMap<FString, MegalithMeshAcoustics::FAcousticProperties> MaterialProps;
	int32 TotalHits = 0;
	...
	for (const FVector& SampleOrigin : Origins)
	{
		for (int32 i = 0; i < RaysPerOrigin; ++i)
		{
			...
			if (World->LineTraceSingleByChannel(Hit, SampleOrigin, SampleOrigin + Dir * MaxDist, ECC_Visibility, QP))
			{
				...
				auto Props = MegalithMeshAcoustics::GetPropertiesForSurface(Surface);
...
FMegalithActionResult FMegalithMeshAudioActions::EstimateFootstepSound(const TSharedPtr<FJsonObject>& Params)
{
	FVector Location;
...
FMegalithActionResult FMegalithMeshAudioActions::AnalyzeRoomAcoustics(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithRoomAcoustics), true);
	QP.bReturnPhysicalMaterial = true;
	...
	for (const auto& Pair : MaterialHits)
	{
		...
		auto Props = MegalithMeshAcoustics::GetPropertiesForName(Pair.Key);
		TotalAbsorption += AreaM2 * Props.Absorption;
	}

	// Sabine RT60
	float RT60 = MegalithMeshAcoustics::ComputeSabineRT60(VolumeM3, TotalAbsorption);
	auto AcousticType = MegalithMeshAcoustics::ClassifyRT60(RT60);
	...
	Result->SetStringField(TEXT("classification"), MegalithMeshAcoustics::AcousticTypeToString(AcousticType));
	...
	// Reverb suggestion
	auto Suggestion = MegalithMeshAcoustics::SuggestReverbSettings(RT60, MaterialFractions);
...
FMegalithActionResult FMegalithMeshAudioActions::AnalyzeSoundPropagation(const TSharedPtr<FJsonObject>& Params)
{
	FVector From, To;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("from"), From))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: from"));
	}
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("to"), To))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: to"));
	}
	...
	float DistAtten = MegalithMeshAcoustics::ComputeDistanceAttenuation(Distance);
	...
	if (bIncludeOcclusion)
	{
		int32 WallCount;
		float TotalLossdB;
		float OcclusionFactor = MegalithMeshAcoustics::TraceOcclusion(World, From, To, WallCount, TotalLossdB);
		...
		auto IndirectResult = MegalithMeshAcoustics::FindIndirectNavmeshPath(World, From, To);
...
FMegalithActionResult FMegalithMeshAudioActions::FindLoudSurfaces(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithMeshUtils::GetEditorWorld();
	if (!World)
	{
	...
	FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithLoudSurface), true);
	QP.bReturnPhysicalMaterial = true;
...
FMegalithActionResult FMegalithMeshAudioActions::FindSoundPaths(const TSharedPtr<FJsonObject>& Params)
{
	FVector From, To;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("from"), From))
	{
	...
	auto SoundPaths = MegalithMeshAcoustics::FindSoundPaths(World, From, To, MaxBounces, CandidateSurfaces);
...
FMegalithActionResult FMegalithMeshAudioActions::CanAiHearFrom(const TSharedPtr<FJsonObject>& Params)
{
	FVector AiLocation, PlayerLocation;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("ai_location"), AiLocation))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: ai_location"));
	}
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("player_location"), PlayerLocation))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: player_location"));
	}
	...
	MegalithMeshAcoustics::FAcousticProperties FloorProps;

	if (Params->TryGetStringField(TEXT("surface_type"), SurfaceOverride) && !SurfaceOverride.IsEmpty())
	{
		FloorProps = MegalithMeshAcoustics::GetPropertiesForName(SurfaceOverride);
	}
	else
	{
		FHitResult Hit;
		EPhysicalSurface Surface = TraceFloorSurface(World, PlayerLocation, Hit);
		FloorProps = MegalithMeshAcoustics::GetPropertiesForSurface(Surface);
	}
	...
	int32 WallCount;
	float TotalLossdB;
	float OcclusionFactor = MegalithMeshAcoustics::TraceOcclusion(World, PlayerLocation, AiLocation, WallCount, TotalLossdB);
	...
	auto IndirectResult = MegalithMeshAcoustics::FindIndirectNavmeshPath(World, PlayerLocation, AiLocation);
	if (IndirectResult.bFound)
	{
		bIndirectFound = true;
...
FMegalithActionResult FMegalithMeshAudioActions::GetStealthMap(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithStealthMap), true);
	QP.bReturnPhysicalMaterial = true;
...
FMegalithActionResult FMegalithMeshAudioActions::FindQuietPath(const TSharedPtr<FJsonObject>& Params)
{
	FVector Start, End;
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("start"), Start))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: start"));
	}
	if (!MegalithMeshUtils::ParseVector(Params, TEXT("end"), End))
	{
		return FMegalithActionResult::Error(TEXT("Missing or invalid required param: end"));
	}
	...
	float MainDist;
	if (!MegalithMeshAnalysis::FindNavPath(World, Start, End, MainPath, MainDist))
	{
		return FMegalithActionResult::Error(TEXT("No navmesh path found between start and end"));
	}

	FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithQuietPath), true);
	QP.bReturnPhysicalMaterial = true;
...
FMegalithActionResult FMegalithMeshAudioActions::SuggestAudioVolumes(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FCollisionQueryParams QP(SCENE_QUERY_STAT(MegalithSuggestAudio), true);
	QP.bReturnPhysicalMaterial = true;
...
FMegalithActionResult FMegalithMeshAudioActions::CreateAudioVolume(const TSharedPtr<FJsonObject>& Params)
{
	FString VolumeName;
	if (!Params->TryGetStringField(TEXT("volume_name"), VolumeName))
	{
	...
	FScopedAudioTransaction Transaction(FText::FromString(TEXT("Megalith: Create Audio Volume")));
	...
	{
		Transaction.Cancel();
		return FMegalithActionResult::Error(TEXT("Failed to spawn AAudioVolume"));
	}
...
FMegalithActionResult FMegalithMeshAudioActions::SetSurfaceType(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: surface_type"));
	}
	...
	FScopedAudioTransaction Transaction(FText::FromString(TEXT("Megalith: Set Surface Type")));
...
FMegalithActionResult FMegalithMeshAudioActions::CreateSurfaceDataTable(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		SavePath = GetDefault<UMegalithSettings>()->SurfaceAcousticsTablePath;
	}

	if (SavePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("No save_path specified and no default in MegalithSettings"));
	}
	...
	FScopedAudioTransaction Transaction(FText::FromString(TEXT("Megalith: Create Surface DataTable")));
	...
	auto Defaults = MegalithMeshAcoustics::GetHardcodedDefaults();
