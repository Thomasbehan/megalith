...
UMegalithMeshHandlePool* FMegalithMeshBuildingActions::Pool = nullptr;

void FMegalithMeshBuildingActions::SetHandlePool(UMegalithMeshHandlePool* InPool)
{
	Pool = InPool;
}
...
void FMegalithMeshBuildingActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_building_from_grid"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBuildingActions::CreateBuildingFromGrid),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("mesh"), TEXT("create_grid_from_rooms"),
		...
		FMegalithActionHandler::CreateStatic(&FMegalithMeshBuildingActions::CreateGridFromRooms),
		FParamSchemaBuilder()
...
bool FMegalithMeshBuildingActions::ParseGrid(const TSharedPtr<FJsonObject>& Params,
...
bool FMegalithMeshBuildingActions::ParseRooms(const TArray<TSharedPtr<FJsonValue>>& RoomsArr,
...
bool FMegalithMeshBuildingActions::ParseDoors(const TArray<TSharedPtr<FJsonValue>>& DoorsArr,
...
bool FMegalithMeshBuildingActions::ParseStairwells(const TArray<TSharedPtr<FJsonValue>>& StairArr,
...
bool FMegalithMeshBuildingActions::IsDoorEdge(int32 FixedAxis, int32 RunPos, bool bVertical, const TArray<FDoorDef>& Doors)
{
...
TArray<FMegalithMeshBuildingActions::FWallSegment> FMegalithMeshBuildingActions::BuildWallSegments(
...
void FMegalithMeshBuildingActions::GenerateWallGeometry(UDynamicMesh* Mesh,
...
void FMegalithMeshBuildingActions::GenerateSlabs(UDynamicMesh* Mesh,
...
void FMegalithMeshBuildingActions::CutDoorOpenings(UDynamicMesh* Mesh, const TArray<FDoorDef>& Doors,
...
void FMegalithMeshBuildingActions::AddDoorTrim(UDynamicMesh* Mesh, const TArray<FDoorDef>& Doors,
	float CellSize, float FloorZ, float ExteriorT, float InteriorT,
	const TArray<TArray<int32>>& Grid, int32 GridW, int32 GridH)
{
	if (Doors.Num() == 0) return;
...
void FMegalithMeshBuildingActions::GenerateEntranceDoorFrames(UDynamicMesh* Mesh,
...
void FMegalithMeshBuildingActions::GenerateStairGeometry(UDynamicMesh* Mesh,
...
void FMegalithMeshBuildingActions::ComputeRoomBounds(TArray<FRoomDef>& Rooms,
...
void FMegalithMeshBuildingActions::ComputeDoorPositions(TArray<FDoorDef>& Doors,
...
TArray<FVector2D> FMegalithMeshBuildingActions::ComputeFootprint(
...
void FMegalithMeshBuildingActions::GenerateIntegratedFacade(UDynamicMesh* Mesh,
...
{
	using FFacadeStyle = FMegalithMeshFacadeActions::FFacadeStyle;
	using FWindowPlacement = FMegalithMeshFacadeActions::FWindowPlacement;
	using FDoorPlacement = FMegalithMeshFacadeActions::FDoorPlacement;

	// Load facade style
	FFacadeStyle Style;
	if (!FMegalithMeshFacadeActions::LoadFacadeStyle(FacadeStyleName, Style))
	{
	...
	for (const FExteriorFaceDef& Face : ExteriorFaces)
	{
		...
		if (bIsGroundFloor)
		{
			WinXPositions = FMegalithMeshFacadeActions::ComputeWindowPositions(
		...
		FMegalithMeshFacadeActions::CutOpeningsSelectionInset(
		...
		// 6. Add window frames
		FMegalithMeshFacadeActions::AddWindowFrames(Mesh, Face, Windows, Style, WallThickness);

		// 7. Add door frames
		FMegalithMeshFacadeActions::AddDoorFrames(Mesh, Face, Doors, Style, WallThickness);

		// 8. Add glass panes
		FMegalithMeshFacadeActions::AddGlassPanes(Mesh, Face, Windows, Style.GlassMaterialId);

		// 9. Belt course above ground floor
		if (bIsGroundFloor && MaxFloorIndex > 0)
		{
			FMegalithMeshFacadeActions::AddBeltCourse(Mesh, Face, Face.Height, Style);
		}

		// 10. Cornice on top floor
		if (Face.FloorIndex == MaxFloorIndex)
		{
			FMegalithMeshFacadeActions::AddCornice(Mesh, Face, Face.Height, Style);
		}

		// 11. Emit window metadata into descriptor
		FVector WidthAxis = FMegalithMeshFacadeActions::GetFaceWidthAxis(Face);
...
FMegalithActionResult FMegalithMeshBuildingActions::CreateBuildingFromGrid(const TSharedPtr<FJsonObject>& Params)
{
	if (!Pool)
	{
		return FMegalithActionResult::Error(GS_ERROR_BUILDING);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: rooms"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: doors"));
	}
	...
	FMegalithMeshProceduralActions::CleanupMesh(Mesh, bHadBooleans);
	...
	AActor* Actor = FMegalithMeshProceduralActions::PlaceMeshInScene(SavePath, Location, Rotation, Label, bSnapToFloor, Folder);
	...
	if (Actor)
	{
		...
		if (bHasFacadeStyle)
		{
			...
			Actor->Tags.Add(FName(TEXT("Megalith.Procedural")));
...
FMegalithActionResult FMegalithMeshBuildingActions::CreateGridFromRooms(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* RoomsArr = nullptr;
	if (!Params->TryGetArrayField(TEXT("rooms"), RoomsArr) || !RoomsArr || RoomsArr->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty 'rooms' array"));
	}
	...
	for (int32 i = 0; i < RoomsArr->Num(); ++i)
	{
		...
		if (!(*RoomsArr)[i]->TryGetObject(RoomObj) || !RoomObj || !(*RoomObj).IsValid())
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("rooms[%d] is not an object"), i));
		}

		FRoomRect Rect;
		if (!(*RoomObj)->TryGetStringField(TEXT("room_id"), Rect.RoomId))
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("rooms[%d] missing 'room_id'"), i));
		}
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("rooms[%d] missing x, y, width, or height"), i));
		}
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("rooms[%d] has invalid dimensions"), i));
		}
