 ...
 *
 * Delegates mesh creation to existing create_parametric_mesh via FMegalithToolRegistry::ExecuteAction.
 * Reads room data from SP6 spatial registry (FMegalithMeshSpatialRegistry).
 * Furniture presets loaded from JSON files in Saved/Megalith/FurniturePresets/.
 */
class FMegalithMeshFurnishingActions
{
public:
...
private:
	...
	static FMegalithActionResult FurnishRoom(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult FurnishBuilding(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult ListFurniturePresets(const TSharedPtr<FJsonObject>& Params);
