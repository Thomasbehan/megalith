 ...
 *
 * All downstream SP calls go through FMegalithToolRegistry::ExecuteAction so SP5 stays
 * decoupled from SP1/SP2/SP3/SP4/SP6 — graceful skip when actions don't exist.
 */
class FMegalithMeshCityBlockActions
{
public:
...
private:
	...
	// ---- Action handlers ----
	static FMegalithActionResult CreateCityBlock(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult CreateLotLayout(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult CreateStreet(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult PlaceStreetFurniture(const TSharedPtr<FJsonObject>& Params);
