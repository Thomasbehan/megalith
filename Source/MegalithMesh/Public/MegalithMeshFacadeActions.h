 ...
 *
 * Public static utilities are shared with FMegalithMeshBuildingActions for integrated
 * facade generation (v3 single-pass architecture).
 */
class FMegalithMeshFacadeActions
{
public:
...
private:
	...
	// ---- Action handlers ----
	static FMegalithActionResult GenerateFacade(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult ListFacadeStyles(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult ApplyHorrorDamage(const TSharedPtr<FJsonObject>& Params);
