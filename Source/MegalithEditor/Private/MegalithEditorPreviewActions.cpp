...
//
// Phase 3 of plan: 2026-05-26-megalith-editor-preview-expansion.md.
//
...
// MegalithEditorActions.cpp::RenderAndSaveCapture. Declarations live in the
// public MegalithEditorActions.h header (Phase 3 block); registrations live
// in MegalithEditorActions.cpp::RegisterActions.
//
...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithPreviewActions, Log, All);
...
namespace
{
	...
	/** Build default timestamp-suffixed output path under Saved/Screenshots/Megalith/<Bucket>/. */
	static FString DefaultOutputPath(const TCHAR* Bucket)
	{
		const FString Timestamp = FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S"));
		return FPaths::ProjectDir() / TEXT("Saved/Screenshots/Megalith") / Bucket /
...
FMegalithActionResult FMegalithEditorActions::HandleCaptureMaterialGrid(
	const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("Params object is null"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("material_paths array is required"));
	}
	if (MaterialPathsArr->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("material_paths array is empty"));
	}
	if (MaterialPathsArr->Num() > 16)
	{
		return FMegalithActionResult::Error(TEXT("material_paths array exceeds 16-entry limit"));
	}
	...
	for (const TSharedPtr<FJsonValue>& Val : *MaterialPathsArr)
	{
		...
		if (!Material)
		{
			UE_LOG(LogMegalithPreviewActions, Warning,
	...
	if (!World)
	{
		return FMegalithActionResult::Error(TEXT("PreviewScene has no UWorld"));
	}
	...
	if (!bSuccess)
	{
		return FMegalithActionResult::Error(TEXT("capture_material_grid: render or save failed"));
	}
...
FMegalithActionResult FMegalithEditorActions::HandleCaptureWithOverlay(
	const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
	...
	if (!bSuccess)
	{
		return FMegalithActionResult::Error(TEXT("capture_with_overlay: render or save failed"));
	}
