...
#include "MegalithStructFieldResolver.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithPieObject, Log, All);

namespace MegalithPieObject
{
	UWorld* FindPieWorld()
	{
		if (!GEditor)
		...
		const UObject* OwnerForExport, FString& OutTypeName)
	{
		...
		// MegalithAnimation runtime ReadResolvedValue.
		FString Exported;
	...
	TSharedPtr<FJsonValue> ReadDottedValue(UObject* Obj, const FString& Path, FString& OutTypeName)
	{
		...
		const MegalithStructField::FResolved Resolved = MegalithStructField::Resolve(Obj, Path);
		if (!Resolved.Leaf)
		{
...
namespace
{
	using namespace MegalithPieObject;
		...
		const TSharedPtr<FJsonObject>& Json, UObject* Owner, int32 Depth, FString& OutError)
	{
		...
		{
			const TPair<FString, TSharedPtr<FJsonValue>> Field(MegalithKeyToString(FieldIt.Key), FieldIt.Value);
			// Resolve the friendly field name within THIS struct to its internal property.
			// (The shared MegalithStructField resolver needs a UObject container; a bare
...
void FMegalithPieObjectActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_get_object_properties"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetObjectProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_call_function"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCallFunction),
		FParamSchemaBuilder()
	...
	UE_LOG(LogMegalithPieObject, Log, TEXT("MegalithEditor: registered 2 live-PIE object actions"));
}

// ── pie_get_object_properties ──────────────────────────────────────────

FMegalithActionResult FMegalithPieObjectActions::HandleGetObjectProperties(const TSharedPtr<FJsonObject>& Params)
{
	const MegalithPieObject::FResolvedObject Resolved = MegalithPieObject::Resolve(Params);
	if (!Resolved.bSuccess)
	{
		return FMegalithActionResult::Error(Resolved.Error);
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("pie_get_object_properties requires a non-empty 'properties' array of dotted paths"));
	}
	...
	for (const TSharedPtr<FJsonValue>& V : *PropsArr)
	{
		...
		FString TypeName;
		TSharedPtr<FJsonValue> Val = MegalithPieObject::ReadDottedValue(Resolved.Object, Path, TypeName);
		if (Val.IsValid())
		{
...
FMegalithActionResult FMegalithPieObjectActions::HandleCallFunction(const TSharedPtr<FJsonObject>& Params)
{
	const MegalithPieObject::FResolvedObject Resolved = MegalithPieObject::Resolve(Params);
	if (!Resolved.bSuccess)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("pie_call_function requires a 'function' name"));
	}
	...
	if (!Frame.Memory)
	{
		return FMegalithActionResult::Error(TEXT("Failed to allocate the function parameter frame"));
	}
	...
	for (TFieldIterator<FProperty> It(Function); It && (It->PropertyFlags & CPF_Parm); ++It)
	{
		...
		FString TypeName;
		TSharedPtr<FJsonValue> Val = MegalithPieObject::ReadLeafValue(Prop, ValuePtr, Obj, TypeName);
