...
//
// Phase 2 of plan: 2026-05-26-megalith-editor-preview-expansion.md.
//
...
// MegalithEditorActions.cpp::RegisterActions; declarations in the public
// MegalithEditorActions.h header.
//
...
// in the plan) — keys go directly on FMegalithActionResult::Result, not
...
FMegalithActionResult FMegalithEditorActions::HandleInspectMaterialPBR(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("Missing params"));
	}
...
FMegalithActionResult FMegalithEditorActions::HandleInspectTextureChannels(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("Missing params"));
	...
	if (bEmitSplits)
	{
		...
		if (OutputDir.IsEmpty())
		{
			OutputDir = FPaths::ProjectDir()
				/ TEXT("Saved/Tests/Megalith/InspectTexture")
				/ Texture->GetName();
		}
	...
	// MegalithEditor never reach here (the whole module is Editor-typed), but
