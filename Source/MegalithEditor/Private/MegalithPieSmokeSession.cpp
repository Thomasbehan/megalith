...
#include "MegalithEditorActions.h" // FMegalithLogCapture / FMegalithLogEntry
#include "MegalithPieObjectActions.h" // Gap 9: shared PIE-object resolver + dotted read
...
// MegalithEditor). FCsvProfiler is gated behind CSV_PROFILER (WITH_ENGINE && !UE_BUILD_SHIPPING
...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithPieSmoke, Log, All);
...
namespace
{
	...
	// selector fields. Mirrors MegalithPieObject::Resolve's matching (actor_label/object_name
	...
	void FireActorSetup(FPieSmokeSession& Session, UWorld* PieWorld)
	{
		...
		for (const FPieSmokeActorSetupEntry& Entry : Session.ActorSetups)
		{
			...
			for (int32 i = 0; i < SpawnCount; ++i)
			{
				...
				Spawned->SetFolderPath(FName(TEXT("Megalith/ActorSetup")));
				++Result.SpawnedCount;
...
void FPieSmokeSessionManager::EnsureObserver()
{
	...
	TickerHandle = FTSTicker::GetCoreTicker().AddTicker(
		TEXT("MegalithPieSmokeObserver"), 0.0f,
	...
	if (Session.bTimeseries)
	{
		...
		if (bDue && Session.TimeseriesSamples.Num() < Session.MaxSamples)
		{
			...
			if (Target)
			{
				...
				{
					FString TypeName;
					TSharedPtr<FJsonValue> Val = MegalithPieObject::ReadDottedValue(Target, Path, TypeName);
					if (Val.IsValid())
					{
	...
	if (Session.bCaptureFrames && !Session.bCaptureDeferred)
	{
		...
		if (bDue)
		{
			...
			else if (Session.CaptureFrameIndex == 0 && SampleTime > Session.CaptureInterval * 2.0)
			{
				...
				UE_LOG(LogMegalithPieSmoke, Warning,
