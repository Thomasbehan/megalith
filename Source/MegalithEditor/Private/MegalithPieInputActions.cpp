...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithPieInput, Log, All);
...
namespace
{
	...
	// Mirrors MegalithPieObject::FindPieWorld so the input actions share one notion of
	// "the running PIE world".
	UWorld* GetPieWorld()
	{
		return MegalithPieObject::FindPieWorld();
	}
...
void FMegalithPieInputActions::RegisterPieEndHook()
{
...
void FMegalithPieInputActions::UnregisterPieEndHook()
{
	if (GPrePieEndedHandle.IsValid())
	{
...
namespace
{
	...
	const UInputAction* ResolveInputAction(const FString& Spec, FString& OutError)
	{
		...
		if (FirstMatch)
		{
			if (MatchCount > 1)
			{
				UE_LOG(LogMegalithPieInput, Warning,
...
void FMegalithPieInputActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_set_control_rotation"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetControlRotation),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_inject_input_action"),
		...
		FMegalithActionHandler::CreateStatic(&HandleInjectInputAction),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("pie_possess_spectator_free"),
		...
		FMegalithActionHandler::CreateStatic(&HandlePossessSpectatorFree),
		FParamSchemaBuilder()
			.Required(TEXT("enable"), TEXT("bool"), TEXT("true = detach to a free-fly spectator; false = re-possess the original pawn."))
			.Optional(TEXT("player_index"), TEXT("number"), TEXT("Local player index (default 0)."), TEXT("0"))
			.Build());

	UE_LOG(LogMegalithPieInput, Log, TEXT("MegalithEditor: registered 3 PIE input/control actions"));
}
...
FMegalithActionResult FMegalithPieInputActions::HandleSetControlRotation(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* PieWorld = GetPieWorld();
	if (!PieWorld)
	{
		return FMegalithActionResult::Error(TEXT("PIE not running — start Play-In-Editor first"));
	}
...
FMegalithActionResult FMegalithPieInputActions::HandleInjectInputAction(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* PieWorld = GetPieWorld();
	if (!PieWorld)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("pie_inject_input_action requires 'input_action' (asset path or name)"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("pie_inject_input_action 'value' invalid: %s"), *ValueError));
	}
...
FMegalithActionResult FMegalithPieInputActions::HandlePossessSpectatorFree(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* PieWorld = GetPieWorld();
	if (!PieWorld)
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("pie_possess_spectator_free requires 'enable' (bool)"));
	}
