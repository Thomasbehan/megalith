...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithPieTimeseries, Log, All);

void FMegalithPieTimeseries::RegisterActions(FMegalithToolRegistry& Registry)
{
	// Registered under the "animation" namespace string (see header). MegalithEditor owns
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("sample_pie_timeseries"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSamplePieTimeseries),
		FParamSchemaBuilder()
	...
	UE_LOG(LogMegalithPieTimeseries, Log, TEXT("MegalithEditor: registered sample_pie_timeseries under the 'animation' namespace"));
}

FMegalithActionResult FMegalithPieTimeseries::HandleSamplePieTimeseries(const TSharedPtr<FJsonObject>& Params)
{
	return FMegalithEditorActions::StartTimeseriesSession(Params);
}
