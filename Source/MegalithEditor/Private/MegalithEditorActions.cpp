...
// C4996 deprecation resolution). MegalithEditor already PrivateDependsOn
// MegalithSource (MegalithEditor.Build.cs) — one-way Editor -> Source borrow.
#include "MegalithSourceDatabase.h"
...
#include "MegalithPieSmokeSession.h"
...
FMegalithLogCapture* FMegalithEditorActions::CachedLogCapture = nullptr;
double FMegalithEditorActions::LastCompileTimestamp = 0.0;
FString FMegalithEditorActions::LastCompileResult = TEXT("none");
bool FMegalithEditorActions::bIsCompiling = false;
bool FMegalithEditorActions::bPatchApplied = false;
double FMegalithEditorActions::LastCompileEndTimestamp = 0.0;

// --- Log capture ---

void FMegalithLogCapture::Serialize(const TCHAR* V, ELogVerbosity::Type Verbosity, const FName& Category)
{
	FScopeLock ScopeLock(&Lock);

	FMegalithLogEntry Entry;
...
TArray<FMegalithLogEntry> FMegalithLogCapture::GetRecentEntries(int32 Count) const
{
	FScopeLock ScopeLock(&Lock);
	TArray<FMegalithLogEntry> Result;
...
TArray<FMegalithLogEntry> FMegalithLogCapture::SearchEntries(const FString& Pattern, const FString& CategoryFilter, ELogVerbosity::Type MaxVerbosity, int32 Limit) const
{
	FScopeLock ScopeLock(&Lock);
	TArray<FMegalithLogEntry> Result;
	...
	for (int32 i = 0; i < Total && Result.Num() < Limit; ++i)
	{
		...
		const FMegalithLogEntry& Entry = RingBuffer[Idx];
...
TArray<FString> FMegalithLogCapture::GetActiveCategories() const
{
	FScopeLock ScopeLock(&Lock);
	TSet<FString> Categories;
	for (const FMegalithLogEntry& Entry : RingBuffer)
	{
...
int32 FMegalithLogCapture::GetCountByVerbosity(ELogVerbosity::Type Verbosity) const
{
	FScopeLock ScopeLock(&Lock);
	switch (Verbosity)
	{
...
int32 FMegalithLogCapture::GetTotalCount() const
{
	FScopeLock ScopeLock(&Lock);
	return TotalFatal + TotalError + TotalWarning + TotalLog + TotalVerbose;
}

TArray<FMegalithLogEntry> FMegalithLogCapture::GetEntriesSince(double SinceTimestamp, const TArray<FName>& CategoryFilter, ELogVerbosity::Type MaxVerbosity, int32 Limit) const
{
	FScopeLock ScopeLock(&Lock);
	TArray<FMegalithLogEntry> Result;
...
int32 FMegalithLogCapture::CountErrorsSince(double SinceTimestamp) const
{
	FScopeLock ScopeLock(&Lock);
	int32 Count = 0;
	int32 Total = RingBuffer.Num();
...
static TSharedPtr<FJsonObject> LogEntryToJson(const FMegalithLogEntry& Entry)
{
...
void FMegalithEditorActions::InitLiveCodingDelegate()
{
#if PLATFORM_WINDOWS
	ILiveCodingModule* LC = FModuleManager::GetModulePtr<ILiveCodingModule>(LIVE_CODING_MODULE_NAME);
	if (LC)
	{
		LC->GetOnPatchCompleteDelegate().AddStatic(&FMegalithEditorActions::OnLiveCodingPatchComplete);
	}
#endif
}

void FMegalithEditorActions::OnLiveCodingPatchComplete()
{
	bIsCompiling = false;
	bPatchApplied = true;
...
void FMegalithEditorActions::RegisterActions(FMegalithLogCapture* LogCapture)
{
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("trigger_build"),
		...
		FMegalithActionHandler::CreateStatic(&HandleTriggerBuild),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_build_errors"),
		TEXT("Get build errors and warnings, scoped to a time window and bucketed into compile_errors (LogLiveCoding/LogCompile/LogLinker) vs other_errors. By default excludes LogPython + LogMegalith capture noise from the headline error_count (still visible under other_errors). Window precedence: since_marker > since_iso > since_seconds/since > last compile."),
		FMegalithActionHandler::CreateStatic(&HandleGetBuildErrors),
		FParamSchemaBuilder()
			...
			.Optional(TEXT("exclude_categories"), TEXT("array"), TEXT("Categories bucketed under other_errors and kept OUT of the headline error_count (still returned — never hidden). Default [LogPython, LogMegalith]."))
			.Build());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_recent_logs"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetRecentLogs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("search_logs"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSearchLogs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("tail_log"),
		...
		FMegalithActionHandler::CreateStatic(&HandleTailLog),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_log_categories"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetLogCategories),
		MakeShared<FJsonObject>());

	Registry.RegisterAction(TEXT("editor"), TEXT("get_log_stats"),
		TEXT("Get log statistics by verbosity level"),
		FMegalithActionHandler::CreateStatic(&HandleGetLogStats),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_compile_output"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetCompileOutput),
		MakeShared<FJsonObject>());

	Registry.RegisterAction(TEXT("editor"), TEXT("get_crash_context"),
		TEXT("Get last crash/ensure context information"),
		FMegalithActionHandler::CreateStatic(&HandleGetCrashContext),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_console_command"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRunConsoleCommand),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("start_pie"),
		...
		FMegalithActionHandler::CreateStatic(&HandleStartPIE),
		MakeShared<FJsonObject>());

	Registry.RegisterAction(TEXT("editor"), TEXT("stop_pie"),
		TEXT("Stop the active Play-In-Editor session."),
		FMegalithActionHandler::CreateStatic(&HandleStopPIE),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("list_dirty_packages"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListDirtyPackages),
		FParamSchemaBuilder()
			.Optional(TEXT("scope_paths"), TEXT("array"), TEXT("Array of /Game path prefixes to filter by (e.g. [\"/Game/Tests/Megalith\"]). Omit for all dirty packages. Transient/in-memory packages are excluded unless include_transient=true."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("save_packages"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSavePackages),
		FParamSchemaBuilder()
			.Required(TEXT("packages"), TEXT("array"), TEXT("Array of long package names (e.g. [\"/Game/Tests/Megalith/DA_Foo\"]) to save."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_pie_smoke"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRunPieSmoke),
		FParamSchemaBuilder()
			.OptionalAssetPath(TEXT("map"), TEXT("Level asset path to load before PIE (e.g. /Game/Tests/Megalith/Maps/M_Harness). Omit to use the current editor level."))
			.Optional(TEXT("marker"), TEXT("string"), TEXT("Marker token emitted to the log; post-marker pattern matching counts only lines after it. Default MEGALITH_SMOKE."), TEXT("MEGALITH_SMOKE"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("list_errored_blueprints"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListErroredBlueprints),
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("poll_pie_smoke"),
		...
		FMegalithActionHandler::CreateStatic(&HandlePollPieSmoke),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("stop_pie_smoke"),
		...
		FMegalithActionHandler::CreateStatic(&HandleStopPieSmoke),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_pie_movement_clip"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCapturePieMovementClip),
		FParamSchemaBuilder()
			.OptionalAssetPath(TEXT("map"), TEXT("Level asset path to load before PIE. Omit to use the current editor level."))
			.Optional(TEXT("marker"), TEXT("string"), TEXT("Log marker token. Default MEGALITH_CLIP."), TEXT("MEGALITH_CLIP"))
			.Optional(TEXT("duration"), TEXT("number"), TEXT("Seconds the editor loop advances PIE before the session auto-completes (clamped 0-120). Default 5."), TEXT("5"))
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Directory for frame PNGs. Disk-relative, absolute, OR a virtual /Game/... path (resolved to <project>/Content/...). Default Saved/Screenshots/Megalith/PieClip/<timestamp>/. The resolved absolute dir is echoed as resolved_output_dir; a virtual path that can't be written is a hard error (no silent no-op)."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("create_nav_harness_map"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateNavHarnessMap),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("path"), TEXT("Asset path for the new UWorld (e.g. /Game/Tests/Megalith/Maps/M_NavHarness)."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("author_map_settings"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAuthorMapSettings),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_scene_preview"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCaptureScenePreview),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("inspect_material_pbr"),
		...
		FMegalithActionHandler::CreateStatic(&HandleInspectMaterialPBR),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("inspect_texture_channels"),
		...
		FMegalithActionHandler::CreateStatic(&HandleInspectTextureChannels),
		FParamSchemaBuilder()
			...
			.OptionalDiskPath(TEXT("output_dir"), TEXT("Output directory for split PNGs (default: Saved/Tests/Megalith/InspectTexture/<TextureName>/)"))
			.Build());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_material_grid"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCaptureMaterialGrid),
		FParamSchemaBuilder()
			.Required(TEXT("material_paths"), TEXT("array"), TEXT("Array of UMaterialInterface asset paths (1..16). Each becomes one grid cell."))
			.OptionalDiskPath(TEXT("output_path"), TEXT("Output PNG path. Default: Saved/Screenshots/Megalith/CaptureMaterialGrid/<timestamp>.png"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_with_overlay"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCaptureWithOverlay),
		FParamSchemaBuilder()
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Output PNG path. Default: Saved/Screenshots/Megalith/CaptureWithOverlay/<timestamp>.png"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_sequence_frames"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCaptureSequenceFrames),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Asset path to preview"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_anim_frames"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCaptureAnimFrames),
		FParamSchemaBuilder()
			...
			.Optional(TEXT("resolution"), TEXT("array"), TEXT("[width, height]. Default [512,512]."), TEXT("[512,512]"))
			.OptionalDiskPath(TEXT("output_dir"), TEXT("Directory for frame PNGs. Disk-relative, absolute, OR a virtual /Game/... path (resolved to <project>/Content/...). Default Saved/Screenshots/Megalith/AnimFrames/<timestamp>_<asset>/. The resolved absolute dir is echoed as resolved_output_dir."))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("import_texture"),
		...
		FMegalithActionHandler::CreateStatic(&HandleImportTexture),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("stitch_flipbook"),
		...
		FMegalithActionHandler::CreateStatic(&HandleStitchFlipbook),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("delete_assets"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDeleteAssets),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_viewport_info"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetViewportInfo),
		MakeShared<FJsonObject>());
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("capture_system_gif"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCaptureSystemGif),
		FParamSchemaBuilder()
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Output directory (default: Saved/Screenshots/Megalith/GIF_<timestamp>)"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("list_automation_tests"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListAutomationTests),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_automation_tests"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRunAutomationTests),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("run_python"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRunPython),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("load_level"),
		...
		FMegalithActionHandler::CreateStatic(&HandleLoadLevel),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithEditorActions::HandleTriggerBuild(const TSharedPtr<FJsonObject>& Params)
{
#if PLATFORM_WINDOWS
	ILiveCodingModule* LiveCoding = FModuleManager::GetModulePtr<ILiveCodingModule>(LIVE_CODING_MODULE_NAME);
	if (!LiveCoding)
	{
		return FMegalithActionResult::Error(TEXT("Live Coding module not available"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("A compile is already in progress"));
	}
...
#else
	return FMegalithActionResult::Error(TEXT("Live Coding is only available on Windows"));
#endif
}
...
FMegalithActionResult FMegalithEditorActions::HandleGetBuildErrors(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
	...
	if (Params->TryGetStringField(TEXT("since_marker"), SinceMarker) && !SinceMarker.IsEmpty() && CachedLogCapture)
	{
		...
		TArray<FMegalithLogEntry> Marks = CachedLogCapture->SearchEntries(
			SinceMarker, FString(), ELogVerbosity::VeryVerbose, FMegalithLogCapture::MaxEntries);
		if (Marks.Num() > 0)
		{
	...
	{
		ExcludeCategories = { TEXT("LogPython"), TEXT("LogMegalith") };
	}
	...
	if (CachedLogCapture)
	{
		TArray<FMegalithLogEntry> Entries = CachedLogCapture->GetEntriesSince(
			SinceTimestamp, CategoryFilter, ELogVerbosity::Warning, 500);

		for (const FMegalithLogEntry& Entry : Entries)
		{
...
TArray<TSharedPtr<FJsonValue>> FMegalithEditorActions::BuildFixHints(const TArray<TSharedPtr<FJsonValue>>& ErrorObjs)
{
	TArray<TSharedPtr<FJsonValue>> Hints;

	// Resolve the borrowed source DB (may be null — degrade gracefully).
	FMegalithSourceDatabase* SourceDb = nullptr;
	if (IsInGameThread() && GEditor)
	{
		if (UMegalithSourceSubsystem* SS = GEditor->GetEditorSubsystem<UMegalithSourceSubsystem>())
		{
		...
		else if (Message.Contains(TEXT("C4996")))
		{
			...
			if (SourceDb && SourceDb->IsOpen())
			{
				...
				if (!Ident.IsEmpty())
				{
					...
					TMap<FString, FMegalithDeprecationRow> Dep = SourceDb->GetDeprecationsBatch(Names);
					if (const FMegalithDeprecationRow* Row = Dep.Find(LookupName))
					{
...
FMegalithActionResult FMegalithEditorActions::HandleGetBuildStatus(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FMegalithActionResult FMegalithEditorActions::HandleGetBuildSummary(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FMegalithActionResult FMegalithEditorActions::HandleSearchBuildOutput(const TSharedPtr<FJsonObject>& Params)
{
	FString Pattern = Params->GetStringField(TEXT("pattern"));
	if (Pattern.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: pattern"));
	}
	...
	if (CachedLogCapture)
	{
		...
		TArray<FMegalithLogEntry> Entries = CachedLogCapture->SearchEntries(
...
FMegalithActionResult FMegalithEditorActions::HandleGetCompileOutput(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FMegalithActionResult FMegalithEditorActions::HandleGetRecentLogs(const TSharedPtr<FJsonObject>& Params)
{
	int32 Count = 100;
	...
	if (CachedLogCapture)
	{
		TArray<FMegalithLogEntry> Entries = CachedLogCapture->GetRecentEntries(Count);
		for (const FMegalithLogEntry& Entry : Entries)
		{
...
FMegalithActionResult FMegalithEditorActions::HandleSearchLogs(const TSharedPtr<FJsonObject>& Params)
{
	FString Pattern = Params->GetStringField(TEXT("pattern"));
	...
	if (CachedLogCapture)
	{
		TArray<FMegalithLogEntry> Entries = CachedLogCapture->SearchEntries(Pattern, Category, MaxVerbosity, Limit);
		for (const FMegalithLogEntry& Entry : Entries)
		{
			LogArr.Add(MakeShared<FJsonValueObject>(LogEntryToJson(Entry)));
		}
...
FMegalithActionResult FMegalithEditorActions::HandleTailLog(const TSharedPtr<FJsonObject>& Params)
{
	int32 Count = 50;
	if (Params->HasField(TEXT("count")))
	{
...
FMegalithActionResult FMegalithEditorActions::HandleGetLogCategories(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FMegalithActionResult FMegalithEditorActions::HandleGetLogStats(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FMegalithActionResult FMegalithEditorActions::HandleGetCrashContext(const TSharedPtr<FJsonObject>& Params)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
	...
	if (CachedLogCapture)
	{
		TArray<FMegalithLogEntry> ErrorEntries = CachedLogCapture->SearchEntries(
			TEXT(""), TEXT(""), ELogVerbosity::Error, 20);
		TArray<TSharedPtr<FJsonValue>> RecentErrors;
		for (const FMegalithLogEntry& Entry : ErrorEntries)
		{
...
// ---------------------------------------------------------------------------
FMegalithActionResult FMegalithEditorActions::HandleRunConsoleCommand(const TSharedPtr<FJsonObject>& Params)
{
	FString Command;
	if (!Params->TryGetStringField(TEXT("command"), Command) || Command.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required field: command"));
	}
	...
	if (!TargetWorld)
	{
		return FMegalithActionResult::Error(TEXT("No usable world found (no PIE active and no editor world)"));
	}
	...
	else
	{
		if (!GEngine)
		{
			return FMegalithActionResult::Error(TEXT("GEngine is null — run_console_command requires engine context."));
		}
...
// ---------------------------------------------------------------------------
UWorld* FMegalithEditorActions::FindActivePieWorld()
{
	if (!GEditor)
	{
		return nullptr;
...
namespace
{
	...
	bool EnsureNoResidentPieWorldBeforeMapLoad(FString& OutError)
	{
		...
		UWorld* ResidentPie = FMegalithEditorActions::FindActivePieWorld();
		if (!ResidentPie)
		{
		...
		{
			if (!FMegalithEditorActions::FindActivePieWorld())
			{
...
bool FMegalithEditorActions::StartPieInternal(FString& OutError, bool bSuppressModals)
{
	if (!GUnrealEd)
	{
...
bool FMegalithEditorActions::StopPieInternal()
{
	if (!GEditor)
	{
		return false;
...
FMegalithActionResult FMegalithEditorActions::HandleStartPIE(const TSharedPtr<FJsonObject>& Params)
{
	if (!GUnrealEd) return FMegalithActionResult::Error(TEXT("GUnrealEd not available"));
	...
	if (FindActivePieWorld())
	{
		...
		return FMegalithActionResult::Success(AlreadyRunning);
	}

	FString StartError;
	if (!StartPieInternal(StartError))
	{
		return FMegalithActionResult::Error(StartError);
	}
...
FMegalithActionResult FMegalithEditorActions::HandleStopPIE(const TSharedPtr<FJsonObject>& Params)
{
	if (!GEditor) return FMegalithActionResult::Error(TEXT("GEditor not available"));
...
bool FMegalithEditorActions::RenderAndSaveCapture(
	...
	const FString& OutputPath)
{
	...
	if (!RTResource)
	{
		UE_LOG(LogMegalith, Error, TEXT("CaptureScenePreview: Failed to get RT resource"));
		return false;
	}
	...
	{
		UE_LOG(LogMegalith, Error, TEXT("CaptureScenePreview: ReadPixels failed (read=%d, count=%d)"),
			bReadOk, Pixels.Num());
		return false;
	}
...
bool FMegalithEditorActions::CaptureNiagaraFrame(
...
bool FMegalithEditorActions::CaptureMaterialFrame(
	...
	const FLinearColor& BackgroundColor)
{
	...
	else
	{
		...
		if (!Mesh)
		{
			UE_LOG(LogMegalith, Error, TEXT("CaptureMaterialFrame: Failed to load mesh %s"), *MeshPath);
			return false;
		}
...
FMegalithActionResult FMegalithEditorActions::HandleCaptureScenePreview(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("asset_path and asset_type are required"));
	}
	...
	if (Params->HasField(TEXT("camera")))
	{
		...
		if (!Params->TryGetObjectField(TEXT("camera"), CameraObj))
		{
			...
			{
				ParsedCamera = FMegalithJsonUtils::Parse(CameraStr);
	...
	else
	{
		...
		OutputPath = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Megalith") /
	...
	if (!bSuccess)
	{
		return FMegalithActionResult::Error(TEXT("Capture failed — check log for details"));
	}
...
FMegalithActionResult FMegalithEditorActions::HandleCaptureSequenceFrames(
	const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString AssetType = Params->GetStringField(TEXT("asset_type"));
	...
	{
		return FMegalithActionResult::Error(TEXT("timestamps array is required"));
	}
	...
	if (Timestamps.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("timestamps array is empty"));
	}
	...
	else
	{
		FString Timestamp = FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S"));
		FString SafeName = FPaths::GetBaseFilename(AssetPath);
		OutputDir = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Megalith") /
	...
	if (!AssetType.Equals(TEXT("niagara"), ESearchCase::IgnoreCase))
	{
		return FMegalithActionResult::Error(TEXT("capture_sequence_frames currently only supports asset_type: niagara"));
	}
...
FMegalithActionResult FMegalithEditorActions::HandleCaptureAnimFrames(
	const TSharedPtr<FJsonObject>& Params)
{
	check(IsInGameThread());
	...
	if (!Asset)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load asset: %s"), *AssetPath));
	}
	...
	if (!Params->TryGetStringField(TEXT("output_dir"), OutputDir) || OutputDir.IsEmpty())
	{
		...
		OutputDir = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Megalith/AnimFrames") /
	...
	{
		return FMegalithActionResult::Error(ResolvedError);
	}
	FString FilenamePrefix = TEXT("frame");
...
FMegalithActionResult FMegalithEditorActions::HandleImportTexture(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("source_path and destination are required"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Import failed — no assets imported"));
	}

	UTexture2D* Texture = Cast<UTexture2D>(ImportedAssets[0]);
	if (!Texture)
	{
		return FMegalithActionResult::Error(TEXT("Imported asset is not a Texture2D"));
	}
	...
	if (Params->HasField(TEXT("settings")))
	{
		...
		else
		{
			...
			{
				ParsedSettings = FMegalithJsonUtils::Parse(SettingsStr);
			}
		}
...
FMegalithActionResult FMegalithEditorActions::HandleStitchFlipbook(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	if (DestPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("dest_path is required"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("frame_paths array is required and must not be empty"));
	}
	...
	if (FramePaths.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No valid file paths in frame_paths"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("grid must be an array of [columns, rows]"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("grid columns and rows must be positive"));
	}
	...
	if (!Texture)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create UTexture2D"));
	}
	...
	if (!bSaved)
	{
		return FMegalithActionResult::Error(TEXT("Failed to save flipbook texture package"));
	}
...
FMegalithActionResult FMegalithEditorActions::HandleGetViewportInfo(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!ViewportClient)
	{
		return FMegalithActionResult::Error(TEXT("No active viewport found"));
	}
...
FMegalithActionResult FMegalithEditorActions::HandleDeleteAssets(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("asset_paths array is required and must not be empty"));
	}
	...
	if (AssetPaths.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No valid paths in asset_paths"));
	}
...
// ============================================================================
FMegalithActionResult FMegalithEditorActions::HandleCaptureSystemGif(
	const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	...
	UNiagaraSystem* System = LoadObject<UNiagaraSystem>(nullptr, *AssetPath);
	if (!System)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load Niagara system: %s"), *AssetPath));
	...
	// Capture frames
	FMegalithActionResult CaptureResult = HandleCaptureSequenceFrames(CaptureParams);
	if (!CaptureResult.bSuccess)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Frame capture failed: %s"), *CaptureResult.ErrorMessage));
...
namespace MegalithAutomationDetail
{
...
FMegalithActionResult FMegalithEditorActions::HandleListAutomationTests(const TSharedPtr<FJsonObject>& Params)
{
	FString Prefix;
	if (Params.IsValid())
	{
	...
	MegalithAutomationDetail::CollectMatchingTests(Prefix, Tests);
	...
	{
		TSharedPtr<FJsonObject> Obj = MakeShared<FJsonObject>();
		Obj->SetStringField(TEXT("full_path"), MegalithAutomationDetail::GetTestFullPath(Info));
...
FMegalithActionResult FMegalithEditorActions::HandleRunAutomationTests(const TSharedPtr<FJsonObject>& Params)
{
	FString Prefix;
	if (!Params.IsValid() || !Params->TryGetStringField(TEXT("prefix"), Prefix) || Prefix.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Required parameter: prefix (string, e.g. 'MazeLegends.Bow')"));
	}
	...
	MegalithAutomationDetail::CollectMatchingTests(Prefix, MatchingTests);
	...
	for (int32 i = 0; i < TestsToRun; ++i)
	{
		...
		const FString FullPath = MegalithAutomationDetail::GetTestFullPath(Info);
...
FMegalithActionResult FMegalithEditorActions::HandleRunPython(const TSharedPtr<FJsonObject>& Params)
{
	FString Command;
	if (!Params.IsValid() || !Params->TryGetStringField(TEXT("command"), Command) || Command.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: command"));
	}
...
FMegalithActionResult FMegalithEditorActions::HandleLoadLevel(const TSharedPtr<FJsonObject>& Params)
{
	FString Path;
	if (!Params.IsValid() || !Params->TryGetStringField(TEXT("path"), Path) || Path.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: path"));
	}

	if (!GEditor)
	{
		return FMegalithActionResult::Error(TEXT("GEditor is null — load_level requires editor context."));
	}

	ULevelEditorSubsystem* LevelEd = GEditor->GetEditorSubsystem<ULevelEditorSubsystem>();
	if (!LevelEd)
	{
		return FMegalithActionResult::Error(TEXT("ULevelEditorSubsystem is unavailable."));
	}
	...
	{
		return FMegalithActionResult::Error(PieGuardError);
	}
	...
	{
		return FMegalithActionResult::Error(StaleWorldError);
	}
...
namespace MegalithEditorPackages
{
...
FMegalithActionResult FMegalithEditorActions::HandleListDirtyPackages(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorPackages;
...
FMegalithActionResult FMegalithEditorActions::HandleSavePackages(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorPackages;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: packages (non-empty array of long package names)"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("packages array contained no valid package names"));
	}
...
namespace MegalithEditorPieSmoke
{
	...
	static bool EntryMatches(const FMegalithLogEntry& Entry, const FString& Pattern)
	{
	...
	static TSharedPtr<FJsonObject> CountGroupInBucket(const TArray<FMegalithLogEntry>& Bucket,
		const TArray<FString>& Patterns, int32& OutGroupTotal, int32& OutDistinctMatched)
	{
		...
		{
			int32 Count = 0;
			for (const FMegalithLogEntry& Entry : Bucket)
			{
	...
	static bool AnalysePostMarkerGrouped(FMegalithLogCapture* LogCapture, double MarkerTimestamp,
		const FPieSmokeSession& S, TSharedPtr<FJsonObject>& Report)
	{
		TArray<FMegalithLogEntry> ActiveBucket;
		TArray<FMegalithLogEntry> TeardownBucket;

		if (LogCapture)
		{
			const TArray<FMegalithLogEntry> Entries = LogCapture->GetEntriesSince(
				MarkerTimestamp, /*CategoryFilter*/{}, ELogVerbosity::VeryVerbose, FMegalithLogCapture::MaxEntries);
		...
		{
			int32 Count = 0;
			for (const FMegalithLogEntry& Entry : ActiveBucket)
			{
				if (EntryMatches(Entry, Pattern)) { ++Count; }
			}
			if (Count > 0)
	...
	{
		const bool bPieResident = (FMegalithEditorActions::FindActivePieWorld() != nullptr);
	...
	static TSharedPtr<FJsonObject> BuildSessionReport(const FPieSmokeSession& S, bool bFull,
		FMegalithLogCapture* LogCapture)
	{
		TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
...
FMegalithActionResult FMegalithEditorActions::HandleRunPieSmoke(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorPieSmoke;

	if (!GEditor || !GUnrealEd)
	{
		return FMegalithActionResult::Error(TEXT("run_pie_smoke requires editor context (GEditor/GUnrealEd)."));
	}
	if (FindActivePieWorld())
	{
		return FMegalithActionResult::Error(TEXT("A PIE session is already running — stop it before run_pie_smoke."));
	}

	FString Marker = TEXT("MEGALITH_SMOKE");
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to start PIE: %s"), *StartError));
	}
	...
	UE_LOG(LogMegalith, Display, TEXT("%s begin (map=%s)"), *Marker,
...
FMegalithActionResult FMegalithEditorActions::StartTimeseriesSession(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorPieSmoke;

	if (!GEditor || !GUnrealEd)
	{
		return FMegalithActionResult::Error(TEXT("sample_pie_timeseries requires editor context (GEditor/GUnrealEd)."));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("sample_pie_timeseries requires one of: actor, pawn_class, object_name"));
	}
	...
	if (VarPaths.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("sample_pie_timeseries requires a non-empty 'variables' array of dotted paths"));
	}
	...
	if (FindActivePieWorld())
	{
		return FMegalithActionResult::Error(TEXT("A PIE session is already running — stop it before sample_pie_timeseries."));
	}
	...
	UWorld* PieWorld = FindActivePieWorld();
	const FString Marker = TEXT("MEGALITH_TIMESERIES");
	UE_LOG(LogMegalith, Display, TEXT("%s begin (map=%s)"), *Marker,
		PieWorld ? *PieWorld->GetMapName() : TEXT("<current>"));
...
FMegalithActionResult FMegalithEditorActions::HandlePollPieSmoke(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorPieSmoke;
	...
	{
		return FMegalithActionResult::Error(TEXT("poll_pie_smoke requires a session_id."));
	}
	...
	if (!Session)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Unknown PIE-smoke session '%s'."), *SessionId));
	}
	...
	return FMegalithActionResult::Success(
		BuildSessionReport(*Session, bFull, Mgr.GetLogCapture()));
}

FMegalithActionResult FMegalithEditorActions::HandleStopPieSmoke(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorPieSmoke;
...
FMegalithActionResult FMegalithEditorActions::HandleListErroredBlueprints(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FErroredBlueprintEntry> Errored;
	ScanErroredBlueprints(Errored);
...
FMegalithActionResult FMegalithEditorActions::HandleCapturePieMovementClip(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorPieSmoke;

	if (!GEditor || !GUnrealEd)
	{
		return FMegalithActionResult::Error(TEXT("capture_pie_movement_clip requires editor context (GEditor/GUnrealEd)."));
	}
	if (FindActivePieWorld())
	{
		return FMegalithActionResult::Error(TEXT("A PIE session is already running — stop it before capture_pie_movement_clip."));
	}

	FString Marker = TEXT("MEGALITH_CLIP");
	Params->TryGetStringField(TEXT("marker"), Marker);
	...
	{
		const FString Stamp = FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S"));
		OutputDir = FPaths::ProjectDir() / TEXT("Saved/Screenshots/Megalith/PieClip") / Stamp;
	}
	...
	{
		return FMegalithActionResult::Error(OutputDirError);
	}
	...
	UWorld* PieWorld = FindActivePieWorld();
	UE_LOG(LogMegalith, Display, TEXT("%s begin"), *Marker);
	if (PieWorld)
	{
		RunScripts(Params, PieWorld);
	}
...
// runtime string dispatch (FMegalithToolRegistry::ExecuteAction) so MegalithEditor
// takes NO compile-time dependency on MegalithAI / the NavigationSystem module.
// ---------------------------------------------------------------------------

namespace MegalithEditorNavHarness
{
	...
	static bool TryApplyLeaf(FProperty* Prop, void* ValuePtr, const TSharedPtr<FJsonValue>& Value, UObject* Owner, FString& OutWhyUnsupported)
	{
		...
		else if (FStructProperty* StructProp = CastField<FStructProperty>(Prop))
		{
			...
			// reflection text importer — same pattern as MegalithMaterialActions.
	...
	static void ApplyProperties(AActor* Actor, const TSharedPtr<FJsonObject>& PropObj, TArray<FString>& OutApplied, TArray<FString>& OutSkipped)
	{
		...
		for (const auto& Pair : PropObj->Values)
		{
			...
			// MegalithReflectionWalker.cpp (ctor UnrealType.h:4455, EmptyValues,
			// AddUninitializedValues, GetRawPtr) + the object-inner set in
			// MegalithAIStateTreeActions.cpp:2622-2627.
...
FMegalithActionResult FMegalithEditorActions::HandleCreateNavHarnessMap(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorNavHarness;

	if (!GEditor)
	{
		return FMegalithActionResult::Error(TEXT("create_nav_harness_map requires editor context (GEditor)."));
	}
	...
	//    then load it as the active editor world so spawns + nav target it.
	{
		...
		const FMegalithActionResult CreateRes = Registry.ExecuteAction(TEXT("editor"), TEXT("create_empty_map"), CreateParams);
	...
	if (!World)
	{
		return FMegalithActionResult::Error(TEXT("No editor world after loading the harness map."));
	}
	...
	// 6. Nav bounds — default sized to the floor footprint unless overridden.
	{
		...
		const FMegalithActionResult NavRes = Registry.ExecuteAction(TEXT("ai"), TEXT("add_nav_bounds_volume"), NavParams);
	...
	//    for async tile generation).
	{
		...
		const FMegalithActionResult RebuildRes = Registry.ExecuteAction(TEXT("ai"), TEXT("rebuild_navigation"), RebuildParams);
	...
	if (NavPoints.Num() > 0)
	{
		...
		const FMegalithActionResult ValidateRes = Registry.ExecuteAction(TEXT("ai"), TEXT("validate_nav_points"), ValidateParams);
	...
	// 9. Save the level package now that actors + nav exist.
	{
		...
		const FMegalithActionResult SaveRes = Registry.ExecuteAction(TEXT("editor"), TEXT("save_packages"), SaveParams);
...
// ---------------------------------------------------------------------------
FMegalithActionResult FMegalithEditorActions::HandleAuthorMapSettings(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithEditorNavHarness;

	if (!GEditor)
	{
		return FMegalithActionResult::Error(TEXT("author_map_settings requires editor context (GEditor)."));
	}
	if (!Params.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("author_map_settings requires a params object."));
	}
	...
	if (!World)
	{
		return FMegalithActionResult::Error(TEXT("No editor world to author."));
	}
	...
	if (bSave)
	{
		...
		SaveParams->SetArrayField(TEXT("packages"), PkgArr);
		const FMegalithActionResult SaveRes =
			FMegalithToolRegistry::Get().ExecuteAction(TEXT("editor"), TEXT("save_packages"), SaveParams);
		Result->SetBoolField(TEXT("saved"), SaveRes.bSuccess);
		if (!SaveRes.bSuccess)
		{
			Result->SetStringField(TEXT("save_error"), SaveRes.ErrorMessage);
