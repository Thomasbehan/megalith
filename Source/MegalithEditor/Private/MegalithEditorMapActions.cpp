...
//                                    (already used elsewhere in MegalithEditor).
...
//                                    (mirrors MegalithCommonUIHelpers.cpp:70-75
//                                     and MegalithEditorActions.cpp asset-save
//                                     sites).
//
...
//   * get_module_status: empty input list (returns all Megalith modules),
...
namespace
{
	...
	/** Asset-registry + in-memory FindObject existence check. Mirrors MegalithAI::EnsureAssetPathFree. */
...
void FMegalithEditorMapActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("create_empty_map"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateEmptyMap),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("path"), TEXT("Asset path under /Game/... where the new UWorld is saved (e.g. /Game/Tests/Megalith/Audio/Map_Test)"))
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("get_module_status"),
		TEXT("Report plugin-enabled + module-loaded status for Megalith (or arbitrary) modules. Wraps IPluginManager + FModuleManager. If module_names is omitted or empty, returns rows for all Megalith modules. Unknown module names return a row with enabled=false, loaded=false (no error)."),
		FMegalithActionHandler::CreateStatic(&HandleGetModuleStatus),
		FParamSchemaBuilder()
			.Optional(TEXT("module_names"), TEXT("array"), TEXT("Optional array of module name strings. Omit to query all Megalith modules."))
			.Build());
}
...
FMegalithActionResult FMegalithEditorMapActions::HandleCreateEmptyMap(const TSharedPtr<FJsonObject>& Params)
{
	FString InPath;
	...
	if (!Factory)
	{
		return FMegalithActionResult::Error(TEXT("Failed to construct UWorldFactory"));
	}
...
FMegalithActionResult FMegalithEditorMapActions::HandleGetModuleStatus(const TSharedPtr<FJsonObject>& Params)
{
	...
	//    that belongs to the Megalith plugin (the canonical use case for
	...
	{
		TSharedPtr<IPlugin> MegalithPlugin = IPluginManager::Get().FindPlugin(TEXT("Megalith"));
		if (MegalithPlugin.IsValid())
		{
			for (const FModuleDescriptor& ModDesc : MegalithPlugin->GetDescriptor().Modules)
			{
