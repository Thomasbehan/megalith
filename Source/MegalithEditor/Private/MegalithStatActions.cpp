...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithStat, Log, All);
...
void FMegalithStatActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("editor"), TEXT("get_stat_group_values"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetStatGroupValues),
		FParamSchemaBuilder()
	...
	UE_LOG(LogMegalithStat, Log, TEXT("MegalithEditor: registered 1 stat-group readout action"));
}

#if STATS

FMegalithActionResult FMegalithStatActions::HandleGetStatGroupValues(const TSharedPtr<FJsonObject>& Params)
{
	FString GroupNameRaw;
	if (!Params->TryGetStringField(TEXT("group_name"), GroupNameRaw) || GroupNameRaw.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("get_stat_group_values requires 'group_name' (e.g. 'STATGROUP_Anim' or 'Anim')"));
	}
	...
	if (SampleFrames > 240)
	{
		return FMegalithActionResult::Error(TEXT("sample_frames too large (max 240) — the stats history ring only retains a small number of frames"));
	}
