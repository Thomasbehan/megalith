...
// MegalithComboGraphBulkFillAdapter — Phase 5 Step 8 adapter (LAST).
//
...
//     `megalith_discover("combograph")` action surface stays identical across
//     dev + release builds.
//
...
#define LOCTEXT_NAMESPACE "MegalithComboGraphBulkFillAdapter"

namespace MegalithComboGraphBulkFillInternal
{
	...
	static FDryRunReport HandleEffectContainers(const FBulkFillSpec& Spec)
	{
		...
		FScopedTransaction Transaction(LOCTEXT("ComboGraphBulkFill_EC", "Megalith ComboGraph Bulk Fill — Effect Containers"));
		Asset->Modify();
	...
	static FDryRunReport HandleEdges(const FBulkFillSpec& Spec)
	{
		...
		Asset->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("ComboGraphBulkFill_Edges", "Megalith ComboGraph Bulk Fill — Edges"));
		Asset->Modify();
		Asset->PreEditChange(nullptr);
...
FDryRunReport FMegalithComboGraphBulkFillAdapter::ComboGraphBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithComboGraphBulkFillInternal;
...
FSchemaDescriptor FMegalithComboGraphBulkFillAdapter::ComboGraphDescribe(const FString& TargetAsset)
{
	using namespace MegalithComboGraphBulkFillInternal;
...
void FMegalithComboGraphBulkFillAdapter::Register()
{
	// H5 invariant — Register() runs unconditionally.
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("combograph"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithComboGraphBulkFillAdapter::ComboGraphBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithComboGraphBulkFillAdapter::ComboGraphDescribe));
}

void FMegalithComboGraphBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("combograph"));
}
