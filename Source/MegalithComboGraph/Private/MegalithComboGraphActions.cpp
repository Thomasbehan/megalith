...
void FMegalithComboGraphActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// ── Read/Inspect ──
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("list_combo_graphs"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListComboGraphs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("get_combo_graph_info"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetComboGraphInfo),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("get_combo_node_effects"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetComboNodeEffects),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("validate_combo_graph"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateComboGraph),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ComboGraph asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("create_combo_graph"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateComboGraph),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("add_combo_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddComboNode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ComboGraph asset path"))
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("add_combo_edge"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddComboEdge),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ComboGraph asset path"))
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("set_combo_node_effects"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetComboNodeEffects),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ComboGraph asset path"))
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("set_combo_node_cues"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetComboNodeCues),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ComboGraph asset path"))
			.Required(TEXT("node_index"), TEXT("number"), TEXT("Index of the node in AllNodes"))
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("create_combo_ability"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateComboAbility),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("link_ability_to_combo_graph"),
		...
		FMegalithActionHandler::CreateStatic(&HandleLinkAbilityToComboGraph),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("scaffold_combo_from_montages"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldComboFromMontages),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("combograph"), TEXT("layout_combo_graph"),
		...
		FMegalithActionHandler::CreateStatic(&HandleLayoutComboGraph),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ComboGraph asset path"))
	...
	UE_LOG(LogMegalithComboGraph, Log, TEXT("Registered 13 combograph actions"));
}
...
FMegalithActionResult FMegalithComboGraphActions::HandleListComboGraphs(const TSharedPtr<FJsonObject>& Params)
{
	UClass* ComboGraphClass = FindComboGraphClass();
	if (!ComboGraphClass)
	{
		return FMegalithActionResult::Error(TEXT("ComboGraph class not found. Is the ComboGraph plugin loaded?"));
	}
...
FMegalithActionResult FMegalithComboGraphActions::HandleGetComboGraphInfo(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithComboGraphActions::HandleGetComboNodeEffects(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithComboGraphActions::HandleValidateComboGraph(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
FMegalithActionResult FMegalithComboGraphActions::HandleCreateComboGraph(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (AssetName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Could not extract asset name from save_path"));
	}
	...
	if (!FactoryClass)
	{
		return FMegalithActionResult::Error(TEXT("ComboGraphFactory class not found. Is ComboGraphEditor loaded?"));
	}
	...
	if (!Factory)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create ComboGraphFactory instance"));
	}
	...
	if (!NewGraph)
	{
		return FMegalithActionResult::Error(TEXT("FactoryCreateNew returned null"));
	}
...
FMegalithActionResult FMegalithComboGraphActions::HandleAddComboNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'animation_asset'"));
	}
	...
	if (NodeType.Equals(TEXT("sequence"), ESearchCase::IgnoreCase))
	{
		...
		if (!NodeClass)
		{
			return FMegalithActionResult::Error(TEXT("ComboGraphNodeSequence class not found"));
		}
	}
	else
	{
		...
		if (!NodeClass)
		{
			return FMegalithActionResult::Error(TEXT("ComboGraphNodeMontage class not found"));
		}
	}
	...
	if (!NewNode)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create combo node"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Failed to add node to AllNodes array"));
	}
	...
	if (ParentNodeIndex >= 0)
	{
		...
		{
			UE_LOG(LogMegalithComboGraph, Warning,
...
FMegalithActionResult FMegalithComboGraphActions::HandleAddComboEdge(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	if (!EdgeClass)
	{
		return FMegalithActionResult::Error(TEXT("ComboGraphEdge class not found"));
	}

	UObject* Edge = NewObject<UObject>(Graph, EdgeClass);
	if (!Edge)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create edge"));
	}
...
FMegalithActionResult FMegalithComboGraphActions::HandleSetComboNodeEffects(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'effects' (must be a JSON object)"));
	}
	...
	if (!MapProp)
	{
		return FMegalithActionResult::Error(TEXT("EffectsContainerMap property not found on node. Is this a ComboGraphNodeAnimBase?"));
	}
	...
	{
		FString TagName = MegalithKeyToString(Pair.Key);
...
FMegalithActionResult FMegalithComboGraphActions::HandleSetComboNodeCues(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'cues' (must be a JSON object)"));
	}
	...
	if (!MapProp)
	{
		return FMegalithActionResult::Error(TEXT("CuesContainerMap property not found on node"));
	}
...
FMegalithActionResult FMegalithComboGraphActions::HandleCreateComboAbility(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	if (!NewBP)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create ability Blueprint"));
	}
...
FMegalithActionResult FMegalithComboGraphActions::HandleLinkAbilityToComboGraph(const TSharedPtr<FJsonObject>& Params)
{
	FString AbilityPath = Params->GetStringField(TEXT("ability_path"));
	if (AbilityPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'ability_path'"));
	}

	FString ComboGraphPath = Params->GetStringField(TEXT("combo_graph"));
	if (ComboGraphPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'combo_graph'"));
	}
	...
	if (!TaskClass)
	{
		return FMegalithActionResult::Error(TEXT("ComboGraphAbilityTask_StartGraph class not found"));
	}
	...
	if (ExistingTaskNode)
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Found StartComboGraph task node but could not find ComboGraph pin"));
		}
	}
	else
	{
		...
		if (!FactoryFunc)
		{
			return FMegalithActionResult::Error(TEXT("Could not find StartGraph factory function on task class"));
		}
...
FMegalithActionResult FMegalithComboGraphActions::HandleScaffoldComboFromMontages(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty())
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required param 'montages' (array of asset paths)"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("No valid montage paths in 'montages' array"));
	}

	// Step 1: Create the combo graph
	TSharedPtr<FJsonObject> CreateParams = MakeShared<FJsonObject>();
	CreateParams->SetStringField(TEXT("save_path"), SavePath);
	FMegalithActionResult CreateResult = HandleCreateComboGraph(CreateParams);
	...
	for (int32 i = 0; i < MontagePaths.Num(); i++)
	{
		...
		FMegalithActionResult NodeResult = HandleAddComboNode(NodeParams);
	...
	for (int32 i = 0; i < NodeIndices.Num() - 1; i++)
	{
		...
		FMegalithActionResult EdgeResult = HandleAddComboEdge(EdgeParams);
...
FMegalithActionResult FMegalithComboGraphActions::HandleLayoutComboGraph(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Asset is not a ComboGraph: %s"), *AssetPath));
	}
	...
	if (!EdGraphProp)
	{
		return FMegalithActionResult::Error(TEXT("Could not find EditorGraph property on ComboGraph"));
	}

	UEdGraph* EdGraph = Cast<UEdGraph>(EdGraphProp->GetObjectPropertyValue(EdGraphProp->ContainerPtrToValuePtr<void>(Graph)));
	if (!EdGraph)
	{
		return FMegalithActionResult::Error(TEXT("EditorGraph is null"));
	}

	if (EdGraph->Nodes.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("EditorGraph has no nodes"));
	}
...
void FMegalithComboGraphActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	UE_LOG(LogMegalithComboGraph, Log, TEXT("ComboGraph not installed — no combograph actions registered"));
}
