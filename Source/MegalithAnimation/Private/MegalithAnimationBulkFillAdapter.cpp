...
// MegalithAnimationBulkFillAdapter — Phase 5 Step 6 adapter.
//
...
//
// PoseSearch writes route via FMegalithReflectionWalker against the asset's
...
#define LOCTEXT_NAMESPACE "MegalithAnimationBulkFillAdapter"

namespace MegalithAnimationBulkFillInternal
{
	...
	static FDryRunReport HandlePoseSearchDatabase(const FBulkFillSpec& Spec)
	{
		...
		FScopedTransaction Transaction(LOCTEXT("AnimBulkFill_PSDB", "Megalith Animation Bulk Fill — PoseSearch DB"));
		Asset->Modify();
...
FDryRunReport FMegalithAnimationBulkFillAdapter::AnimationBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithAnimationBulkFillInternal;
...
FSchemaDescriptor FMegalithAnimationBulkFillAdapter::AnimationDescribe(const FString& TargetAsset)
{
	using namespace MegalithAnimationBulkFillInternal;
...
void FMegalithAnimationBulkFillAdapter::Register()
{
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("animation"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithAnimationBulkFillAdapter::AnimationBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithAnimationBulkFillAdapter::AnimationDescribe));
}

void FMegalithAnimationBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("animation"));
}
