...
#include "MegalithJsonUtils.h" // MegalithKeyToString (5.7/5.8 JSON key shim)
...
// DuplicateAsset (EditorScriptingUtilities — already a MegalithAnimation dep).
...
void FMegalithChooserActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// --- inspect_chooser ---
	Registry.RegisterAction(TEXT("chooser"), TEXT("inspect_chooser"),
		...
		FMegalithActionHandler::CreateStatic(&HandleInspectChooser),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("duplicate_chooser_tree"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateChooserTree),
		FParamSchemaBuilder()
			.Required(TEXT("source_assets"), TEXT("array"), TEXT("Array of source UChooserTable asset paths to duplicate"))
			.Required(TEXT("destination_folder"), TEXT("string"), TEXT("Destination content folder, e.g. /Game/Tests/Megalith"))
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("set_context_object_class"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetContextObjectClass),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("UChooserTable asset path"))
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("set_result_asset_reference"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetResultAssetReference),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("UChooserTable asset path"))
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("set_evaluate_chooser_result_reference"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetEvaluateChooserResultReference),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("chooser"), TEXT("validate_chooser"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateChooser),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("UChooserTable asset path"))
...
namespace
{
	FMegalithActionResult ChooserUnavailable()
	{
		return FMegalithActionResult::Error(TEXT("Chooser plugin not available"));
	}
}

FMegalithActionResult FMegalithChooserActions::HandleInspectChooser(const TSharedPtr<FJsonObject>&)        { return ChooserUnavailable(); }
FMegalithActionResult FMegalithChooserActions::HandleDuplicateChooserTree(const TSharedPtr<FJsonObject>&)  { return ChooserUnavailable(); }
FMegalithActionResult FMegalithChooserActions::HandleSetContextObjectClass(const TSharedPtr<FJsonObject>&) { return ChooserUnavailable(); }
FMegalithActionResult FMegalithChooserActions::HandleSetResultAssetReference(const TSharedPtr<FJsonObject>&){ return ChooserUnavailable(); }
FMegalithActionResult FMegalithChooserActions::HandleSetEvaluateChooserResultReference(const TSharedPtr<FJsonObject>&){ return ChooserUnavailable(); }
FMegalithActionResult FMegalithChooserActions::HandleValidateChooser(const TSharedPtr<FJsonObject>&)       { return ChooserUnavailable(); }
...
namespace
{
	...
	FString MakeDestAssetPath(const FString& DestFolder, const FString& SourceAssetPath)
	{
		...
		const FString ShortName = FMegalithAssetUtils::GetAssetName(SourceAssetPath);
	 ...
	 * Mirrors ApplyInputBinding (MegalithChooserAuthoringActions.cpp) const-side: reach the
...
FMegalithActionResult FMegalithChooserActions::HandleInspectChooser(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));

	UChooserTable* Table = FMegalithAssetUtils::LoadAssetByPath<UChooserTable>(AssetPath);
	if (!Table)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("ChooserTable not found: %s"), *AssetPath));
	}
	...
	if (bRecursive)
	{
		...
		Root->SetObjectField(TEXT("tree"), MegalithChooserTree::CollectTree(Table, VisitedTables));
	}
...
FMegalithActionResult FMegalithChooserActions::HandleDuplicateChooserTree(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* SourcesPtr = nullptr;
	if (!Params->TryGetArrayField(TEXT("source_assets"), SourcesPtr) || !SourcesPtr)
	{
		return FMegalithActionResult::Error(TEXT("Missing required array parameter: source_assets"));
	}
	const FString DestFolder = Params->GetStringField(TEXT("destination_folder"));
	if (DestFolder.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: destination_folder"));
	}
	...
	if (Params->TryGetObjectField(TEXT("remap_rules"), RemapObj) && RemapObj && RemapObj->IsValid())
	{
		for (const auto& Pair : (*RemapObj)->Values)
		{
			...
			{
				Remap.Add(NormalizePackagePath(MegalithKeyToString(Pair.Key)), Val);
			}
		}
	}
	...
	if (Remap.Num() > 0)
	{
				...
				(FInstancedStruct& S, int32 RowIdx, const FString& Where)
			{
				if (FAssetChooser* Hard = S.GetMutablePtr<FAssetChooser>())
				{
					if (Hard->Asset)
					{
						...
						if (NewRef != OldRef)
						{
							if (UObject* Loaded = FMegalithAssetUtils::LoadAssetByPath(NewRef))
							{
								Hard->Asset = Loaded;
								++RefsRemapped;
				...
				else if (FEvaluateChooser* Eval = S.GetMutablePtr<FEvaluateChooser>())
				{
					...
					if (Eval->Chooser)
					{
						...
						if (NewRef != OldRef)
						{
							if (UChooserTable* NewChild = FMegalithAssetUtils::LoadAssetByPath<UChooserTable>(NewRef))
							{
		...
		for (FDupRecord& Rec : DupRecords)
		{
			...
			if (DupTable->ParentTable)
			{
				...
				if (UChooserTable* P = FMegalithAssetUtils::LoadAssetByPath<UChooserTable>(NewParent))
				{
			...
			if (DupTable->RootChooser)
			{
				...
				if (UChooserTable* R = FMegalithAssetUtils::LoadAssetByPath<UChooserTable>(NewRoot))
				{
...
FMegalithActionResult FMegalithChooserActions::HandleSetContextObjectClass(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: class_path"));
	}
	...
	if (!NewClass)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Could not resolve class: %s"), *ClassPath));
	}
	...
	if (!Owner)
	{
		return FMegalithActionResult::Error(TEXT("ChooserTable has no context owner"));
	}
	...
	else
	{
		...
		if (!ClassEntry)
		{
			return FMegalithActionResult::Error(TEXT("No class-typed context parameter (FContextObjectTypeClass) found on this chooser"));
		}
	}
...
FMegalithActionResult FMegalithChooserActions::HandleSetResultAssetReference(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: row_or_column"));
	}
	const int32 Row = static_cast<int32>(RowVal);

	if (NewAssetPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: asset_path_value"));
	}
	...
	if (FAssetChooser* Hard = RowStruct.GetMutablePtr<FAssetChooser>())
	{
		...
		UObject* Loaded = FMegalithAssetUtils::LoadAssetByPath(NewAssetPath);
		if (!Loaded)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Could not load asset for hard reference: %s"), *NewAssetPath));
		}
		Hard->Asset = Loaded;
	}
	...
	return FMegalithActionResult::Success(Root);
#else
	return FMegalithActionResult::Error(TEXT("ResultsStructs is editor-only data; not available in this build"));
#endif
}
...
FMegalithActionResult FMegalithChooserActions::HandleSetEvaluateChooserResultReference(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: row"));
	}
	const int32 Row = static_cast<int32>(RowVal);

	if (ChildChooserPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: child_chooser_path"));
	}
	...
	UChooserTable* ChildTable = FMegalithAssetUtils::LoadAssetByPath<UChooserTable>(ChildChooserPath);
	if (!ChildTable)
	{
		return FMegalithActionResult::Error(FString::Printf(
...
FMegalithActionResult FMegalithChooserActions::HandleValidateChooser(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	for (int32 r = 0; r < Table->ResultsStructs.Num(); ++r)
	{
		...
		else if (!AssetRef.IsEmpty() && !FMegalithAssetUtils::AssetExists(AssetRef))
		{
			Issues.Add(MakeShared<FJsonValueString>(FString::Printf(
