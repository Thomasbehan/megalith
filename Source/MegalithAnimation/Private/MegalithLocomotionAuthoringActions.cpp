...
void FMegalithLocomotionAuthoringActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_root_motion_speed"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetRootMotionSpeed),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("bake_distance_curve"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBakeDistanceCurve),
		FParamSchemaBuilder()
...
namespace
{
	 ...
	 *  reflective resolution (MegalithAnimationActions.cpp:4300). Returns nullptr if the plugin is not present. */
...
FMegalithActionResult FMegalithLocomotionAuthoringActions::HandleGetRootMotionSpeed(const TSharedPtr<FJsonObject>& Params)
{
	const FString AnimPath = Params->GetStringField(TEXT("anim_path"));

	UAnimSequence* Seq = FMegalithAssetUtils::LoadAssetByPath<UAnimSequence>(AnimPath);
	if (!Seq)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("AnimSequence not found: %s"), *AnimPath));
	}
...
FMegalithActionResult FMegalithLocomotionAuthoringActions::HandleBakeDistanceCurve(const TSharedPtr<FJsonObject>& Params)
{
	const FString AnimPath = Params->GetStringField(TEXT("anim_path"));
	...
	if (!UAnimationModifiersAssetUserData::AddAnimationModifierOfClass(Seq, ModifierClass))
	{
		...
		return FMegalithActionResult::Error(TEXT("Failed to register DistanceCurveModifier into the AnimationModifiers stack"));
	}
	...
	{
		GEditor->EndTransaction();
		return FMegalithActionResult::Error(TEXT("AnimationModifiersAssetUserData missing after AddAnimationModifierOfClass"));
	}
	...
	{
		GEditor->EndTransaction();
		return FMegalithActionResult::Error(TEXT("DistanceCurveModifier instance not found in stack after registration"));
	}
