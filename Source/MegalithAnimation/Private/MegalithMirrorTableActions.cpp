...
void FMegalithMirrorTableActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("create_mirror_data_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateMirrorDataTable),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("set_schema_mirror_data_table"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetSchemaMirrorDataTable),
		FParamSchemaBuilder()
...
/** Access the private 'Skeletons' UPROPERTY on UPoseSearchSchema via reflection (mirror of the
  * Channels reflection helper in MegalithPoseSearchActions.cpp). Returns nullptr on failure. */
...
FMegalithActionResult FMegalithMirrorTableActions::HandleCreateMirrorDataTable(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!MDT) return FMegalithActionResult::Error(TEXT("Failed to create UMirrorDataTable object"));
...
FMegalithActionResult FMegalithMirrorTableActions::HandleSetSchemaMirrorDataTable(const TSharedPtr<FJsonObject>& Params)
{
	...
	UMirrorDataTable* MDT = FMegalithAssetUtils::LoadAssetByPath<UMirrorDataTable>(MirrorTablePath);
	if (!MDT)
		return FMegalithActionResult::Error(FString::Printf(TEXT("UMirrorDataTable not found: %s"), *MirrorTablePath));

	TArray<FPoseSearchRoledSkeleton>* Skeletons = GetSchemaSkeletonsMutable(Schema);
	if (!Skeletons)
		return FMegalithActionResult::Error(TEXT("Failed to access Skeletons property on schema via reflection"));
