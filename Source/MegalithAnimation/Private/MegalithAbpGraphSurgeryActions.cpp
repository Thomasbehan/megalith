...
// Resolvers shared with MegalithAbpWriteActions.cpp (same module, internal linkage there).
...
// ---------------------------------------------------------------------------
// Compile-with-error-harvest (mirrors MegalithBlueprintCompileActions.cpp:98-144)
// ---------------------------------------------------------------------------
...
void FMegalithAbpGraphSurgeryActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("rebuild_evaluate_chooser_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRebuildEvaluateChooserNode),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("replace_evaluate_chooser_nodes"),
		...
		FMegalithActionHandler::CreateStatic(&HandleReplaceEvaluateChooserNodes),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("anim_blueprint"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("add_evaluate_chooser_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddEvaluateChooserNode),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("wire_chooser_to_motion_matching"),
		...
		FMegalithActionHandler::CreateStatic(&HandleWireChooserToMotionMatching),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("bind_chooser_database_via_threadsafe"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBindChooserDatabaseViaThreadSafe),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("bind_threadsafe_update_function"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBindThreadsafeUpdateFunction),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("abp_path"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("duplicate_reparent_and_sanitize"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateReparentAndSanitize),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("source_abp"), TEXT("Source Animation Blueprint asset path"))
			.RequiredAssetPath(TEXT("destination_path"), TEXT("Destination ABP asset path for the duplicate (e.g. /Game/Tests/Megalith/ABP_Copy)"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("find_node_slice"),
		...
		FMegalithActionHandler::CreateStatic(&HandleFindNodeSlice),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("anim_blueprint"), TEXT("Animation Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("remove_node_slice"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveNodeSlice),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("anim_blueprint"), TEXT("Animation Blueprint asset path"))
			.Required(TEXT("graph_name"), TEXT("string"), TEXT("Graph containing the seed node"))
			.Required(TEXT("seed_node_ref"), TEXT("string"), TEXT("UObject name of the seed node"))
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleFindNodeSlice(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!ABP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Animation Blueprint not found: %s"), *AssetPath));
	}
	...
	if (!Graph)
	{
		return FMegalithActionResult::Error(GraphErr);
	}

	UEdGraphNode* Seed = FindNodeByNameBP(ABP, SeedRef, Graph);
	if (!Seed)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Seed node '%s' not found in graph '%s'"), *SeedRef, *GraphName));
	}
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleRemoveNodeSlice(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("anim_blueprint"));
	const FString GraphName = Params->GetStringField(TEXT("graph_name"));
	const FString SeedRef = Params->GetStringField(TEXT("seed_node_ref"));
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleDuplicateReparentAndSanitize(const TSharedPtr<FJsonObject>& Params)
{
	...
	UAnimBlueprint* SourceABP = FMegalithAssetUtils::LoadAssetByPath<UAnimBlueprint>(SourcePath);
	if (!SourceABP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Source Animation Blueprint not found: %s"), *SourcePath));
	}
	...
	if (!NewParent)
	{
		return FMegalithActionResult::Error(ParentErr);
	}
	...
	if (!bDryRun)
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Destination already exists: %s"), *DestPath));
		}
		UObject* Dup = UEditorAssetLibrary::DuplicateAsset(SourcePath, DestPath);
		UAnimBlueprint* DupABP = Cast<UAnimBlueprint>(Dup);
		if (!DupABP)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("DuplicateAsset failed: %s -> %s"), *SourcePath, *DestPath));
		}
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleRebuildEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("anim_blueprint"));
	const FString GraphName = Params->GetStringField(TEXT("graph_name"));
	...
	UChooserTable* Chooser = FMegalithAssetUtils::LoadAssetByPath<UChooserTable>(ChooserPath);
	if (!Chooser)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Chooser table not found: %s"), *ChooserPath));
	}
	...
	if (!OldNode)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node '%s' not found in graph '%s'"), *NodeRef, *GraphName));
	}
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleReplaceEvaluateChooserNodes(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = Params->GetStringField(TEXT("anim_blueprint"));
	bool bDryRun = true;
	Params->TryGetBoolField(TEXT("dry_run"), bDryRun);
	...
	{
		return FMegalithActionResult::Error(TEXT("replacements array is required"));
	}
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleAddEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!NewNode)
	{
		return FMegalithActionResult::Error(TEXT("Reflective UK2Node_EvaluateChooser2 spawn failed (no 'Chooser' FObjectProperty or node could not be created)."));
	}
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleWireChooserToMotionMatching(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath        = Params->GetStringField(TEXT("abp_path"));
	...
	if (!ChooserNode)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Chooser node '%s' not found in graph '%s'"), *ChooserNodeRef, *GraphName));
	}
	UEdGraphNode* MMNode = FindNodeByNameBP(ABP, MMNodeRef, Graph);
	if (!MMNode)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Motion Matching node '%s' not found in graph '%s'"), *MMNodeRef, *GraphName));
	}
	...
	if (!SourcePin)
	{
		return FMegalithActionResult::Error(TEXT("Chooser node has no output result pin to wire"));
	}
	...
	if (!TargetPin)
	{
		return FMegalithActionResult::Error(TEXT("Motion Matching node has no 'Database' input pin. Ensure the node is a UAnimGraphNode_MotionMatching with the Database pin shown."));
	}

	const UEdGraphSchema* Schema = Graph->GetSchema();
	if (!Schema)
	{
		return FMegalithActionResult::Error(TEXT("Graph has no schema"));
	}
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleBindChooserDatabaseViaThreadSafe(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath     = Params->GetStringField(TEXT("abp_path"));
	const FString ChooserPath = Params->GetStringField(TEXT("chooser_path"));
	const FString MMNodeRef   = Params->GetStringField(TEXT("mm_node"));
	...
	if (DbVar.IsEmpty()) return FMegalithActionResult::Error(TEXT("selected_database_var is required"));
	if (MMNodeRef.IsEmpty()) return FMegalithActionResult::Error(TEXT("mm_node is required"));
	...
	if (FObjectProperty* ChooserProp = FindFProperty<FObjectProperty>(EvalClass, TEXT("Chooser")))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Chooser binding is null after spawn — node would be pruned. Aborting."));
		}
	}

	const UEdGraphSchema* Schema = FuncGraph->GetSchema();
	if (!Schema) return FMegalithActionResult::Error(TEXT("Function graph has no schema"));
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleRebuildEvaluateChooserNode(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FMegalithActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). Evaluate-Chooser node surgery is disabled in this build."));
}
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleAddEvaluateChooserNode(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FMegalithActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). add_evaluate_chooser_node is disabled in this build."));
}

FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleWireChooserToMotionMatching(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FMegalithActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). wire_chooser_to_motion_matching is disabled in this build."));
}

FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleBindChooserDatabaseViaThreadSafe(const TSharedPtr<FJsonObject>& /*Params*/)
{
	return FMegalithActionResult::Error(TEXT("Chooser plugin not available (WITH_CHOOSER=0). bind_chooser_database_via_threadsafe is disabled in this build."));
}
 ...
 * Mirrors the established Megalith resolution order: TryFindTypeSlow (path or short name) ->
...
FMegalithActionResult FMegalithAbpGraphSurgeryActions::HandleBindThreadsafeUpdateFunction(const TSharedPtr<FJsonObject>& Params)
{
	const FString AbpPath = Params->GetStringField(TEXT("abp_path"));
	...
	{
		return FMegalithActionResult::Error(TEXT("function is required and must be an object { class, name }"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("function.class and function.name are both required"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("result_target is required and must be an object { var, [node], [pin] }"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("result_target.var is required (the self variable to store the call result into; it must already exist on the ABP)"));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Unsupported function signature for a thread-safe call: %s"), *GuardError));
	}
	...
	if (!FuncGraph)
	{
		...
		if (!FuncGraph)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create function graph: %s"), *FuncGraphName));
		}
		FBlueprintEditorUtils::AddFunctionGraph<UClass>(ABP, FuncGraph, /*bIsUserCreated=*/true, /*SignatureFromObject=*/(UClass*)nullptr);
		bFuncCreated = true;
	}
	...
	if (!EntryNode)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("No function entry node in graph '%s'"), *FuncGraphName));
	}
	EntryNode->Modify();
	EntryNode->MetaData.bThreadSafe = true;
	...
	if (Params->TryGetArrayField(TEXT("arg_bindings"), ArgBindings) && ArgBindings)
	{
		for (const TSharedPtr<FJsonValue>& V : *ArgBindings)
		{
			...
			if (!V->TryGetObject(EntryPtr) || !EntryPtr)
			{
				return FMegalithActionResult::Error(TEXT("each arg_bindings entry must be an object { pin, value } or { pin, self }"));
			}
			const TSharedPtr<FJsonObject>& Entry = *EntryPtr;
			FString PinName;
			...
			if (PinName.IsEmpty())
			{
				return FMegalithActionResult::Error(TEXT("arg_bindings entry missing required 'pin'"));
			}
			...
			{
				return FMegalithActionResult::Error(FString::Printf(TEXT("arg_bindings pin '%s' is an exec pin — only data input pins are bindable"), *PinName));
			}
			...
			if (bWantSelf)
			{
				...
				{
					return FMegalithActionResult::Error(FString::Printf(TEXT("failed to wire Self into arg pin '%s'"), *PinName));
				}
				++Wired;
			}
			else
