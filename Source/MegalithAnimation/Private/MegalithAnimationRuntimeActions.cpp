...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithAnimRuntime, Log, All);
...
namespace
{
	...
	TSharedPtr<FJsonValue> ReadPropertyValue(UObject* Obj, const FString& VarName, FString& OutTypeName)
	{
		...
		const MegalithStructField::FResolved Resolved = MegalithStructField::Resolve(Obj, VarName);
		if (!Resolved.Leaf)
		{
			OutTypeName = TEXT("<not found>");
			return nullptr;
		...
		const FParityTolerance& Tol, const TSharedPtr<FJsonObject>& OutEntry)
	{
		...
		const MegalithStructField::FResolved ResA = MegalithStructField::Resolve(A, VarName);
		const MegalithStructField::FResolved ResB = MegalithStructField::Resolve(B, VarName);
...
void FMegalithAnimationRuntimeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("animation"), TEXT("sample_pie_anim_instance"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSamplePIEAnimInstance),
		FParamSchemaBuilder()
	...
	UE_LOG(LogMegalithAnimRuntime, Log, TEXT("MegalithAnimation Runtime: registered 1 action"));
}

// ── sample_pie_anim_instance ────────────────────────────────────────

FMegalithActionResult FMegalithAnimationRuntimeActions::HandleSamplePIEAnimInstance(const TSharedPtr<FJsonObject>& Params)
{
