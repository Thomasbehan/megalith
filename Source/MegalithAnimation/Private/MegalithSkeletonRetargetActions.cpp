...
bool FMegalithSkeletonRetargetActions::ParseTranslationRetargetMode(
		...
		const UScriptStruct* ConcreteType, const void* StructMemory, const UObject* Owner)
	{
		...
		for (TFieldIterator<FProperty> It(ConcreteType); It; ++It)
		{
			...
			Out->SetField(Prop->GetName(),
				FMegalithReflectionReader::PropertyToJsonValue(Prop, ValuePtr, Owner));
		}
		return Out;
	}
...
void FMegalithSkeletonRetargetActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("animation"), TEXT("set_bone_translation_retargeting"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetBoneTranslationRetargeting),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_bone_translation_retargeting"),
		TEXT("Read the current per-bone translation retargeting mode for a Skeleton. Returns one entry "
			 ...
			 "those bones."),
		FMegalithActionHandler::CreateStatic(&HandleGetBoneTranslationRetargeting),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("skeleton_path"), TEXT("Skeleton asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("set_ik_rig_bone_settings"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetIkRigBoneSettings),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_ik_rig_bone_settings"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetIkRigBoneSettings),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("ik_rig_path"), TEXT("IK Rig (UIKRigDefinition) asset path"))
...
FMegalithActionResult FMegalithSkeletonRetargetActions::HandleSetBoneTranslationRetargeting(
	const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Params->TryGetArrayField(TEXT("entries"), EntriesArr))
	{
		for (const TSharedPtr<FJsonValue>& Val : *EntriesArr)
		{
			...
			{
				return FMegalithActionResult::Error(TEXT("Each item in 'entries' must be an object {bone, mode}."));
			}

			FString BoneName;
			if (!(*EntryObj)->TryGetStringField(TEXT("bone"), BoneName) || BoneName.IsEmpty())
			{
				return FMegalithActionResult::Error(TEXT("Each entry requires a non-empty 'bone' field."));
			}
...
FMegalithActionResult FMegalithSkeletonRetargetActions::HandleGetBoneTranslationRetargeting(
	const TSharedPtr<FJsonObject>& Params)
{
	const FString SkeletonPath = Params->GetStringField(TEXT("skeleton_path"));
...
FMegalithActionResult FMegalithSkeletonRetargetActions::HandleSetIkRigBoneSettings(
	const TSharedPtr<FJsonObject>& Params)
{
	const FString IkRigPath = Params->GetStringField(TEXT("ik_rig_path"));

	UIKRigDefinition* IkRig = FMegalithAssetUtils::LoadAssetByPath<UIKRigDefinition>(IkRigPath);
	if (!IkRig)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("IK Rig not found: %s"), *IkRigPath));
	}

	FString BoneNameStr;
	if (!Params->TryGetStringField(TEXT("bone_name"), BoneNameStr) || BoneNameStr.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("'bone_name' is required and must be non-empty."));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("'settings' is required and must be an object of {field: value} pairs."));
	}
	const TSharedPtr<FJsonObject>& SettingsObj = *SettingsObjPtr;
	if (SettingsObj->Values.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("'settings' must contain at least one field to set."));
	}
	...
	for (int32 SolverIndex = 0; SolverIndex < NumSolvers; ++SolverIndex)
	{
		...
		for (const auto& Pair : SettingsObj->Values)
		{
			...
			AppliedObj->SetField(TEXT("value"),
				FMegalithReflectionReader::PropertyToJsonValue(Prop, ValuePtr, IkRig));
...
FMegalithActionResult FMegalithSkeletonRetargetActions::HandleGetIkRigBoneSettings(
	const TSharedPtr<FJsonObject>& Params)
{
	const FString IkRigPath = Params->GetStringField(TEXT("ik_rig_path"));
