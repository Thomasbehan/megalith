...
void FMegalithControlRigWriteActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("get_control_rig_graph"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetControlRigGraph),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("add_control_rig_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddControlRigNode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ControlRigBlueprint asset path"))
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("connect_control_rig_pins"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConnectControlRigPins),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("ControlRigBlueprint asset path"))
...
FMegalithActionResult FMegalithControlRigWriteActions::HandleGetControlRigGraph(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!CRB) return FMegalithActionResult::Error(Error);

	URigVMGraph* Graph = GetGraphFromBlueprint(CRB, GraphName, Error);
	if (!Graph) return FMegalithActionResult::Error(Error);
...
FMegalithActionResult FMegalithControlRigWriteActions::HandleAddControlRigNode(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString StructPath = Params->GetStringField(TEXT("struct_path"));

	if (StructPath.IsEmpty())
		return FMegalithActionResult::Error(TEXT("struct_path is required"));
	...
	URigVMGraph* Graph = CRB->GetDefaultModel();
	if (!Graph) return FMegalithActionResult::Error(TEXT("Control Rig has no default graph"));
	...
	if (!NewNode)
	{
		...
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to add unit node from struct path: %s"), *StructPath));
	}
	...
	if (Params->TryGetObjectField(TEXT("pin_defaults"), PinDefaultsObj) && PinDefaultsObj && (*PinDefaultsObj)->Values.Num() > 0)
	{
		for (const auto& KV : (*PinDefaultsObj)->Values)
		{
			...
			else
			{
				...
				FJsonSerializer::Serialize(KV.Value, MegalithKeyToString(KV.Key), Writer);
				Value = JsonStr;
			}
			...
			if (!bSetOk)
			{
				UE_LOG(LogTemp, Warning, TEXT("Megalith: Failed to set pin default %s = %s"), *PinPath, *Value);
			}
		}
	}
...
FMegalithActionResult FMegalithControlRigWriteActions::HandleConnectControlRigPins(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	if (SourcePin.IsEmpty() || TargetPin.IsEmpty())
		return FMegalithActionResult::Error(TEXT("source_pin and target_pin are required"));
