...
void FMegalithAnimLayoutActions::RegisterActions(FMegalithToolRegistry& Registry)
{
...
/** Format a single graph via IMegalithGraphFormatter. Returns a JSON object with results. */
TSharedPtr<FJsonObject> FormatSingleGraph(const FString& GraphLabel, UEdGraph* Graph, bool bExplicitBA, FString& OutError)
{
	...
	FString FormatError;
	bool bSuccess = IMegalithGraphFormatter::Get().FormatGraph(Graph, NodesFormatted, FormatError);
 ...
 *   - bForceBuiltin: always built-in (formatter 'builtin'/'megalith').
	...
	float ColumnSpacing, float RowSpacing, FString& OutError)
{
	...
	const bool bBAAvailable = IMegalithGraphFormatter::IsAvailable()
		&& IMegalithGraphFormatter::Get().SupportsGraph(Graph);
...
FMegalithActionResult FMegalithAnimLayoutActions::HandleAutoLayout(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Validate formatter param. 'megalith' is accepted as an alias for 'builtin'
	// (the built-in layered layout IS the Megalith-native formatter now).
	if (Formatter != TEXT("auto") && Formatter != TEXT("blueprint_assist")
		&& Formatter != TEXT("builtin") && Formatter != TEXT("megalith"))
	{
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("Unknown formatter '%s'. Supported: 'auto', 'blueprint_assist', 'builtin', 'megalith'"), *Formatter));
	}
	...
	if (!ABP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("AnimBlueprint not found: %s"), *AssetPath));
	}

	const bool bExplicitBA = (Formatter == TEXT("blueprint_assist"));
	const bool bForceBuiltin = (Formatter == TEXT("builtin") || Formatter == TEXT("megalith"));
	...
	if (GraphName.Equals(TEXT("all"), ESearchCase::IgnoreCase))
	{
		...
		if (AllGraphs.Num() == 0)
		{
			return FMegalithActionResult::Error(TEXT("No graphs found in this Animation Blueprint"));
		}
	...
	if (GraphName.Equals(TEXT("AnimGraph"), ESearchCase::IgnoreCase) || GraphName.IsEmpty())
	{
		...
		if (!TargetGraph)
		{
			return FMegalithActionResult::Error(TEXT("No AnimGraph found in this Animation Blueprint"));
		}
	}
	else
	{
