...
/**
 * IK Retargeter op-stack + retarget-pose authoring actions for Megalith (animation namespace).
 *
 ...
 * BY VALUE (copy-mutate-set). The deprecated megalithic settings structs in
 ...
 */
class MEGALITHANIMATION_API FMegalithRetargetSettingsActions
{
public:
...
private:
	...
	static FMegalithActionResult HandleAlignRetargetPose(const TSharedPtr<FJsonObject>& Params);

	// T1-R2 — retarget pose read/write (v1: from_reference + bone_deltas[]; from_animation DEFERRED)
	static FMegalithActionResult HandleGetRetargetPose(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleSetRetargetPose(const TSharedPtr<FJsonObject>& Params);

	// T1-R3 — FK/IK chains op settings read/write
	static FMegalithActionResult HandleGetRetargetChainSettings(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleSetRetargetChainSettings(const TSharedPtr<FJsonObject>& Params);

	// T1-R5 — Pelvis Motion op settings + Speed Planting / ground-lock
	static FMegalithActionResult HandleSetRetargetRootSettings(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleEnableFootGroundLock(const TSharedPtr<FJsonObject>& Params);
