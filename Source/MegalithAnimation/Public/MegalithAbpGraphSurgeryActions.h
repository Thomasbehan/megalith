...
/**
 * AnimBlueprint graph-surgery actions for Megalith (animation namespace).
 *
 * Higher-risk graph repair kept separate from the stable MegalithAbpWriteActions
 ...
 */
class MEGALITHANIMATION_API FMegalithAbpGraphSurgeryActions
{
public:
...
private:
	...
	static FMegalithActionResult HandleRebuildEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleReplaceEvaluateChooserNodes(const TSharedPtr<FJsonObject>& Params);
	...
	static FMegalithActionResult HandleAddEvaluateChooserNode(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleWireChooserToMotionMatching(const TSharedPtr<FJsonObject>& Params);
	...
	static FMegalithActionResult HandleBindChooserDatabaseViaThreadSafe(const TSharedPtr<FJsonObject>& Params);
	...
	static FMegalithActionResult HandleBindThreadsafeUpdateFunction(const TSharedPtr<FJsonObject>& Params);

	// F11 — duplicate + reparent + dependency classification
	static FMegalithActionResult HandleDuplicateReparentAndSanitize(const TSharedPtr<FJsonObject>& Params);

	// F12 — directional node-slice compute / removal
	static FMegalithActionResult HandleFindNodeSlice(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleRemoveNodeSlice(const TSharedPtr<FJsonObject>& Params);
};
