...
/**
 * Locomotion AUTHORING actions for Megalith — registered under the existing
 * `animation` namespace (alongside the other MegalithAnimation action classes).
 *
 * 2 actions:
 ...
 * Library appears anywhere in this class, so MegalithAnimation.Build.cs needs no
 ...
 */
class MEGALITHANIMATION_API FMegalithLocomotionAuthoringActions
{
public:
...
private:
	static FMegalithActionResult HandleGetRootMotionSpeed(const TSharedPtr<FJsonObject>& Params);   // T1-L2
	static FMegalithActionResult HandleBakeDistanceCurve(const TSharedPtr<FJsonObject>& Params);     // T1-L3
};
