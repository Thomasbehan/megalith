...
/**
 * Chooser table AUTHORING actions for Megalith — registered under the existing
 * `chooser` namespace (alongside FMegalithChooserActions, which owns the read /
 ...
 * link (mirrors MegalithChooserActions.cpp:99-105).
 *
 ...
 */
class MEGALITHANIMATION_API FMegalithChooserAuthoringActions
{
public:
...
private:
	static FMegalithActionResult HandleCreateChooserTable(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleAddChooserColumn(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleAddChooserRow(const TSharedPtr<FJsonObject>& Params);
	static FMegalithActionResult HandleSetChooserCell(const TSharedPtr<FJsonObject>& Params);
};
