...
public class MegalithGAS : ModuleRules
{
	public MegalithGAS(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		PublicDependencyModuleNames.AddRange(new string[]
		{
			...
			// UMG: needed publicly because MegalithGASAttributeBindingClassExtension subclasses
		...
		{
			"MegalithCore", "MegalithBlueprint",
			...
			if (Directory.Exists(ProjectPluginsDir))
			{
				bHasGBA = Directory.Exists(
					...
					|| Directory.GetDirectories(
						ProjectPluginsDir, "GameplayAbilities*",
			...
			if (!bHasGBA)
			{
				...
				if (Directory.Exists(MarketplaceDir))
				{
					bHasGBA = Directory.GetDirectories(
						MarketplaceDir, "GameplayAbilities*",
						SearchOption.TopDirectoryOnly).Length > 0;
				}
