...
void FMegalithGASInspectActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("export_gas_manifest"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExportGASManifest),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("snapshot_gas_state"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSnapshotGASState),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_tag_state"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetTagState),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_cooldown_state"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetCooldownState),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor name, label, or path in PIE world"))
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("trace_ability_activation"),
		...
		FMegalithActionHandler::CreateStatic(&HandleTraceAbilityActivation),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor name, label, or path in PIE world"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("compare_gas_states"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCompareGASStates),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithGASInspectActions::HandleExportGASManifest(const TSharedPtr<FJsonObject>& Params)
{
	...
	for (const FAssetData& AssetData : AllBPs)
	{
		...
		UGameplayAbility* AbilityCDO = MegalithGAS::GetBlueprintCDO<UGameplayAbility>(BP);
	...
	for (const FAssetData& AssetData : AllBPs)
	{
		...
		UObject* Obj = AssetData.GetAsset();
		UBlueprint* BP = Cast<UBlueprint>(Obj);
		if (!BP || !MegalithGAS::IsAttributeSetBlueprint(BP)) continue;

		UAttributeSet* SetCDO = MegalithGAS::GetBlueprintCDO<UAttributeSet>(BP);
		if (!SetCDO) continue;
	...
	if (!OutputPath.IsEmpty())
	{
		...
		if (!bWritten)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to write manifest to: %s"), *OutputPath));
		}
	}
...
FMegalithActionResult FMegalithGASInspectActions::HandleSnapshotGASState(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* World = MegalithGAS::GetPIEWorld();
	if (!World)
	{
...
FMegalithActionResult FMegalithGASInspectActions::HandleGetTagState(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorIdent;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor"), ActorIdent, Err))
		return Err;

	AActor* Actor = MegalithGAS::FindActorInPIE(ActorIdent);
	if (!Actor)
	{
...
FMegalithActionResult FMegalithGASInspectActions::HandleGetCooldownState(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorIdent;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor"), ActorIdent, Err))
...
FMegalithActionResult FMegalithGASInspectActions::HandleTraceAbilityActivation(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorIdent;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor"), ActorIdent, Err))
	...
	if (!MegalithGAS::RequireStringParam(Params, TEXT("ability_class"), AbilityClassStr, Err))
		return Err;
	...
	// ── Check 6: Blocked by other abilities ──
	{
		...
		FProperty* AbilityTagsProp = MegalithGAS::FindAbilityAssetTagsProperty(AbilityCDO->GetClass());
...
FMegalithActionResult FMegalithGASInspectActions::HandleCompareGASStates(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: snapshot_a (object)"));
	}
	if (!Params->TryGetObjectField(TEXT("snapshot_b"), SnapshotB) || !*SnapshotB)
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: snapshot_b (object)"));
	}
