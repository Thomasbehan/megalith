...
void FMegalithGASTagActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("add_gameplay_tags"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddGameplayTags),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_tag_hierarchy"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetTagHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("search_tag_usage"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSearchTagUsage),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_tag_hierarchy"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldTagHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("rename_tag"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRenameTag),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("remove_gameplay_tags"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveGameplayTags),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_tag_consistency"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateTagConsistency),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("audit_tag_naming"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAuditTagNaming),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("export_tag_hierarchy"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExportTagHierarchy),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("import_tag_hierarchy"),
		...
		FMegalithActionHandler::CreateStatic(&HandleImportTagHierarchy),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithGASTagActions::HandleAddGameplayTags(const TSharedPtr<FJsonObject>& Params)
{
	// Parse required tags array
	TArray<FString> Tags = MegalithGAS::ParseStringArray(Params, TEXT("tags"));
	if (Tags.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: tags"));
	}
	...
	else
	{
		// ── DataTable mode ──
		FString Error;
		UObject* Existing = MegalithGAS::LoadAssetFromPath(TablePath, Error);
		...
		if (!DataTable)
		{
			...
			FString ExistError;
			if (!MegalithGAS::EnsureAssetPathFree(TablePath, AssetName, ExistError))
			{
				return FMegalithActionResult::Error(ExistError);
			}

			UPackage* Package = MegalithGAS::GetOrCreatePackage(TablePath, Error);
			if (!Package)
			{
...
FMegalithActionResult FMegalithGASTagActions::HandleGetTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithGASTagActions::HandleSearchTagUsage(const TSharedPtr<FJsonObject>& Params)
{
	FString TagString;
	FMegalithActionResult ErrorResult;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("tag"), TagString, ErrorResult))
	{
		return ErrorResult;
	}
...
FMegalithActionResult FMegalithGASTagActions::HandleScaffoldTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	FString Preset;
	FMegalithActionResult ErrorResult;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("preset"), Preset, ErrorResult))
	{
		return ErrorResult;
	}
	...
	FMegalithActionResult AddResult = HandleAddGameplayTags(AddParams);
...
FMegalithActionResult FMegalithGASTagActions::HandleRenameTag(const TSharedPtr<FJsonObject>& Params)
{
	FString OldTag, NewTag;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("old_tag"), OldTag, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("new_tag"), NewTag, Err)) return Err;
...
FMegalithActionResult FMegalithGASTagActions::HandleRemoveGameplayTags(const TSharedPtr<FJsonObject>& Params)
{
	TArray<FString> Tags = MegalithGAS::ParseStringArray(Params, TEXT("tags"));
	if (Tags.Num() == 0)
	{
...
FMegalithActionResult FMegalithGASTagActions::HandleValidateTagConsistency(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithGASTagActions::HandleAuditTagNaming(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithGASTagActions::HandleExportTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!OutputPath.IsEmpty())
	{
		...
		if (!bWritten)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to write to: %s"), *OutputPath));
		}
	}
...
FMegalithActionResult FMegalithGASTagActions::HandleImportTagHierarchy(const TSharedPtr<FJsonObject>& Params)
{
	FString SourcePath;
	FMegalithActionResult Err;
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to read file: %s"), *SourcePath));
	}
	...
	if (ImportedTags.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No tags found in source file"));
	}
