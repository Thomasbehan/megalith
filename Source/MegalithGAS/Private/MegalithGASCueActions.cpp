...
void FMegalithGASCueActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("create_gameplay_cue_notify"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateGameplayCueNotify),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("link_cue_to_effect"),
		...
		FMegalithActionHandler::CreateStatic(&HandleLinkCueToEffect),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("unlink_cue_from_effect"),
		...
		FMegalithActionHandler::CreateStatic(&HandleUnlinkCueFromEffect),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("effect_path"), TEXT("GameplayEffect Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_cue_info"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetCueInfo),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("list_gameplay_cues"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListGameplayCues),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("set_cue_parameters"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetCueParameters),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("find_cue_triggers"),
		...
		FMegalithActionHandler::CreateStatic(&HandleFindCueTriggers),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_cue_coverage"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateCueCoverage),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("batch_create_cues"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBatchCreateCues),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_cue_library"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldCueLibrary),
		FParamSchemaBuilder()
...
/** Load a GE Blueprint (wraps MegalithGAS::LoadGameplayEffectBP). */
bool LoadGEForCue(
...
{
	FString Error;
	if (!MegalithGAS::LoadGameplayEffectBP(EffectPath, OutBP, OutGE, Error))
	{
...
FMegalithActionResult FMegalithGASCueActions::HandleCreateGameplayCueNotify(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!MegalithGAS::RequireStringParam(Params, TEXT("cue_tag"), CueTag, Err)) return Err;
...
FMegalithActionResult FMegalithGASCueActions::HandleLinkCueToEffect(const TSharedPtr<FJsonObject>& Params)
{
	FString EffectPath, CueTag;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("effect_path"), EffectPath, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("cue_tag"), CueTag, Err)) return Err;
	...
	{
		TSharedPtr<FJsonObject> Result = MegalithGAS::MakeAssetResult(EffectPath,
	...
	else
	{
		...
		UE_LOG(LogMegalithGAS, Warning, TEXT("Could not find GameplayCues property on GE CDO"));
		return FMegalithActionResult::Error(TEXT("Failed to access GameplayCues property on GameplayEffect CDO"));
	}
...
FMegalithActionResult FMegalithGASCueActions::HandleUnlinkCueFromEffect(const TSharedPtr<FJsonObject>& Params)
{
	FString EffectPath, CueTag;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("effect_path"), EffectPath, Err)) return Err;
...
FMegalithActionResult FMegalithGASCueActions::HandleGetCueInfo(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FMegalithActionResult Err;
	...
	FString Error;
	UObject* Obj = MegalithGAS::LoadAssetFromPath(AssetPath, Error);
	UBlueprint* BP = Cast<UBlueprint>(Obj);
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load GCN Blueprint: %s — %s"), *AssetPath, *Error));
	}

	UObject* CDO = (BP->GeneratedClass) ? BP->GeneratedClass->GetDefaultObject() : nullptr;
	if (!CDO)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to get CDO for: %s"), *AssetPath));
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayCue Notify"), *AssetPath));
	}
...
FMegalithActionResult FMegalithGASCueActions::HandleListGameplayCues(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithGASCueActions::HandleSetCueParameters(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("asset_path"), AssetPath, Err)) return Err;
	...
	for (const FParamMapping& Mapping : Mappings)
	{
		...
		for (const FString& PropName : Mapping.PropertyNames)
		{
			...
			if (Mapping.bIsAssetRef)
			{
				...
				else if (FObjectPropertyBase* ObjProp = CastField<FObjectPropertyBase>(Prop))
				{
					...
					UObject* RefObj = FMegalithAssetUtils::LoadAssetByPath(ExpectedClass, ParamValue);
					if (RefObj)
					{
				...
				{
					UObject* RefObj = FMegalithAssetUtils::LoadAssetByPath(UClass::StaticClass(), ParamValue);
					if (RefObj)
					{
		...
		if (!bSet)
		{
			UE_LOG(LogMegalithGAS, Warning, TEXT("Could not set param '%s' — no matching property found on CDO of %s"), *Mapping.ParamName, *CDO->GetClass()->GetName());
		}
	}

	if (SetProperties.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No parameters were set. Check that the property names match the cue type."));
	}
...
FMegalithActionResult FMegalithGASCueActions::HandleFindCueTriggers(const TSharedPtr<FJsonObject>& Params)
{
	FString CueTag;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("cue_tag"), CueTag, Err)) return Err;
...
FMegalithActionResult FMegalithGASCueActions::HandleValidateCueCoverage(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithGASCueActions::HandleBatchCreateCues(const TSharedPtr<FJsonObject>& Params)
{
	const TArray<TSharedPtr<FJsonValue>>* CuesArray;
	if (!Params->TryGetArrayField(TEXT("cues"), CuesArray) || CuesArray->Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: cues"));
	}
	...
	for (const TSharedPtr<FJsonValue>& CueVal : *CuesArray)
	{
		...
		FMegalithActionResult CreateResult = HandleCreateGameplayCueNotify(CreateParams);
...
FMegalithActionResult FMegalithGASCueActions::HandleScaffoldCueLibrary(const TSharedPtr<FJsonObject>& Params)
{
	FString Preset, SavePathPrefix;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("preset"), Preset, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("save_path_prefix"), SavePathPrefix, Err)) return Err;
	...
	FMegalithActionResult BatchResult = HandleBatchCreateCues(BatchParams);
