...
UBlueprint* LoadBlueprintFromParams(const TSharedPtr<FJsonObject>& Params, FString& OutAssetPath, FString& OutError)
{
	...
	{
		if (UBlueprint* BP = Cast<UBlueprint>(FMegalithAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), FullPath)))
		{
			return BP;
		}
	}
...
{
	UObject* Obj = FMegalithAssetUtils::LoadAssetByPath(UObject::StaticClass(), AssetPath);
	if (!Obj)
	{
...
UPackage* GetOrCreatePackage(const FString& SavePath, FString& OutError)
{
	...
	{
		UE_LOG(LogMegalithGAS, Warning, TEXT("GetOrCreatePackage rejected path: %s"), *ValidationError);
	...
	if (NormalizedName != PackageName)
	{
		...
		UE_LOG(LogMegalithGAS, Warning, TEXT("GetOrCreatePackage rejected path: %s"), *OutError);
		return nullptr;
	}
		...
		for (const auto& Val : *TagArray)
		{
			...
			if (Val->TryGetString(TagStr) && !TagStr.IsEmpty())
			{
				...
				else
				{
					...
					UE_LOG(LogMegalithGAS, Warning, TEXT("Gameplay tag not found: '%s' (field=%s)"), *TagStr, *FieldName);
