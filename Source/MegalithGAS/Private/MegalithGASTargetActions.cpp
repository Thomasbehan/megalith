...
void FMegalithGASTargetActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("create_target_actor"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateTargetActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("configure_target_actor"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureTargetActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("add_targeting_to_ability"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddTargetingToAbility),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_fps_targeting"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldFPSTargeting),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("ability_path"), TEXT("Ability Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_targeting"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateTargeting),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("ability_path"), TEXT("Ability Blueprint asset path"))
			.Build());
}
...
FMegalithActionResult FMegalithGASTargetActions::HandleCreateTargetActor(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!MegalithGAS::RequireStringParam(Params, TEXT("targeting_type"), TargetingType, Err)) return Err;
	...
	if (!NewBP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create TargetActor Blueprint at: %s"), *SavePath));
	}
...
FMegalithActionResult FMegalithGASTargetActions::HandleConfigureTargetActor(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FMegalithActionResult Err;
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load TargetActor BP: %s — %s"), *AssetPath, *Error));
	}

	UObject* CDO = BP->GeneratedClass->GetDefaultObject();
	if (!CDO || !CDO->GetClass()->IsChildOf(AGameplayAbilityTargetActor::StaticClass()))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayAbilityTargetActor"), *AssetPath));
	}
	...
	if (SetProps.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No properties were changed. Check parameter names and TargetActor type."));
	}
...
FMegalithActionResult FMegalithGASTargetActions::HandleAddTargetingToAbility(const TSharedPtr<FJsonObject>& Params)
{
	FString AbilityPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("ability_path"), AbilityPath, Err)) return Err;

	FString Error;
	UObject* Obj = MegalithGAS::LoadAssetFromPath(AbilityPath, Error);
	UBlueprint* BP = Cast<UBlueprint>(Obj);
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load ability: %s — %s"), *AbilityPath, *Error));
	}
	if (!MegalithGAS::IsAbilityBlueprint(BP))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayAbility Blueprint"), *AbilityPath));
	}
	...
	{
		UObject* ClassObj = FMegalithAssetUtils::LoadAssetByPath(UClass::StaticClass(), TargetActorClassPath);
		TargetActorClass = Cast<UClass>(ClassObj);
		if (!TargetActorClass)
		{
			// Try loading as Blueprint
			UBlueprint* TABP = Cast<UBlueprint>(FMegalithAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), TargetActorClassPath));
	...
	if (!EventGraph)
	{
		return FMegalithActionResult::Error(TEXT("No EventGraph found in ability Blueprint"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithGAS::MakeAssetResult(AbilityPath,
...
FMegalithActionResult FMegalithGASTargetActions::HandleScaffoldFPSTargeting(const TSharedPtr<FJsonObject>& Params)
{
	FString AbilityPath, Mode;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("ability_path"), AbilityPath, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("mode"), Mode, Err)) return Err;
	...
	FString Error;
	UObject* AbilityObj = MegalithGAS::LoadAssetFromPath(AbilityPath, Error);
	UBlueprint* AbilityBP = Cast<UBlueprint>(AbilityObj);
	if (!AbilityBP || !MegalithGAS::IsAbilityBlueprint(AbilityBP))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("'%s' is not a valid ability Blueprint — %s"), *AbilityPath, *Error));
	}
	...
	FMegalithActionResult CreateResult = HandleCreateTargetActor(CreateTAParams);
	...
	else
	{
		...
		UObject* ExistingTA = MegalithGAS::LoadAssetFromPath(TASavePath, Error);
		if (ExistingTA)
		{
			TAClassPath = TASavePath;
	...
	FMegalithActionResult WireResult = HandleAddTargetingToAbility(WireParams);
...
FMegalithActionResult FMegalithGASTargetActions::HandleValidateTargeting(const TSharedPtr<FJsonObject>& Params)
{
	FString AbilityPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("ability_path"), AbilityPath, Err)) return Err;
	...
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load: %s — %s"), *AbilityPath, *Error));
	}
	if (!MegalithGAS::IsAbilityBlueprint(BP))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("'%s' is not a GameplayAbility Blueprint"), *AbilityPath));
