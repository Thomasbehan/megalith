...
void FMegalithGASInputActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("setup_ability_input_binding"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetupAbilityInputBinding),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("bind_ability_to_input"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBindAbilityToInput),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("batch_bind_abilities"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBatchBindAbilities),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path of the actor"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_ability_input_bindings"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAbilityInputBindings),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path of the actor"))
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_input_binding_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldInputBindingComponent),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithGASInputActions::HandleSetupAbilityInputBinding(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult ErrorResult;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, ErrorResult))
	{
		return ErrorResult;
	}

	FString BindingMode;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("binding_mode"), BindingMode, ErrorResult))
	{
		return ErrorResult;
	}
	...
	FString AssetPath;
	FString LoadError;
	UBlueprint* BP = MegalithGAS::LoadBlueprintFromParams(Params, AssetPath, LoadError);
	if (!BP)
	{
		// Try with actor_path since the helper checks asset_path
		FString Error2;
		UObject* Obj = MegalithGAS::LoadAssetFromPath(ActorPath, Error2);
...
FMegalithActionResult FMegalithGASInputActions::HandleBindAbilityToInput(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult ErrorResult;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, ErrorResult))
	...
	if (!MegalithGAS::RequireStringParam(Params, TEXT("ability_class"), AbilityClass, ErrorResult))
	{
		return ErrorResult;
	}

	FString InputAction;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("input_action"), InputAction, ErrorResult))
	{
		return ErrorResult;
	}
	...
	UObject* AbilityObj = MegalithGAS::LoadAssetFromPath(AbilityClass, AbilityError);
	...
	if (BindingMode == TEXT("enhanced_input"))
	{
		...
		FString IAError;
		UObject* IAObj = MegalithGAS::LoadAssetFromPath(InputAction, IAError);
...
FMegalithActionResult FMegalithGASInputActions::HandleBatchBindAbilities(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: bindings (array)"));
	}
	...
	for (int32 i = 0; i < BindingsArray->Num(); ++i)
	{
		...
		FMegalithActionResult SingleResult = HandleBindAbilityToInput(SingleParams);
...
FMegalithActionResult FMegalithGASInputActions::HandleGetAbilityInputBindings(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
...
FMegalithActionResult FMegalithGASInputActions::HandleScaffoldInputBindingComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: input_config (object)"));
	}
	...
	FString HeaderContent = FString::Printf(TEXT(
		"// Auto-generated by Megalith GAS — AbilityInputBindingComponent\n"
