// MegalithGASAttributeBindingClassExtension.cpp
...
FGameplayAttribute UMegalithGASAttributeBindingClassExtension::ResolveAttribute(const FString& AttrSetClassPath, FName PropertyName)
{
...
UAbilitySystemComponent* UMegalithGASAttributeBindingClassExtension::ResolveASC(UUserWidget* UW, const FMegalithGASAttributeBindingSpec& Spec) const
{
    if (!UW) return nullptr;
    switch (Spec.Owner)
    {
        case EMegalithAttrBindOwner::OwningPlayerPawn:
        {
        ...
        case EMegalithAttrBindOwner::OwningPlayerState:
        {
            if (APlayerController* PC = UW->GetOwningPlayer())
            {
        ...
        case EMegalithAttrBindOwner::OwningPlayerController:
        {
            return GetASCFromActorRuntime(UW->GetOwningPlayer());
        }
        case EMegalithAttrBindOwner::SelfActor:
        {
            return GetASCFromActorRuntime(Cast<AActor>(UW->GetOuter()));
        }
        case EMegalithAttrBindOwner::NamedSocket:
        {
...
void UMegalithGASAttributeBindingClassExtension::Initialize(UUserWidget* UserWidget)
{
...
void UMegalithGASAttributeBindingClassExtension::Construct(UUserWidget* UserWidget)
{
...
void UMegalithGASAttributeBindingClassExtension::Destruct(UUserWidget* UserWidget)
{
    if (UserWidget)
    {
...
void UMegalithGASAttributeBindingClassExtension::SubscribeRow(UUserWidget* UW, int32 RowIndex, const FMegalithGASAttributeBindingSpec& Spec, FInstanceState& State)
{
...
void UMegalithGASAttributeBindingClassExtension::OnAttributeChanged(const FOnAttributeChangeData& Data, TWeakObjectPtr<UUserWidget> WeakUW, int32 RowIndex)
{
    ...
    FActiveSub& Sub = State->SubsByRow[RowIndex];
    const FMegalithGASAttributeBindingSpec& Spec = Bindings[RowIndex];
    ...
    if (Spec.UpdatePolicy != EMegalithAttrBindUpdate::OnChangeSmoothed)
    {
        Sub.CurrentDisplayedValue = Sub.TargetValue;
    }
...
void UMegalithGASAttributeBindingClassExtension::ApplyValue(UUserWidget* UW, const FMegalithGASAttributeBindingSpec& Spec, FActiveSub& Sub) const
{
    ...
    // Phase J F9: per-fire trace at Verbose (shipping-silent by default; enable LogMegalithGAS Verbose to surface).
    ...
    EMegalithAttrBindFormat Format = Spec.Format;
    ...
    {
        FString Out;
        if (Format == EMegalithAttrBindFormat::FormatString && !Spec.FormatPayload.IsEmpty())
        {
        ...
        else if (Format == EMegalithAttrBindFormat::IntText
              || (Format == EMegalithAttrBindFormat::Auto && FMath::IsNearlyEqual(V, FMath::TruncToFloat(V))))
        {
            Out = FString::Printf(TEXT("%d"), FMath::RoundToInt(V));
        }
        else if (Format == EMegalithAttrBindFormat::FloatText)
        {
            int32 Decimals = 0;
