...
void FMegalithGASScaffoldActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("bootstrap_gas_foundation"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBootstrapGASFoundation),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_gas_setup"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateGASSetup),
		FParamSchemaBuilder()
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_gas_project"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldGASProject),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_damage_pipeline"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldDamagePipeline),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_status_effect"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldStatusEffect),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("scaffold_weapon_ability"),
		...
		FMegalithActionHandler::CreateStatic(&HandleScaffoldWeaponAbility),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("grant_ability_to_pawn"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGrantAbilityToPawn),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithGASScaffoldActions::HandleBootstrapGASFoundation(const TSharedPtr<FJsonObject>& Params)
{
	FString ProjectName;
	FMegalithActionResult ErrorResult;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("project_name"), ProjectName, ErrorResult))
	{
		return ErrorResult;
	}
	...
	else
	{
		...
		FString HeaderContent = FString::Printf(TEXT(
			"// Auto-generated by Megalith GAS Bootstrap\n"
			"#pragma once\n"
			"\n"
...
FMegalithActionResult FMegalithGASScaffoldActions::HandleValidateGASSetup(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithGASScaffoldActions::HandleScaffoldGASProject(const TSharedPtr<FJsonObject>& Params)
{
	FString Preset;
	...
	FString ProjectName = FApp::GetProjectName();
	{
		...
		FMegalithActionResult BootstrapResult = HandleBootstrapGASFoundation(BootstrapParams);
	...
	TArray<FString> ActorPaths = MegalithGAS::ParseStringArray(Params, TEXT("actor_paths"));
	...
	if (ActorPaths.Num() > 0)
	{
		for (const FString& ActorPath : ActorPaths)
		{
			...
			FString LoadError;
			UObject* Obj = MegalithGAS::LoadAssetFromPath(ActorPath, LoadError);
	...
	return FMegalithActionResult::Success(ProjectResult);
}
...
FMegalithActionResult FMegalithGASScaffoldActions::HandleScaffoldDamagePipeline(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("save_path"), SavePath, Err)) return Err;

	TArray<FString> DamageTypes = MegalithGAS::ParseStringArray(Params, TEXT("damage_types"));
	if (DamageTypes.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: damage_types"));
	}
	...
	for (const FString& DmgType : DamageTypes)
	{
		...
		FString ExistError;
		if (!MegalithGAS::EnsureAssetPathFree(GEPath, GEName, ExistError))
		{
	...
	return FMegalithActionResult::Success(PipelineResult);
}
...
FMegalithActionResult FMegalithGASScaffoldActions::HandleScaffoldStatusEffect(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath, Name;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("save_path"), SavePath, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("name"), Name, Err)) return Err;
	...
	TArray<FString> RemovesTags = MegalithGAS::ParseStringArray(Config, TEXT("removes_tags"));
	...
	if (!GE_CDO)
	{
		return FMegalithActionResult::Error(TEXT("Failed to get CDO after compile"));
	}
	...
	return FMegalithActionResult::Success(StatusResult);
}
...
FMegalithActionResult FMegalithGASScaffoldActions::HandleScaffoldWeaponAbility(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath, WeaponType;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("save_path"), SavePath, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("weapon_type"), WeaponType, Err)) return Err;
	...
	UGameplayAbility* AbilityCDO = MegalithGAS::GetBlueprintCDO<UGameplayAbility>(NewBP);
	if (!AbilityCDO)
	{
		return FMegalithActionResult::Error(TEXT("Failed to get Ability CDO after compile"));
	}
	...
	// Configure AbilityTags
	{
		...
		{
			FProperty* Prop = MegalithGAS::FindAbilityAssetTagsProperty(AbilityCDO->GetClass());
			if (Prop)
			{
	...
	return FMegalithActionResult::Success(WeaponResult);
}
...
namespace
{
	/** Mirror of the resolver in MegalithGASASCActions.cpp. Asset-path-first, falls back to native class lookup + asset-registry scan for bare BP names. */
...
FMegalithActionResult FMegalithGASScaffoldActions::HandleGrantAbilityToPawn(const TSharedPtr<FJsonObject>& Params)
{
	FString PawnPath, AbilityClassId;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("pawn_bp_path"), PawnPath, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("ability_class_path"), AbilityClassId, Err)) return Err;
	...
	// MegalithGAS::LoadBlueprintFromParams because it expects the legacy
	// "asset_path" key — this F8 action uses "pawn_bp_path" to match the spec.
	FString LoadError;
	UObject* Obj = MegalithGAS::LoadAssetFromPath(PawnPath, LoadError);
	UBlueprint* BP = Cast<UBlueprint>(Obj);
	if (!BP)
	{
