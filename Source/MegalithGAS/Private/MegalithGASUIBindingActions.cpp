// MegalithGASUIBindingActions.cpp
...
namespace
{
    ...
    {
        // FMegalithAssetUtils::ResolveAssetPath normalizes leading-`/`-missing paths
        ...
        UObject* Raw = FMegalithAssetUtils::LoadAssetByPath(AssetPath);
        if (!Raw)
        {
     ...
     */
    bool ParseOwner(const FString& S, EMegalithAttrBindOwner& OutOwner, FName& OutSocketTag, FString& OutError)
    {
        if (S.IsEmpty() || S.Equals(TEXT("owning_player_pawn"), ESearchCase::IgnoreCase))
        {
            OutOwner = EMegalithAttrBindOwner::OwningPlayerPawn;
            return true;
        }
        if (S.Equals(TEXT("owning_player_state"), ESearchCase::IgnoreCase))
        {
            OutOwner = EMegalithAttrBindOwner::OwningPlayerState;
            return true;
        }
        if (S.Equals(TEXT("owning_player_controller"), ESearchCase::IgnoreCase))
        {
            OutOwner = EMegalithAttrBindOwner::OwningPlayerController;
            return true;
        }
        if (S.Equals(TEXT("self_actor"), ESearchCase::IgnoreCase))
        {
            OutOwner = EMegalithAttrBindOwner::SelfActor;
            return true;
        }
        if (S.StartsWith(TEXT("named_socket"), ESearchCase::IgnoreCase))
        {
            OutOwner = EMegalithAttrBindOwner::NamedSocket;
            FString L, R;
    ...
    FString OwnerToString(EMegalithAttrBindOwner Owner)
    {
        switch (Owner)
        {
            case EMegalithAttrBindOwner::OwningPlayerPawn:       return TEXT("owning_player_pawn");
            case EMegalithAttrBindOwner::OwningPlayerState:      return TEXT("owning_player_state");
            case EMegalithAttrBindOwner::OwningPlayerController: return TEXT("owning_player_controller");
            case EMegalithAttrBindOwner::SelfActor:              return TEXT("self_actor");
            case EMegalithAttrBindOwner::NamedSocket:            return TEXT("named_socket");
        }
        return TEXT("owning_player_pawn");
    }

    EMegalithAttrBindFormat ParseFormat(const FString& S, FString& OutPayload)
    {
        // Recognized: auto | percent_0_1 | int_text | float_text:N | format_string:<tpl> | gradient:<lo->hi>
        if (S.IsEmpty() || S.Equals(TEXT("auto"), ESearchCase::IgnoreCase)) return EMegalithAttrBindFormat::Auto;
        if (S.Equals(TEXT("percent_0_1"), ESearchCase::IgnoreCase)) return EMegalithAttrBindFormat::PercentZeroOne;
        if (S.Equals(TEXT("int_text"), ESearchCase::IgnoreCase)) return EMegalithAttrBindFormat::IntText;
        if (S.StartsWith(TEXT("float_text"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EMegalithAttrBindFormat::FloatText;
        }
        if (S.StartsWith(TEXT("format_string"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EMegalithAttrBindFormat::FormatString;
        }
        if (S.StartsWith(TEXT("gradient"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EMegalithAttrBindFormat::Gradient;
        }
        if (S.StartsWith(TEXT("threshold"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R)) OutPayload = R;
            return EMegalithAttrBindFormat::Threshold;
        }
        return EMegalithAttrBindFormat::Auto;
    }
    ...
    FString FormatToString(EMegalithAttrBindFormat F)
    {
        switch (F)
        {
            case EMegalithAttrBindFormat::Auto:           return TEXT("auto");
            case EMegalithAttrBindFormat::PercentZeroOne: return TEXT("percent_0_1");
            case EMegalithAttrBindFormat::IntText:        return TEXT("int_text");
            case EMegalithAttrBindFormat::FloatText:      return TEXT("float_text");
            case EMegalithAttrBindFormat::FormatString:   return TEXT("format_string");
            case EMegalithAttrBindFormat::Gradient:       return TEXT("gradient");
            case EMegalithAttrBindFormat::Threshold:      return TEXT("threshold");
        }
        return TEXT("auto");
    }

    EMegalithAttrBindUpdate ParseUpdate(const FString& S, float& OutSmoothing)
    {
        if (S.IsEmpty() || S.Equals(TEXT("on_change"), ESearchCase::IgnoreCase)) return EMegalithAttrBindUpdate::OnChange;
        if (S.Equals(TEXT("tick"), ESearchCase::IgnoreCase)) return EMegalithAttrBindUpdate::Tick;
        if (S.StartsWith(TEXT("on_change_smoothed"), ESearchCase::IgnoreCase))
        {
            FString L, R;
            if (S.Split(TEXT(":"), &L, &R) && !R.IsEmpty()) OutSmoothing = FCString::Atof(*R);
            return EMegalithAttrBindUpdate::OnChangeSmoothed;
        }
        return EMegalithAttrBindUpdate::OnChange;
    }

    FString UpdateToString(EMegalithAttrBindUpdate U)
    {
        switch (U)
        {
            case EMegalithAttrBindUpdate::OnChange:         return TEXT("on_change");
            case EMegalithAttrBindUpdate::Tick:             return TEXT("tick");
            case EMegalithAttrBindUpdate::OnChangeSmoothed: return TEXT("on_change_smoothed");
        }
        return TEXT("on_change");
    }

    /** Validate (widget_class, target_property) is a supported pair. Adds warnings for borderline cases. */
    bool ValidateWidgetProperty(UWidget* Widget, FName Prop, EMegalithAttrBindFormat& Format,
                                bool bHasMaxAttribute, TArray<FString>& Warnings, FString& OutError)
    {
        ...
        if (C->IsChildOf(UProgressBar::StaticClass()) && Prop == TEXT("Percent"))
        {
            ...
            if (Format == EMegalithAttrBindFormat::Auto) Format = EMegalithAttrBindFormat::PercentZeroOne;
            return true;
        }
        ...
        {
            if (Format == EMegalithAttrBindFormat::Auto)
            {
                Format = bHasMaxAttribute ? EMegalithAttrBindFormat::FormatString : EMegalithAttrBindFormat::IntText;
            }
        ...
        {
            if (Format != EMegalithAttrBindFormat::Gradient)
            {
        ...
        {
            if (Format == EMegalithAttrBindFormat::Auto) Format = EMegalithAttrBindFormat::Threshold;
            return true;
        }
        ...
        {
            if (Format == EMegalithAttrBindFormat::Auto) Format = EMegalithAttrBindFormat::FloatText;
    ...
    UMegalithGASUIBindingBlueprintExtension* GetOrCreateExtension(UWidgetBlueprint* WBP)
    {
        if (!WBP) return nullptr;
        UMegalithGASUIBindingBlueprintExtension* Existing =
            UWidgetBlueprintExtension::GetExtension<UMegalithGASUIBindingBlueprintExtension>(WBP);
        if (Existing) return Existing;
        return UWidgetBlueprintExtension::RequestExtension<UMegalithGASUIBindingBlueprintExtension>(WBP);
    }
    ...
    TSharedPtr<FJsonObject> SerializeBindingRow(const FMegalithGASAttributeBindingSpec& Spec, int32 Index, UWidgetBlueprint* WBP)
    {
...
void FMegalithGASUIBindingActions::RegisterActions(FMegalithToolRegistry& Registry)
{
    ...
    Registry.RegisterAction(TEXT("gas"), TEXT("bind_widget_to_attribute"),    BindDesc,
        FMegalithActionHandler::CreateStatic(&HandleBindWidgetToAttribute), BuildBindSchema());
    Registry.RegisterAction(TEXT("gas"), TEXT("unbind_widget_attribute"),    UnbindDesc,
        FMegalithActionHandler::CreateStatic(&HandleUnbindWidgetAttribute), BuildUnbindSchema());
    Registry.RegisterAction(TEXT("gas"), TEXT("list_attribute_bindings"),    ListDesc,
        FMegalithActionHandler::CreateStatic(&HandleListAttributeBindings), BuildListSchema());
    Registry.RegisterAction(TEXT("gas"), TEXT("clear_widget_attribute_bindings"), ClearDesc,
        FMegalithActionHandler::CreateStatic(&HandleClearWidgetAttributeBindings), BuildClearSchema());
...
FMegalithActionResult FMegalithGASUIBindingActions::HandleBindWidgetToAttribute(const TSharedPtr<FJsonObject>& Params)
{
    FString WbpPath, WidgetNameStr, TargetPropStr, AttrStr;
    FMegalithActionResult Err;
    if (!MegalithGAS::RequireStringParam(Params, TEXT("wbp_path"),        WbpPath,       Err)) return Err;
    if (!MegalithGAS::RequireStringParam(Params, TEXT("widget_name"),     WidgetNameStr, Err)) return Err;
    if (!MegalithGAS::RequireStringParam(Params, TEXT("target_property"), TargetPropStr, Err)) return Err;
    ...
    if (!WBP) return FMegalithActionResult::Error(LoadErr);
    ...
    {
        return FMegalithActionResult::Error(ParseErr);
    }
    ...
    if (!MaxAttrStr.IsEmpty())
    {
        ...
        {
            return FMegalithActionResult::Error(FString::Printf(TEXT("max_attribute parse error: %s"), *MaxParseErr));
        }
        bHasMax = true;
    }
    ...
    EMegalithAttrBindFormat Format = ParseFormat(FormatStr, FormatPayload);

    // Phase J F3: pre-validate user-supplied format_string payloads for required slots.
    if (Format == EMegalithAttrBindFormat::FormatString)
    {
        FString FmtErr;
        if (!ValidateFormatStringPayload(FormatPayload, bHasMax, FmtErr))
        {
            return FMegalithActionResult::Error(FmtErr);
        }
    }
    ...
    {
        return FMegalithActionResult::Error(ValidErr);
    }
    ...
    if (Format == EMegalithAttrBindFormat::FormatString && FormatPayload.IsEmpty())
    {
    ...
    // Update policy.
    float Smoothing = 6.f;
    EMegalithAttrBindUpdate UpdatePolicy = ParseUpdate(UpdateStr, Smoothing);
    ...
    EMegalithAttrBindOwner Owner = EMegalithAttrBindOwner::OwningPlayerPawn;
    FName OwnerTag;
    FString OwnerErr;
    if (!ParseOwner(OwnerStr, Owner, OwnerTag, OwnerErr))
    {
        return FMegalithActionResult::Error(OwnerErr);
    }

    // Build the spec.
    FMegalithGASAttributeBindingSpec Spec;
    ...
    UMegalithGASUIBindingBlueprintExtension* Ext = GetOrCreateExtension(WBP);
    if (!Ext)
    {
        return FMegalithActionResult::Error(TEXT("Failed to obtain UMegalithGASUIBindingBlueprintExtension"));
    }
    ...
    Result->SetStringField(TEXT("extension_class"), TEXT("UMegalithGASAttributeBindingClassExtension"));
...
FMegalithActionResult FMegalithGASUIBindingActions::HandleUnbindWidgetAttribute(const TSharedPtr<FJsonObject>& Params)
{
    ...
    UMegalithGASUIBindingBlueprintExtension* Ext =
        UWidgetBlueprintExtension::GetExtension<UMegalithGASUIBindingBlueprintExtension>(WBP);
    if (!Ext)
    {
        return FMegalithActionResult::Error(TEXT("No GAS attribute bindings on this WBP"));
    }
...
FMegalithActionResult FMegalithGASUIBindingActions::HandleListAttributeBindings(const TSharedPtr<FJsonObject>& Params)
{
    FString WbpPath;
    FMegalithActionResult Err;
    if (!MegalithGAS::RequireStringParam(Params, TEXT("wbp_path"), WbpPath, Err)) return Err;
    ...
    Result->SetStringField(TEXT("note"),
        TEXT("These are GAS attribute bindings (Megalith). Distinct from ui::get_widget_bindings which reads UMG FDelegateRuntimeBinding."));

    // Phase J F9: read-only listing — Verbose so it doesn't spam under frequent UI inspection traffic.
    UE_LOG(LogMegalithGAS, Verbose,
...
FMegalithActionResult FMegalithGASUIBindingActions::HandleClearWidgetAttributeBindings(const TSharedPtr<FJsonObject>& Params)
{
    FString WbpPath;
    FMegalithActionResult Err;
    if (!MegalithGAS::RequireStringParam(Params, TEXT("wbp_path"), WbpPath, Err)) return Err;
