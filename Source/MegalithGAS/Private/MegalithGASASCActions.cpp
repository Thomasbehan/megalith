...
// LogMegalithGAS declared in MegalithGASInternal.h, defined in MegalithGASModule.cpp

// ---------------------------------------------------------------------------
// Registration
// ---------------------------------------------------------------------------

void FMegalithGASASCActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("gas"), TEXT("add_asc_to_actor"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddASCToActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("configure_asc"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureASC),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("setup_asc_init"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetupASCInit),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("setup_ability_system_interface"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetupAbilitySystemInterface),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("apply_asc_template"),
		...
		FMegalithActionHandler::CreateStatic(&HandleApplyASCTemplate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("set_default_abilities"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetDefaultAbilities),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("set_default_effects"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetDefaultEffects),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path containing the ASC"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("set_default_attribute_sets"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetDefaultAttributeSets),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path containing the ASC"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("set_asc_replication_mode"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetASCReplicationMode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path containing the ASC"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("validate_asc_setup"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateASCSetup),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("actor_path"), TEXT("Blueprint asset path for the actor"))
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("grant_ability"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGrantAbility),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("revoke_ability"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRevokeAbility),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
			.Required(TEXT("ability_class"), TEXT("string"), TEXT("GameplayAbility class name or asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_asc_snapshot"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetASCSnapshot),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
	...
	Registry.RegisterAction(TEXT("gas"), TEXT("get_all_ascs"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAllASCs),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithGASASCActions::HandleAddASCToActor(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath, Error;
	UBlueprint* BP = MegalithGAS::LoadBlueprintFromParams(Params, ActorPath, Error);
	if (!BP)
	{
	...
	if (!NewNode)
	{
		...
		return FMegalithActionResult::Error(TEXT("Failed to create ASC SCS node"));
	}
...
FMegalithActionResult FMegalithGASASCActions::HandleConfigureASC(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath, Error;
	UBlueprint* BP = MegalithGAS::LoadBlueprintFromParams(Params, ActorPath, Error);
	if (!BP)
	...
	// --- Default abilities ---
	TArray<FString> AbilityPaths = MegalithGAS::ParseStringArray(Params, TEXT("default_abilities"));
	...
	// --- Default effects ---
	TArray<FString> EffectPaths = MegalithGAS::ParseStringArray(Params, TEXT("default_effects"));
...
FMegalithActionResult FMegalithGASASCActions::HandleSetupASCInit(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath, Error;
	UBlueprint* BP = MegalithGAS::LoadBlueprintFromParams(Params, ActorPath, Error);
	if (!BP)
	...
	if (!MegalithGAS::RequireStringParam(Params, TEXT("location"), Location, ParamError))
	{
		return ParamError;
	}
...
FMegalithActionResult FMegalithGASASCActions::HandleSetupAbilitySystemInterface(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath, Error;
	UBlueprint* BP = MegalithGAS::LoadBlueprintFromParams(Params, ActorPath, Error);
	if (!BP)
	...
	FString ASCLocation;
	FMegalithActionResult ParamError;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("asc_location"), ASCLocation, ParamError))
	{
		return ParamError;
	}
...
FMegalithActionResult FMegalithGASASCActions::HandleApplyASCTemplate(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath, TemplateName;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
	...
	if (Params->TryGetObjectField(TEXT("overrides"), OverridesPtr) && OverridesPtr && (*OverridesPtr).IsValid())
	{
		...
		TArray<FString> OvAttrSets = MegalithGAS::ParseStringArray(Ov, TEXT("attribute_sets"));
		if (OvAttrSets.Num() > 0) Def.AttributeSets = OvAttrSets;

		TArray<FString> OvAbilities = MegalithGAS::ParseStringArray(Ov, TEXT("abilities"));
		if (OvAbilities.Num() > 0) Def.Abilities = OvAbilities;

		TArray<FString> OvEffects = MegalithGAS::ParseStringArray(Ov, TEXT("effects"));
...
FMegalithActionResult FMegalithGASASCActions::HandleSetDefaultAbilities(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;

	TArray<FString> AbilityPaths = MegalithGAS::ParseStringArray(Params, TEXT("abilities"));
	if (AbilityPaths.Num() == 0)
	{
...
FMegalithActionResult FMegalithGASASCActions::HandleSetDefaultEffects(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;

	TArray<FString> EffectPaths = MegalithGAS::ParseStringArray(Params, TEXT("effects"));
	if (EffectPaths.Num() == 0)
	{
...
FMegalithActionResult FMegalithGASASCActions::HandleSetDefaultAttributeSets(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
...
FMegalithActionResult FMegalithGASASCActions::HandleSetASCReplicationMode(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath, ModeStr;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("mode"), ModeStr, Err)) return Err;
	...
	if (!ASCNode)
	{
		return FMegalithActionResult::Error(TEXT("No AbilitySystemComponent found on this Blueprint. Call add_asc_to_actor first."));
	}
...
FMegalithActionResult FMegalithGASASCActions::HandleValidateASCSetup(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorPath;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor_path"), ActorPath, Err)) return Err;
...
FMegalithActionResult FMegalithGASASCActions::HandleGrantAbility(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorId, AbilityClassId;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor"), ActorId, Err)) return Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("ability_class"), AbilityClassId, Err)) return Err;
...
FMegalithActionResult FMegalithGASASCActions::HandleRevokeAbility(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorId, AbilityClassId;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor"), ActorId, Err)) return Err;
...
FMegalithActionResult FMegalithGASASCActions::HandleGetASCSnapshot(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorId;
	FMegalithActionResult Err;
	if (!MegalithGAS::RequireStringParam(Params, TEXT("actor"), ActorId, Err)) return Err;
	...
	if (bIncludeAbilities)
	{
		...
		for (const FGameplayAbilitySpec& Spec : ASC->GetActivatableAbilities())
		{
			...
			{
				AbilObj->SetField(TEXT("ability_tags"), MegalithGAS::TagContainerToJson(AbilityTags));
			}
	...
	if (bIncludeTags)
	{
		...
		Result->SetField(TEXT("owned_tags"), MegalithGAS::TagContainerToJson(OwnedTags));
...
FMegalithActionResult FMegalithGASASCActions::HandleGetAllASCs(const TSharedPtr<FJsonObject>& Params)
{
