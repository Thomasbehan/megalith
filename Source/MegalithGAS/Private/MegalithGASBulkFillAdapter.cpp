...
// MegalithGASBulkFillAdapter — Phase 2 adapter for the FMegalithBulkFillRegistry.
...
// `megalith_discover("gas")` action surface stays identical across build flavours.
//
// NOTE: AttributeInit DataTable writes do NOT need the
// MegalithBlueprintEditCradle (transient-subobject reparent + FOverridableManager
...
namespace MegalithGASBulkFillInternal
{
	...
	// Mirrors MegalithBlueprintBulkFillAdapter.cpp::MakeResolveFailureReport.
	...
	// MegalithGASAttributeActions.cpp:217-220 (private to that TU).
	...
	// MegalithGASAttributeActions.cpp:455-478 (private to that TU). Accepts both
	...
	static UClass* ResolveAttributeSetClass(const FString& AttrSet)
	{
		...
		{
			UObject* Asset = FMegalithAssetUtils::LoadAssetByPath(AttrSet);
	...
	static FProperty* ResolveAttributeOnSet(UClass* SetClass, const FString& AttrName)
	{
		...
		// FindPropertyForwarding pattern at MegalithReflectionWalker.h:65.
	...
	//              MegalithGASAttributeActions.cpp:2270-2298).
	//
		...
		FDryRunReport& OutReport)
	{
		...
		// semantics. Mirrors MegalithGASAttributeActions.cpp:2270-2298.
...
// ---------------------------------------------------------------------------
FDryRunReport FMegalithGASBulkFillAdapter::GasBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithGASBulkFillInternal;
	...
	UObject* Asset = FMegalithAssetUtils::LoadAssetByPath(Spec.TargetAsset);
	...
	if (Spec.bDryRun)
	{
		...
		{
			const FString RowName = MegalithKeyToString(RowKV.Key);
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"MegalithGASBulkFillAdapter", "GasBulkFill",
		"Megalith GAS Bulk Fill"));
	DataTable->Modify();
...
#else // WITH_GBA
	// H5 stub-adapter branch — keeps `megalith_discover("gas")` action surface
...
// ---------------------------------------------------------------------------
FSchemaDescriptor FMegalithGASBulkFillAdapter::GasDescribe(const FString& TargetAsset)
{
	using namespace MegalithGASBulkFillInternal;
	...
	{
		UObject* Asset = FMegalithAssetUtils::LoadAssetByPath(TargetAsset);
...
// ---------------------------------------------------------------------------
// Registration entry-points (called from FMegalithGASModule::StartupModule
// / ShutdownModule).
//
// **H5 invariant**: Register() runs unconditionally — the body switches on
// WITH_GBA, not the registration. This guarantees `megalith_discover("gas")`
// returns the same row in dev and release builds.
// ---------------------------------------------------------------------------
void FMegalithGASBulkFillAdapter::Register()
{
	...
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("gas"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithGASBulkFillAdapter::GasBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithGASBulkFillAdapter::GasDescribe));
}

void FMegalithGASBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("gas"));
}
