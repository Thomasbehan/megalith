// MegalithGASAttributeBindingClassExtension.h
...
#include "MegalithGASAttributeBindingClassExtension.generated.h"
...
UCLASS(MinimalAPI)
class UMegalithGASAttributeBindingClassExtension : public UWidgetBlueprintGeneratedClassExtension
{
    GENERATED_BODY()

public:
    /** Persisted bindings, copied from the editor-side UMegalithGASUIBindingBlueprintExtension at compile time. */
    UPROPERTY()
...
private:
    ...
    UAbilitySystemComponent* ResolveASC(UUserWidget* UW, const FMegalithGASAttributeBindingSpec& Spec) const;
    ...
    void SubscribeRow(UUserWidget* UW, int32 RowIndex, const FMegalithGASAttributeBindingSpec& Spec, FInstanceState& State);

    /** Apply pipeline — coerce float (or float pair) -> widget property. */
    void ApplyValue(UUserWidget* UW, const FMegalithGASAttributeBindingSpec& Spec, FActiveSub& Sub) const;
