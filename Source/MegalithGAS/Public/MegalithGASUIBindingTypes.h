// MegalithGASUIBindingTypes.h
// USTRUCT + UENUM declarations for the Megalith UI<->GAS attribute binding feature (Phase H1).
//
// These types are persisted on a Widget Blueprint via the editor-side
// UMegalithGASUIBindingBlueprintExtension, and copied at compile time onto
// the runtime UMegalithGASAttributeBindingClassExtension which lives on the
...
#include "MegalithGASUIBindingTypes.generated.h"

/** How the binding resolves the AbilitySystemComponent owner at runtime. */
UENUM(BlueprintType)
enum class EMegalithAttrBindOwner : uint8
{
...
UENUM(BlueprintType)
enum class EMegalithAttrBindFormat : uint8
{
...
UENUM(BlueprintType)
enum class EMegalithAttrBindUpdate : uint8
{
...
struct MEGALITHGAS_API FMegalithGASAttributeBindingSpec
{
    GENERATED_BODY()
    ...
    UPROPERTY(EditAnywhere, Category = "GAS Binding")
    EMegalithAttrBindFormat Format = EMegalithAttrBindFormat::Auto;
    ...
    /** Update strategy. */
    UPROPERTY(EditAnywhere, Category = "GAS Binding")
    EMegalithAttrBindUpdate UpdatePolicy = EMegalithAttrBindUpdate::OnChange;
    ...
    bool operator==(const FMegalithGASAttributeBindingSpec& Other) const
    {
