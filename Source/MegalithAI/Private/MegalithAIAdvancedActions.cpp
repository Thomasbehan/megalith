...
namespace
{
	UMassEntityConfigAsset* LoadMassConfigFromParams(const TSharedPtr<FJsonObject>& Params, FString& OutAssetPath, FString& OutError)
	{
		...
		UMassEntityConfigAsset* Config = FMegalithAssetUtils::LoadAssetByPath<UMassEntityConfigAsset>(OutAssetPath);
		if (!Config)
		{
...
void FMegalithAIAdvancedActions::RegisterActions(FMegalithToolRegistry& Registry)
{
#if WITH_MASSENTITY
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("list_mass_entity_configs"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListMassEntityConfigs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_mass_entity_config"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetMassEntityConfig),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("create_mass_entity_config"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateMassEntityConfig),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("add_mass_trait"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddMassTrait),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("MassEntityConfigAsset asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("remove_mass_trait"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveMassTrait),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("MassEntityConfigAsset asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("list_mass_traits"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListMassTraits),
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("list_mass_processors"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListMassProcessors),
		FParamSchemaBuilder().Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("validate_mass_entity_config"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateMassEntityConfig),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("MassEntityConfigAsset asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_mass_entity_stats"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetMassEntityStats),
		FParamSchemaBuilder().Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("list_zone_graphs"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListZoneGraphs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("query_zone_lanes"),
		...
		FMegalithActionHandler::CreateStatic(&HandleQueryZoneLanes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_zone_lane_info"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetZoneLaneInfo),
		FParamSchemaBuilder()
...
// 219. list_mass_entity_configs
FMegalithActionResult FMegalithAIAdvancedActions::HandleListMassEntityConfigs(const TSharedPtr<FJsonObject>& Params)
{
...
// 220. get_mass_entity_config
FMegalithActionResult FMegalithAIAdvancedActions::HandleGetMassEntityConfig(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UMassEntityConfigAsset* Config = LoadMassConfigFromParams(Params, AssetPath, Error);
	if (!Config) return FMegalithActionResult::Error(Error);
...
// 221. create_mass_entity_config
FMegalithActionResult FMegalithAIAdvancedActions::HandleCreateMassEntityConfig(const TSharedPtr<FJsonObject>& Params)
{
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Create MassEntityConfig")));
	...
	if (!NewConfig)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create UMassEntityConfigAsset"));
	}
	...
	{
		ParentConfigPath = FMegalithAssetUtils::ResolveAssetPath(ParentConfigPath);
		UMassEntityConfigAsset* ParentConfig = FMegalithAssetUtils::LoadAssetByPath<UMassEntityConfigAsset>(ParentConfigPath);
		if (ParentConfig)
		{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Parent config not found at '%s'"), *ParentConfigPath));
		}
	}
...
// 222. add_mass_trait
FMegalithActionResult FMegalithAIAdvancedActions::HandleAddMassTrait(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UMassEntityConfigAsset* Config = LoadMassConfigFromParams(Params, AssetPath, Error);
	if (!Config) return FMegalithActionResult::Error(Error);
	...
	if (!MegalithAI::RequireStringParam(Params, TEXT("trait_class"), TraitClassName, ErrResult))
	{
		return ErrResult;
	}
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Add Mass Trait")));

	UMassEntityTraitBase* NewTrait = NewObject<UMassEntityTraitBase>(Config, TraitClass);
	if (!NewTrait)
	{
		return FMegalithActionResult::Error(TEXT("Failed to instantiate trait"));
	}
...
// 223. remove_mass_trait
FMegalithActionResult FMegalithAIAdvancedActions::HandleRemoveMassTrait(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UMassEntityConfigAsset* Config = LoadMassConfigFromParams(Params, AssetPath, Error);
	if (!Config) return FMegalithActionResult::Error(Error);
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Remove Mass Trait")));
	...
	if (RemovedIndex == INDEX_NONE)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Trait '%s' not found on config"), *TraitClassName));
	}
...
// 224. list_mass_traits
FMegalithActionResult FMegalithAIAdvancedActions::HandleListMassTraits(const TSharedPtr<FJsonObject>& Params)
{
	TArray<TSharedPtr<FJsonValue>> TraitArr;
...
// 225. list_mass_processors
FMegalithActionResult FMegalithAIAdvancedActions::HandleListMassProcessors(const TSharedPtr<FJsonObject>& Params)
{
...
// 226. validate_mass_entity_config
FMegalithActionResult FMegalithAIAdvancedActions::HandleValidateMassEntityConfig(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UMassEntityConfigAsset* Config = LoadMassConfigFromParams(Params, AssetPath, Error);
	if (!Config) return FMegalithActionResult::Error(Error);
...
// 230. get_mass_entity_stats
FMegalithActionResult FMegalithAIAdvancedActions::HandleGetMassEntityStats(const TSharedPtr<FJsonObject>& Params)
{
	UWorld* PIEWorld = MegalithAI::GetPIEWorld();
	if (!PIEWorld)
	{
		return FMegalithActionResult::Error(TEXT("No PIE world found — stats require Play In Editor to be running"));
	}

	UMassEntitySubsystem* MassSubsystem = PIEWorld->GetSubsystem<UMassEntitySubsystem>();
	if (!MassSubsystem)
	{
		return FMegalithActionResult::Error(TEXT("UMassEntitySubsystem not found — MassEntity may not be active in this world"));
	}
...
// 227. list_zone_graphs
FMegalithActionResult FMegalithAIAdvancedActions::HandleListZoneGraphs(const TSharedPtr<FJsonObject>& Params)
{
...
// 228. query_zone_lanes
FMegalithActionResult FMegalithAIAdvancedActions::HandleQueryZoneLanes(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'location' (object with x, y, z)"));
	}
	...
	if (!World)
	{
		return FMegalithActionResult::Error(TEXT("No world available for zone graph query"));
	}

	UZoneGraphSubsystem* ZGSubsystem = World->GetSubsystem<UZoneGraphSubsystem>();
	if (!ZGSubsystem)
	{
		return FMegalithActionResult::Error(TEXT("UZoneGraphSubsystem not found"));
	}
...
// 229. get_zone_lane_info
FMegalithActionResult FMegalithAIAdvancedActions::HandleGetZoneLaneInfo(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params->HasField(TEXT("lane_handle")))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'lane_handle'"));
	}
	...
	if (!Storage)
	{
		return FMegalithActionResult::Error(TEXT("No ZoneGraph storage available"));
	}

	if (LaneIndex < 0 || LaneIndex >= Storage->Lanes.Num())
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Lane index %d out of range (0-%d)"), LaneIndex, Storage->Lanes.Num() - 1));
	}
