		...
		else if (TypeStr == TEXT("Object"))
		{
			...
			if (ExtraParams.IsValid())
			{
				...
				if (!BaseClassName.IsEmpty())
				{
					...
					{
						UE_LOG(LogMegalithAI, Warning, TEXT("Could not find base_class '%s', defaulting to UObject"), *BaseClassName);
					}
				}
			}
			if (!ObjKey->BaseClass)
			...
			if (ExtraParams.IsValid())
			{
				...
				if (!EnumName.IsEmpty())
				{
					...
					else
					{
						...
						EnumKey->EnumName = EnumName;
						UE_LOG(LogMegalithAI, Warning, TEXT("Could not find enum '%s', storing name for later resolution"), *EnumName);
					}
				}
			}
			KeyType = EnumKey;
...
void FMegalithAIBlackboardActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 1. create_blackboard
	Registry.RegisterAction(TEXT("ai"), TEXT("create_blackboard"),
		TEXT("Create a new Blackboard Data asset"),
		FMegalithActionHandler::CreateStatic(&HandleCreateBlackboard),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_blackboard"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetBlackboard),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("list_blackboards"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListBlackboards),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("delete_blackboard"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDeleteBlackboard),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("duplicate_blackboard"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateBlackboard),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("add_bb_key"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddBBKey),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blackboard asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("remove_bb_key"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveBBKey),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blackboard asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("rename_bb_key"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRenameBBKey),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blackboard asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_bb_key_details"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetBBKeyDetails),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blackboard asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("batch_add_bb_keys"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBatchAddBBKeys),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blackboard asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("set_bb_parent"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetBBParent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blackboard asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("compare_blackboards"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCompareBlackboards),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleCreateBlackboard(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath;
	...
	if (!BB)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create UBlackboardData object"));
	}
	...
	{
		UObject* ParentObj = FMegalithAssetUtils::LoadAssetByPath(UBlackboardData::StaticClass(), ParentPath);
		UBlackboardData* ParentBB = Cast<UBlackboardData>(ParentObj);
		if (!ParentBB)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Parent blackboard not found: %s"), *ParentPath));
		}
		BB->Parent = ParentBB;
		BB->UpdateParentKeys();
		BB->UpdateKeyIDs();
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(PackagePath, TEXT("Blackboard created"));
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleGetBlackboard(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlackboardData* BB = MegalithAI::LoadBlackboardFromParams(Params, AssetPath, Error);
	if (!BB)
	{
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleListBlackboards(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleDeleteBlackboard(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FMegalithActionResult ErrResult;
	...
	UObject* Asset = FMegalithAssetUtils::LoadAssetByPath(UBlackboardData::StaticClass(), AssetPath);
	if (!Asset)
	{
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleDuplicateBlackboard(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Load source
	UBlackboardData* SourceBB = Cast<UBlackboardData>(FMegalithAssetUtils::LoadAssetByPath(UBlackboardData::StaticClass(), SourcePath));
	if (!SourceBB)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Source blackboard not found: %s"), *SourcePath));
	}
	...
	FString PathError;
	const bool bDestFree = MegalithAI::EnsureAssetPathFree(DestPath, DestAssetName, PathError);
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleAddBBKey(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlackboardData* BB = MegalithAI::LoadBlackboardFromParams(Params, AssetPath, Error);
	if (!BB)
	...
	FString KeyName;
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("key_name"), KeyName, ErrResult))
	{
		return ErrResult;
	}

	FString KeyTypeStr;
	if (!MegalithAI::RequireStringParam(Params, TEXT("key_type"), KeyTypeStr, ErrResult))
	{
		return ErrResult;
	}
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Key '%s' already exists in this blackboard"), *KeyName));
	}
	if (IsKeyInParentChain(BB, KeyFName))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Key '%s' already exists in parent blackboard chain"), *KeyName));
	}
	...
	if (!KeyType)
	{
		return FMegalithActionResult::Error(TypeError);
	}

	// Build entry
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Add BB Key")));
	BB->Modify();
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, FString::Printf(TEXT("Key '%s' (%s) added"), *KeyName, *KeyTypeStr));
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleRemoveBBKey(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlackboardData* BB = MegalithAI::LoadBlackboardFromParams(Params, AssetPath, Error);
	if (!BB)
	...
	if (Idx == INDEX_NONE)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Key '%s' not found in blackboard (not in own keys — inherited keys cannot be removed from child)"), *KeyName));
	}

	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Remove BB Key")));
	BB->Modify();
	BB->Keys.RemoveAt(Idx);
	BB->UpdateKeyIDs();
	BB->MarkPackageDirty();

	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, FString::Printf(TEXT("Key '%s' removed"), *KeyName));
	Result->SetNumberField(TEXT("key_count"), BB->Keys.Num());
	return FMegalithActionResult::Success(Result);
}
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleRenameBBKey(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlackboardData* BB = MegalithAI::LoadBlackboardFromParams(Params, AssetPath, Error);
	if (!BB)
	...
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("old_name"), OldName, ErrResult))
	{
		return ErrResult;
	}
	if (!MegalithAI::RequireStringParam(Params, TEXT("new_name"), NewName, ErrResult))
	{
		return ErrResult;
	}
	...
	if (Idx == INDEX_NONE)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Key '%s' not found"), *OldName));
	}

	// Check new name doesn't conflict
	if (FindKeyIndex(BB, NewFName) != INDEX_NONE)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Key '%s' already exists"), *NewName));
	}
	if (IsKeyInParentChain(BB, NewFName))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Key '%s' conflicts with inherited key"), *NewName));
	}

	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Rename BB Key")));
	BB->Modify();
	BB->Keys[Idx].EntryName = NewFName;
	BB->UpdateKeyIDs();
	BB->MarkPackageDirty();

	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, FString::Printf(TEXT("Key renamed: '%s' -> '%s'"), *OldName, *NewName));
	return FMegalithActionResult::Success(Result);
}
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleGetBBKeyDetails(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlackboardData* BB = MegalithAI::LoadBlackboardFromParams(Params, AssetPath, Error);
	if (!BB)
	...
	if (!FoundEntry)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Key '%s' not found in blackboard or parent chain"), *KeyName));
	}
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleBatchAddBBKeys(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlackboardData* BB = MegalithAI::LoadBlackboardFromParams(Params, AssetPath, Error);
	if (!BB)
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: keys (array)"));
	}

	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Batch Add BB Keys")));
	BB->Modify();
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleSetBBParent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlackboardData* BB = MegalithAI::LoadBlackboardFromParams(Params, AssetPath, Error);
	if (!BB)
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Set BB Parent")));
	BB->Modify();
	...
	if (ParentPath.IsEmpty() || ParentPath == TEXT("none") || ParentPath == TEXT("null"))
	{
		...
		TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, TEXT("Parent cleared"));
		return FMegalithActionResult::Success(Result);
	}
	...
	if (ParentBB == BB)
	{
		return FMegalithActionResult::Error(TEXT("Cannot set a blackboard as its own parent"));
	}
	{
		...
		while (Check)
		{
			if (Check == BB)
			{
				return FMegalithActionResult::Error(TEXT("Circular parent reference detected"));
			}
			Check = Check->Parent;
		}
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, FString::Printf(TEXT("Parent set to '%s'"), *ParentPath));
...
FMegalithActionResult FMegalithAIBlackboardActions::HandleCompareBlackboards(const TSharedPtr<FJsonObject>& Params)
{
	FString PathA, PathB;
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("path_a"), PathA, ErrResult))
	{
		return ErrResult;
	}
	if (!MegalithAI::RequireStringParam(Params, TEXT("path_b"), PathB, ErrResult))
	{
		return ErrResult;
	}

	UBlackboardData* BBA = Cast<UBlackboardData>(FMegalithAssetUtils::LoadAssetByPath(UBlackboardData::StaticClass(), PathA));
	UBlackboardData* BBB = Cast<UBlackboardData>(FMegalithAssetUtils::LoadAssetByPath(UBlackboardData::StaticClass(), PathB));

	if (!BBA) return FMegalithActionResult::Error(FString::Printf(TEXT("Blackboard A not found: %s"), *PathA));
	if (!BBB) return FMegalithActionResult::Error(FString::Printf(TEXT("Blackboard B not found: %s"), *PathB));
