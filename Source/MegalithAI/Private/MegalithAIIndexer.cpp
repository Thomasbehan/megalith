...
void FAIIndexer::EnsureTablesExist(FMegalithIndexDatabase& DB)
{
...
bool FAIIndexer::IndexAsset(const FAssetData& AssetData, UObject* LoadedAsset, FMegalithIndexDatabase& DB, int64 AssetId)
{
...
void FAIIndexer::IndexBehaviorTree(UBehaviorTree* BT, const FString& AssetPath, FMegalithIndexDatabase& DB)
{
	FSQLiteDatabase* RawDB = DB.GetRawDatabase();
	if (!RawDB || !BT) return;
...
void FAIIndexer::IndexBlackboard(UBlackboardData* BB, const FString& AssetPath, FMegalithIndexDatabase& DB)
{
	FSQLiteDatabase* RawDB = DB.GetRawDatabase();
	if (!RawDB || !BB) return;
...
void FAIIndexer::IndexAIController(UBlueprint* BP, const FString& AssetPath, FMegalithIndexDatabase& DB)
{
	FSQLiteDatabase* RawDB = DB.GetRawDatabase();
