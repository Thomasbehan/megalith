...
#define PIE_GUARD() \
	...
	{ \
		return FMegalithActionResult::Error(TEXT("This action requires Play-In-Editor to be running")); \
	}
...
namespace
{
	...
	AActor* ResolveActorParam(const TSharedPtr<FJsonObject>& Params, const FString& ParamName, FMegalithActionResult& OutError)
	{
		FString ActorId = Params->GetStringField(ParamName);
		if (ActorId.IsEmpty())
		{
			OutError = FMegalithActionResult::Error(FString::Printf(TEXT("Missing required param '%s'"), *ParamName));
			return nullptr;
		}

		AActor* Actor = MegalithAI::FindActorInPIE(ActorId);
		if (!Actor)
		{
			OutError = FMegalithActionResult::Error(FString::Printf(TEXT("Actor '%s' not found in PIE world"), *ActorId));
			return nullptr;
		}
		return Actor;
	}
	...
	AAIController* ResolveAIController(const TSharedPtr<FJsonObject>& Params, FMegalithActionResult& OutError)
	{
		...
		if (!AIC)
		{
			OutError = FMegalithActionResult::Error(FString::Printf(
	...
	UBlackboardComponent* ResolveBBComponent(const TSharedPtr<FJsonObject>& Params, FMegalithActionResult& OutError)
	{
		...
		if (!BB)
		{
			OutError = FMegalithActionResult::Error(TEXT("AI controller has no Blackboard component"));
			return nullptr;
		}
		return BB;
	}
	...
	UBehaviorTreeComponent* ResolveBTComponent(const TSharedPtr<FJsonObject>& Params, FMegalithActionResult& OutError)
	{
		AAIController* AIC = ResolveAIController(Params, OutError);
		if (!AIC) return nullptr;
		...
		if (!BTComp)
		{
			OutError = FMegalithActionResult::Error(TEXT("AI controller has no BehaviorTree component"));
			return nullptr;
		}
		return BTComp;
	}
...
void FMegalithAIRuntimeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_get_bb_value"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeGetBBValue),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_set_bb_value"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeSetBBValue),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_clear_bb_value"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeClearBBValue),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_get_bt_state"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeGetBTState),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_start_bt"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeStartBT),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_stop_bt"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeStopBT),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_get_bt_execution_path"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeGetBTExecutionPath),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_get_perceived_actors"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeGetPerceivedActors),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_check_perception"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeCheckPerception),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_report_noise"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeReportNoise),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_get_st_active_states"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeGetSTActiveStates),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_send_st_event"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeSendSTEvent),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label, name, or path in PIE"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_find_smart_objects"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeFindSmartObjects),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("runtime_run_eqs_query"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeRunEQSQuery),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeGetBBValue(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	if (KeyName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'key_name'"));
	}

	FBlackboard::FKey KeyID = BB->GetKeyID(FName(*KeyName));
	if (KeyID == FBlackboard::InvalidKey)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blackboard key '%s' not found"), *KeyName));
	}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeSetBBValue(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Cannot determine type for BB key '%s'"), *KeyName));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'value'"));
	}
	...
	else if (KeyTypeName.Contains(TEXT("Vector")))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Vector value must be 'X,Y,Z' format"));
		}
	}
	else if (KeyTypeName.Contains(TEXT("Rotator")))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Rotator value must be 'Pitch,Yaw,Roll' format"));
		}
	}
	else if (KeyTypeName.Contains(TEXT("Object")))
	{
		// Try to find the object in PIE world by name/label
		AActor* Obj = MegalithAI::FindActorInPIE(ValueStr);
		if (!Obj)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Cannot find actor '%s' in PIE for Object BB key"), *ValueStr));
		}
	...
	else if (KeyTypeName.Contains(TEXT("Class")))
	{
		...
		if (!C)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Class not found: '%s'"), *ValueStr));
		}
		BB->SetValueAsClass(KeyFName, C);
	}
	else
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Unsupported BB key type: %s"), *KeyTypeName));
	}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeClearBBValue(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeGetBTState(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeStartBT(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	if (!BTPath.IsEmpty())
	{
		BT = Cast<UBehaviorTree>(MegalithAI::ResolveAsset(UBehaviorTree::StaticClass(), BTPath));
		if (!BT)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("BehaviorTree not found: %s"), *BTPath));
		}
	}
	...
	if (!BT)
	{
		return FMegalithActionResult::Error(TEXT("No BehaviorTree specified and none currently assigned"));
	}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeStopBT(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeGetBTExecutionPath(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeGetPerceivedActors(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	if (!PerComp)
	{
		return FMegalithActionResult::Error(TEXT("AI controller has no Perception component"));
	}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeCheckPerception(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'observer_actor'"));
	}
	AActor* ObserverActor = MegalithAI::FindActorInPIE(ObserverId);
	if (!ObserverActor)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Observer actor '%s' not found in PIE"), *ObserverId));
	}

	AAIController* AIC = GetAIControllerFromActor(ObserverActor);
	if (!AIC)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Observer '%s' has no AI controller"), *ObserverId));
	}

	UAIPerceptionComponent* PerComp = AIC->GetPerceptionComponent();
	if (!PerComp)
	{
		return FMegalithActionResult::Error(TEXT("Observer's AI controller has no Perception component"));
	}
	...
	if (TargetId.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'target_actor'"));
	}
	AActor* TargetActor = MegalithAI::FindActorInPIE(TargetId);
	if (!TargetActor)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Target actor '%s' not found in PIE"), *TargetId));
	}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeReportNoise(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	if (!World)
	{
		return FMegalithActionResult::Error(TEXT("No PIE world found"));
	}
	...
	else
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("Missing required param 'location' — use {\"x\":N,\"y\":N,\"z\":N} or \"X,Y,Z\""));
		}
		TArray<FString> Parts;
		LocationStr.Replace(TEXT("("), TEXT("")).Replace(TEXT(")"), TEXT("")).ParseIntoArray(Parts, TEXT(","));
		if (Parts.Num() < 3)
		{
			return FMegalithActionResult::Error(TEXT("Location must be {x,y,z} object or 'X,Y,Z' string"));
		}
	...
	{
		Instigator = MegalithAI::FindActorInPIE(InstigatorStr);
		if (!Instigator)
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Instigator actor '%s' not found in PIE"), *InstigatorStr));
		}
	}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeGetSTActiveStates(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	if (!STComp)
	{
		return FMegalithActionResult::Error(TEXT("No StateTree component found on actor or its AI controller"));
	}
	...
	return FMegalithActionResult::Success(Result);
#else
	return FMegalithActionResult::Error(TEXT("StateTree support not compiled (WITH_STATETREE=0)"));
#endif
}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeSendSTEvent(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'event_tag'"));
	}

	FGameplayTag EventTag = FGameplayTag::RequestGameplayTag(FName(*EventTagStr), false);
	if (!EventTag.IsValid())
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Gameplay tag '%s' is not registered"), *EventTagStr));
	}
	...
	if (!STComp->IsRunning())
	{
		return FMegalithActionResult::Error(TEXT("StateTree component is not running"));
	}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeFindSmartObjects(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	if (!SOSubsystem)
	{
		return FMegalithActionResult::Error(TEXT("SmartObjectSubsystem not available in PIE world"));
	}
	...
	return FMegalithActionResult::Success(Result);
#else
	return FMegalithActionResult::Error(TEXT("SmartObjects support not compiled (WITH_SMARTOBJECTS=0)"));
#endif
}
...
FMegalithActionResult FMegalithAIRuntimeActions::HandleRuntimeRunEQSQuery(const TSharedPtr<FJsonObject>& Params)
{
	PIE_GUARD();
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'query_path'"));
	}

	UEnvQuery* QueryTemplate = Cast<UEnvQuery>(FMegalithAssetUtils::LoadAssetByPath(UEnvQuery::StaticClass(), QueryPath));
	if (!QueryTemplate)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("EQS query not found: %s"), *QueryPath));
	}
	...
	if (!EQSManager)
	{
		return FMegalithActionResult::Error(TEXT("EnvQueryManager not available in PIE world"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("EQS query returned no result"));
	}

	if (!QueryResult->IsSuccessful())
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("EQS query failed with status %d"), static_cast<int32>(QueryResult->GetRawStatus())));
	}
