// MegalithAIPerceptionScaffoldActions.cpp
...
//                                 (mirrors MegalithAIPerceptionActions:421-429,
//                                  MegalithAINavigationActions:1567-1593).
//   * UAIPerceptionComponent::ConfigureSense / GetSenseConfig<T>
//                              — AIModule/Public/Perception/AIPerceptionComponent.h
//                                 (mirrors MegalithAIPerceptionActions:249-263).
...
#include "MegalithAIPerceptionScaffoldActions.h"
...
void FMegalithAIPerceptionScaffoldActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("ai"), TEXT("add_perception_to_actor"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddPerceptionToActor),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithAIPerceptionScaffoldActions::HandleAddPerceptionToActor(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorBPPath;
	if (!Params->TryGetStringField(TEXT("actor_bp_path"), ActorBPPath) || ActorBPPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required parameter: actor_bp_path"));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing or empty required parameter: senses (array). Supported: [\"Sight\", \"Hearing\", \"Damage\"]."));
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("senses array contained no recognizable values."));
	}

	// Load BP via the centralized 4-tier resolver. Accept any UBlueprint with
	// an Actor-subclass GeneratedClass.
	FString LoadError;
	UObject* Obj = MegalithAI::LoadAssetFromPath(ActorBPPath, LoadError);
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Add Perception To Actor")));
	...
	if (!PerceptionNode)
	{
		...
		if (!PerceptionNode)
		{
			...
			return FMegalithActionResult::Error(TEXT("Failed to create UAIPerceptionComponent SCS node"));
		}
	...
	{
		return FMegalithActionResult::Error(TEXT("Perception SCS node has no component template"));
	}

	// 2. Add each requested sense config (using ConfigureSense — public API; same
	//    pattern as MegalithAIPerceptionActions::FindOrCreateSenseConfig).
