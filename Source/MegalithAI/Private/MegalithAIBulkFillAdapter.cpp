...
// MegalithAIBulkFillAdapter — Phase 5 Step 1 adapter for the FMegalithBulkFillRegistry.
...
//
// The adapter delegates all per-field reflection writes to FMegalithReflectionWalker,
...
#define LOCTEXT_NAMESPACE "MegalithAIBulkFillAdapter"

namespace MegalithAIBulkFillInternal
{
	...
	// FMegalithReflectionWalker::WriteTree against the target's full reflection
	...
	static FDryRunReport HandleEQSTests(const FBulkFillSpec& Spec)
	{
		...
		FScopedTransaction Transaction(LOCTEXT("AIBulkFill_EQS", "Megalith AI Bulk Fill — EQS Tests"));
		Asset->Modify();
	...
	static FDryRunReport HandleBlackboardKeys(const FBulkFillSpec& Spec)
	{
		...
		if (Spec.bDryRun)
		{
			...
			Report = FMegalithReflectionWalker::InspectTree(Root, BB->GetClass(), BB, Spec);
			Report.bWouldApply = false;
			return Report;
		}

		BB->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("AIBulkFill_BB", "Megalith AI Bulk Fill — Blackboard Keys"));
		BB->Modify();
		BB->PreEditChange(nullptr);

		TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
		Root->SetField(TEXT("Keys"), KeysVal);
		Report = FMegalithReflectionWalker::WriteTree(Root, BB->GetClass(), BB, BB, Spec);
	...
	static FDryRunReport HandleSmartObjectSlots(const FBulkFillSpec& Spec)
	{
		...
		Asset->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("AIBulkFill_SO", "Megalith AI Bulk Fill — SmartObject Slots"));
		Asset->Modify();
		Asset->PreEditChange(nullptr);
...
// ---------------------------------------------------------------------------
FDryRunReport FMegalithAIBulkFillAdapter::AIBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithAIBulkFillInternal;
...
// ---------------------------------------------------------------------------
FSchemaDescriptor FMegalithAIBulkFillAdapter::AIDescribe(const FString& TargetAsset)
{
	using namespace MegalithAIBulkFillInternal;
...
// ---------------------------------------------------------------------------
void FMegalithAIBulkFillAdapter::Register()
{
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("ai"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithAIBulkFillAdapter::AIBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithAIBulkFillAdapter::AIDescribe));
}

void FMegalithAIBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("ai"));
}
