...
namespace MegalithAI
{

// =============================================================================
//  ResolveAsset — backwards-compat shim. Forwards to canonical 4-tier resolver
//  in MegalithCore (FMegalithAssetUtils::LoadAssetByPath).
//
//  History: K1 introduced this function in MegalithAI as a 4-tier resolver
//  (registry-first, class-mismatch-terminal). The full implementation has been
//  centralized into FMegalithAssetUtils so MegalithGAS / MegalithBlueprint /
//  future modules share a single source of truth. This shim preserves the
//  MegalithAI::ResolveAsset(UClass*, const FString&) call surface byte-identically.
// =============================================================================

UObject* ResolveAsset(UClass* ExpectedClass, const FString& Path)
{
	return FMegalithAssetUtils::LoadAssetByPath(ExpectedClass, Path);
}
...
UPackage* GetOrCreatePackage(const FString& SavePath, FString& OutError)
{
	...
	if (const FString ValidationError = MegalithCore::ValidatePackagePath(PackageName, NormalizedName); !ValidationError.IsEmpty())
	{
		UE_LOG(LogMegalithAI, Warning, TEXT("GetOrCreatePackage rejected path: %s"), *ValidationError);
	...
	if (NormalizedName != PackageName)
	{
		...
		UE_LOG(LogMegalithAI, Warning, TEXT("GetOrCreatePackage rejected path: %s"), *OutError);
		return nullptr;
	}
...
bool RequireStringParam(const TSharedPtr<FJsonObject>& Params, const FString& ParamName, FString& OutValue, FMegalithActionResult& OutError)
{
