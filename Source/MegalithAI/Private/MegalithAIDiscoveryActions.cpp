...
void FMegalithAIDiscoveryActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// 203. get_ai_overview
	Registry.RegisterAction(TEXT("ai"), TEXT("get_ai_overview"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAIOverview),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("list_ai_node_types"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListAINodeTypes),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("search_ai_assets"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSearchAIAssets),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("validate_ai_data_flow"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateAIDataFlow),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("find_eqs_references"),
		...
		FMegalithActionHandler::CreateStatic(&HandleFindEQSReferences),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("find_so_references"),
		...
		FMegalithActionHandler::CreateStatic(&HandleFindSOReferences),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("lint_behavior_tree"),
		...
		FMegalithActionHandler::CreateStatic(&HandleLintBehaviorTree),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("lint_state_tree"),
		...
		FMegalithActionHandler::CreateStatic(&HandleLintStateTree),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("detect_ai_circular_references"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDetectAICircularReferences),
		FParamSchemaBuilder()
			.OptionalAssetPath(TEXT("path_filter"), TEXT("Only scan assets under this path prefix"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("export_ai_manifest"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExportAIManifest),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_ai_behavior_summary"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAIBehaviorSummary),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleGetAIOverview(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleListAINodeTypes(const TSharedPtr<FJsonObject>& Params)
{
	FString System;
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("system"), System, ErrResult))
	{
		return ErrResult;
	}
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleSearchAIAssets(const TSharedPtr<FJsonObject>& Params)
{
	FString Query;
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("query"), Query, ErrResult))
	{
		return ErrResult;
	}
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleValidateAIDataFlow(const TSharedPtr<FJsonObject>& Params)
{
	FString ControllerPath = Params->GetStringField(TEXT("controller_path"));
	if (ControllerPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: controller_path"));
	}

	UBlueprint* BP = Cast<UBlueprint>(MegalithAI::ResolveAsset(UBlueprint::StaticClass(), ControllerPath));
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint not found at '%s'"), *ControllerPath));
	}
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleFindEQSReferences(const TSharedPtr<FJsonObject>& Params)
{
	FString EQSPath = Params->GetStringField(TEXT("eqs_path"));
	if (EQSPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: eqs_path"));
	}
	...
	if (GEditor)
	{
		UMegalithIndexSubsystem* IndexSub = GEditor->GetEditorSubsystem<UMegalithIndexSubsystem>();
		if (IndexSub)
		{
			FMegalithIndexDatabase* DB = IndexSub->GetDatabase();
			if (DB)
			{
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleFindSOReferences(const TSharedPtr<FJsonObject>& Params)
{
	FString SOPath = Params->GetStringField(TEXT("so_path"));
	if (SOPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: so_path"));
	}
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleLintBehaviorTree(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBehaviorTree* BT = MegalithAI::LoadBehaviorTreeFromParams(Params, AssetPath, Error);
	if (!BT) return FMegalithActionResult::Error(Error);
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleLintStateTree(const TSharedPtr<FJsonObject>& Params)
{
#if WITH_STATETREE
	...
	AssetPath = FMegalithAssetUtils::ResolveAssetPath(AssetPath);

	UStateTree* ST = FMegalithAssetUtils::LoadAssetByPath<UStateTree>(AssetPath);
	if (!ST)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("StateTree not found at '%s'"), *AssetPath));
	}
	...
	if (!EditorData)
	{
		return FMegalithActionResult::Error(TEXT("StateTree has no editor data"));
	}
	...
	return FMegalithActionResult::Success(Result);
#else
	return FMegalithActionResult::Error(TEXT("Editor data not available"));
#endif

#else
	return FMegalithActionResult::Error(TEXT("StateTree module not available (WITH_STATETREE=0)"));
#endif
}
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleDetectAICircularReferences(const TSharedPtr<FJsonObject>& Params)
{
	FString PathFilter = Params->GetStringField(TEXT("path_filter"));
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleExportAIManifest(const TSharedPtr<FJsonObject>& Params)
{
	FString PathFilter = Params->GetStringField(TEXT("path_filter"));
...
FMegalithActionResult FMegalithAIDiscoveryActions::HandleGetAIBehaviorSummary(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	UBehaviorTree* BT = FMegalithAssetUtils::LoadAssetByPath<UBehaviorTree>(AssetPath);
	if (BT)
	{
		TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
		Result->SetStringField(TEXT("asset_path"), AssetPath);
	...
	return FMegalithActionResult::Error(FString::Printf(TEXT("Asset at '%s' is not a BehaviorTree or StateTree"), *AssetPath));
}
