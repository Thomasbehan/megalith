...
void FMegalithAIPerceptionActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("add_perception_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddPerceptionComponent),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_perception_config"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetPerceptionConfig),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("configure_sight_sense"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureSightSense),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("configure_hearing_sense"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureHearingSense),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("configure_damage_sense"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureDamageSense),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("configure_touch_sense"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureTouchSense),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
			.Optional(TEXT("affiliation"), TEXT("object"), TEXT("Detection affiliation: {enemies: bool, neutrals: bool, friendlies: bool}"))
			.Optional(TEXT("max_age"), TEXT("number"), TEXT("Stimulus max age in seconds (0 = never expires). Reflection-write to UAISenseConfig::MaxAge."))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("remove_sense"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRemoveSense),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("add_stimuli_source_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddStimuliSourceComponent),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("configure_stimuli_source"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureStimuliSource),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Actor Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("validate_perception_setup"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidatePerceptionSetup),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_ai_system_config"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAISystemConfig),
		FParamSchemaBuilder()
			.Build());
}
...
USCS_Node* FMegalithAIPerceptionActions::FindPerceptionNode(UBlueprint* BP)
{
...
USCS_Node* FMegalithAIPerceptionActions::FindStimuliSourceNode(UBlueprint* BP)
{
	if (!BP || !BP->SimpleConstructionScript) return nullptr;
...
UAIPerceptionComponent* FMegalithAIPerceptionActions::GetPerceptionTemplate(USCS_Node* Node)
{
	if (!Node || !Node->ComponentTemplate) return nullptr;
	return Cast<UAIPerceptionComponent>(Node->ComponentTemplate);
}

void FMegalithAIPerceptionActions::ParseAffiliation(const TSharedPtr<FJsonObject>& Params, const FString& FieldName,
...
TSharedPtr<FJsonObject> FMegalithAIPerceptionActions::AffiliationToJson(const FAISenseAffiliationFilter& Filter)
{
...
TSenseConfig* FMegalithAIPerceptionActions::FindOrCreateSenseConfig(UAIPerceptionComponent* PerceptionComp)
{
...
template<typename TSenseConfig>
TSenseConfig* FMegalithAIPerceptionActions::FindSenseConfig(UAIPerceptionComponent* PerceptionComp)
{
	if (!PerceptionComp) return nullptr;
	return PerceptionComp->GetSenseConfig<TSenseConfig>();
}

UClass* FMegalithAIPerceptionActions::ResolveSenseConfigClass(const FString& SenseType)
{
...
TSharedPtr<FJsonObject> FMegalithAIPerceptionActions::SenseConfigToJson(UAISenseConfig* SenseConfig)
{
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleAddPerceptionComponent(const TSharedPtr<FJsonObject>& Params)
{
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Add Perception Component")));
	...
	if (!NewNode)
	{
		...
		return FMegalithActionResult::Error(TEXT("Failed to create perception component SCS node"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, TEXT("AIPerceptionComponent added"));
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleGetPerceptionConfig(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlueprint* BP = MegalithAI::LoadAIControllerFromParams(Params, AssetPath, Error);
	if (!BP)
	...
	if (!PerceptionComp)
	{
		return FMegalithActionResult::Error(TEXT("Failed to get perception component template"));
	}
...
namespace
{
	...
	bool ResolvePerceptionContext(const TSharedPtr<FJsonObject>& Params, FPerceptionContext& Ctx, FMegalithActionResult& OutError)
	{
		FString Error;
		Ctx.BP = MegalithAI::LoadAIControllerFromParams(Params, Ctx.AssetPath, Error);
		if (!Ctx.BP)
		{
		...
		Ctx.PerceptionNode = FMegalithAIPerceptionActions::FindPerceptionNode(Ctx.BP);
		...
		Ctx.PerceptionComp = FMegalithAIPerceptionActions::GetPerceptionTemplate(Ctx.PerceptionNode);
		if (!Ctx.PerceptionComp)
		{
			OutError = FMegalithActionResult::Error(TEXT("Failed to get perception component template"));
			return false;
		}
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleConfigureSightSense(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Radius <= 0.0)
	{
		return FMegalithActionResult::Error(TEXT("radius must be a positive number"));
	}

	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Configure Sight Sense")));

	UAISenseConfig_Sight* Sight = FindOrCreateSenseConfig<UAISenseConfig_Sight>(Ctx.PerceptionComp);
	if (!Sight)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create sight sense config"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(Ctx.AssetPath, TEXT("Sight sense configured"));
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleConfigureHearingSense(const TSharedPtr<FJsonObject>& Params)
{
	FPerceptionContext Ctx;
	FMegalithActionResult ErrResult;
	if (!ResolvePerceptionContext(Params, Ctx, ErrResult))
	...
	if (Range <= 0.0)
	{
		return FMegalithActionResult::Error(TEXT("range must be a positive number"));
	}

	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Configure Hearing Sense")));

	UAISenseConfig_Hearing* Hearing = FindOrCreateSenseConfig<UAISenseConfig_Hearing>(Ctx.PerceptionComp);
	if (!Hearing)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create hearing sense config"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(Ctx.AssetPath, TEXT("Hearing sense configured"));
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleConfigureDamageSense(const TSharedPtr<FJsonObject>& Params)
{
	FPerceptionContext Ctx;
	FMegalithActionResult ErrResult;
	if (!ResolvePerceptionContext(Params, Ctx, ErrResult))
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Configure Damage Sense")));

	UAISenseConfig_Damage* Damage = FindOrCreateSenseConfig<UAISenseConfig_Damage>(Ctx.PerceptionComp);
	if (!Damage)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create damage sense config"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(Ctx.AssetPath, TEXT("Damage sense configured"));
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleConfigureTouchSense(const TSharedPtr<FJsonObject>& Params)
{
	FPerceptionContext Ctx;
	FMegalithActionResult ErrResult;
	if (!ResolvePerceptionContext(Params, Ctx, ErrResult))
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Configure Touch Sense")));

	UAISenseConfig_Touch* Touch = FindOrCreateSenseConfig<UAISenseConfig_Touch>(Ctx.PerceptionComp);
	if (!Touch)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create touch sense config"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(Ctx.AssetPath, TEXT("Touch sense configured"));
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleRemoveSense(const TSharedPtr<FJsonObject>& Params)
{
	FPerceptionContext Ctx;
	FMegalithActionResult ErrResult;
	if (!ResolvePerceptionContext(Params, Ctx, ErrResult))
	...
	FString SenseType;
	if (!MegalithAI::RequireStringParam(Params, TEXT("sense_type"), SenseType, ErrResult))
	{
		return ErrResult;
	}
	...
	if (!SensesConfigProp)
	{
		return FMegalithActionResult::Error(TEXT("Failed to find SensesConfig property via reflection"));
	}
	...
	if (!SensesArray)
	{
		return FMegalithActionResult::Error(TEXT("Failed to access SensesConfig array"));
	}
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Remove Sense")));
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(Ctx.AssetPath,
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleAddStimuliSourceComponent(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FMegalithActionResult ErrResult;
	...
	UBlueprint* BP = Cast<UBlueprint>(FMegalithAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), AssetPath));
	if (!BP)
	{
	...
	// Parse sense types
	TArray<FString> SenseTypes = MegalithAI::ParseStringArray(Params, TEXT("register_as_source_for"));
	if (SenseTypes.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("register_as_source_for must contain at least one sense type"));
	}
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Add Stimuli Source Component")));
	...
	{
		BP->Status = SavedStatus;
		return FMegalithActionResult::Error(TEXT("Failed to create stimuli source SCS node"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, TEXT("StimuliSourceComponent added"));
	Result->SetStringField(TEXT("variable_name"), NewNode->GetVariableName().ToString());
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleConfigureStimuliSource(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("asset_path"), AssetPath, ErrResult))
	...
	if (!StimuliComp)
	{
		return FMegalithActionResult::Error(TEXT("Failed to get stimuli source component template"));
	}

	TArray<FString> SenseTypes = MegalithAI::ParseStringArray(Params, TEXT("sense_types"));
	if (SenseTypes.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("sense_types must contain at least one sense type"));
	}
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Configure Stimuli Source")));
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, TEXT("Stimuli source configured"));
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleValidatePerceptionSetup(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlueprint* BP = MegalithAI::LoadAIControllerFromParams(Params, AssetPath, Error);
	if (!BP)
...
FMegalithActionResult FMegalithAIPerceptionActions::HandleGetAISystemConfig(const TSharedPtr<FJsonObject>& Params)
{
