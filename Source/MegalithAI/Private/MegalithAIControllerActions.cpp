...
void FMegalithAIControllerActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("create_ai_controller"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateAIController),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_ai_controller"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAIController),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("list_ai_controllers"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListAIControllers),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("set_ai_controller_bt"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetAIControllerBT),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("set_pawn_ai_controller_class"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetPawnAIControllerClass),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("set_ai_controller_flags"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetAIControllerFlags),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("set_ai_team"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetAITeam),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_ai_team"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAITeam),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("AIController Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("spawn_ai_actor"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSpawnAIActor),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("ai"), TEXT("get_ai_actors"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetAIActors),
		FParamSchemaBuilder()
...
FMegalithActionResult FMegalithAIControllerActions::HandleCreateAIController(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath;
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Create AI Controller")));
	...
	if (!NewBP)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create AIController Blueprint"));
	}
	...
	if (!BTPath.IsEmpty())
	{
		UBehaviorTree* BT = Cast<UBehaviorTree>(FMegalithAssetUtils::LoadAssetByPath(UBehaviorTree::StaticClass(), BTPath));
		if (BT)
		{
			bBTValid = true;
		...
		{
			UE_LOG(LogMegalithAI, Warning, TEXT("BT not found: %s"), *BTPath);
		}
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(PackagePath, TEXT("AI Controller Blueprint created"));
...
FMegalithActionResult FMegalithAIControllerActions::HandleGetAIController(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithAIControllerActions::HandleListAIControllers(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithAIControllerActions::HandleSetAIControllerBT(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlueprint* BP = MegalithAI::LoadAIControllerFromParams(Params, AssetPath, Error);
	if (!BP)
	...
	FString BTPath;
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("bt_path"), BTPath, ErrResult))
	{
		return ErrResult;
	}
	...
	if (!BT)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Behavior Tree not found: %s"), *BTPath));
	}
	...
	if (!BBPath.IsEmpty())
	{
		BB = Cast<UBlackboardData>(FMegalithAssetUtils::LoadAssetByPath(UBlackboardData::StaticClass(), BBPath));
		if (!BB)
		{
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Set AI Controller BT")));
	...
	if (!BP->GeneratedClass)
	{
		return FMegalithActionResult::Error(TEXT("Failed to compile AIController Blueprint"));
	}

	AAIController* CDO = Cast<AAIController>(BP->GeneratedClass->GetDefaultObject());
	if (!CDO)
	{
		return FMegalithActionResult::Error(TEXT("Failed to get AIController CDO"));
	}
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, TEXT("AI Controller BT updated"));
...
FMegalithActionResult FMegalithAIControllerActions::HandleSetPawnAIControllerClass(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!MegalithAI::RequireStringParam(Params, TEXT("controller_class"), ControllerClassStr, ErrResult))
	{
		return ErrResult;
	}

	// Load the Pawn/Character Blueprint
	UBlueprint* PawnBP = Cast<UBlueprint>(FMegalithAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), BlueprintPath));
	if (!PawnBP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint not found: %s"), *BlueprintPath));
	}

	if (!PawnBP->GeneratedClass || !PawnBP->GeneratedClass->IsChildOf(APawn::StaticClass()))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("'%s' is not a Pawn/Character Blueprint"), *BlueprintPath));
	}
	...
	UBlueprint* ControllerBP = Cast<UBlueprint>(FMegalithAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), ControllerClassStr));
	...
	if (!ControllerClass)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Controller class not found: %s"), *ControllerClassStr));
	}

	if (!ControllerClass->IsChildOf(AController::StaticClass()))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("'%s' is not a Controller class"), *ControllerClassStr));
	}

	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Set Pawn AI Controller Class")));

	APawn* CDO = Cast<APawn>(PawnBP->GeneratedClass->GetDefaultObject());
	if (!CDO)
	{
		return FMegalithActionResult::Error(TEXT("Failed to get Pawn CDO"));
	}

	CDO->AIControllerClass = ControllerClass;
	PawnBP->MarkPackageDirty();

	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(BlueprintPath, TEXT("AIControllerClass updated on Pawn CDO"));
...
FMegalithActionResult FMegalithAIControllerActions::HandleSetAIControllerFlags(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlueprint* BP = MegalithAI::LoadAIControllerFromParams(Params, AssetPath, Error);
	if (!BP)
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Set AI Controller Flags")));
	...
	if (ChangedCount == 0)
	{
		return FMegalithActionResult::Error(TEXT("No valid flags provided. Supported: wants_player_state, allow_strafe, start_ai_on_possess, skip_extra_los_checks"));
	}
...
FMegalithActionResult FMegalithAIControllerActions::HandleSetAITeam(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlueprint* BP = MegalithAI::LoadAIControllerFromParams(Params, AssetPath, Error);
	if (!BP)
	...
	{
		return FMegalithActionResult::Error(TEXT("team_id must be 0-255 (255 = NoTeam)"));
	}
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Set AI Team")));
	...
	TSharedPtr<FJsonObject> Result = MegalithAI::MakeAssetResult(AssetPath, TEXT("Team ID updated"));
...
FMegalithActionResult FMegalithAIControllerActions::HandleGetAITeam(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath, Error;
	UBlueprint* BP = MegalithAI::LoadAIControllerFromParams(Params, AssetPath, Error);
	if (!BP)
	...
	if (!BP->GeneratedClass)
	{
		return FMegalithActionResult::Error(TEXT("Blueprint has no GeneratedClass — needs compilation"));
	}
...
FMegalithActionResult FMegalithAIControllerActions::HandleSpawnAIActor(const TSharedPtr<FJsonObject>& Params)
{
	FString ClassPath;
	FMegalithActionResult ErrResult;
	if (!MegalithAI::RequireStringParam(Params, TEXT("class_path"), ClassPath, ErrResult))
	{
		return ErrResult;
	}
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'location' as {x,y,z}"));
	}
	...
	UBlueprint* BP = Cast<UBlueprint>(FMegalithAssetUtils::LoadAssetByPath(UBlueprint::StaticClass(), ClassPath));
	if (!BP || !BP->GeneratedClass)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint not found or not compiled: %s"), *ClassPath));
	}
	...
	FScopedTransaction Transaction(FText::FromString(TEXT("Megalith: Spawn AI Actor")));
...
FMegalithActionResult FMegalithAIControllerActions::HandleGetAIActors(const TSharedPtr<FJsonObject>& Params)
{
