 ...
 * and AIController Blueprints. Registers itself into MegalithIndex at startup.
 * Lives in MegalithAI to avoid polluting the core index with optional AI deps.
 *
 * Skeleton version -- BT, BB, Controller indexing only.
 * StateTree, EQS, SmartObject, Perception indexing added in later phases.
 */
class FAIIndexer : public IMegalithIndexer
{
public:
...
private:
	...
	void IndexBehaviorTree(class UBehaviorTree* BT, const FString& AssetPath, FMegalithIndexDatabase& DB);

	/** Index a single BlackboardData asset */
	void IndexBlackboard(class UBlackboardData* BB, const FString& AssetPath, FMegalithIndexDatabase& DB);

	/** Index an AIController Blueprint (CDO inspection) */
	void IndexAIController(class UBlueprint* BP, const FString& AssetPath, FMegalithIndexDatabase& DB);
	...
	void EnsureTablesExist(FMegalithIndexDatabase& DB);
