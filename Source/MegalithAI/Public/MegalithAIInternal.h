...
DECLARE_LOG_CATEGORY_EXTERN(LogMegalithAI, Log, All);

namespace MegalithAI
{
	/**
	 * Backwards-compat shim — delegates to FMegalithAssetUtils::LoadAssetByPath.
	 *
	 * The canonical 4-tier resolver now lives in MegalithCore (see
	 * FMegalithAssetUtils::LoadAssetByPath in MegalithAssetUtils.h) so all
	 * Megalith modules share a single source of truth. New code should call
	 * FMegalithAssetUtils::LoadAssetByPath directly. This function is retained
	 * for byte-identical behaviour at existing MegalithAI callsites.
	 *
