...
public class MegalithAI : ModuleRules
{
	...
	// Keep BYTE-IDENTICAL with the copies in MegalithMesh/MegalithIndex/MegalithAudio/MegalithAnimation.
	...
	public MegalithAI(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		{
			"MegalithCore", "MegalithBlueprint", "MegalithIndex",
		...
		// MEGALITH_RELEASE_BUILD=1 forces all optional plugin deps OFF so binary
		// release zips never hard-link against plugins the end-user may not have
		// enabled. Mirrors the pattern in MegalithGAS.Build.cs / MegalithUI.Build.cs.
		// Origin: GitHub issue #30 (MegalithMesh.dll hard-linked GeometryScriptingCore).
		...
		// Engine plugin (EnabledByDefault:false) but listed NON-optional in Megalith.uplugin,
		// so it is force-enabled when the user enables Megalith. Issue #71: gate on ENABLEMENT
		...
		// Issue #71 PRIMARY FIX: Optional:true in Megalith.uplugin, so NOT transitively
		// enabled when the user enables Megalith. Ships on disk on every install →
		...
		// MassGameplay is Optional:true in Megalith.uplugin → NOT transitively enabled; disk presence
		...
		// Issue #71 PRIMARY FIX: Optional:true in Megalith.uplugin, ships-but-off on every
