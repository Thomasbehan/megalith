...
// MegalithAudioBulkFillAdapter — Phase 5 Step 4 adapter.
//
...
//     Reflection writes via FMegalithReflectionWalker — these classes carry
...
//     make_release.ps1 $LeakSentinels (M6 invariant). MegalithAudio.Build.cs
...
#define LOCTEXT_NAMESPACE "MegalithAudioBulkFillAdapter"

namespace MegalithAudioBulkFillInternal
{
	...
	static FDryRunReport HandleReflectionFill(const FBulkFillSpec& Spec, const TCHAR* KindLabel)
	{
		...
		if (Spec.bDryRun)
		{
			Report = FMegalithReflectionWalker::InspectTree(*PropsObj, Typed->GetClass(), Typed, Spec);
			Report.bWouldApply = false;
			return Report;
		}

		Typed->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("AudioBulkFill_Refl", "Megalith Audio Bulk Fill"));
		Typed->Modify();
		Typed->PreEditChange(nullptr);

		Report = FMegalithReflectionWalker::WriteTree(*PropsObj, Typed->GetClass(), Typed, Typed, Spec);
...
FDryRunReport FMegalithAudioBulkFillAdapter::AudioBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithAudioBulkFillInternal;
	...
	if (FillKind == TEXT("MetaSoundInputs"))
	{
		...
		if (Spec.bDryRun)
		{
			Report = FMegalithReflectionWalker::InspectTree(*InputsObj, Asset->GetClass(), Asset, Spec);
			Report.bWouldApply = false;
			return Report;
		}

		Asset->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("AudioBulkFill_MS", "Megalith Audio Bulk Fill — MetaSound Inputs"));
		Asset->Modify();
		Asset->PreEditChange(nullptr);

		Report = FMegalithReflectionWalker::WriteTree(*InputsObj, Asset->GetClass(), Asset, Asset, Spec);
		if (Spec.bStrict && Report.Errors > 0)
		{
			Transaction.Cancel();
			Report.bWouldApply = false;
...
FSchemaDescriptor FMegalithAudioBulkFillAdapter::AudioDescribe(const FString& TargetAsset)
{
	using namespace MegalithAudioBulkFillInternal;
...
void FMegalithAudioBulkFillAdapter::Register()
{
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("audio"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithAudioBulkFillAdapter::AudioBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithAudioBulkFillAdapter::AudioDescribe));
}

void FMegalithAudioBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("audio"));
}
