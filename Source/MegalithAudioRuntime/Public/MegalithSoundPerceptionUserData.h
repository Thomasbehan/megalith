...
#include "MegalithSoundPerceptionUserData.generated.h"
 ...
 *
 * Runtime side: UMegalithAudioPerceptionSubsystem hooks UAudioComponent::OnAudioPlayStateChangedNative.
...
UCLASS(BlueprintType, MinimalAPI,
       meta = (DisplayName = "Megalith: Sound Perception Binding"))
class UMegalithSoundPerceptionUserData : public UAssetUserData
{
	GENERATED_BODY()

public:
	/** Master switch — author can disable a binding without removing the UserData entirely. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Megalith|Perception")
	bool bEnabled = true;

	/** Loudness multiplier passed to FAINoiseEvent::Loudness. Modifies effective range. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Megalith|Perception", meta = (ClampMin = "0.0", UIMin = "0.0"))
	float Loudness = 1.0f;
	...
	// FName GetMegalithPerceptionTag() {} is queried via AssetRegistry tags after PostEditChange.
};
