...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDComponent, Log, All);
...
void FMegalithLogicDriverComponentActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("get_sm_component_config"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetSMComponentConfig),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("add_sm_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleAddSMComponent),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("configure_sm_component"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureSMComponent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("blueprint_path"), TEXT("Actor Blueprint asset path"))
	...
	UE_LOG(LogMegalithLDComponent, Log, TEXT("MegalithLogicDriver Component: registered 3 actions"));
}

FMegalithActionResult FMegalithLogicDriverComponentActions::HandleGetSMComponentConfig(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid() || !Params->HasField(TEXT("blueprint_path")))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: blueprint_path"));
	}
	...
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load Blueprint at '%s'"), *BPPath));
	}

	if (!BP->SimpleConstructionScript)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Blueprint '%s' has no SimpleConstructionScript (not an Actor BP?)"), *BPPath));
	}
...
FMegalithActionResult FMegalithLogicDriverComponentActions::HandleAddSMComponent(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid() || !Params->HasField(TEXT("blueprint_path")))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: blueprint_path"));
	...
	if (!SMCompClass)
	{
		return FMegalithActionResult::Error(TEXT("SMStateMachineComponent class not found. Is Logic Driver Pro loaded?"));
	}
	...
	if (!NewNode)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create SCS node for SM component"));
	}
	...
	if (!SMPath.IsEmpty() && NewNode->ComponentTemplate)
	{
		...
		FString LoadError;
		UBlueprint* SMBP = MegalithLD::LoadSMBlueprint(SMPath, LoadError);
		...
		{
			UE_LOG(LogMegalithLDComponent, Warning, TEXT("Could not load SM Blueprint at '%s' to set StateMachineClass: %s"), *SMPath, *LoadError);
		}
	}
...
FMegalithActionResult FMegalithLogicDriverComponentActions::HandleConfigureSMComponent(const TSharedPtr<FJsonObject>& Params)
{
	if (!Params.IsValid() || !Params->HasField(TEXT("blueprint_path")))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param: blueprint_path"));
	...
	if (!BP->SimpleConstructionScript)
	{
		return FMegalithActionResult::Error(TEXT("Blueprint has no SimpleConstructionScript (not an Actor BP?)"));
	}
