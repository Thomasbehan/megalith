...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDAsset, Log, All);
...
//  RegisterActions
// ============================================================
void FMegalithLogicDriverAssetActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("get_state_machine"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetStateMachine),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("list_state_machines"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListStateMachines),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("delete_state_machine"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDeleteStateMachine),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("duplicate_state_machine"),
		...
		FMegalithActionHandler::CreateStatic(&HandleDuplicateStateMachine),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("create_node_blueprint"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCreateNodeBlueprint),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("get_node_blueprint"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetNodeBlueprint),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("list_node_blueprints"),
		...
		FMegalithActionHandler::CreateStatic(&HandleListNodeBlueprints),
		FParamSchemaBuilder()
	...
	UE_LOG(LogMegalithLDAsset, Log, TEXT("MegalithLogicDriver Asset: registered 8 actions"));
}

// ============================================================
//  1. create_state_machine
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleCreateStateMachine(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (AssetName.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Could not determine asset name from save_path"));
	}

	// Find factory via FindFirstObject (hybrid approach — marketplace plugin may be precompiled)
	UClass* FactoryClass = MegalithLD::GetSMBlueprintFactoryClass();
	if (!FactoryClass)
	{
		return FMegalithActionResult::Error(TEXT("SMBlueprintFactory class not found. Is Logic Driver Pro loaded?"));
	}
	...
	if (!Factory)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create SMBlueprintFactory instance"));
	}
	...
	FString ExistError;
	if (!MegalithLD::EnsureAssetPathFree(Package, SavePath, AssetName, ExistError))
	{
	...
	if (!SupportedClass)
	{
		...
		SupportedClass = MegalithLD::GetSMBlueprintClass();
	}
	...
	if (SMBlueprint)
	{
		...
		MegalithLD::CompileSMBlueprint(SMBlueprint, CompileError);
...
//  2. get_state_machine
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleGetStateMachine(const TSharedPtr<FJsonObject>& Params)
{
	...
	TSharedPtr<FJsonObject> Structure = MegalithLD::SMStructureToJson(SMBlueprint);
	if (!Structure.IsValid())
	{
		return FMegalithActionResult::Error(TEXT("Failed to serialize state machine structure"));
	}
...
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleListStateMachines(const TSharedPtr<FJsonObject>& Params)
{
	FString PathFilter;
	...
	FARFilter Filter;
	UClass* SMBPClass = MegalithLD::GetSMBlueprintClass();
	if (!SMBPClass)
	{
		return FMegalithActionResult::Error(TEXT("Logic Driver not loaded — SMBlueprint class not found"));
	}
...
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleDeleteStateMachine(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
...
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleDuplicateStateMachine(const TSharedPtr<FJsonObject>& Params)
{
	FString SourcePath = Params->GetStringField(TEXT("source_path"));
	if (SourcePath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'source_path'"));
	}

	FString DestPath = Params->GetStringField(TEXT("dest_path"));
	if (DestPath.IsEmpty())
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'dest_path'"));
	}

	// Verify source exists and is an SM Blueprint
	FString LoadError;
	UBlueprint* SourceBP = MegalithLD::LoadSMBlueprint(SourcePath, LoadError);
	if (!SourceBP)
	{
		return FMegalithActionResult::Error(LoadError);
	}
	...
	if (!MegalithLD::EnsureAssetPathFree(nullptr, DestPath, DestName, DupExistError))
	{
		return FMegalithActionResult::Error(DupExistError);
	}
...
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleCreateNodeBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	FString AssetName = Params->GetStringField(TEXT("name"));
	FString NodeType = Params->GetStringField(TEXT("node_type")).ToLower();

	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'save_path'"));
	if (AssetName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'name'"));
	if (NodeType.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'node_type'"));

	if (NodeType != TEXT("state") && NodeType != TEXT("transition")
		&& NodeType != TEXT("conduit") && NodeType != TEXT("state_machine"))
	{
		return FMegalithActionResult::Error(TEXT("node_type must be: state, transition, conduit, state_machine"));
	}

	// Get factory
	UClass* FactoryClass = MegalithLD::GetSMNodeBlueprintFactoryClass();
	if (!FactoryClass)
	{
		return FMegalithActionResult::Error(TEXT("SMNodeBlueprintFactory class not found. Is Logic Driver Pro loaded?"));
	}

	UFactory* Factory = NewObject<UFactory>(GetTransientPackage(), FactoryClass);
	if (!Factory)
	{
		return FMegalithActionResult::Error(TEXT("Failed to create SMNodeBlueprintFactory instance"));
	}
	...
	if (!SupportedClass)
	{
		SupportedClass = MegalithLD::GetSMNodeBlueprintClass();
	}
	...
	if (!NewAsset)
	{
		return FMegalithActionResult::Error(TEXT("FactoryCreateNew returned null for Node Blueprint"));
	}
	...
	{
		FString CompileError;
		MegalithLD::CompileSMBlueprint(NodeBP, CompileError);
	}
...
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleGetNodeBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty())
	{
	...
	UClass* NodeBPClass = MegalithLD::GetSMNodeBlueprintClass();
	if (!NodeBPClass)
	{
		return FMegalithActionResult::Error(TEXT("Logic Driver not loaded — SMNodeBlueprint class not found"));
	}
	...
	if (!BP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load Blueprint at '%s'"), *AssetPath));
	}
...
// ============================================================
FMegalithActionResult FMegalithLogicDriverAssetActions::HandleListNodeBlueprints(const TSharedPtr<FJsonObject>& Params)
{
	UClass* NodeBPClass = MegalithLD::GetSMNodeBlueprintClass();
	if (!NodeBPClass)
	{
