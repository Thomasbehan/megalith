...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDNode, Log, All);
...
FNodeLookupResult LoadAndFindNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		Result.Error = FMegalithActionResult::Error(TEXT("Missing required param 'asset_path'"));
		return Result;
	}
	...
	if (NodeGuid.IsEmpty())
	{
		Result.Error = FMegalithActionResult::Error(TEXT("Missing required param 'node_guid' (or 'transition_guid')"));
		return Result;
	}

	FString LoadError;
	Result.Blueprint = MegalithLD::LoadSMBlueprint(AssetPath, LoadError);
	if (!Result.Blueprint)
	{
		Result.Error = FMegalithActionResult::Error(LoadError);
		return Result;
	}

	Result.Graph = MegalithLD::GetRootGraph(Result.Blueprint);
	if (!Result.Graph)
	{
		Result.Error = FMegalithActionResult::Error(TEXT("No root SM graph found"));
		return Result;
	}

	Result.Node = MegalithLD::FindNodeByGuid(Result.Graph, NodeGuid);
	if (!Result.Node)
	{
		Result.Error = FMegalithActionResult::Error(FString::Printf(TEXT("Node with GUID '%s' not found"), *NodeGuid));
		return Result;
	}
...
void FMegalithLogicDriverNodeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// ── configure_state (29) ──
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("configure_state"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureState),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("configure_transition"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureTransition),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("configure_conduit"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureConduit),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("set_transition_condition"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetTransitionCondition),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("set_state_tags"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetStateTags),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
			.Required(TEXT("node_guid"), TEXT("string"), TEXT("State node GUID"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("get_exposed_properties"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetExposedProperties),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("set_exposed_property"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetExposedProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("configure_state_machine_node"),
		...
		FMegalithActionHandler::CreateStatic(&HandleConfigureStateMachineNode),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
	...
	UE_LOG(LogMegalithLDNode, Log, TEXT("MegalithLogicDriver Node: registered 8 actions"));
}

// ── configure_state (29) ────────────────────────────────────────────

FMegalithActionResult FMegalithLogicDriverNodeActions::HandleConfigureState(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node is type '%s', expected 'state' or 'any_state'"), *NodeType));
	}
	...
	if (Params->HasField(TEXT("always_update")))
	{
		...
		if (SetBoolProp(Node, TEXT("bAlwaysUpdate"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Property 'bAlwaysUpdate' not found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("disable_tick_transition")))
	{
		...
		if (SetBoolProp(Node, TEXT("bDisableTickTransitionEvaluation"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Property 'bDisableTickTransitionEvaluation' not found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("exclude_from_any_state")))
	{
		...
		if (SetBoolProp(Node, TEXT("bExcludeFromAnyState"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Property 'bExcludeFromAnyState' not found on %s"), *Node->GetClass()->GetName());
	}

	if (Applied.Num() == 0)
	{
		{ TSharedPtr<FJsonObject> R = MakeShared<FJsonObject>(); R->SetStringField(TEXT("message"), TEXT("No properties were set (none provided or none matched)")); return FMegalithActionResult::Success(R); }
	}
...
FMegalithActionResult FMegalithLogicDriverNodeActions::HandleConfigureTransition(const TSharedPtr<FJsonObject>& Params)
{
	FNodeLookupResult Lookup = LoadAndFindNode(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node is type '%s', expected 'transition'"), *NodeType));
	}
	...
	if (Params->HasField(TEXT("priority")))
	{
		...
		if (SetIntProp(Node, TEXT("PriorityOrder"), Val))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Property 'PriorityOrder' not found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("color")))
	{
		...
		if (SetColorProp(Node, TEXT("NodeTintColor"), ColorStr))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("No color property found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("eval_mode")))
	{
		...
		if (SetEnumPropFromString(Node, TEXT("ConditionalEvaluationType"), ModeStr))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("No eval mode property found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("can_eval_with_start_state")))
	{
		...
		if (SetBoolProp(Node, TEXT("bCanEvaluateWithStartState"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("No can_eval_with_start_state property found on %s"), *Node->GetClass()->GetName());
	}
...
FMegalithActionResult FMegalithLogicDriverNodeActions::HandleConfigureConduit(const TSharedPtr<FJsonObject>& Params)
{
	FNodeLookupResult Lookup = LoadAndFindNode(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node is type '%s', expected 'conduit'"), *NodeType));
	}
	...
	if (Params->HasField(TEXT("eval_with_transitions")))
	{
		...
		if (SetBoolProp(Node, TEXT("bEvalWithTransitions"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Property 'bEvalWithTransitions' not found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("conduit_as_state")))
	{
		...
		if (SetBoolProp(Node, TEXT("bConduitAsState"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("No conduit_as_state property found on %s"), *Node->GetClass()->GetName());
	}
...
FMegalithActionResult FMegalithLogicDriverNodeActions::HandleSetTransitionCondition(const TSharedPtr<FJsonObject>& Params)
{
	FNodeLookupResult Lookup = LoadAndFindNode(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'condition_type'"));
	}
	...
	else if (ConditionType == TEXT("tag_check"))
	{
		...
		{
			return FMegalithActionResult::Error(TEXT("tag_check requires 'tag' in params"));
		}
		...
		if (TagsProp && TagsProp->Struct == FGameplayTagContainer::StaticStruct())
		{
			...
			{
				UE_LOG(LogMegalithLDNode, Warning, TEXT("Gameplay tag '%s' is not registered"), *TagString);
		...
		{
			UE_LOG(LogMegalithLDNode, Warning, TEXT("No GameplayTagContainer property found on transition %s"), *Node->GetClass()->GetName());
		}
...
FMegalithActionResult FMegalithLogicDriverNodeActions::HandleGetExposedProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	...
	auto ExtractExposedProps = [](UEdGraphNode* Node) -> TSharedPtr<FJsonObject>
	{
		...
		NodeResult->SetStringField(TEXT("node_type"), MegalithLD::GetNodeType(Node));
	...
	{
		// Single node
		UEdGraphNode* Node = MegalithLD::FindNodeByGuid(RootGraph, TargetGuid);
		if (!Node) return FMegalithActionResult::Error(FString::Printf(TEXT("Node '%s' not found"), *TargetGuid));
...
FMegalithActionResult FMegalithLogicDriverNodeActions::HandleSetExposedProperty(const TSharedPtr<FJsonObject>& Params)
{
	FNodeLookupResult Lookup = LoadAndFindNode(Params);
	if (!Lookup.bSuccess) return Lookup.Error;

	FString PropertyName = Params->GetStringField(TEXT("property_name"));
	if (PropertyName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'property_name'"));
	...
	if (!Prop)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Property '%s' not found on node class '%s'"),
	...
	if (!ImportResult)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to set property '%s' to '%s' (ImportText failed)"),
			*PropertyName, *Value));
	}
...
FMegalithActionResult FMegalithLogicDriverNodeActions::HandleConfigureStateMachineNode(const TSharedPtr<FJsonObject>& Params)
{
	FNodeLookupResult Lookup = LoadAndFindNode(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node is type '%s', expected 'state_machine'"), *NodeType));
	}
	...
	if (Params->HasField(TEXT("reuse_if_not_end_state")))
	{
		...
		if (SetBoolProp(Node, TEXT("bReuseIfNotEndState"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("No reuse_if_not_end_state property found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("reuse_current_state")))
	{
		...
		if (SetBoolProp(Node, TEXT("bReuseCurrentState"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Property 'bReuseCurrentState' not found on %s"), *Node->GetClass()->GetName());
	}
	...
	if (Params->HasField(TEXT("allow_independent_tick")))
	{
		...
		if (SetBoolProp(Node, TEXT("bAllowIndependentTick"), bVal))
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Property 'bAllowIndependentTick' not found on %s"), *Node->GetClass()->GetName());
	}
...
FMegalithActionResult FMegalithLogicDriverNodeActions::HandleSetStateTags(const TSharedPtr<FJsonObject>& Params)
{
	FNodeLookupResult Lookup = LoadAndFindNode(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
	...
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Node is type '%s', expected a state-like node"), *NodeType));
	}

	if (!Params->HasField(TEXT("gameplay_tags")))
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'gameplay_tags'"));
	}
	...
	for (const TSharedPtr<FJsonValue>& TagVal : TagArray)
	{
		...
		else
		{
			...
			UE_LOG(LogMegalithLDNode, Warning, TEXT("Gameplay tag '%s' not registered — skipping"), *TagString);
		}
	}
