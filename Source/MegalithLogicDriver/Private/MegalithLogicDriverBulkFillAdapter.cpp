...
// MegalithLogicDriverBulkFillAdapter — Phase 5 Step 7 adapter.
//
...
#define LOCTEXT_NAMESPACE "MegalithLogicDriverBulkFillAdapter"

namespace MegalithLogicDriverBulkFillInternal
{
	...
	static FDryRunReport HandleStatePropertiesBulk(const FBulkFillSpec& Spec)
	{
		...
		if (!Spec.bDryRun)
		{
			...
			Transaction = MakeShared<FScopedTransaction>(
				LOCTEXT("LogicDriverBulkFill_States", "Megalith LogicDriver Bulk Fill — State Properties"));
			CDO->Modify();
		...
		{
			const FString StateName = MegalithKeyToString(StateKV.Key);
			...
			FDryRunReport Sub = Spec.bDryRun
				? FMegalithReflectionWalker::InspectTree(*StatePropsObj, SMClass, CDO, Spec)
				: FMegalithReflectionWalker::WriteTree(*StatePropsObj, SMClass, CDO, CDO, Spec);
...
FDryRunReport FMegalithLogicDriverBulkFillAdapter::LogicDriverBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithLogicDriverBulkFillInternal;
...
FSchemaDescriptor FMegalithLogicDriverBulkFillAdapter::LogicDriverDescribe(const FString& TargetAsset)
{
	using namespace MegalithLogicDriverBulkFillInternal;
	...
	FSchemaDescriptor Out = FMegalithReflectionWalker::DescribeStruct(SMClass);
...
void FMegalithLogicDriverBulkFillAdapter::Register()
{
	// H5 invariant — Register() runs unconditionally.
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("logicdriver"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithLogicDriverBulkFillAdapter::LogicDriverBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithLogicDriverBulkFillAdapter::LogicDriverDescribe));
}

void FMegalithLogicDriverBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("logicdriver"));
}
