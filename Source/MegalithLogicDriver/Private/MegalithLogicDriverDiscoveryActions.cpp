...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDDiscovery, Log, All);

void FMegalithLogicDriverDiscoveryActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("get_sm_overview"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetSMOverview),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("validate_state_machine"),
		...
		FMegalithActionHandler::CreateStatic(&HandleValidateStateMachine),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("find_sm_references"),
		...
		FMegalithActionHandler::CreateStatic(&HandleFindSMReferences),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("visualize_sm_as_text"),
		...
		FMegalithActionHandler::CreateStatic(&HandleVisualizeSMAsText),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("explain_state_machine"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExplainStateMachine),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("find_node_class_usages"),
		...
		FMegalithActionHandler::CreateStatic(&HandleFindNodeClassUsages),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("node_bp_path"), TEXT("Node Blueprint asset path to search for"))
			.Build());

	UE_LOG(LogMegalithLDDiscovery, Log, TEXT("MegalithLogicDriver Discovery: registered 6 actions"));
}

FMegalithActionResult FMegalithLogicDriverDiscoveryActions::HandleGetSMOverview(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithLogicDriverDiscoveryActions::HandleValidateStateMachine(const TSharedPtr<FJsonObject>& Params)
{
	...
	for (UEdGraphNode* RawNode : RootGraph->Nodes)
	{
		...
		if (NodeType == TEXT("state") || NodeType == TEXT("state_machine"))
		{
			...
			Info.bIsEndState = MegalithLD::GetBoolProperty(RawNode, TEXT("bIsEndState"));
			...
			for (UEdGraphPin* Pin : RawNode->Pins)
			{
				if (Pin && Pin->Direction == EGPD_Input)
				{
					for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
					{
						if (LinkedPin && LinkedPin->GetOwningNode())
						{
							...
							FString SourceType = MegalithLD::GetNodeType(SourceNode);
				...
				else if (Pin && Pin->Direction == EGPD_Output)
				{
					for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
					{
						if (LinkedPin && LinkedPin->GetOwningNode())
						{
							FString TargetType = MegalithLD::GetNodeType(LinkedPin->GetOwningNode());
...
FMegalithActionResult FMegalithLogicDriverDiscoveryActions::HandleFindSMReferences(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'asset_path'"));
...
FMegalithActionResult FMegalithLogicDriverDiscoveryActions::HandleFindNodeClassUsages(const TSharedPtr<FJsonObject>& Params)
{
	FString NodeBPPath = Params->GetStringField(TEXT("node_bp_path"));
	if (NodeBPPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'node_bp_path'"));
	...
	if (!NodeBP)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load Node Blueprint at '%s'"), *NodeBPPath));
	}
	...
	UClass* SMBPClass = MegalithLD::GetSMBlueprintClass();
	if (!SMBPClass) return FMegalithActionResult::Error(TEXT("SMBlueprint class not found"));
	...
	for (const FAssetData& SMAsset : SMAssets)
	{
		...
		SMsScanned++;
		UEdGraph* RootGraph = MegalithLD::GetRootGraph(SMBP);
		...
		for (UEdGraphNode* Node : RootGraph->Nodes)
		{
			...
			if (bFound)
			{
				...
				Usage->SetStringField(TEXT("node_type"), MegalithLD::GetNodeType(Node));
...
FMegalithActionResult FMegalithLogicDriverDiscoveryActions::HandleVisualizeSMAsText(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString Format = Params->GetStringField(TEXT("format")).ToLower();
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'asset_path'"));
	if (Format.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'format'"));
	if (Format != TEXT("ascii") && Format != TEXT("mermaid") && Format != TEXT("dot"))
	{
		return FMegalithActionResult::Error(TEXT("format must be: ascii, mermaid, dot"));
	}
	...
	for (UEdGraphNode* RawNode : RootGraph->Nodes)
	{
			...
			|| NodeType == TEXT("state_machine") || NodeType == TEXT("any_state"))
		{
			...
			Info.Type = NodeType;
			Info.bIsEnd = MegalithLD::GetBoolProperty(RawNode, TEXT("bIsEndState"));
			...
			for (UEdGraphPin* Pin : RawNode->Pins)
			{
				if (Pin && Pin->Direction == EGPD_Input)
				{
					for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
					{
						if (LinkedPin && LinkedPin->GetOwningNode())
						{
							FString SourceType = MegalithLD::GetNodeType(LinkedPin->GetOwningNode());
							if (SourceType == TEXT("entry"))
							{
								Info.bIsInitial = true;
							}
...
FMegalithActionResult FMegalithLogicDriverDiscoveryActions::HandleExplainStateMachine(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'asset_path'"));
	...
	for (UEdGraphNode* Node : RootGraph->Nodes)
	{
			...
			|| NodeType == TEXT("state_machine") || NodeType == TEXT("any_state"))
		{
			...
			Data.bIsEnd = MegalithLD::GetBoolProperty(Node, TEXT("bIsEndState"));
