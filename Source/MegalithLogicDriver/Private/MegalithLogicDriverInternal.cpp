...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDInternal, Log, All);

namespace MegalithLD
{
...
static UClass* FindLDClass(const TCHAR* ClassName)
{
	...
	if (!Found)
	{
		UE_LOG(LogMegalithLDInternal, Warning, TEXT("Logic Driver class '%s' not found"), ClassName);
	}
	return Found;
}
