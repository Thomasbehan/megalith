...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDTextGraph, Log, All);
...
void FMegalithLogicDriverTextGraphActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("get_text_graph_content"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetTextGraphContent),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("get_dialogue_flow"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetDialogueFlow),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
			.Build());

	UE_LOG(LogMegalithLDTextGraph, Log, TEXT("MegalithLogicDriver TextGraph: registered 2 actions"));
}

FMegalithActionResult FMegalithLogicDriverTextGraphActions::HandleGetTextGraphContent(const TSharedPtr<FJsonObject>& Params)
{
	...
	auto ProcessNode = [&](UEdGraphNode* Node)
	{
		...
		Entry->SetStringField(TEXT("node_type"), MegalithLD::GetNodeType(Node));
...
FMegalithActionResult FMegalithLogicDriverTextGraphActions::HandleGetDialogueFlow(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'asset_path'"));
	...
	for (UEdGraphNode* RawNode : RootGraph->Nodes)
	{
		if (!RawNode || MegalithLD::GetNodeType(RawNode) != TEXT("transition")) continue;
