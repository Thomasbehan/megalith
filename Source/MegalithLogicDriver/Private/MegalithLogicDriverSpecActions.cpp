...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDSpec, Log, All);

void FMegalithLogicDriverSpecActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("export_sm_json"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExportSMJson),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("build_sm_from_spec"),
		...
		FMegalithActionHandler::CreateStatic(&HandleBuildSMFromSpec),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("export_sm_spec"),
		...
		FMegalithActionHandler::CreateStatic(&HandleExportSMSpec),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("SM Blueprint asset path"))
			.Build());
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("import_sm_json"),
		...
		FMegalithActionHandler::CreateStatic(&HandleImportSMJson),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("compare_state_machines"),
		...
		FMegalithActionHandler::CreateStatic(&HandleCompareStateMachines),
		FParamSchemaBuilder()
	...
	UE_LOG(LogMegalithLDSpec, Log, TEXT("MegalithLogicDriver Spec: registered 5 actions"));
}

FMegalithActionResult FMegalithLogicDriverSpecActions::HandleExportSMJson(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithLogicDriverSpecActions::HandleImportSMJson(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (JsonPathOrData.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'json_path_or_data'"));
	...
	if (JsonPathOrData.Contains(TEXT("/")) || JsonPathOrData.Contains(TEXT("\\")))
	{
		...
		{
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to read file: %s"), *JsonPathOrData));
		}
	}
	else
	{
	...
	{
		return FMegalithActionResult::Error(TEXT("Failed to parse JSON spec"));
	}
...
FMegalithActionResult FMegalithLogicDriverSpecActions::HandleBuildSMFromSpec(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'save_path'"));
	...
	{
		return FMegalithActionResult::Error(TEXT("Missing required param 'spec' (object)"));
	}
	...
	if (!FactoryClass)
	{
		return FMegalithActionResult::Error(TEXT("SMBlueprintFactory not found. Is Logic Driver Pro loaded?"));
	}

	UFactory* Factory = NewObject<UFactory>(GetTransientPackage(), FactoryClass);
	if (!Factory) return FMegalithActionResult::Error(TEXT("Failed to create SMBlueprintFactory instance"));

	// Derive asset name from save path
	FString AssetName = FPackageName::GetShortName(SavePath);

	UPackage* Package = CreatePackage(*SavePath);
	if (!Package) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to create package at: %s"), *SavePath));
	Package->FullyLoad();
	...
	if (!SupportedClass) SupportedClass = MegalithLD::GetSMBlueprintClass();

	UObject* NewAsset = Factory->FactoryCreateNew(
		SupportedClass, Package, FName(*AssetName),
		RF_Public | RF_Standalone, nullptr, GWarn);
	if (!NewAsset) return FMegalithActionResult::Error(TEXT("FactoryCreateNew returned null"));

	UBlueprint* SMBlueprint = Cast<UBlueprint>(NewAsset);
	if (!SMBlueprint) return FMegalithActionResult::Error(TEXT("Created asset is not a Blueprint"));

	Package->MarkPackageDirty();
	FAssetRegistryModule::AssetCreated(NewAsset);

	UEdGraph* RootGraph = MegalithLD::GetRootGraph(SMBlueprint);
	if (!RootGraph) return FMegalithActionResult::Error(TEXT("No root SM graph found in new Blueprint"));
	...
	if (!StateClass) return FMegalithActionResult::Error(TEXT("SMGraphNode_StateNode class not found"));

	// Find entry node
	UEdGraphNode* EntryNode = nullptr;
	for (UEdGraphNode* Node : RootGraph->Nodes)
	{
		if (Node && MegalithLD::GetNodeType(Node) == TEXT("entry"))
		{
			EntryNode = Node;
			break;
		}
	...
	if (Spec->HasField(TEXT("nested_sms")) && SMNodeClass)
	{
		...
		for (const TSharedPtr<FJsonValue>& NV : NestedSMs)
		{
			...
			if (NO->HasField(TEXT("sm_path")))
			{
				...
				FString LoadError;
				UBlueprint* RefBP = MegalithLD::LoadSMBlueprint(SMPath, LoadError);
	...
	bool bCompiled = MegalithLD::CompileSMBlueprint(SMBlueprint, CompileError);
	...
	{
		MegalithLD::SetNodeName(Pair.Value, Pair.Key);
	}
...
FMegalithActionResult FMegalithLogicDriverSpecActions::HandleExportSMSpec(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
		...
		if (NodeType == TEXT("state"))
		{
			...
			for (UEdGraphPin* Pin : RawNode->Pins)
			{
				if (Pin && Pin->Direction == EGPD_Input)
				{
					...
					{
						if (LP && LP->GetOwningNode() && MegalithLD::GetNodeType(LP->GetOwningNode()) == TEXT("entry"))
						{
							bIsInitial = true;
						}
					}
			...
			bool bIsEnd = MegalithLD::GetBoolProperty(RawNode, TEXT("bIsEndState"));
...
FMegalithActionResult FMegalithLogicDriverSpecActions::HandleCompareStateMachines(const TSharedPtr<FJsonObject>& Params)
{
	FString PathA = Params->GetStringField(TEXT("path_a"));
	FString PathB = Params->GetStringField(TEXT("path_b"));
	if (PathA.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'path_a'"));
	if (PathB.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'path_b'"));

	// Load both SMs
	FString LoadErrorA, LoadErrorB;
	UBlueprint* BPA = MegalithLD::LoadSMBlueprint(PathA, LoadErrorA);
	if (!BPA) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load A: %s"), *LoadErrorA));

	UBlueprint* BPB = MegalithLD::LoadSMBlueprint(PathB, LoadErrorB);
	if (!BPB) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load B: %s"), *LoadErrorB));

	UEdGraph* GraphA = MegalithLD::GetRootGraph(BPA);
	UEdGraph* GraphB = MegalithLD::GetRootGraph(BPB);
	if (!GraphA) return FMegalithActionResult::Error(TEXT("No root graph in SM A"));
	if (!GraphB) return FMegalithActionResult::Error(TEXT("No root graph in SM B"));
