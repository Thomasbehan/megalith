...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithLDRuntime, Log, All);
...
namespace
{
	...
	FPIELookupResult FindSMInstanceInPIE(const TSharedPtr<FJsonObject>& Params)
	{
		...
		{
			Result.Error = FMegalithActionResult::Error(TEXT("Missing required param 'actor'"));
			return Result;
		}
		...
		{
			Result.Error = FMegalithActionResult::Error(TEXT("PIE not running — start Play-In-Editor first"));
			return Result;
		}
		...
		if (!Result.Actor)
		{
			Result.Error = FMegalithActionResult::Error(FString::Printf(TEXT("Actor '%s' not found in PIE world"), *ActorName));
			return Result;
		}

		// Find SM component
		UClass* SMCompClass = MegalithLD::GetSMComponentClass();
		if (!SMCompClass)
		{
			Result.Error = FMegalithActionResult::Error(TEXT("SMStateMachineComponent class not found"));
			return Result;
		}
		...
		if (!Result.Component)
		{
			Result.Error = FMegalithActionResult::Error(FString::Printf(
		...
		Result.SMInstance = MegalithLD::GetObjectProperty(Result.Component, TEXT("Instance"));
		if (!Result.SMInstance)
		{
			Result.SMInstance = MegalithLD::GetObjectProperty(Result.Component, TEXT("StateMachineInstance"));
		}
		if (!Result.SMInstance)
		{
			Result.Error = FMegalithActionResult::Error(TEXT("SM component has no active SM instance (is the SM started?)"));
			return Result;
		}
...
void FMegalithLogicDriverRuntimeActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("runtime_get_sm_state"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeGetSMState),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("runtime_start_sm"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeStartSM),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label or name in PIE world"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("runtime_stop_sm"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeStopSM),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label or name in PIE world"))
			.Optional(TEXT("component_name"), TEXT("string"), TEXT("SM component name"))
			.Build());
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("runtime_restart_sm"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeRestartSM),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label or name in PIE world"))
			.Optional(TEXT("component_name"), TEXT("string"), TEXT("SM component name"))
			.Build());
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("runtime_switch_state"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeSwitchState),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label or name in PIE world"))
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("runtime_evaluate_transitions"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeEvaluateTransitions),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label or name in PIE world"))
			.Optional(TEXT("component_name"), TEXT("string"), TEXT("SM component name"))
			.Build());
	...
	Registry.RegisterAction(TEXT("logicdriver"), TEXT("runtime_get_state_history"),
		...
		FMegalithActionHandler::CreateStatic(&HandleRuntimeGetStateHistory),
		FParamSchemaBuilder()
			.Required(TEXT("actor"), TEXT("string"), TEXT("Actor label or name in PIE world"))
			.Optional(TEXT("component_name"), TEXT("string"), TEXT("SM component name"))
	...
	UE_LOG(LogMegalithLDRuntime, Log, TEXT("MegalithLogicDriver Runtime: registered 7 actions"));
}

// ── runtime_get_sm_state ────────────────────────────────────────────

FMegalithActionResult FMegalithLogicDriverRuntimeActions::HandleRuntimeGetSMState(const TSharedPtr<FJsonObject>& Params)
{
	...
	FString ActiveStateName = MegalithLD::GetStringProperty(SMInst, TEXT("ActiveStateName"));
	if (ActiveStateName.IsEmpty())
	{
		ActiveStateName = MegalithLD::GetStringProperty(SMInst, TEXT("CurrentStateName"));
	}

	float TimeInState = MegalithLD::GetFloatProperty(SMInst, TEXT("TimeInState"));
	bool bIsActive = MegalithLD::GetBoolProperty(SMInst, TEXT("bIsActive"));
	if (!bIsActive)
	{
		// Try alternative property name
		bIsActive = MegalithLD::GetBoolProperty(SMInst, TEXT("bHasStarted"));
	}
...
FMegalithActionResult FMegalithLogicDriverRuntimeActions::HandleRuntimeStartSM(const TSharedPtr<FJsonObject>& Params)
{
	FPIELookupResult Lookup = FindSMInstanceInPIE(Params);
...
FMegalithActionResult FMegalithLogicDriverRuntimeActions::HandleRuntimeStopSM(const TSharedPtr<FJsonObject>& Params)
{
	FPIELookupResult Lookup = FindSMInstanceInPIE(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
...
FMegalithActionResult FMegalithLogicDriverRuntimeActions::HandleRuntimeRestartSM(const TSharedPtr<FJsonObject>& Params)
{
	FPIELookupResult Lookup = FindSMInstanceInPIE(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
...
FMegalithActionResult FMegalithLogicDriverRuntimeActions::HandleRuntimeSwitchState(const TSharedPtr<FJsonObject>& Params)
{
	FPIELookupResult Lookup = FindSMInstanceInPIE(Params);
	if (!Lookup.bSuccess) return Lookup.Error;

	FString StateGuidStr = Params->GetStringField(TEXT("state_guid"));
	if (StateGuidStr.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required param 'state_guid'"));

	FGuid StateGuid;
	if (!FGuid::Parse(StateGuidStr, StateGuid))
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Invalid GUID: '%s'"), *StateGuidStr));
	}
...
FMegalithActionResult FMegalithLogicDriverRuntimeActions::HandleRuntimeEvaluateTransitions(const TSharedPtr<FJsonObject>& Params)
{
	FPIELookupResult Lookup = FindSMInstanceInPIE(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
...
FMegalithActionResult FMegalithLogicDriverRuntimeActions::HandleRuntimeGetStateHistory(const TSharedPtr<FJsonObject>& Params)
{
	FPIELookupResult Lookup = FindSMInstanceInPIE(Params);
	if (!Lookup.bSuccess) return Lookup.Error;
	...
	bool bIsActive = MegalithLD::GetBoolProperty(Lookup.SMInstance, TEXT("bIsActive"));
