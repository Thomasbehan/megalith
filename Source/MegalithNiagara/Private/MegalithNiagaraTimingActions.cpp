...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithNiagaraTiming, Log, All);

// ============================================================================
//  Local file-static helpers (mirror MegalithNiagaraActions.cpp:686-705 pattern)
// ============================================================================

namespace MegalithNiagaraTimingLocal
{
	static FMegalithActionResult SuccessStr(const FString& Msg)
	{
	...
	static FMegalithActionResult SuccessObj(const TSharedRef<FJsonObject>& Obj)
	{
	...
	static UNiagaraSystem* LoadSystem(const FString& SystemPath)
	{
		...
		if (!System)
		{
			UE_LOG(LogMegalithNiagaraTiming, Error, TEXT("Failed to load Niagara system: %s"), *SystemPath);
		}
		return System;
	}
}

using namespace MegalithNiagaraTimingLocal;

// ============================================================================
//  Registration
// ============================================================================

void FMegalithNiagaraTimingActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_system_timing"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetSystemTiming),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_warmup_profile"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetWarmupProfile),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_fixed_tick_delta"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetFixedTickDelta),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_require_current_frame_data"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetRequireCurrentFrameData),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_emitter_loop_profile"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetEmitterLoopProfile),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_emitter_timing_summary"),
		...
		FMegalithActionHandler::CreateStatic(&HandleGetEmitterTimingSummary),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_sim_stage_iteration_count"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetSimStageIterationCount),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_sim_stage_execute_behavior"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetSimStageExecuteBehavior),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter handle id/name (same convention as set_simulation_stage_property)"))
			.Optional(TEXT("stage_index"), TEXT("integer"), TEXT("0-based index into the emitter's simulation stages (use this OR stage_name)"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_particle_lifetime"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetParticleLifetime),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset"))
...
FMegalithActionResult FMegalithNiagaraTimingActions::HandleGetSystemTiming(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithNiagaraTimingActions::HandleSetWarmupProfile(const TSharedPtr<FJsonObject>& Params)
{
	const FString SystemPath = GetAssetPath(Params);
	...
	if (!WarmupTimeJV.IsValid() || WarmupTimeJV->Type != EJson::Number)
		return FMegalithActionResult::Error(TEXT("Missing required field: warmup_time (number)"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetWarmupProfile", "Set Niagara Warmup Profile"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraTimingActions::HandleSetFixedTickDelta(const TSharedPtr<FJsonObject>& Params)
{
	const FString SystemPath = GetAssetPath(Params);
	...
	if (!EnabledJV.IsValid() || EnabledJV->Type != EJson::Boolean)
		return FMegalithActionResult::Error(TEXT("Missing required field: enabled (bool)"));
	...
	if (!EnabledProp)
		return FMegalithActionResult::Error(TEXT("UPROPERTY 'bFixedTickDelta' not found on UNiagaraSystem"));
	...
	if (bHasDeltaTime)
	{
		...
		if (!DeltaProp)
			return FMegalithActionResult::Error(TEXT("UPROPERTY 'FixedTickDeltaTime' not found on UNiagaraSystem"));
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetFixedTickDelta", "Set Niagara Fixed Tick Delta"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraTimingActions::HandleSetRequireCurrentFrameData(const TSharedPtr<FJsonObject>& Params)
{
	const FString SystemPath = GetAssetPath(Params);

	TSharedPtr<FJsonValue> RequireJV = Params->TryGetField(TEXT("require"));
	if (!RequireJV.IsValid() || RequireJV->Type != EJson::Boolean)
		return FMegalithActionResult::Error(TEXT("Missing required field: require (bool)"));
	...
	if (!Prop)
		return FMegalithActionResult::Error(TEXT("UPROPERTY 'bRequireCurrentFrameData' not found on UNiagaraSystem"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetRequireCurrentFrameData", "Set Niagara Require Current Frame Data"));
	System->Modify();
...
// MegalithNiagaraActions.cpp:3506 accepts module_name="EmitterState" through
// FindModuleNode's name-fallback (MegalithNiagaraActions.cpp:1481 —
...
namespace MegalithNiagaraTimingLocal
{
	...
	static FMegalithActionResult DispatchModuleInput(
	...
	{
		if (!Value.IsValid())
			return FMegalithActionResult::Error(TEXT("Internal: module-input alias passed null value"));
		...
		return FMegalithToolRegistry::Get().ExecuteAction(
		...
		const FString& InputName)
	{
		...
		FMegalithActionResult R = FMegalithToolRegistry::Get().ExecuteAction(
	...
	static FMegalithActionResult DispatchModuleSwitch(
		const FString& SystemPath,
		const FString& Emitter,
		const FString& ModuleName,
	...
	// HandleGetModuleInputValue (MegalithNiagaraActions.cpp:6991-7022) writes
	...
	// MegalithNiagara include path). Access strategy:
	...
	static FMegalithActionResult WriteStatelessLoopProfile(
		UObject* StatelessEmitter,
		const TSharedPtr<FJsonObject>& Params,
		TArray<TSharedPtr<FJsonValue>>& OutWarnings)
	{
		if (!StatelessEmitter)
			return FMegalithActionResult::Error(TEXT("WriteStatelessLoopProfile: null stateless emitter"));
		...
		return FMegalithActionResult::Success(Resp);
	}
...
FMegalithActionResult FMegalithNiagaraTimingActions::HandleSetEmitterLoopProfile(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithNiagaraTimingLocal;

	const FString SystemPath = GetAssetPath(Params);
	if (SystemPath.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));
	...
	if (!bAnyField)
		return FMegalithActionResult::Error(TEXT("Must supply at least one of: loop_behavior, loop_duration_mode, loop_duration, loop_delay, loop_count, loop_delay_enabled"));
	...
	if (Emitter.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required field: emitter (string) — required when asset_path is a UNiagaraSystem"));
	...
	// FMegalithNiagaraActions::FindEmitterHandleIndex (private static), so we
	...
	auto AppendWriteFailure = [&Warnings](const FString& Field, const FMegalithActionResult& R)
	{
		Warnings.Add(MakeShared<FJsonValueString>(FString::Printf(
	...
	if (!BehaviorCanonical.IsEmpty())
	{
		...
		FMegalithActionResult R = DispatchModuleSwitch(
	...
	if (DurationJV.IsValid() && DurationJV->Type == EJson::Number)
	{
		...
		FMegalithActionResult R = DispatchModuleInput(
...
FMegalithActionResult FMegalithNiagaraTimingActions::HandleGetEmitterTimingSummary(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithNiagaraTimingLocal;
...
// (MegalithNiagaraActions.cpp:11495-11652). We dispatch through the public
// registry rather than calling FMegalithNiagaraActions::HandleSetSimulationStageProperty
...
namespace MegalithNiagaraTimingLocal
{
	...
	// then dispatch via FMegalithToolRegistry::Get().ExecuteAction.
	static FMegalithActionResult DispatchSimStageAlias(
		const TSharedPtr<FJsonObject>& Params,
		...
		const TSharedPtr<FJsonValue>& Value)
	{
		...
		if (!Params->TryGetStringField(TEXT("emitter"), Emitter) || Emitter.IsEmpty())
			return FMegalithActionResult::Error(TEXT("Missing required field: emitter (string)"));
		...
		if (!Value.IsValid())
			return FMegalithActionResult::Error(TEXT("Internal: alias passed null value"));
...
FMegalithActionResult FMegalithNiagaraTimingActions::HandleSetSimStageIterationCount(const TSharedPtr<FJsonObject>& Params)
{
	...
	// MegalithNiagaraActions.cpp:11600-11635. The fallback stringifies a JSON Number
	...
	if (!IterJV.IsValid() || IterJV->Type != EJson::Number)
		return FMegalithActionResult::Error(TEXT("Missing required field: iterations (integer)"));

	const int32 Iterations = static_cast<int32>(IterJV->AsNumber());
	if (Iterations < 0)
		return FMegalithActionResult::Error(TEXT("iterations must be >= 0"));

	TSharedPtr<FJsonValue> Value = MakeShared<FJsonValueString>(FString::Printf(TEXT("(Value=%d)"), Iterations));
	return MegalithNiagaraTimingLocal::DispatchSimStageAlias(Params, TEXT("NumIterations"), Value);
}

FMegalithActionResult FMegalithNiagaraTimingActions::HandleSetSimStageExecuteBehavior(const TSharedPtr<FJsonObject>& Params)
{
	// behavior: string (required) — one of Always / OnSimulationReset / NotOnSimulationReset.
	// The canonical handler at MegalithNiagaraActions.cpp:11572-11578 accepts both
	...
	if (!Params->TryGetStringField(TEXT("behavior"), Behavior) || Behavior.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required field: behavior (string: 'Always' | 'OnSimulationReset' | 'NotOnSimulationReset')"));

	TSharedPtr<FJsonValue> Value = MakeShared<FJsonValueString>(Behavior);
	return MegalithNiagaraTimingLocal::DispatchSimStageAlias(Params, TEXT("ExecuteBehavior"), Value);
}

FMegalithActionResult FMegalithNiagaraTimingActions::HandleSetParticleLifetime(const TSharedPtr<FJsonObject>& Params)
{
	using namespace MegalithNiagaraTimingLocal;
	...
	if (!MinJV.IsValid() || MinJV->Type != EJson::Number)
		return FMegalithActionResult::Error(TEXT("Missing required field: min (number)"));
	...
	FMegalithActionResult ModeResult = DispatchModuleSwitch(
