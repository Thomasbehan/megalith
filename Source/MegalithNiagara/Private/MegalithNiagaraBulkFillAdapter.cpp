...
// MegalithNiagaraBulkFillAdapter — Phase 5 Step 2 adapter.
...
#define LOCTEXT_NAMESPACE "MegalithNiagaraBulkFillAdapter"

namespace MegalithNiagaraBulkFillInternal
{
	...
	{
		return FMegalithAssetUtils::LoadAssetByPath(AssetPath);
	}
	...
	static FDryRunReport HandleDataInterfaceArray(const FBulkFillSpec& Spec)
	{
		...
		if (Spec.bDryRun)
		{
			Report = FMegalithReflectionWalker::InspectTree(Root, Asset->GetClass(), Asset, Spec);
			Report.bWouldApply = false;
			return Report;
		}

		Asset->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("NiagaraBulkFill_DIA", "Megalith Niagara Bulk Fill — DI Array"));
		Asset->Modify();
		Asset->PreEditChange(nullptr);

		Report = FMegalithReflectionWalker::WriteTree(Root, Asset->GetClass(), Asset, Asset, Spec);
	...
	static FDryRunReport HandleCurve(const FBulkFillSpec& Spec)
	{
		...
		Asset->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("NiagaraBulkFill_Curve", "Megalith Niagara Bulk Fill — Curve"));
		Asset->Modify();
		Asset->PreEditChange(nullptr);
	...
	static FDryRunReport HandleParameterOverrides(const FBulkFillSpec& Spec)
	{
		...
		if (Spec.bDryRun)
		{
			Report = FMegalithReflectionWalker::InspectTree(ParamsObj, Asset->GetClass(), Asset, Spec);
			Report.bWouldApply = false;
			return Report;
		}

		Asset->SetFlags(RF_Transactional);
		FScopedTransaction Transaction(LOCTEXT("NiagaraBulkFill_Params", "Megalith Niagara Bulk Fill — Parameter Overrides"));
		Asset->Modify();
		Asset->PreEditChange(nullptr);

		Report = FMegalithReflectionWalker::WriteTree(ParamsObj, Asset->GetClass(), Asset, Asset, Spec);
		if (Spec.bStrict && Report.Errors > 0)
		{
			Transaction.Cancel();
			Report.bWouldApply = false;
...
FDryRunReport FMegalithNiagaraBulkFillAdapter::NiagaraBulkFill(const FBulkFillSpec& Spec)
{
	using namespace MegalithNiagaraBulkFillInternal;
...
FSchemaDescriptor FMegalithNiagaraBulkFillAdapter::NiagaraDescribe(const FString& TargetAsset)
{
	using namespace MegalithNiagaraBulkFillInternal;
	...
	FSchemaDescriptor Out = FMegalithReflectionWalker::DescribeStruct(Asset->GetClass());
	Out.FieldPath = TargetAsset;
	return Out;
}

void FMegalithNiagaraBulkFillAdapter::Register()
{
	FMegalithBulkFillRegistry::Get().RegisterAdapter(
		TEXT("niagara"),
		FMegalithBulkFillRegistry::FBulkFillAdapter(&FMegalithNiagaraBulkFillAdapter::NiagaraBulkFill),
		FMegalithBulkFillRegistry::FDescribeAdapter(&FMegalithNiagaraBulkFillAdapter::NiagaraDescribe));
}

void FMegalithNiagaraBulkFillAdapter::Unregister()
{
	FMegalithBulkFillRegistry::Get().UnregisterAdapter(TEXT("niagara"));
}
