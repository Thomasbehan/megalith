...
#include "MegalithPackagePathValidator.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogMegalithNiagara, Log, All);
...
// widens PrivateIncludePaths into NiagaraEditor/Private (see MegalithNiagara.Build.cs). The
...
// (see MegalithNiagara.Build.cs, gated on WITH_NIAGARA_WIZARD_PRIVATE) and forward-declaring the
...
// WITH_NIAGARA_WIZARD_PRIVATE off (release builds already do via MEGALITH_RELEASE_BUILD) to fall
...
namespace MegalithNiagaraHelpers
{
...
// Helper: wrap a string result in a FJsonObject for FMegalithActionResult::Success
static FMegalithActionResult NA_SuccessStr(const FString& Msg)
{
...
static FMegalithActionResult NA_SuccessObj(const TSharedRef<FJsonObject>& Obj)
{
	return FMegalithActionResult::Success(Obj);
}
...
namespace
{
	enum class EMegalithSemanticDetailLevel
	{
		Compact,
		Full
	};
	...
	static bool TryParseSemanticDetailLevel(
		...
		EMegalithSemanticDetailLevel& OutDetailLevel,
		FString& OutError)
	{
		...
		{
			OutDetailLevel = EMegalithSemanticDetailLevel::Compact;
			return true;
		}

		if (DetailLevel == TEXT("full"))
		{
			OutDetailLevel = EMegalithSemanticDetailLevel::Full;
			return true;
		}
	...
	struct FMegalithNiagaraStageModule
	{
		FString StageName;
		FString ModuleName;
		FString ModuleGuid;
	...
	struct FMegalithNiagaraEventGeneratorInfo
	{
		FString EventName;
		FString ModuleName;
		FString ModuleGuid;
	...
	struct FMegalithNiagaraTopologyEdge
	{
	...
	struct FMegalithNiagaraEmitterSemantic
	{
		FString Name;
		FString Guid;
		...
		FString RoleHint;
		TArray<FMegalithNiagaraStageModule> LocationModules;
		TArray<FMegalithNiagaraEventGeneratorInfo> EventGenerators;
		TArray<FMegalithNiagaraTopologyEdge> IncomingEvents;
		TArray<FMegalithNiagaraTopologyEdge> OutgoingLinks;
	...
	static void CollectEmitterModules(UNiagaraSystem* System, const FNiagaraEmitterHandle& Handle, TArray<FMegalithNiagaraStageModule>& OutModules)
	{
		OutModules.Reset();
		...
		for (const auto& [Usage, StageName] : StageUsages)
		{
			...
			MegalithNiagaraHelpers::GetOrderedModuleNodes(*OutputNode, Modules);
			for (UNiagaraNodeFunctionCall* ModuleNode : Modules)
			{
				...
				FMegalithNiagaraStageModule Module;
	...
	static void CollectTopologyEdges(UNiagaraSystem* System, TArray<FMegalithNiagaraTopologyEdge>& OutEdges)
	{
		OutEdges.Reset();
		if (!System)
		{
		...
		for (const FNiagaraEmitterHandle& TargetHandle : Handles)
		{
			...
			{
				FMegalithNiagaraTopologyEdge Edge;
	...
	static FMegalithNiagaraEmitterSemantic AnalyzeEmitterSemantic(
		UNiagaraSystem* System,
		const FNiagaraEmitterHandle& Handle,
		const int32 EmitterIndex,
		const TArray<FMegalithNiagaraTopologyEdge>& TopologyEdges)
	{
		FMegalithNiagaraEmitterSemantic Semantic;
		...
		TArray<FMegalithNiagaraStageModule> Modules;
		CollectEmitterModules(System, Handle, Modules);
		for (const FMegalithNiagaraStageModule& Module : Modules)
		{
			FString EventName;
			if (TryGetGeneratedEventName(Module.ModuleName, EventName))
			{
				FMegalithNiagaraEventGeneratorInfo Generator;
		...
		for (const FMegalithNiagaraTopologyEdge& Edge : TopologyEdges)
		{
		...
		for (const FMegalithNiagaraTopologyEdge& Edge : Semantic.IncomingEvents)
		{
		...
		for (const FMegalithNiagaraEventGeneratorInfo& Generator : Semantic.EventGenerators)
		{
	...
	static TSharedRef<FJsonObject> MakeStageModuleJson(const FMegalithNiagaraStageModule& Module)
	{
	...
	static TSharedRef<FJsonObject> MakeEventGeneratorJson(const FMegalithNiagaraEventGeneratorInfo& Generator)
	{
		TSharedRef<FJsonObject> Obj = MakeShared<FJsonObject>();
	...
	static TSharedRef<FJsonObject> MakeTopologyEdgeJson(
		const FMegalithNiagaraTopologyEdge& Edge,
		const EMegalithSemanticDetailLevel DetailLevel)
	{
		const bool bFull = DetailLevel == EMegalithSemanticDetailLevel::Full;
		TSharedRef<FJsonObject> Obj = MakeShared<FJsonObject>();
	...
	static void AppendEmitterSemanticJson(
		...
		const FMegalithNiagaraEmitterSemantic& Semantic,
		const EMegalithSemanticDetailLevel DetailLevel)
	{
		const bool bFull = DetailLevel == EMegalithSemanticDetailLevel::Full;
		...
		for (const FMegalithNiagaraTopologyEdge& Incoming : Semantic.IncomingEvents)
		{
		...
		for (const FMegalithNiagaraTopologyEdge& Outgoing : Semantic.OutgoingLinks)
		{
		...
		for (const FMegalithNiagaraStageModule& Module : Semantic.LocationModules)
		{
...
FString FMegalithNiagaraActions::JsonObjectToString(const TSharedRef<FJsonObject>& JsonObj)
{
	FString Out;
...
FString FMegalithNiagaraActions::JsonArrayToString(const TArray<TSharedPtr<FJsonValue>>& JsonArray)
{
	FString Out;
	TSharedRef<TJsonWriter<>> W = TJsonWriterFactory<>::Create(&Out);
...
FString FMegalithNiagaraActions::JsonValueToString(const TSharedPtr<FJsonValue>& Value)
{
...
UEnum* FMegalithNiagaraActions::TryGetStaticSwitchEnum(UEdGraphPin* SwitchPin, UNiagaraNodeFunctionCall* ModuleNode)
{
...
bool FMegalithNiagaraActions::ResolveStaticSwitchEnumValue(UEnum* Enum, const FString& RequestedValue, FString& OutRawValue, FString* OutDisplayValue)
{
...
void FMegalithNiagaraActions::AddStaticSwitchEnumMetadata(TSharedRef<FJsonObject> JsonObj, UEnum* Enum, const FString& RawValue)
{
	if (!Enum) return;
...
UNiagaraSystem* FMegalithNiagaraActions::LoadSystem(const FString& SystemPath)
{
	UNiagaraSystem* System = FMegalithAssetUtils::LoadAssetByPath<UNiagaraSystem>(SystemPath);
	if (!System)
	{
		UE_LOG(LogMegalithNiagara, Error, TEXT("Failed to load Niagara system: %s"), *SystemPath);
	}
	return System;
}

int32 FMegalithNiagaraActions::FindEmitterHandleIndex(UNiagaraSystem* System, const FString& HandleIdOrName)
{
...
bool FMegalithNiagaraActions::ResolveScriptUsage(const FString& UsageString, ENiagaraScriptUsage& OutUsage)
{
...
bool FMegalithNiagaraActions::IsSimulationStageUsageString(const FString& UsageString)
{
...
bool FMegalithNiagaraActions::IsParticleEventUsageString(const FString& UsageString)
{
	ENiagaraScriptUsage Usage;
	return ResolveScriptUsage(UsageString, Usage)
...
bool FMegalithNiagaraActions::ResolveSimulationStageSelector(UNiagaraSystem* System, const FString& EmitterHandleId,
...
bool FMegalithNiagaraActions::ResolveEventHandlerSelector(UNiagaraSystem* System, const FString& EmitterHandleId,
...
FString FMegalithNiagaraActions::UsageToString(ENiagaraScriptUsage Usage)
{
	switch (Usage)
	{
...
UNiagaraGraph* FMegalithNiagaraActions::GetGraphForUsage(UNiagaraSystem* System, const FString& EmitterHandleId, ENiagaraScriptUsage Usage)
{
...
UNiagaraNodeOutput* FMegalithNiagaraActions::FindOutputNode(UNiagaraSystem* System, const FString& EmitterHandleId,
...
UNiagaraNodeFunctionCall* FMegalithNiagaraActions::FindModuleNode(UNiagaraSystem* System, const FString& EmitterHandleId,
	const FString& NodeGuidStr, ENiagaraScriptUsage* OutUsage, FGuid* OutUsageId)
{
	...
	for (ENiagaraScriptUsage Usage : AllUsages)
	{
		...
		MegalithNiagaraHelpers::GetOrderedModuleNodes(*Out, Mods);
...
UClass* FMegalithNiagaraActions::ResolveRendererClass(const FString& RendererClass)
{
...
UNiagaraRendererProperties* FMegalithNiagaraActions::GetRenderer(UNiagaraSystem* System, const FString& EmitterHandleId,
...
FNiagaraTypeDefinition FMegalithNiagaraActions::ResolveNiagaraType(const FString& TypeName, bool* bOutFellBack)
{
	...
	DICandidate.ReplaceInline(TEXT("-"), TEXT(""), ESearchCase::CaseSensitive);
	if (UClass* DIClass = MegalithNiagaraHelpers::ResolveNiagaraDataInterfaceClass(DICandidate))
	{
		return FNiagaraTypeDefinition(DIClass);
	}

	UE_LOG(LogMegalithNiagara, Warning, TEXT("ResolveNiagaraType: Unknown type '%s', defaulting to float"), *TypeName);
	if (bOutFellBack) *bOutFellBack = true;
	return FNiagaraTypeDefinition::GetFloatDef();
}

FString FMegalithNiagaraActions::SerializeParameterValue(const FNiagaraVariable& Variable, const FNiagaraParameterStore& Store)
{
...
FNiagaraVariable FMegalithNiagaraActions::MakeUserVariable(const FString& ParamName, const FNiagaraTypeDefinition& TypeDef)
{
	...
	TArray<TSharedPtr<FJsonValue>>& OutArray)
{
	...
	for (const FNiagaraVariableWithOffset& VWO : Variables)
	{
		...
		P->SetStringField(TEXT("value"), FMegalithNiagaraActions::SerializeParameterValue(Var, Store));
...
void FMegalithNiagaraActions::RegisterActions(FMegalithToolRegistry& Registry)
{
	// System (8)
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_emitter"), TEXT("Add an emitter to a Niagara system"),
		FMegalithActionHandler::CreateStatic(&HandleAddEmitter),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_emitter"), TEXT("Remove an emitter from a Niagara system"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveEmitter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("duplicate_emitter"), TEXT("Duplicate an emitter within a Niagara system"),
		FMegalithActionHandler::CreateStatic(&HandleDuplicateEmitter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_emitter_enabled"), TEXT("Enable or disable an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleSetEmitterEnabled),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("reorder_emitters"), TEXT("Reorder emitters in a system"),
		FMegalithActionHandler::CreateStatic(&HandleReorderEmitters),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_emitter_property"), TEXT("Set an emitter property"),
		FMegalithActionHandler::CreateStatic(&HandleSetEmitterProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("request_compile"), TEXT("Request compilation of a Niagara system"),
		FMegalithActionHandler::CreateStatic(&HandleRequestCompile),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_system"), TEXT("Create a new Niagara system"),
		FMegalithActionHandler::CreateStatic(&HandleCreateSystem),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_stateless_emitter"), TEXT("**Phase 0 stub.** Create a standalone UNiagaraStatelessEmitter (Lightweight Emitter) asset. Not yet implemented."),
		FMegalithActionHandler::CreateStatic(&HandleCreateStatelessEmitter),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_ordered_modules"), TEXT("Get ordered modules in a script stage"),
		FMegalithActionHandler::CreateStatic(&HandleGetOrderedModules),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_module_inputs"), TEXT("Get inputs for a module node"),
		FMegalithActionHandler::CreateStatic(&HandleGetModuleInputs),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_module_graph"), TEXT("Get the node graph of a module script"),
		FMegalithActionHandler::CreateStatic(&HandleGetModuleGraph),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_custom_hlsl_text"), TEXT("Read the Custom HLSL source text from a Niagara script's CustomHlsl node"),
		FMegalithActionHandler::CreateStatic(&HandleGetCustomHLSLText),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_custom_hlsl_text"), TEXT("Overwrite the Custom HLSL source text on a Niagara script's CustomHlsl node"),
		FMegalithActionHandler::CreateStatic(&HandleSetCustomHLSLText),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("script_path"), TEXT("Niagara script asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_module"), TEXT("Add a module to a script stage"),
		FMegalithActionHandler::CreateStatic(&HandleAddModule),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_module"), TEXT("Remove a module from a script stage"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveModule),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("move_module"), TEXT("Move a module to a new index"),
		FMegalithActionHandler::CreateStatic(&HandleMoveModule),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_module_enabled"), TEXT("Enable or disable a module"),
		FMegalithActionHandler::CreateStatic(&HandleSetModuleEnabled),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_module_input_value"), TEXT("Set a module input value"),
		FMegalithActionHandler::CreateStatic(&HandleSetModuleInputValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_module_input_binding"), TEXT("Bind a module input to a parameter"),
		FMegalithActionHandler::CreateStatic(&HandleSetModuleInputBinding),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_module_input_di"), TEXT("Set a data interface on a module input"),
		FMegalithActionHandler::CreateStatic(&HandleSetModuleInputDI),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_module_from_hlsl"), TEXT("Create a Niagara module script from custom HLSL"),
		FMegalithActionHandler::CreateStatic(&HandleCreateModuleFromHLSL),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_function_from_hlsl"), TEXT("Create a Niagara function script from custom HLSL"),
		FMegalithActionHandler::CreateStatic(&HandleCreateFunctionFromHLSL),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_all_parameters"), TEXT("Get all parameters in a system"),
		FMegalithActionHandler::CreateStatic(&HandleGetAllParameters),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_user_parameters"), TEXT("Get user-exposed parameters"),
		FMegalithActionHandler::CreateStatic(&HandleGetUserParameters),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_parameter_value"), TEXT("Get a parameter value"),
		FMegalithActionHandler::CreateStatic(&HandleGetParameterValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_parameter_type"), TEXT("Get info about a Niagara type"),
		FMegalithActionHandler::CreateStatic(&HandleGetParameterType),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("trace_parameter_binding"), TEXT("Trace where a parameter is used"),
		FMegalithActionHandler::CreateStatic(&HandleTraceParameterBinding),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_user_parameter"), TEXT("Add a user parameter"),
		FMegalithActionHandler::CreateStatic(&HandleAddUserParameter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_user_parameter"), TEXT("Remove a user parameter"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveUserParameter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_parameter_default"), TEXT("Set a parameter default value"),
		FMegalithActionHandler::CreateStatic(&HandleSetParameterDefault),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("parameter"), TEXT("string"), TEXT("Parameter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_curve_value"), TEXT("Set curve keys on a module input"),
		FMegalithActionHandler::CreateStatic(&HandleSetCurveValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_renderer"), TEXT("Add a renderer to an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleAddRenderer),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_renderer"), TEXT("Remove a renderer from an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveRenderer),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_renderer_material"), TEXT("Set renderer material"),
		FMegalithActionHandler::CreateStatic(&HandleSetRendererMaterial),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_renderer_property"), TEXT("Set a renderer property"),
		FMegalithActionHandler::CreateStatic(&HandleSetRendererProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("renderer_index"), TEXT("integer"), TEXT("Renderer index"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_renderer_bindings"), TEXT("Get renderer attribute bindings"),
		FMegalithActionHandler::CreateStatic(&HandleGetRendererBindings),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("renderer_index"), TEXT("integer"), TEXT("Renderer index"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_renderer_binding"), TEXT("Set a renderer attribute binding"),
		FMegalithActionHandler::CreateStatic(&HandleSetRendererBinding),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("renderer_index"), TEXT("integer"), TEXT("Renderer index"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_emitters"), TEXT("List all emitters in a Niagara system"),
		FMegalithActionHandler::CreateStatic(&HandleListEmitters),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_renderers"), TEXT("List all renderers on a specific emitter"),
		FMegalithActionHandler::CreateStatic(&HandleListRenderers),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Build());
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_module_scripts"), TEXT("Search available Niagara module scripts by keyword"),
		FMegalithActionHandler::CreateStatic(&HandleListModuleScripts),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_renderer_properties"), TEXT("List editable properties on a renderer"),
		FMegalithActionHandler::CreateStatic(&HandleListRendererProperties),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("batch_execute"), TEXT("Execute multiple operations in one transaction"),
		FMegalithActionHandler::CreateStatic(&HandleBatchExecute),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_system_from_spec"), TEXT("Create a full system from JSON spec"),
		FMegalithActionHandler::CreateStatic(&HandleCreateSystemFromSpec),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_di_functions"), TEXT("Get data interface function signatures"),
		FMegalithActionHandler::CreateStatic(&HandleGetDIFunctions),
		FParamSchemaBuilder()
			.Required(TEXT("di_class"), TEXT("string"), TEXT("Data interface class name"))
			.Build());
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_compiled_gpu_hlsl"), TEXT("Get compiled GPU HLSL for an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleGetCompiledGPUHLSL),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Build());
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_system_diagnostics"), TEXT("Get compile errors, warnings, renderer issues, and script stats"),
		FMegalithActionHandler::CreateStatic(&HandleGetSystemDiagnostics),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_system_property"), TEXT("Read a system-level property (WarmupTime, bDeterminism, RandomSeed, etc.)"),
		FMegalithActionHandler::CreateStatic(&HandleGetSystemProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_system_property"), TEXT("Set a system-level property (WarmupTime, bDeterminism, etc.)"),
		FMegalithActionHandler::CreateStatic(&HandleSetSystemProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_static_switch_value"), TEXT("Set a static switch value on a module"),
		FMegalithActionHandler::CreateStatic(&HandleSetStaticSwitchValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_system_summary"), TEXT("One-call overview of an entire Niagara system, including emitter topology, event flow, location semantics, role hints, params, renderers, and module counts. AI guidance: start with compact for orientation; if emitters may be linked by events, switch to full before reasoning about where particles spawn or explode."),
		FMegalithActionHandler::CreateStatic(&HandleGetSystemSummary),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_emitter_summary"), TEXT("Deep view of a single emitter, including modules per stage, renderers, event flow, spawn-location semantics, and role hints. AI guidance: use full when this emitter may send or receive Niagara events from other emitters."),
		FMegalithActionHandler::CreateStatic(&HandleGetEmitterSummary),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_emitter_properties"), TEXT("List all editable properties on FVersionedNiagaraEmitterData with current values"),
		FMegalithActionHandler::CreateStatic(&HandleListEmitterProperties),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_module_input_value"), TEXT("Read the current override value for a specific module input"),
		FMegalithActionHandler::CreateStatic(&HandleGetModuleInputValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("configure_curve_keys"), TEXT("Set keys on a DataInterface curve input (NiagaraDataInterfaceCurve/ColorCurve). For plain float inputs use set_curve_value instead"),
		FMegalithActionHandler::CreateStatic(&HandleConfigureCurveKeys),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node GUID or name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("configure_data_interface"), TEXT("Set arbitrary properties on a DI attached to a module input via reflection"),
		FMegalithActionHandler::CreateStatic(&HandleConfigureDataInterface),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node GUID or name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("duplicate_system"), TEXT("Clone an entire Niagara system to a new path"),
		FMegalithActionHandler::CreateStatic(&HandleDuplicateSystem),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_fixed_bounds"), TEXT("Set explicit bounds on system or emitter for GPU performance"),
		FMegalithActionHandler::CreateStatic(&HandleSetFixedBounds),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_effect_type"), TEXT("Assign a UNiagaraEffectType for scalability and cull distance"),
		FMegalithActionHandler::CreateStatic(&HandleSetEffectType),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_emitter"), TEXT("Add a minimal empty emitter to a system (no template needed)"),
		FMegalithActionHandler::CreateStatic(&HandleCreateEmitter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("export_system_spec"), TEXT("Reverse-engineer an existing system into create_system_from_spec-compatible JSON"),
		FMegalithActionHandler::CreateStatic(&HandleExportSystemSpec),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_dynamic_input"), TEXT("Attach a dynamic input script to a module input pin"),
		FMegalithActionHandler::CreateStatic(&HandleAddDynamicInput),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node GUID or name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_dynamic_input_value"), TEXT("Set an input value on a dynamic input node"),
		FMegalithActionHandler::CreateStatic(&HandleSetDynamicInputValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("search_dynamic_inputs"), TEXT("Browse available dynamic input scripts with optional type filtering"),
		FMegalithActionHandler::CreateStatic(&HandleSearchDynamicInputs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_event_handler"), TEXT("Add an inter-emitter event handler (death, collision, location events). This only creates the handler and its ParticleEventScript container. It does NOT auto-add ReceiveDeathEvent/ReceiveLocationEvent modules. source_emitter is required for inter-emitter event handlers; unresolved handlers are rejected instead of being created with an empty SourceEmitterID. If the handler must consume source payloads such as Position/Velocity/Color, add the matching Receive<Event> module to the particle_event script and set the needed payload fields to Apply. For death-triggered bursts like fireworks, Position usually must be Apply."),
		FMegalithActionHandler::CreateStatic(&HandleAddEventHandler),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("validate_system"), TEXT("Pre-compile validation: check common misconfigurations plus inter-emitter event-chain, spawn-location, and persistent-id issues"),
		FMegalithActionHandler::CreateStatic(&HandleValidateSystem),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_simulation_stage"), TEXT("Add a simulation stage to an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleAddSimulationStage),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_spawn_shape"),
		...
		FMegalithActionHandler::CreateStatic(&HandleSetSpawnShape),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_dynamic_inputs"), TEXT("List dynamic inputs attached to a module's inputs"),
		FMegalithActionHandler::CreateStatic(&HandleListDynamicInputs),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_dynamic_input_tree"), TEXT("Get recursive tree of all inputs on a module showing dynamic input structure"),
		FMegalithActionHandler::CreateStatic(&HandleGetDynamicInputTree),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node GUID"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_dynamic_input"), TEXT("Remove a dynamic input from a module pin, cleaning up sub-nodes"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveDynamicInput),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_dynamic_input_value"), TEXT("Read a value from a dynamic input sub-pin"),
		FMegalithActionHandler::CreateStatic(&HandleGetDynamicInputValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_dynamic_input_inputs"), TEXT("Discover inputs on an unattached dynamic input script"),
		FMegalithActionHandler::CreateStatic(&HandleGetDynamicInputInputs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("rename_emitter"), TEXT("Rename an emitter in a system"),
		FMegalithActionHandler::CreateStatic(&HandleRenameEmitter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_emitter_property"), TEXT("Read a single emitter property by name"),
		FMegalithActionHandler::CreateStatic(&HandleGetEmitterProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_available_renderers"), TEXT("List all available Niagara renderer types with descriptions"),
		FMegalithActionHandler::CreateStatic(&HandleListAvailableRenderers),
		FParamSchemaBuilder()
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_renderer_mesh"), TEXT("Assign a StaticMesh to a MeshRenderer slot with optional scale/rotation/pivot"),
		FMegalithActionHandler::CreateStatic(&HandleSetRendererMesh),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
			.Required(TEXT("renderer_index"), TEXT("integer"), TEXT("Renderer index"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("configure_ribbon"), TEXT("High-level ribbon/trail/beam setup with presets (trail, beam, lightning, tube)"),
		FMegalithActionHandler::CreateStatic(&HandleConfigureRibbon),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("configure_subuv"), TEXT("Set up SubUV/flipbook animation on a sprite renderer"),
		FMegalithActionHandler::CreateStatic(&HandleConfigureSubUV),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_event_handlers"), TEXT("Read all event handlers on an emitter with full properties"),
		FMegalithActionHandler::CreateStatic(&HandleGetEventHandlers),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_event_handler_property"), TEXT("Modify an event handler property (ExecutionMode, SpawnNumber, MaxEventsPerFrame, SourceEventName, etc.)"),
		FMegalithActionHandler::CreateStatic(&HandleSetEventHandlerProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_event_handler"), TEXT("Remove an event handler from an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveEventHandler),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
			.Optional(TEXT("handler_index"), TEXT("integer"), TEXT("Event handler index"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_simulation_stages"), TEXT("Read all simulation stages on an emitter with full properties"),
		FMegalithActionHandler::CreateStatic(&HandleGetSimulationStages),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_simulation_stage_property"), TEXT("Modify a simulation stage property (IterationSource, NumIterations, SimulationStageName, etc.)"),
		FMegalithActionHandler::CreateStatic(&HandleSetSimulationStageProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_simulation_stage"), TEXT("Remove a simulation stage from an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveSimulationStage),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
			.Optional(TEXT("stage_index"), TEXT("integer"), TEXT("Simulation stage index"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_module_output_parameters"), TEXT("Discover what particle attributes a module writes to (output variables)"),
		FMegalithActionHandler::CreateStatic(&HandleGetModuleOutputParameters),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node GUID"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_npc"), TEXT("Create a Niagara Parameter Collection (NPC) asset"),
		FMegalithActionHandler::CreateStatic(&HandleCreateNPC),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_npc"), TEXT("Read all parameters and defaults from a Niagara Parameter Collection"),
		FMegalithActionHandler::CreateStatic(&HandleGetNPC),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("add_npc_parameter"), TEXT("Add a parameter to a Niagara Parameter Collection"),
		FMegalithActionHandler::CreateStatic(&HandleAddNPCParameter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("NPC asset path"))
			.Required(TEXT("name"), TEXT("string"), TEXT("Parameter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("remove_npc_parameter"), TEXT("Remove a parameter from a Niagara Parameter Collection"),
		FMegalithActionHandler::CreateStatic(&HandleRemoveNPCParameter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("NPC asset path"))
			.Required(TEXT("name"), TEXT("string"), TEXT("Parameter name to remove"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_npc_default"), TEXT("Set the default value for a parameter in a Niagara Parameter Collection"),
		FMegalithActionHandler::CreateStatic(&HandleSetNPCDefault),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("NPC asset path"))
			.Required(TEXT("name"), TEXT("string"), TEXT("Parameter name"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("create_effect_type"), TEXT("Create a UNiagaraEffectType asset with optional initial settings"),
		FMegalithActionHandler::CreateStatic(&HandleCreateEffectType),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_effect_type"), TEXT("Read all settings from a UNiagaraEffectType asset"),
		FMegalithActionHandler::CreateStatic(&HandleGetEffectType),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_effect_type_property"), TEXT("Set a property on a UNiagaraEffectType via reflection"),
		FMegalithActionHandler::CreateStatic(&HandleSetEffectTypeProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Effect type asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_available_parameters"), TEXT("List all parameters available for binding in a system (user, engine, particle, emitter, system attributes)"),
		FMegalithActionHandler::CreateStatic(&HandleGetAvailableParameters),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("preview_system"), TEXT("Capture a preview screenshot of a Niagara system via editor preview scene"),
		FMegalithActionHandler::CreateStatic(&HandlePreviewSystem),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			...
			.OptionalDiskPath(TEXT("output_path"), TEXT("Absolute output file path (default: auto-generated in Saved/Screenshots/Megalith)"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("diff_systems"), TEXT("Compare two Niagara systems and return structured diff of emitters, modules, inputs, renderers, parameters, properties"),
		FMegalithActionHandler::CreateStatic(&HandleDiffSystems),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("save_emitter_as_template"), TEXT("Extract a configured emitter from a system and save as standalone emitter asset"),
		FMegalithActionHandler::CreateStatic(&HandleSaveEmitterAsTemplate),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("clone_module_overrides"), TEXT("Copy inline value overrides and bindings from one module to another (same script required)"),
		FMegalithActionHandler::CreateStatic(&HandleCloneModuleOverrides),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("save_system"), TEXT("Save a Niagara asset (system, script, NPC, effect type) to disk"),
		FMegalithActionHandler::CreateStatic(&HandleSaveSystem),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_static_switch_value"), TEXT("Get static switch value(s) on a module — omit input to list all switches"),
		FMegalithActionHandler::CreateStatic(&HandleGetStaticSwitchValue),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
			.Required(TEXT("module_node"), TEXT("string"), TEXT("Module node GUID"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("import_system_spec"), TEXT("Overwrite an existing Niagara system with a JSON spec (removes all emitters/params, applies spec fresh)"),
		FMegalithActionHandler::CreateStatic(&HandleImportSystemSpec),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_di_properties"), TEXT("Inspect editable properties on a Niagara DataInterface class (CDO reflection)"),
		FMegalithActionHandler::CreateStatic(&HandleGetDIProperties),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("clear_emitter_modules"), TEXT("Remove all modules from an emitter, optionally filtered by stage"),
		FMegalithActionHandler::CreateStatic(&HandleClearEmitterModules),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_module_script_inputs"), TEXT("Introspect a module script's inputs WITHOUT adding it to an emitter"),
		FMegalithActionHandler::CreateStatic(&HandleGetModuleScriptInputs),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_scalability_settings"), TEXT("Read scalability settings from a NiagaraEffectType asset"),
		FMegalithActionHandler::CreateStatic(&HandleGetScalabilitySettings),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("set_scalability_settings"), TEXT("Set scalability settings on a NiagaraEffectType asset"),
		FMegalithActionHandler::CreateStatic(&HandleSetScalabilitySettings),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("NiagaraEffectType asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_systems"), TEXT("Search/list Niagara system assets in the project"),
		FMegalithActionHandler::CreateStatic(&HandleListSystems),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("duplicate_module"), TEXT("Duplicate a module within or across emitters (copies script + all overrides)"),
		FMegalithActionHandler::CreateStatic(&HandleDuplicateModule),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("source_emitter"), TEXT("string"), TEXT("Source emitter name or handle ID"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("get_emitter_parent"), TEXT("Get the parent emitter asset of an emitter in a system (read-only)"),
		FMegalithActionHandler::CreateStatic(&HandleGetEmitterParent),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Required(TEXT("emitter"), TEXT("string"), TEXT("Emitter name or handle ID"))
			.Build());
	Registry.RegisterAction(TEXT("niagara"), TEXT("rename_user_parameter"), TEXT("Rename a user parameter and update all module bindings that reference it"),
		FMegalithActionHandler::CreateStatic(&HandleRenameUserParameter),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("search_by_parameter"), TEXT("Find Niagara systems exposing a user parameter whose name (case-insensitive) contains the query. Optional type filter."),
		FMegalithActionHandler::CreateStatic(&HandleSearchByParameter),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("search_by_data_interface"), TEXT("Find Niagara systems using a Data Interface whose class name (case-insensitive) contains the query. Per-system traversal via FNiagaraDataInterfaceUtilities::ForEachDataInterface."),
		FMegalithActionHandler::CreateStatic(&HandleSearchByDataInterface),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("query_niagara"), TEXT("Structured-filter query over all systems. Conditions joined by AND, comma- or AND-separated: emitters>N / emitters<N / emitters=N, sim_target=GPU|CPU, has_renderer=<name>. Deterministic DSL, NOT natural language."),
		FMegalithActionHandler::CreateStatic(&HandleQueryNiagara),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("find_similar_systems"), TEXT("Rank systems by structural similarity to a reference system. Score = weighted blend of emitter-count proximity, renderer-class-set Jaccard, and module-name-set Jaccard. Reference scores 1.0 against itself."),
		FMegalithActionHandler::CreateStatic(&HandleFindSimilarSystems),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("search_by_material"), TEXT("Find Niagara systems whose emitter renderers reference a given material. Walks each system's emitters' renderers' material bindings (Sprite/Ribbon/Mesh)."),
		FMegalithActionHandler::CreateStatic(&HandleSearchByMaterial),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("find_niagara_references"), TEXT("Find all assets that reference a given Niagara asset, via the Asset Registry referencer graph (IAssetRegistry::GetReferencers)."),
		FMegalithActionHandler::CreateStatic(&HandleFindNiagaraReferences),
		FParamSchemaBuilder()
	...
	Registry.RegisterAction(TEXT("niagara"), TEXT("list_system_data_interfaces"), TEXT("Enumerate the Data Interfaces actually USED BY a given system (per-system traversal via FNiagaraDataInterfaceUtilities::ForEachDataInterface). Distinct from get_di_properties (CDO-class reflection only)."),
		FMegalithActionHandler::CreateStatic(&HandleListSystemDataInterfaces),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Niagara system asset path"))
			.Build());
}
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddEmitter(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (EmitterAssetPath.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param 'emitter_asset': provide a NiagaraEmitter asset path"));
	FString EmitterName = Params->HasField(TEXT("name")) ? Params->GetStringField(TEXT("name")) : FString();

	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));

	UNiagaraEmitter* EmitterAsset = FMegalithAssetUtils::LoadAssetByPath<UNiagaraEmitter>(EmitterAssetPath);
	if (!EmitterAsset) return FMegalithActionResult::Error(FString::Printf(
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "AddEmitter", "Add Emitter"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveEmitter(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (Index == INDEX_NONE) return FMegalithActionResult::Error(TEXT("Emitter handle not found"));

	const FNiagaraEmitterHandle& Handle = System->GetEmitterHandles()[Index];
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemoveEmitter", "Remove Emitter"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleDuplicateEmitter(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (Index == INDEX_NONE) return FMegalithActionResult::Error(TEXT("Source emitter not found"));

	const FNiagaraEmitterHandle& Src = System->GetEmitterHandles()[Index];
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "DupEmitter", "Duplicate Emitter"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetEmitterEnabled(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetEmEnabled", "Set Emitter Enabled"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleReorderEmitters(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (OrderArr.Num() != Current.Num())
		return FMegalithActionResult::Error(FString::Printf(TEXT("Provided %d IDs but system has %d emitters"), OrderArr.Num(), Current.Num()));
	...
	for (const TSharedPtr<FJsonValue>& V : OrderArr)
	{
		...
		if (Idx == INDEX_NONE) return FMegalithActionResult::Error(FString::Printf(TEXT("Handle '%s' not found"), *V->AsString()));
		NewOrder.Add(Current[Idx]);
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "ReorderEm", "Reorder Emitters"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetEmitterProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (!JV.IsValid())
		return FMegalithActionResult::Error(TEXT("Missing required field: value"));
	...
	if (!ED) return FMegalithActionResult::Error(TEXT("No emitter data"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetEmProp", "Set Emitter Property"));
	System->Modify();
	bool bOk = false;
	...
	return bOk ? NA_SuccessStr(TEXT("Property set")) : FMegalithActionResult::Error(TEXT("Unknown property"));
}

FMegalithActionResult FMegalithNiagaraActions::HandleRequestCompile(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleCreateSystem(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (const FString PathError = MegalithCore::ValidatePackagePath(SavePath); !PathError.IsEmpty())
	{
		return FMegalithActionResult::Error(PathError);
	}

	if (!TemplatePath.IsEmpty())
	{
		UNiagaraSystem* Template = FMegalithAssetUtils::LoadAssetByPath<UNiagaraSystem>(TemplatePath);
		if (!Template) return FMegalithActionResult::Error(TEXT("Failed to load template"));
		...
		if (!Dup) return FMegalithActionResult::Error(TEXT("Failed to duplicate template"));
	...
	if (!Pkg) return FMegalithActionResult::Error(TEXT("Failed to create package"));

	UNiagaraSystem* NS = NewObject<UNiagaraSystem>(Pkg, FName(*AssetName), RF_Public | RF_Standalone | RF_Transactional);
	if (!NS) return FMegalithActionResult::Error(TEXT("Failed to create system"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleCreateStatelessEmitter(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!StatelessClass) return FMegalithActionResult::Error(TEXT("UNiagaraStatelessEmitter UClass not found - Niagara plugin may not be loaded"));

	UObject* Emitter = NewObject<UObject>(
		Pkg, StatelessClass, FName(*AssetName), RF_Public | RF_Standalone | RF_Transactional);
	if (!Emitter) return FMegalithActionResult::Error(TEXT("Failed to create stateless emitter"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetOrderedModules(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	else
	{
		...
		if (Usage == ENiagaraScriptUsage::ParticleSimulationStageScript)
		{
			...
			{
				return FMegalithActionResult::Error(StageError);
			}
		...
		else if (Usage == ENiagaraScriptUsage::ParticleEventScript)
		{
			...
			{
				return FMegalithActionResult::Error(EventError);
			}
	...
	for (ENiagaraScriptUsage Usage : UsagesToQuery)
	{
		...
		TArray<UNiagaraNodeFunctionCall*> ModuleNodes;
		MegalithNiagaraHelpers::GetOrderedModuleNodes(*OutputNode, ModuleNodes);
		...
		for (int32 i = 0; i < ModuleNodes.Num(); ++i)
		{
			...
			TOptional<bool> bEn = MegalithNiagaraHelpers::GetModuleIsEnabled(*N);
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetModuleInputs(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (!EmitterHandleId.IsEmpty() && FindEmitterHandleIndex(System, EmitterHandleId) == INDEX_NONE)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Emitter '%s' not found. Use list_emitters to get valid emitter names or GUIDs."), *EmitterHandleId));
	...
	if (!ModuleNode) return FMegalithActionResult::Error(TEXT("Module node not found"));
	...
	for (const FNiagaraVariable& Input : Inputs)
	{
		...
		FName ShortName = MegalithNiagaraHelpers::StripModulePrefix(Input.GetName());
		...
		UEdGraphPin* OP = MegalithNiagaraHelpers::GetStackFunctionInputOverridePin(*ModuleNode, AH);
		if (OP)
		{
			...
			if (OP->LinkedTo.Num() > 0)
			{
				...
				if (UNiagaraNodeInput* LI = Cast<UNiagaraNodeInput>(OP->LinkedTo[0]->GetOwningNode()))
				{
					...
					if (DI)
					{
						TSharedPtr<FJsonObject> CurveData = MegalithNiagaraHelpers::SerializeDICurveData(DI);
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetModuleGraph(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!Script) return FMegalithActionResult::Error(TEXT("Failed to load script"));

	UNiagaraScriptSource* Src = Cast<UNiagaraScriptSource>(Script->GetLatestSource());
	if (!Src || !Src->NodeGraph) return FMegalithActionResult::Error(TEXT("No graph available"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetCustomHLSLText(const TSharedPtr<FJsonObject>& Params)
{
	FString ScriptPath = Params->GetStringField(TEXT("script_path"));
	...
	if (!NodeGuidStr.IsEmpty() && !bHasGuid)
		return FMegalithActionResult::Error(TEXT("Invalid node_guid GUID"));

	TArray<UNiagaraNodeCustomHlsl*> HlslNodes;
	Src->NodeGraph->GetNodesOfClass<UNiagaraNodeCustomHlsl>(HlslNodes);
	if (HlslNodes.Num() == 0)
		return FMegalithActionResult::Error(TEXT("No CustomHlsl node found in script"));
	...
	if (bHasGuid)
	{
		...
		if (!TargetNode)
			return FMegalithActionResult::Error(FString::Printf(TEXT("CustomHlsl node '%s' not found"), *NodeGuidStr));
	}
	...
	if (!HlslProp)
		return FMegalithActionResult::Error(TEXT("CustomHlsl property not found on CustomHlsl node"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetCustomHLSLText(const TSharedPtr<FJsonObject>& Params)
{
	FString ScriptPath = Params->GetStringField(TEXT("script_path"));
	...
	if (HlslText.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required field: hlsl"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetCustomHlslText", "Set Custom HLSL Text"));
	Script->Modify();
	Src->NodeGraph->Modify();
	TargetNode->Modify();
	HlslProp->SetPropertyValue_InContainer(TargetNode, HlslText);
	TargetNode->MarkNodeRequiresSynchronization(TEXT("MegalithSetCustomHlslText"), true);
	GEditor->EndTransaction();
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddModule(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ScriptUsage = Params->GetStringField(TEXT("usage"));
	...
	if (!ModScript)
	{
		...
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load module script '%s'"), *ModuleScriptPath));
	}
	...
	if (!OutputNode)
	{
		...
		return FMegalithActionResult::Error(TEXT("No output node"));
	}
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "AddMod", "Add Module"));
	System->Modify();
	UNiagaraNodeFunctionCall* NewNode = FNiagaraStackGraphUtilities::AddScriptModuleToStack(ModScript, *OutputNode, Index);
	GEditor->EndTransaction();

	if (!NewNode) return FMegalithActionResult::Error(TEXT("AddScriptModuleToStack failed"));
	...
	if (Usage == ENiagaraScriptUsage::ParticleUpdateScript && ModuleScriptBaseName.Contains(TEXT("GenerateDeathEvent")))
	{
		...
		if (EmitterIdx != INDEX_NONE)
		{
			...
			if (TargetEmitterData && !TargetEmitterData->bRequiresPersistentIDs)
			{
				...
				FMegalithActionResult PersistentIdResult = HandleSetEmitterProperty(PersistentIdParams);
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveModule(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!MN) return FMegalithActionResult::Error(TEXT("Module node not found"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemMod", "Remove Module"));
	System->Modify();
	MegalithNiagaraHelpers::RemoveModuleFromStack(*System, EmitterGuid, *MN);
	GEditor->EndTransaction();
	System->RequestCompile(false);
...
FMegalithActionResult FMegalithNiagaraActions::HandleMoveModule(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!OutputNode) return FMegalithActionResult::Error(TEXT("No output node"));

	// Build the node group array from the PM chain
	TArray<MegalithNiagaraHelpers::FStackNodeGroup> Groups;
	MegalithNiagaraHelpers::GetStackNodeGroups(*OutputNode, Groups);
	...
	if (GroupIdx == INDEX_NONE)
		return FMegalithActionResult::Error(TEXT("Module not found in stack node groups"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "MoveMod", "Move Module"));
	System->Modify();

	// Save the group we're moving
	MegalithNiagaraHelpers::FStackNodeGroup MovedGroup = Groups[GroupIdx];

	// Disconnect from current position — splice neighbors together
	MegalithNiagaraHelpers::DisconnectGroup(MovedGroup, Groups[GroupIdx - 1], Groups[GroupIdx + 1]);

	// Re-fetch groups after the chain changed
	TArray<MegalithNiagaraHelpers::FStackNodeGroup> UpdatedGroups;
	MegalithNiagaraHelpers::GetStackNodeGroups(*OutputNode, UpdatedGroups);
	...
	MegalithNiagaraHelpers::ConnectGroup(MovedGroup, UpdatedGroups[TargetGroupIdx - 1], UpdatedGroups[TargetGroupIdx]);
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetModuleEnabled(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetModEn", "Set Module Enabled"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetModuleInputValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	{
		FName ShortName = MegalithNiagaraHelpers::StripModulePrefix(In.GetName());
	...
	if (!bInputFound)
	{
		...
		for (const FNiagaraVariable& In : Inputs) { ValidNames.Add(MegalithNiagaraHelpers::StripModulePrefix(In.GetName()).ToString()); }
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("Input '%s' not found on module. Valid inputs: [%s]"),
			*InputName, *FString::Join(ValidNames, TEXT(", "))));
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetModIn", "Set Module Input"));
	System->Modify();
	...
	if (bCustomHlslFallback)
	{
		...
		{
			GEditor->EndTransaction();
			return FMegalithActionResult::Error(TEXT("Failed to find pin on FunctionCall node"));
		}
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetModuleInputBinding(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetModBind", "Set Module Binding"));
	System->Modify();
	...
	if (Graph) MegalithNiagaraHelpers::GetParametersForContext(Graph, *System, KnownParams);
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetModuleInputDI(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	UClass* DIUClass = MegalithNiagaraHelpers::ResolveNiagaraDataInterfaceClass(DIClass, &DIDiagnostic);
	if (!DIUClass)
	{
		return FMegalithActionResult::Error(DIDiagnostic.IsEmpty() ? TEXT("DI class not found") : DIDiagnostic);
	}
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetModDI", "Set Module DI"));
	System->Modify();
	...
	if (DIInst && DIConfig.IsValid())
	{
		...
		bool bCurveApplied = MegalithNiagaraHelpers::ApplyCurveConfig(DIInst, DIConfig);
		bCurveConfigApplied = bCurveApplied;

		// Try grid-specific config (handles Grid2D, Grid3D, NeighborGrid3D)
		if (!bCurveApplied)
		{
			bGridApplied = MegalithNiagaraHelpers::ApplyGridConfig(DIInst, DIConfig);
		}
		...
		if (!bCurveApplied && !bGridApplied)
		{
			...
			{
				if (CurveFieldNames.Contains(MegalithKeyToString(Pair.Key))) { bHadCurveFields = true; break; }
			}
			if (bHadCurveFields)
			{
				UE_LOG(LogMegalithNiagara, Warning, TEXT("set_module_input_di: config had curve fields but DI type '%s' didn't match any known curve DI. Curve keys were NOT applied."), *DIUClass->GetName());
			}
		}
		...
		{
			if (bIsCurveDI && CurveKeys.Contains(MegalithKeyToString(Pair.Key))) continue;
			if (bIsGridDI && GridKeys.Contains(MegalithKeyToString(Pair.Key))) continue;

			FProperty* Prop = DIUClass->FindPropertyByName(FName(*MegalithKeyToString(Pair.Key)));
			if (!Prop) continue;
...
FMegalithActionResult FMegalithNiagaraActions::CreateScriptFromHLSL(const TSharedPtr<FJsonObject>& Params, ENiagaraScriptUsage Usage)
{
	...
	if (Name.IsEmpty()) return FMegalithActionResult::Error(TEXT("'name' is required"));
	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("'save_path' is required"));
	if (HlslBody.IsEmpty()) return FMegalithActionResult::Error(TEXT("'hlsl' is required"));
	...
	if (!Script) return FMegalithActionResult::Error(TEXT("Failed to create NiagaraScript"));
	Script->Usage = Usage;
	...
	if (bIsModule)
	{
		...
		UEdGraphPin* HlslMapIn = MegalithNiagaraHelpers::GetParameterMapPin(*HlslNode, EGPD_Input);
		UEdGraphPin* HlslMapOut = MegalithNiagaraHelpers::GetParameterMapPin(*HlslNode, EGPD_Output);
	...
	HlslNode->MarkNodeRequiresSynchronization(TEXT("MegalithHLSL"), true);
...
FMegalithActionResult FMegalithNiagaraActions::HandleCreateModuleFromHLSL(const TSharedPtr<FJsonObject>& Params)
{
	return CreateScriptFromHLSL(Params, ENiagaraScriptUsage::Module);
}

FMegalithActionResult FMegalithNiagaraActions::HandleCreateFunctionFromHLSL(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetAllParameters(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetUserParameters(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetParameterValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	return FMegalithActionResult::Error(TEXT("Parameter not found"));
}

FMegalithActionResult FMegalithNiagaraActions::HandleGetParameterType(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithNiagaraActions::HandleTraceParameterBinding(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString ParamName = Params->GetStringField(TEXT("parameter"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddUserParameter(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (ParamName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Parameter name is required — pass as \"name\" field"));

	if (TypeName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Parameter type is required — pass as \"type\" field. Valid types: float, int, bool, vec2, vec3, vec4, color, position, quat, matrix"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "AddUP", "Add User Parameter"));
	System->Modify();
	FNiagaraUserRedirectionParameterStore& US = System->GetExposedParameters();
		...
		// GetUserParameters() returns bare names but the store indexes by "User.X" form)
		{
			...
			if (bFoundInStore)
			{
				...
				if (TD == FNiagaraTypeDefinition::GetBoolDef())
				{
					...
					{
						UE_LOG(LogMegalithNiagara, Warning, TEXT("add_user_parameter: Bool value readback mismatch for '%s' — wrote %s, read %s (editor cascade may overwrite)"),
							*ParamName,
				...
				else if (TD == FNiagaraTypeDefinition::GetVec3Def() || TD == FNiagaraTypeDefinition::GetPositionDef())
				{
					...
					{
						UE_LOG(LogMegalithNiagara, Warning, TEXT("add_user_parameter: Vec3f value readback mismatch for '%s' — wrote (%f,%f,%f), read (%f,%f,%f) (editor cascade may overwrite)"),
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveUserParameter(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	for (const FNiagaraVariable& P : UP)
	{
		if (P.GetName().ToString().Equals(Search, ESearchCase::IgnoreCase))
		{
			GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemUP", "Remove User Parameter"));
			System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetParameterDefault(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString ParamName = Params->GetStringField(TEXT("parameter"));
	TSharedPtr<FJsonValue> JV = Params->TryGetField(TEXT("value"));
	...
	if (!bFound) return FMegalithActionResult::Error(TEXT("Parameter not found"));

	// Let the transaction complete and all editor cascades settle first.
	// Then write the value in a suppressed scope so it sticks.
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetPDef", "Set Parameter Default"));
	System->Modify();
	GEditor->EndTransaction();
	...
	return bOk ? NA_SuccessStr(TEXT("Default set")) : FMegalithActionResult::Error(TEXT("Unsupported type"));
}

FMegalithActionResult FMegalithNiagaraActions::HandleSetCurveValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (!KeysField.IsValid())
		return FMegalithActionResult::Error(TEXT("Missing required field: keys"));
	...
	else if (KeysField->Type == EJson::String)
	{
		...
		if (!FJsonSerializer::Deserialize(Reader, Keys))
			return FMegalithActionResult::Error(TEXT("Failed to parse keys string"));
	}
	else
	{
		return FMegalithActionResult::Error(TEXT("'keys' must be an array"));
	}
	...
	if (!MN) return FMegalithActionResult::Error(TEXT("Module not found"));
	TArray<FNiagaraVariable> Inputs;
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetCurve", "Set Curve"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddRenderer(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (!RC) return FMegalithActionResult::Error(TEXT("Unknown renderer class"));

	int32 EIdx = FindEmitterHandleIndex(System, EmitterHandleId);
	if (EIdx == INDEX_NONE) return FMegalithActionResult::Error(TEXT("Emitter not found"));

	const FNiagaraEmitterHandle& Handle = System->GetEmitterHandles()[EIdx];
	FVersionedNiagaraEmitterData* ED = Handle.GetEmitterData();
	if (!ED) return FMegalithActionResult::Error(TEXT("No emitter data"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "AddRend", "Add Renderer"));
	System->Modify();
	...
	if (!NR) { GEditor->EndTransaction(); return FMegalithActionResult::Error(TEXT("Failed to create renderer")); }
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveRenderer(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (!Rend) return FMegalithActionResult::Error(TEXT("Renderer not found"));

	int32 EIdx = FindEmitterHandleIndex(System, EmitterHandleId);
	const FNiagaraEmitterHandle& Handle = System->GetEmitterHandles()[EIdx];
	FVersionedNiagaraEmitter EI = Handle.GetInstance();

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemRend", "Remove Renderer"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetRendererMaterial(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
	...
	if (!bClear)
	{
		...
		if (!Mat) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load material '%s'"), *MaterialPath));
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetRendMat", "Set Renderer Material"));
	System->Modify();
	Rend->Modify();
	bool bOk = false;
	...
	GEditor->EndTransaction();
	if (bOk) System->RequestCompile(false);
	return bOk ? NA_SuccessStr(bClear ? TEXT("Material cleared") : TEXT("Material set")) : FMegalithActionResult::Error(TEXT("Unsupported renderer type"));
}

FMegalithActionResult FMegalithNiagaraActions::HandleSetRendererProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
	...
	if (!Prop) return FMegalithActionResult::Error(FString::Printf(TEXT("Property '%s' not found"), *PropertyName));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetRendProp", "Set Renderer Property"));
	System->Modify();
	Rend->Modify();
	...
	GEditor->EndTransaction();
	if (bOk) System->RequestCompile(false);
	return bOk ? NA_SuccessStr(TEXT("Property set")) : FMegalithActionResult::Error(TEXT("Failed to set property"));
}

FMegalithActionResult FMegalithNiagaraActions::HandleGetRendererBindings(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetRendererBinding(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
	...
	if (!BP) return FMegalithActionResult::Error(TEXT("Binding property not found"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetRendBind", "Set Renderer Binding"));
	System->Modify();
	Rend->Modify();
	...
	GEditor->EndTransaction();
	if (bOk) System->RequestCompile(false);
	return bOk ? NA_SuccessStr(TEXT("Binding set")) : FMegalithActionResult::Error(TEXT("Failed to set binding"));
}
...
FMegalithActionResult FMegalithNiagaraActions::HandleBatchExecute(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (!OpsField.IsValid())
		return FMegalithActionResult::Error(TEXT("Missing required field: operations"));
	...
	else if (OpsField->Type == EJson::String)
	{
		...
		if (!FJsonSerializer::Deserialize(Reader, Ops))
			return FMegalithActionResult::Error(TEXT("Failed to parse operations string"));
	}
	else
	{
		return FMegalithActionResult::Error(TEXT("'operations' must be an array"));
	}
	...
	if (bAnyWrites)
	{
		GEditor->BeginTransaction(NSLOCTEXT("Megalith", "BatchExec", "Batch Execute"));
		System->Modify();
	}
	...
	for (int32 i = 0; i < Ops.Num(); ++i)
	{
		...
		FMegalithActionResult SubResult = FMegalithActionResult::Error(TEXT("Unknown op"));
		...
		// Layout (Phase 3b)
		else if (OpName == TEXT("auto_layout")) SubResult = FMegalithNiagaraLayoutActions::HandleAutoLayout(SubParams);
...
FMegalithActionResult FMegalithNiagaraActions::HandleCreateSystemFromSpec(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!SpecField.IsValid())
		return FMegalithActionResult::Error(TEXT("Missing required field: spec"));
	...
	else if (SpecField->Type == EJson::String)
	{
		...
		if (!FJsonSerializer::Deserialize(Reader, Spec) || !Spec.IsValid())
			return FMegalithActionResult::Error(TEXT("Failed to parse spec string"));
	}
	else
	{
		return FMegalithActionResult::Error(TEXT("'spec' must be an object"));
	}

	if (!Spec.IsValid())
		return FMegalithActionResult::Error(TEXT("Failed to parse spec"));
	...
	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("save_path required (provide at params root or inside spec)"));
	...
	FMegalithActionResult CreateResult = HandleCreateSystem(CreateParams);
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetDIFunctions(const TSharedPtr<FJsonObject>& Params)
{
	FString DIClassName = Params->GetStringField(TEXT("di_class"));
	FString DIDiagnostic;
	UClass* DIC = MegalithNiagaraHelpers::ResolveNiagaraDataInterfaceClass(DIClassName, &DIDiagnostic);
	if (!DIC)
		return FMegalithActionResult::Error(DIDiagnostic.IsEmpty() ? TEXT("DI class not found") : DIDiagnostic);

	UNiagaraDataInterface* TempDI = NewObject<UNiagaraDataInterface>(GetTransientPackage(), DIC);
	if (!TempDI) return FMegalithActionResult::Error(TEXT("Failed to create DI instance"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetCompiledGPUHLSL(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (ED->SimTarget != ENiagaraSimTarget::GPUComputeSim)
		return FMegalithActionResult::Error(TEXT("Emitter is not GPU simulation"));

	UNiagaraScript* GPU = ED->GetGPUComputeScript();
	if (!GPU) return FMegalithActionResult::Error(TEXT("No GPU compute script"));
...
#if WITH_EDITORONLY_DATA
	{
		...
		{
			UE_LOG(LogMegalithNiagara, Log, TEXT("HLSL not available — requesting compilation"));
			System->RequestCompile(false);
	...
	if (System->HasOutstandingCompilationRequests())
		UE_LOG(LogMegalithNiagara, Warning, TEXT("System has outstanding compilation requests"));
	...
	else return FMegalithActionResult::Error(TEXT("Compilation completed but produced no GPU HLSL. The emitter may have compile errors or may not use GPU simulation."));
#else
	return FMegalithActionResult::Error(TEXT("HLSL only available in editor builds"));
#endif
...
FMegalithActionResult FMegalithNiagaraActions::HandleListEmitters(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
...
FMegalithActionResult FMegalithNiagaraActions::HandleListRenderers(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
...
// Kept in the named MegalithNiagaraHelpers namespace (NOT an anonymous namespace) and NA_-prefixed
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetSystemDiagnostics(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));
	...
	for (const FNiagaraEmitterHandle& Handle : System->GetEmitterHandles())
	{
		...
		auto CollectModuleScriptPaths = [&](ENiagaraScriptUsage Usage) -> TArray<FString>
		{
			...
			{
				TArray<UNiagaraNodeFunctionCall*> Modules;
				MegalithNiagaraHelpers::GetOrderedModuleNodes(*OutNode, Modules);
		...
		static const MegalithNiagaraHelpers::FNA_RendererAttributeHazardRule HazardRules[] = {
		...
		for (const MegalithNiagaraHelpers::FNA_RendererAttributeHazardRule& Rule : HazardRules)
		{
			if (!MegalithNiagaraHelpers::NA_HasRendererClass(ED->GetRenderers(), Rule.RendererClassName))
			{
				continue;
			}

			const bool bHasConsumer = MegalithNiagaraHelpers::NA_HasConsumerModule(ParticleSpawnPaths, Rule.ConsumerToken)
				|| MegalithNiagaraHelpers::NA_HasConsumerModule(ParticleUpdatePaths, Rule.ConsumerToken);
			if (!bHasConsumer)
			{
				continue;
			}

			const bool bHasAttributeInit = MegalithNiagaraHelpers::NA_HasAttributeInitializationModule(ParticleSpawnPaths, Rule.FriendlyAttributeName);
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleListModuleScripts(const TSharedPtr<FJsonObject>& Params)
{
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleListRendererProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
	...
	if (!Renderers.IsValidIndex(RendererIndex))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Renderer index %d out of range (0-%d)"), RendererIndex, Renderers.Num() - 1));

	UNiagaraRendererProperties* Rend = Renderers[RendererIndex];
	if (!Rend) return FMegalithActionResult::Error(TEXT("Renderer is null"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetSystemProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString PropertyName = Params->GetStringField(TEXT("property"));
	if (PropertyName.IsEmpty()) PropertyName = Params->GetStringField(TEXT("property_name"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetSystemProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString PropertyName = Params->GetStringField(TEXT("property"));
	if (PropertyName.IsEmpty()) PropertyName = Params->GetStringField(TEXT("property_name"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetSysProp", "Set System Property"));
	System->Modify();
	bool bOk = false;
	...
	return bOk ? NA_SuccessStr(TEXT("System property set")) : FMegalithActionResult::Error(
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetStaticSwitchValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!EmitterHandleId.IsEmpty() && FindEmitterHandleIndex(System, EmitterHandleId) == INDEX_NONE)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Emitter '%s' not found"), *EmitterHandleId));
	...
	if (!CalledGraph)
		return FMegalithActionResult::Error(TEXT("Module has no script graph — cannot enumerate static switches"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetStaticSwitch", "Set Static Switch"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetSystemSummary(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));
	EMegalithSemanticDetailLevel DetailLevel = EMegalithSemanticDetailLevel::Compact;
	FString DetailError;
	if (!TryParseSemanticDetailLevel(Params, DetailLevel, DetailError))
	{
		return FMegalithActionResult::Error(DetailError);
	}
	const TArray<FNiagaraEmitterHandle>& Handles = System->GetEmitterHandles();
	TArray<FMegalithNiagaraTopologyEdge> TopologyEdges;
	CollectTopologyEdges(System, TopologyEdges);

	TSharedRef<FJsonObject> R = MakeShared<FJsonObject>();
	R->SetStringField(TEXT("system_name"), System->GetName());
	R->SetStringField(TEXT("detail_level"), DetailLevel == EMegalithSemanticDetailLevel::Full ? TEXT("full") : TEXT("compact"));
	...
	for (int32 i = 0; i < Handles.Num(); ++i)
	{
		const FNiagaraEmitterHandle& Handle = Handles[i];
		FVersionedNiagaraEmitterData* ED = Handle.GetEmitterData();
		const FMegalithNiagaraEmitterSemantic Semantic = AnalyzeEmitterSemantic(System, Handle, i, TopologyEdges);
		TSharedRef<FJsonObject> EObj = MakeShared<FJsonObject>();
		EObj->SetStringField(TEXT("name"), Handle.GetName().ToString());
		EObj->SetNumberField(TEXT("index"), i);
		EObj->SetBoolField(TEXT("enabled"), Handle.GetIsEnabled());
	...
	if (DetailLevel == EMegalithSemanticDetailLevel::Full)
	{
		for (const FMegalithNiagaraTopologyEdge& Edge : TopologyEdges)
		{
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetEmitterSummary(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	TArray<FMegalithNiagaraTopologyEdge> TopologyEdges;
	CollectTopologyEdges(System, TopologyEdges);
	const FMegalithNiagaraEmitterSemantic Semantic = AnalyzeEmitterSemantic(System, Handle, EIdx, TopologyEdges);
...
FMegalithActionResult FMegalithNiagaraActions::HandleListEmitterProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetModuleInputValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!bInputFound)
	{
		TArray<FString> ValidNames;
		for (const FNiagaraVariable& In : Inputs) ValidNames.Add(MegalithNiagaraHelpers::StripModulePrefix(In.GetName()).ToString());
		return FMegalithActionResult::Error(FString::Printf(TEXT("Input '%s' not found. Valid: [%s]"), *InputName, *FString::Join(ValidNames, TEXT(", "))));
	}

	TSharedRef<FJsonObject> R = MakeShared<FJsonObject>();
	R->SetStringField(TEXT("input"), MegalithNiagaraHelpers::StripModulePrefix(MatchedFullName).ToString());
	...
	FNiagaraParameterHandle AH = FNiagaraParameterHandle::CreateAliasedModuleParameterHandle(
		FNiagaraParameterHandle(MatchedFullName), MN);
	UEdGraphPin* OP = MegalithNiagaraHelpers::GetStackFunctionInputOverridePin(*MN, AH);
...
UNiagaraDataInterface* FMegalithNiagaraActions::FindDIFromOverridePin(UNiagaraNodeFunctionCall* ModuleNode, const FName& MatchedFullName, const FNiagaraTypeDefinition& InputType)
{
	FNiagaraParameterHandle AH = FNiagaraParameterHandle::CreateAliasedModuleParameterHandle(
...
UNiagaraNodeFunctionCall* FMegalithNiagaraActions::FindFunctionCallNode(UNiagaraSystem* System, const FString& EmitterHandleId, const FString& NodeGuidStr)
{
	FGuid TargetGuid;
...
FMegalithActionResult FMegalithNiagaraActions::HandleConfigureCurveKeys(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	for (const FNiagaraVariable& In : Inputs)
	{
		FName Short = MegalithNiagaraHelpers::StripModulePrefix(In.GetName());
	...
	if (!bFound) return FMegalithActionResult::Error(FString::Printf(TEXT("Input '%s' not found"), *InputName));
	if (!InputType.IsDataInterface()) return FMegalithActionResult::Error(FString::Printf(
	...
	if (!DI)
	{
		...
		UClass* ExpectedDIClass = const_cast<UClass*>(InputType.GetClass());
		if (!ExpectedDIClass) return FMegalithActionResult::Error(TEXT("Cannot determine DI class for this input"));

		// Create DI and assign via SetDataInterfaceValueForFunctionInput
		FNiagaraParameterHandle AH = FNiagaraParameterHandle::CreateAliasedModuleParameterHandle(
			FNiagaraParameterHandle(MatchedFullName), MN);
		GEditor->BeginTransaction(NSLOCTEXT("Megalith", "CreateCurveDI", "Create Curve DI"));
		System->Modify();
		UEdGraphPin& OP = FNiagaraStackGraphUtilities::GetOrCreateStackFunctionInputOverridePin(
			*MN, AH, InputType, FGuid(), FGuid());
	...
	if (!DI) return FMegalithActionResult::Error(TEXT("Failed to find or create curve DI on this input"));

	// Parse keys
	TSharedPtr<FJsonValue> KeysField = Params->TryGetField(TEXT("keys"));
	if (!KeysField.IsValid()) return FMegalithActionResult::Error(TEXT("Missing required field: keys"));
	TArray<TSharedPtr<FJsonValue>> Keys;
	...
	if (Keys.Num() == 0) return FMegalithActionResult::Error(TEXT("keys array is empty"));
	...
	bool bApplied = MegalithNiagaraHelpers::ApplyCurveConfig(DI, Config);
	if (!bApplied) return FMegalithActionResult::Error(TEXT("Failed to apply curve keys — DI type may not be a supported curve type"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleConfigureDataInterface(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!Properties.IsValid() || Properties->Values.Num() == 0)
		return FMegalithActionResult::Error(TEXT("Missing or empty 'properties' object"));
	...
	if (!bInputFound) return FMegalithActionResult::Error(FString::Printf(TEXT("Input '%s' not found"), *InputName));

	UNiagaraDataInterface* DI = FindDIFromOverridePin(MN, MatchedFullName, InputType);
	if (!DI) return FMegalithActionResult::Error(TEXT("No DataInterface found on this input. Use set_module_input_di to create one first."));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "ConfigDI", "Configure Data Interface"));
	System->Modify();
	...
	{
		FProperty* Prop = DI->GetClass()->FindPropertyByName(FName(*MegalithKeyToString(Pair.Key)));
		if (!Prop) { PropsNotFound.Add(MegalithKeyToString(Pair.Key)); continue; }
		...
		else if (CastField<FStructProperty>(Prop))
		{
			if (Pair.Value->Type == EJson::Object)
			{
				...
				for (auto& KV : Obj->Values)
				{
					...
					bFirst = false;
					ValStr += MegalithKeyToString(KV.Key) + TEXT("=") + KV.Value->AsString();
				}
				ValStr += TEXT(")");
			}
			else { ValStr = Pair.Value->AsString(); }
		...
		{
			PropsSet.Add(MegalithKeyToString(Pair.Key));
		}
		else
		{
			TSharedRef<FJsonObject> FailEntry = MakeShared<FJsonObject>();
			FailEntry->SetStringField(TEXT("property"), MegalithKeyToString(Pair.Key));
			FailEntry->SetStringField(TEXT("value"), ValStr);
			FailEntry->SetStringField(TEXT("error"), FString::Printf(TEXT("ImportText_Direct failed for property '%s' with value '%s'"), *MegalithKeyToString(Pair.Key), *ValStr));
...
FMegalithActionResult FMegalithNiagaraActions::HandleDuplicateSystem(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: save_path"));
	...
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load source system"));
	...
	if (!DupSystem) return FMegalithActionResult::Error(TEXT("DuplicateAsset failed"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetFixedBounds(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (bEnabled)
	{
		...
		if (!MinV.IsSet() || !MaxV.IsSet())
			return FMegalithActionResult::Error(TEXT("Both 'min' and 'max' are required as [x,y,z] arrays when enabling bounds"));
	}

	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetBounds", "Set Fixed Bounds"));
	System->Modify();
	...
	{
		int32 EIdx = FindEmitterHandleIndex(System, EmitterHandleId);
		if (EIdx == INDEX_NONE) { GEditor->EndTransaction(); return FMegalithActionResult::Error(TEXT("Emitter not found")); }
		FVersionedNiagaraEmitterData* ED = System->GetEmitterHandles()[EIdx].GetEmitterData();
		if (!ED) { GEditor->EndTransaction(); return FMegalithActionResult::Error(TEXT("No emitter data")); }
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetEffectType(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EffectTypePath = Params->GetStringField(TEXT("effect_type"));
	if (EffectTypePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: effect_type"));

	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetET", "Set Effect Type"));
	System->Modify();
	...
	{
		UNiagaraEffectType* ET = FMegalithAssetUtils::LoadAssetByPath<UNiagaraEffectType>(EffectTypePath);
		if (!ET) { GEditor->EndTransaction(); return FMegalithActionResult::Error(TEXT("Failed to load effect type")); }
...
FMegalithActionResult FMegalithNiagaraActions::HandleCreateEmitter(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (EmitterName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: name"));
	...
	UNiagaraEmitter* EmitterAsset = FMegalithAssetUtils::LoadAssetByPath<UNiagaraEmitter>(MinimalTemplate);
	if (!EmitterAsset) return FMegalithActionResult::Error(TEXT("Failed to load Minimal emitter template"));
	...
	FMegalithActionResult AER = HandleAddEmitter(AEP);
	...
	if (SimTarget == TEXT("gpu") && !EmitterId.IsEmpty())
	{
		...
		FMegalithActionResult PropResult = HandleSetEmitterProperty(SP);
...
FMegalithActionResult FMegalithNiagaraActions::HandleExportSystemSpec(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddDynamicInput(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (DynInputPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: dynamic_input_script"));
	...
	if (!DynScript) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load dynamic input script: %s"), *DynInputPath));

	// Get or create override pin
	FNiagaraParameterHandle AH = FNiagaraParameterHandle::CreateAliasedModuleParameterHandle(
		FNiagaraParameterHandle(MatchedFullName), MN);

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "AddDynInput", "Add Dynamic Input"));
	System->Modify();
	...
	{
		GEditor->EndTransaction();
		return FMegalithActionResult::Error(TEXT("SetDynamicInputForFunctionInput returned null node"));
	}
	...
	{
		TSharedRef<FJsonObject> IO = MakeShared<FJsonObject>();
		IO->SetStringField(TEXT("name"), MegalithNiagaraHelpers::StripModulePrefix(In.GetName()).ToString());
		IO->SetStringField(TEXT("type"), In.GetType().GetName());
		...
		UEdGraphPin* DynOP = MegalithNiagaraHelpers::GetStackFunctionInputOverridePin(*OutDynNode, DynAH);
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetDynamicInputValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (DynNodeGuid.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: dynamic_input_node"));
	...
	if (!DynNode) return FMegalithActionResult::Error(FString::Printf(TEXT("Dynamic input node '%s' not found"), *DynNodeGuid));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSearchDynamicInputs(const TSharedPtr<FJsonObject>& Params)
{
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddEventHandler(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (EventName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: event_name"));
	...
	{
		return FMegalithActionResult::Error(TEXT("source_emitter is required for add_event_handler. Self-events are not created implicitly; pass source_emitter explicitly so the handler does not end up unresolved."));
	}
	...
	if (!SourceEmitterStr.IsEmpty())
	{
		...
		if (SrcIdx == INDEX_NONE) return FMegalithActionResult::Error(FString::Printf(TEXT("Source emitter '%s' not found"), *SourceEmitterStr));
		SourceEmitterGuid = System->GetEmitterHandles()[SrcIdx].GetId();
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "AddEvtHandler", "Add Event Handler"));
	System->Modify();
	...
	{
		GEditor->EndTransaction();
		return FMegalithActionResult::Error(TEXT("Failed to get emitter instance"));
	}
	...
	if (ED->GraphSource)
	{
		if (UNiagaraScriptSource* GraphSource = Cast<UNiagaraScriptSource>(ED->GraphSource))
		{
			...
			{
				MegalithNiagaraHelpers::ResetGraphForOutputLocal(*GraphSource->NodeGraph,
...
FMegalithActionResult FMegalithNiagaraActions::HandleValidateSystem(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(TEXT("Failed to load system"));
	...
	// by AppendEmitterSemanticJson::guid and FMegalithNiagaraTopologyEdge::SourceEmitterId,
	...
	TMap<FGuid, FMegalithNiagaraEmitterSemantic> SemanticCache;
	...
	for (int32 EmitterIndex = 0; EmitterIndex < Handles.Num(); ++EmitterIndex)
	{
		...
		const FMegalithNiagaraEmitterSemantic& Semantic = SemanticCache.FindChecked(Handle.GetId());
		...
		if (Semantic.IncomingEvents.Num() > 0)
		{
			for (const FMegalithNiagaraTopologyEdge& Incoming : Semantic.IncomingEvents)
			{
				...
				MegalithNiagaraHelpers::GetOrderedModuleNodes(*EventOutput, EventModules);
			...
			for (const FMegalithNiagaraTopologyEdge& Incoming : Semantic.IncomingEvents)
			{
				...
				{
					if (const FMegalithNiagaraEmitterSemantic* SourceSemantic = SemanticCache.Find(SourceGuid))
					{
						for (const FMegalithNiagaraEventGeneratorInfo& Generator : SourceSemantic->EventGenerators)
						{
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddSimulationStage(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load system '%s'"), *SystemPath));

	FString EmitterName = Params->GetStringField(TEXT("emitter"));
	int32 HandleIdx = FindEmitterHandleIndex(System, EmitterName);
	if (HandleIdx == INDEX_NONE) return FMegalithActionResult::Error(FString::Printf(TEXT("Emitter '%s' not found"), *EmitterName));
	...
	if (!Emitter)
	{
		return FMegalithActionResult::Error(TEXT("Emitter instance is null"));
	}
	FVersionedNiagaraEmitterData* ED = Emitter->GetLatestEmitterData();
	if (!ED) return FMegalithActionResult::Error(TEXT("Failed to get emitter data"));
	...
	if (Source && Source->NodeGraph)
	{
		MegalithNiagaraHelpers::ResetGraphForOutputLocal(*Source->NodeGraph,
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetSpawnShape(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (AssetPath.IsEmpty() || EmitterName.IsEmpty() || Shape.IsEmpty())
		return FMegalithActionResult::Error(TEXT("asset_path, emitter, and shape are required"));
	...
	if (bReplaceExisting)
	{
		...
		FMegalithActionResult ModResult = HandleGetOrderedModules(GetModParams);
				...
				for (const TSharedPtr<FJsonValue>& ModVal : *ModulesArr)
				{
					...
					if (ModName.Contains(TEXT("InitializeParticle")))
					{
						...
						if (!bAlreadySimPos)
						{
							...
							FMegalithActionResult SwitchResult = HandleSetStaticSwitchValue(SwitchParams);
	...
	FMegalithActionResult AddResult = HandleAddModule(AddParams);
	...
	if (!NodeGuid.IsEmpty() && Params->TryGetObjectField(TEXT("params"), ShapeParamsObj) && ShapeParamsObj->IsValid())
	{
		for (const auto& Pair : (*ShapeParamsObj)->Values)
		{
			...
			FString InputName = MegalithKeyToString(Pair.Key);
			...
			FMegalithActionResult SetResult = HandleSetModuleInputValue(SetParams);
			if (SetResult.bSuccess)
				ParamsSet.Add(MegalithKeyToString(Pair.Key));
			else
				Warnings.Add(FString::Printf(TEXT("Failed to set param '%s': %s"), *MegalithKeyToString(Pair.Key), *SetResult.ErrorMessage));
		}
	}
...
// --------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleListDynamicInputs(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	for (const FNiagaraVariable& Input : Inputs)
	{
		...
		TSharedRef<FJsonObject> Entry = MakeShared<FJsonObject>();
		Entry->SetStringField(TEXT("input_name"), MegalithNiagaraHelpers::StripModulePrefix(Input.GetName()).ToString());
...
namespace
{
		...
		int32 MaxDepth)
	{
		...
		for (const FNiagaraVariable& Input : Inputs)
		{
			...
			UEdGraphPin* OP = MegalithNiagaraHelpers::GetStackFunctionInputOverridePin(*FuncNode, AH);
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetDynamicInputTree(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!Tree.IsValid()) return FMegalithActionResult::Error(TEXT("Failed to build input tree"));
...
namespace
{
	...
	void RemoveDynamicInputFromPin(UEdGraphPin& OverridePin, UEdGraph* Graph)
	{
		...
		if (UNiagaraNodeFunctionCall* DynNode = Cast<UNiagaraNodeFunctionCall>(LinkedNode))
		{
			...
			UEdGraphNode* DynOverrideNode = MegalithNiagaraHelpers::GetStackFunctionOverrideNode(*DynNode);
			if (DynOverrideNode)
			{
				TArray<UEdGraphPin*> SubPins;
				MegalithNiagaraHelpers::GetOverridePinsForFunction(DynOverrideNode, *DynNode, SubPins);
				...
				for (UEdGraphPin* Pin : DynOverrideNode->Pins)
				{
					...
					if (NN && Pin == MegalithNiagaraHelpers::GetParameterMapPin(*NN, EGPD_Input)) continue;
				...
				if (!bHasOtherPins)
				{
					...
					if (OvNN)
					{
						UEdGraphPin* OvMapIn = MegalithNiagaraHelpers::GetParameterMapPin(*OvNN, EGPD_Input);
						UEdGraphPin* OvMapOut = MegalithNiagaraHelpers::GetParameterMapPin(*OvNN, EGPD_Output);
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveDynamicInput(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!DynNodeGuid.IsEmpty())
	{
		...
		if (!OverridePin) return FMegalithActionResult::Error(TEXT("Could not find the override pin connected to this dynamic input"));
	}
	else if (!ModuleNodeGuid.IsEmpty() && !InputName.IsEmpty())
	{
		...
		FNiagaraParameterHandle AH = FNiagaraParameterHandle::CreateAliasedModuleParameterHandle(
			FNiagaraParameterHandle(MatchedFullName), MN);
		OverridePin = MegalithNiagaraHelpers::GetStackFunctionInputOverridePin(*MN, AH);
		if (!OverridePin) return FMegalithActionResult::Error(TEXT("No override pin found for this input — no dynamic input attached"));
		if (OverridePin->LinkedTo.Num() == 0) return FMegalithActionResult::Error(TEXT("Input has no dynamic input attached"));
		...
		if (!Cast<UNiagaraNodeFunctionCall>(OverridePin->LinkedTo[0]->GetOwningNode()))
			return FMegalithActionResult::Error(TEXT("Input is linked to a parameter binding, not a dynamic input"));
	}
	else
	{
		return FMegalithActionResult::Error(TEXT("Provide either (module_node + input) or dynamic_input_node"));
	}

	if (!OverridePin || !Graph)
		return FMegalithActionResult::Error(TEXT("Failed to resolve override pin or graph"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemoveDynInput", "Remove Dynamic Input"));
	System->Modify();
...
// --------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleGetDynamicInputValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString DynNodeGuid = Params->GetStringField(TEXT("dynamic_input_node"));
	...
	if (DynNodeGuid.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: dynamic_input_node"));
	if (InputName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: input"));
	...
	ENiagaraScriptUsage FoundUsage = ENiagaraScriptUsage::ParticleUpdateScript; // sensible default
	{
		...
		for (int32 Safety = 0; Safety < 100 && Current; ++Safety)
		{
			...
			UEdGraphPin* MapOut = MegalithNiagaraHelpers::GetParameterMapPin(*Current, EGPD_Output);
	...
	for (const FNiagaraVariable& In : Inputs)
	{
		...
		UEdGraphPin* OP = MegalithNiagaraHelpers::GetStackFunctionInputOverridePin(*DynNode, AH);
	...
	TArray<FString> ValidNames;
	for (const FNiagaraVariable& In : Inputs)
		ValidNames.Add(MegalithNiagaraHelpers::StripModulePrefix(In.GetName()).ToString());
...
// --------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleGetDynamicInputInputs(const TSharedPtr<FJsonObject>& Params)
{
	FString ScriptPath = Params->GetStringField(TEXT("script_path"));
	if (ScriptPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: script_path"));

	UNiagaraScript* Script = LoadObject<UNiagaraScript>(nullptr, *ScriptPath);
	if (!Script) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load script: %s"), *ScriptPath));

	UNiagaraScriptSource* Src = Cast<UNiagaraScriptSource>(Script->GetLatestSource());
	if (!Src || !Src->NodeGraph) return FMegalithActionResult::Error(TEXT("Script has no graph"));
	...
	for (UNiagaraNodeInput* InputNode : InputNodes)
	{
		...
		TSharedRef<FJsonObject> IO = MakeShared<FJsonObject>();
		FName ShortName = MegalithNiagaraHelpers::StripModulePrefix(InputNode->Input.GetName());
		IO->SetStringField(TEXT("name"), ShortName.ToString());
...
// --------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleListAvailableRenderers(const TSharedPtr<FJsonObject>& Params)
{
...
// --------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleSetRendererMesh(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
	...
	if (MeshPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: mesh"));
	...
	if (!MeshRend) return FMegalithActionResult::Error(TEXT("Renderer is not a MeshRenderer. Use add_renderer with class 'mesh' first."));

	UStaticMesh* SM = LoadObject<UStaticMesh>(nullptr, *MeshPath);
	if (!SM) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load StaticMesh: %s"), *MeshPath));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetRendMesh", "Set Renderer Mesh"));
	System->Modify();
	MeshRend->Modify();
...
// --------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleConfigureRibbon(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
	...
	if (!RibbonRend) return FMegalithActionResult::Error(TEXT("Renderer is not a RibbonRenderer. Use add_renderer with class 'ribbon' first."));
	...
	auto SetProp = [&](const FString& PropName, const FString& Value)
	{
		...
		FMegalithActionResult R = HandleSetRendererProperty(SubParams);
		if (R.bSuccess)
	...
	auto SetBinding = [&](const FString& BindingName, const FString& Attribute)
	{
		...
		FMegalithActionResult R = HandleSetRendererBinding(SubParams);
		if (R.bSuccess)
...
// --------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleConfigureSubUV(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	int32 RendererIndex = static_cast<int32>(Params->GetNumberField(TEXT("renderer_index")));
	...
	if (Columns <= 0 || Rows <= 0)
		return FMegalithActionResult::Error(TEXT("columns and rows must be positive integers"));
	...
	if (!SpriteRend && !MeshRend)
		return FMegalithActionResult::Error(TEXT("Renderer is not a SpriteRenderer or MeshRenderer"));
	...
	// Step 1: Set SubImageSize via HandleSetRendererProperty
	{
		...
		FMegalithActionResult R = HandleSetRendererProperty(SubParams);
		if (!R.bSuccess)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to set SubImageSize: %s"), *R.ErrorMessage));
	}
		...
		if (AddResult.bSuccess && AddResult.Result.IsValid())
		{
			...
			if (!AnimModuleNodeGuid.IsEmpty())
			{
				if (Params->HasField(TEXT("playback_mode")))
				{
					...
					FMegalithActionResult R = HandleSetStaticSwitchValue(SetParams);
				...
				if (Params->HasField(TEXT("start_frame")))
				{
					...
					FMegalithActionResult R = HandleSetModuleInputValue(SetParams);
...
FMegalithActionResult FMegalithNiagaraActions::HandleRenameEmitter(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	if (NewName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param 'name'"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RenameEmitter", "Rename Emitter"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetEmitterProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString PropertyName = Params->GetStringField(TEXT("property"));
	...
	if (PropertyName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param 'property'"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleCreateNPC(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	FString Namespace = Params->GetStringField(TEXT("namespace"));

	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: save_path"));
	if (Namespace.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: namespace"));
	...
	if (Existing)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("NPC already exists at '%s'"), *FullPath));
	}
	...
	if (!NPC) return FMegalithActionResult::Error(TEXT("Failed to create NPC"));
	...
	{
		UE_LOG(LogMegalithNiagara, Warning, TEXT("Could not find Namespace property on UNiagaraParameterCollection — namespace not set"));
	}
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetNPC(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));

	UNiagaraParameterCollection* NPC = FMegalithAssetUtils::LoadAssetByPath<UNiagaraParameterCollection>(AssetPath);
	if (!NPC) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load NPC at '%s'"), *AssetPath));
...
FMegalithActionResult FMegalithNiagaraActions::HandleAddNPCParameter(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	FString ParamName = Params->GetStringField(TEXT("name"));
	FString TypeName = Params->GetStringField(TEXT("type"));
	...
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));
	if (ParamName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: name"));
	if (TypeName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: type"));

	UNiagaraParameterCollection* NPC = FMegalithAssetUtils::LoadAssetByPath<UNiagaraParameterCollection>(AssetPath);
	if (!NPC) return FMegalithActionResult::Error(TEXT("Failed to load NPC"));

	FNiagaraTypeDefinition TD = ResolveNiagaraType(TypeName);

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "AddNPCParam", "Add NPC Parameter"));
	NPC->Modify();
	...
	if (AddedIdx == INDEX_NONE) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to add parameter '%s' — it may already exist"), *ParamName));
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveNPCParameter(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	FString ParamName = Params->GetStringField(TEXT("name"));
	...
	if (!Found) return FMegalithActionResult::Error(FString::Printf(TEXT("Parameter '%s' not found in NPC"), *ParamName));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemoveNPCParam", "Remove NPC Parameter"));
	NPC->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetNPCDefault(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	FString ParamName = Params->GetStringField(TEXT("name"));
	...
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));
	if (ParamName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: name"));
	if (!ValueJV.IsValid()) return FMegalithActionResult::Error(TEXT("Missing required field: value"));

	UNiagaraParameterCollection* NPC = FMegalithAssetUtils::LoadAssetByPath<UNiagaraParameterCollection>(AssetPath);
	if (!NPC) return FMegalithActionResult::Error(TEXT("Failed to load NPC"));

	UNiagaraParameterCollectionInstance* DefaultInst = NPC->GetDefaultInstance();
	if (!DefaultInst) return FMegalithActionResult::Error(TEXT("Failed to get NPC default instance"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetNPCDefault", "Set NPC Default"));
	NPC->Modify();
	DefaultInst->Modify();
	...
	if (!bSet) return FMegalithActionResult::Error(FString::Printf(TEXT("Unsupported type '%s' for set_npc_default"), *TD.GetName()));
...
FMegalithActionResult FMegalithNiagaraActions::HandleCreateEffectType(const TSharedPtr<FJsonObject>& Params)
{
	FString SavePath = Params->GetStringField(TEXT("save_path"));
	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: save_path"));
	...
	if (Existing)
	{
		return FMegalithActionResult::Error(FString::Printf(TEXT("Effect type already exists at '%s'"), *FullPath));
	}
	...
	if (!ET) return FMegalithActionResult::Error(TEXT("Failed to create effect type"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetEffectType(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));

	UNiagaraEffectType* ET = FMegalithAssetUtils::LoadAssetByPath<UNiagaraEffectType>(AssetPath);
	if (!ET) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load effect type at '%s'"), *AssetPath));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetEffectTypeProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	FString PropertyName = Params->GetStringField(TEXT("property"));
	TSharedPtr<FJsonValue> JV = Params->TryGetField(TEXT("value"));
	...
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));
	if (PropertyName.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: property"));
	if (!JV.IsValid()) return FMegalithActionResult::Error(TEXT("Missing required field: value"));

	UNiagaraEffectType* ET = FMegalithAssetUtils::LoadAssetByPath<UNiagaraEffectType>(AssetPath);
	if (!ET) return FMegalithActionResult::Error(TEXT("Failed to load effect type"));

	FProperty* Prop = ET->GetClass()->FindPropertyByName(FName(*PropertyName));
	if (!Prop) return FMegalithActionResult::Error(FString::Printf(TEXT("Property '%s' not found on UNiagaraEffectType"), *PropertyName));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetEffectTypeProp", "Set Effect Type Property"));
	ET->Modify();
	...
	if (!bOk) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to set property '%s'"), *PropertyName));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetAvailableParameters(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (SystemPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));
...
FMegalithActionResult FMegalithNiagaraActions::HandlePreviewSystem(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));
	...
	// No compile-time dependency on MegalithEditor — pure runtime dispatch
	...
	FMegalithActionResult EditorResult = FMegalithToolRegistry::Get().ExecuteAction(
		TEXT("editor"), TEXT("capture_scene_preview"), EditorParams);

	if (!EditorResult.bSuccess)
	{
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("Preview capture failed: %s. Ensure the MegalithEditor module is loaded."),
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetEventHandlers(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetEventHandlerProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString PropertyName = Params->GetStringField(TEXT("property"));
	...
	if (HandlerIndex < 0 && UsageIdStr.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Must provide handler_index or usage_id"));
	...
	if (!UsageIdStr.IsEmpty())
	{
		FGuid UsageId;
		if (!FGuid::Parse(UsageIdStr, UsageId))
			return FMegalithActionResult::Error(TEXT("Invalid usage_id GUID"));
		Handler = ED->GetEventHandlerByIdUnsafe(UsageId);
		if (!Handler)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Event handler with usage_id '%s' not found"), *UsageIdStr));
	}
	else
	{
		const TArray<FNiagaraEventScriptProperties>& Handlers = ED->GetEventHandlers();
		if (!Handlers.IsValidIndex(HandlerIndex))
			return FMegalithActionResult::Error(FString::Printf(TEXT("Handler index %d out of range (0-%d)"), HandlerIndex, Handlers.Num() - 1));
		...
		if (!Handler)
			return FMegalithActionResult::Error(TEXT("Failed to get mutable event handler pointer"));
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetEvtHandlerProp", "Set Event Handler Property"));
	System->Modify();
	...
	if (PropertyName == TEXT("ExecutionMode") || PropertyName == TEXT("execution_mode"))
	{
		...
		else { GEditor->EndTransaction(); return FMegalithActionResult::Error(FString::Printf(TEXT("Unknown execution mode '%s'"), *ValueStr)); }
		bSet = true;
	}
	...
	else if (PropertyName == TEXT("SourceEmitterID") || PropertyName == TEXT("source_emitter_id") || PropertyName == TEXT("source_emitter"))
	{
		...
		else
		{
			...
			if (SrcIdx == INDEX_NONE) { GEditor->EndTransaction(); return FMegalithActionResult::Error(FString::Printf(TEXT("Source emitter '%s' not found"), *ValueStr)); }
	...
	if (!bSet)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Unknown event handler property '%s'. Valid: ExecutionMode, SpawnNumber, MaxEventsPerFrame, SourceEventName, bRandomSpawnNumber, MinSpawnNumber, UpdateAttributeInitialValues, SourceEmitterID"), *PropertyName));
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveEventHandler(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	FVersionedNiagaraEmitter VersionedEmitter = EmitterHandle.GetInstance();
	UNiagaraEmitter* Emitter = VersionedEmitter.Emitter;
	if (!Emitter) return FMegalithActionResult::Error(TEXT("Emitter instance is null"));
	...
	else
	{
		...
		if (Handlers[HandlerIndex].Script)
			...
			return FMegalithActionResult::Error(TEXT("Handler has no script — cannot determine usage ID"));
	}
	...
	FString RemovedEventName;
	{
		...
		if (HandlerPtr)
			...
			return FMegalithActionResult::Error(TEXT("Event handler not found by usage_id"));
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemoveEvtHandler", "Remove Event Handler"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetSimulationStages(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSetSimulationStageProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString PropertyName = Params->GetStringField(TEXT("property"));
	...
	if (StageIndex < 0 && StageName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Must provide stage_index or stage_name"));
	...
	{
		if (!Stages.IsValidIndex(StageIndex))
			return FMegalithActionResult::Error(FString::Printf(TEXT("Stage index %d out of range (0-%d)"), StageIndex, Stages.Num() - 1));
		TargetStage = Stages[StageIndex];
	}
	else
	{
		...
		if (!TargetStage)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Simulation stage '%s' not found"), *StageName));
	}

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetSimStageProp", "Set Simulation Stage Property"));
	System->Modify();
	TargetStage->Modify();
	...
	else if (UNiagaraSimulationStageGeneric* Generic = Cast<UNiagaraSimulationStageGeneric>(TargetStage))
	{
		if (PropertyName == TEXT("IterationSource") || PropertyName == TEXT("iteration_source"))
		{
			...
			else { GEditor->EndTransaction(); return FMegalithActionResult::Error(FString::Printf(TEXT("Unknown iteration source '%s'. Valid: Particles, DataInterface, DirectSet"), *ValueStr)); }
			bSet = true;
		}
		else if (PropertyName == TEXT("ExecuteBehavior") || PropertyName == TEXT("execute_behavior"))
		{
			...
			else { GEditor->EndTransaction(); return FMegalithActionResult::Error(FString::Printf(TEXT("Unknown execute behavior '%s'"), *ValueStr)); }
			bSet = true;
		}
...
FMegalithActionResult FMegalithNiagaraActions::HandleRemoveSimulationStage(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RemoveSimStage", "Remove Simulation Stage"));
	System->Modify();
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetModuleOutputParameters(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	UEdGraphPin* MapOut = MegalithNiagaraHelpers::GetParameterMapPin(*MN, EGPD_Output);
	if (MapOut)
	{
...
static TSharedPtr<FJsonObject> ExportSpecForDiff(UNiagaraSystem* System)
{
	...
	FMegalithActionResult SpecResult = FMegalithNiagaraActions::HandleExportSystemSpec(Params);
	...
	const FString& Context)
{
	...
	for (auto& Pair : A->Values) AllKeys.Add(MegalithKeyToString(Pair.Key));
	for (auto& Pair : B->Values) AllKeys.Add(MegalithKeyToString(Pair.Key));
...
FMegalithActionResult FMegalithNiagaraActions::HandleDiffSystems(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (PathA.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path_a"));
	if (PathB.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path_b"));

	UNiagaraSystem* SysA = LoadSystem(PathA);
	if (!SysA) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load system A: %s"), *PathA));
	UNiagaraSystem* SysB = LoadSystem(PathB);
	if (!SysB) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load system B: %s"), *PathB));
	...
	if (!SpecA.IsValid() || !SpecB.IsValid())
		return FMegalithActionResult::Error(TEXT("Failed to export one or both system specs"));
...
FMegalithActionResult FMegalithNiagaraActions::HandleSaveEmitterAsTemplate(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString SavePath = Params->GetStringField(TEXT("save_path"));

	if (EmitterHandleId.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: emitter"));
	if (SavePath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: save_path"));
	...
	if (!SourceEmitter) return FMegalithActionResult::Error(TEXT("Source emitter is null"));
	...
	if (FindObject<UPackage>(nullptr, *PackageName))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Asset already exists at: %s"), *PackageName));

	// Create the package
	UPackage* NewPackage = CreatePackage(*PackageName);
	if (!NewPackage) return FMegalithActionResult::Error(TEXT("Failed to create package"));
	...
	if (!NewEmitter) return FMegalithActionResult::Error(TEXT("DuplicateObject failed"));
	...
	if (!bSaved)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Package save failed: %s"), *PackageFilename));
...
FMegalithActionResult FMegalithNiagaraActions::HandleCloneModuleOverrides(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (SrcModuleGuid.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: source_module"));
	if (TgtModuleGuid.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: target_module"));
	...
	if (!SrcNode) return FMegalithActionResult::Error(TEXT("Source module node not found"));

	UNiagaraNodeFunctionCall* TgtNode = FindModuleNode(System, TgtEmitterId, TgtModuleGuid, &TgtUsage);
	if (!TgtNode) return FMegalithActionResult::Error(TEXT("Target module node not found"));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "CloneOverrides", "Clone Module Overrides"));
	System->Modify();
	...
	for (const FNiagaraVariable& In : SrcInputs)
	{
		...
		UEdGraphPin* SrcOP = MegalithNiagaraHelpers::GetStackFunctionInputOverridePin(*SrcNode, SrcAH);
		if (!SrcOP) continue;
		...
		if (SrcOP->LinkedTo.Num() > 0)
		{
			...
			if (UNiagaraNodeInput* InputNode = Cast<UNiagaraNodeInput>(LinkedNode))
			{
				...
				FMegalithActionResult BindResult = HandleSetModuleInputBinding(BindParams);
		...
		else if (!SrcOP->DefaultValue.IsEmpty())
		{
			...
			FMegalithActionResult ValResult = HandleSetModuleInputValue(ValParams);
...
FMegalithActionResult FMegalithNiagaraActions::HandleSaveSystem(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	...
	// Load via FMegalithAssetUtils which handles path normalization
	UObject* LoadedAsset = FMegalithAssetUtils::LoadAssetByPath<UObject>(AssetPath);
	if (!LoadedAsset)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load asset at '%s'"), *AssetPath));
	...
	if (!bIsNiagara)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Asset '%s' is %s — not a Niagara asset type"), *AssetPath, *LoadedAsset->GetClass()->GetName()));
...
FMegalithActionResult FMegalithNiagaraActions::HandleGetStaticSwitchValue(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString ModuleNodeGuid = Params->GetStringField(TEXT("module_node"));
	...
	if (!bInputFound)
	{
		...
		for (const FNiagaraVariable& In : SwitchInputs) ValidNames.Add(In.GetName().ToString());
		if (ValidNames.Num() == 0)
			return FMegalithActionResult::Error(FString::Printf(TEXT("Input '%s' not found — this module has no static switches"), *InputName));
		return FMegalithActionResult::Error(FString::Printf(
			TEXT("Static switch '%s' not found. Valid static switches: [%s]"), *InputName, *FString::Join(ValidNames, TEXT(", "))));
	}
	...
	if (!SwitchPin)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Static switch pin '%s' not found on module node"), *InputName));
...
int32 FMegalithNiagaraActions::ApplySpecToSystem(UNiagaraSystem* System, const FString& SystemPath,
	const TSharedPtr<FJsonObject>& Spec, TArray<FString>& OutErrors)
{
	...
	if (Spec->HasField(TEXT("user_parameters")))
	{
		for (const TSharedPtr<FJsonValue>& PV : Spec->GetArrayField(TEXT("user_parameters")))
		{
			...
			FMegalithActionResult AUP = HandleAddUserParameter(AP);
		...
		for (const TSharedPtr<FJsonValue>& EV : Spec->GetArrayField(TEXT("emitters")))
		{
			...
			if (EO->HasField(TEXT("properties")))
			{
				...
				for (auto& P : Props->Values)
				{
					...
					FMegalithActionResult EPR = HandleSetEmitterProperty(SP);
			...
			if (EO->HasField(TEXT("modules")))
			{
				...
				for (int32 MI = 0; MI < Mods.Num(); ++MI)
				{
					...
					FMegalithActionResult AMR = HandleAddModule(AMP);
					...
					if (MO->HasField(TEXT("inputs")))
					{
						...
						for (auto& IP : Ins->Values)
						{
							...
							FMegalithActionResult SIVR = HandleSetModuleInputValue(SIP);
					...
					if (MO->HasField(TEXT("bindings")))
					{
						...
						for (auto& BP2 : Binds->Values)
						{
							...
							FMegalithActionResult SIBR = HandleSetModuleInputBinding(SBP);
					...
					if (MO->HasField(TEXT("static_switches")))
					{
						...
						for (auto& SW : Switches->Values)
						{
							...
							FMegalithActionResult SSR = HandleSetStaticSwitchValue(SSP);
			...
			if (EO->HasField(TEXT("renderers")))
			{
				for (const TSharedPtr<FJsonValue>& RV : EO->GetArrayField(TEXT("renderers")))
				{
					...
					FMegalithActionResult ARR = HandleAddRenderer(ARP);
					...
					if (RO->HasField(TEXT("material")))
					{
						...
						FMegalithActionResult SRMR = HandleSetRendererMaterial(SMP);
					...
					if (RO->HasField(TEXT("properties")))
					{
						...
						for (auto& RP : RProps->Values)
						{
							...
							FMegalithActionResult SRPR = HandleSetRendererProperty(SRP);
...
FMegalithActionResult FMegalithNiagaraActions::HandleImportSystemSpec(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (!Mode.Equals(TEXT("overwrite"), ESearchCase::IgnoreCase) && !Mode.Equals(TEXT("merge"), ESearchCase::IgnoreCase))
		return FMegalithActionResult::Error(FString::Printf(TEXT("Unsupported mode '%s' — use 'overwrite' or 'merge'"), *Mode));

	UNiagaraSystem* System = LoadSystem(SystemPath);
	if (!System)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load system '%s'"), *SystemPath));
	...
	if (Mode.Equals(TEXT("merge"), ESearchCase::IgnoreCase))
	{
		...
		if (Spec->HasField(TEXT("user_parameters")))
		{
			...
			for (const TSharedPtr<FJsonValue>& PVal : ParamArr)
			{
				...
				FMegalithActionResult R = HandleAddUserParameter(AddParams);
				if (!R.bSuccess)
				{
	...
	else
	{
		...
		GEditor->BeginTransaction(NSLOCTEXT("Megalith", "ImportSpec", "Import System Spec"));
		System->Modify();
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleGetDIProperties(const TSharedPtr<FJsonObject>& Params)
{
	FString DIClassName = Params->GetStringField(TEXT("di_class"));
	if (DIClassName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: di_class"));

	FString DIDiagnostic;
	UClass* DIC = MegalithNiagaraHelpers::ResolveNiagaraDataInterfaceClass(DIClassName, &DIDiagnostic);
	if (!DIC)
		return FMegalithActionResult::Error(DIDiagnostic.IsEmpty()
	...
	if (!CDO) return FMegalithActionResult::Error(TEXT("Failed to get CDO"));
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleClearEmitterModules(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));
	FString UsageFilter = Params->HasField(TEXT("usage")) ? Params->GetStringField(TEXT("usage")).ToLower() : TEXT("all");
	...
	else
	{
		...
		if (!ResolveScriptUsage(UsageFilter, SingleUsage))
			return FMegalithActionResult::Error(FString::Printf(TEXT("Unknown usage '%s'. Use: particle_update, particle_spawn, emitter_update, emitter_spawn, or all"), *UsageFilter));
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "ClearModules", "Clear Emitter Modules"));
	System->Modify();
	...
	for (int32 i = AllModulesToRemove.Num() - 1; i >= 0; --i)
	{
		...
		if (MegalithNiagaraHelpers::RemoveModuleFromStack(*System, EmitterGuid, *AllModulesToRemove[i]))
		{
			RemovedCount++;
		}
	}
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleGetModuleScriptInputs(const TSharedPtr<FJsonObject>& Params)
{
	FString ScriptPath = Params->GetStringField(TEXT("script_path"));
	if (ScriptPath.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: script_path"));

	UNiagaraScript* Script = LoadObject<UNiagaraScript>(nullptr, *ScriptPath);
	if (!Script)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load script '%s'"), *ScriptPath));
	...
	if (!Source || !Source->NodeGraph)
		return FMegalithActionResult::Error(TEXT("Script has no graph source"));
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleGetScalabilitySettings(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	if (AssetPath.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: asset_path"));
	...
	if (!EffectType)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load effect type '%s'"), *AssetPath));
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleSetScalabilitySettings(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = NA_GetAssetPath(Params);
	if (AssetPath.IsEmpty())
		return FMegalithActionResult::Error(TEXT("Missing required param: asset_path"));
	...
	if (SettingsJsonArr.Num() == 0)
		return FMegalithActionResult::Error(TEXT("'settings' array is empty"));

	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "SetScalability", "Set Scalability Settings"));
	EffectType->Modify();
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleListSystems(const TSharedPtr<FJsonObject>& Params)
{
	FString Search = Params->HasField(TEXT("search")) ? Params->GetStringField(TEXT("search")) : TEXT("");
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleDuplicateModule(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (SystemPath.IsEmpty() || SrcEmitter.IsEmpty() || SrcModuleGuid.IsEmpty())
		return FMegalithActionResult::Error(TEXT("asset_path, source_emitter, and source_module_node are required"));
	...
	if (!SrcNode->FunctionScript)
		return FMegalithActionResult::Error(TEXT("Source module has no script"));
	...
	FMegalithActionResult AddResult = HandleAddModule(AddParams);
	if (!AddResult.bSuccess)
		return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to add module: %s"), *AddResult.ErrorMessage));
	...
	FMegalithActionResult CloneResult = HandleCloneModuleOverrides(CloneParams);
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleGetEmitterParent(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	FString EmitterHandleId = Params->GetStringField(TEXT("emitter"));

	if (SystemPath.IsEmpty() || EmitterHandleId.IsEmpty())
		return FMegalithActionResult::Error(TEXT("asset_path and emitter are required"));
	...
	const FNiagaraEmitterHandle& Handle = System->GetEmitterHandles()[EIdx];
	FVersionedNiagaraEmitter VersionedEmitter = Handle.GetInstance();
	if (!VersionedEmitter.Emitter) return FMegalithActionResult::Error(TEXT("Emitter instance is null"));

	const FVersionedNiagaraEmitterData* ED = VersionedEmitter.GetEmitterData();
	if (!ED) return FMegalithActionResult::Error(TEXT("Emitter data is null"));
...
// ============================================================================
FMegalithActionResult FMegalithNiagaraActions::HandleRenameUserParameter(const TSharedPtr<FJsonObject>& Params)
{
	FString SystemPath = NA_GetAssetPath(Params);
	...
	if (SystemPath.IsEmpty() || OldName.IsEmpty() || NewName.IsEmpty())
		return FMegalithActionResult::Error(TEXT("asset_path, old_name, and new_name are required"));
	...
	if (OldSearch.Equals(NewSearch, ESearchCase::IgnoreCase))
		return FMegalithActionResult::Error(TEXT("Old and new names are the same"));
	...
	if (!bFound)
		return FMegalithActionResult::Error(FString::Printf(TEXT("User parameter '%s' not found"), *OldName));

	// Check new name doesn't already exist
	for (const FNiagaraVariable& P : UP)
	{
		if (P.GetName().ToString().Equals(NewSearch, ESearchCase::IgnoreCase))
			return FMegalithActionResult::Error(FString::Printf(TEXT("Parameter '%s' already exists"), *NewName));
	}
	...
	GEditor->BeginTransaction(NSLOCTEXT("Megalith", "RenameUP", "Rename User Parameter"));
	System->Modify();
	...
	for (int32 EIdx = 0; EIdx < System->GetEmitterHandles().Num(); EIdx++)
	{
		...
		for (ENiagaraScriptUsage Usage : Usages)
		{
			...
			for (UNiagaraNodeFunctionCall* ModNode : ModuleNodes)
			{
				...
				UEdGraphPin* PMInput = MegalithNiagaraHelpers::GetParameterMapPin(*ModNode, EGPD_Input);
...
// ----------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleSearchByParameter(const TSharedPtr<FJsonObject>& Params)
{
	const FString ParamQuery = Params->HasField(TEXT("parameter_name")) ? Params->GetStringField(TEXT("parameter_name")) : TEXT("");
	if (ParamQuery.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: parameter_name"));
...
// ----------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleSearchByDataInterface(const TSharedPtr<FJsonObject>& Params)
{
	const FString DIQuery = Params->HasField(TEXT("di_class")) ? Params->GetStringField(TEXT("di_class")) : TEXT("");
	if (DIQuery.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: di_class"));
	const FString Folder = ReadFolderFilter(Params);
	const int32 Limit = ReadLimit(Params, 50);
...
// ----------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleQueryNiagara(const TSharedPtr<FJsonObject>& Params)
{
	const FString QueryString = Params->HasField(TEXT("query_string")) ? Params->GetStringField(TEXT("query_string")) : TEXT("");
	if (QueryString.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: query_string"));
	const FString Folder = ReadFolderFilter(Params);
	const int32 Limit = ReadLimit(Params, 50);
	...
	if (Conds.Num() == 0)
	{
		return FMegalithActionResult::Error(TEXT("No valid conditions parsed. Supported: emitters>N / emitters<N / emitters=N, sim_target=GPU|CPU, has_renderer=<name>"));
	}
...
// ----------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleFindSimilarSystems(const TSharedPtr<FJsonObject>& Params)
{
	const FString RefPath = NA_GetAssetPath(Params);
	if (RefPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));
	...
	if (!RefSys) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load reference system '%s'"), *RefPath));
...
// ----------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleSearchByMaterial(const TSharedPtr<FJsonObject>& Params)
{
	FString MaterialPath = Params->HasField(TEXT("material_path")) ? Params->GetStringField(TEXT("material_path")) : TEXT("");
	if (MaterialPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: material_path"));
	const FString Folder = ReadFolderFilter(Params);
	const int32 Limit = ReadLimit(Params, 50);
...
// ----------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleFindNiagaraReferences(const TSharedPtr<FJsonObject>& Params)
{
...
// ----------------------------------------------------------------------------
FMegalithActionResult FMegalithNiagaraActions::HandleListSystemDataInterfaces(const TSharedPtr<FJsonObject>& Params)
{
	const FString AssetPath = NA_GetAssetPath(Params);
	if (AssetPath.IsEmpty()) return FMegalithActionResult::Error(TEXT("Missing required field: asset_path"));

	UNiagaraSystem* Sys = LoadSystem(AssetPath);
	if (!Sys) return FMegalithActionResult::Error(FString::Printf(TEXT("Failed to load system '%s'"), *AssetPath));
